...
| 13 April 2012 | Revision 2.14.4 | Correction for FogBugz entry 536: change bullet 5 in Clause 4.2.3from L_RECEIPTDATE=O_ORDERDATE +random value[130] to L_RECEIPTDATE=L_SHIPDATE + random value [1 30]. |
...
Comment: The benchmark does not include any test or measure to verify continuous database availability or particular system features which would make the benchmarked configuration appropriate for 7 x 2 4 operation. References to continuous availability and 7 x 2 4 operation are included in the benchmark specification to provide a more complete picture of the anticipated decision support environment. A configuration offering less that 7 x 2 4 availability can produce compliant benchmark results as long as it meets all the requirements described in this specification.
...
Comment S F ^ * l 0 , 0 0 0 are populated
...
Column Name Datatype Requirements Comment C_CUSTKEY Identifier S F ^ * 1 5 0 , 0 0 0 are populated
...
4. Multi-column constraints L_SHIPDATE < = L _RECEIPTDATE
...
1.5.8 Table names should match those provided in Clause 1.4.1. In cases where a table name conflicts with a reserved word in a given implementation, delimited identifiers or an alternate meaningful name may be chosen.
...
1.5.10 Column names must match those provided in Clause 1
...
An initial seed (seed0) is first selected as the time stamp of the end of the database load time expressed in the format mmddhhmmss where mm is the month, dd the day, hh the hour, mm the minutes and ss the seconds. This seed is used to seed the Power test of Run 1. Further seeds (for the Throughput test) are chosen as sec d 0 + 1 , seed0 + 2 , . . . , sec d 0 + n where s is the number of throughput streams selected by the vendor. This process leads to mathsf s + 1 seeds required for Run 1 of a benchmark with s streams. The seeds for Run 2 can be the same as those for Run 1 (see 5.3.2). However, should the test sponsor decide to use different seeds for Run 2 from those used for Run 1, the sponsor must use a selection process similar to that of Run 1. The seeds must again be of the form seed0, sec d 0 + 1 , $\mathrm { s e e d } 0 + 2 , . . . . , \mathrm { s e e d } 0 + \mathrm { s } ,$ , where and seed0 is be the time stamp of the end of Run 1, expressed in the format defined above.
...
1.1.3.5 The precision of all values contained in the query validation output data must adhere to the following rules:
...
1.1.4.1 Executable query text must be generated according to the requirements of Clause 2.1.2 and Clause 2.1.3. . QGen is a TPC provided software package that must be used to generate the query text.
...
1.2.3.2 Minor query modifications can be used to produce executable query text by modifying either a functional query definition or an approved variant of that definition.
...
1.3.1 To validate the compliance of the executable query text, the following validation test must be executed by the tes sponsor and the results reported in the full disclosure report:
...
l_returnflag, l_linestatus, sum(l_quantity) as sum_qty, sum(l_extendedprice) as sum_base_price, sum(l_extendedprice*(1-l_discount)) as sum_disc_price, sum(l_extendedprice*(1-l_discount) ^ * ( 1 + 1 _ t a x ) ) as sum_charge, avg(l_quantity) as avg_qty, avg(l_extendedprice) as avg_price, avg(l_discount) as avg_disc, count(*) as count_order 
from lineitem 
...
min(ps_supplycost) from partsupp, supplier, nation, region where p_partkey = ps_partkey and s_suppkey = ps_suppkey and s_nationkey = n _nationkey and n_regionkey = boldsymbol r _regionkey and r_name = '[REGION]' order by s_acctbal desc, n_name, s_name, p_partkey;
...
where c_mktsegment = '[SEGMENT]' and c_custkey equiv _ 0 _custkey and l_orderkey = 0 _orderkey and o_orderdate < date '[DATE]' and l_shipdate > date '[DATE]' 
...
from orders 
where o_orderdate > = date '[DATE]' and o_orderdate < date '[DATE]' + interval '3' month and exists ( select * from lineitem where l_orderkey = 0 _orderkey and l_commitdate < 1 receiptdate ) 
...
where c_custkey equiv _ 0 _custkey and l_orderkey = 0 _orderkey and l_suppkey = s _suppkey and c_nationkey = s _nationkey and s_nationkey = n _nationkey and n_regionkey = r _regionkey and r_name = '[REGION]' and o_orderdate > = date '[DATE]' and o_orderdate < date '[DATE]' + interval '1' year 
group by n_name 
...
1. R E G I O N = A S I A ; 
2. D A T E = 1 9 9 4 – 0 1 – 0 1 .
...
1. D A T E = 1 9 9 4 – 0 1 – 0 1 ; 
2. D I S C O U N T = 0 . 0 6 ; 
3. QUANTITY = 2 4 .
...
from ( select n1.n_name as supp_nation, n2.n_name as cust_nation, extract(year from l_shipdate) as l_year, l_extendedprice * (1 - l_discount) as volume from supplier, lineitem, orders, customer, nation n1, nation n2 where s_suppkey = l_suppkey and o_orderkey = l_orderkey and c_custkey = o _custkey and s_nationkey = n l . n _nationkey and c_nationkey = tt n 2 . tt n _nationkey and ( (n1.n_name = '[NATION1]' and n2.n_name = '[NATION2]') or (n1.n_name = '[NATION2]' and n2.n_name = '[NATION1]') ) and l_shipdate between date '1995-01-01' and date '1996-12-31' ) as shipping 
...
from ( select extract(year from o_orderdate) as o_year, l_extendedprice * (1-l_discount) as volume, n2.n_name as nation from part, supplier, lineitem, orders, customer, nation n1, nation n2, region where p_partkey = l_partkey and s_suppkey = 1 _suppkey and l_orderkey = 0 _orderkey and o_custkey = c _custkey and c_nationkey = mathfrak n 1 . mathfrak n _nationkey and n1.n_regionkey = r _regionkey and r_name = '[REGION]' and s_nationkey = tt n 2 . tt n . _nationkey and o_orderdate between date '1995-01-01' and date '1996-12-31' and p_type = '[TYPE]' ) as all_nations 
group by o_year 
order by o_year;
...
where c_custkey equiv _ 0 _custkey and l_orderkey = 0 _orderkey and o_orderdate > = date '[DATE]' and o_orderdate < date '[DATE]' + interval '3' month and l_returnflag = " R " and c_nationkey = n _nationkey 
...
where ps_suppkey = s _suppkey and s_nationkey = n _nationkey and n_name = '[NATION]' 
group by ps_partkey having sum(ps_supplycost * ps_availqty) > ( select sum(ps_supplycost * ps_availqty) * [FRACTION] from partsupp, supplier, nation where ps_suppkey = s _suppkey and s_nationkey = n _nationkey and n_name = '[NATION]' ) 
order by value desc;
...
l_shipmode, sum(case when o_orderpriority = 1 1 -URGENT' or o_orderpriority = 1 2 -HIGH' then 1 else 0 end) as high_line_count, sum(case when o_orderpriority textless textless 1 -URGENT' and o_orderpriority textless > 2 -HIGH' then 1 else 0 end) as low_line_count 
...
from ( select c_custkey, count(o_orderkey) from customer left outer join orders on c_custkey equiv _ 0 _custkey and o_comment not like $\mathrm { ^ { * } \mathrm { \% } [ W O R D 1 ] ^ { \mathrm { \% } } [ W O R D 2 ] \mathrm { \% } ^ { \mathrm { \prime } } } ,$ group by c_custkey )as c_orders (c_custkey, c_count) 
group by c_count 
...
from partsupp, part 
where p_partkey = ps_partkey and p_brand < > '[BRAND]' and p_type not like [ T Y P E ] % ^ prime and p_size in ([SIZE1], [SIZE2], [SIZE3], [SIZE4], [SIZE5], [SIZE6], [SIZE7], [SIZE8]) and ps_suppkey not in ( select s_suppkey from supplier where s_comment like '%Customer%Complaints%' ) 
...
from customer, orders, lineitem 
where o_orderkey in ( select l_orderkey from lineitem group by l_orderkey having sum(l_quantity) > [QUANTITY] ) and c_custkey equiv _ 0 _custkey and o_orderkey = l_orderkey 
...
2. Q U A N T I T Y 2 = 1 0 . 
3. Q U A N T I T Y 3 = 2 0 . 
4. BRAND1 = Brand#12. 
5. BRAND2 = Brand#23. 
6. BRAND3 = Brand#34.
...
1.5.3.3 The proper implementation of the refresh functions must be validated by the independent auditor who may request additional tests to ascertain that the refresh functions execute in accordance with the benchmark requirements.
...
LOOP S F ^ ast 1 5 0 0 ) TIMES DELETE FROM ORDERS WHERE O_ORDERKEY = [value] DELETE FROM LINEITEM WHERE L_ORDERKEY = [value] 
END LOOP
...
which truncates n to the mathfrak p ^ t h decimal place (e.g., trunc ( 1 . 3 5 7 , 2 ) = 1 . 3 5 ) ).
...
where L_ORDERKEY = [o_key] and L_LINENUMBER = [ 1 _ k e y ] Set [ototal] = [ototal] - 
...
1.3.2.3 If data is replicated, as permitted under Clause 1.5.7, each copy must meet the consistency condition defined in Clause 3.3.2.1.
...
TPC BenchmarkTM H Standard Specification Revision 3.0
...
T _ 1 = A n instance of the ACID Transaction;

T _ 2 = A n instance of the ACID Transaction;

T _ 3 = Any of the TPC-H queries 1 to 22 or an instance of the ACID query;

T _ n = Any arbitrary transaction.

Although arbitrary, the transaction T _ n shall not do dirty writes. 
...
4.1.1.2 The test database must be scaled as defined in Clause 4.1
...
1.3.1 Scale factors used for the test database must be chosen from the set of fixed scale factors defined as follows:
...
Comment 2: The maximum size of the test database for a valid performance test is currently set at 100000 (i.e., SF = 1 0 0 , 0 0 0 Omega ). The TPC recognizes that additional benchmark development work is necessary to allow TPC-H to scale beyond that limit.
...
4.2.2.3 The notation random value [x y] represents a random value between x and y inclusively, with a mean of ( x + y ) / 2 , and with the same number of digits of precision as shown. For example, [0.01 100.00] has 10,000 unique values, whereas [1100] has only 100 unique values.
...
Let country_code be the sub-string representation of the number ( i + 1 0 ) , 
...
4.2.2.10 The term text string[min, max] represents a substring of a 3 0 0 ~ M B string populated according to the pseudo text grammar defined in Clause 4.2.2.14. The length of the substring is a random number between min and max inclusive. The substring offset is randomly chosen.
...
S ~ F ~ ^ * 2 0 0 , 0 0 0 rows in the PART table with:

P_PARTKEY unique within [ S F * 2 0 0 , 0 0 0 ]
...
O_ORDERKEY unique within [ S F * 1 , 5 0 0 , 0 0 0 * 4 ] .
...
O_CLERK text appended with minimum 9 digits with leading zeros ["Clerk#", C] where C = random value [000000001 ( S F ^ * 1 0 0 0 ) big ] . 
...
L_ORDERKEY ${ \mathbf \xi } = { \mathbf O } _ { \mathbf { \xi } }_ORDERKEY. L_PARTKEY random value [1 ( \mathrm { S F } * 2 0 0 , 0 0 0 ) ].\mathrm { L \_ S U P P K E Y = ( L \_ P A R T K E Y + ( i \ast ( ( \ S / 4 \pi ) + ( i n t ) ( L \_ p a r t k e y - 1 \ ) / S ) ) } )) modulo\mathrm { S } + 1$ where i is the corresponding supplier within [0 3] and $\mathrm { S } = \mathrm { S F } ^ { * } 1 0 \mathrm { , 0 0 0 }$ . 
L_LINENUMBER unique within [7]. 
L_QUANTITY random value [1 50]. 
L_EXTENDEDPRICE = L _QUANTITY * P_RETAILPRICE Where P_RETAILPRICE is from the part with P_PARTKEY = L _PARTKEY. 
...
L_SH $\mathrm { \Delta I I P D A T E = O \_ O R D E R D A ^ { \prime } }TE + random value [1 121].\mathrm { \Delta L \ C O M M I T D A T E = O \ O R D E R D A T E }$ + random value [30 90]. 
L_RECEIPTDATE = L _SHIPDATE + random value [1 30]. 
...
1.2.5.2 Table 4: LINEITEM Cardinality shows the cardinality of the LINEITEM table at all authorized scale factors. 
...
1.3.1 The process of building the test database is known as database load. Database load consists of timed and untimed components. However, all components must be fully disclosed (see Clause 8.3.5.6).
...
1.2.4 The driver submits each TPC-H query for execution by the SUT via the session associated with the corresponding query stream.
...
5.3.5.4 The query submission order of an ordering number, O(s), is given in Appendix A by the ordered set with reference s. Comment: For tests where the list of ordered sets in Appendix A is exhausted, the last reference in the list must be followed by the first reference in the list (i.e., wrapping around to s = 0 0 ).

# 5.3.6 Measurement Interval

5.3.6.1 The measurement interval, T _ s , for the throughput test is measured as follows:
...
1.4.1.1 The results of the power test are used to compute the TPC-H query processing power at the chosen database size. It is defined as the inverse of the geometric mean of the timing intervals, and must be computed as:

Where:

QI(i,0) is the timing interval, in seconds, of query Q _ i within the single query stream of the power test (see Clause 5.3.7)

RI(j,0) is the timing interval, in seconds, of refresh function R F _ j within the single query stream of the power test (see Clause 5.3.7)
...
5.4.1.4 If the ratio between the longest query timing interval and the shortest query timing interval in the power test is greater than 1000 (i.e., $\mathrm { \ m a x [ Q I ( i , 0 ) ] / m i n [ Q I ( i , 0 ) ] > 1 0 0 0 } $ ), then all query timing intervals which are smaller than $\operatorname* { m a x } [ \mathrm { Q I ( i , 0 ) } ] / 1 0 0 0must be increased to\mathfrak { n a x } [ \mathrm { Q I ( i , 0 ) } ] / 1 0 0 0$ . The quantity max[QI(i,0)]/1000 must be treated as a timing interval as specified in Clause 5.3.7.5 for the purposes of computing the TPC-H Power@Size.
...
T _ s is the measurement interval defined in Clause 5.3
...
5.4.4.2 The units of Price-per-kQphH@Size are expressed as in Clause 7 of TPC Pricing Specification. In the United States the price performance is expressed as USD per kQphH@Size rounded to the highest cent (e.g., 12.123must be shown as\12.13 USD for price/performance).
...
1.4.5.2 Any TPC-H result is comparable to other TPC-H results regardless of the number of query streams used during the test (as long as the scale factors chosen for their respective test databases were the same).
...
1.1.2.3 The communication interface used must be an industry standard interface, such as Ethernet, Token Ring, or RS232.
...
1.3.1.3 Explicit response to individual disclosure requirements specified in the body of earlier sections of this document must be provided.

1.3.1.4 Diagrams of both measured and priced configurations must be provided, accompanied by a description of the differ ences. This includes, but is not limited to:
...
1 6 x SCSI-2 Fast Controllers 
Disk: 9 6 times 2 . 1 gigabyte SCSI-2 drives
...
1.3.4.1 Listings must be provided for all table definition statements and all other statements used to set-up the test and qualification databases. All listings must be reported in the supporting files archive.
...
Comment: For the reporting of configured disk capacity, gigabyte (GB) is defined to be 2 ^ sim 3 0 bytes. Since disk manufacturers typically report disk size using base ten (i.e., G B = 1 0 ^ wedge 9 ), it may be necessary to convert the advertised size from base ten to base two.
...
total number of nodes used/total number of processors used with their types and speeds in G H z / total number of cores used/total number of threads used; • Main and cache memory sizes; • Network and I/O connectivity; • Disk quantity and geometry.
...
1.4.2.6 The final section of the implementation Overview should contain a note stating:
...
1.2.2.9 Verify that the transaction requirements are met by the implementation of the refresh functions.
...
X ^ + 5 ^ * power(10,-m-1), call it y $\mathrm { y } ^ { \ast } \mathrm { p o w e r } ( 1 0 , \mathrm { m } ), call it z truncate\mathbf { Z }to an integer value, call it q;\mathsf { q / p o w e r } ( 1 0 , \mathrm { m } )$ to obtain the rounded value.

Rounding Examples

round(45.897,1) 
$\mathrm { y } { = } 4 5 . 8 9 7 { + } 0 . 0 5 { = } 4 5 . 9 4 7\mathrm { z } { = } 4 5 9 . 4 7\scriptstyle { \mathrm { q } } = 4 5 9z { = } 4 5 . 9round(45.213,1)\mathrm { y } { = } 4 5 . 2 1 3 { + } 0 . 0 5 { = } 4 5 . 2 6 3$ 
z=452
q=452 
z=45
round(45.897,0) 
$\mathrm { y } { = } 4 5 . 8 9 7 { + } 0 . 5 { = } 4 6 . 3 9 7z=46.397 q=46{ \tt z } { = } 4 6$
...
from ( select extract(year from o_orderdate) as o_year, l_extendedprice * (1 - l_discount) as volume, n2.n_name as nation from part, supplier, lineitem, orders, customer, nation n1, nation n2, region where p_partkey = l_partkey and s_suppkey = l_suppkey and l_orderkey = 0 _orderkey and o_custkey = c _custkey and c_nationkey = n1.n_nationkey and n1.n_regionkey = r _regionkey and r_name $\mathbf { \Sigma } = \mathbf { \ ' } [ \mathrm { R E G I O N } ] \mathbf { \Psi }and s_nationkey= { \tt n } 2 . { \tt n }$ _nationkey and o_orderdate between date '1995-01-01' and date '1996-12-3 and p_type = '[TYPE]’ ) all_nations 
group by o_year 
order by o_year;
...
from customer left outer join orders on c_custkey equiv _ 0 _custkey and o_comment not like $\mathrm { ^ { 1 0 } / _ { 0 } [ W O R D 1 ] ^ { \circ } / _ { 0 } [ W O R D 2 ] ^ { \circ } / _ { 0 } ^ { \circ } }$ 
group by c_custkey;
