...
# Orchestrator
def analyze_q9_unique_patterns_workflow(input_file, output_dir):
    df = load_q9_data(input_file)
    unique_patterns = extract_all_unique_patterns(df)
    export_unique_patterns(unique_patterns, output_dir)
...
# Extract all unique patterns across all Q9 queries
def extract_all_unique_patterns(df):
    all_patterns = set()
    ...
    df_patterns = pd.DataFrame({'pattern': sorted(list(all_patterns))})
    return df_patterns
...
# Export unique patterns to CSV with timestamp
def export_unique_patterns(unique_patterns, output_dir):
    csv_dir = Path(output_dir) / 'csv'
    csv_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_file = csv_dir / f'q9_unique_patterns_{timestamp}.csv'
    unique_patterns.to_csv(output_file, sep=';', index=False)

if __name__ == '__main__':
    if len(sys.argv) != 3:
        sys.exit(1)
    analyze_q9_unique_patterns_workflow(sys.argv[1], sys.argv[2])
