...
├── 02_Query_Prediction.py            # Bottom-up prediction workflow on test set
├── 03_Query_Evaluation.py            # Evaluate query-level predictions and create visualizations
├── 04_Operator_Analysis.py           # Cross-evaluation of node type prediction accuracy
├── 05_Time_Statistics.py             # Statistical analysis of operator timing patterns
...
Model Files (execution_time/{operator}/model.pkl, start_time/{operator}/model.pkl)
   ↓
[02] 02_Query_Prediction.py
   ↓
predictions.csv
   ↓
[03] 03_Query_Evaluation.py
   ↓
Query-level MRE + Visualizations
   ↓
┌─────────────────────────┬──────────────────────────┐
│                         │                          │
[04] 04_Operator_Analysis.py    [05] 05_Time_Statistics.py
│                         │                          │
...
**Usage**:
```bash
python3 02_Query_Prediction.py <test_file> <overview_file> <models_dir> --output-file <output_file>
```

**Example**:
```bash
python3 02_Query_Prediction.py \
...
**Inputs**:
- `predictions_csv` - Path to predictions CSV from 02_Query_Prediction.py
- `--output-dir` - Output directory for evaluation results

**Outputs**:
- `overall_mre.csv` - Query-level aggregated error metrics
- `template_mre.csv` - Per-template MRE statistics
- `template_mre_plot.png` - Bar chart visualization of template performance

**Usage**:
```bash
python3 03_Query_Evaluation.py <predictions_csv> --output-dir <output_dir>
```

**Example**:
```bash
python3 03_Query_Evaluation.py \
...
**Usage**:
```bash
python3 04_Operator_Analysis.py <predictions_csv> --output-dir <output_dir>
```

**Example**:
```bash
python3 04_Operator_Analysis.py \
    ../Model/predictions/predictions.csv \
    --output-dir ./evaluation_results
```
...
**Usage**:
```bash
python3 05_Time_Statistics.py <dataset_csv> --output-dir <output_dir>
```

**Example**:
```bash
python3 05_Time_Statistics.py \
