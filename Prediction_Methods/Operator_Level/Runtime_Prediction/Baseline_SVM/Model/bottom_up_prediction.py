...
def predict_operators_bottom_up(query_ops, predictions, children_map, df_overview, models_dir):
    leaf_nodes = query_ops[query_ops['node_type'].isin(['Seq Scan', 'Index Scan', 'Index Only Scan'])]
    ...
    remaining = query_ops[~query_ops['node_type'].isin(['Seq Scan', 'Index Scan', 'Index Only Scan'])].copy()
...
def load_model(models_dir, operator, target):
    ...
    model_path = Path(models_dir) / target / operator_normalized / 'model.pkl'
...
def get_model_features(df_overview, operator, target):
    operator_normalized = operator.replace(' ', '_')
    row = df_overview[(df_overview['operator'] == operator_normalized) & 
...
if __name__ == "__main__":
    ...
    models_directory = sys.argv[3]
    output_csv = sys.argv[4]
    
    run_prediction_workflow(test_csv, overview_csv, models_directory, output_csv)
