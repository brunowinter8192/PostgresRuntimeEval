...
def collect_operator_results(dataset_dir, output_dir):
    ...
    for operator in OPERATORS_FOLDER_NAMES:
        operator_path = Path(dataset_dir) / operator / f'{operator}.csv'
        if not operator_path.exists():
            continue
        for target in TARGET_TYPES:
            tasks.append((operator, target))

    results = Parallel(n_jobs=-1)(
        delayed(process_operator_target)(operator, target, dataset_dir, output_dir)
