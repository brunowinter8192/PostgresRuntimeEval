...
def train_final_patterns(
    selected_patterns_file: str,
    pattern_ffs_file: str,
    pattern_metadata_file: str,
    training_file: str,
    output_dir: str,
    n_jobs: int
) -> None:
    selected_patterns = load_selected_patterns(selected_patterns_file)
    pattern_metadata = load_pattern_metadata(pattern_metadata_file)
    pattern_ffs = load_pattern_ffs(pattern_ffs_file)
    df_training = load_training_data(training_file)

    selected_with_length = selected_patterns.merge(
        pattern_metadata[['pattern_hash', 'pattern_length']],
        on='pattern_hash',
        how='left'
    )
    trainable = filter_trainable_patterns(selected_with_length, pattern_ffs)
    ...
    export_training_summary(results, output_dir)


# FUNCTIONS

# Load selected pattern hashes from CSV
def load_selected_patterns(file_path: str) -> pd.DataFrame:
    return pd.read_csv(file_path, delimiter=';')


# Load pattern metadata with pattern_length
def load_pattern_metadata(file_path: str) -> pd.DataFrame:
    return pd.read_csv(file_path, delimiter=';')


# Load pattern FFS features from overview CSV
def load_pattern_ffs(file_path: str) -> dict:
...
# Filter patterns that have valid FFS for both targets
def filter_trainable_patterns(selected_patterns: pd.DataFrame, pattern_ffs: dict) -> pd.DataFrame:
    trainable = []
...
def train_single_pattern(
    ...
    aggregated_rows = collect_pattern_occurrences(
        pattern_hash, pattern_length, df_training
    )
    ...
    save_pattern_model(
        output_dir, pattern_hash, model_exec, model_start,
        exec_features_valid, start_features_valid
    )
...
# Collect all pattern occurrences from training data
...
if __name__ == '__main__':
    ...
    parser.add_argument('selected_patterns_file', help='Path to selected_patterns.csv')
    parser.add_argument('pattern_ffs_file', help='Path to pattern_ffs_overview.csv')
    parser.add_argument('pattern_metadata_file', help='Path to 06_patterns_by_frequency.csv (for pattern_length)')
    ...
    train_final_patterns(
        args.selected_patterns_file,
        args.pattern_ffs_file,
        args.pattern_metadata_file,
        args.training_file,
        args.output_dir,
        args.n_jobs
    )
