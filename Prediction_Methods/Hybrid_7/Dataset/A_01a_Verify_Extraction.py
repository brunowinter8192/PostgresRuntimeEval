...
# ORCHESTRATOR
def verify_extraction(patterns_csv: str, patterns_dir: str, output_dir: str) -> None:
...
def load_pattern_info(patterns_csv: str) -> pd.DataFrame:
    ...
    return df[['pattern_hash', 'pattern_string', 'pattern_length', 'operator_count', 'occurrence_count']]
...
def verify_all_patterns(pattern_info: pd.DataFrame, patterns_dir: str) -> pd.DataFrame:
    ...
    for _, row in pattern_info.iterrows():
        ...
        expected_rows = operator_count * occurrence_count

        pattern_file = base_dir / pattern_hash / 'training.csv'
...
if __name__ == '__main__':
    ...
    verify_extraction(args.patterns_csv, args.patterns_dir, args.output_dir)
