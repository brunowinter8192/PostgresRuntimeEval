...
# From mapping_config.py: Leaf operator types for cleaning
...
def load_leaf_pattern_mapping(csv_file: str) -> dict:
    ...
    for _, row in df.iterrows():
        ...
        folder_name = convert_pattern_to_folder_name(pattern_key)
        mapping[folder_name] = (leaf_status == 'True' or leaf_status == True)

    return mapping

# Convert pattern string to folder name format
def convert_pattern_to_folder_name(pattern_key):
...
def identify_columns_to_remove(df, parent_prefix):
    ...
    if parent_prefix:
        parent_child_features = [
            f'{parent_prefix}_st1',
            f'{parent_prefix}_rt1',
            f'{parent_prefix}_st2',
            f'{parent_prefix}_rt2'
        ]

        for col in parent_child_features:
            if col in df.columns:
                columns_to_remove.append(col)

    child_st_rt_suffixes = [
        '_Outer_st1', '_Outer_rt1', '_Outer_st2', '_Outer_rt2',
        '_Inner_st1', '_Inner_rt1', '_Inner_st2', '_Inner_rt2'
    ]

    for col in df.columns:
        if any(col.endswith(suffix) for suffix in child_st_rt_suffixes):
