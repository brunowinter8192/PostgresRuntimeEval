...
└── csv/                                           # [outputs]
    └── 01_baseline_patterns_depth1plus_{ts}.csv   # Pattern inventory with occurrences
```
...
The key configuration for Baseline vs All Templates:
...
**Depth Check (Line 46):**
```python
if parent_depth < 1:  # Hybrid_1: Only patterns from depth 1+ (root excluded)
    continue
# vs Hybrid_2:
...
# Hybrid_2 does NOT have this filter
```

### Output Naming

- Hybrid_1: `01_baseline_patterns_depth1plus_{ts}.csv`
- Hybrid_2: `01_baseline_patterns_depth0plus_{ts}.csv`
...
**Outputs:**
- `csv/01_baseline_patterns_depth1plus_{timestamp}.csv`
...
```bash
python 01_Find_Patterns.py <path_to_operator_dataset>.csv --output-dir .
```
