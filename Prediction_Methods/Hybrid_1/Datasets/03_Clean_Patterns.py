...
# From mapping_config.py: Leaf operators, child suffixes, and folder name conversion
from mapping_config import (
    LEAF_OPERATORS,
    CHILD_ACTUAL_SUFFIXES,
    CHILD_TIMING_SUFFIXES,
    PARENT_CHILD_FEATURES,
    pattern_to_folder_name
)
...
def get_pattern_folders(patterns_path):
    return [d for d in patterns_path.iterdir() if d.is_dir() and not d.name.startswith('.') and d.name != 'csv']
...
def identify_columns_to_remove(df, parent_prefix):
    ...
    if parent_prefix:
        parent_features = [f'{parent_prefix}{suffix}' for suffix in PARENT_CHILD_FEATURES]
