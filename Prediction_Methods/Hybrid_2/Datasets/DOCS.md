...
### Hybrid_2 vs Hybrid_1 Unterschiede

**Depth Check in Scripts:**
- 01_Extract_Patterns.py (Zeile 69): `if parent_depth < 0:` (Root eingeschlossen)
- 03_Aggregate_Patterns.py (Zeile 157): `if row['depth'] < 0:` (Root eingeschlossen)
- Hybrid_1: Verwendet `< 1` (Root ausgeschlossen)

**Operator Filter:**
- Hybrid_2: KEIN REQUIRED_OPERATORS Filter (alle Pattern-Typen)
- Hybrid_1: REQUIRED_OPERATORS Filter aktiv (nur Gather, Hash, Hash Join, Nested Loop, Seq Scan)

**Auswirkung:**
- Hybrid_2 extrahiert ~39 Patterns (inkl. Root-Level wie Sort_Aggregate_Outer)
- Hybrid_1 extrahiert ~21 Patterns (gefiltert, ohne Root-Level)
...
```
A_01a - Verify_Extraction   [pattern_csv + Pattern folders → extraction verification CSV]
A_01b - Verify_Aggregation  [pattern_csv + Pattern folders → aggregation verification CSV]
```
