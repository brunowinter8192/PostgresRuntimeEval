...
├── 03_Predict_Queries.py          # Predict queries using optimized execution plan
...
├── A_01c_Time_Analysis.py         # Operator range analysis (independent)
├── csv/                           # Script outputs
│   ├── 01_cleaned_ffs_YYYYMMDD_HHMMSS.csv
│   └── predictions.csv
...
**Pass-Through Operator Concept:**
- **Pass-Through Operators:** Operators with startup_total_ratio > 0.95 (Incremental Sort, Merge Join, Sort, Hash, Limit, Gather Merge, Aggregate)
- **Excluded as pattern roots** in Data_Generation phase (cannot be root of patterns)
- **Can appear as children** within patterns
- **Prediction strategy:** Inherit prediction from first child operator/pattern (no separate model prediction)
- **Rationale:** Reduces prediction errors for operators that primarily pass through child timing

**3-Level Fallback Strategy:**
1. **Pattern Match:** If operator matches a pattern - Use pattern model
2. **Pass-Through:** If operator is PT operator with children - Inherit child prediction
3. **Operator Fallback:** Otherwise - Use operator-level model from Operator_Level/Runtime_Prediction/Baseline_SVM
...
csv/01_cleaned_ffs_{timestamp}.csv  (Pattern+Plan Leaf child features removed)
    |
    v
02_Train_Models.py  (Train on cleaned FFS)
    |
    v
Model/{target}/{pattern_hash}/model.pkl  (64 models: 32 patterns x 2 targets)
    |
    v
03_Predict_Queries.py  (Greedy pattern matching + 3-level fallback)
    |
    v
csv/predictions.csv
    |
...
- 02 requires: Cleaned FFS + Pattern Datasets
- 03 requires: Test Data + Pattern Info + Models (Pattern + Operator)
- A_01a requires: Predictions CSV
- A_01b requires: Predictions CSV
- A_01c requires: Operator Dataset (independent)
...
**Outputs:**
- `Model/{target}/{pattern_hash}/model.pkl` - Trained SVM models (64 models total: 32 patterns x 2 targets)
...
**Purpose:** Predict queries using optimized execution plan with 3-level fallback (Pattern, Pass-Through, Operator)

**Rationale:**
Hybrid_6 combines Hybrid_5's greedy pattern matching with Hybrid_4's pass-through logic. For each query, create an optimized execution plan using greedy pattern matching (largest patterns first, top-down), then execute with 3-level fallback: (1) Pattern model for matching patterns, (2) Inheritance for pass-through operators, (3) Operator model for remaining operators.
...
2. For each query:
   ...
   - Create optimized execution plan (Greedy, Top-Down, Hash-based pattern matching):
     - Try pattern match (largest first)
     - If no match and PT operator with children - Mark as passthrough
     - Otherwise - Mark as operator
   - Execute plan step by step:
     - **Pattern Steps:** Aggregate pattern features + child predictions - Predict with Pattern Model
     - **Pass-Through Steps:** Inherit prediction from first child (no model prediction)
...
**Inputs:**
- `predictions_file` - Predictions CSV from 03_Predict_Queries.py
...
**Outputs:**
- `pattern_mean_mre_total_pct_{timestamp}.csv`
- `pattern_mean_mre_startup_pct_{timestamp}.csv`
- `operator_mean_mre_total_pct_{timestamp}.csv`
- `operator_mean_mre_startup_pct_{timestamp}.csv`
- `node_type_mean_mre_total_pct_{timestamp}.csv`
- `node_type_mean_mre_startup_pct_{timestamp}.csv`
