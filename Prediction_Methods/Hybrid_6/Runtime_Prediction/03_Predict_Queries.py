...
# From mapping_config.py: Pattern definitions, target types, and pass-through operator detection
from mapping_config import PATTERNS, TARGET_TYPES, is_passthrough_operator
...
def predict_single_query(df_query, pattern_info, plan_leaf_mapping, pattern_features, operator_features, pattern_models, operator_models):
    ...
    execution_plan = sort_execution_plan_bottom_up(execution_plan, df_query)
...
def build_optimized_plan_greedy(root, pattern_info, df_query):
    ...
    for node in all_nodes_sorted:
        ...
        if matched_pattern:
            ...
            if is_passthrough_operator(node.node_type) and len(node.children) > 0:
                step = {
                    'type': 'passthrough',
                    ...
                    'depth': node.depth,
                    'first_child_df_index': node.children[0].df_index,
                    'first_child_node_id': node.children[0].node_id
                }
            else:
                step = {
                    'type': 'operator',
...
# Sort execution plan bottom-up for correct pass-through execution
def sort_execution_plan_bottom_up(execution_plan, df_query):
    def get_step_depth(step):
        if 'depth' in step:
            return step['depth']
        return df_query.iloc[step['root_df_index']]['depth']

    return sorted(execution_plan, key=get_step_depth, reverse=True)
...
def execute_prediction_plan(execution_plan, df_query, plan_leaf_mapping, pattern_features, operator_features, pattern_models, operator_models):
    ...
    for step in execution_plan:
        if step['type'] == 'pattern':
            ...
            child_node_id = step['first_child_node_id']

            if child_node_id in prediction_cache:
                pred_start = prediction_cache[child_node_id]['predicted_startup_time']
                pred_exec = prediction_cache[child_node_id]['predicted_total_time']
            else:
                pred_start, pred_exec = 0.0, 0.0
...
def export_predictions(predictions, output_dir):
    ...
    df_predictions = df_predictions.sort_values(['query_file', 'node_id'])
