...
def evaluate_cross_predictions(predictions_file, output_dir):
    ...
    df_passthrough = filter_by_prediction_type(df, 'passthrough')
    ...
    passthrough_total, passthrough_startup = create_pivot_tables(df_passthrough)
    overall_total, overall_startup = create_pivot_tables(df)
    export_results(pattern_total, pattern_startup, operator_total, operator_startup, passthrough_total, passthrough_startup, overall_total, overall_startup, output_dir)
...
# Export all pivot tables to CSV
def export_results(pattern_total, pattern_startup, operator_total, operator_startup, passthrough_total, passthrough_startup, overall_total, overall_startup, output_dir):
    ...
    if not passthrough_total.empty:
        passthrough_total.index.name = 'node_type (Passthrough Mean MRE Total Time %)'
        output_file = output_path / f'passthrough_mean_mre_total_pct_{timestamp}.csv'
        passthrough_total.to_csv(output_file, sep=';')

    if not passthrough_startup.empty:
        passthrough_startup.index.name = 'node_type (Passthrough Mean MRE Startup Time %)'
        output_file = output_path / f'passthrough_mean_mre_startup_pct_{timestamp}.csv'
        passthrough_startup.to_csv(output_file, sep=';')
