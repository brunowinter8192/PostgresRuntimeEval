...
# ORCHESTRATOR
def verify_extraction(patterns_csv: str, datasets_dir: str, output_dir: str) -> None:
...
def verify_all_patterns(pattern_info: pd.DataFrame, datasets_dir: str) -> pd.DataFrame:
    ...
    for _, row in pattern_info.iterrows():
        ...
        operator_count = PATTERN_OPERATOR_COUNT.get(pattern_hash, 0)
...
def export_results(results: pd.DataFrame, output_dir: str) -> None:
    ...
    output_file = csv_dir / f'A_01a_verification_{timestamp}.csv'
...
if __name__ == '__main__':
    ...
    verify_extraction(args.patterns_csv, args.datasets_dir, args.output_dir)
