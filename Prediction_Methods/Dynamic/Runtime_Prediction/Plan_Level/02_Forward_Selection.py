#!/usr/bin/env python3

# INFRASTRUCTURE

import argparse
import importlib
import re
from pathlib import Path

import numpy as np
import pandas as pd
from sklearn.metrics import make_scorer
from sklearn.model_selection import StratifiedKFold, cross_val_score
from sklearn.pipeline import Pipeline

# From ffs_config.py: FFS parameters and model config
from ffs_config import METADATA_COLUMNS, TARGET, FFS_CONFIG, SVM_CONFIG

# ORCHESTRATOR

def ffs_workflow(dataset_dir: str, output_dir: str) -> None:
    dataset_path = Path(dataset_dir)
    output_path = Path(output_dir)
    template_folders = sorted([d for d in dataset_path.iterdir() if d.is_dir()])

    for template_folder in template_folders:
        process_template(template_folder, output_path)


def process_template(template_folder: Path, output_path: Path) -> None:
    template_name = template_folder.name
    training_file = template_folder / 'training.csv'

    if not training_file.exists():
        return

    template_output = output_path / template_name
    template_output.mkdir(parents=True, exist_ok=True)

    df = load_dataset(training_file)
    feature_cols = get_feature_columns(df)
    X = df[feature_cols]
    y = df[TARGET]
    template_ids = extract_template_ids(df)

    all_seed_results = run_multi_seed_selection(X, y, feature_cols, template_ids)
    export_results(all_seed_results, template_output)


# FUNCTIONS

# Load dataset from CSV with semicolon delimiter
def load_dataset(input_csv: Path) -> pd.DataFrame:
    return pd.read_csv(input_csv, delimiter=';')


# Get feature columns excluding metadata
def get_feature_columns(df: pd.DataFrame) -> list:
    return [col for col in df.columns if col not in METADATA_COLUMNS]


# Extract template IDs from template column
def extract_template_ids(df: pd.DataFrame) -> np.ndarray:
    return df['template'].apply(extract_template_id).values


# Extract numeric template ID from template string
def extract_template_id(template_str) -> int:
    match = re.search(r'q(\d+)', str(template_str).lower())
    if match:
        return int(match.group(1))
    return 0


# Run forward selection across multiple seeds
def run_multi_seed_selection(X: pd.DataFrame, y: pd.Series, feature_cols: list, template_ids: np.ndarray) -> list:
    all_seed_results = []

    for seed in FFS_CONFIG['seeds']:
        cv = StratifiedKFold(n_splits=FFS_CONFIG['n_splits'], shuffle=True, random_state=seed)
        selected_features, results_df = forward_selection(X, y, feature_cols, template_ids, cv)

        final_mre = results_df.iloc[-1]['mre']
        n_features = len(selected_features)

        seed_result = {
            'seed': seed,
            'n_features': n_features,
            'final_mre': final_mre,
            'selected_features': ', '.join(selected_features)
        }
        all_seed_results.append(seed_result)

    return all_seed_results


# Forward selection algorithm with minimum features constraint
def forward_selection(X: pd.DataFrame, y: pd.Series, feature_cols: list, template_ids: np.ndarray, cv) -> tuple:
    selected = []
    remaining = list(feature_cols)
    results = []
    best_score = float('inf')
    iteration = 0

    while remaining:
        iteration += 1
        scores = {}

        for feature in remaining:
            test_features = selected + [feature]
            score = evaluate_features(X, y, test_features, template_ids, cv)
            scores[feature] = score

        best_feature = min(scores, key=scores.get)
        best_new_score = scores[best_feature]

        if iteration <= FFS_CONFIG['min_features']:
            should_add = True
        else:
            improvement = (best_score - best_new_score) / best_score
            should_add = improvement >= 0

        if should_add:
            selected.append(best_feature)
            remaining.remove(best_feature)
            best_score = best_new_score

            results.append({
                'iteration': iteration,
                'feature_added': best_feature,
                'mre': best_score,
                'n_features': len(selected),
                'features': ', '.join(selected)
            })
        else:
            break

    return selected, pd.DataFrame(results)


# Evaluate feature set using cross-validation
def evaluate_features(X: pd.DataFrame, y: pd.Series, features: list, template_ids: np.ndarray, cv) -> float:
    model_module = importlib.import_module(SVM_CONFIG['module'])
    model_class = getattr(model_module, SVM_CONFIG['class_name'])
    model = model_class(**SVM_CONFIG['params'])

    scaler_module = importlib.import_module(SVM_CONFIG['scaler_module'])
    scaler_class = getattr(scaler_module, SVM_CONFIG['scaler_class'])
    estimator = Pipeline([
        ('scaler', scaler_class()),
        ('model', model)
    ])

    scorer = make_scorer(mean_relative_error, greater_is_better=False)
    scores = cross_val_score(
        estimator,
        X[features],
        y,
        cv=cv.split(X[features], template_ids),
        scoring=scorer,
        n_jobs=-1
    )

    return -scores.mean()


# Calculate mean relative error metric
def mean_relative_error(y_true, y_pred) -> float:
    epsilon = 1e-6
    relative_errors = np.abs((y_true - y_pred) / (y_true + epsilon))
    return np.mean(relative_errors)


# Export results to CSV file
def export_results(all_seed_results: list, output_dir: Path) -> None:
    summary_df = pd.DataFrame(all_seed_results)
    summary_df.to_csv(output_dir / 'ffs_summary.csv', sep=';', index=False)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("dataset_dir", help="Path to Dataset/ containing template folders")
    parser.add_argument("--output-dir", required=True, help="Output directory for FFS results")

    args = parser.parse_args()

    ffs_workflow(args.dataset_dir, args.output_dir)
