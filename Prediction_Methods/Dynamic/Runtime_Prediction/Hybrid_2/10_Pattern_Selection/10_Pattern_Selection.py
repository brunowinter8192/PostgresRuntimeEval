...
def run_pattern_selection(
    ...
    if strategy == 'error':
        run_error_selection(
            sorted_patterns, pattern_occurrences, df_training, df_test,
            operator_models, operator_ffs, output_dir, pretrained_dir,
            epsilon, strategy, report, max_iteration
        )
    else:
        run_static_selection(
            sorted_patterns, pattern_occurrences, df_training, df_test,
            operator_models, operator_ffs, output_dir, pretrained_dir,
            min_error_threshold, epsilon, strategy, report, max_iteration
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('sorted_patterns_file', help='Path to sorted patterns CSV (07_patterns_by_*.csv)')
    ...
    parser.add_argument('--pretrained-dir', required=True, help='Path to pretrained pattern models')
    parser.add_argument('--pattern-occurrences-file', required=True, help='Path to 06_test_pattern_occurrences.csv')
    ...
    parser.add_argument('--max-iteration', type=int, default=None, help='Max iterations (default: all)')
