...
Extended with MD report generation for debugging and documentation.
"""
...
def run_prediction_workflow(test_file, overview_file, models_dir, output_file, md_query=None, report=False, report_dir=None):
    ...
    if report:
        ...
        export_md_report(md_query, test_file, overview_file, models_dir, query_ops, predictions, steps, report_dir)
    else:
    ...
    for idx, leaf in leaf_nodes.iterrows():
        ...
        if not model_exists(models_dir, node_type, 'execution_time'):
            ...
            steps.append({
                ...
                'is_leaf': True,
                'prediction_type': 'passthrough',
                'model_path_exec': None,
                'model_path_start': None,
                'features_exec': [],
                'features_start': [],
                'input_values_exec': {},
                'input_values_start': {},
...
def export_md_report(query_file, test_file, overview_file, models_dir, query_ops, predictions, steps, report_dir=None, plan_hash=None):
    ...
    lines = []
    lines.append('# Query Prediction Report (Dynamic/LOTO)')
    lines.append('')
    ...
    for step in steps:
        ...
        prediction_type = step.get('prediction_type', 'model')
        type_marker = ' [PASSTHROUGH]' if prediction_type == 'passthrough' else ''

        lines.append(f"### Step {step['step']}: Node {step['node_id']} ({step['node_type']}){leaf_marker}{type_marker}")
        lines.append('')

        if prediction_type == 'passthrough':
            lines.append('**Prediction Type:** PASSTHROUGH (no model available, using max child prediction)')
            lines.append('')
        else:
    ...
    lines.append('')
    lines.append('| Node | Type | Pred Type | Actual ST | Actual RT | Pred ST | Pred RT | MRE ST (%) | MRE RT (%) |')
    lines.append('|------|------|-----------|-----------|-----------|---------|---------|------------|------------|')
    ...
    for step in steps:
        ...
        pred_type = step.get('prediction_type', 'model')
        ...
        lines.append(f"| {step['node_id']} | {step['node_type']} | {pred_type} | {actual_st:.2f} | {actual_rt:.2f} | {pred_st:.2f} | {pred_rt:.2f} | {mre_st:.1f} | {mre_rt:.1f} |")
