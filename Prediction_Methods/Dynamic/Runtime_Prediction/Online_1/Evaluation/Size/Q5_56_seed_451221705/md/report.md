...
**Test Query:** Q5_56_seed_451221705
...
```
Node 8921 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8922 (Aggregate) [consumed]
    Node 8923 (Gather Merge) [consumed]
      Node 8924 (Aggregate)
        Node 8925 (Sort) [PATTERN: 3e2d5a00]
          Node 8926 (Hash Join) [consumed]
            Node 8927 (Nested Loop)
              Node 8928 (Hash Join)
                Node 8929 (Seq Scan) - LEAF
                Node 8930 (Hash) [PATTERN: 444761fb]
                  Node 8931 (Hash Join) [consumed]
                    Node 8932 (Seq Scan) [consumed] - LEAF
                    Node 8933 (Hash) [consumed]
                      Node 8934 (Hash Join) [consumed]
                        Node 8935 (Seq Scan) - LEAF
                        Node 8936 (Hash)
                          Node 8937 (Seq Scan) - LEAF
              Node 8938 (Index Scan) - LEAF
            Node 8939 (Hash)
              Node 8940 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8930 | 8921, 8922, 8923, 8925, 8926, 8931, 8932, 8933, 8934 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8921 | 8922, 8923, 8925, 8926, 8930, 8931, 8932, 8933, 8934 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8925 | 8921, 8922, 8923, 8926, 8930, 8931, 8932, 8933, 8934 |
...
|------|------|--------|-----------|-----|--------|
| 8921 | Sort | 1077.24 | 2934.42 | 172.4% | pattern |
| 8924 | Aggregate | 1072.45 | 1128.76 | 5.3% | operator |
| 8925 | Sort | 1072.17 | 1835.93 | 71.2% | pattern |
| 8927 | Nested Loop | 1057.74 | 1188.47 | 12.4% | operator |
| 8939 | Hash | 5.56 | 18.67 | 235.9% | operator |
| 8928 | Hash Join | 222.36 | 253.07 | 13.8% | operator |
| 8938 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 8940 | Seq Scan | 5.04 | 12.36 | 145.4% | operator |
| 8929 | Seq Scan | 168.27 | 159.81 | 5.0% | operator |
| 8930 | Hash | 38.78 | 46.85 | 20.8% | pattern |
| 8935 | Seq Scan | 0.01 | 11.81 | 98302.3% | operator |
| 8936 | Hash | 0.05 | 21.05 | 39623.1% | operator |
| 8937 | Seq Scan | 0.05 | 22.93 | 45766.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8937 (Seq Scan) - LEAF
...
### Step 2: Node 8935 (Seq Scan) - LEAF
...
### Step 4: Node 8929 (Seq Scan) - LEAF
...
### Step 5: Node 8930 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8921, 8922, 8923, 8925, 8926, 8931, 8932, 8933, 8934
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8938 (Index Scan) - LEAF
...
### Step 8: Node 8940 (Seq Scan) - LEAF
...
### Step 11: Node 8925 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8921, 8922, 8923, 8926, 8930, 8931, 8932, 8933, 8934
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8921 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8922, 8923, 8925, 8926, 8930, 8931, 8932, 8933, 8934
- **Input Features:**
