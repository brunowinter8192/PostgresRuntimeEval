...
**Test Query:** Q8_122_seed_992687751
...
```
Node 13753 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13754 (Gather Merge) [consumed]
    Node 13755 (Sort)
      Node 13756 (Hash Join)
        Node 13757 (Nested Loop)
          Node 13758 (Hash Join)
            Node 13759 (Nested Loop)
              Node 13760 (Hash Join)
                Node 13761 (Seq Scan) - LEAF
                Node 13762 (Hash) [PATTERN: 444761fb]
                  Node 13763 (Hash Join) [consumed]
                    Node 13764 (Seq Scan) [consumed] - LEAF
                    Node 13765 (Hash) [consumed]
                      Node 13766 (Hash Join) [consumed]
                        Node 13767 (Seq Scan) - LEAF
                        Node 13768 (Hash)
                          Node 13769 (Seq Scan) - LEAF
              Node 13770 (Index Scan) - LEAF
            Node 13771 (Hash)
              Node 13772 (Seq Scan) - LEAF
          Node 13773 (Index Scan) - LEAF
        Node 13774 (Hash)
          Node 13775 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13762 | 13753, 13754, 13763, 13764, 13765, 13766 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13753 | 13754, 13762, 13763, 13764, 13765, 13766 |
...
|------|------|--------|-----------|-----|--------|
| 13753 | Aggregate | 1202.44 | 1125.52 | 6.4% | pattern |
| 13755 | Sort | 1195.64 | 1046.22 | 12.5% | operator |
| 13756 | Hash Join | 1195.38 | 785.61 | 34.3% | operator |
| 13757 | Nested Loop | 1195.04 | 1198.29 | 0.3% | operator |
| 13774 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 13758 | Hash Join | 1190.82 | 785.60 | 34.0% | operator |
| 13773 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13775 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13759 | Nested Loop | 1145.76 | 1198.29 | 4.6% | operator |
| 13771 | Hash | 40.18 | 19.97 | 50.3% | operator |
| 13760 | Hash Join | 211.67 | 251.67 | 18.9% | operator |
| 13770 | Index Scan | 0.04 | 2.88 | 7102.1% | operator |
| 13772 | Seq Scan | 40.09 | 48.86 | 21.9% | operator |
| 13761 | Seq Scan | 160.75 | 159.67 | 0.7% | operator |
| 13762 | Hash | 32.04 | 43.51 | 35.8% | pattern |
| 13767 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 13768 | Hash | 0.07 | 18.14 | 26582.6% | operator |
| 13769 | Seq Scan | 0.07 | 23.36 | 35297.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13769 (Seq Scan) - LEAF
...
### Step 2: Node 13767 (Seq Scan) - LEAF
...
### Step 4: Node 13761 (Seq Scan) - LEAF
...
### Step 5: Node 13762 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13753, 13754, 13763, 13764, 13765, 13766
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13770 (Index Scan) - LEAF
...
### Step 8: Node 13772 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - total_cost=70271.9900
- **Output:** st=113.62, rt=785.60

### Step 12: Node 13773 (Index Scan) - LEAF
...
### Step 13: Node 13775 (Seq Scan) - LEAF
...
### Step 14: Node 13757 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.8000
  - total_cost=70523.1000
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 13774 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.3600
  - total_cost=70529.2900
- **Output:** st=113.62, rt=785.61

### Step 17: Node 13755 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70569.5300
  - total_cost=70571.6000
- **Output:** st=1044.95, rt=1046.22

### Step 18: Node 13753 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13754, 13762, 13763, 13764, 13765, 13766
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2574
  ...
  - Aggregate_total_cost=71973.6900
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2574
  - GatherMerge_Outer_nt1=830
  ...
  - GatherMerge_Outer_total_cost=71874.0900
- **Output:** st=1122.60, rt=1125.52
