...
**Test Query:** Q7_144_seed_1173176433
...
```
Node 11365 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11366 (Gather Merge) [consumed]
    Node 11367 (Sort)
      Node 11368 (Hash Join)
        Node 11369 (Nested Loop)
          Node 11370 (Hash Join) [PATTERN: 895c6e8c]
            Node 11371 (Seq Scan) [consumed] - LEAF
            Node 11372 (Hash) [consumed]
              Node 11373 (Hash Join) [PATTERN: f4cb205a]
                Node 11374 (Seq Scan) [consumed] - LEAF
                Node 11375 (Hash) [consumed]
                  Node 11376 (Seq Scan) [consumed] - LEAF
          Node 11377 (Index Scan) - LEAF
        Node 11378 (Hash)
          Node 11379 (Hash Join) [PATTERN: f4cb205a]
            Node 11380 (Seq Scan) [consumed] - LEAF
            Node 11381 (Hash) [consumed]
              Node 11382 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11373 | 11365, 11366, 11370, 11371, 11372, 11374, 11375, 11376, 11379, 11380, 11381, 11382 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11379 | 11365, 11366, 11370, 11371, 11372, 11373, 11374, 11375, 11376, 11380, 11381, 11382 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11365 | 11366, 11370, 11371, 11372, 11373, 11374, 11375, 11376, 11379, 11380, 11381, 11382 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11370 | 11365, 11366, 11371, 11372, 11373, 11374, 11375, 11376, 11379, 11380, 11381, 11382 |
...
|------|------|--------|-----------|-----|--------|
| 11365 | Aggregate | 1195.43 | 1163.13 | 2.7% | pattern |
| 11367 | Sort | 1188.75 | 1109.16 | 6.7% | operator |
| 11368 | Hash Join | 1188.09 | 843.11 | 29.0% | operator |
| 11369 | Nested Loop | 1180.74 | 1215.99 | 3.0% | operator |
| 11378 | Hash | 3.23 | 27.74 | 758.1% | operator |
| 11370 | Hash Join | 207.77 | 304.89 | 46.7% | pattern |
| 11377 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11379 | Hash Join | 3.19 | 429.36 | 13368.1% | pattern |
| 11373 | Hash Join | 33.74 | 429.38 | 1172.5% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11373 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11365, 11366, 11370, 11371, 11372, 11374, 11375, 11376, 11379, 11380, 11381, 11382
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11370 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11365, 11366, 11371, 11372, 11373, 11374, 11375, 11376, 11379, 11380, 11381, 11382
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11377 (Index Scan) - LEAF
...
### Step 4: Node 11379 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11365, 11366, 11370, 11371, 11372, 11373, 11374, 11375, 11376, 11380, 11381, 11382
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11365 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11366, 11370, 11371, 11372, 11373, 11374, 11375, 11376, 11379, 11380, 11381, 11382
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
