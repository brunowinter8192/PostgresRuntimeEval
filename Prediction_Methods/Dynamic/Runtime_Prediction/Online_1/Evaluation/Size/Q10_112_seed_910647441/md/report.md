...
**Test Query:** Q10_112_seed_910647441
...
```
Node 19539 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19540 (Sort) [consumed]
    Node 19541 (Aggregate) [consumed]
      Node 19542 (Gather) [consumed]
        Node 19543 (Hash Join)
          Node 19544 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19545 (Nested Loop) [consumed]
              Node 19546 (Seq Scan) - LEAF
              Node 19547 (Index Scan) - LEAF
            Node 19548 (Hash) [consumed]
              Node 19549 (Seq Scan) - LEAF
          Node 19550 (Hash)
            Node 19551 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19539 | 19540, 19541, 19542, 19544, 19545, 19548 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19544 | 19539, 19540, 19541, 19542, 19545, 19548 |
...
|------|------|--------|-----------|-----|--------|
| 19539 | Limit | 1145.45 | 1199.47 | 4.7% | pattern |
| 19543 | Hash Join | 1103.69 | 1095.51 | 0.7% | operator |
| 19544 | Hash Join | 1100.86 | 1120.19 | 1.8% | pattern |
| 19550 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 19551 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 19546 | Seq Scan | 164.77 | 159.74 | 3.1% | operator |
| 19547 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 19549 | Seq Scan | 28.62 | 26.64 | 6.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19546 (Seq Scan) - LEAF
...
### Step 2: Node 19547 (Index Scan) - LEAF
...
### Step 3: Node 19549 (Seq Scan) - LEAF
...
### Step 4: Node 19551 (Seq Scan) - LEAF
...
### Step 5: Node 19544 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19539, 19540, 19541, 19542, 19545, 19548
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18043
  - HashJoin_nt1=18043
...
### Step 8: Node 19539 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19540, 19541, 19542, 19544, 19545, 19548
- **Input Features:**
