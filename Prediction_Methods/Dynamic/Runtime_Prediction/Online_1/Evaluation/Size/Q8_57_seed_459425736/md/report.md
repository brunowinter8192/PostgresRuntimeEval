...
**Test Query:** Q8_57_seed_459425736
...
```
Node 15547 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15548 (Gather Merge) [consumed]
    Node 15549 (Sort)
      Node 15550 (Hash Join)
        Node 15551 (Nested Loop)
          Node 15552 (Hash Join)
            Node 15553 (Nested Loop)
              Node 15554 (Hash Join)
                Node 15555 (Seq Scan) - LEAF
                Node 15556 (Hash) [PATTERN: 444761fb]
                  Node 15557 (Hash Join) [consumed]
                    Node 15558 (Seq Scan) [consumed] - LEAF
                    Node 15559 (Hash) [consumed]
                      Node 15560 (Hash Join) [consumed]
                        Node 15561 (Seq Scan) - LEAF
                        Node 15562 (Hash)
                          Node 15563 (Seq Scan) - LEAF
              Node 15564 (Index Scan) - LEAF
            Node 15565 (Hash)
              Node 15566 (Seq Scan) - LEAF
          Node 15567 (Index Scan) - LEAF
        Node 15568 (Hash)
          Node 15569 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15556 | 15547, 15548, 15557, 15558, 15559, 15560 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15547 | 15548, 15556, 15557, 15558, 15559, 15560 |
...
|------|------|--------|-----------|-----|--------|
| 15547 | Aggregate | 1211.33 | 1131.48 | 6.6% | pattern |
| 15549 | Sort | 1206.78 | 1046.21 | 13.3% | operator |
| 15550 | Hash Join | 1206.54 | 785.61 | 34.9% | operator |
| 15551 | Nested Loop | 1206.22 | 1198.29 | 0.7% | operator |
| 15568 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 15552 | Hash Join | 1202.57 | 785.60 | 34.7% | operator |
| 15567 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15569 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 15553 | Nested Loop | 1157.01 | 1198.29 | 3.6% | operator |
| 15565 | Hash | 41.16 | 19.97 | 51.5% | operator |
| 15554 | Hash Join | 216.14 | 251.67 | 16.4% | operator |
| 15564 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15566 | Seq Scan | 41.04 | 48.87 | 19.1% | operator |
| 15555 | Seq Scan | 160.25 | 159.67 | 0.4% | operator |
| 15556 | Hash | 37.13 | 43.51 | 17.2% | pattern |
| 15561 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 15562 | Hash | 0.16 | 18.14 | 11311.4% | operator |
| 15563 | Seq Scan | 0.16 | 23.36 | 14875.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15563 (Seq Scan) - LEAF
...
### Step 2: Node 15561 (Seq Scan) - LEAF
...
### Step 4: Node 15555 (Seq Scan) - LEAF
...
### Step 5: Node 15556 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15547, 15548, 15557, 15558, 15559, 15560
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15564 (Index Scan) - LEAF
...
### Step 8: Node 15566 (Seq Scan) - LEAF
...
### Step 12: Node 15567 (Index Scan) - LEAF
...
### Step 13: Node 15569 (Seq Scan) - LEAF
...
### Step 14: Node 15551 (Nested Loop)
...
### Step 18: Node 15547 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15548, 15556, 15557, 15558, 15559, 15560
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2449
