...
**Test Query:** Q10_107_seed_869627286
...
```
Node 19461 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19462 (Sort) [consumed]
    Node 19463 (Aggregate) [consumed]
      Node 19464 (Gather) [consumed]
        Node 19465 (Hash Join)
          Node 19466 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19467 (Nested Loop) [consumed]
              Node 19468 (Seq Scan) - LEAF
              Node 19469 (Index Scan) - LEAF
            Node 19470 (Hash) [consumed]
              Node 19471 (Seq Scan) - LEAF
          Node 19472 (Hash)
            Node 19473 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19461 | 19462, 19463, 19464, 19466, 19467, 19470 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19466 | 19461, 19462, 19463, 19464, 19467, 19470 |
...
|------|------|--------|-----------|-----|--------|
| 19461 | Limit | 1145.96 | 1199.47 | 4.7% | pattern |
| 19465 | Hash Join | 1093.53 | 1095.48 | 0.2% | operator |
| 19466 | Hash Join | 1090.56 | 1120.19 | 2.7% | pattern |
| 19472 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 19473 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 19468 | Seq Scan | 159.71 | 159.74 | 0.0% | operator |
| 19469 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 19471 | Seq Scan | 28.86 | 26.64 | 7.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19468 (Seq Scan) - LEAF
...
### Step 2: Node 19469 (Index Scan) - LEAF
...
### Step 3: Node 19471 (Seq Scan) - LEAF
...
### Step 4: Node 19473 (Seq Scan) - LEAF
...
### Step 5: Node 19466 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19461, 19462, 19463, 19464, 19467, 19470
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17901
  - HashJoin_nt1=17901
...
### Step 8: Node 19461 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19462, 19463, 19464, 19466, 19467, 19470
- **Input Features:**
