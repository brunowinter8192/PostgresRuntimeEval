...
**Test Query:** Q8_50_seed_401997519
...
```
Node 15386 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15387 (Gather Merge) [consumed]
    Node 15388 (Sort)
      Node 15389 (Hash Join)
        Node 15390 (Nested Loop)
          Node 15391 (Hash Join)
            Node 15392 (Nested Loop)
              Node 15393 (Hash Join)
                Node 15394 (Seq Scan) - LEAF
                Node 15395 (Hash) [PATTERN: 444761fb]
                  Node 15396 (Hash Join) [consumed]
                    Node 15397 (Seq Scan) [consumed] - LEAF
                    Node 15398 (Hash) [consumed]
                      Node 15399 (Hash Join) [consumed]
                        Node 15400 (Seq Scan) - LEAF
                        Node 15401 (Hash)
                          Node 15402 (Seq Scan) - LEAF
              Node 15403 (Index Scan) - LEAF
            Node 15404 (Hash)
              Node 15405 (Seq Scan) - LEAF
          Node 15406 (Index Scan) - LEAF
        Node 15407 (Hash)
          Node 15408 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15395 | 15386, 15387, 15396, 15397, 15398, 15399 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15386 | 15387, 15395, 15396, 15397, 15398, 15399 |
...
|------|------|--------|-----------|-----|--------|
| 15386 | Aggregate | 1220.02 | 1127.83 | 7.6% | pattern |
| 15388 | Sort | 1214.58 | 1046.22 | 13.9% | operator |
| 15389 | Hash Join | 1214.29 | 785.61 | 35.3% | operator |
| 15390 | Nested Loop | 1213.94 | 1198.29 | 1.3% | operator |
| 15407 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 15391 | Hash Join | 1209.79 | 785.60 | 35.1% | operator |
| 15406 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15408 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 15392 | Nested Loop | 1164.72 | 1198.29 | 2.9% | operator |
| 15404 | Hash | 40.62 | 19.97 | 50.8% | operator |
| 15393 | Hash Join | 211.40 | 251.67 | 19.0% | operator |
| 15403 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 15405 | Seq Scan | 40.52 | 48.86 | 20.6% | operator |
| 15394 | Seq Scan | 159.52 | 159.67 | 0.1% | operator |
| 15395 | Hash | 33.85 | 43.51 | 28.5% | pattern |
| 15400 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 15401 | Hash | 0.18 | 18.14 | 9814.8% | operator |
| 15402 | Seq Scan | 0.18 | 23.36 | 12951.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15402 (Seq Scan) - LEAF
...
### Step 2: Node 15400 (Seq Scan) - LEAF
...
### Step 4: Node 15394 (Seq Scan) - LEAF
...
### Step 5: Node 15395 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15386, 15387, 15396, 15397, 15398, 15399
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15403 (Index Scan) - LEAF
...
### Step 8: Node 15405 (Seq Scan) - LEAF
...
### Step 12: Node 15406 (Index Scan) - LEAF
...
### Step 13: Node 15408 (Seq Scan) - LEAF
...
### Step 14: Node 15390 (Nested Loop)
...
### Step 18: Node 15386 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15387, 15395, 15396, 15397, 15398, 15399
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2524
