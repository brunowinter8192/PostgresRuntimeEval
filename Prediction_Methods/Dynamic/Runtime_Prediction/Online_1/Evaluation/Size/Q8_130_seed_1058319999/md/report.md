...
**Test Query:** Q8_130_seed_1058319999
...
```
Node 13960 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13961 (Gather Merge) [consumed]
    Node 13962 (Sort)
      Node 13963 (Hash Join)
        Node 13964 (Nested Loop)
          Node 13965 (Hash Join)
            Node 13966 (Nested Loop)
              Node 13967 (Hash Join)
                Node 13968 (Seq Scan) - LEAF
                Node 13969 (Hash) [PATTERN: 444761fb]
                  Node 13970 (Hash Join) [consumed]
                    Node 13971 (Seq Scan) [consumed] - LEAF
                    Node 13972 (Hash) [consumed]
                      Node 13973 (Hash Join) [consumed]
                        Node 13974 (Seq Scan) - LEAF
                        Node 13975 (Hash)
                          Node 13976 (Seq Scan) - LEAF
              Node 13977 (Index Scan) - LEAF
            Node 13978 (Hash)
              Node 13979 (Seq Scan) - LEAF
          Node 13980 (Index Scan) - LEAF
        Node 13981 (Hash)
          Node 13982 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13969 | 13960, 13961, 13970, 13971, 13972, 13973 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13960 | 13961, 13969, 13970, 13971, 13972, 13973 |
...
|------|------|--------|-----------|-----|--------|
| 13960 | Aggregate | 1235.44 | 1133.67 | 8.2% | pattern |
| 13962 | Sort | 1228.79 | 1046.21 | 14.9% | operator |
| 13963 | Hash Join | 1228.54 | 785.61 | 36.1% | operator |
| 13964 | Nested Loop | 1228.22 | 1198.29 | 2.4% | operator |
| 13981 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 13965 | Hash Join | 1224.24 | 785.60 | 35.8% | operator |
| 13980 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13982 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 13966 | Nested Loop | 1178.08 | 1198.29 | 1.7% | operator |
| 13978 | Hash | 40.97 | 19.97 | 51.3% | operator |
| 13967 | Hash Join | 214.82 | 251.67 | 17.2% | operator |
| 13977 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 13979 | Seq Scan | 40.91 | 48.88 | 19.5% | operator |
| 13968 | Seq Scan | 159.77 | 159.67 | 0.1% | operator |
| 13969 | Hash | 36.08 | 43.51 | 20.6% | pattern |
| 13974 | Seq Scan | 0.01 | 12.22 | 93938.0% | operator |
| 13975 | Hash | 0.06 | 18.14 | 29164.8% | operator |
| 13976 | Seq Scan | 0.06 | 23.36 | 39497.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13976 (Seq Scan) - LEAF
...
### Step 2: Node 13974 (Seq Scan) - LEAF
...
### Step 4: Node 13968 (Seq Scan) - LEAF
...
### Step 5: Node 13969 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13960, 13961, 13970, 13971, 13972, 13973
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13977 (Index Scan) - LEAF
...
### Step 8: Node 13979 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9709
  - startup_cost=9919.9900
  - total_cost=70271.4700
- **Output:** st=113.62, rt=785.60

### Step 12: Node 13980 (Index Scan) - LEAF
...
### Step 13: Node 13982 (Seq Scan) - LEAF
...
### Step 14: Node 13964 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.2700
  - total_cost=70504.4300
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 13981 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.8400
  - total_cost=70510.2800
- **Output:** st=113.62, rt=785.61

### Step 17: Node 13962 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70547.2000
  - total_cost=70549.1200
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 13960 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13961, 13969, 13970, 13971, 13972, 13973
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2387
  - Aggregate_nt1=2387
  ...
  - Aggregate_startup_cost=71547.2400
  - Aggregate_total_cost=71925.1100
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2387
  - GatherMerge_Outer_nt1=770
  ...
  - GatherMerge_Outer_startup_cost=71547.2400
  - GatherMerge_Outer_total_cost=71829.6300
