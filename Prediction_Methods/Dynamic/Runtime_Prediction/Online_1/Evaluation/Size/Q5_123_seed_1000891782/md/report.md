...
**Test Query:** Q5_123_seed_1000891782
**Timestamp:** 2026-01-18 16:48:05
...
```
Node 7401 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7402 (Aggregate) [consumed]
    Node 7403 (Gather Merge) [consumed]
      Node 7404 (Aggregate)
        Node 7405 (Sort) [PATTERN: 3e2d5a00]
          Node 7406 (Hash Join) [consumed]
            Node 7407 (Nested Loop)
              Node 7408 (Hash Join)
                Node 7409 (Seq Scan) - LEAF
                Node 7410 (Hash) [PATTERN: 444761fb]
                  Node 7411 (Hash Join) [consumed]
                    Node 7412 (Seq Scan) [consumed] - LEAF
                    Node 7413 (Hash) [consumed]
                      Node 7414 (Hash Join) [consumed]
                        Node 7415 (Seq Scan) - LEAF
                        Node 7416 (Hash)
                          Node 7417 (Seq Scan) - LEAF
              Node 7418 (Index Scan) - LEAF
            Node 7419 (Hash)
              Node 7420 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7410 | 7401, 7402, 7403, 7405, 7406, 7411, 7412, 7413, 7414 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7401 | 7402, 7403, 7405, 7406, 7410, 7411, 7412, 7413, 7414 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7405 | 7401, 7402, 7403, 7406, 7410, 7411, 7412, 7413, 7414 |
...
|------|------|--------|-----------|-----|--------|
| 7401 | Sort | 1062.32 | 2934.42 | 176.2% | pattern |
| 7404 | Aggregate | 1056.76 | 1128.74 | 6.8% | operator |
| 7405 | Sort | 1056.50 | 1835.96 | 73.8% | pattern |
| 7407 | Nested Loop | 1044.48 | 1188.57 | 13.8% | operator |
| 7419 | Hash | 3.48 | 18.67 | 435.9% | operator |
| 7408 | Hash Join | 212.87 | 253.07 | 18.9% | operator |
| 7418 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 7420 | Seq Scan | 2.96 | 12.36 | 317.4% | operator |
| 7409 | Seq Scan | 166.21 | 159.81 | 3.8% | operator |
| 7410 | Hash | 34.14 | 46.85 | 37.2% | pattern |
| 7415 | Seq Scan | 0.01 | 11.81 | 107247.9% | operator |
| 7416 | Hash | 0.06 | 21.05 | 36835.5% | operator |
| 7417 | Seq Scan | 0.06 | 22.93 | 41597.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7417 (Seq Scan) - LEAF
...
### Step 2: Node 7415 (Seq Scan) - LEAF
...
### Step 4: Node 7409 (Seq Scan) - LEAF
...
### Step 5: Node 7410 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7401, 7402, 7403, 7405, 7406, 7411, 7412, 7413, 7414
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
- **Output:** st=17.19, rt=253.07

### Step 7: Node 7418 (Index Scan) - LEAF
...
### Step 8: Node 7420 (Seq Scan) - LEAF
...
- **Output:** st=100.32, rt=1188.57

### Step 10: Node 7419 (Hash)
...
### Step 11: Node 7405 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7401, 7402, 7403, 7406, 7410, 7411, 7412, 7413, 7414
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=2382
  - HashJoin_Outer_nt1=59546
  ...
  - HashJoin_Outer_total_cost=56906.9000
  - Sort_np=0
  - Sort_nt=2382
  - Sort_nt1=2382
  - Sort_nt2=0
  ...
  - Sort_sel=1.0000
  - Sort_startup_cost=57040.5100
  - Sort_total_cost=57046.4600
- **Output:** st=1835.95, rt=1835.96

### Step 12: Node 7404 (Aggregate)
...
- **Output:** st=1065.11, rt=1128.74

### Step 13: Node 7401 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7402, 7403, 7405, 7406, 7410, 7411, 7412, 7413, 7414
- **Input Features:**
  ...
  - GatherMerge_Outer_startup_cost=58040.5500
  - GatherMerge_Outer_total_cost=58079.4500
  - Sort_np=0
  - Sort_nt=25
  - Sort_nt1=25
  - Sort_nt2=0
