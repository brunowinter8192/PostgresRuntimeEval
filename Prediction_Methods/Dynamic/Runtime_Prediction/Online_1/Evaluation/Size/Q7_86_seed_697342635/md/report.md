...
**Test Query:** Q7_86_seed_697342635
...
```
Node 12913 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12914 (Gather Merge) [consumed]
    Node 12915 (Sort)
      Node 12916 (Hash Join)
        Node 12917 (Nested Loop)
          Node 12918 (Hash Join) [PATTERN: 895c6e8c]
            Node 12919 (Seq Scan) [consumed] - LEAF
            Node 12920 (Hash) [consumed]
              Node 12921 (Hash Join) [PATTERN: f4cb205a]
                Node 12922 (Seq Scan) [consumed] - LEAF
                Node 12923 (Hash) [consumed]
                  Node 12924 (Seq Scan) [consumed] - LEAF
          Node 12925 (Index Scan) - LEAF
        Node 12926 (Hash)
          Node 12927 (Hash Join) [PATTERN: f4cb205a]
            Node 12928 (Seq Scan) [consumed] - LEAF
            Node 12929 (Hash) [consumed]
              Node 12930 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12921 | 12913, 12914, 12918, 12919, 12920, 12922, 12923, 12924, 12927, 12928, 12929, 12930 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12927 | 12913, 12914, 12918, 12919, 12920, 12921, 12922, 12923, 12924, 12928, 12929, 12930 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12913 | 12914, 12918, 12919, 12920, 12921, 12922, 12923, 12924, 12927, 12928, 12929, 12930 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12918 | 12913, 12914, 12919, 12920, 12921, 12922, 12923, 12924, 12927, 12928, 12929, 12930 |
...
|------|------|--------|-----------|-----|--------|
| 12913 | Aggregate | 1206.97 | 1163.13 | 3.6% | pattern |
| 12915 | Sort | 1199.22 | 1109.16 | 7.5% | operator |
| 12916 | Hash Join | 1198.62 | 843.11 | 29.7% | operator |
| 12917 | Nested Loop | 1191.94 | 1215.99 | 2.0% | operator |
| 12926 | Hash | 2.97 | 27.74 | 833.2% | operator |
| 12918 | Hash Join | 212.64 | 304.89 | 43.4% | pattern |
| 12925 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12927 | Hash Join | 2.92 | 429.36 | 14599.2% | pattern |
| 12921 | Hash Join | 36.73 | 429.38 | 1068.9% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12921 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12913, 12914, 12918, 12919, 12920, 12922, 12923, 12924, 12927, 12928, 12929, 12930
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12918 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12913, 12914, 12919, 12920, 12921, 12922, 12923, 12924, 12927, 12928, 12929, 12930
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12925 (Index Scan) - LEAF
...
### Step 4: Node 12927 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12913, 12914, 12918, 12919, 12920, 12921, 12922, 12923, 12924, 12928, 12929, 12930
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12913 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12914, 12918, 12919, 12920, 12921, 12922, 12923, 12924, 12927, 12928, 12929, 12930
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
