...
**Test Query:** Q8_149_seed_1214196588
...
```
Node 14420 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14421 (Gather Merge) [consumed]
    Node 14422 (Sort)
      Node 14423 (Hash Join)
        Node 14424 (Nested Loop)
          Node 14425 (Hash Join)
            Node 14426 (Nested Loop)
              Node 14427 (Hash Join)
                Node 14428 (Seq Scan) - LEAF
                Node 14429 (Hash) [PATTERN: 444761fb]
                  Node 14430 (Hash Join) [consumed]
                    Node 14431 (Seq Scan) [consumed] - LEAF
                    Node 14432 (Hash) [consumed]
                      Node 14433 (Hash Join) [consumed]
                        Node 14434 (Seq Scan) - LEAF
                        Node 14435 (Hash)
                          Node 14436 (Seq Scan) - LEAF
              Node 14437 (Index Scan) - LEAF
            Node 14438 (Hash)
              Node 14439 (Seq Scan) - LEAF
          Node 14440 (Index Scan) - LEAF
        Node 14441 (Hash)
          Node 14442 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14429 | 14420, 14421, 14430, 14431, 14432, 14433 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14420 | 14421, 14429, 14430, 14431, 14432, 14433 |
...
|------|------|--------|-----------|-----|--------|
| 14420 | Aggregate | 1262.49 | 1133.65 | 10.2% | pattern |
| 14422 | Sort | 1256.48 | 1046.21 | 16.7% | operator |
| 14423 | Hash Join | 1256.23 | 785.61 | 37.5% | operator |
| 14424 | Nested Loop | 1255.91 | 1198.29 | 4.6% | operator |
| 14441 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 14425 | Hash Join | 1251.88 | 785.60 | 37.2% | operator |
| 14440 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14442 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 14426 | Nested Loop | 1207.49 | 1198.29 | 0.8% | operator |
| 14438 | Hash | 39.93 | 19.97 | 50.0% | operator |
| 14427 | Hash Join | 217.63 | 251.67 | 15.6% | operator |
| 14437 | Index Scan | 0.04 | 2.88 | 6599.7% | operator |
| 14439 | Seq Scan | 39.84 | 48.88 | 22.7% | operator |
| 14428 | Seq Scan | 161.28 | 159.67 | 1.0% | operator |
| 14429 | Hash | 37.52 | 43.51 | 16.0% | pattern |
| 14434 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14435 | Hash | 0.21 | 18.14 | 8581.4% | operator |
| 14436 | Seq Scan | 0.21 | 23.36 | 11240.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14436 (Seq Scan) - LEAF
...
### Step 2: Node 14434 (Seq Scan) - LEAF
...
### Step 4: Node 14428 (Seq Scan) - LEAF
...
### Step 5: Node 14429 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14420, 14421, 14430, 14431, 14432, 14433
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14437 (Index Scan) - LEAF
...
### Step 8: Node 14439 (Seq Scan) - LEAF
...
### Step 12: Node 14440 (Index Scan) - LEAF
...
### Step 13: Node 14442 (Seq Scan) - LEAF
...
### Step 14: Node 14424 (Nested Loop)
...
### Step 18: Node 14420 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14421, 14429, 14430, 14431, 14432, 14433
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2376
  - Aggregate_nt1=2376
  ...
  - Aggregate_total_cost=71921.8800
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2376
  - GatherMerge_Outer_nt1=766
  ...
  - GatherMerge_Outer_total_cost=71826.8400
