...
**Test Query:** Q5_122_seed_992687751
**Timestamp:** 2026-01-18 16:48:06
...
```
Node 7381 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7382 (Aggregate) [consumed]
    Node 7383 (Gather Merge) [consumed]
      Node 7384 (Aggregate)
        Node 7385 (Sort) [PATTERN: 3e2d5a00]
          Node 7386 (Hash Join) [consumed]
            Node 7387 (Nested Loop)
              Node 7388 (Hash Join)
                Node 7389 (Seq Scan) - LEAF
                Node 7390 (Hash) [PATTERN: 444761fb]
                  Node 7391 (Hash Join) [consumed]
                    Node 7392 (Seq Scan) [consumed] - LEAF
                    Node 7393 (Hash) [consumed]
                      Node 7394 (Hash Join) [consumed]
                        Node 7395 (Seq Scan) - LEAF
                        Node 7396 (Hash)
                          Node 7397 (Seq Scan) - LEAF
              Node 7398 (Index Scan) - LEAF
            Node 7399 (Hash)
              Node 7400 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7390 | 7381, 7382, 7383, 7385, 7386, 7391, 7392, 7393, 7394 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7381 | 7382, 7383, 7385, 7386, 7390, 7391, 7392, 7393, 7394 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7385 | 7381, 7382, 7383, 7386, 7390, 7391, 7392, 7393, 7394 |
...
|------|------|--------|-----------|-----|--------|
| 7381 | Sort | 1065.91 | 2934.42 | 175.3% | pattern |
| 7384 | Aggregate | 1061.36 | 1128.78 | 6.4% | operator |
| 7385 | Sort | 1061.08 | 1835.91 | 73.0% | pattern |
| 7387 | Nested Loop | 1049.64 | 1188.39 | 13.2% | operator |
| 7399 | Hash | 2.92 | 18.67 | 540.5% | operator |
| 7388 | Hash Join | 216.86 | 253.07 | 16.7% | operator |
| 7398 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 7400 | Seq Scan | 2.42 | 12.36 | 411.6% | operator |
| 7389 | Seq Scan | 169.32 | 159.81 | 5.6% | operator |
| 7390 | Hash | 33.48 | 46.85 | 39.9% | pattern |
| 7395 | Seq Scan | 0.01 | 11.81 | 117982.7% | operator |
| 7396 | Hash | 0.06 | 21.05 | 37495.1% | operator |
| 7397 | Seq Scan | 0.05 | 22.93 | 43170.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7397 (Seq Scan) - LEAF
...
### Step 2: Node 7395 (Seq Scan) - LEAF
...
### Step 4: Node 7389 (Seq Scan) - LEAF
...
### Step 5: Node 7390 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7381, 7382, 7383, 7385, 7386, 7391, 7392, 7393, 7394
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7398 (Index Scan) - LEAF
...
### Step 8: Node 7400 (Seq Scan) - LEAF
...
### Step 11: Node 7385 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7381, 7382, 7383, 7386, 7390, 7391, 7392, 7393, 7394
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7381 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7382, 7383, 7385, 7386, 7390, 7391, 7392, 7393, 7394
- **Input Features:**
