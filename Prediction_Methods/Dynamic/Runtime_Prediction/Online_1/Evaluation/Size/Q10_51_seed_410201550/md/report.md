...
**Test Query:** Q10_51_seed_410201550
...
```
Node 20618 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20619 (Sort) [consumed]
    Node 20620 (Aggregate) [consumed]
      Node 20621 (Gather) [consumed]
        Node 20622 (Hash Join)
          Node 20623 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20624 (Nested Loop) [consumed]
              Node 20625 (Seq Scan) - LEAF
              Node 20626 (Index Scan) - LEAF
            Node 20627 (Hash) [consumed]
              Node 20628 (Seq Scan) - LEAF
          Node 20629 (Hash)
            Node 20630 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20618 | 20619, 20620, 20621, 20623, 20624, 20627 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20623 | 20618, 20619, 20620, 20621, 20624, 20627 |
...
|------|------|--------|-----------|-----|--------|
| 20618 | Limit | 1156.87 | 1199.47 | 3.7% | pattern |
| 20622 | Hash Join | 1105.32 | 1095.57 | 0.9% | operator |
| 20623 | Hash Join | 1102.47 | 1120.19 | 1.6% | pattern |
| 20629 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20630 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 20625 | Seq Scan | 163.12 | 159.74 | 2.1% | operator |
| 20626 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20628 | Seq Scan | 27.38 | 26.64 | 2.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20625 (Seq Scan) - LEAF
...
### Step 2: Node 20626 (Index Scan) - LEAF
...
### Step 3: Node 20628 (Seq Scan) - LEAF
...
### Step 4: Node 20630 (Seq Scan) - LEAF
...
### Step 5: Node 20623 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20618, 20619, 20620, 20621, 20624, 20627
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18240
  - HashJoin_nt1=18240
...
### Step 8: Node 20618 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20619, 20620, 20621, 20623, 20624, 20627
- **Input Features:**
