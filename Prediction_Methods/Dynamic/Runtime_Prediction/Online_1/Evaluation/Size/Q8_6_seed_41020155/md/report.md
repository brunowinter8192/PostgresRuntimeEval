...
**Test Query:** Q8_6_seed_41020155
**Timestamp:** 2026-01-18 18:40:53
...
```
Node 15869 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15870 (Gather Merge) [consumed]
    Node 15871 (Sort)
      Node 15872 (Hash Join)
        Node 15873 (Nested Loop)
          Node 15874 (Hash Join)
            Node 15875 (Nested Loop)
              Node 15876 (Hash Join)
                Node 15877 (Seq Scan) - LEAF
                Node 15878 (Hash) [PATTERN: 444761fb]
                  Node 15879 (Hash Join) [consumed]
                    Node 15880 (Seq Scan) [consumed] - LEAF
                    Node 15881 (Hash) [consumed]
                      Node 15882 (Hash Join) [consumed]
                        Node 15883 (Seq Scan) - LEAF
                        Node 15884 (Hash)
                          Node 15885 (Seq Scan) - LEAF
              Node 15886 (Index Scan) - LEAF
            Node 15887 (Hash)
              Node 15888 (Seq Scan) - LEAF
          Node 15889 (Index Scan) - LEAF
        Node 15890 (Hash)
          Node 15891 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15878 | 15869, 15870, 15879, 15880, 15881, 15882 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15869 | 15870, 15878, 15879, 15880, 15881, 15882 |
...
|------|------|--------|-----------|-----|--------|
| 15869 | Aggregate | 1232.07 | 1133.68 | 8.0% | pattern |
| 15871 | Sort | 1227.27 | 1046.21 | 14.8% | operator |
| 15872 | Hash Join | 1227.05 | 785.61 | 36.0% | operator |
| 15873 | Nested Loop | 1226.76 | 1198.29 | 2.3% | operator |
| 15890 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 15874 | Hash Join | 1223.24 | 785.60 | 35.8% | operator |
| 15889 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15891 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 15875 | Nested Loop | 1175.64 | 1198.29 | 1.9% | operator |
| 15887 | Hash | 42.82 | 19.97 | 53.4% | operator |
| 15876 | Hash Join | 214.49 | 251.67 | 17.3% | operator |
| 15886 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 15888 | Seq Scan | 42.71 | 48.89 | 14.5% | operator |
| 15877 | Seq Scan | 159.47 | 159.67 | 0.1% | operator |
| 15878 | Hash | 36.21 | 43.51 | 20.2% | pattern |
| 15883 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 15884 | Hash | 0.14 | 18.14 | 12500.1% | operator |
| 15885 | Seq Scan | 0.14 | 23.36 | 16707.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15885 (Seq Scan) - LEAF
...
### Step 2: Node 15883 (Seq Scan) - LEAF
...
### Step 4: Node 15877 (Seq Scan) - LEAF
...
### Step 5: Node 15878 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15869, 15870, 15879, 15880, 15881, 15882
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15886 (Index Scan) - LEAF
...
### Step 8: Node 15888 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9721
  - startup_cost=9919.8000
  - total_cost=70271.2800
- **Output:** st=113.62, rt=785.60

### Step 12: Node 15889 (Index Scan) - LEAF
...
### Step 13: Node 15891 (Seq Scan) - LEAF
...
### Step 14: Node 15873 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70497.8900
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15890 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.6500
  - total_cost=70503.6200
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15871 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70539.3800
  - total_cost=70541.2500
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 15869 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15870, 15878, 15879, 15880, 15881, 15882
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2321
  - Aggregate_nt1=2321
  ...
  - Aggregate_startup_cost=71539.4200
  - Aggregate_total_cost=71906.8500
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2321
  - GatherMerge_Outer_nt1=749
  ...
  - GatherMerge_Outer_startup_cost=71539.4200
  - GatherMerge_Outer_total_cost=71814.0100
