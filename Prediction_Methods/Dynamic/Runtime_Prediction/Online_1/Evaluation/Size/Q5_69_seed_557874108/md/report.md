...
**Test Query:** Q5_69_seed_557874108
...
```
Node 9201 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9202 (Aggregate) [consumed]
    Node 9203 (Gather Merge) [consumed]
      Node 9204 (Aggregate)
        Node 9205 (Sort) [PATTERN: 3e2d5a00]
          Node 9206 (Hash Join) [consumed]
            Node 9207 (Nested Loop)
              Node 9208 (Hash Join)
                Node 9209 (Seq Scan) - LEAF
                Node 9210 (Hash) [PATTERN: 444761fb]
                  Node 9211 (Hash Join) [consumed]
                    Node 9212 (Seq Scan) [consumed] - LEAF
                    Node 9213 (Hash) [consumed]
                      Node 9214 (Hash Join) [consumed]
                        Node 9215 (Seq Scan) - LEAF
                        Node 9216 (Hash)
                          Node 9217 (Seq Scan) - LEAF
              Node 9218 (Index Scan) - LEAF
            Node 9219 (Hash)
              Node 9220 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9210 | 9201, 9202, 9203, 9205, 9206, 9211, 9212, 9213, 9214 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9201 | 9202, 9203, 9205, 9206, 9210, 9211, 9212, 9213, 9214 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9205 | 9201, 9202, 9203, 9206, 9210, 9211, 9212, 9213, 9214 |
...
|------|------|--------|-----------|-----|--------|
| 9201 | Sort | 1052.52 | 2934.42 | 178.8% | pattern |
| 9204 | Aggregate | 1045.46 | 1128.78 | 8.0% | operator |
| 9205 | Sort | 1045.22 | 1835.91 | 75.6% | pattern |
| 9207 | Nested Loop | 1033.70 | 1188.39 | 15.0% | operator |
| 9219 | Hash | 2.77 | 18.67 | 573.3% | operator |
| 9208 | Hash Join | 210.55 | 253.07 | 20.2% | operator |
| 9218 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 9220 | Seq Scan | 2.25 | 12.36 | 448.3% | operator |
| 9209 | Seq Scan | 165.72 | 159.81 | 3.6% | operator |
| 9210 | Hash | 31.43 | 46.85 | 49.0% | pattern |
| 9215 | Seq Scan | 0.01 | 11.81 | 147503.4% | operator |
| 9216 | Hash | 0.05 | 21.05 | 38887.5% | operator |
| 9217 | Seq Scan | 0.05 | 22.93 | 44867.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9217 (Seq Scan) - LEAF
...
### Step 2: Node 9215 (Seq Scan) - LEAF
...
### Step 4: Node 9209 (Seq Scan) - LEAF
...
### Step 5: Node 9210 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9201, 9202, 9203, 9205, 9206, 9211, 9212, 9213, 9214
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9218 (Index Scan) - LEAF
...
### Step 8: Node 9220 (Seq Scan) - LEAF
...
### Step 11: Node 9205 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9201, 9202, 9203, 9206, 9210, 9211, 9212, 9213, 9214
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9201 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9202, 9203, 9205, 9206, 9210, 9211, 9212, 9213, 9214
- **Input Features:**
