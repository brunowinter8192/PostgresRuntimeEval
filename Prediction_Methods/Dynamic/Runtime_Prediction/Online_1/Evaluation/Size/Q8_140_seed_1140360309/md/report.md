...
**Test Query:** Q8_140_seed_1140360309
...
```
Node 14213 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14214 (Gather Merge) [consumed]
    Node 14215 (Sort)
      Node 14216 (Hash Join)
        Node 14217 (Nested Loop)
          Node 14218 (Hash Join)
            Node 14219 (Nested Loop)
              Node 14220 (Hash Join)
                Node 14221 (Seq Scan) - LEAF
                Node 14222 (Hash) [PATTERN: 444761fb]
                  Node 14223 (Hash Join) [consumed]
                    Node 14224 (Seq Scan) [consumed] - LEAF
                    Node 14225 (Hash) [consumed]
                      Node 14226 (Hash Join) [consumed]
                        Node 14227 (Seq Scan) - LEAF
                        Node 14228 (Hash)
                          Node 14229 (Seq Scan) - LEAF
              Node 14230 (Index Scan) - LEAF
            Node 14231 (Hash)
              Node 14232 (Seq Scan) - LEAF
          Node 14233 (Index Scan) - LEAF
        Node 14234 (Hash)
          Node 14235 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14222 | 14213, 14214, 14223, 14224, 14225, 14226 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14213 | 14214, 14222, 14223, 14224, 14225, 14226 |
...
|------|------|--------|-----------|-----|--------|
| 14213 | Aggregate | 1256.69 | 1132.38 | 9.9% | pattern |
| 14215 | Sort | 1249.85 | 1046.21 | 16.3% | operator |
| 14216 | Hash Join | 1249.60 | 785.61 | 37.1% | operator |
| 14217 | Nested Loop | 1249.29 | 1198.29 | 4.1% | operator |
| 14234 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 14218 | Hash Join | 1245.35 | 785.60 | 36.9% | operator |
| 14233 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14235 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 14219 | Nested Loop | 1200.93 | 1198.29 | 0.2% | operator |
| 14231 | Hash | 39.91 | 19.97 | 50.0% | operator |
| 14220 | Hash Join | 216.60 | 251.67 | 16.2% | operator |
| 14230 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14232 | Seq Scan | 39.85 | 48.87 | 22.6% | operator |
| 14221 | Seq Scan | 158.52 | 159.67 | 0.7% | operator |
| 14222 | Hash | 38.68 | 43.51 | 12.5% | pattern |
| 14227 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14228 | Hash | 0.27 | 18.14 | 6546.2% | operator |
| 14229 | Seq Scan | 0.27 | 23.36 | 8552.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14229 (Seq Scan) - LEAF
...
### Step 2: Node 14227 (Seq Scan) - LEAF
...
### Step 4: Node 14221 (Seq Scan) - LEAF
...
### Step 5: Node 14222 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14213, 14214, 14223, 14224, 14225, 14226
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14230 (Index Scan) - LEAF
...
### Step 8: Node 14232 (Seq Scan) - LEAF
...
### Step 12: Node 14233 (Index Scan) - LEAF
...
### Step 13: Node 14235 (Seq Scan) - LEAF
...
### Step 14: Node 14217 (Nested Loop)
...
### Step 18: Node 14213 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14214, 14222, 14223, 14224, 14225, 14226
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2431
