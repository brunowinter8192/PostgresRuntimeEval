...
**Test Query:** Q5_40_seed_319957209
...
```
Node 8581 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8582 (Aggregate) [consumed]
    Node 8583 (Gather Merge) [consumed]
      Node 8584 (Aggregate)
        Node 8585 (Sort) [PATTERN: 3e2d5a00]
          Node 8586 (Hash Join) [consumed]
            Node 8587 (Nested Loop)
              Node 8588 (Hash Join)
                Node 8589 (Seq Scan) - LEAF
                Node 8590 (Hash) [PATTERN: 444761fb]
                  Node 8591 (Hash Join) [consumed]
                    Node 8592 (Seq Scan) [consumed] - LEAF
                    Node 8593 (Hash) [consumed]
                      Node 8594 (Hash Join) [consumed]
                        Node 8595 (Seq Scan) - LEAF
                        Node 8596 (Hash)
                          Node 8597 (Seq Scan) - LEAF
              Node 8598 (Index Scan) - LEAF
            Node 8599 (Hash)
              Node 8600 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8590 | 8581, 8582, 8583, 8585, 8586, 8591, 8592, 8593, 8594 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8581 | 8582, 8583, 8585, 8586, 8590, 8591, 8592, 8593, 8594 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8585 | 8581, 8582, 8583, 8586, 8590, 8591, 8592, 8593, 8594 |
...
|------|------|--------|-----------|-----|--------|
| 8581 | Sort | 1118.20 | 2934.42 | 162.4% | pattern |
| 8584 | Aggregate | 1111.43 | 1128.78 | 1.6% | operator |
| 8585 | Sort | 1111.18 | 1835.92 | 65.2% | pattern |
| 8587 | Nested Loop | 1099.68 | 1188.42 | 8.1% | operator |
| 8599 | Hash | 1.96 | 18.67 | 853.1% | operator |
| 8588 | Hash Join | 226.42 | 253.07 | 11.8% | operator |
| 8598 | Index Scan | 0.08 | -0.22 | 385.7% | operator |
| 8600 | Seq Scan | 1.44 | 12.36 | 761.3% | operator |
| 8589 | Seq Scan | 173.40 | 159.81 | 7.8% | operator |
| 8590 | Hash | 39.33 | 46.85 | 19.1% | pattern |
| 8595 | Seq Scan | 0.01 | 11.81 | 98302.3% | operator |
| 8596 | Hash | 0.06 | 21.05 | 35583.5% | operator |
| 8597 | Seq Scan | 0.06 | 22.93 | 40133.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8597 (Seq Scan) - LEAF
...
### Step 2: Node 8595 (Seq Scan) - LEAF
...
### Step 4: Node 8589 (Seq Scan) - LEAF
...
### Step 5: Node 8590 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8581, 8582, 8583, 8585, 8586, 8591, 8592, 8593, 8594
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8598 (Index Scan) - LEAF
...
### Step 8: Node 8600 (Seq Scan) - LEAF
...
### Step 11: Node 8585 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8581, 8582, 8583, 8586, 8590, 8591, 8592, 8593, 8594
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8581 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8582, 8583, 8585, 8586, 8590, 8591, 8592, 8593, 8594
- **Input Features:**
