...
**Test Query:** Q10_41_seed_328161240
...
```
Node 20475 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20476 (Sort) [consumed]
    Node 20477 (Aggregate) [consumed]
      Node 20478 (Gather) [consumed]
        Node 20479 (Hash Join)
          Node 20480 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20481 (Nested Loop) [consumed]
              Node 20482 (Seq Scan) - LEAF
              Node 20483 (Index Scan) - LEAF
            Node 20484 (Hash) [consumed]
              Node 20485 (Seq Scan) - LEAF
          Node 20486 (Hash)
            Node 20487 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20475 | 20476, 20477, 20478, 20480, 20481, 20484 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20480 | 20475, 20476, 20477, 20478, 20481, 20484 |
...
|------|------|--------|-----------|-----|--------|
| 20475 | Limit | 1164.28 | 1199.47 | 3.0% | pattern |
| 20479 | Hash Join | 1113.00 | 1095.55 | 1.6% | operator |
| 20480 | Hash Join | 1110.08 | 1120.19 | 0.9% | pattern |
| 20486 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20487 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 20482 | Seq Scan | 160.80 | 159.74 | 0.7% | operator |
| 20483 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20485 | Seq Scan | 27.20 | 26.64 | 2.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20482 (Seq Scan) - LEAF
...
### Step 2: Node 20483 (Index Scan) - LEAF
...
### Step 3: Node 20485 (Seq Scan) - LEAF
...
### Step 4: Node 20487 (Seq Scan) - LEAF
...
### Step 5: Node 20480 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20475, 20476, 20477, 20478, 20481, 20484
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18184
  - HashJoin_nt1=18184
...
### Step 8: Node 20475 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20476, 20477, 20478, 20480, 20481, 20484
- **Input Features:**
