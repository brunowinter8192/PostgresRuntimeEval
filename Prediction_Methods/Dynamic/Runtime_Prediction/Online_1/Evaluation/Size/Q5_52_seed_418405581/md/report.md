...
**Test Query:** Q5_52_seed_418405581
...
```
Node 8841 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8842 (Aggregate) [consumed]
    Node 8843 (Gather Merge) [consumed]
      Node 8844 (Aggregate)
        Node 8845 (Sort) [PATTERN: 3e2d5a00]
          Node 8846 (Hash Join) [consumed]
            Node 8847 (Nested Loop)
              Node 8848 (Hash Join)
                Node 8849 (Seq Scan) - LEAF
                Node 8850 (Hash) [PATTERN: 444761fb]
                  Node 8851 (Hash Join) [consumed]
                    Node 8852 (Seq Scan) [consumed] - LEAF
                    Node 8853 (Hash) [consumed]
                      Node 8854 (Hash Join) [consumed]
                        Node 8855 (Seq Scan) - LEAF
                        Node 8856 (Hash)
                          Node 8857 (Seq Scan) - LEAF
              Node 8858 (Index Scan) - LEAF
            Node 8859 (Hash)
              Node 8860 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8850 | 8841, 8842, 8843, 8845, 8846, 8851, 8852, 8853, 8854 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8841 | 8842, 8843, 8845, 8846, 8850, 8851, 8852, 8853, 8854 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8845 | 8841, 8842, 8843, 8846, 8850, 8851, 8852, 8853, 8854 |
...
|------|------|--------|-----------|-----|--------|
| 8841 | Sort | 1086.64 | 2934.42 | 170.0% | pattern |
| 8844 | Aggregate | 1081.94 | 1128.76 | 4.3% | operator |
| 8845 | Sort | 1081.67 | 1835.93 | 69.7% | pattern |
| 8847 | Nested Loop | 1068.46 | 1188.47 | 11.2% | operator |
| 8859 | Hash | 4.56 | 18.67 | 309.3% | operator |
| 8848 | Hash Join | 218.71 | 253.07 | 15.7% | operator |
| 8858 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 8860 | Seq Scan | 3.97 | 12.36 | 211.2% | operator |
| 8849 | Seq Scan | 170.06 | 159.81 | 6.0% | operator |
| 8850 | Hash | 35.19 | 46.85 | 33.2% | pattern |
| 8855 | Seq Scan | 0.01 | 11.81 | 117982.7% | operator |
| 8856 | Hash | 0.19 | 21.05 | 11218.9% | operator |
| 8857 | Seq Scan | 0.18 | 22.93 | 12363.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8857 (Seq Scan) - LEAF
...
### Step 2: Node 8855 (Seq Scan) - LEAF
...
### Step 4: Node 8849 (Seq Scan) - LEAF
...
### Step 5: Node 8850 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8841, 8842, 8843, 8845, 8846, 8851, 8852, 8853, 8854
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8858 (Index Scan) - LEAF
...
### Step 8: Node 8860 (Seq Scan) - LEAF
...
### Step 11: Node 8845 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8841, 8842, 8843, 8846, 8850, 8851, 8852, 8853, 8854
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8841 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8842, 8843, 8845, 8846, 8850, 8851, 8852, 8853, 8854
- **Input Features:**
