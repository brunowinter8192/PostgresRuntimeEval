...
**Test Query:** Q8_63_seed_508649922
...
```
Node 15708 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15709 (Gather Merge) [consumed]
    Node 15710 (Sort)
      Node 15711 (Hash Join)
        Node 15712 (Nested Loop)
          Node 15713 (Hash Join)
            Node 15714 (Nested Loop)
              Node 15715 (Hash Join)
                Node 15716 (Seq Scan) - LEAF
                Node 15717 (Hash) [PATTERN: 444761fb]
                  Node 15718 (Hash Join) [consumed]
                    Node 15719 (Seq Scan) [consumed] - LEAF
                    Node 15720 (Hash) [consumed]
                      Node 15721 (Hash Join) [consumed]
                        Node 15722 (Seq Scan) - LEAF
                        Node 15723 (Hash)
                          Node 15724 (Seq Scan) - LEAF
              Node 15725 (Index Scan) - LEAF
            Node 15726 (Hash)
              Node 15727 (Seq Scan) - LEAF
          Node 15728 (Index Scan) - LEAF
        Node 15729 (Hash)
          Node 15730 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15717 | 15708, 15709, 15718, 15719, 15720, 15721 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15708 | 15709, 15717, 15718, 15719, 15720, 15721 |
...
|------|------|--------|-----------|-----|--------|
| 15708 | Aggregate | 1214.89 | 1133.29 | 6.7% | pattern |
| 15710 | Sort | 1209.59 | 1046.21 | 13.5% | operator |
| 15711 | Hash Join | 1209.36 | 785.61 | 35.0% | operator |
| 15712 | Nested Loop | 1209.03 | 1198.29 | 0.9% | operator |
| 15729 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 15713 | Hash Join | 1205.37 | 785.60 | 34.8% | operator |
| 15728 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15730 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 15714 | Nested Loop | 1158.45 | 1198.29 | 3.4% | operator |
| 15726 | Hash | 40.74 | 19.97 | 51.0% | operator |
| 15715 | Hash Join | 213.31 | 251.67 | 18.0% | operator |
| 15725 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15727 | Seq Scan | 40.63 | 48.88 | 20.3% | operator |
| 15716 | Seq Scan | 158.25 | 159.67 | 0.9% | operator |
| 15717 | Hash | 35.94 | 43.51 | 21.1% | pattern |
| 15722 | Seq Scan | 0.05 | 12.22 | 26476.0% | operator |
| 15723 | Hash | 0.16 | 18.14 | 11530.9% | operator |
| 15724 | Seq Scan | 0.15 | 23.36 | 15269.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15724 (Seq Scan) - LEAF
...
### Step 2: Node 15722 (Seq Scan) - LEAF
...
### Step 4: Node 15716 (Seq Scan) - LEAF
...
### Step 5: Node 15717 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15708, 15709, 15718, 15719, 15720, 15721
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15725 (Index Scan) - LEAF
...
### Step 8: Node 15727 (Seq Scan) - LEAF
...
### Step 12: Node 15728 (Index Scan) - LEAF
...
### Step 13: Node 15730 (Seq Scan) - LEAF
...
### Step 14: Node 15712 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70506.9200
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15729 (Hash)
...
- **Input Features:**
  ...
  - total_cost=70512.8200
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15710 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70550.1700
  - total_cost=70552.1200
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 15708 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15709, 15717, 15718, 15719, 15720, 15721
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2413
  ...
  - Aggregate_sel=0.9971
  - Aggregate_startup_cost=71550.2100
  - Aggregate_total_cost=71932.0700
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2413
  - GatherMerge_Outer_nt1=778
  ...
  - GatherMerge_Outer_startup_cost=71550.2100
  - GatherMerge_Outer_total_cost=71835.6900
- **Output:** st=1130.30, rt=1133.29
