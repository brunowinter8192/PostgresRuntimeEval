...
**Test Query:** Q5_58_seed_467629767
...
```
Node 8961 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8962 (Aggregate) [consumed]
    Node 8963 (Gather Merge) [consumed]
      Node 8964 (Aggregate)
        Node 8965 (Sort) [PATTERN: 3e2d5a00]
          Node 8966 (Hash Join) [consumed]
            Node 8967 (Nested Loop)
              Node 8968 (Hash Join)
                Node 8969 (Seq Scan) - LEAF
                Node 8970 (Hash) [PATTERN: 444761fb]
                  Node 8971 (Hash Join) [consumed]
                    Node 8972 (Seq Scan) [consumed] - LEAF
                    Node 8973 (Hash) [consumed]
                      Node 8974 (Hash Join) [consumed]
                        Node 8975 (Seq Scan) - LEAF
                        Node 8976 (Hash)
                          Node 8977 (Seq Scan) - LEAF
              Node 8978 (Index Scan) - LEAF
            Node 8979 (Hash)
              Node 8980 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8970 | 8961, 8962, 8963, 8965, 8966, 8971, 8972, 8973, 8974 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8961 | 8962, 8963, 8965, 8966, 8970, 8971, 8972, 8973, 8974 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8965 | 8961, 8962, 8963, 8966, 8970, 8971, 8972, 8973, 8974 |
...
|------|------|--------|-----------|-----|--------|
| 8961 | Sort | 1081.05 | 2934.42 | 171.4% | pattern |
| 8964 | Aggregate | 1076.41 | 1128.76 | 4.9% | operator |
| 8965 | Sort | 1076.16 | 1835.93 | 70.6% | pattern |
| 8967 | Nested Loop | 1064.31 | 1188.47 | 11.7% | operator |
| 8979 | Hash | 3.13 | 18.67 | 496.1% | operator |
| 8968 | Hash Join | 220.36 | 253.07 | 14.8% | operator |
| 8978 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 8980 | Seq Scan | 2.61 | 12.36 | 374.1% | operator |
| 8969 | Seq Scan | 169.37 | 159.81 | 5.6% | operator |
| 8970 | Hash | 37.47 | 46.85 | 25.0% | pattern |
| 8975 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 8976 | Hash | 0.06 | 21.05 | 33317.8% | operator |
| 8977 | Seq Scan | 0.06 | 22.93 | 38122.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8977 (Seq Scan) - LEAF
...
### Step 2: Node 8975 (Seq Scan) - LEAF
...
### Step 4: Node 8969 (Seq Scan) - LEAF
...
### Step 5: Node 8970 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8961, 8962, 8963, 8965, 8966, 8971, 8972, 8973, 8974
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8978 (Index Scan) - LEAF
...
### Step 8: Node 8980 (Seq Scan) - LEAF
...
### Step 11: Node 8965 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8961, 8962, 8963, 8966, 8970, 8971, 8972, 8973, 8974
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8961 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8962, 8963, 8965, 8966, 8970, 8971, 8972, 8973, 8974
- **Input Features:**
