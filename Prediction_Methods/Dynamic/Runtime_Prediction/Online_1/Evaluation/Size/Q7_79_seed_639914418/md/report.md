...
**Test Query:** Q7_79_seed_639914418
...
```
Node 12769 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12770 (Gather Merge) [consumed]
    Node 12771 (Sort)
      Node 12772 (Hash Join)
        Node 12773 (Nested Loop)
          Node 12774 (Hash Join) [PATTERN: 895c6e8c]
            Node 12775 (Seq Scan) [consumed] - LEAF
            Node 12776 (Hash) [consumed]
              Node 12777 (Hash Join) [PATTERN: f4cb205a]
                Node 12778 (Seq Scan) [consumed] - LEAF
                Node 12779 (Hash) [consumed]
                  Node 12780 (Seq Scan) [consumed] - LEAF
          Node 12781 (Index Scan) - LEAF
        Node 12782 (Hash)
          Node 12783 (Hash Join) [PATTERN: f4cb205a]
            Node 12784 (Seq Scan) [consumed] - LEAF
            Node 12785 (Hash) [consumed]
              Node 12786 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12777 | 12769, 12770, 12774, 12775, 12776, 12778, 12779, 12780, 12783, 12784, 12785, 12786 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12783 | 12769, 12770, 12774, 12775, 12776, 12777, 12778, 12779, 12780, 12784, 12785, 12786 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12769 | 12770, 12774, 12775, 12776, 12777, 12778, 12779, 12780, 12783, 12784, 12785, 12786 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12774 | 12769, 12770, 12775, 12776, 12777, 12778, 12779, 12780, 12783, 12784, 12785, 12786 |
...
|------|------|--------|-----------|-----|--------|
| 12769 | Aggregate | 1210.95 | 1163.13 | 3.9% | pattern |
| 12771 | Sort | 1203.96 | 1109.16 | 7.9% | operator |
| 12772 | Hash Join | 1203.37 | 843.11 | 29.9% | operator |
| 12773 | Nested Loop | 1195.59 | 1215.99 | 1.7% | operator |
| 12782 | Hash | 3.85 | 27.74 | 621.0% | operator |
| 12774 | Hash Join | 213.41 | 304.89 | 42.9% | pattern |
| 12781 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12783 | Hash Join | 3.81 | 429.36 | 11178.2% | pattern |
| 12777 | Hash Join | 34.98 | 429.38 | 1127.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12777 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12769, 12770, 12774, 12775, 12776, 12778, 12779, 12780, 12783, 12784, 12785, 12786
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12774 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12769, 12770, 12775, 12776, 12777, 12778, 12779, 12780, 12783, 12784, 12785, 12786
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12781 (Index Scan) - LEAF
...
### Step 4: Node 12783 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12769, 12770, 12774, 12775, 12776, 12777, 12778, 12779, 12780, 12784, 12785, 12786
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12769 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12770, 12774, 12775, 12776, 12777, 12778, 12779, 12780, 12783, 12784, 12785, 12786
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
