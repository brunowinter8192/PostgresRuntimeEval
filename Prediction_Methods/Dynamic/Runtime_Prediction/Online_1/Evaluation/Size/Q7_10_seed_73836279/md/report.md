...
**Test Query:** Q7_10_seed_73836279
...
```
Node 10681 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10682 (Gather Merge) [consumed]
    Node 10683 (Sort)
      Node 10684 (Hash Join)
        Node 10685 (Nested Loop)
          Node 10686 (Hash Join) [PATTERN: 895c6e8c]
            Node 10687 (Seq Scan) [consumed] - LEAF
            Node 10688 (Hash) [consumed]
              Node 10689 (Hash Join) [PATTERN: f4cb205a]
                Node 10690 (Seq Scan) [consumed] - LEAF
                Node 10691 (Hash) [consumed]
                  Node 10692 (Seq Scan) [consumed] - LEAF
          Node 10693 (Index Scan) - LEAF
        Node 10694 (Hash)
          Node 10695 (Hash Join) [PATTERN: f4cb205a]
            Node 10696 (Seq Scan) [consumed] - LEAF
            Node 10697 (Hash) [consumed]
              Node 10698 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10689 | 10681, 10682, 10686, 10687, 10688, 10690, 10691, 10692, 10695, 10696, 10697, 10698 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10695 | 10681, 10682, 10686, 10687, 10688, 10689, 10690, 10691, 10692, 10696, 10697, 10698 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10681 | 10682, 10686, 10687, 10688, 10689, 10690, 10691, 10692, 10695, 10696, 10697, 10698 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10686 | 10681, 10682, 10687, 10688, 10689, 10690, 10691, 10692, 10695, 10696, 10697, 10698 |
...
|------|------|--------|-----------|-----|--------|
| 10681 | Aggregate | 1194.39 | 1163.13 | 2.6% | pattern |
| 10683 | Sort | 1185.75 | 1109.16 | 6.5% | operator |
| 10684 | Hash Join | 1185.16 | 843.11 | 28.9% | operator |
| 10685 | Nested Loop | 1177.67 | 1215.99 | 3.3% | operator |
| 10694 | Hash | 3.81 | 27.74 | 627.6% | operator |
| 10686 | Hash Join | 215.60 | 304.89 | 41.4% | pattern |
| 10693 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10695 | Hash Join | 3.77 | 429.36 | 11285.9% | pattern |
| 10689 | Hash Join | 36.63 | 429.38 | 1072.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10689 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10681, 10682, 10686, 10687, 10688, 10690, 10691, 10692, 10695, 10696, 10697, 10698
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10686 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10681, 10682, 10687, 10688, 10689, 10690, 10691, 10692, 10695, 10696, 10697, 10698
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10693 (Index Scan) - LEAF
...
### Step 4: Node 10695 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10681, 10682, 10686, 10687, 10688, 10689, 10690, 10691, 10692, 10696, 10697, 10698
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10681 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10682, 10686, 10687, 10688, 10689, 10690, 10691, 10692, 10695, 10696, 10697, 10698
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
