...
**Test Query:** Q8_24_seed_188692713
...
```
Node 14719 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14720 (Gather Merge) [consumed]
    Node 14721 (Sort)
      Node 14722 (Hash Join)
        Node 14723 (Nested Loop)
          Node 14724 (Hash Join)
            Node 14725 (Nested Loop)
              Node 14726 (Hash Join)
                Node 14727 (Seq Scan) - LEAF
                Node 14728 (Hash) [PATTERN: 444761fb]
                  Node 14729 (Hash Join) [consumed]
                    Node 14730 (Seq Scan) [consumed] - LEAF
                    Node 14731 (Hash) [consumed]
                      Node 14732 (Hash Join) [consumed]
                        Node 14733 (Seq Scan) - LEAF
                        Node 14734 (Hash)
                          Node 14735 (Seq Scan) - LEAF
              Node 14736 (Index Scan) - LEAF
            Node 14737 (Hash)
              Node 14738 (Seq Scan) - LEAF
          Node 14739 (Index Scan) - LEAF
        Node 14740 (Hash)
          Node 14741 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14728 | 14719, 14720, 14729, 14730, 14731, 14732 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14719 | 14720, 14728, 14729, 14730, 14731, 14732 |
...
|------|------|--------|-----------|-----|--------|
| 14719 | Aggregate | 1245.20 | 1133.68 | 9.0% | pattern |
| 14721 | Sort | 1240.01 | 1046.21 | 15.6% | operator |
| 14722 | Hash Join | 1239.76 | 785.61 | 36.6% | operator |
| 14723 | Nested Loop | 1239.44 | 1198.29 | 3.3% | operator |
| 14740 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 14724 | Hash Join | 1235.52 | 785.60 | 36.4% | operator |
| 14739 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14741 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 14725 | Nested Loop | 1191.89 | 1198.29 | 0.5% | operator |
| 14737 | Hash | 39.16 | 19.97 | 49.0% | operator |
| 14726 | Hash Join | 218.74 | 251.67 | 15.1% | operator |
| 14736 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14738 | Seq Scan | 39.05 | 48.88 | 25.2% | operator |
| 14727 | Seq Scan | 165.73 | 159.67 | 3.7% | operator |
| 14728 | Hash | 34.85 | 43.51 | 24.8% | pattern |
| 14733 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 14734 | Hash | 0.23 | 18.14 | 7653.9% | operator |
| 14735 | Seq Scan | 0.23 | 23.36 | 10057.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14735 (Seq Scan) - LEAF
...
### Step 2: Node 14733 (Seq Scan) - LEAF
...
### Step 4: Node 14727 (Seq Scan) - LEAF
...
### Step 5: Node 14728 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14719, 14720, 14729, 14730, 14731, 14732
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14736 (Index Scan) - LEAF
...
### Step 8: Node 14738 (Seq Scan) - LEAF
...
### Step 12: Node 14739 (Index Scan) - LEAF
...
### Step 13: Node 14741 (Seq Scan) - LEAF
...
### Step 14: Node 14723 (Nested Loop)
...
### Step 18: Node 14719 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14720, 14728, 14729, 14730, 14731, 14732
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2383
  - Aggregate_nt1=2383
