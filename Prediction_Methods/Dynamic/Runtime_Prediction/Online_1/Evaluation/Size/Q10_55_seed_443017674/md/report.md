...
**Test Query:** Q10_55_seed_443017674
...
```
Node 20670 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20671 (Sort) [consumed]
    Node 20672 (Aggregate) [consumed]
      Node 20673 (Gather) [consumed]
        Node 20674 (Hash Join)
          Node 20675 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20676 (Nested Loop) [consumed]
              Node 20677 (Seq Scan) - LEAF
              Node 20678 (Index Scan) - LEAF
            Node 20679 (Hash) [consumed]
              Node 20680 (Seq Scan) - LEAF
          Node 20681 (Hash)
            Node 20682 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20670 | 20671, 20672, 20673, 20675, 20676, 20679 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20675 | 20670, 20671, 20672, 20673, 20676, 20679 |
...
|------|------|--------|-----------|-----|--------|
| 20670 | Limit | 1143.76 | 1199.47 | 4.9% | pattern |
| 20674 | Hash Join | 1100.94 | 1095.59 | 0.5% | operator |
| 20675 | Hash Join | 1098.04 | 1120.19 | 2.0% | pattern |
| 20681 | Hash | 0.02 | 15.71 | 74713.2% | operator |
| 20682 | Seq Scan | 0.01 | 6.98 | 46424.7% | operator |
| 20677 | Seq Scan | 161.80 | 159.74 | 1.3% | operator |
| 20678 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20680 | Seq Scan | 27.77 | 26.64 | 4.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20677 (Seq Scan) - LEAF
...
### Step 2: Node 20678 (Index Scan) - LEAF
...
### Step 3: Node 20680 (Seq Scan) - LEAF
...
### Step 4: Node 20682 (Seq Scan) - LEAF
...
### Step 5: Node 20675 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20670, 20671, 20672, 20673, 20676, 20679
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18337
  - HashJoin_nt1=18337
...
### Step 8: Node 20670 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20671, 20672, 20673, 20675, 20676, 20679
- **Input Features:**
