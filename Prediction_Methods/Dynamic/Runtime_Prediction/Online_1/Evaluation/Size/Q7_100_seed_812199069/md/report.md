...
**Test Query:** Q7_100_seed_812199069
...
```
Node 10501 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10502 (Gather Merge) [consumed]
    Node 10503 (Sort)
      Node 10504 (Hash Join)
        Node 10505 (Nested Loop)
          Node 10506 (Hash Join) [PATTERN: 895c6e8c]
            Node 10507 (Seq Scan) [consumed] - LEAF
            Node 10508 (Hash) [consumed]
              Node 10509 (Hash Join) [PATTERN: f4cb205a]
                Node 10510 (Seq Scan) [consumed] - LEAF
                Node 10511 (Hash) [consumed]
                  Node 10512 (Seq Scan) [consumed] - LEAF
          Node 10513 (Index Scan) - LEAF
        Node 10514 (Hash)
          Node 10515 (Hash Join) [PATTERN: f4cb205a]
            Node 10516 (Seq Scan) [consumed] - LEAF
            Node 10517 (Hash) [consumed]
              Node 10518 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10509 | 10501, 10502, 10506, 10507, 10508, 10510, 10511, 10512, 10515, 10516, 10517, 10518 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10515 | 10501, 10502, 10506, 10507, 10508, 10509, 10510, 10511, 10512, 10516, 10517, 10518 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10501 | 10502, 10506, 10507, 10508, 10509, 10510, 10511, 10512, 10515, 10516, 10517, 10518 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10506 | 10501, 10502, 10507, 10508, 10509, 10510, 10511, 10512, 10515, 10516, 10517, 10518 |
...
|------|------|--------|-----------|-----|--------|
| 10501 | Aggregate | 1204.01 | 1163.13 | 3.4% | pattern |
| 10503 | Sort | 1197.00 | 1109.16 | 7.3% | operator |
| 10504 | Hash Join | 1196.34 | 843.11 | 29.5% | operator |
| 10505 | Nested Loop | 1187.84 | 1215.99 | 2.4% | operator |
| 10514 | Hash | 3.35 | 27.74 | 727.2% | operator |
| 10506 | Hash Join | 214.50 | 304.89 | 42.1% | pattern |
| 10513 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10515 | Hash Join | 3.31 | 429.36 | 12887.4% | pattern |
| 10509 | Hash Join | 36.37 | 429.38 | 1080.8% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10509 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10501, 10502, 10506, 10507, 10508, 10510, 10511, 10512, 10515, 10516, 10517, 10518
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10506 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10501, 10502, 10507, 10508, 10509, 10510, 10511, 10512, 10515, 10516, 10517, 10518
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10513 (Index Scan) - LEAF
...
### Step 4: Node 10515 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10501, 10502, 10506, 10507, 10508, 10509, 10510, 10511, 10512, 10516, 10517, 10518
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10501 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10502, 10506, 10507, 10508, 10509, 10510, 10511, 10512, 10515, 10516, 10517, 10518
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
