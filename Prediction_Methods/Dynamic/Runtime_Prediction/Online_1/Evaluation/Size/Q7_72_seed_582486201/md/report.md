...
**Test Query:** Q7_72_seed_582486201
...
```
Node 12643 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12644 (Gather Merge) [consumed]
    Node 12645 (Sort)
      Node 12646 (Hash Join)
        Node 12647 (Nested Loop)
          Node 12648 (Hash Join) [PATTERN: 895c6e8c]
            Node 12649 (Seq Scan) [consumed] - LEAF
            Node 12650 (Hash) [consumed]
              Node 12651 (Hash Join) [PATTERN: f4cb205a]
                Node 12652 (Seq Scan) [consumed] - LEAF
                Node 12653 (Hash) [consumed]
                  Node 12654 (Seq Scan) [consumed] - LEAF
          Node 12655 (Index Scan) - LEAF
        Node 12656 (Hash)
          Node 12657 (Hash Join) [PATTERN: f4cb205a]
            Node 12658 (Seq Scan) [consumed] - LEAF
            Node 12659 (Hash) [consumed]
              Node 12660 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12651 | 12643, 12644, 12648, 12649, 12650, 12652, 12653, 12654, 12657, 12658, 12659, 12660 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12657 | 12643, 12644, 12648, 12649, 12650, 12651, 12652, 12653, 12654, 12658, 12659, 12660 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12643 | 12644, 12648, 12649, 12650, 12651, 12652, 12653, 12654, 12657, 12658, 12659, 12660 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12648 | 12643, 12644, 12649, 12650, 12651, 12652, 12653, 12654, 12657, 12658, 12659, 12660 |
...
|------|------|--------|-----------|-----|--------|
| 12643 | Aggregate | 1200.22 | 1163.13 | 3.1% | pattern |
| 12645 | Sort | 1193.64 | 1109.16 | 7.1% | operator |
| 12646 | Hash Join | 1192.98 | 843.11 | 29.3% | operator |
| 12647 | Nested Loop | 1185.62 | 1215.99 | 2.6% | operator |
| 12656 | Hash | 3.50 | 27.74 | 693.8% | operator |
| 12648 | Hash Join | 214.93 | 304.89 | 41.9% | pattern |
| 12655 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12657 | Hash Join | 3.45 | 429.36 | 12356.1% | pattern |
| 12651 | Hash Join | 37.59 | 429.38 | 1042.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12651 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12643, 12644, 12648, 12649, 12650, 12652, 12653, 12654, 12657, 12658, 12659, 12660
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12648 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12643, 12644, 12649, 12650, 12651, 12652, 12653, 12654, 12657, 12658, 12659, 12660
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12655 (Index Scan) - LEAF
...
### Step 4: Node 12657 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12643, 12644, 12648, 12649, 12650, 12651, 12652, 12653, 12654, 12658, 12659, 12660
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12643 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12644, 12648, 12649, 12650, 12651, 12652, 12653, 12654, 12657, 12658, 12659, 12660
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
