        ...
        Node 10775 (Nested Loop)
          Node 10776 (Hash Join) [PATTERN: 895c6e8c]
            ...
            Node 10778 (Hash) [consumed]
              Node 10779 (Hash Join) [PATTERN: f4cb205a]
                Node 10780 (Seq Scan) [consumed] - LEAF
                Node 10781 (Hash) [consumed]
                  Node 10782 (Seq Scan) [consumed] - LEAF
          Node 10783 (Index Scan) - LEAF
        Node 10784 (Hash)
          Node 10785 (Hash Join) [PATTERN: f4cb205a]
            Node 10786 (Seq Scan) [consumed] - LEAF
            Node 10787 (Hash) [consumed]
              Node 10788 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10779 | 10771, 10772, 10776, 10777, 10778, 10780, 10781, 10782, 10785, 10786, 10787, 10788 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10785 | 10771, 10772, 10776, 10777, 10778, 10779, 10780, 10781, 10782, 10786, 10787, 10788 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10771 | 10772, 10776, 10777, 10778, 10779, 10780, 10781, 10782, 10785, 10786, 10787, 10788 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10776 | 10771, 10772, 10777, 10778, 10779, 10780, 10781, 10782, 10785, 10786, 10787, 10788 |
...
|------|------|--------|-----------|-----|--------|
| 10771 | Aggregate | 1209.32 | 1163.13 | 3.8% | pattern |
| 10773 | Sort | 1202.18 | 1109.16 | 7.7% | operator |
| 10774 | Hash Join | 1201.58 | 843.11 | 29.8% | operator |
| 10775 | Nested Loop | 1192.89 | 1215.99 | 1.9% | operator |
| 10784 | Hash | 4.83 | 27.74 | 474.3% | operator |
| 10776 | Hash Join | 213.21 | 304.89 | 43.0% | pattern |
| 10783 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 10785 | Hash Join | 4.78 | 429.36 | 8880.6% | pattern |
| 10779 | Hash Join | 35.26 | 429.38 | 1117.8% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10779 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10771, 10772, 10776, 10777, 10778, 10780, 10781, 10782, 10785, 10786, 10787, 10788
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10776 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10771, 10772, 10777, 10778, 10779, 10780, 10781, 10782, 10785, 10786, 10787, 10788
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10783 (Index Scan) - LEAF
...
### Step 4: Node 10785 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10771, 10772, 10776, 10777, 10778, 10779, 10780, 10781, 10782, 10786, 10787, 10788
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10771 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10772, 10776, 10777, 10778, 10779, 10780, 10781, 10782, 10785, 10786, 10787, 10788
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
