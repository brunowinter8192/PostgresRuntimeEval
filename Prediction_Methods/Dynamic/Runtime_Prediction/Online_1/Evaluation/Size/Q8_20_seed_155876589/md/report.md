...
**Test Query:** Q8_20_seed_155876589
...
```
Node 14627 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14628 (Gather Merge) [consumed]
    Node 14629 (Sort)
      Node 14630 (Hash Join)
        Node 14631 (Nested Loop)
          Node 14632 (Hash Join)
            Node 14633 (Nested Loop)
              Node 14634 (Hash Join)
                Node 14635 (Seq Scan) - LEAF
                Node 14636 (Hash) [PATTERN: 444761fb]
                  Node 14637 (Hash Join) [consumed]
                    Node 14638 (Seq Scan) [consumed] - LEAF
                    Node 14639 (Hash) [consumed]
                      Node 14640 (Hash Join) [consumed]
                        Node 14641 (Seq Scan) - LEAF
                        Node 14642 (Hash)
                          Node 14643 (Seq Scan) - LEAF
              Node 14644 (Index Scan) - LEAF
            Node 14645 (Hash)
              Node 14646 (Seq Scan) - LEAF
          Node 14647 (Index Scan) - LEAF
        Node 14648 (Hash)
          Node 14649 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14636 | 14627, 14628, 14637, 14638, 14639, 14640 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14627 | 14628, 14636, 14637, 14638, 14639, 14640 |
...
|------|------|--------|-----------|-----|--------|
| 14627 | Aggregate | 1242.52 | 1131.27 | 9.0% | pattern |
| 14629 | Sort | 1237.00 | 1046.21 | 15.4% | operator |
| 14630 | Hash Join | 1236.74 | 785.61 | 36.5% | operator |
| 14631 | Nested Loop | 1236.41 | 1198.29 | 3.1% | operator |
| 14648 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 14632 | Hash Join | 1232.63 | 785.60 | 36.3% | operator |
| 14647 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14649 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 14633 | Nested Loop | 1187.98 | 1198.29 | 0.9% | operator |
| 14645 | Hash | 40.23 | 19.97 | 50.4% | operator |
| 14634 | Hash Join | 217.19 | 251.67 | 15.9% | operator |
| 14644 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14646 | Seq Scan | 40.10 | 48.87 | 21.9% | operator |
| 14635 | Seq Scan | 163.12 | 159.67 | 2.1% | operator |
| 14636 | Hash | 35.39 | 43.51 | 23.0% | pattern |
| 14641 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14642 | Hash | 0.20 | 18.14 | 8838.0% | operator |
| 14643 | Seq Scan | 0.20 | 23.36 | 11523.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14643 (Seq Scan) - LEAF
...
### Step 2: Node 14641 (Seq Scan) - LEAF
...
### Step 4: Node 14635 (Seq Scan) - LEAF
...
### Step 5: Node 14636 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14627, 14628, 14637, 14638, 14639, 14640
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14644 (Index Scan) - LEAF
...
### Step 8: Node 14646 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9696
  - startup_cost=9920.1800
  - total_cost=70271.6500
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14647 (Index Scan) - LEAF
...
### Step 13: Node 14649 (Seq Scan) - LEAF
...
### Step 14: Node 14631 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70510.9700
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14648 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.0200
  - total_cost=70516.9400
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14629 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70555.0200
  - total_cost=70556.9900
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14627 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14628, 14636, 14637, 14638, 14639, 14640
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2453
  ...
  - Aggregate_sel=0.9808
  - Aggregate_startup_cost=71555.0600
  - Aggregate_total_cost=71942.4400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2453
  - GatherMerge_Outer_nt1=791
  ...
  - GatherMerge_Outer_sel=3.1011
  - GatherMerge_Outer_startup_cost=71555.0600
  - GatherMerge_Outer_total_cost=71845.2600
- **Output:** st=1128.29, rt=1131.27
