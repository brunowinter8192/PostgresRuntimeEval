...
**Test Query:** Q8_75_seed_607098294
...
```
Node 16007 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16008 (Gather Merge) [consumed]
    Node 16009 (Sort)
      Node 16010 (Hash Join)
        Node 16011 (Nested Loop)
          Node 16012 (Hash Join)
            Node 16013 (Nested Loop)
              Node 16014 (Hash Join)
                Node 16015 (Seq Scan) - LEAF
                Node 16016 (Hash) [PATTERN: 444761fb]
                  Node 16017 (Hash Join) [consumed]
                    Node 16018 (Seq Scan) [consumed] - LEAF
                    Node 16019 (Hash) [consumed]
                      Node 16020 (Hash Join) [consumed]
                        Node 16021 (Seq Scan) - LEAF
                        Node 16022 (Hash)
                          Node 16023 (Seq Scan) - LEAF
              Node 16024 (Index Scan) - LEAF
            Node 16025 (Hash)
              Node 16026 (Seq Scan) - LEAF
          Node 16027 (Index Scan) - LEAF
        Node 16028 (Hash)
          Node 16029 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16016 | 16007, 16008, 16017, 16018, 16019, 16020 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16007 | 16008, 16016, 16017, 16018, 16019, 16020 |
...
|------|------|--------|-----------|-----|--------|
| 16007 | Aggregate | 1209.81 | 1133.66 | 6.3% | pattern |
| 16009 | Sort | 1204.47 | 1046.21 | 13.1% | operator |
| 16010 | Hash Join | 1204.23 | 785.61 | 34.8% | operator |
| 16011 | Nested Loop | 1203.93 | 1198.29 | 0.5% | operator |
| 16028 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 16012 | Hash Join | 1200.29 | 785.60 | 34.5% | operator |
| 16027 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16029 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 16013 | Nested Loop | 1158.56 | 1198.29 | 3.4% | operator |
| 16025 | Hash | 37.25 | 19.97 | 46.4% | operator |
| 16014 | Hash Join | 205.91 | 251.67 | 22.2% | operator |
| 16024 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 16026 | Seq Scan | 37.16 | 48.89 | 31.6% | operator |
| 16015 | Seq Scan | 156.32 | 159.67 | 2.1% | operator |
| 16016 | Hash | 31.72 | 43.51 | 37.2% | pattern |
| 16021 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 16022 | Hash | 0.15 | 18.14 | 11996.1% | operator |
| 16023 | Seq Scan | 0.15 | 23.36 | 15685.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16023 (Seq Scan) - LEAF
...
### Step 2: Node 16021 (Seq Scan) - LEAF
...
### Step 4: Node 16015 (Seq Scan) - LEAF
...
### Step 5: Node 16016 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16007, 16008, 16017, 16018, 16019, 16020
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16024 (Index Scan) - LEAF
...
### Step 8: Node 16026 (Seq Scan) - LEAF
...
### Step 12: Node 16027 (Index Scan) - LEAF
...
### Step 13: Node 16029 (Seq Scan) - LEAF
...
### Step 14: Node 16011 (Nested Loop)
...
### Step 18: Node 16007 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16008, 16016, 16017, 16018, 16019, 16020
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2319
  - Aggregate_nt1=2319
