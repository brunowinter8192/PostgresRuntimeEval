...
**Test Query:** Q10_144_seed_1173176433
...
```
Node 19994 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19995 (Sort) [consumed]
    Node 19996 (Aggregate) [consumed]
      Node 19997 (Gather) [consumed]
        Node 19998 (Hash Join)
          Node 19999 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20000 (Nested Loop) [consumed]
              Node 20001 (Seq Scan) - LEAF
              Node 20002 (Index Scan) - LEAF
            Node 20003 (Hash) [consumed]
              Node 20004 (Seq Scan) - LEAF
          Node 20005 (Hash)
            Node 20006 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19994 | 19995, 19996, 19997, 19999, 20000, 20003 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19999 | 19994, 19995, 19996, 19997, 20000, 20003 |
...
|------|------|--------|-----------|-----|--------|
| 19994 | Limit | 1139.16 | 1199.47 | 5.3% | pattern |
| 19998 | Hash Join | 1085.79 | 1095.56 | 0.9% | operator |
| 19999 | Hash Join | 1082.82 | 1120.19 | 3.5% | pattern |
| 20005 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20006 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 20001 | Seq Scan | 159.94 | 159.74 | 0.1% | operator |
| 20002 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20004 | Seq Scan | 27.34 | 26.64 | 2.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20001 (Seq Scan) - LEAF
...
### Step 2: Node 20002 (Index Scan) - LEAF
...
### Step 3: Node 20004 (Seq Scan) - LEAF
...
### Step 4: Node 20006 (Seq Scan) - LEAF
...
### Step 5: Node 19999 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19994, 19995, 19996, 19997, 20000, 20003
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18231
  - HashJoin_nt1=18231
...
- **Output:** st=54.22, rt=1095.56

### Step 8: Node 19994 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19995, 19996, 19997, 19999, 20000, 20003
- **Input Features:**
