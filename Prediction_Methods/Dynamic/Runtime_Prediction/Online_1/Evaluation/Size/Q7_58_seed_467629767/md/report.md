...
**Test Query:** Q7_58_seed_467629767
...
```
Node 12355 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12356 (Gather Merge) [consumed]
    Node 12357 (Sort)
      Node 12358 (Hash Join)
        Node 12359 (Nested Loop)
          Node 12360 (Hash Join) [PATTERN: 895c6e8c]
            Node 12361 (Seq Scan) [consumed] - LEAF
            Node 12362 (Hash) [consumed]
              Node 12363 (Hash Join) [PATTERN: f4cb205a]
                Node 12364 (Seq Scan) [consumed] - LEAF
                Node 12365 (Hash) [consumed]
                  Node 12366 (Seq Scan) [consumed] - LEAF
          Node 12367 (Index Scan) - LEAF
        Node 12368 (Hash)
          Node 12369 (Hash Join) [PATTERN: f4cb205a]
            Node 12370 (Seq Scan) [consumed] - LEAF
            Node 12371 (Hash) [consumed]
              Node 12372 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12363 | 12355, 12356, 12360, 12361, 12362, 12364, 12365, 12366, 12369, 12370, 12371, 12372 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12369 | 12355, 12356, 12360, 12361, 12362, 12363, 12364, 12365, 12366, 12370, 12371, 12372 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12355 | 12356, 12360, 12361, 12362, 12363, 12364, 12365, 12366, 12369, 12370, 12371, 12372 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12360 | 12355, 12356, 12361, 12362, 12363, 12364, 12365, 12366, 12369, 12370, 12371, 12372 |
...
|------|------|--------|-----------|-----|--------|
| 12355 | Aggregate | 1197.21 | 1163.13 | 2.8% | pattern |
| 12357 | Sort | 1191.31 | 1109.16 | 6.9% | operator |
| 12358 | Hash Join | 1190.60 | 843.11 | 29.2% | operator |
| 12359 | Nested Loop | 1183.05 | 1215.99 | 2.8% | operator |
| 12368 | Hash | 3.73 | 27.74 | 643.2% | operator |
| 12360 | Hash Join | 213.07 | 304.89 | 43.1% | pattern |
| 12367 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12369 | Hash Join | 3.69 | 429.36 | 11535.8% | pattern |
| 12363 | Hash Join | 34.63 | 429.38 | 1140.1% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12363 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12355, 12356, 12360, 12361, 12362, 12364, 12365, 12366, 12369, 12370, 12371, 12372
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12360 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12355, 12356, 12361, 12362, 12363, 12364, 12365, 12366, 12369, 12370, 12371, 12372
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12367 (Index Scan) - LEAF
...
### Step 4: Node 12369 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12355, 12356, 12360, 12361, 12362, 12363, 12364, 12365, 12366, 12370, 12371, 12372
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12355 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12356, 12360, 12361, 12362, 12363, 12364, 12365, 12366, 12369, 12370, 12371, 12372
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
