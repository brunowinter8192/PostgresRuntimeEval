          ...
          Node 16288 (Hash Join)
            Node 16289 (Nested Loop)
              Node 16290 (Hash Join)
                ...
                Node 16292 (Hash) [PATTERN: 444761fb]
                  Node 16293 (Hash Join) [consumed]
                    Node 16294 (Seq Scan) [consumed] - LEAF
                    Node 16295 (Hash) [consumed]
                      Node 16296 (Hash Join) [consumed]
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16292 | 16283, 16284, 16293, 16294, 16295, 16296 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16283 | 16284, 16292, 16293, 16294, 16295, 16296 |
...
|------|------|--------|-----------|-----|--------|
| 16283 | Aggregate | 1231.68 | 1133.68 | 8.0% | pattern |
| 16285 | Sort | 1226.24 | 1046.21 | 14.7% | operator |
| 16286 | Hash Join | 1226.01 | 785.61 | 35.9% | operator |
| 16287 | Nested Loop | 1225.70 | 1198.29 | 2.2% | operator |
| 16304 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 16288 | Hash Join | 1221.83 | 785.60 | 35.7% | operator |
| 16303 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16305 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 16289 | Nested Loop | 1176.66 | 1198.29 | 1.8% | operator |
| 16301 | Hash | 40.96 | 19.97 | 51.2% | operator |
| 16290 | Hash Join | 212.75 | 251.67 | 18.3% | operator |
| 16300 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 16302 | Seq Scan | 40.84 | 48.89 | 19.7% | operator |
| 16291 | Seq Scan | 160.79 | 159.67 | 0.7% | operator |
| 16292 | Hash | 33.98 | 43.51 | 28.1% | pattern |
| 16297 | Seq Scan | 0.02 | 12.22 | 71811.4% | operator |
| 16298 | Hash | 0.20 | 18.14 | 9110.2% | operator |
| 16299 | Seq Scan | 0.19 | 23.36 | 11942.4% | operator |
...
### Step 4: Node 16291 (Seq Scan) - LEAF
...
### Step 5: Node 16292 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16283, 16284, 16293, 16294, 16295, 16296
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16300 (Index Scan) - LEAF
...
### Step 8: Node 16302 (Seq Scan) - LEAF
...
### Step 12: Node 16303 (Index Scan) - LEAF
...
### Step 13: Node 16305 (Seq Scan) - LEAF
...
### Step 14: Node 16287 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70538.2700
  - total_cost=70540.1300
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 16283 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16284, 16292, 16293, 16294, 16295, 16296
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2312
  - Aggregate_nt1=2312
  ...
  - Aggregate_total_cost=71904.3100
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2312
  - GatherMerge_Outer_nt1=746
  ...
  - GatherMerge_Outer_total_cost=71811.8300
