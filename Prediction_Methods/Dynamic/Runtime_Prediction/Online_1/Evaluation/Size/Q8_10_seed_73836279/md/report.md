...
**Test Query:** Q8_10_seed_73836279
...
```
Node 13431 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13432 (Gather Merge) [consumed]
    Node 13433 (Sort)
      Node 13434 (Hash Join)
        Node 13435 (Nested Loop)
          Node 13436 (Hash Join)
            Node 13437 (Nested Loop)
              Node 13438 (Hash Join)
                Node 13439 (Seq Scan) - LEAF
                Node 13440 (Hash) [PATTERN: 444761fb]
                  Node 13441 (Hash Join) [consumed]
                    Node 13442 (Seq Scan) [consumed] - LEAF
                    Node 13443 (Hash) [consumed]
                      Node 13444 (Hash Join) [consumed]
                        Node 13445 (Seq Scan) - LEAF
                        Node 13446 (Hash)
                          Node 13447 (Seq Scan) - LEAF
              Node 13448 (Index Scan) - LEAF
            Node 13449 (Hash)
              Node 13450 (Seq Scan) - LEAF
          Node 13451 (Index Scan) - LEAF
        Node 13452 (Hash)
          Node 13453 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13440 | 13431, 13432, 13441, 13442, 13443, 13444 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13431 | 13432, 13440, 13441, 13442, 13443, 13444 |
...
|------|------|--------|-----------|-----|--------|
| 13431 | Aggregate | 1202.17 | 1132.74 | 5.8% | pattern |
| 13433 | Sort | 1195.37 | 1046.21 | 12.5% | operator |
| 13434 | Hash Join | 1195.13 | 785.61 | 34.3% | operator |
| 13435 | Nested Loop | 1194.81 | 1198.29 | 0.3% | operator |
| 13452 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 13436 | Hash Join | 1191.25 | 785.60 | 34.1% | operator |
| 13451 | Index Scan | 0.01 | 12.42 | 248369.7% | operator |
| 13453 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13437 | Nested Loop | 1147.45 | 1198.29 | 4.4% | operator |
| 13449 | Hash | 39.25 | 19.97 | 49.1% | operator |
| 13438 | Hash Join | 207.72 | 251.67 | 21.2% | operator |
| 13448 | Index Scan | 0.04 | 2.88 | 7102.1% | operator |
| 13450 | Seq Scan | 39.14 | 48.87 | 24.9% | operator |
| 13439 | Seq Scan | 153.80 | 159.67 | 3.8% | operator |
| 13440 | Hash | 34.95 | 43.51 | 24.5% | pattern |
| 13445 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13446 | Hash | 0.14 | 18.14 | 12588.2% | operator |
| 13447 | Seq Scan | 0.14 | 23.36 | 16587.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13447 (Seq Scan) - LEAF
...
### Step 2: Node 13445 (Seq Scan) - LEAF
...
### Step 4: Node 13439 (Seq Scan) - LEAF
...
### Step 5: Node 13440 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13431, 13432, 13441, 13442, 13443, 13444
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13448 (Index Scan) - LEAF
...
### Step 8: Node 13450 (Seq Scan) - LEAF
...
### Step 12: Node 13451 (Index Scan) - LEAF
...
### Step 13: Node 13453 (Seq Scan) - LEAF
...
### Step 14: Node 13435 (Nested Loop)
...
### Step 18: Node 13431 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13432, 13440, 13441, 13442, 13443, 13444
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2424
