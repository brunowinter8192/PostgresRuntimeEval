...
**Test Query:** Q8_82_seed_664526511
...
```
Node 16191 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16192 (Gather Merge) [consumed]
    Node 16193 (Sort)
      Node 16194 (Hash Join)
        Node 16195 (Nested Loop)
          Node 16196 (Hash Join)
            Node 16197 (Nested Loop)
              Node 16198 (Hash Join)
                Node 16199 (Seq Scan) - LEAF
                Node 16200 (Hash) [PATTERN: 444761fb]
                  Node 16201 (Hash Join) [consumed]
                    Node 16202 (Seq Scan) [consumed] - LEAF
                    Node 16203 (Hash) [consumed]
                      Node 16204 (Hash Join) [consumed]
                        Node 16205 (Seq Scan) - LEAF
                        Node 16206 (Hash)
                          Node 16207 (Seq Scan) - LEAF
              Node 16208 (Index Scan) - LEAF
            Node 16209 (Hash)
              Node 16210 (Seq Scan) - LEAF
          Node 16211 (Index Scan) - LEAF
        Node 16212 (Hash)
          Node 16213 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16200 | 16191, 16192, 16201, 16202, 16203, 16204 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16191 | 16192, 16200, 16201, 16202, 16203, 16204 |
...
|------|------|--------|-----------|-----|--------|
| 16191 | Aggregate | 1218.19 | 1133.66 | 6.9% | pattern |
| 16193 | Sort | 1211.72 | 1046.21 | 13.7% | operator |
| 16194 | Hash Join | 1211.48 | 785.61 | 35.2% | operator |
| 16195 | Nested Loop | 1211.18 | 1198.29 | 1.1% | operator |
| 16212 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 16196 | Hash Join | 1207.33 | 785.60 | 34.9% | operator |
| 16211 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16213 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 16197 | Nested Loop | 1163.22 | 1198.29 | 3.0% | operator |
| 16209 | Hash | 38.59 | 19.97 | 48.2% | operator |
| 16198 | Hash Join | 213.72 | 251.67 | 17.8% | operator |
| 16208 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 16210 | Seq Scan | 38.49 | 48.88 | 27.0% | operator |
| 16199 | Seq Scan | 158.09 | 159.67 | 1.0% | operator |
| 16200 | Hash | 37.17 | 43.51 | 17.1% | pattern |
| 16205 | Seq Scan | 0.01 | 12.22 | 101774.5% | operator |
| 16206 | Hash | 0.06 | 18.14 | 29164.8% | operator |
| 16207 | Seq Scan | 0.06 | 23.36 | 40179.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16207 (Seq Scan) - LEAF
...
### Step 2: Node 16205 (Seq Scan) - LEAF
...
### Step 4: Node 16199 (Seq Scan) - LEAF
...
### Step 5: Node 16200 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16191, 16192, 16201, 16202, 16203, 16204
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16208 (Index Scan) - LEAF
...
### Step 8: Node 16210 (Seq Scan) - LEAF
...
### Step 12: Node 16211 (Index Scan) - LEAF
...
### Step 13: Node 16213 (Seq Scan) - LEAF
...
### Step 14: Node 16195 (Nested Loop)
...
### Step 18: Node 16191 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16192, 16200, 16201, 16202, 16203, 16204
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2341
  - Aggregate_nt1=2341
  ...
  - Aggregate_total_cost=71912.2400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2341
  ...
  - GatherMerge_Outer_total_cost=71818.6000
