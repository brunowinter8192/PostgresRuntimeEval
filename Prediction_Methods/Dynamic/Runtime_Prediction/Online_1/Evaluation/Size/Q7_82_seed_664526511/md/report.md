...
**Test Query:** Q7_82_seed_664526511
...
```
Node 12841 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12842 (Gather Merge) [consumed]
    Node 12843 (Sort)
      Node 12844 (Hash Join)
        Node 12845 (Nested Loop)
          Node 12846 (Hash Join) [PATTERN: 895c6e8c]
            Node 12847 (Seq Scan) [consumed] - LEAF
            Node 12848 (Hash) [consumed]
              Node 12849 (Hash Join) [PATTERN: f4cb205a]
                Node 12850 (Seq Scan) [consumed] - LEAF
                Node 12851 (Hash) [consumed]
                  Node 12852 (Seq Scan) [consumed] - LEAF
          Node 12853 (Index Scan) - LEAF
        Node 12854 (Hash)
          Node 12855 (Hash Join) [PATTERN: f4cb205a]
            Node 12856 (Seq Scan) [consumed] - LEAF
            Node 12857 (Hash) [consumed]
              Node 12858 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12849 | 12841, 12842, 12846, 12847, 12848, 12850, 12851, 12852, 12855, 12856, 12857, 12858 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12855 | 12841, 12842, 12846, 12847, 12848, 12849, 12850, 12851, 12852, 12856, 12857, 12858 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12841 | 12842, 12846, 12847, 12848, 12849, 12850, 12851, 12852, 12855, 12856, 12857, 12858 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12846 | 12841, 12842, 12847, 12848, 12849, 12850, 12851, 12852, 12855, 12856, 12857, 12858 |
...
|------|------|--------|-----------|-----|--------|
| 12841 | Aggregate | 1183.33 | 1163.13 | 1.7% | pattern |
| 12843 | Sort | 1176.77 | 1109.16 | 5.7% | operator |
| 12844 | Hash Join | 1176.17 | 843.11 | 28.3% | operator |
| 12845 | Nested Loop | 1169.59 | 1215.99 | 4.0% | operator |
| 12854 | Hash | 2.90 | 27.74 | 857.7% | operator |
| 12846 | Hash Join | 210.54 | 304.89 | 44.8% | pattern |
| 12853 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12855 | Hash Join | 2.85 | 429.36 | 14970.7% | pattern |
| 12849 | Hash Join | 30.01 | 429.38 | 1330.8% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12849 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12841, 12842, 12846, 12847, 12848, 12850, 12851, 12852, 12855, 12856, 12857, 12858
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12846 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12841, 12842, 12847, 12848, 12849, 12850, 12851, 12852, 12855, 12856, 12857, 12858
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12853 (Index Scan) - LEAF
...
### Step 4: Node 12855 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12841, 12842, 12846, 12847, 12848, 12849, 12850, 12851, 12852, 12856, 12857, 12858
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12841 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12842, 12846, 12847, 12848, 12849, 12850, 12851, 12852, 12855, 12856, 12857, 12858
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
