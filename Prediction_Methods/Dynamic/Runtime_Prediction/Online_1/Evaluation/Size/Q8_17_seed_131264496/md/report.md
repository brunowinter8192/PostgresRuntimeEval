...
**Test Query:** Q8_17_seed_131264496
...
```
Node 14535 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14536 (Gather Merge) [consumed]
    Node 14537 (Sort)
      Node 14538 (Hash Join)
        Node 14539 (Nested Loop)
          Node 14540 (Hash Join)
            Node 14541 (Nested Loop)
              Node 14542 (Hash Join)
                Node 14543 (Seq Scan) - LEAF
                Node 14544 (Hash) [PATTERN: 444761fb]
                  Node 14545 (Hash Join) [consumed]
                    Node 14546 (Seq Scan) [consumed] - LEAF
                    Node 14547 (Hash) [consumed]
                      Node 14548 (Hash Join) [consumed]
                        Node 14549 (Seq Scan) - LEAF
                        Node 14550 (Hash)
                          Node 14551 (Seq Scan) - LEAF
              Node 14552 (Index Scan) - LEAF
            Node 14553 (Hash)
              Node 14554 (Seq Scan) - LEAF
          Node 14555 (Index Scan) - LEAF
        Node 14556 (Hash)
          Node 14557 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14544 | 14535, 14536, 14545, 14546, 14547, 14548 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14535 | 14536, 14544, 14545, 14546, 14547, 14548 |
...
|------|------|--------|-----------|-----|--------|
| 14535 | Aggregate | 1249.60 | 1130.53 | 9.5% | pattern |
| 14537 | Sort | 1240.98 | 1046.21 | 15.7% | operator |
| 14538 | Hash Join | 1240.74 | 785.61 | 36.7% | operator |
| 14539 | Nested Loop | 1240.44 | 1198.29 | 3.4% | operator |
| 14556 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 14540 | Hash Join | 1236.53 | 785.60 | 36.5% | operator |
| 14555 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14557 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 14541 | Nested Loop | 1197.57 | 1198.29 | 0.1% | operator |
| 14553 | Hash | 34.46 | 19.97 | 42.1% | operator |
| 14542 | Hash Join | 217.06 | 251.67 | 15.9% | operator |
| 14552 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14554 | Seq Scan | 34.41 | 48.87 | 42.0% | operator |
| 14543 | Seq Scan | 160.33 | 159.67 | 0.4% | operator |
| 14544 | Hash | 38.96 | 43.51 | 11.7% | pattern |
| 14549 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 14550 | Hash | 0.18 | 18.14 | 9707.6% | operator |
| 14551 | Seq Scan | 0.18 | 23.36 | 12807.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14551 (Seq Scan) - LEAF
...
### Step 2: Node 14549 (Seq Scan) - LEAF
...
### Step 4: Node 14543 (Seq Scan) - LEAF
...
### Step 5: Node 14544 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14535, 14536, 14545, 14546, 14547, 14548
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14552 (Index Scan) - LEAF
...
### Step 8: Node 14554 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - total_cost=70271.6900
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14555 (Index Scan) - LEAF
...
### Step 13: Node 14557 (Seq Scan) - LEAF
...
### Step 14: Node 14539 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.5000
  - total_cost=70512.5200
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14556 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.0600
  - total_cost=70518.5200
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14537 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70556.8700
  - total_cost=70558.8600
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14535 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14536, 14544, 14545, 14546, 14547, 14548
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2468
  ...
  - Aggregate_sel=0.9749
  - Aggregate_startup_cost=71556.9100
  - Aggregate_total_cost=71946.3700
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2468
  - GatherMerge_Outer_nt1=796
  ...
  - GatherMerge_Outer_startup_cost=71556.9100
  - GatherMerge_Outer_total_cost=71848.8900
- **Output:** st=1127.56, rt=1130.53
