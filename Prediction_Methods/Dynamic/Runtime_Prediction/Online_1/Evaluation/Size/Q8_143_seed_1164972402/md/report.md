...
**Test Query:** Q8_143_seed_1164972402
...
```
Node 14282 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14283 (Gather Merge) [consumed]
    Node 14284 (Sort)
      Node 14285 (Hash Join)
        Node 14286 (Nested Loop)
          Node 14287 (Hash Join)
            Node 14288 (Nested Loop)
              Node 14289 (Hash Join)
                Node 14290 (Seq Scan) - LEAF
                Node 14291 (Hash) [PATTERN: 444761fb]
                  Node 14292 (Hash Join) [consumed]
                    Node 14293 (Seq Scan) [consumed] - LEAF
                    Node 14294 (Hash) [consumed]
                      Node 14295 (Hash Join) [consumed]
                        Node 14296 (Seq Scan) - LEAF
                        Node 14297 (Hash)
                          Node 14298 (Seq Scan) - LEAF
              Node 14299 (Index Scan) - LEAF
            Node 14300 (Hash)
              Node 14301 (Seq Scan) - LEAF
          Node 14302 (Index Scan) - LEAF
        Node 14303 (Hash)
          Node 14304 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14291 | 14282, 14283, 14292, 14293, 14294, 14295 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14282 | 14283, 14291, 14292, 14293, 14294, 14295 |
...
|------|------|--------|-----------|-----|--------|
| 14282 | Aggregate | 1232.05 | 1133.66 | 8.0% | pattern |
| 14284 | Sort | 1225.75 | 1046.21 | 14.6% | operator |
| 14285 | Hash Join | 1225.49 | 785.61 | 35.9% | operator |
| 14286 | Nested Loop | 1225.16 | 1198.29 | 2.2% | operator |
| 14303 | Hash | 0.02 | 13.72 | 76144.9% | operator |
| 14287 | Hash Join | 1221.57 | 785.60 | 35.7% | operator |
| 14302 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14304 | Seq Scan | 0.01 | 6.51 | 43313.1% | operator |
| 14288 | Nested Loop | 1175.44 | 1198.29 | 1.9% | operator |
| 14300 | Hash | 40.93 | 19.97 | 51.2% | operator |
| 14289 | Hash Join | 216.88 | 251.67 | 16.0% | operator |
| 14299 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14301 | Seq Scan | 40.82 | 48.89 | 19.8% | operator |
| 14290 | Seq Scan | 159.35 | 159.67 | 0.2% | operator |
| 14291 | Hash | 39.39 | 43.51 | 10.5% | pattern |
| 14296 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14297 | Hash | 0.06 | 18.14 | 29644.5% | operator |
| 14298 | Seq Scan | 0.06 | 23.36 | 40179.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14298 (Seq Scan) - LEAF
...
### Step 2: Node 14296 (Seq Scan) - LEAF
...
### Step 4: Node 14290 (Seq Scan) - LEAF
...
### Step 5: Node 14291 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14282, 14283, 14292, 14293, 14294, 14295
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14299 (Index Scan) - LEAF
...
### Step 8: Node 14301 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9722
  - startup_cost=9919.7900
  - total_cost=70271.2700
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14302 (Index Scan) - LEAF
...
### Step 13: Node 14304 (Seq Scan) - LEAF
...
### Step 14: Node 14286 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.0700
  - total_cost=70497.5700
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14303 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.6400
  - total_cost=70503.3000
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14284 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70539.0100
  - total_cost=70540.8800
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14282 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14283, 14291, 14292, 14293, 14294, 14295
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2319
  - Aggregate_nt1=2319
  ...
  - Aggregate_startup_cost=71539.0500
  - Aggregate_total_cost=71906.1600
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2319
  - GatherMerge_Outer_nt1=748
  ...
  - GatherMerge_Outer_sel=3.1003
  - GatherMerge_Outer_startup_cost=71539.0500
  - GatherMerge_Outer_total_cost=71813.4000
- **Output:** st=1130.67, rt=1133.66
