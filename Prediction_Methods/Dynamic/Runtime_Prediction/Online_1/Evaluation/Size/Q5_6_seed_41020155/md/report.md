...
```
Node 9221 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9222 (Aggregate) [consumed]
    Node 9223 (Gather Merge) [consumed]
      Node 9224 (Aggregate)
        Node 9225 (Sort) [PATTERN: 3e2d5a00]
          Node 9226 (Hash Join) [consumed]
            Node 9227 (Nested Loop)
              Node 9228 (Hash Join)
                Node 9229 (Seq Scan) - LEAF
                Node 9230 (Hash) [PATTERN: 444761fb]
                  Node 9231 (Hash Join) [consumed]
                    Node 9232 (Seq Scan) [consumed] - LEAF
                    Node 9233 (Hash) [consumed]
                      Node 9234 (Hash Join) [consumed]
                        Node 9235 (Seq Scan) - LEAF
                        Node 9236 (Hash)
                          Node 9237 (Seq Scan) - LEAF
              Node 9238 (Index Scan) - LEAF
            Node 9239 (Hash)
              Node 9240 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9230 | 9221, 9222, 9223, 9225, 9226, 9231, 9232, 9233, 9234 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9221 | 9222, 9223, 9225, 9226, 9230, 9231, 9232, 9233, 9234 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9225 | 9221, 9222, 9223, 9226, 9230, 9231, 9232, 9233, 9234 |
...
|------|------|--------|-----------|-----|--------|
| 9221 | Sort | 1060.74 | 2934.42 | 176.6% | pattern |
| 9224 | Aggregate | 1056.30 | 1128.79 | 6.9% | operator |
| 9225 | Sort | 1056.04 | 1835.90 | 73.8% | pattern |
| 9227 | Nested Loop | 1042.56 | 1188.37 | 14.0% | operator |
| 9239 | Hash | 4.54 | 18.67 | 310.9% | operator |
| 9228 | Hash Join | 217.95 | 253.07 | 16.1% | operator |
| 9238 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 9240 | Seq Scan | 3.99 | 12.36 | 209.8% | operator |
| 9229 | Seq Scan | 169.74 | 159.81 | 5.8% | operator |
| 9230 | Hash | 35.17 | 46.85 | 33.2% | pattern |
| 9235 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 9236 | Hash | 0.06 | 21.05 | 36198.7% | operator |
| 9237 | Seq Scan | 0.05 | 22.93 | 42369.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9237 (Seq Scan) - LEAF
...
### Step 2: Node 9235 (Seq Scan) - LEAF
...
### Step 4: Node 9229 (Seq Scan) - LEAF
...
### Step 5: Node 9230 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9221, 9222, 9223, 9225, 9226, 9231, 9232, 9233, 9234
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9238 (Index Scan) - LEAF
...
### Step 8: Node 9240 (Seq Scan) - LEAF
...
### Step 11: Node 9225 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9221, 9222, 9223, 9226, 9230, 9231, 9232, 9233, 9234
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9221 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9222, 9223, 9225, 9226, 9230, 9231, 9232, 9233, 9234
- **Input Features:**
