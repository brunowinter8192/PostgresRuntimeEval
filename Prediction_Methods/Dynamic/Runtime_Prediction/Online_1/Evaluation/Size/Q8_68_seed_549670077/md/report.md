...
**Test Query:** Q8_68_seed_549670077
...
```
Node 15823 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15824 (Gather Merge) [consumed]
    Node 15825 (Sort)
      Node 15826 (Hash Join)
        Node 15827 (Nested Loop)
          Node 15828 (Hash Join)
            Node 15829 (Nested Loop)
              Node 15830 (Hash Join)
                Node 15831 (Seq Scan) - LEAF
                Node 15832 (Hash) [PATTERN: 444761fb]
                  Node 15833 (Hash Join) [consumed]
                    Node 15834 (Seq Scan) [consumed] - LEAF
                    Node 15835 (Hash) [consumed]
                      Node 15836 (Hash Join) [consumed]
                        Node 15837 (Seq Scan) - LEAF
                        Node 15838 (Hash)
                          Node 15839 (Seq Scan) - LEAF
              Node 15840 (Index Scan) - LEAF
            Node 15841 (Hash)
              Node 15842 (Seq Scan) - LEAF
          Node 15843 (Index Scan) - LEAF
        Node 15844 (Hash)
          Node 15845 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15832 | 15823, 15824, 15833, 15834, 15835, 15836 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15823 | 15824, 15832, 15833, 15834, 15835, 15836 |
...
|------|------|--------|-----------|-----|--------|
| 15823 | Aggregate | 1218.34 | 1130.30 | 7.2% | pattern |
| 15825 | Sort | 1213.93 | 1046.21 | 13.8% | operator |
| 15826 | Hash Join | 1213.67 | 785.61 | 35.3% | operator |
| 15827 | Nested Loop | 1213.33 | 1198.29 | 1.2% | operator |
| 15844 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 15828 | Hash Join | 1209.34 | 785.60 | 35.0% | operator |
| 15843 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15845 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 15829 | Nested Loop | 1162.25 | 1198.29 | 3.1% | operator |
| 15841 | Hash | 42.63 | 19.97 | 53.2% | operator |
| 15830 | Hash Join | 214.06 | 251.67 | 17.6% | operator |
| 15840 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15842 | Seq Scan | 42.57 | 48.87 | 14.8% | operator |
| 15831 | Seq Scan | 159.68 | 159.67 | 0.0% | operator |
| 15832 | Hash | 36.56 | 43.51 | 19.0% | pattern |
| 15837 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 15838 | Hash | 0.20 | 18.14 | 8972.1% | operator |
| 15839 | Seq Scan | 0.20 | 23.36 | 11819.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15839 (Seq Scan) - LEAF
...
### Step 2: Node 15837 (Seq Scan) - LEAF
...
### Step 4: Node 15831 (Seq Scan) - LEAF
...
### Step 5: Node 15832 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15823, 15824, 15833, 15834, 15835, 15836
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15840 (Index Scan) - LEAF
...
### Step 8: Node 15842 (Seq Scan) - LEAF
...
### Step 12: Node 15843 (Index Scan) - LEAF
...
### Step 13: Node 15845 (Seq Scan) - LEAF
...
### Step 14: Node 15827 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70513.1400
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15844 (Hash)
...
- **Input Features:**
  ...
  - total_cost=70519.1400
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15825 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70557.6100
  - total_cost=70559.6000
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 15823 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15824, 15832, 15833, 15834, 15835, 15836
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2473
  ...
  - Aggregate_sel=0.9729
  - Aggregate_startup_cost=71557.6500
  - Aggregate_total_cost=71947.8000
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2473
  - GatherMerge_Outer_nt1=798
  ...
  - GatherMerge_Outer_startup_cost=71557.6500
  - GatherMerge_Outer_total_cost=71850.2200
- **Output:** st=1127.33, rt=1130.30
