...
Node 15363 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15364 (Gather Merge) [consumed]
    Node 15365 (Sort)
      Node 15366 (Hash Join)
        Node 15367 (Nested Loop)
          Node 15368 (Hash Join)
            Node 15369 (Nested Loop)
              Node 15370 (Hash Join)
                Node 15371 (Seq Scan) - LEAF
                Node 15372 (Hash) [PATTERN: 444761fb]
                  Node 15373 (Hash Join) [consumed]
                    Node 15374 (Seq Scan) [consumed] - LEAF
                    Node 15375 (Hash) [consumed]
                      Node 15376 (Hash Join) [consumed]
                        Node 15377 (Seq Scan) - LEAF
                        Node 15378 (Hash)
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15372 | 15363, 15364, 15373, 15374, 15375, 15376 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15363 | 15364, 15372, 15373, 15374, 15375, 15376 |
...
|------|------|--------|-----------|-----|--------|
| 15363 | Aggregate | 1235.95 | 1126.04 | 8.9% | pattern |
| 15365 | Sort | 1228.34 | 1046.22 | 14.8% | operator |
| 15366 | Hash Join | 1228.10 | 785.61 | 36.0% | operator |
| 15367 | Nested Loop | 1226.83 | 1198.29 | 2.3% | operator |
| 15384 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 15368 | Hash Join | 1222.98 | 785.60 | 35.8% | operator |
| 15383 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15385 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 15369 | Nested Loop | 1178.25 | 1198.29 | 1.7% | operator |
| 15381 | Hash | 38.72 | 19.97 | 48.4% | operator |
| 15370 | Hash Join | 214.29 | 251.67 | 17.4% | operator |
| 15380 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15382 | Seq Scan | 38.62 | 48.86 | 26.5% | operator |
| 15371 | Seq Scan | 160.22 | 159.67 | 0.3% | operator |
| 15372 | Hash | 35.88 | 43.51 | 21.3% | pattern |
| 15377 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 15378 | Hash | 0.06 | 18.14 | 29644.5% | operator |
| 15379 | Seq Scan | 0.06 | 23.36 | 40179.7% | operator |
...
### Step 2: Node 15377 (Seq Scan) - LEAF
...
### Step 4: Node 15371 (Seq Scan) - LEAF
...
### Step 5: Node 15372 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15363, 15364, 15373, 15374, 15375, 15376
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15380 (Index Scan) - LEAF
...
### Step 8: Node 15382 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9675
  - startup_cost=9920.4900
  - total_cost=70271.9700
- **Output:** st=113.62, rt=785.60

### Step 12: Node 15383 (Index Scan) - LEAF
...
### Step 13: Node 15385 (Seq Scan) - LEAF
...
### Step 14: Node 15367 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.7700
  - total_cost=70522.1700
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15384 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.3400
  - total_cost=70528.3400
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15365 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70568.4100
  - total_cost=70570.4800
- **Output:** st=1044.95, rt=1046.22

### Step 18: Node 15363 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15364, 15372, 15373, 15374, 15375, 15376
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2563
  ...
  - Aggregate_total_cost=71971.0500
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2563
  - GatherMerge_Outer_nt1=827
  ...
  - GatherMerge_Outer_total_cost=71871.6700
- **Output:** st=1123.11, rt=1126.04
