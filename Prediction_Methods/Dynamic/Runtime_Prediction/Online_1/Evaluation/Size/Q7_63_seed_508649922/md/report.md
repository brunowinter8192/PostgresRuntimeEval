...
**Test Query:** Q7_63_seed_508649922
...
```
Node 12463 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12464 (Gather Merge) [consumed]
    Node 12465 (Sort)
      Node 12466 (Hash Join)
        Node 12467 (Nested Loop)
          Node 12468 (Hash Join) [PATTERN: 895c6e8c]
            Node 12469 (Seq Scan) [consumed] - LEAF
            Node 12470 (Hash) [consumed]
              Node 12471 (Hash Join) [PATTERN: f4cb205a]
                Node 12472 (Seq Scan) [consumed] - LEAF
                Node 12473 (Hash) [consumed]
                  Node 12474 (Seq Scan) [consumed] - LEAF
          Node 12475 (Index Scan) - LEAF
        Node 12476 (Hash)
          Node 12477 (Hash Join) [PATTERN: f4cb205a]
            Node 12478 (Seq Scan) [consumed] - LEAF
            Node 12479 (Hash) [consumed]
              Node 12480 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12471 | 12463, 12464, 12468, 12469, 12470, 12472, 12473, 12474, 12477, 12478, 12479, 12480 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12477 | 12463, 12464, 12468, 12469, 12470, 12471, 12472, 12473, 12474, 12478, 12479, 12480 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12463 | 12464, 12468, 12469, 12470, 12471, 12472, 12473, 12474, 12477, 12478, 12479, 12480 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12468 | 12463, 12464, 12469, 12470, 12471, 12472, 12473, 12474, 12477, 12478, 12479, 12480 |
...
|------|------|--------|-----------|-----|--------|
| 12463 | Aggregate | 1203.35 | 1163.13 | 3.3% | pattern |
| 12465 | Sort | 1193.61 | 1109.16 | 7.1% | operator |
| 12466 | Hash Join | 1193.01 | 843.11 | 29.3% | operator |
| 12467 | Nested Loop | 1185.04 | 1215.99 | 2.6% | operator |
| 12476 | Hash | 3.97 | 27.74 | 598.3% | operator |
| 12468 | Hash Join | 224.64 | 304.89 | 35.7% | pattern |
| 12475 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12477 | Hash Join | 3.93 | 429.36 | 10828.0% | pattern |
| 12471 | Hash Join | 35.90 | 429.38 | 1096.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12471 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12463, 12464, 12468, 12469, 12470, 12472, 12473, 12474, 12477, 12478, 12479, 12480
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12468 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12463, 12464, 12469, 12470, 12471, 12472, 12473, 12474, 12477, 12478, 12479, 12480
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12475 (Index Scan) - LEAF
...
### Step 4: Node 12477 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12463, 12464, 12468, 12469, 12470, 12471, 12472, 12473, 12474, 12478, 12479, 12480
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12463 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12464, 12468, 12469, 12470, 12471, 12472, 12473, 12474, 12477, 12478, 12479, 12480
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
