...
**Test Query:** Q5_20_seed_155876589
...
```
Node 8141 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8142 (Aggregate) [consumed]
    Node 8143 (Gather Merge) [consumed]
      Node 8144 (Aggregate)
        Node 8145 (Sort) [PATTERN: 3e2d5a00]
          Node 8146 (Hash Join) [consumed]
            Node 8147 (Nested Loop)
              Node 8148 (Hash Join)
                Node 8149 (Seq Scan) - LEAF
                Node 8150 (Hash) [PATTERN: 444761fb]
                  Node 8151 (Hash Join) [consumed]
                    Node 8152 (Seq Scan) [consumed] - LEAF
                    Node 8153 (Hash) [consumed]
                      Node 8154 (Hash Join) [consumed]
                        Node 8155 (Seq Scan) - LEAF
                        Node 8156 (Hash)
                          Node 8157 (Seq Scan) - LEAF
              Node 8158 (Index Scan) - LEAF
            Node 8159 (Hash)
              Node 8160 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8150 | 8141, 8142, 8143, 8145, 8146, 8151, 8152, 8153, 8154 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8141 | 8142, 8143, 8145, 8146, 8150, 8151, 8152, 8153, 8154 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8145 | 8141, 8142, 8143, 8146, 8150, 8151, 8152, 8153, 8154 |
...
|------|------|--------|-----------|-----|--------|
| 8141 | Sort | 1068.55 | 2934.42 | 174.6% | pattern |
| 8144 | Aggregate | 1063.69 | 1128.78 | 6.1% | operator |
| 8145 | Sort | 1063.45 | 1835.91 | 72.6% | pattern |
| 8147 | Nested Loop | 1051.64 | 1188.39 | 13.0% | operator |
| 8159 | Hash | 3.31 | 18.67 | 464.3% | operator |
| 8148 | Hash Join | 219.59 | 253.07 | 15.2% | operator |
| 8158 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 8160 | Seq Scan | 2.83 | 12.36 | 337.3% | operator |
| 8149 | Seq Scan | 171.09 | 159.81 | 6.6% | operator |
| 8150 | Hash | 35.03 | 46.85 | 33.7% | pattern |
| 8155 | Seq Scan | 0.01 | 11.81 | 117982.7% | operator |
| 8156 | Hash | 0.06 | 21.05 | 35583.5% | operator |
| 8157 | Seq Scan | 0.06 | 22.93 | 40133.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8157 (Seq Scan) - LEAF
...
### Step 2: Node 8155 (Seq Scan) - LEAF
...
### Step 4: Node 8149 (Seq Scan) - LEAF
...
### Step 5: Node 8150 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8141, 8142, 8143, 8145, 8146, 8151, 8152, 8153, 8154
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8158 (Index Scan) - LEAF
...
### Step 8: Node 8160 (Seq Scan) - LEAF
...
### Step 11: Node 8145 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8141, 8142, 8143, 8146, 8150, 8151, 8152, 8153, 8154
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8141 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8142, 8143, 8145, 8146, 8150, 8151, 8152, 8153, 8154
- **Input Features:**
