...
**Test Query:** Q5_66_seed_533262015
...
```
Node 9141 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9142 (Aggregate) [consumed]
    Node 9143 (Gather Merge) [consumed]
      Node 9144 (Aggregate)
        Node 9145 (Sort) [PATTERN: 3e2d5a00]
          Node 9146 (Hash Join) [consumed]
            Node 9147 (Nested Loop)
              Node 9148 (Hash Join)
                Node 9149 (Seq Scan) - LEAF
                Node 9150 (Hash) [PATTERN: 444761fb]
                  Node 9151 (Hash Join) [consumed]
                    Node 9152 (Seq Scan) [consumed] - LEAF
                    Node 9153 (Hash) [consumed]
                      Node 9154 (Hash Join) [consumed]
                        Node 9155 (Seq Scan) - LEAF
                        Node 9156 (Hash)
                          Node 9157 (Seq Scan) - LEAF
              Node 9158 (Index Scan) - LEAF
            Node 9159 (Hash)
              Node 9160 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9150 | 9141, 9142, 9143, 9145, 9146, 9151, 9152, 9153, 9154 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9141 | 9142, 9143, 9145, 9146, 9150, 9151, 9152, 9153, 9154 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9145 | 9141, 9142, 9143, 9146, 9150, 9151, 9152, 9153, 9154 |
...
|------|------|--------|-----------|-----|--------|
| 9141 | Sort | 1054.41 | 2934.42 | 178.3% | pattern |
| 9144 | Aggregate | 1049.87 | 1128.76 | 7.5% | operator |
| 9145 | Sort | 1049.61 | 1835.93 | 74.9% | pattern |
| 9147 | Nested Loop | 1037.94 | 1188.47 | 14.5% | operator |
| 9159 | Hash | 3.09 | 18.67 | 504.6% | operator |
| 9148 | Hash Join | 212.72 | 253.07 | 19.0% | operator |
| 9158 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 9160 | Seq Scan | 2.59 | 12.36 | 377.6% | operator |
| 9149 | Seq Scan | 169.08 | 159.81 | 5.5% | operator |
| 9150 | Hash | 31.01 | 46.85 | 51.1% | pattern |
| 9155 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 9156 | Hash | 0.13 | 21.05 | 16347.8% | operator |
| 9157 | Seq Scan | 0.13 | 22.93 | 18101.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9157 (Seq Scan) - LEAF
...
### Step 2: Node 9155 (Seq Scan) - LEAF
...
### Step 4: Node 9149 (Seq Scan) - LEAF
...
### Step 5: Node 9150 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9141, 9142, 9143, 9145, 9146, 9151, 9152, 9153, 9154
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9158 (Index Scan) - LEAF
...
### Step 8: Node 9160 (Seq Scan) - LEAF
...
### Step 11: Node 9145 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9141, 9142, 9143, 9146, 9150, 9151, 9152, 9153, 9154
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9141 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9142, 9143, 9145, 9146, 9150, 9151, 9152, 9153, 9154
- **Input Features:**
