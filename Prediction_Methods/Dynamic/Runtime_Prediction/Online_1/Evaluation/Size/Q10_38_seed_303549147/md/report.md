...
**Test Query:** Q10_38_seed_303549147
...
```
Node 20423 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20424 (Sort) [consumed]
    Node 20425 (Aggregate) [consumed]
      Node 20426 (Gather) [consumed]
        Node 20427 (Hash Join)
          Node 20428 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20429 (Nested Loop) [consumed]
              Node 20430 (Seq Scan) - LEAF
              Node 20431 (Index Scan) - LEAF
            Node 20432 (Hash) [consumed]
              Node 20433 (Seq Scan) - LEAF
          Node 20434 (Hash)
            Node 20435 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20423 | 20424, 20425, 20426, 20428, 20429, 20432 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20428 | 20423, 20424, 20425, 20426, 20429, 20432 |
...
|------|------|--------|-----------|-----|--------|
| 20423 | Limit | 1157.65 | 1199.47 | 3.6% | pattern |
| 20427 | Hash Join | 1098.88 | 1095.51 | 0.3% | operator |
| 20428 | Hash Join | 1096.00 | 1120.19 | 2.2% | pattern |
| 20434 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 20435 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 20430 | Seq Scan | 161.62 | 159.74 | 1.2% | operator |
| 20431 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20433 | Seq Scan | 29.81 | 26.64 | 10.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20430 (Seq Scan) - LEAF
...
### Step 2: Node 20431 (Index Scan) - LEAF
...
### Step 3: Node 20433 (Seq Scan) - LEAF
...
### Step 4: Node 20435 (Seq Scan) - LEAF
...
### Step 5: Node 20428 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20423, 20424, 20425, 20426, 20429, 20432
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18043
  - HashJoin_nt1=18043
...
### Step 8: Node 20423 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20424, 20425, 20426, 20428, 20429, 20432
- **Input Features:**
