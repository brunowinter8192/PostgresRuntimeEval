...
**Test Query:** Q10_149_seed_1214196588
...
```
Node 20059 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20060 (Sort) [consumed]
    Node 20061 (Aggregate) [consumed]
      Node 20062 (Gather) [consumed]
        Node 20063 (Hash Join)
          Node 20064 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20065 (Nested Loop) [consumed]
              Node 20066 (Seq Scan) - LEAF
              Node 20067 (Index Scan) - LEAF
            Node 20068 (Hash) [consumed]
              Node 20069 (Seq Scan) - LEAF
          Node 20070 (Hash)
            Node 20071 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20059 | 20060, 20061, 20062, 20064, 20065, 20068 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20064 | 20059, 20060, 20061, 20062, 20065, 20068 |
...
|------|------|--------|-----------|-----|--------|
| 20059 | Limit | 1132.78 | 1199.47 | 5.9% | pattern |
| 20063 | Hash Join | 1095.76 | 1095.39 | 0.0% | operator |
| 20064 | Hash Join | 1093.27 | 1120.19 | 2.5% | pattern |
| 20070 | Hash | 0.01 | 15.71 | 130823.1% | operator |
| 20071 | Seq Scan | 0.01 | 6.98 | 77441.2% | operator |
| 20066 | Seq Scan | 166.70 | 159.75 | 4.2% | operator |
| 20067 | Index Scan | 0.06 | -0.52 | 935.8% | operator |
| 20069 | Seq Scan | 29.46 | 26.64 | 9.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20066 (Seq Scan) - LEAF
...
### Step 2: Node 20067 (Index Scan) - LEAF
...
### Step 3: Node 20069 (Seq Scan) - LEAF
...
### Step 4: Node 20071 (Seq Scan) - LEAF
...
### Step 5: Node 20064 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20059, 20060, 20061, 20062, 20065, 20068
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17592
  - HashJoin_nt1=17592
  ...
  - NestedLoop_Outer_nt=17592
  - NestedLoop_Outer_nt1=17775
...
- **Output:** st=54.16, rt=1095.39

### Step 8: Node 20059 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20060, 20061, 20062, 20064, 20065, 20068
- **Input Features:**
  - Aggregate_Outer_np=0
  - Aggregate_Outer_nt=54536
  - Aggregate_Outer_nt1=54536
  - Aggregate_Outer_nt2=0
  - Aggregate_Outer_parallel_workers=0
  - Aggregate_Outer_plan_width=280
  - Aggregate_Outer_reltuples=0.0000
  - Aggregate_Outer_sel=1.0000
  - Aggregate_Outer_startup_cost=87437.1500
  - Aggregate_Outer_total_cost=88118.8500
  - Gather_Outer_np=0
