...
**Test Query:** Q10_57_seed_459425736
...
```
Node 20696 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20697 (Sort) [consumed]
    Node 20698 (Aggregate) [consumed]
      Node 20699 (Gather) [consumed]
        Node 20700 (Hash Join)
          Node 20701 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20702 (Nested Loop) [consumed]
              Node 20703 (Seq Scan) - LEAF
              Node 20704 (Index Scan) - LEAF
            Node 20705 (Hash) [consumed]
              Node 20706 (Seq Scan) - LEAF
          Node 20707 (Hash)
            Node 20708 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20696 | 20697, 20698, 20699, 20701, 20702, 20705 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20701 | 20696, 20697, 20698, 20699, 20702, 20705 |
...
|------|------|--------|-----------|-----|--------|
| 20696 | Limit | 1141.47 | 1199.47 | 5.1% | pattern |
| 20700 | Hash Join | 1080.65 | 1095.53 | 1.4% | operator |
| 20701 | Hash Join | 1077.67 | 1120.19 | 3.9% | pattern |
| 20707 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 20708 | Seq Scan | 0.01 | 6.98 | 53582.3% | operator |
| 20703 | Seq Scan | 160.67 | 159.74 | 0.6% | operator |
| 20704 | Index Scan | 0.06 | -0.52 | 949.6% | operator |
| 20706 | Seq Scan | 27.21 | 26.64 | 2.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20703 (Seq Scan) - LEAF
...
### Step 2: Node 20704 (Index Scan) - LEAF
...
### Step 3: Node 20706 (Seq Scan) - LEAF
...
### Step 4: Node 20708 (Seq Scan) - LEAF
...
### Step 5: Node 20701 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20696, 20697, 20698, 20699, 20702, 20705
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18089
  - HashJoin_nt1=18089
...
### Step 8: Node 20696 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20697, 20698, 20699, 20701, 20702, 20705
- **Input Features:**
