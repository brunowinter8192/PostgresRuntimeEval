...
**Test Query:** Q5_38_seed_303549147
...
```
Node 8521 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8522 (Aggregate) [consumed]
    Node 8523 (Gather Merge) [consumed]
      Node 8524 (Aggregate)
        Node 8525 (Sort) [PATTERN: 3e2d5a00]
          Node 8526 (Hash Join) [consumed]
            Node 8527 (Nested Loop)
              Node 8528 (Hash Join)
                Node 8529 (Seq Scan) - LEAF
                Node 8530 (Hash) [PATTERN: 444761fb]
                  Node 8531 (Hash Join) [consumed]
                    Node 8532 (Seq Scan) [consumed] - LEAF
                    Node 8533 (Hash) [consumed]
                      Node 8534 (Hash Join) [consumed]
                        Node 8535 (Seq Scan) - LEAF
                        Node 8536 (Hash)
                          Node 8537 (Seq Scan) - LEAF
              Node 8538 (Index Scan) - LEAF
            Node 8539 (Hash)
              Node 8540 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8530 | 8521, 8522, 8523, 8525, 8526, 8531, 8532, 8533, 8534 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8521 | 8522, 8523, 8525, 8526, 8530, 8531, 8532, 8533, 8534 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8525 | 8521, 8522, 8523, 8526, 8530, 8531, 8532, 8533, 8534 |
...
|------|------|--------|-----------|-----|--------|
| 8521 | Sort | 1061.13 | 2934.42 | 176.5% | pattern |
| 8524 | Aggregate | 1055.57 | 1128.78 | 6.9% | operator |
| 8525 | Sort | 1055.31 | 1835.92 | 74.0% | pattern |
| 8527 | Nested Loop | 1043.51 | 1188.42 | 13.9% | operator |
| 8539 | Hash | 2.88 | 18.67 | 548.3% | operator |
| 8528 | Hash Join | 218.98 | 253.07 | 15.6% | operator |
| 8538 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 8540 | Seq Scan | 2.24 | 12.36 | 452.5% | operator |
| 8529 | Seq Scan | 168.94 | 159.81 | 5.4% | operator |
| 8530 | Hash | 36.17 | 46.85 | 29.5% | pattern |
| 8535 | Seq Scan | 0.01 | 11.81 | 147503.4% | operator |
| 8536 | Hash | 0.16 | 21.05 | 13058.3% | operator |
| 8537 | Seq Scan | 0.16 | 22.93 | 14600.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8537 (Seq Scan) - LEAF
...
### Step 2: Node 8535 (Seq Scan) - LEAF
...
### Step 4: Node 8529 (Seq Scan) - LEAF
...
### Step 5: Node 8530 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8521, 8522, 8523, 8525, 8526, 8531, 8532, 8533, 8534
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8538 (Index Scan) - LEAF
...
### Step 8: Node 8540 (Seq Scan) - LEAF
...
### Step 11: Node 8525 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8521, 8522, 8523, 8526, 8530, 8531, 8532, 8533, 8534
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8521 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8522, 8523, 8525, 8526, 8530, 8531, 8532, 8533, 8534
- **Input Features:**
