...
**Test Query:** Q8_134_seed_1091136123
...
```
Node 14052 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14053 (Gather Merge) [consumed]
    Node 14054 (Sort)
      Node 14055 (Hash Join)
        Node 14056 (Nested Loop)
          Node 14057 (Hash Join)
            Node 14058 (Nested Loop)
              Node 14059 (Hash Join)
                Node 14060 (Seq Scan) - LEAF
                Node 14061 (Hash) [PATTERN: 444761fb]
                  Node 14062 (Hash Join) [consumed]
                    Node 14063 (Seq Scan) [consumed] - LEAF
                    Node 14064 (Hash) [consumed]
                      Node 14065 (Hash Join) [consumed]
                        Node 14066 (Seq Scan) - LEAF
                        Node 14067 (Hash)
                          Node 14068 (Seq Scan) - LEAF
              Node 14069 (Index Scan) - LEAF
            Node 14070 (Hash)
              Node 14071 (Seq Scan) - LEAF
          Node 14072 (Index Scan) - LEAF
        Node 14073 (Hash)
          Node 14074 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14061 | 14052, 14053, 14062, 14063, 14064, 14065 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14052 | 14053, 14061, 14062, 14063, 14064, 14065 |
...
|------|------|--------|-----------|-----|--------|
| 14052 | Aggregate | 1238.79 | 1128.80 | 8.9% | pattern |
| 14054 | Sort | 1231.53 | 1046.21 | 15.0% | operator |
| 14055 | Hash Join | 1231.28 | 785.61 | 36.2% | operator |
| 14056 | Nested Loop | 1230.97 | 1198.29 | 2.7% | operator |
| 14073 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 14057 | Hash Join | 1227.33 | 785.60 | 36.0% | operator |
| 14072 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14074 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 14058 | Nested Loop | 1181.36 | 1198.29 | 1.4% | operator |
| 14070 | Hash | 41.47 | 19.97 | 51.8% | operator |
| 14059 | Hash Join | 213.30 | 251.67 | 18.0% | operator |
| 14069 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14071 | Seq Scan | 41.34 | 48.87 | 18.2% | operator |
| 14060 | Seq Scan | 157.04 | 159.67 | 1.7% | operator |
| 14061 | Hash | 38.93 | 43.51 | 11.8% | pattern |
| 14066 | Seq Scan | 0.02 | 12.22 | 64241.8% | operator |
| 14067 | Hash | 0.17 | 18.14 | 10387.9% | operator |
| 14068 | Seq Scan | 0.17 | 23.36 | 13723.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14068 (Seq Scan) - LEAF
...
### Step 2: Node 14066 (Seq Scan) - LEAF
...
### Step 4: Node 14060 (Seq Scan) - LEAF
...
### Step 5: Node 14061 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14052, 14053, 14062, 14063, 14064, 14065
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14069 (Index Scan) - LEAF
...
### Step 8: Node 14071 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - total_cost=70271.7900
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14072 (Index Scan) - LEAF
...
### Step 13: Node 14074 (Seq Scan) - LEAF
...
### Step 14: Node 14056 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.6000
  - total_cost=70516.2500
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14073 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.1600
  - total_cost=70522.3100
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14054 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70561.3300
  - total_cost=70563.3500
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14052 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14053, 14061, 14062, 14063, 14064, 14065
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2504
  ...
  - Aggregate_sel=0.9609
  - Aggregate_startup_cost=71561.3700
  - Aggregate_total_cost=71955.8100
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2504
  - GatherMerge_Outer_nt1=808
  ...
  - GatherMerge_Outer_startup_cost=71561.3700
  - GatherMerge_Outer_total_cost=71857.6100
- **Output:** st=1125.85, rt=1128.80
