...
**Test Query:** Q10_4_seed_24612093
...
```
Node 20592 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20593 (Sort) [consumed]
    Node 20594 (Aggregate) [consumed]
      Node 20595 (Gather) [consumed]
        Node 20596 (Hash Join)
          Node 20597 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20598 (Nested Loop) [consumed]
              Node 20599 (Seq Scan) - LEAF
              Node 20600 (Index Scan) - LEAF
            Node 20601 (Hash) [consumed]
              Node 20602 (Seq Scan) - LEAF
          Node 20603 (Hash)
            Node 20604 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20592 | 20593, 20594, 20595, 20597, 20598, 20601 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20597 | 20592, 20593, 20594, 20595, 20598, 20601 |
...
|------|------|--------|-----------|-----|--------|
| 20592 | Limit | 1144.52 | 1199.47 | 4.8% | pattern |
| 20596 | Hash Join | 1092.59 | 1095.48 | 0.3% | operator |
| 20597 | Hash Join | 1089.16 | 1120.19 | 2.8% | pattern |
| 20603 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20604 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20599 | Seq Scan | 163.47 | 159.74 | 2.3% | operator |
| 20600 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20602 | Seq Scan | 28.82 | 26.64 | 7.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20599 (Seq Scan) - LEAF
...
### Step 2: Node 20600 (Index Scan) - LEAF
...
### Step 3: Node 20602 (Seq Scan) - LEAF
...
### Step 4: Node 20604 (Seq Scan) - LEAF
...
### Step 5: Node 20597 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20592, 20593, 20594, 20595, 20598, 20601
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17901
  - HashJoin_nt1=17901
...
### Step 8: Node 20592 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20593, 20594, 20595, 20597, 20598, 20601
- **Input Features:**
