...
**Test Query:** Q10_34_seed_270733023
...
```
Node 20371 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20372 (Sort) [consumed]
    Node 20373 (Aggregate) [consumed]
      Node 20374 (Gather) [consumed]
        Node 20375 (Hash Join)
          Node 20376 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20377 (Nested Loop) [consumed]
              Node 20378 (Seq Scan) - LEAF
              Node 20379 (Index Scan) - LEAF
            Node 20380 (Hash) [consumed]
              Node 20381 (Seq Scan) - LEAF
          Node 20382 (Hash)
            Node 20383 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20371 | 20372, 20373, 20374, 20376, 20377, 20380 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20376 | 20371, 20372, 20373, 20374, 20377, 20380 |
...
|------|------|--------|-----------|-----|--------|
| 20371 | Limit | 1152.05 | 1199.47 | 4.1% | pattern |
| 20375 | Hash Join | 1102.10 | 1095.46 | 0.6% | operator |
| 20376 | Hash Join | 1099.14 | 1120.19 | 1.9% | pattern |
| 20382 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 20383 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20378 | Seq Scan | 161.58 | 159.74 | 1.1% | operator |
| 20379 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20381 | Seq Scan | 28.18 | 26.64 | 5.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20378 (Seq Scan) - LEAF
...
### Step 2: Node 20379 (Index Scan) - LEAF
...
### Step 3: Node 20381 (Seq Scan) - LEAF
...
### Step 4: Node 20383 (Seq Scan) - LEAF
...
### Step 5: Node 20376 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20371, 20372, 20373, 20374, 20377, 20380
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17827
  - HashJoin_nt1=17827
...
### Step 8: Node 20371 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20372, 20373, 20374, 20376, 20377, 20380
- **Input Features:**
