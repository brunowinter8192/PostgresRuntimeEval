...
**Test Query:** Q5_81_seed_656322480
...
```
Node 9481 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9482 (Aggregate) [consumed]
    Node 9483 (Gather Merge) [consumed]
      Node 9484 (Aggregate)
        Node 9485 (Sort) [PATTERN: 3e2d5a00]
          Node 9486 (Hash Join) [consumed]
            Node 9487 (Nested Loop)
              Node 9488 (Hash Join)
                Node 9489 (Seq Scan) - LEAF
                Node 9490 (Hash) [PATTERN: 444761fb]
                  Node 9491 (Hash Join) [consumed]
                    Node 9492 (Seq Scan) [consumed] - LEAF
                    Node 9493 (Hash) [consumed]
                      Node 9494 (Hash Join) [consumed]
                        Node 9495 (Seq Scan) - LEAF
                        Node 9496 (Hash)
                          Node 9497 (Seq Scan) - LEAF
              Node 9498 (Index Scan) - LEAF
            Node 9499 (Hash)
              Node 9500 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9490 | 9481, 9482, 9483, 9485, 9486, 9491, 9492, 9493, 9494 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9481 | 9482, 9483, 9485, 9486, 9490, 9491, 9492, 9493, 9494 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9485 | 9481, 9482, 9483, 9486, 9490, 9491, 9492, 9493, 9494 |
...
|------|------|--------|-----------|-----|--------|
| 9481 | Sort | 1084.10 | 2934.42 | 170.7% | pattern |
| 9484 | Aggregate | 1078.96 | 1128.78 | 4.6% | operator |
| 9485 | Sort | 1078.70 | 1835.92 | 70.2% | pattern |
| 9487 | Nested Loop | 1065.53 | 1188.42 | 11.5% | operator |
| 9499 | Hash | 2.98 | 18.67 | 526.8% | operator |
| 9488 | Hash Join | 220.49 | 253.07 | 14.8% | operator |
| 9498 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 9500 | Seq Scan | 2.44 | 12.36 | 406.5% | operator |
| 9489 | Seq Scan | 168.73 | 159.81 | 5.3% | operator |
| 9490 | Hash | 38.17 | 46.85 | 22.7% | pattern |
| 9495 | Seq Scan | 0.01 | 11.81 | 147503.4% | operator |
| 9496 | Hash | 0.48 | 21.05 | 4258.8% | operator |
| 9497 | Seq Scan | 0.48 | 22.93 | 4677.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9497 (Seq Scan) - LEAF
...
### Step 2: Node 9495 (Seq Scan) - LEAF
...
### Step 4: Node 9489 (Seq Scan) - LEAF
...
### Step 5: Node 9490 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9481, 9482, 9483, 9485, 9486, 9491, 9492, 9493, 9494
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9498 (Index Scan) - LEAF
...
### Step 8: Node 9500 (Seq Scan) - LEAF
...
### Step 11: Node 9485 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9481, 9482, 9483, 9486, 9490, 9491, 9492, 9493, 9494
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9481 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9482, 9483, 9485, 9486, 9490, 9491, 9492, 9493, 9494
- **Input Features:**
