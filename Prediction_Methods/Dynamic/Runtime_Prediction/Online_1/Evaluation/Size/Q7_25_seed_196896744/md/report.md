        ...
        Node 11711 (Nested Loop)
          Node 11712 (Hash Join) [PATTERN: 895c6e8c]
            ...
            Node 11714 (Hash) [consumed]
              Node 11715 (Hash Join) [PATTERN: f4cb205a]
                Node 11716 (Seq Scan) [consumed] - LEAF
                Node 11717 (Hash) [consumed]
                  Node 11718 (Seq Scan) [consumed] - LEAF
          Node 11719 (Index Scan) - LEAF
        Node 11720 (Hash)
          Node 11721 (Hash Join) [PATTERN: f4cb205a]
            Node 11722 (Seq Scan) [consumed] - LEAF
            Node 11723 (Hash) [consumed]
              Node 11724 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11715 | 11707, 11708, 11712, 11713, 11714, 11716, 11717, 11718, 11721, 11722, 11723, 11724 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11721 | 11707, 11708, 11712, 11713, 11714, 11715, 11716, 11717, 11718, 11722, 11723, 11724 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11707 | 11708, 11712, 11713, 11714, 11715, 11716, 11717, 11718, 11721, 11722, 11723, 11724 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11712 | 11707, 11708, 11713, 11714, 11715, 11716, 11717, 11718, 11721, 11722, 11723, 11724 |
...
|------|------|--------|-----------|-----|--------|
| 11707 | Aggregate | 1212.29 | 1163.13 | 4.1% | pattern |
| 11709 | Sort | 1204.48 | 1109.16 | 7.9% | operator |
| 11710 | Hash Join | 1203.82 | 843.11 | 30.0% | operator |
| 11711 | Nested Loop | 1196.53 | 1215.99 | 1.6% | operator |
| 11720 | Hash | 3.58 | 27.74 | 674.5% | operator |
| 11712 | Hash Join | 207.12 | 304.89 | 47.2% | pattern |
| 11719 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11721 | Hash Join | 3.51 | 429.36 | 12143.0% | pattern |
| 11715 | Hash Join | 32.01 | 429.38 | 1241.4% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11715 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11707, 11708, 11712, 11713, 11714, 11716, 11717, 11718, 11721, 11722, 11723, 11724
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11712 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11707, 11708, 11713, 11714, 11715, 11716, 11717, 11718, 11721, 11722, 11723, 11724
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11719 (Index Scan) - LEAF
...
### Step 4: Node 11721 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11707, 11708, 11712, 11713, 11714, 11715, 11716, 11717, 11718, 11722, 11723, 11724
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11707 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11708, 11712, 11713, 11714, 11715, 11716, 11717, 11718, 11721, 11722, 11723, 11724
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
