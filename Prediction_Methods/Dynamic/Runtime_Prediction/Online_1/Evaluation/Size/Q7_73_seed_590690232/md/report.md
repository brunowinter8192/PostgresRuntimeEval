...
**Test Query:** Q7_73_seed_590690232
...
```
Node 12661 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12662 (Gather Merge) [consumed]
    Node 12663 (Sort)
      Node 12664 (Hash Join)
        Node 12665 (Nested Loop)
          Node 12666 (Hash Join) [PATTERN: 895c6e8c]
            Node 12667 (Seq Scan) [consumed] - LEAF
            Node 12668 (Hash) [consumed]
              Node 12669 (Hash Join) [PATTERN: f4cb205a]
                Node 12670 (Seq Scan) [consumed] - LEAF
                Node 12671 (Hash) [consumed]
                  Node 12672 (Seq Scan) [consumed] - LEAF
          Node 12673 (Index Scan) - LEAF
        Node 12674 (Hash)
          Node 12675 (Hash Join) [PATTERN: f4cb205a]
            Node 12676 (Seq Scan) [consumed] - LEAF
            Node 12677 (Hash) [consumed]
              Node 12678 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12669 | 12661, 12662, 12666, 12667, 12668, 12670, 12671, 12672, 12675, 12676, 12677, 12678 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12675 | 12661, 12662, 12666, 12667, 12668, 12669, 12670, 12671, 12672, 12676, 12677, 12678 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12661 | 12662, 12666, 12667, 12668, 12669, 12670, 12671, 12672, 12675, 12676, 12677, 12678 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12666 | 12661, 12662, 12667, 12668, 12669, 12670, 12671, 12672, 12675, 12676, 12677, 12678 |
...
|------|------|--------|-----------|-----|--------|
| 12661 | Aggregate | 1212.72 | 1163.13 | 4.1% | pattern |
| 12663 | Sort | 1203.61 | 1109.16 | 7.8% | operator |
| 12664 | Hash Join | 1202.98 | 843.11 | 29.9% | operator |
| 12665 | Nested Loop | 1196.49 | 1215.99 | 1.6% | operator |
| 12674 | Hash | 2.72 | 27.74 | 920.7% | operator |
| 12666 | Hash Join | 211.42 | 304.89 | 44.2% | pattern |
| 12673 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12675 | Hash Join | 2.67 | 429.36 | 15969.0% | pattern |
| 12669 | Hash Join | 33.78 | 429.38 | 1171.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12669 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12661, 12662, 12666, 12667, 12668, 12670, 12671, 12672, 12675, 12676, 12677, 12678
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12666 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12661, 12662, 12667, 12668, 12669, 12670, 12671, 12672, 12675, 12676, 12677, 12678
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12673 (Index Scan) - LEAF
...
### Step 4: Node 12675 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12661, 12662, 12666, 12667, 12668, 12669, 12670, 12671, 12672, 12676, 12677, 12678
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12661 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12662, 12666, 12667, 12668, 12669, 12670, 12671, 12672, 12675, 12676, 12677, 12678
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
