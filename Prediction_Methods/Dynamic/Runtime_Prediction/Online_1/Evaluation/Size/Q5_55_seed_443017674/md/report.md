...
**Test Query:** Q5_55_seed_443017674
...
```
Node 8901 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8902 (Aggregate) [consumed]
    Node 8903 (Gather Merge) [consumed]
      Node 8904 (Aggregate)
        Node 8905 (Sort) [PATTERN: 3e2d5a00]
          Node 8906 (Hash Join) [consumed]
            Node 8907 (Nested Loop)
              Node 8908 (Hash Join)
                Node 8909 (Seq Scan) - LEAF
                Node 8910 (Hash) [PATTERN: 444761fb]
                  Node 8911 (Hash Join) [consumed]
                    Node 8912 (Seq Scan) [consumed] - LEAF
                    Node 8913 (Hash) [consumed]
                      Node 8914 (Hash Join) [consumed]
                        Node 8915 (Seq Scan) - LEAF
                        Node 8916 (Hash)
                          Node 8917 (Seq Scan) - LEAF
              Node 8918 (Index Scan) - LEAF
            Node 8919 (Hash)
              Node 8920 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8910 | 8901, 8902, 8903, 8905, 8906, 8911, 8912, 8913, 8914 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8901 | 8902, 8903, 8905, 8906, 8910, 8911, 8912, 8913, 8914 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8905 | 8901, 8902, 8903, 8906, 8910, 8911, 8912, 8913, 8914 |
...
|------|------|--------|-----------|-----|--------|
| 8901 | Sort | 1081.04 | 2934.42 | 171.4% | pattern |
| 8904 | Aggregate | 1075.96 | 1128.79 | 4.9% | operator |
| 8905 | Sort | 1075.71 | 1835.90 | 70.7% | pattern |
| 8907 | Nested Loop | 1061.60 | 1188.37 | 11.9% | operator |
| 8919 | Hash | 5.48 | 18.67 | 240.5% | operator |
| 8908 | Hash Join | 214.34 | 253.07 | 18.1% | operator |
| 8918 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 8920 | Seq Scan | 4.97 | 12.36 | 148.7% | operator |
| 8909 | Seq Scan | 167.00 | 159.81 | 4.3% | operator |
| 8910 | Hash | 34.57 | 46.85 | 35.5% | pattern |
| 8915 | Seq Scan | 0.01 | 11.81 | 168589.6% | operator |
| 8916 | Hash | 0.06 | 21.05 | 36198.7% | operator |
| 8917 | Seq Scan | 0.05 | 22.93 | 42369.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8917 (Seq Scan) - LEAF
...
### Step 2: Node 8915 (Seq Scan) - LEAF
...
### Step 4: Node 8909 (Seq Scan) - LEAF
...
### Step 5: Node 8910 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8901, 8902, 8903, 8905, 8906, 8911, 8912, 8913, 8914
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8918 (Index Scan) - LEAF
...
### Step 8: Node 8920 (Seq Scan) - LEAF
...
### Step 11: Node 8905 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8901, 8902, 8903, 8906, 8910, 8911, 8912, 8913, 8914
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8901 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8902, 8903, 8905, 8906, 8910, 8911, 8912, 8913, 8914
- **Input Features:**
