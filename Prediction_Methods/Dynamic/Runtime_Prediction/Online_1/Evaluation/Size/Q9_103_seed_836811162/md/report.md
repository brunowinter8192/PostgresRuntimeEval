...
**Test Query:** Q9_103_seed_836811162
...
```
Node 16706 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 16707 (Aggregate) [consumed]
    Node 16708 (Gather)
      Node 16709 (Aggregate)
        Node 16710 (Nested Loop)
          Node 16711 (Hash Join) [PATTERN: bb930825]
            Node 16712 (Seq Scan) [consumed] - LEAF
            Node 16713 (Hash) [consumed]
              Node 16714 (Hash Join) [consumed]
                Node 16715 (Hash Join)
                  Node 16716 (Nested Loop)
                    Node 16717 (Seq Scan) - LEAF
                    Node 16718 (Index Scan) - LEAF
                  Node 16719 (Hash)
                    Node 16720 (Seq Scan) - LEAF
                Node 16721 (Hash)
                  Node 16722 (Seq Scan) - LEAF
          Node 16723 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | bb930825 | 16711 | 16706, 16707, 16712, 16713, 16714 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 16706 | 16707, 16711, 16712, 16713, 16714 |
...
|------|------|--------|-----------|-----|--------|
| 16706 | Sort | 1189.79 | 1046.01 | 12.1% | pattern |
| 16708 | Gather | 1188.52 | 955.76 | 19.6% | operator |
| 16709 | Aggregate | 1169.43 | 1141.46 | 2.4% | operator |
| 16710 | Nested Loop | 1145.30 | 1252.17 | 9.3% | operator |
| 16711 | Hash Join | 973.75 | 134.80 | 86.2% | pattern |
| 16723 | Index Scan | 0.00 | -5.54 | 184812.7% | operator |
| 16715 | Hash Join | 162.34 | 837.43 | 415.8% | operator |
| 16721 | Hash | 13.35 | 14.95 | 12.0% | operator |
| 16716 | Nested Loop | 155.75 | 1251.76 | 703.7% | operator |
| 16719 | Hash | 4.58 | 15.94 | 248.4% | operator |
| 16722 | Seq Scan | 13.34 | 2.94 | 78.0% | operator |
| 16717 | Seq Scan | 18.84 | 44.44 | 135.9% | operator |
| 16718 | Index Scan | 0.08 | -1.39 | 1903.2% | operator |
| 16720 | Seq Scan | 4.08 | 9.45 | 131.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16717 (Seq Scan) - LEAF
...
### Step 2: Node 16718 (Index Scan) - LEAF
...
### Step 3: Node 16720 (Seq Scan) - LEAF
...
### Step 6: Node 16722 (Seq Scan) - LEAF
...
### Step 9: Node 16711 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** bb930825 (Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)])
- **Consumes:** Nodes 16706, 16707, 16712, 16713, 16714
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 16723 (Index Scan) - LEAF
...
### Step 11: Node 16710 (Nested Loop)
...
### Step 14: Node 16706 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 16707, 16711, 16712, 16713, 16714
- **Input Features:**
