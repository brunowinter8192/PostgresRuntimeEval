...
**Test Query:** Q7_1_seed_0101000000
...
```
Node 11599 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11600 (Gather Merge) [consumed]
    Node 11601 (Sort)
      Node 11602 (Hash Join)
        Node 11603 (Nested Loop)
          Node 11604 (Hash Join) [PATTERN: 895c6e8c]
            Node 11605 (Seq Scan) [consumed] - LEAF
            Node 11606 (Hash) [consumed]
              Node 11607 (Hash Join) [PATTERN: f4cb205a]
                Node 11608 (Seq Scan) [consumed] - LEAF
                Node 11609 (Hash) [consumed]
                  Node 11610 (Seq Scan) [consumed] - LEAF
          Node 11611 (Index Scan) - LEAF
        Node 11612 (Hash)
          Node 11613 (Hash Join) [PATTERN: f4cb205a]
            Node 11614 (Seq Scan) [consumed] - LEAF
            Node 11615 (Hash) [consumed]
              Node 11616 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11607 | 11599, 11600, 11604, 11605, 11606, 11608, 11609, 11610, 11613, 11614, 11615, 11616 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11613 | 11599, 11600, 11604, 11605, 11606, 11607, 11608, 11609, 11610, 11614, 11615, 11616 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11599 | 11600, 11604, 11605, 11606, 11607, 11608, 11609, 11610, 11613, 11614, 11615, 11616 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11604 | 11599, 11600, 11605, 11606, 11607, 11608, 11609, 11610, 11613, 11614, 11615, 11616 |
...
|------|------|--------|-----------|-----|--------|
| 11599 | Aggregate | 1167.08 | 1163.13 | 0.3% | pattern |
| 11601 | Sort | 1160.36 | 1109.16 | 4.4% | operator |
| 11602 | Hash Join | 1159.77 | 843.11 | 27.3% | operator |
| 11603 | Nested Loop | 1152.99 | 1215.99 | 5.5% | operator |
| 11612 | Hash | 3.13 | 27.74 | 787.2% | operator |
| 11604 | Hash Join | 212.14 | 304.89 | 43.7% | pattern |
| 11611 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 11613 | Hash Join | 3.08 | 429.36 | 13840.4% | pattern |
| 11607 | Hash Join | 32.33 | 429.38 | 1228.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11607 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11599, 11600, 11604, 11605, 11606, 11608, 11609, 11610, 11613, 11614, 11615, 11616
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11604 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11599, 11600, 11605, 11606, 11607, 11608, 11609, 11610, 11613, 11614, 11615, 11616
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11611 (Index Scan) - LEAF
...
### Step 4: Node 11613 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11599, 11600, 11604, 11605, 11606, 11607, 11608, 11609, 11610, 11614, 11615, 11616
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11599 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11600, 11604, 11605, 11606, 11607, 11608, 11609, 11610, 11613, 11614, 11615, 11616
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
