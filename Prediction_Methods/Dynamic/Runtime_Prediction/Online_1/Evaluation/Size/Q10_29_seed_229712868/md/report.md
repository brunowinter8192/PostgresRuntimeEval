...
**Test Query:** Q10_29_seed_229712868
...
```
Node 20293 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20294 (Sort) [consumed]
    Node 20295 (Aggregate) [consumed]
      Node 20296 (Gather) [consumed]
        Node 20297 (Hash Join)
          Node 20298 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20299 (Nested Loop) [consumed]
              Node 20300 (Seq Scan) - LEAF
              Node 20301 (Index Scan) - LEAF
            Node 20302 (Hash) [consumed]
              Node 20303 (Seq Scan) - LEAF
          Node 20304 (Hash)
            Node 20305 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20293 | 20294, 20295, 20296, 20298, 20299, 20302 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20298 | 20293, 20294, 20295, 20296, 20299, 20302 |
...
|------|------|--------|-----------|-----|--------|
| 20293 | Limit | 1166.33 | 1199.47 | 2.8% | pattern |
| 20297 | Hash Join | 1104.47 | 1095.50 | 0.8% | operator |
| 20298 | Hash Join | 1101.58 | 1120.19 | 1.7% | pattern |
| 20304 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20305 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 20300 | Seq Scan | 161.84 | 159.74 | 1.3% | operator |
| 20301 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20303 | Seq Scan | 25.05 | 26.64 | 6.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20300 (Seq Scan) - LEAF
...
### Step 2: Node 20301 (Index Scan) - LEAF
...
### Step 3: Node 20303 (Seq Scan) - LEAF
...
### Step 4: Node 20305 (Seq Scan) - LEAF
...
### Step 5: Node 20298 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20293, 20294, 20295, 20296, 20299, 20302
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17978
  - HashJoin_nt1=17978
...
### Step 8: Node 20293 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20294, 20295, 20296, 20298, 20299, 20302
- **Input Features:**
