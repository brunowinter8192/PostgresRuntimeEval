...
**Test Query:** Q7_78_seed_631710387
...
```
Node 12751 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12752 (Gather Merge) [consumed]
    Node 12753 (Sort)
      Node 12754 (Hash Join)
        Node 12755 (Nested Loop)
          Node 12756 (Hash Join) [PATTERN: 895c6e8c]
            Node 12757 (Seq Scan) [consumed] - LEAF
            Node 12758 (Hash) [consumed]
              Node 12759 (Hash Join) [PATTERN: f4cb205a]
                Node 12760 (Seq Scan) [consumed] - LEAF
                Node 12761 (Hash) [consumed]
                  Node 12762 (Seq Scan) [consumed] - LEAF
          Node 12763 (Index Scan) - LEAF
        Node 12764 (Hash)
          Node 12765 (Hash Join) [PATTERN: f4cb205a]
            Node 12766 (Seq Scan) [consumed] - LEAF
            Node 12767 (Hash) [consumed]
              Node 12768 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12759 | 12751, 12752, 12756, 12757, 12758, 12760, 12761, 12762, 12765, 12766, 12767, 12768 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12765 | 12751, 12752, 12756, 12757, 12758, 12759, 12760, 12761, 12762, 12766, 12767, 12768 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12751 | 12752, 12756, 12757, 12758, 12759, 12760, 12761, 12762, 12765, 12766, 12767, 12768 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12756 | 12751, 12752, 12757, 12758, 12759, 12760, 12761, 12762, 12765, 12766, 12767, 12768 |
...
|------|------|--------|-----------|-----|--------|
| 12751 | Aggregate | 1221.62 | 1163.13 | 4.8% | pattern |
| 12753 | Sort | 1214.63 | 1109.16 | 8.7% | operator |
| 12754 | Hash Join | 1214.03 | 843.11 | 30.6% | operator |
| 12755 | Nested Loop | 1206.03 | 1215.99 | 0.8% | operator |
| 12764 | Hash | 4.10 | 27.74 | 577.0% | operator |
| 12756 | Hash Join | 217.14 | 304.89 | 40.4% | pattern |
| 12763 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12765 | Hash Join | 4.05 | 429.36 | 10488.5% | pattern |
| 12759 | Hash Join | 36.20 | 429.38 | 1086.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12759 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12751, 12752, 12756, 12757, 12758, 12760, 12761, 12762, 12765, 12766, 12767, 12768
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12756 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12751, 12752, 12757, 12758, 12759, 12760, 12761, 12762, 12765, 12766, 12767, 12768
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12763 (Index Scan) - LEAF
...
### Step 4: Node 12765 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12751, 12752, 12756, 12757, 12758, 12759, 12760, 12761, 12762, 12766, 12767, 12768
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12751 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12752, 12756, 12757, 12758, 12759, 12760, 12761, 12762, 12765, 12766, 12767, 12768
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
