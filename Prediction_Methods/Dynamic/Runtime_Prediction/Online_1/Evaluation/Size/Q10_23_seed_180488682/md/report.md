...
**Test Query:** Q10_23_seed_180488682
...
```
Node 20215 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20216 (Sort) [consumed]
    Node 20217 (Aggregate) [consumed]
      Node 20218 (Gather) [consumed]
        Node 20219 (Hash Join)
          Node 20220 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20221 (Nested Loop) [consumed]
              Node 20222 (Seq Scan) - LEAF
              Node 20223 (Index Scan) - LEAF
            Node 20224 (Hash) [consumed]
              Node 20225 (Seq Scan) - LEAF
          Node 20226 (Hash)
            Node 20227 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20215 | 20216, 20217, 20218, 20220, 20221, 20224 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20220 | 20215, 20216, 20217, 20218, 20221, 20224 |
...
|------|------|--------|-----------|-----|--------|
| 20215 | Limit | 1143.11 | 1199.47 | 4.9% | pattern |
| 20219 | Hash Join | 1099.54 | 1095.47 | 0.4% | operator |
| 20220 | Hash Join | 1096.71 | 1120.19 | 2.1% | pattern |
| 20226 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 20227 | Seq Scan | 0.01 | 6.98 | 53582.3% | operator |
| 20222 | Seq Scan | 162.66 | 159.74 | 1.8% | operator |
| 20223 | Index Scan | 0.06 | -0.52 | 909.7% | operator |
| 20225 | Seq Scan | 28.65 | 26.64 | 7.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20222 (Seq Scan) - LEAF
...
### Step 2: Node 20223 (Index Scan) - LEAF
...
### Step 3: Node 20225 (Seq Scan) - LEAF
...
### Step 4: Node 20227 (Seq Scan) - LEAF
...
### Step 5: Node 20220 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20215, 20216, 20217, 20218, 20221, 20224
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17881
  - HashJoin_nt1=17881
...
### Step 8: Node 20215 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20216, 20217, 20218, 20220, 20221, 20224
- **Input Features:**
