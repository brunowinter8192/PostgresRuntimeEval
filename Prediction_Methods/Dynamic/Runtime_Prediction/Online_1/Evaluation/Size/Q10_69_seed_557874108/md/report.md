...
**Test Query:** Q10_69_seed_557874108
...
```
Node 20865 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20866 (Sort) [consumed]
    Node 20867 (Aggregate) [consumed]
      Node 20868 (Gather) [consumed]
        Node 20869 (Hash Join)
          Node 20870 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20871 (Nested Loop) [consumed]
              Node 20872 (Seq Scan) - LEAF
              Node 20873 (Index Scan) - LEAF
            Node 20874 (Hash) [consumed]
              Node 20875 (Seq Scan) - LEAF
          Node 20876 (Hash)
            Node 20877 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20865 | 20866, 20867, 20868, 20870, 20871, 20874 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20870 | 20865, 20866, 20867, 20868, 20871, 20874 |
...
|------|------|--------|-----------|-----|--------|
| 20865 | Limit | 1162.25 | 1199.47 | 3.2% | pattern |
| 20869 | Hash Join | 1110.48 | 1095.60 | 1.3% | operator |
| 20870 | Hash Join | 1107.53 | 1120.19 | 1.1% | pattern |
| 20876 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20877 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20872 | Seq Scan | 164.47 | 159.74 | 2.9% | operator |
| 20873 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20875 | Seq Scan | 26.57 | 26.64 | 0.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20872 (Seq Scan) - LEAF
...
### Step 2: Node 20873 (Index Scan) - LEAF
...
### Step 3: Node 20875 (Seq Scan) - LEAF
...
### Step 4: Node 20877 (Seq Scan) - LEAF
...
### Step 5: Node 20870 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20865, 20866, 20867, 20868, 20871, 20874
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18384
  - HashJoin_nt1=18384
...
### Step 8: Node 20865 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20866, 20867, 20868, 20870, 20871, 20874
- **Input Features:**
