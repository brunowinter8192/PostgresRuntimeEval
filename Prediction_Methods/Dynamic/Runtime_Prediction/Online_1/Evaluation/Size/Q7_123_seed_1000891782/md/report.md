...
**Test Query:** Q7_123_seed_1000891782
**Timestamp:** 2026-01-18 17:46:25
...
```
Node 10951 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10952 (Gather Merge) [consumed]
    Node 10953 (Sort)
      Node 10954 (Hash Join)
        Node 10955 (Nested Loop)
          Node 10956 (Hash Join) [PATTERN: 895c6e8c]
            Node 10957 (Seq Scan) [consumed] - LEAF
            Node 10958 (Hash) [consumed]
              Node 10959 (Hash Join) [PATTERN: f4cb205a]
                Node 10960 (Seq Scan) [consumed] - LEAF
                Node 10961 (Hash) [consumed]
                  Node 10962 (Seq Scan) [consumed] - LEAF
          Node 10963 (Index Scan) - LEAF
        Node 10964 (Hash)
          Node 10965 (Hash Join) [PATTERN: f4cb205a]
            Node 10966 (Seq Scan) [consumed] - LEAF
            Node 10967 (Hash) [consumed]
              Node 10968 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10959 | 10951, 10952, 10956, 10957, 10958, 10960, 10961, 10962, 10965, 10966, 10967, 10968 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10965 | 10951, 10952, 10956, 10957, 10958, 10959, 10960, 10961, 10962, 10966, 10967, 10968 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10951 | 10952, 10956, 10957, 10958, 10959, 10960, 10961, 10962, 10965, 10966, 10967, 10968 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10956 | 10951, 10952, 10957, 10958, 10959, 10960, 10961, 10962, 10965, 10966, 10967, 10968 |
...
|------|------|--------|-----------|-----|--------|
| 10951 | Aggregate | 1245.77 | 1163.13 | 6.6% | pattern |
| 10953 | Sort | 1238.16 | 1109.16 | 10.4% | operator |
| 10954 | Hash Join | 1236.99 | 843.11 | 31.8% | operator |
| 10955 | Nested Loop | 1231.01 | 1215.99 | 1.2% | operator |
| 10964 | Hash | 2.15 | 27.74 | 1188.0% | operator |
| 10956 | Hash Join | 222.08 | 304.89 | 37.3% | pattern |
| 10963 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 10965 | Hash Join | 2.12 | 429.36 | 20200.8% | pattern |
| 10959 | Hash Join | 35.68 | 429.38 | 1103.5% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10959 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10951, 10952, 10956, 10957, 10958, 10960, 10961, 10962, 10965, 10966, 10967, 10968
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10956 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10951, 10952, 10957, 10958, 10959, 10960, 10961, 10962, 10965, 10966, 10967, 10968
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10963 (Index Scan) - LEAF
...
### Step 4: Node 10965 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10951, 10952, 10956, 10957, 10958, 10959, 10960, 10961, 10962, 10966, 10967, 10968
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10951 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10952, 10956, 10957, 10958, 10959, 10960, 10961, 10962, 10965, 10966, 10967, 10968
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
