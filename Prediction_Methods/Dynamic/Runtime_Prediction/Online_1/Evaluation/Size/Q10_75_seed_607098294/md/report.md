...
**Test Query:** Q10_75_seed_607098294
...
```
Node 20956 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20957 (Sort) [consumed]
    Node 20958 (Aggregate) [consumed]
      Node 20959 (Gather) [consumed]
        Node 20960 (Hash Join)
          Node 20961 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20962 (Nested Loop) [consumed]
              Node 20963 (Seq Scan) - LEAF
              Node 20964 (Index Scan) - LEAF
            Node 20965 (Hash) [consumed]
              Node 20966 (Seq Scan) - LEAF
          Node 20967 (Hash)
            Node 20968 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20956 | 20957, 20958, 20959, 20961, 20962, 20965 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20961 | 20956, 20957, 20958, 20959, 20962, 20965 |
...
|------|------|--------|-----------|-----|--------|
| 20956 | Limit | 1131.59 | 1199.47 | 6.0% | pattern |
| 20960 | Hash Join | 1094.42 | 1095.39 | 0.1% | operator |
| 20961 | Hash Join | 1091.85 | 1120.19 | 2.6% | pattern |
| 20967 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20968 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 20963 | Seq Scan | 163.64 | 159.75 | 2.4% | operator |
| 20964 | Index Scan | 0.06 | -0.52 | 935.8% | operator |
| 20966 | Seq Scan | 27.94 | 26.64 | 4.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20963 (Seq Scan) - LEAF
...
### Step 2: Node 20964 (Index Scan) - LEAF
...
### Step 3: Node 20966 (Seq Scan) - LEAF
...
### Step 4: Node 20968 (Seq Scan) - LEAF
...
### Step 5: Node 20961 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20956, 20957, 20958, 20959, 20962, 20965
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17592
  - HashJoin_nt1=17592
...
### Step 8: Node 20956 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20957, 20958, 20959, 20961, 20962, 20965
- **Input Features:**
