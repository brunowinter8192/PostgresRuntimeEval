...
```
Node 12391 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12392 (Gather Merge) [consumed]
    Node 12393 (Sort)
      Node 12394 (Hash Join)
        Node 12395 (Nested Loop)
          Node 12396 (Hash Join) [PATTERN: 895c6e8c]
            Node 12397 (Seq Scan) [consumed] - LEAF
            Node 12398 (Hash) [consumed]
              Node 12399 (Hash Join) [PATTERN: f4cb205a]
                Node 12400 (Seq Scan) [consumed] - LEAF
                Node 12401 (Hash) [consumed]
                  Node 12402 (Seq Scan) [consumed] - LEAF
          Node 12403 (Index Scan) - LEAF
        Node 12404 (Hash)
          Node 12405 (Hash Join) [PATTERN: f4cb205a]
            Node 12406 (Seq Scan) [consumed] - LEAF
            Node 12407 (Hash) [consumed]
              Node 12408 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12399 | 12391, 12392, 12396, 12397, 12398, 12400, 12401, 12402, 12405, 12406, 12407, 12408 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12405 | 12391, 12392, 12396, 12397, 12398, 12399, 12400, 12401, 12402, 12406, 12407, 12408 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12391 | 12392, 12396, 12397, 12398, 12399, 12400, 12401, 12402, 12405, 12406, 12407, 12408 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12396 | 12391, 12392, 12397, 12398, 12399, 12400, 12401, 12402, 12405, 12406, 12407, 12408 |
...
|------|------|--------|-----------|-----|--------|
| 12391 | Aggregate | 1221.14 | 1163.13 | 4.8% | pattern |
| 12393 | Sort | 1214.56 | 1109.16 | 8.7% | operator |
| 12394 | Hash Join | 1213.89 | 843.11 | 30.5% | operator |
| 12395 | Nested Loop | 1207.29 | 1215.99 | 0.7% | operator |
| 12404 | Hash | 2.77 | 27.74 | 899.8% | operator |
| 12396 | Hash Join | 215.68 | 304.89 | 41.4% | pattern |
| 12403 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12405 | Hash Join | 2.73 | 429.36 | 15633.3% | pattern |
| 12399 | Hash Join | 35.73 | 429.38 | 1101.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12399 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12391, 12392, 12396, 12397, 12398, 12400, 12401, 12402, 12405, 12406, 12407, 12408
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12396 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12391, 12392, 12397, 12398, 12399, 12400, 12401, 12402, 12405, 12406, 12407, 12408
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12403 (Index Scan) - LEAF
...
### Step 4: Node 12405 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12391, 12392, 12396, 12397, 12398, 12399, 12400, 12401, 12402, 12406, 12407, 12408
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12391 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12392, 12396, 12397, 12398, 12399, 12400, 12401, 12402, 12405, 12406, 12407, 12408
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
