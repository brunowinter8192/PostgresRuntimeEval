...
**Test Query:** Q10_140_seed_1140360309
...
```
Node 19942 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19943 (Sort) [consumed]
    Node 19944 (Aggregate) [consumed]
      Node 19945 (Gather) [consumed]
        Node 19946 (Hash Join)
          Node 19947 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19948 (Nested Loop) [consumed]
              Node 19949 (Seq Scan) - LEAF
              Node 19950 (Index Scan) - LEAF
            Node 19951 (Hash) [consumed]
              Node 19952 (Seq Scan) - LEAF
          Node 19953 (Hash)
            Node 19954 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19942 | 19943, 19944, 19945, 19947, 19948, 19951 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19947 | 19942, 19943, 19944, 19945, 19948, 19951 |
...
|------|------|--------|-----------|-----|--------|
| 19942 | Limit | 1161.50 | 1199.47 | 3.3% | pattern |
| 19946 | Hash Join | 1099.06 | 1095.57 | 0.3% | operator |
| 19947 | Hash Join | 1096.21 | 1120.19 | 2.2% | pattern |
| 19953 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 19954 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 19949 | Seq Scan | 160.38 | 159.74 | 0.4% | operator |
| 19950 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 19952 | Seq Scan | 25.42 | 26.64 | 4.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19949 (Seq Scan) - LEAF
...
### Step 2: Node 19950 (Index Scan) - LEAF
...
### Step 3: Node 19952 (Seq Scan) - LEAF
...
### Step 4: Node 19954 (Seq Scan) - LEAF
...
### Step 5: Node 19947 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19942, 19943, 19944, 19945, 19948, 19951
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18240
  - HashJoin_nt1=18240
...
### Step 8: Node 19942 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19943, 19944, 19945, 19947, 19948, 19951
- **Input Features:**
