...
**Test Query:** Q5_44_seed_352773333
...
```
Node 8661 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8662 (Aggregate) [consumed]
    Node 8663 (Gather Merge) [consumed]
      Node 8664 (Aggregate)
        Node 8665 (Sort) [PATTERN: 3e2d5a00]
          Node 8666 (Hash Join) [consumed]
            Node 8667 (Nested Loop)
              Node 8668 (Hash Join)
                Node 8669 (Seq Scan) - LEAF
                Node 8670 (Hash) [PATTERN: 444761fb]
                  Node 8671 (Hash Join) [consumed]
                    Node 8672 (Seq Scan) [consumed] - LEAF
                    Node 8673 (Hash) [consumed]
                      Node 8674 (Hash Join) [consumed]
                        Node 8675 (Seq Scan) - LEAF
                        Node 8676 (Hash)
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8670 | 8661, 8662, 8663, 8665, 8666, 8671, 8672, 8673, 8674 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8661 | 8662, 8663, 8665, 8666, 8670, 8671, 8672, 8673, 8674 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8665 | 8661, 8662, 8663, 8666, 8670, 8671, 8672, 8673, 8674 |
...
|------|------|--------|-----------|-----|--------|
| 8661 | Sort | 1077.26 | 2934.42 | 172.4% | pattern |
| 8664 | Aggregate | 1071.86 | 1128.78 | 5.3% | operator |
| 8665 | Sort | 1071.61 | 1835.92 | 71.3% | pattern |
| 8667 | Nested Loop | 1059.85 | 1188.42 | 12.1% | operator |
| 8679 | Hash | 3.13 | 18.67 | 497.1% | operator |
| 8668 | Hash Join | 224.08 | 253.07 | 12.9% | operator |
| 8678 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 8680 | Seq Scan | 2.66 | 12.36 | 364.6% | operator |
| 8669 | Seq Scan | 168.68 | 159.81 | 5.3% | operator |
| 8670 | Hash | 40.67 | 46.85 | 15.2% | pattern |
| 8675 | Seq Scan | 0.01 | 11.81 | 90732.9% | operator |
| 8676 | Hash | 0.47 | 21.05 | 4351.0% | operator |
| 8677 | Seq Scan | 0.47 | 22.93 | 4789.8% | operator |
...
### Step 2: Node 8675 (Seq Scan) - LEAF
...
### Step 4: Node 8669 (Seq Scan) - LEAF
...
### Step 5: Node 8670 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8661, 8662, 8663, 8665, 8666, 8671, 8672, 8673, 8674
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8678 (Index Scan) - LEAF
...
### Step 8: Node 8680 (Seq Scan) - LEAF
...
### Step 11: Node 8665 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8661, 8662, 8663, 8666, 8670, 8671, 8672, 8673, 8674
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8661 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8662, 8663, 8665, 8666, 8670, 8671, 8672, 8673, 8674
- **Input Features:**
