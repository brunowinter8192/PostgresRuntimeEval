...
**Test Query:** Q7_68_seed_549670077
...
```
Node 12553 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12554 (Gather Merge) [consumed]
    Node 12555 (Sort)
      Node 12556 (Hash Join)
        Node 12557 (Nested Loop)
          Node 12558 (Hash Join) [PATTERN: 895c6e8c]
            Node 12559 (Seq Scan) [consumed] - LEAF
            Node 12560 (Hash) [consumed]
              Node 12561 (Hash Join) [PATTERN: f4cb205a]
                Node 12562 (Seq Scan) [consumed] - LEAF
                Node 12563 (Hash) [consumed]
                  Node 12564 (Seq Scan) [consumed] - LEAF
          Node 12565 (Index Scan) - LEAF
        Node 12566 (Hash)
          Node 12567 (Hash Join) [PATTERN: f4cb205a]
            Node 12568 (Seq Scan) [consumed] - LEAF
            Node 12569 (Hash) [consumed]
              Node 12570 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12561 | 12553, 12554, 12558, 12559, 12560, 12562, 12563, 12564, 12567, 12568, 12569, 12570 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12567 | 12553, 12554, 12558, 12559, 12560, 12561, 12562, 12563, 12564, 12568, 12569, 12570 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12553 | 12554, 12558, 12559, 12560, 12561, 12562, 12563, 12564, 12567, 12568, 12569, 12570 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12558 | 12553, 12554, 12559, 12560, 12561, 12562, 12563, 12564, 12567, 12568, 12569, 12570 |
...
|------|------|--------|-----------|-----|--------|
| 12553 | Aggregate | 1191.50 | 1163.13 | 2.4% | pattern |
| 12555 | Sort | 1184.64 | 1109.16 | 6.4% | operator |
| 12556 | Hash Join | 1184.01 | 843.11 | 28.8% | operator |
| 12557 | Nested Loop | 1176.95 | 1215.99 | 3.3% | operator |
| 12566 | Hash | 3.09 | 27.74 | 797.8% | operator |
| 12558 | Hash Join | 213.48 | 304.89 | 42.8% | pattern |
| 12565 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12567 | Hash Join | 3.02 | 429.36 | 14093.8% | pattern |
| 12561 | Hash Join | 36.27 | 429.38 | 1083.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12561 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12553, 12554, 12558, 12559, 12560, 12562, 12563, 12564, 12567, 12568, 12569, 12570
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12558 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12553, 12554, 12559, 12560, 12561, 12562, 12563, 12564, 12567, 12568, 12569, 12570
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12565 (Index Scan) - LEAF
...
### Step 4: Node 12567 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12553, 12554, 12558, 12559, 12560, 12561, 12562, 12563, 12564, 12568, 12569, 12570
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12553 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12554, 12558, 12559, 12560, 12561, 12562, 12563, 12564, 12567, 12568, 12569, 12570
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
