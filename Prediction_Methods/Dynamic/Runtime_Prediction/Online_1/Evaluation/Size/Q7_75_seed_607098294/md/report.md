...
**Test Query:** Q7_75_seed_607098294
...
```
Node 12697 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12698 (Gather Merge) [consumed]
    Node 12699 (Sort)
      Node 12700 (Hash Join)
        Node 12701 (Nested Loop)
          Node 12702 (Hash Join) [PATTERN: 895c6e8c]
            Node 12703 (Seq Scan) [consumed] - LEAF
            Node 12704 (Hash) [consumed]
              Node 12705 (Hash Join) [PATTERN: f4cb205a]
                Node 12706 (Seq Scan) [consumed] - LEAF
                Node 12707 (Hash) [consumed]
                  Node 12708 (Seq Scan) [consumed] - LEAF
          Node 12709 (Index Scan) - LEAF
        Node 12710 (Hash)
          Node 12711 (Hash Join) [PATTERN: f4cb205a]
            Node 12712 (Seq Scan) [consumed] - LEAF
            Node 12713 (Hash) [consumed]
              Node 12714 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12705 | 12697, 12698, 12702, 12703, 12704, 12706, 12707, 12708, 12711, 12712, 12713, 12714 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12711 | 12697, 12698, 12702, 12703, 12704, 12705, 12706, 12707, 12708, 12712, 12713, 12714 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12697 | 12698, 12702, 12703, 12704, 12705, 12706, 12707, 12708, 12711, 12712, 12713, 12714 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12702 | 12697, 12698, 12703, 12704, 12705, 12706, 12707, 12708, 12711, 12712, 12713, 12714 |
...
|------|------|--------|-----------|-----|--------|
| 12697 | Aggregate | 1195.86 | 1163.13 | 2.7% | pattern |
| 12699 | Sort | 1189.21 | 1109.16 | 6.7% | operator |
| 12700 | Hash Join | 1188.58 | 843.11 | 29.1% | operator |
| 12701 | Nested Loop | 1181.82 | 1215.99 | 2.9% | operator |
| 12710 | Hash | 2.96 | 27.74 | 837.0% | operator |
| 12702 | Hash Join | 211.25 | 304.89 | 44.3% | pattern |
| 12709 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12711 | Hash Join | 2.92 | 429.36 | 14624.4% | pattern |
| 12705 | Hash Join | 37.18 | 429.38 | 1055.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12705 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12697, 12698, 12702, 12703, 12704, 12706, 12707, 12708, 12711, 12712, 12713, 12714
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12702 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12697, 12698, 12703, 12704, 12705, 12706, 12707, 12708, 12711, 12712, 12713, 12714
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12709 (Index Scan) - LEAF
...
### Step 4: Node 12711 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12697, 12698, 12702, 12703, 12704, 12705, 12706, 12707, 12708, 12712, 12713, 12714
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12697 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12698, 12702, 12703, 12704, 12705, 12706, 12707, 12708, 12711, 12712, 12713, 12714
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
