...
**Test Query:** Q7_55_seed_443017674
...
```
Node 12301 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12302 (Gather Merge) [consumed]
    Node 12303 (Sort)
      Node 12304 (Hash Join)
        Node 12305 (Nested Loop)
          Node 12306 (Hash Join) [PATTERN: 895c6e8c]
            Node 12307 (Seq Scan) [consumed] - LEAF
            Node 12308 (Hash) [consumed]
              Node 12309 (Hash Join) [PATTERN: f4cb205a]
                Node 12310 (Seq Scan) [consumed] - LEAF
                Node 12311 (Hash) [consumed]
                  Node 12312 (Seq Scan) [consumed] - LEAF
          Node 12313 (Index Scan) - LEAF
        Node 12314 (Hash)
          Node 12315 (Hash Join) [PATTERN: f4cb205a]
            Node 12316 (Seq Scan) [consumed] - LEAF
            Node 12317 (Hash) [consumed]
              Node 12318 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12309 | 12301, 12302, 12306, 12307, 12308, 12310, 12311, 12312, 12315, 12316, 12317, 12318 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12315 | 12301, 12302, 12306, 12307, 12308, 12309, 12310, 12311, 12312, 12316, 12317, 12318 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12301 | 12302, 12306, 12307, 12308, 12309, 12310, 12311, 12312, 12315, 12316, 12317, 12318 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12306 | 12301, 12302, 12307, 12308, 12309, 12310, 12311, 12312, 12315, 12316, 12317, 12318 |
...
|------|------|--------|-----------|-----|--------|
| 12301 | Aggregate | 1239.31 | 1163.13 | 6.1% | pattern |
| 12303 | Sort | 1232.11 | 1109.16 | 10.0% | operator |
| 12304 | Hash Join | 1231.46 | 843.11 | 31.5% | operator |
| 12305 | Nested Loop | 1224.39 | 1215.99 | 0.7% | operator |
| 12314 | Hash | 3.14 | 27.74 | 783.0% | operator |
| 12306 | Hash Join | 217.39 | 304.89 | 40.3% | pattern |
| 12313 | Index Scan | 0.03 | 0.96 | 2731.9% | operator |
| 12315 | Hash Join | 3.10 | 429.36 | 13759.4% | pattern |
| 12309 | Hash Join | 36.38 | 429.38 | 1080.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12309 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12301, 12302, 12306, 12307, 12308, 12310, 12311, 12312, 12315, 12316, 12317, 12318
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12306 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12301, 12302, 12307, 12308, 12309, 12310, 12311, 12312, 12315, 12316, 12317, 12318
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12313 (Index Scan) - LEAF
...
### Step 4: Node 12315 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12301, 12302, 12306, 12307, 12308, 12309, 12310, 12311, 12312, 12316, 12317, 12318
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12301 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12302, 12306, 12307, 12308, 12309, 12310, 12311, 12312, 12315, 12316, 12317, 12318
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
