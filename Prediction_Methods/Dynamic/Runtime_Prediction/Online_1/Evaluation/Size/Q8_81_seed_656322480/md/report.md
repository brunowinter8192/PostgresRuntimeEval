...
```
Node 16168 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16169 (Gather Merge) [consumed]
    Node 16170 (Sort)
          ...
          Node 16173 (Hash Join)
            Node 16174 (Nested Loop)
              Node 16175 (Hash Join)
                ...
                Node 16177 (Hash) [PATTERN: 444761fb]
                  Node 16178 (Hash Join) [consumed]
                    Node 16179 (Seq Scan) [consumed] - LEAF
                    Node 16180 (Hash) [consumed]
                      Node 16181 (Hash Join) [consumed]
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16177 | 16168, 16169, 16178, 16179, 16180, 16181 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16168 | 16169, 16177, 16178, 16179, 16180, 16181 |
...
|------|------|--------|-----------|-----|--------|
| 16168 | Aggregate | 1210.61 | 1127.55 | 6.9% | pattern |
...
| 16175 | Hash Join | 212.49 | 251.67 | 18.4% | operator |
...
| 16177 | Hash | 36.45 | 43.51 | 19.4% | pattern |
...
### Step 4: Node 16176 (Seq Scan) - LEAF
...
### Step 5: Node 16177 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16168, 16169, 16178, 16179, 16180, 16181
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16185 (Index Scan) - LEAF
...
### Step 8: Node 16187 (Seq Scan) - LEAF
...
### Step 12: Node 16188 (Index Scan) - LEAF
...
### Step 13: Node 16190 (Seq Scan) - LEAF
...
### Step 14: Node 16172 (Nested Loop)
...
### Step 18: Node 16168 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16169, 16177, 16178, 16179, 16180, 16181
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2530
  ...
  - Aggregate_sel=0.9510
  - Aggregate_startup_cost=71564.3600
  - Aggregate_total_cost=71962.3900
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2530
  - GatherMerge_Outer_nt1=816
  ...
  - GatherMerge_Outer_startup_cost=71564.3600
  - GatherMerge_Outer_total_cost=71863.6700
- **Output:** st=1124.61, rt=1127.55
