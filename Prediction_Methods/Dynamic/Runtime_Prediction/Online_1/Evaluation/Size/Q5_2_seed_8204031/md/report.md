...
```
Node 8341 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8342 (Aggregate) [consumed]
    Node 8343 (Gather Merge) [consumed]
      Node 8344 (Aggregate)
        Node 8345 (Sort) [PATTERN: 3e2d5a00]
          Node 8346 (Hash Join) [consumed]
            Node 8347 (Nested Loop)
              Node 8348 (Hash Join)
                Node 8349 (Seq Scan) - LEAF
                Node 8350 (Hash) [PATTERN: 444761fb]
                  Node 8351 (Hash Join) [consumed]
                    Node 8352 (Seq Scan) [consumed] - LEAF
                    Node 8353 (Hash) [consumed]
                      Node 8354 (Hash Join) [consumed]
                        Node 8355 (Seq Scan) - LEAF
                        Node 8356 (Hash)
                          Node 8357 (Seq Scan) - LEAF
              Node 8358 (Index Scan) - LEAF
            Node 8359 (Hash)
              Node 8360 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8350 | 8341, 8342, 8343, 8345, 8346, 8351, 8352, 8353, 8354 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8341 | 8342, 8343, 8345, 8346, 8350, 8351, 8352, 8353, 8354 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8345 | 8341, 8342, 8343, 8346, 8350, 8351, 8352, 8353, 8354 |
...
|------|------|--------|-----------|-----|--------|
| 8341 | Sort | 1080.65 | 2934.42 | 171.5% | pattern |
| 8344 | Aggregate | 1075.22 | 1128.79 | 5.0% | operator |
| 8345 | Sort | 1074.97 | 1835.90 | 70.8% | pattern |
| 8347 | Nested Loop | 1064.04 | 1188.37 | 11.7% | operator |
| 8359 | Hash | 2.42 | 18.67 | 671.2% | operator |
| 8348 | Hash Join | 219.10 | 253.07 | 15.5% | operator |
| 8358 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 8360 | Seq Scan | 1.91 | 12.36 | 546.4% | operator |
| 8349 | Seq Scan | 168.08 | 159.81 | 4.9% | operator |
| 8350 | Hash | 37.98 | 46.85 | 23.4% | pattern |
| 8355 | Seq Scan | 0.02 | 11.81 | 58941.4% | operator |
| 8356 | Hash | 0.16 | 21.05 | 13309.7% | operator |
| 8357 | Seq Scan | 0.15 | 22.93 | 14695.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8357 (Seq Scan) - LEAF
...
### Step 2: Node 8355 (Seq Scan) - LEAF
...
### Step 4: Node 8349 (Seq Scan) - LEAF
...
### Step 5: Node 8350 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8341, 8342, 8343, 8345, 8346, 8351, 8352, 8353, 8354
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8358 (Index Scan) - LEAF
...
### Step 8: Node 8360 (Seq Scan) - LEAF
...
### Step 11: Node 8345 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8341, 8342, 8343, 8346, 8350, 8351, 8352, 8353, 8354
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8341 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8342, 8343, 8345, 8346, 8350, 8351, 8352, 8353, 8354
- **Input Features:**
