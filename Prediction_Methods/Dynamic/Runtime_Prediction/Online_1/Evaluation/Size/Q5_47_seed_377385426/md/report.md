...
**Test Query:** Q5_47_seed_377385426
...
```
Node 8721 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8722 (Aggregate) [consumed]
    Node 8723 (Gather Merge) [consumed]
      Node 8724 (Aggregate)
        Node 8725 (Sort) [PATTERN: 3e2d5a00]
          Node 8726 (Hash Join) [consumed]
            Node 8727 (Nested Loop)
              Node 8728 (Hash Join)
                Node 8729 (Seq Scan) - LEAF
                Node 8730 (Hash) [PATTERN: 444761fb]
                  Node 8731 (Hash Join) [consumed]
                    Node 8732 (Seq Scan) [consumed] - LEAF
                    Node 8733 (Hash) [consumed]
                      Node 8734 (Hash Join) [consumed]
                        Node 8735 (Seq Scan) - LEAF
                        Node 8736 (Hash)
                          Node 8737 (Seq Scan) - LEAF
              Node 8738 (Index Scan) - LEAF
            Node 8739 (Hash)
              Node 8740 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8730 | 8721, 8722, 8723, 8725, 8726, 8731, 8732, 8733, 8734 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8721 | 8722, 8723, 8725, 8726, 8730, 8731, 8732, 8733, 8734 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8725 | 8721, 8722, 8723, 8726, 8730, 8731, 8732, 8733, 8734 |
...
|------|------|--------|-----------|-----|--------|
| 8721 | Sort | 1058.79 | 2934.42 | 177.1% | pattern |
| 8724 | Aggregate | 1054.02 | 1128.79 | 7.1% | operator |
| 8725 | Sort | 1053.75 | 1835.90 | 74.2% | pattern |
| 8727 | Nested Loop | 1041.39 | 1188.37 | 14.1% | operator |
| 8739 | Hash | 3.96 | 18.67 | 372.1% | operator |
| 8728 | Hash Join | 210.92 | 253.07 | 20.0% | operator |
| 8738 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 8740 | Seq Scan | 3.43 | 12.36 | 260.2% | operator |
| 8729 | Seq Scan | 164.09 | 159.81 | 2.6% | operator |
| 8730 | Hash | 33.28 | 46.85 | 40.8% | pattern |
| 8735 | Seq Scan | 0.01 | 11.81 | 147503.4% | operator |
| 8736 | Hash | 0.16 | 21.05 | 12816.1% | operator |
| 8737 | Seq Scan | 0.16 | 22.93 | 14233.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8737 (Seq Scan) - LEAF
...
### Step 2: Node 8735 (Seq Scan) - LEAF
...
### Step 4: Node 8729 (Seq Scan) - LEAF
...
### Step 5: Node 8730 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8721, 8722, 8723, 8725, 8726, 8731, 8732, 8733, 8734
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8738 (Index Scan) - LEAF
...
### Step 8: Node 8740 (Seq Scan) - LEAF
...
### Step 11: Node 8725 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8721, 8722, 8723, 8726, 8730, 8731, 8732, 8733, 8734
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8721 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8722, 8723, 8725, 8726, 8730, 8731, 8732, 8733, 8734
- **Input Features:**
