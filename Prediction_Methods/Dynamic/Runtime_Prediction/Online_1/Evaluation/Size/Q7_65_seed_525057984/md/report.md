...
**Test Query:** Q7_65_seed_525057984
...
```
Node 12499 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12500 (Gather Merge) [consumed]
    Node 12501 (Sort)
      Node 12502 (Hash Join)
        Node 12503 (Nested Loop)
          Node 12504 (Hash Join) [PATTERN: 895c6e8c]
            Node 12505 (Seq Scan) [consumed] - LEAF
            Node 12506 (Hash) [consumed]
              Node 12507 (Hash Join) [PATTERN: f4cb205a]
                Node 12508 (Seq Scan) [consumed] - LEAF
                Node 12509 (Hash) [consumed]
                  Node 12510 (Seq Scan) [consumed] - LEAF
          Node 12511 (Index Scan) - LEAF
        Node 12512 (Hash)
          Node 12513 (Hash Join) [PATTERN: f4cb205a]
            Node 12514 (Seq Scan) [consumed] - LEAF
            Node 12515 (Hash) [consumed]
              Node 12516 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12507 | 12499, 12500, 12504, 12505, 12506, 12508, 12509, 12510, 12513, 12514, 12515, 12516 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12513 | 12499, 12500, 12504, 12505, 12506, 12507, 12508, 12509, 12510, 12514, 12515, 12516 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12499 | 12500, 12504, 12505, 12506, 12507, 12508, 12509, 12510, 12513, 12514, 12515, 12516 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12504 | 12499, 12500, 12505, 12506, 12507, 12508, 12509, 12510, 12513, 12514, 12515, 12516 |
...
|------|------|--------|-----------|-----|--------|
| 12499 | Aggregate | 1204.78 | 1163.13 | 3.5% | pattern |
| 12501 | Sort | 1198.01 | 1109.16 | 7.4% | operator |
| 12502 | Hash Join | 1197.39 | 843.11 | 29.6% | operator |
| 12503 | Nested Loop | 1189.81 | 1215.99 | 2.2% | operator |
| 12512 | Hash | 3.65 | 27.74 | 660.1% | operator |
| 12504 | Hash Join | 217.19 | 304.89 | 40.4% | pattern |
| 12511 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12513 | Hash Join | 3.60 | 429.36 | 11830.1% | pattern |
| 12507 | Hash Join | 35.96 | 429.38 | 1094.1% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12507 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12499, 12500, 12504, 12505, 12506, 12508, 12509, 12510, 12513, 12514, 12515, 12516
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12504 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12499, 12500, 12505, 12506, 12507, 12508, 12509, 12510, 12513, 12514, 12515, 12516
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12511 (Index Scan) - LEAF
...
### Step 4: Node 12513 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12499, 12500, 12504, 12505, 12506, 12507, 12508, 12509, 12510, 12514, 12515, 12516
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12499 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12500, 12504, 12505, 12506, 12507, 12508, 12509, 12510, 12513, 12514, 12515, 12516
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
