...
**Test Query:** Q10_74_seed_598894263
...
```
Node 20943 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20944 (Sort) [consumed]
    Node 20945 (Aggregate) [consumed]
      Node 20946 (Gather) [consumed]
        Node 20947 (Hash Join)
          Node 20948 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20949 (Nested Loop) [consumed]
              Node 20950 (Seq Scan) - LEAF
              Node 20951 (Index Scan) - LEAF
            Node 20952 (Hash) [consumed]
              Node 20953 (Seq Scan) - LEAF
          Node 20954 (Hash)
            Node 20955 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20943 | 20944, 20945, 20946, 20948, 20949, 20952 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20948 | 20943, 20944, 20945, 20946, 20949, 20952 |
...
|------|------|--------|-----------|-----|--------|
| 20943 | Limit | 1155.87 | 1199.47 | 3.8% | pattern |
| 20947 | Hash Join | 1101.50 | 1095.55 | 0.5% | operator |
| 20948 | Hash Join | 1098.10 | 1120.19 | 2.0% | pattern |
| 20954 | Hash | 0.02 | 15.71 | 82588.3% | operator |
| 20955 | Seq Scan | 0.01 | 6.98 | 46424.7% | operator |
| 20950 | Seq Scan | 159.07 | 159.74 | 0.4% | operator |
| 20951 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20953 | Seq Scan | 30.68 | 26.64 | 13.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20950 (Seq Scan) - LEAF
...
### Step 2: Node 20951 (Index Scan) - LEAF
...
### Step 3: Node 20953 (Seq Scan) - LEAF
...
### Step 4: Node 20955 (Seq Scan) - LEAF
...
### Step 5: Node 20948 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20943, 20944, 20945, 20946, 20949, 20952
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18186
  - HashJoin_nt1=18186
...
### Step 8: Node 20943 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20944, 20945, 20946, 20948, 20949, 20952
- **Input Features:**
