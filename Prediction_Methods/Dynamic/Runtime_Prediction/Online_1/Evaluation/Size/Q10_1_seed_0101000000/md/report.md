...
**Test Query:** Q10_1_seed_0101000000
...
```
Node 20163 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20164 (Sort) [consumed]
    Node 20165 (Aggregate) [consumed]
      Node 20166 (Gather) [consumed]
        Node 20167 (Hash Join)
          Node 20168 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20169 (Nested Loop) [consumed]
              Node 20170 (Seq Scan) - LEAF
              Node 20171 (Index Scan) - LEAF
            Node 20172 (Hash) [consumed]
              Node 20173 (Seq Scan) - LEAF
          Node 20174 (Hash)
            Node 20175 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20163 | 20164, 20165, 20166, 20168, 20169, 20172 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20168 | 20163, 20164, 20165, 20166, 20169, 20172 |
...
|------|------|--------|-----------|-----|--------|
| 20163 | Limit | 1142.52 | 1199.47 | 5.0% | pattern |
| 20167 | Hash Join | 1091.95 | 1095.48 | 0.3% | operator |
| 20168 | Hash Join | 1089.10 | 1120.19 | 2.9% | pattern |
| 20174 | Hash | 0.01 | 15.71 | 130823.1% | operator |
| 20175 | Seq Scan | 0.01 | 6.98 | 87133.8% | operator |
| 20170 | Seq Scan | 162.25 | 159.74 | 1.5% | operator |
| 20171 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20173 | Seq Scan | 28.22 | 26.64 | 5.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20170 (Seq Scan) - LEAF
...
### Step 2: Node 20171 (Index Scan) - LEAF
...
### Step 3: Node 20173 (Seq Scan) - LEAF
...
### Step 4: Node 20175 (Seq Scan) - LEAF
...
### Step 5: Node 20168 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20163, 20164, 20165, 20166, 20169, 20172
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17901
  - HashJoin_nt1=17901
...
### Step 8: Node 20163 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20164, 20165, 20166, 20168, 20169, 20172
- **Input Features:**
