...
**Test Query:** Q7_12_seed_90244341
...
```
Node 11077 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11078 (Gather Merge) [consumed]
    Node 11079 (Sort)
      Node 11080 (Hash Join)
        Node 11081 (Nested Loop)
          Node 11082 (Hash Join) [PATTERN: 895c6e8c]
            Node 11083 (Seq Scan) [consumed] - LEAF
            Node 11084 (Hash) [consumed]
              Node 11085 (Hash Join) [PATTERN: f4cb205a]
                Node 11086 (Seq Scan) [consumed] - LEAF
                Node 11087 (Hash) [consumed]
                  Node 11088 (Seq Scan) [consumed] - LEAF
          Node 11089 (Index Scan) - LEAF
        Node 11090 (Hash)
          Node 11091 (Hash Join) [PATTERN: f4cb205a]
            Node 11092 (Seq Scan) [consumed] - LEAF
            Node 11093 (Hash) [consumed]
              Node 11094 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11085 | 11077, 11078, 11082, 11083, 11084, 11086, 11087, 11088, 11091, 11092, 11093, 11094 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11091 | 11077, 11078, 11082, 11083, 11084, 11085, 11086, 11087, 11088, 11092, 11093, 11094 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11077 | 11078, 11082, 11083, 11084, 11085, 11086, 11087, 11088, 11091, 11092, 11093, 11094 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11082 | 11077, 11078, 11083, 11084, 11085, 11086, 11087, 11088, 11091, 11092, 11093, 11094 |
...
|------|------|--------|-----------|-----|--------|
| 11077 | Aggregate | 1187.55 | 1163.13 | 2.1% | pattern |
| 11079 | Sort | 1181.82 | 1109.16 | 6.1% | operator |
| 11080 | Hash Join | 1181.20 | 843.11 | 28.6% | operator |
| 11081 | Nested Loop | 1173.39 | 1215.99 | 3.6% | operator |
| 11090 | Hash | 4.13 | 27.74 | 572.2% | operator |
| 11082 | Hash Join | 213.68 | 304.89 | 42.7% | pattern |
| 11089 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11091 | Hash Join | 4.08 | 429.36 | 10413.3% | pattern |
| 11085 | Hash Join | 34.10 | 429.38 | 1159.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11085 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11077, 11078, 11082, 11083, 11084, 11086, 11087, 11088, 11091, 11092, 11093, 11094
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11082 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11077, 11078, 11083, 11084, 11085, 11086, 11087, 11088, 11091, 11092, 11093, 11094
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11089 (Index Scan) - LEAF
...
### Step 4: Node 11091 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11077, 11078, 11082, 11083, 11084, 11085, 11086, 11087, 11088, 11092, 11093, 11094
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11077 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11078, 11082, 11083, 11084, 11085, 11086, 11087, 11088, 11091, 11092, 11093, 11094
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
