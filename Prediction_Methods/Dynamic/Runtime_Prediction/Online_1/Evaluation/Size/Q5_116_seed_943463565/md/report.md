...
**Test Query:** Q5_116_seed_943463565
...
```
Node 7241 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7242 (Aggregate) [consumed]
    Node 7243 (Gather Merge) [consumed]
      Node 7244 (Aggregate)
        Node 7245 (Sort) [PATTERN: 3e2d5a00]
          Node 7246 (Hash Join) [consumed]
            Node 7247 (Nested Loop)
              Node 7248 (Hash Join)
                Node 7249 (Seq Scan) - LEAF
                Node 7250 (Hash) [PATTERN: 444761fb]
                  Node 7251 (Hash Join) [consumed]
                    Node 7252 (Seq Scan) [consumed] - LEAF
                    Node 7253 (Hash) [consumed]
                      Node 7254 (Hash Join) [consumed]
                        Node 7255 (Seq Scan) - LEAF
                        Node 7256 (Hash)
                          Node 7257 (Seq Scan) - LEAF
              Node 7258 (Index Scan) - LEAF
            Node 7259 (Hash)
              Node 7260 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7250 | 7241, 7242, 7243, 7245, 7246, 7251, 7252, 7253, 7254 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7241 | 7242, 7243, 7245, 7246, 7250, 7251, 7252, 7253, 7254 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7245 | 7241, 7242, 7243, 7246, 7250, 7251, 7252, 7253, 7254 |
...
|------|------|--------|-----------|-----|--------|
| 7241 | Sort | 1067.88 | 2934.42 | 174.8% | pattern |
| 7244 | Aggregate | 1063.23 | 1128.78 | 6.2% | operator |
| 7245 | Sort | 1062.96 | 1835.91 | 72.7% | pattern |
| 7247 | Nested Loop | 1050.77 | 1188.39 | 13.1% | operator |
| 7259 | Hash | 3.67 | 18.67 | 408.3% | operator |
| 7248 | Hash Join | 216.95 | 253.07 | 16.7% | operator |
| 7258 | Index Scan | 0.07 | -0.22 | 405.6% | operator |
| 7260 | Seq Scan | 2.37 | 12.36 | 422.2% | operator |
| 7249 | Seq Scan | 168.15 | 159.81 | 5.0% | operator |
| 7250 | Hash | 35.38 | 46.85 | 32.4% | pattern |
| 7255 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 7256 | Hash | 0.17 | 21.05 | 11999.6% | operator |
| 7257 | Seq Scan | 0.17 | 22.93 | 13311.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7257 (Seq Scan) - LEAF
...
### Step 2: Node 7255 (Seq Scan) - LEAF
...
### Step 4: Node 7249 (Seq Scan) - LEAF
...
### Step 5: Node 7250 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7241, 7242, 7243, 7245, 7246, 7251, 7252, 7253, 7254
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7258 (Index Scan) - LEAF
...
### Step 8: Node 7260 (Seq Scan) - LEAF
...
### Step 11: Node 7245 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7241, 7242, 7243, 7246, 7250, 7251, 7252, 7253, 7254
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7241 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7242, 7243, 7245, 7246, 7250, 7251, 7252, 7253, 7254
- **Input Features:**
