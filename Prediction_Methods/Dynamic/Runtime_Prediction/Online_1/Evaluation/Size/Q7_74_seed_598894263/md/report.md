...
```
Node 12679 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12680 (Gather Merge) [consumed]
    Node 12681 (Sort)
      Node 12682 (Hash Join)
        Node 12683 (Nested Loop)
          Node 12684 (Hash Join) [PATTERN: 895c6e8c]
            Node 12685 (Seq Scan) [consumed] - LEAF
            Node 12686 (Hash) [consumed]
              Node 12687 (Hash Join) [PATTERN: f4cb205a]
                Node 12688 (Seq Scan) [consumed] - LEAF
                Node 12689 (Hash) [consumed]
                  Node 12690 (Seq Scan) [consumed] - LEAF
          Node 12691 (Index Scan) - LEAF
        Node 12692 (Hash)
          Node 12693 (Hash Join) [PATTERN: f4cb205a]
            Node 12694 (Seq Scan) [consumed] - LEAF
            Node 12695 (Hash) [consumed]
              Node 12696 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12687 | 12679, 12680, 12684, 12685, 12686, 12688, 12689, 12690, 12693, 12694, 12695, 12696 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12693 | 12679, 12680, 12684, 12685, 12686, 12687, 12688, 12689, 12690, 12694, 12695, 12696 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12679 | 12680, 12684, 12685, 12686, 12687, 12688, 12689, 12690, 12693, 12694, 12695, 12696 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12684 | 12679, 12680, 12685, 12686, 12687, 12688, 12689, 12690, 12693, 12694, 12695, 12696 |
...
|------|------|--------|-----------|-----|--------|
| 12679 | Aggregate | 1172.63 | 1163.13 | 0.8% | pattern |
| 12681 | Sort | 1164.76 | 1109.16 | 4.8% | operator |
| 12682 | Hash Join | 1164.17 | 843.11 | 27.6% | operator |
| 12683 | Nested Loop | 1156.38 | 1215.99 | 5.2% | operator |
| 12692 | Hash | 3.25 | 27.74 | 753.9% | operator |
| 12684 | Hash Join | 213.94 | 304.89 | 42.5% | pattern |
| 12691 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 12693 | Hash Join | 3.20 | 429.36 | 13300.8% | pattern |
| 12687 | Hash Join | 37.97 | 429.38 | 1031.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12687 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12679, 12680, 12684, 12685, 12686, 12688, 12689, 12690, 12693, 12694, 12695, 12696
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12684 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12679, 12680, 12685, 12686, 12687, 12688, 12689, 12690, 12693, 12694, 12695, 12696
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12691 (Index Scan) - LEAF
...
### Step 4: Node 12693 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12679, 12680, 12684, 12685, 12686, 12687, 12688, 12689, 12690, 12694, 12695, 12696
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12679 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12680, 12684, 12685, 12686, 12687, 12688, 12689, 12690, 12693, 12694, 12695, 12696
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
