...
**Test Query:** Q8_139_seed_1132156278
...
```
Node 14167 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14168 (Gather Merge) [consumed]
    Node 14169 (Sort)
      Node 14170 (Hash Join)
        Node 14171 (Nested Loop)
          Node 14172 (Hash Join)
            Node 14173 (Nested Loop)
              Node 14174 (Hash Join)
                Node 14175 (Seq Scan) - LEAF
                Node 14176 (Hash) [PATTERN: 444761fb]
                  Node 14177 (Hash Join) [consumed]
                    Node 14178 (Seq Scan) [consumed] - LEAF
                    Node 14179 (Hash) [consumed]
                      Node 14180 (Hash Join) [consumed]
                        Node 14181 (Seq Scan) - LEAF
                        Node 14182 (Hash)
                          Node 14183 (Seq Scan) - LEAF
              Node 14184 (Index Scan) - LEAF
            Node 14185 (Hash)
              Node 14186 (Seq Scan) - LEAF
          Node 14187 (Index Scan) - LEAF
        Node 14188 (Hash)
          Node 14189 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14176 | 14167, 14168, 14177, 14178, 14179, 14180 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14167 | 14168, 14176, 14177, 14178, 14179, 14180 |
...
|------|------|--------|-----------|-----|--------|
| 14167 | Aggregate | 1229.09 | 1133.60 | 7.8% | pattern |
| 14169 | Sort | 1222.85 | 1046.21 | 14.4% | operator |
| 14170 | Hash Join | 1222.61 | 785.61 | 35.7% | operator |
| 14171 | Nested Loop | 1222.30 | 1198.29 | 2.0% | operator |
| 14188 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 14172 | Hash Join | 1218.40 | 785.60 | 35.5% | operator |
| 14187 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14189 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 14173 | Nested Loop | 1173.98 | 1198.29 | 2.1% | operator |
| 14185 | Hash | 40.11 | 19.97 | 50.2% | operator |
| 14174 | Hash Join | 213.84 | 251.67 | 17.7% | operator |
| 14184 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14186 | Seq Scan | 39.98 | 48.88 | 22.2% | operator |
| 14175 | Seq Scan | 160.04 | 159.67 | 0.2% | operator |
| 14176 | Hash | 35.88 | 43.51 | 21.3% | pattern |
| 14181 | Seq Scan | 0.05 | 12.22 | 23870.5% | operator |
| 14182 | Hash | 0.16 | 18.14 | 11383.6% | operator |
| 14183 | Seq Scan | 0.15 | 23.36 | 14972.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14183 (Seq Scan) - LEAF
...
### Step 2: Node 14181 (Seq Scan) - LEAF
...
### Step 4: Node 14175 (Seq Scan) - LEAF
...
### Step 5: Node 14176 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14167, 14168, 14177, 14178, 14179, 14180
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14184 (Index Scan) - LEAF
...
### Step 8: Node 14186 (Seq Scan) - LEAF
...
### Step 12: Node 14187 (Index Scan) - LEAF
...
### Step 13: Node 14189 (Seq Scan) - LEAF
...
### Step 14: Node 14171 (Nested Loop)
...
### Step 18: Node 14167 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14168, 14176, 14177, 14178, 14179, 14180
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2407
