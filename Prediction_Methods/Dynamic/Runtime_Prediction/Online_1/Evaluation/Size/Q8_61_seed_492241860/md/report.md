...
**Test Query:** Q8_61_seed_492241860
...
Node 15662 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15663 (Gather Merge) [consumed]
    Node 15664 (Sort)
      Node 15665 (Hash Join)
        Node 15666 (Nested Loop)
          Node 15667 (Hash Join)
            Node 15668 (Nested Loop)
              Node 15669 (Hash Join)
                Node 15670 (Seq Scan) - LEAF
                Node 15671 (Hash) [PATTERN: 444761fb]
                  Node 15672 (Hash Join) [consumed]
                    Node 15673 (Seq Scan) [consumed] - LEAF
                    Node 15674 (Hash) [consumed]
                      Node 15675 (Hash Join) [consumed]
                        Node 15676 (Seq Scan) - LEAF
                        Node 15677 (Hash)
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15671 | 15662, 15663, 15672, 15673, 15674, 15675 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15662 | 15663, 15671, 15672, 15673, 15674, 15675 |
...
|------|------|--------|-----------|-----|--------|
| 15662 | Aggregate | 1208.69 | 1127.47 | 6.7% | pattern |
| 15664 | Sort | 1203.45 | 1046.22 | 13.1% | operator |
| 15665 | Hash Join | 1203.21 | 785.61 | 34.7% | operator |
| 15666 | Nested Loop | 1202.85 | 1198.29 | 0.4% | operator |
| 15683 | Hash | 0.02 | 13.72 | 68520.4% | operator |
| 15667 | Hash Join | 1199.06 | 785.60 | 34.5% | operator |
| 15682 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15684 | Seq Scan | 0.01 | 6.51 | 43313.1% | operator |
| 15668 | Nested Loop | 1154.19 | 1198.29 | 3.8% | operator |
| 15680 | Hash | 40.31 | 19.97 | 50.5% | operator |
| 15669 | Hash Join | 215.19 | 251.67 | 17.0% | operator |
| 15679 | Index Scan | 0.04 | 2.88 | 7102.1% | operator |
| 15681 | Seq Scan | 40.20 | 48.86 | 21.5% | operator |
| 15670 | Seq Scan | 157.74 | 159.67 | 1.2% | operator |
| 15671 | Hash | 38.46 | 43.51 | 13.1% | pattern |
| 15676 | Seq Scan | 0.02 | 12.22 | 71811.4% | operator |
| 15677 | Hash | 0.18 | 18.14 | 9924.4% | operator |
| 15678 | Seq Scan | 0.18 | 23.36 | 13099.0% | operator |
...
### Step 2: Node 15676 (Seq Scan) - LEAF
...
### Step 4: Node 15670 (Seq Scan) - LEAF
...
### Step 5: Node 15671 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15662, 15663, 15672, 15673, 15674, 15675
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15679 (Index Scan) - LEAF
...
### Step 8: Node 15681 (Seq Scan) - LEAF
...
### Step 12: Node 15682 (Index Scan) - LEAF
...
### Step 13: Node 15684 (Seq Scan) - LEAF
...
### Step 14: Node 15666 (Nested Loop)
...
### Step 18: Node 15662 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15663, 15671, 15672, 15673, 15674, 15675
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2532
  ...
  - Aggregate_total_cost=71963.0500
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2532
  - GatherMerge_Outer_nt1=817
  ...
  - GatherMerge_Outer_sel=3.0991
  - GatherMerge_Outer_startup_cost=71564.7300
  - GatherMerge_Outer_total_cost=71864.2900
- **Output:** st=1124.53, rt=1127.47
