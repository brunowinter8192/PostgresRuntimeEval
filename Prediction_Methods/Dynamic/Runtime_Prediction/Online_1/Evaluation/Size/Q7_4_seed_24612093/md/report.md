...
```
Node 12193 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12194 (Gather Merge) [consumed]
    Node 12195 (Sort)
      Node 12196 (Hash Join)
        Node 12197 (Nested Loop)
          Node 12198 (Hash Join) [PATTERN: 895c6e8c]
            Node 12199 (Seq Scan) [consumed] - LEAF
            Node 12200 (Hash) [consumed]
              Node 12201 (Hash Join) [PATTERN: f4cb205a]
                Node 12202 (Seq Scan) [consumed] - LEAF
                Node 12203 (Hash) [consumed]
                  Node 12204 (Seq Scan) [consumed] - LEAF
          Node 12205 (Index Scan) - LEAF
        Node 12206 (Hash)
          Node 12207 (Hash Join) [PATTERN: f4cb205a]
            Node 12208 (Seq Scan) [consumed] - LEAF
            Node 12209 (Hash) [consumed]
              Node 12210 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12201 | 12193, 12194, 12198, 12199, 12200, 12202, 12203, 12204, 12207, 12208, 12209, 12210 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12207 | 12193, 12194, 12198, 12199, 12200, 12201, 12202, 12203, 12204, 12208, 12209, 12210 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12193 | 12194, 12198, 12199, 12200, 12201, 12202, 12203, 12204, 12207, 12208, 12209, 12210 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12198 | 12193, 12194, 12199, 12200, 12201, 12202, 12203, 12204, 12207, 12208, 12209, 12210 |
...
|------|------|--------|-----------|-----|--------|
| 12193 | Aggregate | 1210.38 | 1163.13 | 3.9% | pattern |
| 12195 | Sort | 1203.64 | 1109.16 | 7.8% | operator |
| 12196 | Hash Join | 1203.00 | 843.11 | 29.9% | operator |
| 12197 | Nested Loop | 1195.37 | 1215.99 | 1.7% | operator |
| 12206 | Hash | 3.36 | 27.74 | 726.2% | operator |
| 12198 | Hash Join | 213.68 | 304.89 | 42.7% | pattern |
| 12205 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12207 | Hash Join | 3.31 | 429.36 | 12875.6% | pattern |
| 12201 | Hash Join | 36.47 | 429.38 | 1077.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12201 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12193, 12194, 12198, 12199, 12200, 12202, 12203, 12204, 12207, 12208, 12209, 12210
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12198 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12193, 12194, 12199, 12200, 12201, 12202, 12203, 12204, 12207, 12208, 12209, 12210
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12205 (Index Scan) - LEAF
...
### Step 4: Node 12207 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12193, 12194, 12198, 12199, 12200, 12201, 12202, 12203, 12204, 12208, 12209, 12210
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12193 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12194, 12198, 12199, 12200, 12201, 12202, 12203, 12204, 12207, 12208, 12209, 12210
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
