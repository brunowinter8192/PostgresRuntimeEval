...
**Test Query:** Q7_56_seed_451221705
...
```
Node 12319 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12320 (Gather Merge) [consumed]
    Node 12321 (Sort)
      Node 12322 (Hash Join)
        Node 12323 (Nested Loop)
          Node 12324 (Hash Join) [PATTERN: 895c6e8c]
            Node 12325 (Seq Scan) [consumed] - LEAF
            Node 12326 (Hash) [consumed]
              Node 12327 (Hash Join) [PATTERN: f4cb205a]
                Node 12328 (Seq Scan) [consumed] - LEAF
                Node 12329 (Hash) [consumed]
                  Node 12330 (Seq Scan) [consumed] - LEAF
          Node 12331 (Index Scan) - LEAF
        Node 12332 (Hash)
          Node 12333 (Hash Join) [PATTERN: f4cb205a]
            Node 12334 (Seq Scan) [consumed] - LEAF
            Node 12335 (Hash) [consumed]
              Node 12336 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12327 | 12319, 12320, 12324, 12325, 12326, 12328, 12329, 12330, 12333, 12334, 12335, 12336 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12333 | 12319, 12320, 12324, 12325, 12326, 12327, 12328, 12329, 12330, 12334, 12335, 12336 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12319 | 12320, 12324, 12325, 12326, 12327, 12328, 12329, 12330, 12333, 12334, 12335, 12336 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12324 | 12319, 12320, 12325, 12326, 12327, 12328, 12329, 12330, 12333, 12334, 12335, 12336 |
...
|------|------|--------|-----------|-----|--------|
| 12319 | Aggregate | 1202.35 | 1163.13 | 3.3% | pattern |
| 12321 | Sort | 1195.42 | 1109.16 | 7.2% | operator |
| 12322 | Hash Join | 1194.79 | 843.11 | 29.4% | operator |
| 12323 | Nested Loop | 1188.41 | 1215.99 | 2.3% | operator |
| 12332 | Hash | 2.49 | 27.74 | 1015.5% | operator |
| 12324 | Hash Join | 213.43 | 304.89 | 42.9% | pattern |
| 12331 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12333 | Hash Join | 2.45 | 429.36 | 17453.7% | pattern |
| 12327 | Hash Join | 35.05 | 429.38 | 1125.1% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12327 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12319, 12320, 12324, 12325, 12326, 12328, 12329, 12330, 12333, 12334, 12335, 12336
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12324 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12319, 12320, 12325, 12326, 12327, 12328, 12329, 12330, 12333, 12334, 12335, 12336
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12331 (Index Scan) - LEAF
...
### Step 4: Node 12333 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12319, 12320, 12324, 12325, 12326, 12327, 12328, 12329, 12330, 12334, 12335, 12336
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12319 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12320, 12324, 12325, 12326, 12327, 12328, 12329, 12330, 12333, 12334, 12335, 12336
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
