...
**Test Query:** Q7_66_seed_533262015
...
```
Node 12517 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12518 (Gather Merge) [consumed]
    Node 12519 (Sort)
      Node 12520 (Hash Join)
        Node 12521 (Nested Loop)
          Node 12522 (Hash Join) [PATTERN: 895c6e8c]
            Node 12523 (Seq Scan) [consumed] - LEAF
            Node 12524 (Hash) [consumed]
              Node 12525 (Hash Join) [PATTERN: f4cb205a]
                Node 12526 (Seq Scan) [consumed] - LEAF
                Node 12527 (Hash) [consumed]
                  Node 12528 (Seq Scan) [consumed] - LEAF
          Node 12529 (Index Scan) - LEAF
        Node 12530 (Hash)
          Node 12531 (Hash Join) [PATTERN: f4cb205a]
            Node 12532 (Seq Scan) [consumed] - LEAF
            Node 12533 (Hash) [consumed]
              Node 12534 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12525 | 12517, 12518, 12522, 12523, 12524, 12526, 12527, 12528, 12531, 12532, 12533, 12534 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12531 | 12517, 12518, 12522, 12523, 12524, 12525, 12526, 12527, 12528, 12532, 12533, 12534 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12517 | 12518, 12522, 12523, 12524, 12525, 12526, 12527, 12528, 12531, 12532, 12533, 12534 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12522 | 12517, 12518, 12523, 12524, 12525, 12526, 12527, 12528, 12531, 12532, 12533, 12534 |
...
|------|------|--------|-----------|-----|--------|
| 12517 | Aggregate | 1211.01 | 1163.13 | 4.0% | pattern |
| 12519 | Sort | 1203.78 | 1109.16 | 7.9% | operator |
| 12520 | Hash Join | 1203.11 | 843.11 | 29.9% | operator |
| 12521 | Nested Loop | 1196.60 | 1215.99 | 1.6% | operator |
| 12530 | Hash | 2.61 | 27.74 | 962.6% | operator |
| 12522 | Hash Join | 214.10 | 304.89 | 42.4% | pattern |
| 12529 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12531 | Hash Join | 2.56 | 429.36 | 16652.4% | pattern |
| 12525 | Hash Join | 35.80 | 429.38 | 1099.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12525 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12517, 12518, 12522, 12523, 12524, 12526, 12527, 12528, 12531, 12532, 12533, 12534
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12522 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12517, 12518, 12523, 12524, 12525, 12526, 12527, 12528, 12531, 12532, 12533, 12534
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12529 (Index Scan) - LEAF
...
### Step 4: Node 12531 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12517, 12518, 12522, 12523, 12524, 12525, 12526, 12527, 12528, 12532, 12533, 12534
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12517 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12518, 12522, 12523, 12524, 12525, 12526, 12527, 12528, 12531, 12532, 12533, 12534
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
