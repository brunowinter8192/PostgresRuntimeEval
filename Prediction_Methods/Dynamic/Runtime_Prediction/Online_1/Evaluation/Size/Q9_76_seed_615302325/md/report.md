...
**Test Query:** Q9_76_seed_615302325
...
```
Node 18880 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 18881 (Aggregate) [consumed]
    Node 18882 (Gather)
      Node 18883 (Aggregate)
        Node 18884 (Nested Loop)
          Node 18885 (Hash Join) [PATTERN: bb930825]
            Node 18886 (Seq Scan) [consumed] - LEAF
            Node 18887 (Hash) [consumed]
              Node 18888 (Hash Join) [consumed]
                Node 18889 (Hash Join)
                  Node 18890 (Nested Loop)
                    Node 18891 (Seq Scan) - LEAF
                    Node 18892 (Index Scan) - LEAF
                  Node 18893 (Hash)
                    Node 18894 (Seq Scan) - LEAF
                Node 18895 (Hash)
                  Node 18896 (Seq Scan) - LEAF
          Node 18897 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | bb930825 | 18885 | 18880, 18881, 18886, 18887, 18888 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 18880 | 18881, 18885, 18886, 18887, 18888 |
...
|------|------|--------|-----------|-----|--------|
| 18880 | Sort | 1196.17 | 1047.04 | 12.5% | pattern |
| 18882 | Gather | 1195.10 | 950.92 | 20.4% | operator |
| 18883 | Aggregate | 1177.50 | 1141.61 | 3.0% | operator |
| 18884 | Nested Loop | 1154.09 | 1252.17 | 8.5% | operator |
| 18885 | Hash Join | 981.41 | 134.80 | 86.3% | pattern |
| 18897 | Index Scan | 0.00 | -5.54 | 184812.7% | operator |
| 18889 | Hash Join | 166.94 | 837.40 | 401.6% | operator |
| 18895 | Hash | 13.37 | 14.95 | 11.8% | operator |
| 18890 | Nested Loop | 159.23 | 1251.76 | 686.1% | operator |
| 18893 | Hash | 5.20 | 15.94 | 206.7% | operator |
| 18896 | Seq Scan | 13.36 | 2.94 | 78.0% | operator |
| 18891 | Seq Scan | 22.29 | 44.51 | 99.7% | operator |
| 18892 | Index Scan | 0.07 | -1.39 | 1951.3% | operator |
| 18894 | Seq Scan | 4.50 | 9.45 | 109.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 18891 (Seq Scan) - LEAF
...
### Step 2: Node 18892 (Index Scan) - LEAF
...
### Step 3: Node 18894 (Seq Scan) - LEAF
...
### Step 6: Node 18896 (Seq Scan) - LEAF
...
### Step 9: Node 18885 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** bb930825 (Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)])
- **Consumes:** Nodes 18880, 18881, 18886, 18887, 18888
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 18897 (Index Scan) - LEAF
...
### Step 11: Node 18884 (Nested Loop)
...
### Step 14: Node 18880 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 18881, 18885, 18886, 18887, 18888
- **Input Features:**
