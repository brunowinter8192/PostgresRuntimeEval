...
**Test Query:** Q5_48_seed_385589457
...
```
Node 8741 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8742 (Aggregate) [consumed]
    Node 8743 (Gather Merge) [consumed]
      Node 8744 (Aggregate)
        Node 8745 (Sort) [PATTERN: 3e2d5a00]
          Node 8746 (Hash Join) [consumed]
            Node 8747 (Nested Loop)
              Node 8748 (Hash Join)
                Node 8749 (Seq Scan) - LEAF
                Node 8750 (Hash) [PATTERN: 444761fb]
                  Node 8751 (Hash Join) [consumed]
                    Node 8752 (Seq Scan) [consumed] - LEAF
                    Node 8753 (Hash) [consumed]
                      Node 8754 (Hash Join) [consumed]
                        Node 8755 (Seq Scan) - LEAF
                        Node 8756 (Hash)
                          Node 8757 (Seq Scan) - LEAF
              Node 8758 (Index Scan) - LEAF
            Node 8759 (Hash)
              Node 8760 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8750 | 8741, 8742, 8743, 8745, 8746, 8751, 8752, 8753, 8754 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8741 | 8742, 8743, 8745, 8746, 8750, 8751, 8752, 8753, 8754 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8745 | 8741, 8742, 8743, 8746, 8750, 8751, 8752, 8753, 8754 |
...
|------|------|--------|-----------|-----|--------|
| 8741 | Sort | 1046.70 | 2934.42 | 180.3% | pattern |
| 8744 | Aggregate | 1041.92 | 1128.78 | 8.3% | operator |
| 8745 | Sort | 1041.66 | 1835.92 | 76.2% | pattern |
| 8747 | Nested Loop | 1028.02 | 1188.42 | 15.6% | operator |
| 8759 | Hash | 4.96 | 18.67 | 276.4% | operator |
| 8748 | Hash Join | 211.65 | 253.07 | 19.6% | operator |
| 8758 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 8760 | Seq Scan | 4.47 | 12.36 | 176.4% | operator |
| 8749 | Seq Scan | 167.44 | 159.81 | 4.6% | operator |
| 8750 | Hash | 31.46 | 46.85 | 48.9% | pattern |
| 8755 | Seq Scan | 0.01 | 11.81 | 147503.4% | operator |
| 8756 | Hash | 0.06 | 21.05 | 38178.6% | operator |
| 8757 | Seq Scan | 0.05 | 22.93 | 43170.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8757 (Seq Scan) - LEAF
...
### Step 2: Node 8755 (Seq Scan) - LEAF
...
### Step 4: Node 8749 (Seq Scan) - LEAF
...
### Step 5: Node 8750 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8741, 8742, 8743, 8745, 8746, 8751, 8752, 8753, 8754
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8758 (Index Scan) - LEAF
...
### Step 8: Node 8760 (Seq Scan) - LEAF
...
### Step 11: Node 8745 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8741, 8742, 8743, 8746, 8750, 8751, 8752, 8753, 8754
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8741 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8742, 8743, 8745, 8746, 8750, 8751, 8752, 8753, 8754
- **Input Features:**
