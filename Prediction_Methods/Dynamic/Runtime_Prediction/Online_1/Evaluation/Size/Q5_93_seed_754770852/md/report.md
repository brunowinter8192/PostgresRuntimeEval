...
**Test Query:** Q5_93_seed_754770852
...
```
Node 9741 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9742 (Aggregate) [consumed]
    Node 9743 (Gather Merge) [consumed]
      Node 9744 (Aggregate)
        Node 9745 (Sort) [PATTERN: 3e2d5a00]
          Node 9746 (Hash Join) [consumed]
            Node 9747 (Nested Loop)
              Node 9748 (Hash Join)
                Node 9749 (Seq Scan) - LEAF
                Node 9750 (Hash) [PATTERN: 444761fb]
                  Node 9751 (Hash Join) [consumed]
                    Node 9752 (Seq Scan) [consumed] - LEAF
                    Node 9753 (Hash) [consumed]
                      Node 9754 (Hash Join) [consumed]
                        Node 9755 (Seq Scan) - LEAF
                        Node 9756 (Hash)
                          Node 9757 (Seq Scan) - LEAF
              Node 9758 (Index Scan) - LEAF
            Node 9759 (Hash)
              Node 9760 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9750 | 9741, 9742, 9743, 9745, 9746, 9751, 9752, 9753, 9754 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9741 | 9742, 9743, 9745, 9746, 9750, 9751, 9752, 9753, 9754 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9745 | 9741, 9742, 9743, 9746, 9750, 9751, 9752, 9753, 9754 |
...
|------|------|--------|-----------|-----|--------|
| 9741 | Sort | 1098.95 | 2934.42 | 167.0% | pattern |
| 9744 | Aggregate | 1093.88 | 1128.78 | 3.2% | operator |
| 9745 | Sort | 1093.63 | 1835.92 | 67.9% | pattern |
| 9747 | Nested Loop | 1080.55 | 1188.42 | 10.0% | operator |
| 9759 | Hash | 4.42 | 18.67 | 322.8% | operator |
| 9748 | Hash Join | 215.50 | 253.07 | 17.4% | operator |
| 9758 | Index Scan | 0.07 | -0.22 | 393.4% | operator |
| 9760 | Seq Scan | 3.69 | 12.36 | 235.1% | operator |
| 9749 | Seq Scan | 167.74 | 159.81 | 4.7% | operator |
| 9750 | Hash | 35.02 | 46.85 | 33.8% | pattern |
| 9755 | Seq Scan | 0.01 | 11.81 | 98302.3% | operator |
| 9756 | Hash | 0.07 | 21.05 | 29552.4% | operator |
| 9757 | Seq Scan | 0.07 | 22.93 | 34647.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9757 (Seq Scan) - LEAF
...
### Step 2: Node 9755 (Seq Scan) - LEAF
...
### Step 4: Node 9749 (Seq Scan) - LEAF
...
### Step 5: Node 9750 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9741, 9742, 9743, 9745, 9746, 9751, 9752, 9753, 9754
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9758 (Index Scan) - LEAF
...
### Step 8: Node 9760 (Seq Scan) - LEAF
...
### Step 11: Node 9745 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9741, 9742, 9743, 9746, 9750, 9751, 9752, 9753, 9754
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9741 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9742, 9743, 9745, 9746, 9750, 9751, 9752, 9753, 9754
- **Input Features:**
