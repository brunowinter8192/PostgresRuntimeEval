...
**Test Query:** Q10_54_seed_434813643
...
```
Node 20657 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20658 (Sort) [consumed]
    Node 20659 (Aggregate) [consumed]
      Node 20660 (Gather) [consumed]
        Node 20661 (Hash Join)
          Node 20662 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20663 (Nested Loop) [consumed]
              Node 20664 (Seq Scan) - LEAF
              Node 20665 (Index Scan) - LEAF
            Node 20666 (Hash) [consumed]
              Node 20667 (Seq Scan) - LEAF
          Node 20668 (Hash)
            Node 20669 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20657 | 20658, 20659, 20660, 20662, 20663, 20666 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20662 | 20657, 20658, 20659, 20660, 20663, 20666 |
...
|------|------|--------|-----------|-----|--------|
| 20657 | Limit | 1147.31 | 1199.47 | 4.5% | pattern |
| 20661 | Hash Join | 1088.98 | 1095.60 | 0.6% | operator |
| 20662 | Hash Join | 1086.05 | 1120.19 | 3.1% | pattern |
| 20668 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 20669 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20664 | Seq Scan | 162.90 | 159.74 | 1.9% | operator |
| 20665 | Index Scan | 0.06 | -0.52 | 949.6% | operator |
| 20667 | Seq Scan | 24.88 | 26.64 | 7.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20664 (Seq Scan) - LEAF
...
### Step 2: Node 20665 (Index Scan) - LEAF
...
### Step 3: Node 20667 (Seq Scan) - LEAF
...
### Step 4: Node 20669 (Seq Scan) - LEAF
...
### Step 5: Node 20662 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20657, 20658, 20659, 20660, 20663, 20666
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18384
  - HashJoin_nt1=18384
...
### Step 8: Node 20657 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20658, 20659, 20660, 20662, 20663, 20666
- **Input Features:**
