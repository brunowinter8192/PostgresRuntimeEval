...
**Test Query:** Q5_67_seed_541466046
...
```
Node 9161 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9162 (Aggregate) [consumed]
    Node 9163 (Gather Merge) [consumed]
      Node 9164 (Aggregate)
        Node 9165 (Sort) [PATTERN: 3e2d5a00]
          Node 9166 (Hash Join) [consumed]
            Node 9167 (Nested Loop)
              Node 9168 (Hash Join)
                Node 9169 (Seq Scan) - LEAF
                Node 9170 (Hash) [PATTERN: 444761fb]
                  Node 9171 (Hash Join) [consumed]
                    Node 9172 (Seq Scan) [consumed] - LEAF
                    Node 9173 (Hash) [consumed]
                      Node 9174 (Hash Join) [consumed]
                        Node 9175 (Seq Scan) - LEAF
                        Node 9176 (Hash)
                          Node 9177 (Seq Scan) - LEAF
              Node 9178 (Index Scan) - LEAF
            Node 9179 (Hash)
              Node 9180 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9170 | 9161, 9162, 9163, 9165, 9166, 9171, 9172, 9173, 9174 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9161 | 9162, 9163, 9165, 9166, 9170, 9171, 9172, 9173, 9174 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9165 | 9161, 9162, 9163, 9166, 9170, 9171, 9172, 9173, 9174 |
...
|------|------|--------|-----------|-----|--------|
| 9161 | Sort | 1034.83 | 2934.42 | 183.6% | pattern |
| 9164 | Aggregate | 1028.86 | 1128.78 | 9.7% | operator |
| 9165 | Sort | 1028.60 | 1835.91 | 78.5% | pattern |
| 9167 | Nested Loop | 1016.32 | 1188.39 | 16.9% | operator |
| 9179 | Hash | 3.90 | 18.67 | 379.4% | operator |
| 9168 | Hash Join | 206.07 | 253.07 | 22.8% | operator |
| 9178 | Index Scan | 0.07 | -0.22 | 405.6% | operator |
| 9180 | Seq Scan | 3.00 | 12.36 | 312.3% | operator |
| 9169 | Seq Scan | 161.90 | 159.81 | 1.3% | operator |
| 9170 | Hash | 31.86 | 46.85 | 47.0% | pattern |
| 9175 | Seq Scan | 0.01 | 11.81 | 84244.8% | operator |
| 9176 | Hash | 0.06 | 21.05 | 33317.8% | operator |
| 9177 | Seq Scan | 0.06 | 22.93 | 41597.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9177 (Seq Scan) - LEAF
...
### Step 2: Node 9175 (Seq Scan) - LEAF
...
### Step 4: Node 9169 (Seq Scan) - LEAF
...
### Step 5: Node 9170 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9161, 9162, 9163, 9165, 9166, 9171, 9172, 9173, 9174
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9178 (Index Scan) - LEAF
...
### Step 8: Node 9180 (Seq Scan) - LEAF
...
### Step 11: Node 9165 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9161, 9162, 9163, 9166, 9170, 9171, 9172, 9173, 9174
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9161 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9162, 9163, 9165, 9166, 9170, 9171, 9172, 9173, 9174
- **Input Features:**
