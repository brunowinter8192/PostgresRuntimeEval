...
**Test Query:** Q10_105_seed_853219224
...
```
Node 19435 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19436 (Sort) [consumed]
    Node 19437 (Aggregate) [consumed]
      Node 19438 (Gather) [consumed]
        Node 19439 (Hash Join)
          Node 19440 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19441 (Nested Loop) [consumed]
              Node 19442 (Seq Scan) - LEAF
              Node 19443 (Index Scan) - LEAF
            Node 19444 (Hash) [consumed]
              Node 19445 (Seq Scan) - LEAF
          Node 19446 (Hash)
            Node 19447 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19435 | 19436, 19437, 19438, 19440, 19441, 19444 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19440 | 19435, 19436, 19437, 19438, 19441, 19444 |
...
|------|------|--------|-----------|-----|--------|
| 19435 | Limit | 1135.81 | 1199.47 | 5.6% | pattern |
| 19439 | Hash Join | 1090.00 | 1095.40 | 0.5% | operator |
| 19440 | Hash Join | 1087.25 | 1120.19 | 3.0% | pattern |
| 19446 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 19447 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 19442 | Seq Scan | 161.21 | 159.75 | 0.9% | operator |
| 19443 | Index Scan | 0.06 | -0.52 | 935.8% | operator |
| 19445 | Seq Scan | 25.95 | 26.64 | 2.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19442 (Seq Scan) - LEAF
...
### Step 2: Node 19443 (Index Scan) - LEAF
...
### Step 3: Node 19445 (Seq Scan) - LEAF
...
### Step 4: Node 19447 (Seq Scan) - LEAF
...
### Step 5: Node 19440 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19435, 19436, 19437, 19438, 19441, 19444
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17639
  - HashJoin_nt1=17639
...
### Step 8: Node 19435 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19436, 19437, 19438, 19440, 19441, 19444
- **Input Features:**
