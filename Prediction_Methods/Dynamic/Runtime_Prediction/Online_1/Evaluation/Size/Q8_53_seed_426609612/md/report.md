...
**Test Query:** Q8_53_seed_426609612
...
```
Node 15455 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15456 (Gather Merge) [consumed]
    Node 15457 (Sort)
      Node 15458 (Hash Join)
        Node 15459 (Nested Loop)
          Node 15460 (Hash Join)
            Node 15461 (Nested Loop)
              Node 15462 (Hash Join)
                Node 15463 (Seq Scan) - LEAF
                Node 15464 (Hash) [PATTERN: 444761fb]
                  Node 15465 (Hash Join) [consumed]
                    Node 15466 (Seq Scan) [consumed] - LEAF
                    Node 15467 (Hash) [consumed]
                      Node 15468 (Hash Join) [consumed]
                        Node 15469 (Seq Scan) - LEAF
                        Node 15470 (Hash)
                          Node 15471 (Seq Scan) - LEAF
              Node 15472 (Index Scan) - LEAF
            Node 15473 (Hash)
              Node 15474 (Seq Scan) - LEAF
          Node 15475 (Index Scan) - LEAF
        Node 15476 (Hash)
          Node 15477 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15464 | 15455, 15456, 15465, 15466, 15467, 15468 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15455 | 15456, 15464, 15465, 15466, 15467, 15468 |
...
|------|------|--------|-----------|-----|--------|
| 15455 | Aggregate | 1200.37 | 1129.58 | 5.9% | pattern |
| 15457 | Sort | 1195.01 | 1046.21 | 12.5% | operator |
| 15458 | Hash Join | 1194.74 | 785.61 | 34.2% | operator |
| 15459 | Nested Loop | 1194.40 | 1198.29 | 0.3% | operator |
| 15476 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 15460 | Hash Join | 1190.68 | 785.60 | 34.0% | operator |
| 15475 | Index Scan | 0.01 | 12.42 | 248369.7% | operator |
| 15477 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 15461 | Nested Loop | 1147.03 | 1198.29 | 4.5% | operator |
| 15473 | Hash | 39.05 | 19.97 | 48.9% | operator |
| 15462 | Hash Join | 214.87 | 251.67 | 17.1% | operator |
| 15472 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15474 | Seq Scan | 38.90 | 48.87 | 25.6% | operator |
| 15463 | Seq Scan | 158.86 | 159.67 | 0.5% | operator |
| 15464 | Hash | 37.52 | 43.51 | 16.0% | pattern |
| 15469 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 15470 | Hash | 0.20 | 18.14 | 8794.2% | operator |
| 15471 | Seq Scan | 0.20 | 23.36 | 11581.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15471 (Seq Scan) - LEAF
...
### Step 2: Node 15469 (Seq Scan) - LEAF
...
### Step 4: Node 15463 (Seq Scan) - LEAF
...
### Step 5: Node 15464 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15455, 15456, 15465, 15466, 15467, 15468
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15472 (Index Scan) - LEAF
...
### Step 8: Node 15474 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9689
  - startup_cost=9920.2800
  - total_cost=70271.7500
- **Output:** st=113.62, rt=785.60

### Step 12: Node 15475 (Index Scan) - LEAF
...
### Step 13: Node 15477 (Seq Scan) - LEAF
...
### Step 14: Node 15459 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.1200
  - total_cost=70520.7300
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15457 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70559.4700
  - total_cost=70561.4800
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 15455 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15456, 15464, 15465, 15466, 15467, 15468
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2488
  ...
  - Aggregate_sel=0.9670
  - Aggregate_startup_cost=71559.5100
  - Aggregate_total_cost=71951.7400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2488
  - GatherMerge_Outer_nt1=803
  ...
  - GatherMerge_Outer_startup_cost=71559.5100
  - GatherMerge_Outer_total_cost=71853.8600
- **Output:** st=1126.62, rt=1129.58
