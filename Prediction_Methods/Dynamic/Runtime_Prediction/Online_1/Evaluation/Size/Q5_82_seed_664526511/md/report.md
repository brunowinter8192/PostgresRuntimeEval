...
**Test Query:** Q5_82_seed_664526511
...
```
Node 9501 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9502 (Aggregate) [consumed]
    Node 9503 (Gather Merge) [consumed]
      Node 9504 (Aggregate)
        Node 9505 (Sort) [PATTERN: 3e2d5a00]
          Node 9506 (Hash Join) [consumed]
            Node 9507 (Nested Loop)
              Node 9508 (Hash Join)
                Node 9509 (Seq Scan) - LEAF
                Node 9510 (Hash) [PATTERN: 444761fb]
                  Node 9511 (Hash Join) [consumed]
                    Node 9512 (Seq Scan) [consumed] - LEAF
                    Node 9513 (Hash) [consumed]
                      Node 9514 (Hash Join) [consumed]
                        Node 9515 (Seq Scan) - LEAF
                        Node 9516 (Hash)
                          Node 9517 (Seq Scan) - LEAF
              Node 9518 (Index Scan) - LEAF
            Node 9519 (Hash)
              Node 9520 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9510 | 9501, 9502, 9503, 9505, 9506, 9511, 9512, 9513, 9514 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9501 | 9502, 9503, 9505, 9506, 9510, 9511, 9512, 9513, 9514 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9505 | 9501, 9502, 9503, 9506, 9510, 9511, 9512, 9513, 9514 |
...
|------|------|--------|-----------|-----|--------|
| 9501 | Sort | 1081.68 | 2934.42 | 171.3% | pattern |
| 9504 | Aggregate | 1076.10 | 1128.74 | 4.9% | operator |
| 9505 | Sort | 1075.86 | 1835.96 | 70.7% | pattern |
| 9507 | Nested Loop | 1063.90 | 1188.57 | 11.7% | operator |
| 9519 | Hash | 3.41 | 18.67 | 447.5% | operator |
| 9508 | Hash Join | 216.01 | 253.07 | 17.2% | operator |
| 9518 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 9520 | Seq Scan | 2.85 | 12.36 | 334.0% | operator |
| 9509 | Seq Scan | 168.77 | 159.81 | 5.3% | operator |
| 9510 | Hash | 34.29 | 46.85 | 36.6% | pattern |
| 9515 | Seq Scan | 0.01 | 11.81 | 90732.9% | operator |
| 9516 | Hash | 0.06 | 21.05 | 36835.5% | operator |
| 9517 | Seq Scan | 0.06 | 22.93 | 41597.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9517 (Seq Scan) - LEAF
...
### Step 2: Node 9515 (Seq Scan) - LEAF
...
### Step 4: Node 9509 (Seq Scan) - LEAF
...
### Step 5: Node 9510 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9501, 9502, 9503, 9505, 9506, 9511, 9512, 9513, 9514
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9518 (Index Scan) - LEAF
...
### Step 8: Node 9520 (Seq Scan) - LEAF
...
### Step 11: Node 9505 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9501, 9502, 9503, 9506, 9510, 9511, 9512, 9513, 9514
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9501 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9502, 9503, 9505, 9506, 9510, 9511, 9512, 9513, 9514
- **Input Features:**
