...
```
Node 15616 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15617 (Gather Merge) [consumed]
    Node 15618 (Sort)
      Node 15619 (Hash Join)
        Node 15620 (Nested Loop)
          Node 15621 (Hash Join)
            Node 15622 (Nested Loop)
              Node 15623 (Hash Join)
                Node 15624 (Seq Scan) - LEAF
                Node 15625 (Hash) [PATTERN: 444761fb]
                  Node 15626 (Hash Join) [consumed]
                    Node 15627 (Seq Scan) [consumed] - LEAF
                    Node 15628 (Hash) [consumed]
                      Node 15629 (Hash Join) [consumed]
                        Node 15630 (Seq Scan) - LEAF
                        Node 15631 (Hash)
                          Node 15632 (Seq Scan) - LEAF
              Node 15633 (Index Scan) - LEAF
            Node 15634 (Hash)
              Node 15635 (Seq Scan) - LEAF
          Node 15636 (Index Scan) - LEAF
        Node 15637 (Hash)
          Node 15638 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15625 | 15616, 15617, 15626, 15627, 15628, 15629 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15616 | 15617, 15625, 15626, 15627, 15628, 15629 |
...
|------|------|--------|-----------|-----|--------|
| 15616 | Aggregate | 1239.36 | 1130.53 | 8.8% | pattern |
| 15618 | Sort | 1230.96 | 1046.21 | 15.0% | operator |
| 15619 | Hash Join | 1230.72 | 785.61 | 36.2% | operator |
| 15620 | Nested Loop | 1230.41 | 1198.29 | 2.6% | operator |
| 15637 | Hash | 0.02 | 13.72 | 80629.9% | operator |
| 15621 | Hash Join | 1226.74 | 785.60 | 36.0% | operator |
| 15636 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15638 | Seq Scan | 0.01 | 6.51 | 49992.0% | operator |
| 15622 | Nested Loop | 1183.60 | 1198.29 | 1.2% | operator |
| 15634 | Hash | 38.68 | 19.97 | 48.4% | operator |
| 15623 | Hash Join | 216.57 | 251.67 | 16.2% | operator |
| 15633 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 15635 | Seq Scan | 38.63 | 48.87 | 26.5% | operator |
| 15624 | Seq Scan | 159.45 | 159.67 | 0.1% | operator |
| 15625 | Hash | 38.38 | 43.51 | 13.4% | pattern |
| 15630 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 15631 | Hash | 0.21 | 18.14 | 8458.6% | operator |
| 15632 | Seq Scan | 0.21 | 23.36 | 11131.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15632 (Seq Scan) - LEAF
...
### Step 2: Node 15630 (Seq Scan) - LEAF
...
### Step 4: Node 15624 (Seq Scan) - LEAF
...
### Step 5: Node 15625 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15616, 15617, 15626, 15627, 15628, 15629
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15633 (Index Scan) - LEAF
...
### Step 8: Node 15635 (Seq Scan) - LEAF
...
### Step 12: Node 15636 (Index Scan) - LEAF
...
### Step 13: Node 15638 (Seq Scan) - LEAF
...
### Step 14: Node 15620 (Nested Loop)
...
### Step 18: Node 15616 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15617, 15625, 15626, 15627, 15628, 15629
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2468
