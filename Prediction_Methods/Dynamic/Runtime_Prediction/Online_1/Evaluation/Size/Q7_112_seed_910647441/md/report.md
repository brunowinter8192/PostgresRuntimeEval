...
**Test Query:** Q7_112_seed_910647441
...
```
Node 10735 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10736 (Gather Merge) [consumed]
    Node 10737 (Sort)
      Node 10738 (Hash Join)
        Node 10739 (Nested Loop)
          Node 10740 (Hash Join) [PATTERN: 895c6e8c]
            Node 10741 (Seq Scan) [consumed] - LEAF
            Node 10742 (Hash) [consumed]
              Node 10743 (Hash Join) [PATTERN: f4cb205a]
                Node 10744 (Seq Scan) [consumed] - LEAF
                Node 10745 (Hash) [consumed]
                  Node 10746 (Seq Scan) [consumed] - LEAF
          Node 10747 (Index Scan) - LEAF
        Node 10748 (Hash)
          Node 10749 (Hash Join) [PATTERN: f4cb205a]
            Node 10750 (Seq Scan) [consumed] - LEAF
            Node 10751 (Hash) [consumed]
              Node 10752 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10743 | 10735, 10736, 10740, 10741, 10742, 10744, 10745, 10746, 10749, 10750, 10751, 10752 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10749 | 10735, 10736, 10740, 10741, 10742, 10743, 10744, 10745, 10746, 10750, 10751, 10752 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10735 | 10736, 10740, 10741, 10742, 10743, 10744, 10745, 10746, 10749, 10750, 10751, 10752 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10740 | 10735, 10736, 10741, 10742, 10743, 10744, 10745, 10746, 10749, 10750, 10751, 10752 |
...
|------|------|--------|-----------|-----|--------|
| 10735 | Aggregate | 1176.29 | 1163.13 | 1.1% | pattern |
| 10737 | Sort | 1168.42 | 1109.16 | 5.1% | operator |
| 10738 | Hash Join | 1167.83 | 843.11 | 27.8% | operator |
| 10739 | Nested Loop | 1161.08 | 1215.99 | 4.7% | operator |
| 10748 | Hash | 3.05 | 27.74 | 810.5% | operator |
| 10740 | Hash Join | 212.58 | 304.89 | 43.4% | pattern |
| 10747 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 10749 | Hash Join | 3.01 | 429.36 | 14169.3% | pattern |
| 10743 | Hash Join | 36.84 | 429.38 | 1065.5% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10743 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10735, 10736, 10740, 10741, 10742, 10744, 10745, 10746, 10749, 10750, 10751, 10752
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10740 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10735, 10736, 10741, 10742, 10743, 10744, 10745, 10746, 10749, 10750, 10751, 10752
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10747 (Index Scan) - LEAF
...
### Step 4: Node 10749 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10735, 10736, 10740, 10741, 10742, 10743, 10744, 10745, 10746, 10750, 10751, 10752
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10735 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10736, 10740, 10741, 10742, 10743, 10744, 10745, 10746, 10749, 10750, 10751, 10752
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
