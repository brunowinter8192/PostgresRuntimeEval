...
**Test Query:** Q10_136_seed_1107544185
...
```
Node 19877 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19878 (Sort) [consumed]
    Node 19879 (Aggregate) [consumed]
      Node 19880 (Gather) [consumed]
        Node 19881 (Hash Join)
          Node 19882 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19883 (Nested Loop) [consumed]
              Node 19884 (Seq Scan) - LEAF
              Node 19885 (Index Scan) - LEAF
            Node 19886 (Hash) [consumed]
              Node 19887 (Seq Scan) - LEAF
          Node 19888 (Hash)
            Node 19889 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19877 | 19878, 19879, 19880, 19882, 19883, 19886 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19882 | 19877, 19878, 19879, 19880, 19883, 19886 |
...
|------|------|--------|-----------|-----|--------|
| 19877 | Limit | 1155.62 | 1199.47 | 3.8% | pattern |
| 19881 | Hash Join | 1114.80 | 1095.55 | 1.7% | operator |
| 19882 | Hash Join | 1111.86 | 1120.19 | 0.7% | pattern |
| 19888 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 19889 | Seq Scan | 0.01 | 6.98 | 53582.3% | operator |
| 19884 | Seq Scan | 165.82 | 159.74 | 3.7% | operator |
| 19885 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 19887 | Seq Scan | 27.95 | 26.64 | 4.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19884 (Seq Scan) - LEAF
...
### Step 2: Node 19885 (Index Scan) - LEAF
...
### Step 3: Node 19887 (Seq Scan) - LEAF
...
### Step 4: Node 19889 (Seq Scan) - LEAF
...
### Step 5: Node 19882 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19877, 19878, 19879, 19880, 19883, 19886
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18183
  - HashJoin_nt1=18183
...
### Step 8: Node 19877 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19878, 19879, 19880, 19882, 19883, 19886
- **Input Features:**
