...
**Test Query:** Q7_145_seed_1181380464
...
```
Node 11383 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11384 (Gather Merge) [consumed]
    Node 11385 (Sort)
      Node 11386 (Hash Join)
        Node 11387 (Nested Loop)
          Node 11388 (Hash Join) [PATTERN: 895c6e8c]
            Node 11389 (Seq Scan) [consumed] - LEAF
            Node 11390 (Hash) [consumed]
              Node 11391 (Hash Join) [PATTERN: f4cb205a]
                Node 11392 (Seq Scan) [consumed] - LEAF
                Node 11393 (Hash) [consumed]
                  Node 11394 (Seq Scan) [consumed] - LEAF
          Node 11395 (Index Scan) - LEAF
        Node 11396 (Hash)
          Node 11397 (Hash Join) [PATTERN: f4cb205a]
            Node 11398 (Seq Scan) [consumed] - LEAF
            Node 11399 (Hash) [consumed]
              Node 11400 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11391 | 11383, 11384, 11388, 11389, 11390, 11392, 11393, 11394, 11397, 11398, 11399, 11400 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11397 | 11383, 11384, 11388, 11389, 11390, 11391, 11392, 11393, 11394, 11398, 11399, 11400 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11383 | 11384, 11388, 11389, 11390, 11391, 11392, 11393, 11394, 11397, 11398, 11399, 11400 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11388 | 11383, 11384, 11389, 11390, 11391, 11392, 11393, 11394, 11397, 11398, 11399, 11400 |
...
|------|------|--------|-----------|-----|--------|
| 11383 | Aggregate | 1188.76 | 1163.13 | 2.2% | pattern |
| 11385 | Sort | 1182.85 | 1109.16 | 6.2% | operator |
| 11386 | Hash Join | 1182.24 | 843.11 | 28.7% | operator |
| 11387 | Nested Loop | 1175.33 | 1215.99 | 3.5% | operator |
| 11396 | Hash | 3.15 | 27.74 | 781.9% | operator |
| 11388 | Hash Join | 214.12 | 304.89 | 42.4% | pattern |
| 11395 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11397 | Hash Join | 3.10 | 429.36 | 13754.9% | pattern |
| 11391 | Hash Join | 35.84 | 429.38 | 1097.9% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11391 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11383, 11384, 11388, 11389, 11390, 11392, 11393, 11394, 11397, 11398, 11399, 11400
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11388 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11383, 11384, 11389, 11390, 11391, 11392, 11393, 11394, 11397, 11398, 11399, 11400
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11395 (Index Scan) - LEAF
...
### Step 4: Node 11397 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11383, 11384, 11388, 11389, 11390, 11391, 11392, 11393, 11394, 11398, 11399, 11400
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11383 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11384, 11388, 11389, 11390, 11391, 11392, 11393, 11394, 11397, 11398, 11399, 11400
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
