...
**Test Query:** Q10_21_seed_164080620
...
```
Node 20189 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20190 (Sort) [consumed]
    Node 20191 (Aggregate) [consumed]
      Node 20192 (Gather) [consumed]
        Node 20193 (Hash Join)
          Node 20194 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20195 (Nested Loop) [consumed]
              Node 20196 (Seq Scan) - LEAF
              Node 20197 (Index Scan) - LEAF
            Node 20198 (Hash) [consumed]
              Node 20199 (Seq Scan) - LEAF
          Node 20200 (Hash)
            Node 20201 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20189 | 20190, 20191, 20192, 20194, 20195, 20198 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20194 | 20189, 20190, 20191, 20192, 20195, 20198 |
...
|------|------|--------|-----------|-----|--------|
| 20189 | Limit | 1176.60 | 1199.47 | 1.9% | pattern |
| 20193 | Hash Join | 1129.92 | 1095.57 | 3.0% | operator |
| 20194 | Hash Join | 1127.02 | 1120.19 | 0.6% | pattern |
| 20200 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 20201 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20196 | Seq Scan | 164.50 | 159.74 | 2.9% | operator |
| 20197 | Index Scan | 0.06 | -0.52 | 909.7% | operator |
| 20199 | Seq Scan | 25.98 | 26.64 | 2.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20196 (Seq Scan) - LEAF
...
### Step 2: Node 20197 (Index Scan) - LEAF
...
### Step 3: Node 20199 (Seq Scan) - LEAF
...
### Step 4: Node 20201 (Seq Scan) - LEAF
...
### Step 5: Node 20194 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20189, 20190, 20191, 20192, 20195, 20198
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18240
  - HashJoin_nt1=18240
...
### Step 8: Node 20189 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20190, 20191, 20192, 20194, 20195, 20198
- **Input Features:**
