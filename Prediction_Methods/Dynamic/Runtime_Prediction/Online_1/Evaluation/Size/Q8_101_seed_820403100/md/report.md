...
**Test Query:** Q8_101_seed_820403100
...
```
Node 13224 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13225 (Gather Merge) [consumed]
    Node 13226 (Sort)
      Node 13227 (Hash Join)
        Node 13228 (Nested Loop)
          Node 13229 (Hash Join)
            Node 13230 (Nested Loop)
              Node 13231 (Hash Join)
                Node 13232 (Seq Scan) - LEAF
                Node 13233 (Hash) [PATTERN: 444761fb]
                  Node 13234 (Hash Join) [consumed]
                    Node 13235 (Seq Scan) [consumed] - LEAF
                    Node 13236 (Hash) [consumed]
                      Node 13237 (Hash Join) [consumed]
                        Node 13238 (Seq Scan) - LEAF
                        Node 13239 (Hash)
                          Node 13240 (Seq Scan) - LEAF
              Node 13241 (Index Scan) - LEAF
            Node 13242 (Hash)
              Node 13243 (Seq Scan) - LEAF
          Node 13244 (Index Scan) - LEAF
        Node 13245 (Hash)
          Node 13246 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13233 | 13224, 13225, 13234, 13235, 13236, 13237 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13224 | 13225, 13233, 13234, 13235, 13236, 13237 |
...
|------|------|--------|-----------|-----|--------|
| 13224 | Aggregate | 1239.22 | 1133.65 | 8.5% | pattern |
| 13226 | Sort | 1233.17 | 1046.21 | 15.2% | operator |
| 13227 | Hash Join | 1232.93 | 785.61 | 36.3% | operator |
| 13228 | Nested Loop | 1232.63 | 1198.29 | 2.8% | operator |
| 13245 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 13229 | Hash Join | 1228.93 | 785.60 | 36.1% | operator |
| 13244 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13246 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13230 | Nested Loop | 1183.71 | 1198.29 | 1.2% | operator |
| 13242 | Hash | 40.88 | 19.97 | 51.1% | operator |
| 13231 | Hash Join | 214.61 | 251.67 | 17.3% | operator |
| 13241 | Index Scan | 0.04 | 2.88 | 6599.7% | operator |
| 13243 | Seq Scan | 40.82 | 48.88 | 19.7% | operator |
| 13232 | Seq Scan | 162.15 | 159.67 | 1.5% | operator |
| 13233 | Hash | 34.84 | 43.51 | 24.9% | pattern |
| 13238 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13239 | Hash | 0.21 | 18.14 | 8665.3% | operator |
| 13240 | Seq Scan | 0.20 | 23.36 | 11352.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13240 (Seq Scan) - LEAF
...
### Step 2: Node 13238 (Seq Scan) - LEAF
...
### Step 4: Node 13232 (Seq Scan) - LEAF
...
### Step 5: Node 13233 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13224, 13225, 13234, 13235, 13236, 13237
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13241 (Index Scan) - LEAF
...
### Step 8: Node 13243 (Seq Scan) - LEAF
...
### Step 12: Node 13244 (Index Scan) - LEAF
...
### Step 13: Node 13246 (Seq Scan) - LEAF
...
### Step 14: Node 13228 (Nested Loop)
...
### Step 18: Node 13224 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13225, 13233, 13234, 13235, 13236, 13237
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2385
  - Aggregate_nt1=2385
  ...
  - Aggregate_total_cost=71924.4200
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2385
  ...
  - GatherMerge_Outer_total_cost=71829.0200
