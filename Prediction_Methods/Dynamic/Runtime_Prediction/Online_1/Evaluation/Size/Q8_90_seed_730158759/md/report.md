...
**Test Query:** Q8_90_seed_730158759
...
```
Node 16398 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16399 (Gather Merge) [consumed]
    Node 16400 (Sort)
      Node 16401 (Hash Join)
        Node 16402 (Nested Loop)
          Node 16403 (Hash Join)
            Node 16404 (Nested Loop)
              Node 16405 (Hash Join)
                Node 16406 (Seq Scan) - LEAF
                Node 16407 (Hash) [PATTERN: 444761fb]
                  Node 16408 (Hash Join) [consumed]
                    Node 16409 (Seq Scan) [consumed] - LEAF
                    Node 16410 (Hash) [consumed]
                      Node 16411 (Hash Join) [consumed]
                        Node 16412 (Seq Scan) - LEAF
                        Node 16413 (Hash)
                          Node 16414 (Seq Scan) - LEAF
              Node 16415 (Index Scan) - LEAF
            Node 16416 (Hash)
              Node 16417 (Seq Scan) - LEAF
          Node 16418 (Index Scan) - LEAF
        Node 16419 (Hash)
          Node 16420 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16407 | 16398, 16399, 16408, 16409, 16410, 16411 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16398 | 16399, 16407, 16408, 16409, 16410, 16411 |
...
|------|------|--------|-----------|-----|--------|
| 16398 | Aggregate | 1211.66 | 1131.10 | 6.6% | pattern |
| 16400 | Sort | 1205.64 | 1046.21 | 13.2% | operator |
| 16401 | Hash Join | 1205.41 | 785.61 | 34.8% | operator |
| 16402 | Nested Loop | 1205.10 | 1198.29 | 0.6% | operator |
| 16419 | Hash | 0.02 | 13.72 | 76144.9% | operator |
| 16403 | Hash Join | 1201.19 | 785.60 | 34.6% | operator |
| 16418 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16420 | Seq Scan | 0.01 | 6.51 | 46414.0% | operator |
| 16404 | Nested Loop | 1154.60 | 1198.29 | 3.8% | operator |
| 16416 | Hash | 42.16 | 19.97 | 52.6% | operator |
| 16405 | Hash Join | 209.13 | 251.67 | 20.3% | operator |
| 16415 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 16417 | Seq Scan | 42.08 | 48.87 | 16.2% | operator |
| 16406 | Seq Scan | 158.27 | 159.67 | 0.9% | operator |
| 16407 | Hash | 32.99 | 43.51 | 31.9% | pattern |
| 16412 | Seq Scan | 0.01 | 12.22 | 87221.0% | operator |
| 16413 | Hash | 0.21 | 18.14 | 8418.4% | operator |
| 16414 | Seq Scan | 0.21 | 23.36 | 11078.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16414 (Seq Scan) - LEAF
...
### Step 2: Node 16412 (Seq Scan) - LEAF
...
### Step 4: Node 16406 (Seq Scan) - LEAF
...
### Step 5: Node 16407 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16398, 16399, 16408, 16409, 16410, 16411
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16415 (Index Scan) - LEAF
...
### Step 8: Node 16417 (Seq Scan) - LEAF
...
### Step 12: Node 16418 (Index Scan) - LEAF
...
### Step 13: Node 16420 (Seq Scan) - LEAF
...
### Step 14: Node 16402 (Nested Loop)
...
### Step 18: Node 16398 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16399, 16407, 16408, 16409, 16410, 16411
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2457
  ...
  - Aggregate_sel=0.9792
  - Aggregate_startup_cost=71555.8000
  - Aggregate_total_cost=71943.7400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2457
  ...
  - GatherMerge_Outer_total_cost=71846.4800
- **Output:** st=1128.12, rt=1131.10
