...
**Test Query:** Q7_113_seed_918851472
...
```
Node 10753 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10754 (Gather Merge) [consumed]
    Node 10755 (Sort)
      Node 10756 (Hash Join)
        Node 10757 (Nested Loop)
          Node 10758 (Hash Join) [PATTERN: 895c6e8c]
            Node 10759 (Seq Scan) [consumed] - LEAF
            Node 10760 (Hash) [consumed]
              Node 10761 (Hash Join) [PATTERN: f4cb205a]
                Node 10762 (Seq Scan) [consumed] - LEAF
                Node 10763 (Hash) [consumed]
                  Node 10764 (Seq Scan) [consumed] - LEAF
          Node 10765 (Index Scan) - LEAF
        Node 10766 (Hash)
          Node 10767 (Hash Join) [PATTERN: f4cb205a]
            Node 10768 (Seq Scan) [consumed] - LEAF
            Node 10769 (Hash) [consumed]
              Node 10770 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10761 | 10753, 10754, 10758, 10759, 10760, 10762, 10763, 10764, 10767, 10768, 10769, 10770 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10767 | 10753, 10754, 10758, 10759, 10760, 10761, 10762, 10763, 10764, 10768, 10769, 10770 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10753 | 10754, 10758, 10759, 10760, 10761, 10762, 10763, 10764, 10767, 10768, 10769, 10770 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10758 | 10753, 10754, 10759, 10760, 10761, 10762, 10763, 10764, 10767, 10768, 10769, 10770 |
...
|------|------|--------|-----------|-----|--------|
| 10753 | Aggregate | 1203.55 | 1163.13 | 3.4% | pattern |
| 10755 | Sort | 1195.59 | 1109.16 | 7.2% | operator |
| 10756 | Hash Join | 1194.95 | 843.11 | 29.4% | operator |
| 10757 | Nested Loop | 1187.38 | 1215.99 | 2.4% | operator |
| 10766 | Hash | 3.81 | 27.74 | 627.6% | operator |
| 10758 | Hash Join | 215.16 | 304.89 | 41.7% | pattern |
| 10765 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10767 | Hash Join | 3.77 | 429.36 | 11298.0% | pattern |
| 10761 | Hash Join | 35.35 | 429.38 | 1114.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10761 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10753, 10754, 10758, 10759, 10760, 10762, 10763, 10764, 10767, 10768, 10769, 10770
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10758 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10753, 10754, 10759, 10760, 10761, 10762, 10763, 10764, 10767, 10768, 10769, 10770
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10765 (Index Scan) - LEAF
...
### Step 4: Node 10767 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10753, 10754, 10758, 10759, 10760, 10761, 10762, 10763, 10764, 10768, 10769, 10770
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10753 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10754, 10758, 10759, 10760, 10761, 10762, 10763, 10764, 10767, 10768, 10769, 10770
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
