...
**Test Query:** Q5_135_seed_1099340154
...
```
Node 7661 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7662 (Aggregate) [consumed]
    Node 7663 (Gather Merge) [consumed]
      Node 7664 (Aggregate)
        Node 7665 (Sort) [PATTERN: 3e2d5a00]
          Node 7666 (Hash Join) [consumed]
            Node 7667 (Nested Loop)
              Node 7668 (Hash Join)
                Node 7669 (Seq Scan) - LEAF
                Node 7670 (Hash) [PATTERN: 444761fb]
                  Node 7671 (Hash Join) [consumed]
                    Node 7672 (Seq Scan) [consumed] - LEAF
                    Node 7673 (Hash) [consumed]
                      Node 7674 (Hash Join) [consumed]
                        Node 7675 (Seq Scan) - LEAF
                        Node 7676 (Hash)
                          Node 7677 (Seq Scan) - LEAF
              Node 7678 (Index Scan) - LEAF
            Node 7679 (Hash)
              Node 7680 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7670 | 7661, 7662, 7663, 7665, 7666, 7671, 7672, 7673, 7674 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7661 | 7662, 7663, 7665, 7666, 7670, 7671, 7672, 7673, 7674 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7665 | 7661, 7662, 7663, 7666, 7670, 7671, 7672, 7673, 7674 |
...
|------|------|--------|-----------|-----|--------|
| 7661 | Sort | 1060.87 | 2934.42 | 176.6% | pattern |
| 7664 | Aggregate | 1056.25 | 1128.74 | 6.9% | operator |
| 7665 | Sort | 1056.01 | 1835.96 | 73.9% | pattern |
| 7667 | Nested Loop | 1042.88 | 1188.57 | 14.0% | operator |
| 7679 | Hash | 4.48 | 18.67 | 316.9% | operator |
| 7668 | Hash Join | 210.02 | 253.07 | 20.5% | operator |
| 7678 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 7680 | Seq Scan | 3.96 | 12.36 | 212.4% | operator |
| 7669 | Seq Scan | 164.35 | 159.81 | 2.8% | operator |
| 7670 | Hash | 32.84 | 46.85 | 42.7% | pattern |
| 7675 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 7676 | Hash | 0.06 | 21.05 | 37495.1% | operator |
| 7677 | Seq Scan | 0.05 | 22.93 | 43170.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7677 (Seq Scan) - LEAF
...
### Step 2: Node 7675 (Seq Scan) - LEAF
...
### Step 4: Node 7669 (Seq Scan) - LEAF
...
### Step 5: Node 7670 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7661, 7662, 7663, 7665, 7666, 7671, 7672, 7673, 7674
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7678 (Index Scan) - LEAF
...
### Step 8: Node 7680 (Seq Scan) - LEAF
...
### Step 11: Node 7665 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7661, 7662, 7663, 7666, 7670, 7671, 7672, 7673, 7674
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7661 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7662, 7663, 7665, 7666, 7670, 7671, 7672, 7673, 7674
- **Input Features:**
