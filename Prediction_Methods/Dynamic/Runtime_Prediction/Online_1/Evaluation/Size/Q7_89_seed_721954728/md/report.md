...
**Test Query:** Q7_89_seed_721954728
...
```
Node 12967 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12968 (Gather Merge) [consumed]
    Node 12969 (Sort)
      Node 12970 (Hash Join)
        Node 12971 (Nested Loop)
          Node 12972 (Hash Join) [PATTERN: 895c6e8c]
            Node 12973 (Seq Scan) [consumed] - LEAF
            Node 12974 (Hash) [consumed]
              Node 12975 (Hash Join) [PATTERN: f4cb205a]
                Node 12976 (Seq Scan) [consumed] - LEAF
                Node 12977 (Hash) [consumed]
                  Node 12978 (Seq Scan) [consumed] - LEAF
          Node 12979 (Index Scan) - LEAF
        Node 12980 (Hash)
          Node 12981 (Hash Join) [PATTERN: f4cb205a]
            Node 12982 (Seq Scan) [consumed] - LEAF
            Node 12983 (Hash) [consumed]
              Node 12984 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12975 | 12967, 12968, 12972, 12973, 12974, 12976, 12977, 12978, 12981, 12982, 12983, 12984 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12981 | 12967, 12968, 12972, 12973, 12974, 12975, 12976, 12977, 12978, 12982, 12983, 12984 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12967 | 12968, 12972, 12973, 12974, 12975, 12976, 12977, 12978, 12981, 12982, 12983, 12984 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12972 | 12967, 12968, 12973, 12974, 12975, 12976, 12977, 12978, 12981, 12982, 12983, 12984 |
...
|------|------|--------|-----------|-----|--------|
| 12967 | Aggregate | 1240.13 | 1163.13 | 6.2% | pattern |
| 12969 | Sort | 1232.91 | 1109.16 | 10.0% | operator |
| 12970 | Hash Join | 1232.18 | 843.11 | 31.6% | operator |
| 12971 | Nested Loop | 1225.33 | 1215.99 | 0.8% | operator |
| 12980 | Hash | 2.90 | 27.74 | 857.3% | operator |
| 12972 | Hash Join | 216.65 | 304.89 | 40.7% | pattern |
| 12979 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12981 | Hash Join | 2.85 | 429.36 | 14954.8% | pattern |
| 12975 | Hash Join | 35.62 | 429.38 | 1105.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12975 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12967, 12968, 12972, 12973, 12974, 12976, 12977, 12978, 12981, 12982, 12983, 12984
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12972 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12967, 12968, 12973, 12974, 12975, 12976, 12977, 12978, 12981, 12982, 12983, 12984
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12979 (Index Scan) - LEAF
...
### Step 4: Node 12981 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12967, 12968, 12972, 12973, 12974, 12975, 12976, 12977, 12978, 12982, 12983, 12984
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12967 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12968, 12972, 12973, 12974, 12975, 12976, 12977, 12978, 12981, 12982, 12983, 12984
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
