...
**Test Query:** Q7_138_seed_1123952247
...
```
Node 11239 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11240 (Gather Merge) [consumed]
    Node 11241 (Sort)
      Node 11242 (Hash Join)
        Node 11243 (Nested Loop)
          Node 11244 (Hash Join) [PATTERN: 895c6e8c]
            Node 11245 (Seq Scan) [consumed] - LEAF
            Node 11246 (Hash) [consumed]
              Node 11247 (Hash Join) [PATTERN: f4cb205a]
                Node 11248 (Seq Scan) [consumed] - LEAF
                Node 11249 (Hash) [consumed]
                  Node 11250 (Seq Scan) [consumed] - LEAF
          Node 11251 (Index Scan) - LEAF
        Node 11252 (Hash)
          Node 11253 (Hash Join) [PATTERN: f4cb205a]
            Node 11254 (Seq Scan) [consumed] - LEAF
            Node 11255 (Hash) [consumed]
              Node 11256 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11247 | 11239, 11240, 11244, 11245, 11246, 11248, 11249, 11250, 11253, 11254, 11255, 11256 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11253 | 11239, 11240, 11244, 11245, 11246, 11247, 11248, 11249, 11250, 11254, 11255, 11256 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11239 | 11240, 11244, 11245, 11246, 11247, 11248, 11249, 11250, 11253, 11254, 11255, 11256 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11244 | 11239, 11240, 11245, 11246, 11247, 11248, 11249, 11250, 11253, 11254, 11255, 11256 |
...
|------|------|--------|-----------|-----|--------|
| 11239 | Aggregate | 1226.43 | 1163.13 | 5.2% | pattern |
| 11241 | Sort | 1217.51 | 1109.16 | 8.9% | operator |
| 11242 | Hash Join | 1216.85 | 843.11 | 30.7% | operator |
| 11243 | Nested Loop | 1209.89 | 1215.99 | 0.5% | operator |
| 11252 | Hash | 3.20 | 27.74 | 766.7% | operator |
| 11244 | Hash Join | 213.45 | 304.89 | 42.8% | pattern |
| 11251 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 11253 | Hash Join | 3.16 | 429.36 | 13504.7% | pattern |
| 11247 | Hash Join | 34.86 | 429.38 | 1131.8% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11247 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11239, 11240, 11244, 11245, 11246, 11248, 11249, 11250, 11253, 11254, 11255, 11256
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11244 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11239, 11240, 11245, 11246, 11247, 11248, 11249, 11250, 11253, 11254, 11255, 11256
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11251 (Index Scan) - LEAF
...
### Step 4: Node 11253 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11239, 11240, 11244, 11245, 11246, 11247, 11248, 11249, 11250, 11254, 11255, 11256
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11239 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11240, 11244, 11245, 11246, 11247, 11248, 11249, 11250, 11253, 11254, 11255, 11256
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
