...
**Test Query:** Q8_28_seed_221508837
...
```
Node 14811 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14812 (Gather Merge) [consumed]
    Node 14813 (Sort)
      Node 14814 (Hash Join)
        Node 14815 (Nested Loop)
          Node 14816 (Hash Join)
            Node 14817 (Nested Loop)
              Node 14818 (Hash Join)
                Node 14819 (Seq Scan) - LEAF
                Node 14820 (Hash) [PATTERN: 444761fb]
                  Node 14821 (Hash Join) [consumed]
                    Node 14822 (Seq Scan) [consumed] - LEAF
                    Node 14823 (Hash) [consumed]
                      Node 14824 (Hash Join) [consumed]
                        Node 14825 (Seq Scan) - LEAF
                        Node 14826 (Hash)
                          Node 14827 (Seq Scan) - LEAF
              Node 14828 (Index Scan) - LEAF
            Node 14829 (Hash)
              Node 14830 (Seq Scan) - LEAF
          Node 14831 (Index Scan) - LEAF
        Node 14832 (Hash)
          Node 14833 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14820 | 14811, 14812, 14821, 14822, 14823, 14824 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14811 | 14812, 14820, 14821, 14822, 14823, 14824 |
...
|------|------|--------|-----------|-----|--------|
| 14811 | Aggregate | 1193.90 | 1132.51 | 5.1% | pattern |
| 14813 | Sort | 1188.67 | 1046.21 | 12.0% | operator |
| 14814 | Hash Join | 1188.42 | 785.61 | 33.9% | operator |
| 14815 | Nested Loop | 1188.12 | 1198.29 | 0.9% | operator |
| 14832 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 14816 | Hash Join | 1184.41 | 785.60 | 33.7% | operator |
| 14831 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14833 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 14817 | Nested Loop | 1141.46 | 1198.29 | 5.0% | operator |
| 14829 | Hash | 38.60 | 19.97 | 48.3% | operator |
| 14818 | Hash Join | 207.56 | 251.67 | 21.2% | operator |
| 14828 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14830 | Seq Scan | 38.50 | 48.87 | 27.0% | operator |
| 14819 | Seq Scan | 157.53 | 159.67 | 1.4% | operator |
| 14820 | Hash | 31.23 | 43.51 | 39.4% | pattern |
| 14825 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 14826 | Hash | 0.20 | 18.14 | 9110.2% | operator |
| 14827 | Seq Scan | 0.19 | 23.36 | 12004.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14827 (Seq Scan) - LEAF
...
### Step 2: Node 14825 (Seq Scan) - LEAF
...
### Step 4: Node 14819 (Seq Scan) - LEAF
...
### Step 5: Node 14820 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14811, 14812, 14821, 14822, 14823, 14824
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14828 (Index Scan) - LEAF
...
### Step 8: Node 14830 (Seq Scan) - LEAF
...
### Step 12: Node 14831 (Index Scan) - LEAF
...
### Step 13: Node 14833 (Seq Scan) - LEAF
...
### Step 14: Node 14815 (Nested Loop)
...
### Step 18: Node 14811 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14812, 14820, 14821, 14822, 14823, 14824
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2429
