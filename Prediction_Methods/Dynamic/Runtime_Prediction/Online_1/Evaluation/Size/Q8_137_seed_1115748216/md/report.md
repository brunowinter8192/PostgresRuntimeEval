...
**Test Query:** Q8_137_seed_1115748216
...
```
Node 14121 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14122 (Gather Merge) [consumed]
    Node 14123 (Sort)
      Node 14124 (Hash Join)
        Node 14125 (Nested Loop)
          Node 14126 (Hash Join)
            Node 14127 (Nested Loop)
              Node 14128 (Hash Join)
                Node 14129 (Seq Scan) - LEAF
                Node 14130 (Hash) [PATTERN: 444761fb]
                  Node 14131 (Hash Join) [consumed]
                    Node 14132 (Seq Scan) [consumed] - LEAF
                    Node 14133 (Hash) [consumed]
                      Node 14134 (Hash Join) [consumed]
                        Node 14135 (Seq Scan) - LEAF
                        Node 14136 (Hash)
                          Node 14137 (Seq Scan) - LEAF
              Node 14138 (Index Scan) - LEAF
            Node 14139 (Hash)
              Node 14140 (Seq Scan) - LEAF
          Node 14141 (Index Scan) - LEAF
        Node 14142 (Hash)
          Node 14143 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14130 | 14121, 14122, 14131, 14132, 14133, 14134 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14121 | 14122, 14130, 14131, 14132, 14133, 14134 |
...
|------|------|--------|-----------|-----|--------|
| 14121 | Aggregate | 1225.22 | 1133.67 | 7.5% | pattern |
| 14123 | Sort | 1219.51 | 1046.21 | 14.2% | operator |
| 14124 | Hash Join | 1219.26 | 785.61 | 35.6% | operator |
| 14125 | Nested Loop | 1218.92 | 1198.29 | 1.7% | operator |
| 14142 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 14126 | Hash Join | 1214.64 | 785.60 | 35.3% | operator |
| 14141 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14143 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 14127 | Nested Loop | 1166.83 | 1198.29 | 2.7% | operator |
| 14139 | Hash | 43.35 | 19.97 | 53.9% | operator |
| 14128 | Hash Join | 213.01 | 251.67 | 18.1% | operator |
| 14138 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14140 | Seq Scan | 43.29 | 48.88 | 12.9% | operator |
| 14129 | Seq Scan | 159.27 | 159.67 | 0.2% | operator |
| 14130 | Hash | 36.20 | 43.51 | 20.2% | pattern |
| 14135 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 14136 | Hash | 0.22 | 18.14 | 8185.0% | operator |
| 14137 | Seq Scan | 0.21 | 23.36 | 10766.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14137 (Seq Scan) - LEAF
...
### Step 2: Node 14135 (Seq Scan) - LEAF
...
### Step 4: Node 14129 (Seq Scan) - LEAF
...
### Step 5: Node 14130 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14121, 14122, 14131, 14132, 14133, 14134
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14138 (Index Scan) - LEAF
...
### Step 8: Node 14140 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9719
  - startup_cost=9919.8400
  - total_cost=70271.3200
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14141 (Index Scan) - LEAF
...
### Step 13: Node 14143 (Seq Scan) - LEAF
...
### Step 14: Node 14125 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.1200
  - total_cost=70499.1300
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14142 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.6900
  - total_cost=70504.8900
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14123 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70540.8700
  - total_cost=70542.7500
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14121 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14122, 14130, 14131, 14132, 14133, 14134
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2334
  - Aggregate_nt1=2334
  ...
  - Aggregate_total_cost=71910.4000
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2334
  - GatherMerge_Outer_nt1=753
  ...
  - GatherMerge_Outer_sel=3.0996
  - GatherMerge_Outer_startup_cost=71540.9100
  - GatherMerge_Outer_total_cost=71817.0400
- **Output:** st=1130.67, rt=1133.67
