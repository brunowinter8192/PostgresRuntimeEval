...
**Test Query:** Q8_99_seed_803995038
...
```
Node 16605 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16606 (Gather Merge) [consumed]
    Node 16607 (Sort)
      Node 16608 (Hash Join)
        Node 16609 (Nested Loop)
          Node 16610 (Hash Join)
            Node 16611 (Nested Loop)
              Node 16612 (Hash Join)
                Node 16613 (Seq Scan) - LEAF
                Node 16614 (Hash) [PATTERN: 444761fb]
                  Node 16615 (Hash Join) [consumed]
                    Node 16616 (Seq Scan) [consumed] - LEAF
                    Node 16617 (Hash) [consumed]
                      Node 16618 (Hash Join) [consumed]
                        Node 16619 (Seq Scan) - LEAF
                        Node 16620 (Hash)
                          Node 16621 (Seq Scan) - LEAF
              Node 16622 (Index Scan) - LEAF
            Node 16623 (Hash)
              Node 16624 (Seq Scan) - LEAF
          Node 16625 (Index Scan) - LEAF
        Node 16626 (Hash)
          Node 16627 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16614 | 16605, 16606, 16615, 16616, 16617, 16618 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16605 | 16606, 16614, 16615, 16616, 16617, 16618 |
...
|------|------|--------|-----------|-----|--------|
| 16605 | Aggregate | 1229.34 | 1132.14 | 7.9% | pattern |
| 16607 | Sort | 1224.31 | 1046.21 | 14.5% | operator |
| 16608 | Hash Join | 1224.06 | 785.61 | 35.8% | operator |
| 16609 | Nested Loop | 1223.72 | 1198.29 | 2.1% | operator |
| 16626 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 16610 | Hash Join | 1219.77 | 785.60 | 35.6% | operator |
| 16625 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16627 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 16611 | Nested Loop | 1175.06 | 1198.29 | 2.0% | operator |
| 16623 | Hash | 40.26 | 19.97 | 50.4% | operator |
| 16612 | Hash Join | 217.90 | 251.67 | 15.5% | operator |
| 16622 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 16624 | Seq Scan | 40.20 | 48.87 | 21.6% | operator |
| 16613 | Seq Scan | 165.48 | 159.67 | 3.5% | operator |
| 16614 | Hash | 34.63 | 43.51 | 25.6% | pattern |
| 16619 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 16620 | Hash | 0.22 | 18.14 | 8261.4% | operator |
| 16621 | Seq Scan | 0.21 | 23.36 | 10868.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16621 (Seq Scan) - LEAF
...
### Step 2: Node 16619 (Seq Scan) - LEAF
...
### Step 4: Node 16613 (Seq Scan) - LEAF
...
### Step 5: Node 16614 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16605, 16606, 16615, 16616, 16617, 16618
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16622 (Index Scan) - LEAF
...
### Step 8: Node 16624 (Seq Scan) - LEAF
...
### Step 12: Node 16625 (Index Scan) - LEAF
...
### Step 13: Node 16627 (Seq Scan) - LEAF
...
### Step 14: Node 16609 (Nested Loop)
...
### Step 18: Node 16605 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16606, 16614, 16615, 16616, 16617, 16618
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2436
  ...
  - Aggregate_sel=0.9877
  - Aggregate_startup_cost=71553.1800
  - Aggregate_total_cost=71938.2200
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2436
  ...
  - GatherMerge_Outer_total_cost=71841.3800
- **Output:** st=1129.16, rt=1132.14
