...
**Test Query:** Q7_102_seed_828607131
...
```
Node 10537 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10538 (Gather Merge) [consumed]
    Node 10539 (Sort)
      Node 10540 (Hash Join)
        Node 10541 (Nested Loop)
          Node 10542 (Hash Join) [PATTERN: 895c6e8c]
            Node 10543 (Seq Scan) [consumed] - LEAF
            Node 10544 (Hash) [consumed]
              Node 10545 (Hash Join) [PATTERN: f4cb205a]
                Node 10546 (Seq Scan) [consumed] - LEAF
                Node 10547 (Hash) [consumed]
                  Node 10548 (Seq Scan) [consumed] - LEAF
          Node 10549 (Index Scan) - LEAF
        Node 10550 (Hash)
          Node 10551 (Hash Join) [PATTERN: f4cb205a]
            Node 10552 (Seq Scan) [consumed] - LEAF
            Node 10553 (Hash) [consumed]
              Node 10554 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10545 | 10537, 10538, 10542, 10543, 10544, 10546, 10547, 10548, 10551, 10552, 10553, 10554 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10551 | 10537, 10538, 10542, 10543, 10544, 10545, 10546, 10547, 10548, 10552, 10553, 10554 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10537 | 10538, 10542, 10543, 10544, 10545, 10546, 10547, 10548, 10551, 10552, 10553, 10554 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10542 | 10537, 10538, 10543, 10544, 10545, 10546, 10547, 10548, 10551, 10552, 10553, 10554 |
...
|------|------|--------|-----------|-----|--------|
| 10537 | Aggregate | 1173.41 | 1163.13 | 0.9% | pattern |
| 10539 | Sort | 1166.82 | 1109.16 | 4.9% | operator |
| 10540 | Hash Join | 1166.23 | 843.11 | 27.7% | operator |
| 10541 | Nested Loop | 1159.05 | 1215.99 | 4.9% | operator |
| 10550 | Hash | 3.34 | 27.74 | 731.6% | operator |
| 10542 | Hash Join | 210.63 | 304.89 | 44.7% | pattern |
| 10549 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 10551 | Hash Join | 3.29 | 429.36 | 12942.6% | pattern |
| 10545 | Hash Join | 34.45 | 429.38 | 1146.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10545 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10537, 10538, 10542, 10543, 10544, 10546, 10547, 10548, 10551, 10552, 10553, 10554
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10542 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10537, 10538, 10543, 10544, 10545, 10546, 10547, 10548, 10551, 10552, 10553, 10554
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10549 (Index Scan) - LEAF
...
### Step 4: Node 10551 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10537, 10538, 10542, 10543, 10544, 10545, 10546, 10547, 10548, 10552, 10553, 10554
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10537 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10538, 10542, 10543, 10544, 10545, 10546, 10547, 10548, 10551, 10552, 10553, 10554
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
