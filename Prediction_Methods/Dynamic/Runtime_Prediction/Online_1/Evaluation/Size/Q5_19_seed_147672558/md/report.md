...
**Test Query:** Q5_19_seed_147672558
...
```
Node 8101 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8102 (Aggregate) [consumed]
    Node 8103 (Gather Merge) [consumed]
      Node 8104 (Aggregate)
        Node 8105 (Sort) [PATTERN: 3e2d5a00]
          Node 8106 (Hash Join) [consumed]
            Node 8107 (Nested Loop)
              Node 8108 (Hash Join)
                Node 8109 (Seq Scan) - LEAF
                Node 8110 (Hash) [PATTERN: 444761fb]
                  Node 8111 (Hash Join) [consumed]
                    Node 8112 (Seq Scan) [consumed] - LEAF
                    Node 8113 (Hash) [consumed]
                      Node 8114 (Hash Join) [consumed]
                        Node 8115 (Seq Scan) - LEAF
                        Node 8116 (Hash)
                          Node 8117 (Seq Scan) - LEAF
              Node 8118 (Index Scan) - LEAF
            Node 8119 (Hash)
              Node 8120 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8110 | 8101, 8102, 8103, 8105, 8106, 8111, 8112, 8113, 8114 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8101 | 8102, 8103, 8105, 8106, 8110, 8111, 8112, 8113, 8114 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8105 | 8101, 8102, 8103, 8106, 8110, 8111, 8112, 8113, 8114 |
...
|------|------|--------|-----------|-----|--------|
| 8101 | Sort | 1052.36 | 2934.42 | 178.8% | pattern |
| 8104 | Aggregate | 1047.03 | 1128.76 | 7.8% | operator |
| 8105 | Sort | 1046.78 | 1835.93 | 75.4% | pattern |
| 8107 | Nested Loop | 1034.28 | 1188.47 | 14.9% | operator |
| 8119 | Hash | 3.06 | 18.67 | 511.2% | operator |
| 8108 | Hash Join | 217.17 | 253.07 | 16.5% | operator |
| 8118 | Index Scan | 0.07 | -0.22 | 405.6% | operator |
| 8120 | Seq Scan | 2.54 | 12.36 | 386.8% | operator |
| 8109 | Seq Scan | 168.56 | 159.81 | 5.2% | operator |
| 8110 | Hash | 35.53 | 46.85 | 31.8% | pattern |
| 8115 | Seq Scan | 0.01 | 11.81 | 147503.4% | operator |
| 8116 | Hash | 0.04 | 21.05 | 48861.0% | operator |
| 8117 | Seq Scan | 0.04 | 22.93 | 57233.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8117 (Seq Scan) - LEAF
...
### Step 2: Node 8115 (Seq Scan) - LEAF
...
### Step 4: Node 8109 (Seq Scan) - LEAF
...
### Step 5: Node 8110 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8101, 8102, 8103, 8105, 8106, 8111, 8112, 8113, 8114
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8118 (Index Scan) - LEAF
...
### Step 8: Node 8120 (Seq Scan) - LEAF
...
### Step 11: Node 8105 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8101, 8102, 8103, 8106, 8110, 8111, 8112, 8113, 8114
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8101 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8102, 8103, 8105, 8106, 8110, 8111, 8112, 8113, 8114
- **Input Features:**
