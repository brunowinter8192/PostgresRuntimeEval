...
**Test Query:** Q7_20_seed_155876589
...
```
Node 11617 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11618 (Gather Merge) [consumed]
    Node 11619 (Sort)
      Node 11620 (Hash Join)
        Node 11621 (Nested Loop)
          Node 11622 (Hash Join) [PATTERN: 895c6e8c]
            Node 11623 (Seq Scan) [consumed] - LEAF
            Node 11624 (Hash) [consumed]
              Node 11625 (Hash Join) [PATTERN: f4cb205a]
                Node 11626 (Seq Scan) [consumed] - LEAF
                Node 11627 (Hash) [consumed]
                  Node 11628 (Seq Scan) [consumed] - LEAF
          Node 11629 (Index Scan) - LEAF
        Node 11630 (Hash)
          Node 11631 (Hash Join) [PATTERN: f4cb205a]
            Node 11632 (Seq Scan) [consumed] - LEAF
            Node 11633 (Hash) [consumed]
              Node 11634 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11625 | 11617, 11618, 11622, 11623, 11624, 11626, 11627, 11628, 11631, 11632, 11633, 11634 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11631 | 11617, 11618, 11622, 11623, 11624, 11625, 11626, 11627, 11628, 11632, 11633, 11634 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11617 | 11618, 11622, 11623, 11624, 11625, 11626, 11627, 11628, 11631, 11632, 11633, 11634 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11622 | 11617, 11618, 11623, 11624, 11625, 11626, 11627, 11628, 11631, 11632, 11633, 11634 |
...
|------|------|--------|-----------|-----|--------|
| 11617 | Aggregate | 1216.34 | 1163.13 | 4.4% | pattern |
| 11619 | Sort | 1208.61 | 1109.16 | 8.2% | operator |
| 11620 | Hash Join | 1207.95 | 843.11 | 30.2% | operator |
| 11621 | Nested Loop | 1200.81 | 1215.99 | 1.3% | operator |
| 11630 | Hash | 3.29 | 27.74 | 742.5% | operator |
| 11622 | Hash Join | 213.71 | 304.89 | 42.7% | pattern |
| 11629 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 11631 | Hash Join | 3.24 | 429.36 | 13143.8% | pattern |
| 11625 | Hash Join | 33.68 | 429.38 | 1175.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11625 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11617, 11618, 11622, 11623, 11624, 11626, 11627, 11628, 11631, 11632, 11633, 11634
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11622 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11617, 11618, 11623, 11624, 11625, 11626, 11627, 11628, 11631, 11632, 11633, 11634
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11629 (Index Scan) - LEAF
...
### Step 4: Node 11631 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11617, 11618, 11622, 11623, 11624, 11625, 11626, 11627, 11628, 11632, 11633, 11634
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11617 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11618, 11622, 11623, 11624, 11625, 11626, 11627, 11628, 11631, 11632, 11633, 11634
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
