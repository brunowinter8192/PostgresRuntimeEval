...
```
Node 11797 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11798 (Gather Merge) [consumed]
    Node 11799 (Sort)
      Node 11800 (Hash Join)
        Node 11801 (Nested Loop)
          Node 11802 (Hash Join) [PATTERN: 895c6e8c]
            Node 11803 (Seq Scan) [consumed] - LEAF
            Node 11804 (Hash) [consumed]
              Node 11805 (Hash Join) [PATTERN: f4cb205a]
                Node 11806 (Seq Scan) [consumed] - LEAF
                Node 11807 (Hash) [consumed]
                  Node 11808 (Seq Scan) [consumed] - LEAF
          Node 11809 (Index Scan) - LEAF
        Node 11810 (Hash)
          Node 11811 (Hash Join) [PATTERN: f4cb205a]
            Node 11812 (Seq Scan) [consumed] - LEAF
            Node 11813 (Hash) [consumed]
              Node 11814 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11805 | 11797, 11798, 11802, 11803, 11804, 11806, 11807, 11808, 11811, 11812, 11813, 11814 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11811 | 11797, 11798, 11802, 11803, 11804, 11805, 11806, 11807, 11808, 11812, 11813, 11814 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11797 | 11798, 11802, 11803, 11804, 11805, 11806, 11807, 11808, 11811, 11812, 11813, 11814 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11802 | 11797, 11798, 11803, 11804, 11805, 11806, 11807, 11808, 11811, 11812, 11813, 11814 |
...
|------|------|--------|-----------|-----|--------|
| 11797 | Aggregate | 1198.32 | 1163.13 | 2.9% | pattern |
| 11799 | Sort | 1191.05 | 1109.16 | 6.9% | operator |
| 11800 | Hash Join | 1190.41 | 843.11 | 29.2% | operator |
| 11801 | Nested Loop | 1181.24 | 1215.99 | 2.9% | operator |
| 11810 | Hash | 5.36 | 27.74 | 417.6% | operator |
| 11802 | Hash Join | 214.43 | 304.89 | 42.2% | pattern |
| 11809 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11811 | Hash Join | 5.31 | 429.36 | 7982.9% | pattern |
| 11805 | Hash Join | 36.11 | 429.38 | 1089.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11805 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11797, 11798, 11802, 11803, 11804, 11806, 11807, 11808, 11811, 11812, 11813, 11814
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11802 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11797, 11798, 11803, 11804, 11805, 11806, 11807, 11808, 11811, 11812, 11813, 11814
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11809 (Index Scan) - LEAF
...
### Step 4: Node 11811 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11797, 11798, 11802, 11803, 11804, 11805, 11806, 11807, 11808, 11812, 11813, 11814
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11797 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11798, 11802, 11803, 11804, 11805, 11806, 11807, 11808, 11811, 11812, 11813, 11814
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
