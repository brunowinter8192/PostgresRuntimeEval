...
**Test Query:** Q10_32_seed_254324961
...
```
Node 20345 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20346 (Sort) [consumed]
    Node 20347 (Aggregate) [consumed]
      Node 20348 (Gather) [consumed]
        Node 20349 (Hash Join)
          Node 20350 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20351 (Nested Loop) [consumed]
              Node 20352 (Seq Scan) - LEAF
              Node 20353 (Index Scan) - LEAF
            Node 20354 (Hash) [consumed]
              Node 20355 (Seq Scan) - LEAF
          Node 20356 (Hash)
            Node 20357 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20345 | 20346, 20347, 20348, 20350, 20351, 20354 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20350 | 20345, 20346, 20347, 20348, 20351, 20354 |
...
|------|------|--------|-----------|-----|--------|
| 20345 | Limit | 1133.89 | 1199.47 | 5.8% | pattern |
| 20349 | Hash Join | 1080.68 | 1095.58 | 1.4% | operator |
| 20350 | Hash Join | 1077.81 | 1120.19 | 3.9% | pattern |
| 20356 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20357 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 20352 | Seq Scan | 162.80 | 159.74 | 1.9% | operator |
| 20353 | Index Scan | 0.06 | -0.52 | 963.7% | operator |
| 20355 | Seq Scan | 24.10 | 26.64 | 10.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20352 (Seq Scan) - LEAF
...
### Step 2: Node 20353 (Index Scan) - LEAF
...
### Step 3: Node 20355 (Seq Scan) - LEAF
...
### Step 4: Node 20357 (Seq Scan) - LEAF
...
### Step 5: Node 20350 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20345, 20346, 20347, 20348, 20351, 20354
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18276
  - HashJoin_nt1=18276
...
### Step 8: Node 20345 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20346, 20347, 20348, 20350, 20351, 20354
- **Input Features:**
