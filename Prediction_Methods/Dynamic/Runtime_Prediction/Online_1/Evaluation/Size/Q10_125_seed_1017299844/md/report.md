...
**Test Query:** Q10_125_seed_1017299844
...
```
Node 19721 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19722 (Sort) [consumed]
    Node 19723 (Aggregate) [consumed]
      Node 19724 (Gather) [consumed]
        Node 19725 (Hash Join)
          Node 19726 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19727 (Nested Loop) [consumed]
              Node 19728 (Seq Scan) - LEAF
              Node 19729 (Index Scan) - LEAF
            Node 19730 (Hash) [consumed]
              Node 19731 (Seq Scan) - LEAF
          Node 19732 (Hash)
            Node 19733 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19721 | 19722, 19723, 19724, 19726, 19727, 19730 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19726 | 19721, 19722, 19723, 19724, 19727, 19730 |
...
|------|------|--------|-----------|-----|--------|
| 19721 | Limit | 1112.27 | 1199.47 | 7.8% | pattern |
| 19725 | Hash Join | 1064.93 | 1095.57 | 2.9% | operator |
| 19726 | Hash Join | 1062.05 | 1120.19 | 5.5% | pattern |
| 19732 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 19733 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 19728 | Seq Scan | 161.79 | 159.74 | 1.3% | operator |
| 19729 | Index Scan | 0.06 | -0.52 | 963.7% | operator |
| 19731 | Seq Scan | 29.51 | 26.64 | 9.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19728 (Seq Scan) - LEAF
...
### Step 2: Node 19729 (Index Scan) - LEAF
...
### Step 3: Node 19731 (Seq Scan) - LEAF
...
### Step 4: Node 19733 (Seq Scan) - LEAF
...
### Step 5: Node 19726 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19721, 19722, 19723, 19724, 19727, 19730
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18240
  - HashJoin_nt1=18240
...
### Step 8: Node 19721 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19722, 19723, 19724, 19726, 19727, 19730
- **Input Features:**
