...
**Test Query:** Q7_103_seed_836811162
...
```
Node 10555 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10556 (Gather Merge) [consumed]
    Node 10557 (Sort)
      Node 10558 (Hash Join)
        Node 10559 (Nested Loop)
          Node 10560 (Hash Join) [PATTERN: 895c6e8c]
            Node 10561 (Seq Scan) [consumed] - LEAF
            Node 10562 (Hash) [consumed]
              Node 10563 (Hash Join) [PATTERN: f4cb205a]
                Node 10564 (Seq Scan) [consumed] - LEAF
                Node 10565 (Hash) [consumed]
                  Node 10566 (Seq Scan) [consumed] - LEAF
          Node 10567 (Index Scan) - LEAF
        Node 10568 (Hash)
          Node 10569 (Hash Join) [PATTERN: f4cb205a]
            Node 10570 (Seq Scan) [consumed] - LEAF
            Node 10571 (Hash) [consumed]
              Node 10572 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10563 | 10555, 10556, 10560, 10561, 10562, 10564, 10565, 10566, 10569, 10570, 10571, 10572 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10569 | 10555, 10556, 10560, 10561, 10562, 10563, 10564, 10565, 10566, 10570, 10571, 10572 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10555 | 10556, 10560, 10561, 10562, 10563, 10564, 10565, 10566, 10569, 10570, 10571, 10572 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10560 | 10555, 10556, 10561, 10562, 10563, 10564, 10565, 10566, 10569, 10570, 10571, 10572 |
...
|------|------|--------|-----------|-----|--------|
| 10555 | Aggregate | 1193.56 | 1163.13 | 2.5% | pattern |
| 10557 | Sort | 1185.89 | 1109.16 | 6.5% | operator |
| 10558 | Hash Join | 1185.25 | 843.11 | 28.9% | operator |
| 10559 | Nested Loop | 1175.89 | 1215.99 | 3.4% | operator |
| 10568 | Hash | 5.62 | 27.74 | 393.5% | operator |
| 10560 | Hash Join | 213.63 | 304.89 | 42.7% | pattern |
| 10567 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10569 | Hash Join | 5.58 | 429.36 | 7597.4% | pattern |
| 10563 | Hash Join | 33.16 | 429.38 | 1195.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10563 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10555, 10556, 10560, 10561, 10562, 10564, 10565, 10566, 10569, 10570, 10571, 10572
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10560 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10555, 10556, 10561, 10562, 10563, 10564, 10565, 10566, 10569, 10570, 10571, 10572
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10567 (Index Scan) - LEAF
...
### Step 4: Node 10569 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10555, 10556, 10560, 10561, 10562, 10563, 10564, 10565, 10566, 10570, 10571, 10572
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10555 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10556, 10560, 10561, 10562, 10563, 10564, 10565, 10566, 10569, 10570, 10571, 10572
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
