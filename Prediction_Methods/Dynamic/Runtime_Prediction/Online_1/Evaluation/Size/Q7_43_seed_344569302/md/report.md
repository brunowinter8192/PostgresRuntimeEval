...
**Test Query:** Q7_43_seed_344569302
...
```
Node 12067 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12068 (Gather Merge) [consumed]
    Node 12069 (Sort)
      Node 12070 (Hash Join)
        Node 12071 (Nested Loop)
          Node 12072 (Hash Join) [PATTERN: 895c6e8c]
            Node 12073 (Seq Scan) [consumed] - LEAF
            Node 12074 (Hash) [consumed]
              Node 12075 (Hash Join) [PATTERN: f4cb205a]
                Node 12076 (Seq Scan) [consumed] - LEAF
                Node 12077 (Hash) [consumed]
                  Node 12078 (Seq Scan) [consumed] - LEAF
          Node 12079 (Index Scan) - LEAF
        Node 12080 (Hash)
          Node 12081 (Hash Join) [PATTERN: f4cb205a]
            Node 12082 (Seq Scan) [consumed] - LEAF
            Node 12083 (Hash) [consumed]
              Node 12084 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12075 | 12067, 12068, 12072, 12073, 12074, 12076, 12077, 12078, 12081, 12082, 12083, 12084 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12081 | 12067, 12068, 12072, 12073, 12074, 12075, 12076, 12077, 12078, 12082, 12083, 12084 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12067 | 12068, 12072, 12073, 12074, 12075, 12076, 12077, 12078, 12081, 12082, 12083, 12084 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12072 | 12067, 12068, 12073, 12074, 12075, 12076, 12077, 12078, 12081, 12082, 12083, 12084 |
...
|------|------|--------|-----------|-----|--------|
| 12067 | Aggregate | 1182.81 | 1163.13 | 1.7% | pattern |
| 12069 | Sort | 1175.64 | 1109.16 | 5.7% | operator |
| 12070 | Hash Join | 1175.02 | 843.11 | 28.2% | operator |
| 12071 | Nested Loop | 1163.94 | 1215.99 | 4.5% | operator |
| 12080 | Hash | 7.29 | 27.74 | 280.6% | operator |
| 12072 | Hash Join | 212.51 | 304.89 | 43.5% | pattern |
| 12079 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 12081 | Hash Join | 7.25 | 429.36 | 5823.9% | pattern |
| 12075 | Hash Join | 32.62 | 429.38 | 1216.4% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12075 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12067, 12068, 12072, 12073, 12074, 12076, 12077, 12078, 12081, 12082, 12083, 12084
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12072 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12067, 12068, 12073, 12074, 12075, 12076, 12077, 12078, 12081, 12082, 12083, 12084
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12079 (Index Scan) - LEAF
...
### Step 4: Node 12081 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12067, 12068, 12072, 12073, 12074, 12075, 12076, 12077, 12078, 12082, 12083, 12084
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12067 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12068, 12072, 12073, 12074, 12075, 12076, 12077, 12078, 12081, 12082, 12083, 12084
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
