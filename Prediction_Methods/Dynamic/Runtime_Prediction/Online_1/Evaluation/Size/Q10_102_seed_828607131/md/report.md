...
**Test Query:** Q10_102_seed_828607131
...
```
Node 19396 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19397 (Sort) [consumed]
    Node 19398 (Aggregate) [consumed]
      Node 19399 (Gather) [consumed]
        Node 19400 (Hash Join)
          Node 19401 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19402 (Nested Loop) [consumed]
              Node 19403 (Seq Scan) - LEAF
              Node 19404 (Index Scan) - LEAF
            Node 19405 (Hash) [consumed]
              Node 19406 (Seq Scan) - LEAF
          Node 19407 (Hash)
            Node 19408 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19396 | 19397, 19398, 19399, 19401, 19402, 19405 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19401 | 19396, 19397, 19398, 19399, 19402, 19405 |
...
|------|------|--------|-----------|-----|--------|
| 19396 | Limit | 1126.50 | 1199.47 | 6.5% | pattern |
| 19400 | Hash Join | 1079.60 | 1095.49 | 1.5% | operator |
| 19401 | Hash Join | 1076.56 | 1120.19 | 4.1% | pattern |
| 19407 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 19408 | Seq Scan | 0.01 | 6.98 | 53582.3% | operator |
| 19403 | Seq Scan | 163.06 | 159.74 | 2.0% | operator |
| 19404 | Index Scan | 0.06 | -0.52 | 949.6% | operator |
| 19406 | Seq Scan | 28.23 | 26.64 | 5.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19403 (Seq Scan) - LEAF
...
### Step 2: Node 19404 (Index Scan) - LEAF
...
### Step 3: Node 19406 (Seq Scan) - LEAF
...
### Step 4: Node 19408 (Seq Scan) - LEAF
...
### Step 5: Node 19401 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19396, 19397, 19398, 19399, 19402, 19405
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17964
  - HashJoin_nt1=17964
...
### Step 8: Node 19396 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19397, 19398, 19399, 19401, 19402, 19405
- **Input Features:**
