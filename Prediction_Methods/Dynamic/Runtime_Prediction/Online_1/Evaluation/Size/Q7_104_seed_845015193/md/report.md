...
**Test Query:** Q7_104_seed_845015193
...
```
Node 10573 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10574 (Gather Merge) [consumed]
    Node 10575 (Sort)
      Node 10576 (Hash Join)
        Node 10577 (Nested Loop)
          Node 10578 (Hash Join) [PATTERN: 895c6e8c]
            Node 10579 (Seq Scan) [consumed] - LEAF
            Node 10580 (Hash) [consumed]
              Node 10581 (Hash Join) [PATTERN: f4cb205a]
                Node 10582 (Seq Scan) [consumed] - LEAF
                Node 10583 (Hash) [consumed]
                  Node 10584 (Seq Scan) [consumed] - LEAF
          Node 10585 (Index Scan) - LEAF
        Node 10586 (Hash)
          Node 10587 (Hash Join) [PATTERN: f4cb205a]
            Node 10588 (Seq Scan) [consumed] - LEAF
            Node 10589 (Hash) [consumed]
              Node 10590 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10581 | 10573, 10574, 10578, 10579, 10580, 10582, 10583, 10584, 10587, 10588, 10589, 10590 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10587 | 10573, 10574, 10578, 10579, 10580, 10581, 10582, 10583, 10584, 10588, 10589, 10590 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10573 | 10574, 10578, 10579, 10580, 10581, 10582, 10583, 10584, 10587, 10588, 10589, 10590 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10578 | 10573, 10574, 10579, 10580, 10581, 10582, 10583, 10584, 10587, 10588, 10589, 10590 |
...
|------|------|--------|-----------|-----|--------|
| 10573 | Aggregate | 1188.49 | 1163.13 | 2.1% | pattern |
| 10575 | Sort | 1181.93 | 1109.16 | 6.2% | operator |
| 10576 | Hash Join | 1181.30 | 843.11 | 28.6% | operator |
| 10577 | Nested Loop | 1173.87 | 1215.99 | 3.6% | operator |
| 10586 | Hash | 3.73 | 27.74 | 644.4% | operator |
| 10578 | Hash Join | 209.88 | 304.89 | 45.3% | pattern |
| 10585 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10587 | Hash Join | 3.68 | 429.36 | 11580.2% | pattern |
| 10581 | Hash Join | 32.85 | 429.38 | 1207.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10581 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10573, 10574, 10578, 10579, 10580, 10582, 10583, 10584, 10587, 10588, 10589, 10590
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10578 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10573, 10574, 10579, 10580, 10581, 10582, 10583, 10584, 10587, 10588, 10589, 10590
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10585 (Index Scan) - LEAF
...
### Step 4: Node 10587 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10573, 10574, 10578, 10579, 10580, 10581, 10582, 10583, 10584, 10588, 10589, 10590
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10573 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10574, 10578, 10579, 10580, 10581, 10582, 10583, 10584, 10587, 10588, 10589, 10590
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
