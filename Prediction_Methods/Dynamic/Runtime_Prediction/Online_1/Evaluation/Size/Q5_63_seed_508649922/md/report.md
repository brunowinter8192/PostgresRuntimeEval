...
**Test Query:** Q5_63_seed_508649922
**Timestamp:** 2026-01-18 17:18:51
...
```
Node 9081 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9082 (Aggregate) [consumed]
    Node 9083 (Gather Merge) [consumed]
      Node 9084 (Aggregate)
        Node 9085 (Sort) [PATTERN: 3e2d5a00]
          Node 9086 (Hash Join) [consumed]
            Node 9087 (Nested Loop)
              Node 9088 (Hash Join)
                Node 9089 (Seq Scan) - LEAF
                Node 9090 (Hash) [PATTERN: 444761fb]
                  Node 9091 (Hash Join) [consumed]
                    Node 9092 (Seq Scan) [consumed] - LEAF
                    Node 9093 (Hash) [consumed]
                      Node 9094 (Hash Join) [consumed]
                        Node 9095 (Seq Scan) - LEAF
                        Node 9096 (Hash)
                          Node 9097 (Seq Scan) - LEAF
              Node 9098 (Index Scan) - LEAF
            Node 9099 (Hash)
              Node 9100 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9090 | 9081, 9082, 9083, 9085, 9086, 9091, 9092, 9093, 9094 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9081 | 9082, 9083, 9085, 9086, 9090, 9091, 9092, 9093, 9094 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9085 | 9081, 9082, 9083, 9086, 9090, 9091, 9092, 9093, 9094 |
...
|------|------|--------|-----------|-----|--------|
| 9081 | Sort | 1079.44 | 2934.42 | 171.8% | pattern |
| 9084 | Aggregate | 1074.28 | 1128.78 | 5.1% | operator |
| 9085 | Sort | 1074.00 | 1835.91 | 70.9% | pattern |
| 9087 | Nested Loop | 1061.49 | 1188.39 | 12.0% | operator |
| 9099 | Hash | 3.76 | 18.67 | 396.2% | operator |
| 9088 | Hash Join | 221.72 | 253.07 | 14.1% | operator |
| 9098 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 9100 | Seq Scan | 3.11 | 12.36 | 297.9% | operator |
| 9089 | Seq Scan | 171.35 | 159.81 | 6.7% | operator |
| 9090 | Hash | 36.43 | 46.85 | 28.6% | pattern |
| 9095 | Seq Scan | 0.01 | 11.81 | 90732.9% | operator |
| 9096 | Hash | 0.15 | 21.05 | 13660.3% | operator |
| 9097 | Seq Scan | 0.15 | 22.93 | 15188.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9097 (Seq Scan) - LEAF
...
### Step 2: Node 9095 (Seq Scan) - LEAF
...
### Step 4: Node 9089 (Seq Scan) - LEAF
...
### Step 5: Node 9090 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9081, 9082, 9083, 9085, 9086, 9091, 9092, 9093, 9094
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9098 (Index Scan) - LEAF
...
### Step 8: Node 9100 (Seq Scan) - LEAF
...
### Step 11: Node 9085 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9081, 9082, 9083, 9086, 9090, 9091, 9092, 9093, 9094
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9081 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9082, 9083, 9085, 9086, 9090, 9091, 9092, 9093, 9094
- **Input Features:**
