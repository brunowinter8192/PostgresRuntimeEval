...
**Test Query:** Q7_149_seed_1214196588
...
```
Node 11455 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11456 (Gather Merge) [consumed]
    Node 11457 (Sort)
      Node 11458 (Hash Join)
        Node 11459 (Nested Loop)
          Node 11460 (Hash Join) [PATTERN: 895c6e8c]
            Node 11461 (Seq Scan) [consumed] - LEAF
            Node 11462 (Hash) [consumed]
              Node 11463 (Hash Join) [PATTERN: f4cb205a]
                Node 11464 (Seq Scan) [consumed] - LEAF
                Node 11465 (Hash) [consumed]
                  Node 11466 (Seq Scan) [consumed] - LEAF
          Node 11467 (Index Scan) - LEAF
        Node 11468 (Hash)
          Node 11469 (Hash Join) [PATTERN: f4cb205a]
            Node 11470 (Seq Scan) [consumed] - LEAF
            Node 11471 (Hash) [consumed]
              Node 11472 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11463 | 11455, 11456, 11460, 11461, 11462, 11464, 11465, 11466, 11469, 11470, 11471, 11472 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11469 | 11455, 11456, 11460, 11461, 11462, 11463, 11464, 11465, 11466, 11470, 11471, 11472 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11455 | 11456, 11460, 11461, 11462, 11463, 11464, 11465, 11466, 11469, 11470, 11471, 11472 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11460 | 11455, 11456, 11461, 11462, 11463, 11464, 11465, 11466, 11469, 11470, 11471, 11472 |
...
|------|------|--------|-----------|-----|--------|
| 11455 | Aggregate | 1211.85 | 1163.13 | 4.0% | pattern |
| 11457 | Sort | 1205.76 | 1109.16 | 8.0% | operator |
| 11458 | Hash Join | 1205.14 | 843.11 | 30.0% | operator |
| 11459 | Nested Loop | 1197.59 | 1215.99 | 1.5% | operator |
| 11468 | Hash | 3.86 | 27.74 | 619.1% | operator |
| 11460 | Hash Join | 215.10 | 304.89 | 41.7% | pattern |
| 11467 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 11469 | Hash Join | 3.81 | 429.36 | 11166.4% | pattern |
| 11463 | Hash Join | 35.54 | 429.38 | 1108.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11463 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11455, 11456, 11460, 11461, 11462, 11464, 11465, 11466, 11469, 11470, 11471, 11472
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11460 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11455, 11456, 11461, 11462, 11463, 11464, 11465, 11466, 11469, 11470, 11471, 11472
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11467 (Index Scan) - LEAF
...
### Step 4: Node 11469 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11455, 11456, 11460, 11461, 11462, 11463, 11464, 11465, 11466, 11470, 11471, 11472
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11455 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11456, 11460, 11461, 11462, 11463, 11464, 11465, 11466, 11469, 11470, 11471, 11472
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
