...
**Test Query:** Q8_74_seed_598894263
...
```
Node 15984 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15985 (Gather Merge) [consumed]
    Node 15986 (Sort)
      Node 15987 (Hash Join)
        Node 15988 (Nested Loop)
          Node 15989 (Hash Join)
            Node 15990 (Nested Loop)
              Node 15991 (Hash Join)
                Node 15992 (Seq Scan) - LEAF
                Node 15993 (Hash) [PATTERN: 444761fb]
                  Node 15994 (Hash Join) [consumed]
                    Node 15995 (Seq Scan) [consumed] - LEAF
                    Node 15996 (Hash) [consumed]
                      Node 15997 (Hash Join) [consumed]
                        Node 15998 (Seq Scan) - LEAF
                        Node 15999 (Hash)
                          Node 16000 (Seq Scan) - LEAF
              Node 16001 (Index Scan) - LEAF
            Node 16002 (Hash)
              Node 16003 (Seq Scan) - LEAF
          Node 16004 (Index Scan) - LEAF
        Node 16005 (Hash)
          Node 16006 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15993 | 15984, 15985, 15994, 15995, 15996, 15997 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15984 | 15985, 15993, 15994, 15995, 15996, 15997 |
...
|------|------|--------|-----------|-----|--------|
| 15984 | Aggregate | 1200.21 | 1130.01 | 5.8% | pattern |
| 15986 | Sort | 1194.83 | 1046.21 | 12.4% | operator |
| 15987 | Hash Join | 1194.57 | 785.61 | 34.2% | operator |
| 15988 | Nested Loop | 1194.25 | 1198.29 | 0.3% | operator |
| 16005 | Hash | 0.02 | 13.72 | 80629.9% | operator |
| 15989 | Hash Join | 1190.17 | 785.60 | 34.0% | operator |
| 16004 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16006 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 15990 | Nested Loop | 1142.56 | 1198.29 | 4.9% | operator |
| 16002 | Hash | 43.02 | 19.97 | 53.6% | operator |
| 15991 | Hash Join | 221.02 | 251.67 | 13.9% | operator |
| 16001 | Index Scan | 0.04 | 2.88 | 7102.1% | operator |
| 16003 | Seq Scan | 42.91 | 48.87 | 13.9% | operator |
| 15992 | Seq Scan | 167.23 | 159.67 | 4.5% | operator |
| 15993 | Hash | 34.93 | 43.51 | 24.6% | pattern |
| 15998 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 15999 | Hash | 0.14 | 18.14 | 12860.1% | operator |
| 16000 | Seq Scan | 0.14 | 23.36 | 16952.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16000 (Seq Scan) - LEAF
...
### Step 2: Node 15998 (Seq Scan) - LEAF
...
### Step 4: Node 15992 (Seq Scan) - LEAF
...
### Step 5: Node 15993 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15984, 15985, 15994, 15995, 15996, 15997
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16001 (Index Scan) - LEAF
...
### Step 8: Node 16003 (Seq Scan) - LEAF
...
### Step 12: Node 16004 (Index Scan) - LEAF
...
### Step 13: Node 16006 (Seq Scan) - LEAF
...
### Step 14: Node 15988 (Nested Loop)
...
### Step 18: Node 15984 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15985, 15993, 15994, 15995, 15996, 15997
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2479
