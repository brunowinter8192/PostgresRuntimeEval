...
**Test Query:** Q8_107_seed_869627286
...
```
Node 13362 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13363 (Gather Merge) [consumed]
    Node 13364 (Sort)
      Node 13365 (Hash Join)
        Node 13366 (Nested Loop)
          Node 13367 (Hash Join)
            Node 13368 (Nested Loop)
              Node 13369 (Hash Join)
                Node 13370 (Seq Scan) - LEAF
                Node 13371 (Hash) [PATTERN: 444761fb]
                  Node 13372 (Hash Join) [consumed]
                    Node 13373 (Seq Scan) [consumed] - LEAF
                    Node 13374 (Hash) [consumed]
                      Node 13375 (Hash Join) [consumed]
                        Node 13376 (Seq Scan) - LEAF
                        Node 13377 (Hash)
                          Node 13378 (Seq Scan) - LEAF
              Node 13379 (Index Scan) - LEAF
            Node 13380 (Hash)
              Node 13381 (Seq Scan) - LEAF
          Node 13382 (Index Scan) - LEAF
        Node 13383 (Hash)
          Node 13384 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13371 | 13362, 13363, 13372, 13373, 13374, 13375 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13362 | 13363, 13371, 13372, 13373, 13374, 13375 |
...
|------|------|--------|-----------|-----|--------|
| 13362 | Aggregate | 1237.39 | 1130.95 | 8.6% | pattern |
| 13364 | Sort | 1230.95 | 1046.21 | 15.0% | operator |
| 13365 | Hash Join | 1230.70 | 785.61 | 36.2% | operator |
| 13366 | Nested Loop | 1230.37 | 1198.29 | 2.6% | operator |
| 13383 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 13367 | Hash Join | 1226.48 | 785.60 | 35.9% | operator |
| 13382 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13384 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 13368 | Nested Loop | 1179.13 | 1198.29 | 1.6% | operator |
| 13380 | Hash | 42.83 | 19.97 | 53.4% | operator |
| 13369 | Hash Join | 220.33 | 251.67 | 14.2% | operator |
| 13379 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 13381 | Seq Scan | 42.74 | 48.87 | 14.4% | operator |
| 13370 | Seq Scan | 162.88 | 159.67 | 2.0% | operator |
| 13371 | Hash | 39.28 | 43.51 | 10.8% | pattern |
| 13376 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13377 | Hash | 0.18 | 18.14 | 9980.1% | operator |
| 13378 | Seq Scan | 0.18 | 23.36 | 13099.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13378 (Seq Scan) - LEAF
...
### Step 2: Node 13376 (Seq Scan) - LEAF
...
### Step 4: Node 13370 (Seq Scan) - LEAF
...
### Step 5: Node 13371 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13362, 13363, 13372, 13373, 13374, 13375
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13379 (Index Scan) - LEAF
...
### Step 8: Node 13381 (Seq Scan) - LEAF
...
### Step 12: Node 13382 (Index Scan) - LEAF
...
### Step 13: Node 13384 (Seq Scan) - LEAF
...
### Step 14: Node 13366 (Nested Loop)
...
### Step 18: Node 13362 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13363, 13371, 13372, 13373, 13374, 13375
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2460
