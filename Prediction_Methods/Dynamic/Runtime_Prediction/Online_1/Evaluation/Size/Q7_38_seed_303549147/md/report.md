...
**Test Query:** Q7_38_seed_303549147
**Timestamp:** 2026-01-18 17:56:23
...
```
Node 11959 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11960 (Gather Merge) [consumed]
    Node 11961 (Sort)
      Node 11962 (Hash Join)
        Node 11963 (Nested Loop)
          Node 11964 (Hash Join) [PATTERN: 895c6e8c]
            Node 11965 (Seq Scan) [consumed] - LEAF
            Node 11966 (Hash) [consumed]
              Node 11967 (Hash Join) [PATTERN: f4cb205a]
                Node 11968 (Seq Scan) [consumed] - LEAF
                Node 11969 (Hash) [consumed]
                  Node 11970 (Seq Scan) [consumed] - LEAF
          Node 11971 (Index Scan) - LEAF
        Node 11972 (Hash)
          Node 11973 (Hash Join) [PATTERN: f4cb205a]
            Node 11974 (Seq Scan) [consumed] - LEAF
            Node 11975 (Hash) [consumed]
              Node 11976 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11967 | 11959, 11960, 11964, 11965, 11966, 11968, 11969, 11970, 11973, 11974, 11975, 11976 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11973 | 11959, 11960, 11964, 11965, 11966, 11967, 11968, 11969, 11970, 11974, 11975, 11976 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11959 | 11960, 11964, 11965, 11966, 11967, 11968, 11969, 11970, 11973, 11974, 11975, 11976 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11964 | 11959, 11960, 11965, 11966, 11967, 11968, 11969, 11970, 11973, 11974, 11975, 11976 |
...
|------|------|--------|-----------|-----|--------|
| 11959 | Aggregate | 1213.36 | 1163.13 | 4.1% | pattern |
| 11961 | Sort | 1205.99 | 1109.16 | 8.0% | operator |
| 11962 | Hash Join | 1205.38 | 843.11 | 30.1% | operator |
| 11963 | Nested Loop | 1199.53 | 1215.99 | 1.4% | operator |
| 11972 | Hash | 1.91 | 27.74 | 1354.8% | operator |
| 11964 | Hash Join | 211.81 | 304.89 | 43.9% | pattern |
| 11971 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11973 | Hash Join | 1.86 | 429.36 | 22922.1% | pattern |
| 11967 | Hash Join | 34.44 | 429.38 | 1146.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11967 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11959, 11960, 11964, 11965, 11966, 11968, 11969, 11970, 11973, 11974, 11975, 11976
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11964 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11959, 11960, 11965, 11966, 11967, 11968, 11969, 11970, 11973, 11974, 11975, 11976
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11971 (Index Scan) - LEAF
...
### Step 4: Node 11973 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11959, 11960, 11964, 11965, 11966, 11967, 11968, 11969, 11970, 11974, 11975, 11976
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11959 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11960, 11964, 11965, 11966, 11967, 11968, 11969, 11970, 11973, 11974, 11975, 11976
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
