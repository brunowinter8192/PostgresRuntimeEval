...
**Test Query:** Q5_51_seed_410201550
...
```
Node 8821 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8822 (Aggregate) [consumed]
    Node 8823 (Gather Merge) [consumed]
      Node 8824 (Aggregate)
        Node 8825 (Sort) [PATTERN: 3e2d5a00]
          Node 8826 (Hash Join) [consumed]
            Node 8827 (Nested Loop)
              Node 8828 (Hash Join)
                Node 8829 (Seq Scan) - LEAF
                Node 8830 (Hash) [PATTERN: 444761fb]
                  Node 8831 (Hash Join) [consumed]
                    Node 8832 (Seq Scan) [consumed] - LEAF
                    Node 8833 (Hash) [consumed]
                      Node 8834 (Hash Join) [consumed]
                        Node 8835 (Seq Scan) - LEAF
                        Node 8836 (Hash)
                          Node 8837 (Seq Scan) - LEAF
              Node 8838 (Index Scan) - LEAF
            Node 8839 (Hash)
              Node 8840 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8830 | 8821, 8822, 8823, 8825, 8826, 8831, 8832, 8833, 8834 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8821 | 8822, 8823, 8825, 8826, 8830, 8831, 8832, 8833, 8834 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8825 | 8821, 8822, 8823, 8826, 8830, 8831, 8832, 8833, 8834 |
...
|------|------|--------|-----------|-----|--------|
| 8821 | Sort | 1054.73 | 2934.42 | 178.2% | pattern |
| 8824 | Aggregate | 1049.57 | 1128.79 | 7.5% | operator |
| 8825 | Sort | 1049.32 | 1835.90 | 75.0% | pattern |
| 8827 | Nested Loop | 1036.95 | 1188.37 | 14.6% | operator |
| 8839 | Hash | 3.82 | 18.67 | 389.0% | operator |
| 8828 | Hash Join | 209.80 | 253.07 | 20.6% | operator |
| 8838 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 8840 | Seq Scan | 3.29 | 12.36 | 276.2% | operator |
| 8829 | Seq Scan | 164.84 | 159.81 | 3.1% | operator |
| 8830 | Hash | 31.85 | 46.85 | 47.1% | pattern |
| 8835 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 8836 | Hash | 0.06 | 21.05 | 37495.1% | operator |
| 8837 | Seq Scan | 0.05 | 22.93 | 42369.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8837 (Seq Scan) - LEAF
...
### Step 2: Node 8835 (Seq Scan) - LEAF
...
### Step 4: Node 8829 (Seq Scan) - LEAF
...
### Step 5: Node 8830 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8821, 8822, 8823, 8825, 8826, 8831, 8832, 8833, 8834
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8838 (Index Scan) - LEAF
...
### Step 8: Node 8840 (Seq Scan) - LEAF
...
### Step 11: Node 8825 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8821, 8822, 8823, 8826, 8830, 8831, 8832, 8833, 8834
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8821 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8822, 8823, 8825, 8826, 8830, 8831, 8832, 8833, 8834
- **Input Features:**
