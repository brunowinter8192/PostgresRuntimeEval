...
**Test Query:** Q10_72_seed_582486201
...
```
Node 20917 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20918 (Sort) [consumed]
    Node 20919 (Aggregate) [consumed]
      Node 20920 (Gather) [consumed]
        Node 20921 (Hash Join)
          Node 20922 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20923 (Nested Loop) [consumed]
              Node 20924 (Seq Scan) - LEAF
              Node 20925 (Index Scan) - LEAF
            Node 20926 (Hash) [consumed]
              Node 20927 (Seq Scan) - LEAF
          Node 20928 (Hash)
            Node 20929 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20917 | 20918, 20919, 20920, 20922, 20923, 20926 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20922 | 20917, 20918, 20919, 20920, 20923, 20926 |
...
|------|------|--------|-----------|-----|--------|
| 20917 | Limit | 1154.80 | 1199.47 | 3.9% | pattern |
| 20921 | Hash Join | 1101.48 | 1095.53 | 0.5% | operator |
| 20922 | Hash Join | 1098.64 | 1120.19 | 2.0% | pattern |
| 20928 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 20929 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 20924 | Seq Scan | 162.69 | 159.74 | 1.8% | operator |
| 20925 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20927 | Seq Scan | 24.67 | 26.64 | 8.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20924 (Seq Scan) - LEAF
...
### Step 2: Node 20925 (Index Scan) - LEAF
...
### Step 3: Node 20927 (Seq Scan) - LEAF
...
### Step 4: Node 20929 (Seq Scan) - LEAF
...
### Step 5: Node 20922 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20917, 20918, 20919, 20920, 20923, 20926
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18089
  - HashJoin_nt1=18089
...
### Step 8: Node 20917 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20918, 20919, 20920, 20922, 20923, 20926
- **Input Features:**
