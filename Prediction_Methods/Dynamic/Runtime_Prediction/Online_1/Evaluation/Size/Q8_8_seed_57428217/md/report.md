...
```
Node 16375 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16376 (Gather Merge) [consumed]
    Node 16377 (Sort)
      Node 16378 (Hash Join)
        Node 16379 (Nested Loop)
          Node 16380 (Hash Join)
            Node 16381 (Nested Loop)
              Node 16382 (Hash Join)
                Node 16383 (Seq Scan) - LEAF
                Node 16384 (Hash) [PATTERN: 444761fb]
                  Node 16385 (Hash Join) [consumed]
                    Node 16386 (Seq Scan) [consumed] - LEAF
                    Node 16387 (Hash) [consumed]
                      Node 16388 (Hash Join) [consumed]
                        Node 16389 (Seq Scan) - LEAF
                        Node 16390 (Hash)
                          Node 16391 (Seq Scan) - LEAF
              Node 16392 (Index Scan) - LEAF
            Node 16393 (Hash)
              Node 16394 (Seq Scan) - LEAF
          Node 16395 (Index Scan) - LEAF
        Node 16396 (Hash)
          Node 16397 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16384 | 16375, 16376, 16385, 16386, 16387, 16388 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16375 | 16376, 16384, 16385, 16386, 16387, 16388 |
...
|------|------|--------|-----------|-----|--------|
| 16375 | Aggregate | 1229.11 | 1132.59 | 7.9% | pattern |
| 16377 | Sort | 1224.00 | 1046.21 | 14.5% | operator |
| 16378 | Hash Join | 1223.76 | 785.61 | 35.8% | operator |
| 16379 | Nested Loop | 1223.46 | 1198.29 | 2.1% | operator |
| 16396 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 16380 | Hash Join | 1219.71 | 785.60 | 35.6% | operator |
| 16395 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16397 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 16381 | Nested Loop | 1173.95 | 1198.29 | 2.1% | operator |
| 16393 | Hash | 41.26 | 19.97 | 51.6% | operator |
| 16382 | Hash Join | 213.60 | 251.67 | 17.8% | operator |
| 16392 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 16394 | Seq Scan | 41.19 | 48.87 | 18.7% | operator |
| 16383 | Seq Scan | 157.30 | 159.67 | 1.5% | operator |
| 16384 | Hash | 37.24 | 43.51 | 16.8% | pattern |
| 16389 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 16390 | Hash | 0.21 | 18.14 | 8499.1% | operator |
| 16391 | Seq Scan | 0.21 | 23.36 | 11186.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16391 (Seq Scan) - LEAF
...
### Step 2: Node 16389 (Seq Scan) - LEAF
...
### Step 4: Node 16383 (Seq Scan) - LEAF
...
### Step 5: Node 16384 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16375, 16376, 16385, 16386, 16387, 16388
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16392 (Index Scan) - LEAF
...
### Step 8: Node 16394 (Seq Scan) - LEAF
...
### Step 12: Node 16395 (Index Scan) - LEAF
...
### Step 13: Node 16397 (Seq Scan) - LEAF
...
### Step 14: Node 16379 (Nested Loop)
...
### Step 18: Node 16375 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16376, 16384, 16385, 16386, 16387, 16388
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2427
