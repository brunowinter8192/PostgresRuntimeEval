...
**Test Query:** Q10_28_seed_221508837
...
```
Node 20280 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20281 (Sort) [consumed]
    Node 20282 (Aggregate) [consumed]
      Node 20283 (Gather) [consumed]
        Node 20284 (Hash Join)
          Node 20285 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20286 (Nested Loop) [consumed]
              Node 20287 (Seq Scan) - LEAF
              Node 20288 (Index Scan) - LEAF
            Node 20289 (Hash) [consumed]
              Node 20290 (Seq Scan) - LEAF
          Node 20291 (Hash)
            Node 20292 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20280 | 20281, 20282, 20283, 20285, 20286, 20289 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20285 | 20280, 20281, 20282, 20283, 20286, 20289 |
...
|------|------|--------|-----------|-----|--------|
| 20280 | Limit | 1164.66 | 1199.47 | 3.0% | pattern |
| 20284 | Hash Join | 1120.19 | 1095.53 | 2.2% | operator |
| 20285 | Hash Join | 1117.28 | 1120.19 | 0.3% | pattern |
| 20291 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 20292 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 20287 | Seq Scan | 163.71 | 159.74 | 2.4% | operator |
| 20288 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20290 | Seq Scan | 29.30 | 26.64 | 9.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20287 (Seq Scan) - LEAF
...
### Step 2: Node 20288 (Index Scan) - LEAF
...
### Step 3: Node 20290 (Seq Scan) - LEAF
...
### Step 4: Node 20292 (Seq Scan) - LEAF
...
### Step 5: Node 20285 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20280, 20281, 20282, 20283, 20286, 20289
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18089
  - HashJoin_nt1=18089
...
### Step 8: Node 20280 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20281, 20282, 20283, 20285, 20286, 20289
- **Input Features:**
