...
**Test Query:** Q10_77_seed_623506356
...
```
Node 20982 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20983 (Sort) [consumed]
    Node 20984 (Aggregate) [consumed]
      Node 20985 (Gather) [consumed]
        Node 20986 (Hash Join)
          Node 20987 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20988 (Nested Loop) [consumed]
              Node 20989 (Seq Scan) - LEAF
              Node 20990 (Index Scan) - LEAF
            Node 20991 (Hash) [consumed]
              Node 20992 (Seq Scan) - LEAF
          Node 20993 (Hash)
            Node 20994 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20982 | 20983, 20984, 20985, 20987, 20988, 20991 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20987 | 20982, 20983, 20984, 20985, 20988, 20991 |
...
|------|------|--------|-----------|-----|--------|
| 20982 | Limit | 1154.72 | 1199.47 | 3.9% | pattern |
| 20986 | Hash Join | 1101.80 | 1095.55 | 0.6% | operator |
| 20987 | Hash Join | 1098.68 | 1120.19 | 2.0% | pattern |
| 20993 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20994 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 20989 | Seq Scan | 158.45 | 159.74 | 0.8% | operator |
| 20990 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20992 | Seq Scan | 26.04 | 26.64 | 2.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20989 (Seq Scan) - LEAF
...
### Step 2: Node 20990 (Index Scan) - LEAF
...
### Step 3: Node 20992 (Seq Scan) - LEAF
...
### Step 4: Node 20994 (Seq Scan) - LEAF
...
### Step 5: Node 20987 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20982, 20983, 20984, 20985, 20988, 20991
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18183
  - HashJoin_nt1=18183
...
### Step 8: Node 20982 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20983, 20984, 20985, 20987, 20988, 20991
- **Input Features:**
