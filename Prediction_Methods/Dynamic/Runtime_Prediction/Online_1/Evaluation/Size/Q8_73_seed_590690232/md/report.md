...
**Test Query:** Q8_73_seed_590690232
...
```
Node 15961 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15962 (Gather Merge) [consumed]
    Node 15963 (Sort)
      Node 15964 (Hash Join)
        Node 15965 (Nested Loop)
          Node 15966 (Hash Join)
            Node 15967 (Nested Loop)
              Node 15968 (Hash Join)
                Node 15969 (Seq Scan) - LEAF
                Node 15970 (Hash) [PATTERN: 444761fb]
                  Node 15971 (Hash Join) [consumed]
                    Node 15972 (Seq Scan) [consumed] - LEAF
                    Node 15973 (Hash) [consumed]
                      Node 15974 (Hash Join) [consumed]
                        Node 15975 (Seq Scan) - LEAF
                        Node 15976 (Hash)
                          Node 15977 (Seq Scan) - LEAF
              Node 15978 (Index Scan) - LEAF
            Node 15979 (Hash)
              Node 15980 (Seq Scan) - LEAF
          Node 15981 (Index Scan) - LEAF
        Node 15982 (Hash)
          Node 15983 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15970 | 15961, 15962, 15971, 15972, 15973, 15974 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15961 | 15962, 15970, 15971, 15972, 15973, 15974 |
...
|------|------|--------|-----------|-----|--------|
| 15961 | Aggregate | 1212.58 | 1131.48 | 6.7% | pattern |
| 15963 | Sort | 1207.31 | 1046.21 | 13.3% | operator |
| 15964 | Hash Join | 1207.05 | 785.61 | 34.9% | operator |
| 15965 | Nested Loop | 1206.68 | 1198.29 | 0.7% | operator |
| 15982 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 15966 | Hash Join | 1202.97 | 785.60 | 34.7% | operator |
| 15981 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15983 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 15967 | Nested Loop | 1157.48 | 1198.29 | 3.5% | operator |
| 15979 | Hash | 40.99 | 19.97 | 51.3% | operator |
| 15968 | Hash Join | 215.81 | 251.67 | 16.6% | operator |
| 15978 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15980 | Seq Scan | 40.88 | 48.87 | 19.5% | operator |
| 15969 | Seq Scan | 162.57 | 159.67 | 1.8% | operator |
| 15970 | Hash | 35.66 | 43.51 | 22.0% | pattern |
| 15975 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 15976 | Hash | 0.14 | 18.14 | 12413.2% | operator |
| 15977 | Seq Scan | 0.14 | 23.36 | 16352.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15977 (Seq Scan) - LEAF
...
### Step 2: Node 15975 (Seq Scan) - LEAF
...
### Step 4: Node 15969 (Seq Scan) - LEAF
...
### Step 5: Node 15970 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15961, 15962, 15971, 15972, 15973, 15974
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15978 (Index Scan) - LEAF
...
### Step 8: Node 15980 (Seq Scan) - LEAF
...
### Step 12: Node 15981 (Index Scan) - LEAF
...
### Step 13: Node 15983 (Seq Scan) - LEAF
...
### Step 14: Node 15965 (Nested Loop)
...
### Step 18: Node 15961 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15962, 15970, 15971, 15972, 15973, 15974
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2449
