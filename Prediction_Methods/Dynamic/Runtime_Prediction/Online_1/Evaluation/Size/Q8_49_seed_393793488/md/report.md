...
**Test Query:** Q8_49_seed_393793488
...
```
Node 15340 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15341 (Gather Merge) [consumed]
    Node 15342 (Sort)
      Node 15343 (Hash Join)
        Node 15344 (Nested Loop)
          Node 15345 (Hash Join)
            Node 15346 (Nested Loop)
              Node 15347 (Hash Join)
                Node 15348 (Seq Scan) - LEAF
                Node 15349 (Hash) [PATTERN: 444761fb]
                  Node 15350 (Hash Join) [consumed]
                    Node 15351 (Seq Scan) [consumed] - LEAF
                    Node 15352 (Hash) [consumed]
                      Node 15353 (Hash Join) [consumed]
                        Node 15354 (Seq Scan) - LEAF
                        Node 15355 (Hash)
                          Node 15356 (Seq Scan) - LEAF
              Node 15357 (Index Scan) - LEAF
            Node 15358 (Hash)
              Node 15359 (Seq Scan) - LEAF
          Node 15360 (Index Scan) - LEAF
        Node 15361 (Hash)
          Node 15362 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15349 | 15340, 15341, 15350, 15351, 15352, 15353 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15340 | 15341, 15349, 15350, 15351, 15352, 15353 |
...
|------|------|--------|-----------|-----|--------|
| 15340 | Aggregate | 1224.72 | 1133.68 | 7.4% | pattern |
| 15342 | Sort | 1219.93 | 1046.21 | 14.2% | operator |
| 15343 | Hash Join | 1219.69 | 785.61 | 35.6% | operator |
| 15344 | Nested Loop | 1219.38 | 1198.29 | 1.7% | operator |
| 15361 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 15345 | Hash Join | 1215.62 | 785.60 | 35.4% | operator |
| 15360 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15362 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 15346 | Nested Loop | 1170.23 | 1198.29 | 2.4% | operator |
| 15358 | Hash | 40.70 | 19.97 | 50.9% | operator |
| 15347 | Hash Join | 213.57 | 251.67 | 17.8% | operator |
| 15357 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 15359 | Seq Scan | 40.65 | 48.88 | 20.3% | operator |
| 15348 | Seq Scan | 157.07 | 159.67 | 1.7% | operator |
| 15349 | Hash | 38.29 | 43.51 | 13.6% | pattern |
| 15354 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 15355 | Hash | 0.20 | 18.14 | 8972.1% | operator |
| 15356 | Seq Scan | 0.20 | 23.36 | 11819.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15356 (Seq Scan) - LEAF
...
### Step 2: Node 15354 (Seq Scan) - LEAF
...
### Step 4: Node 15348 (Seq Scan) - LEAF
...
### Step 5: Node 15349 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15340, 15341, 15350, 15351, 15352, 15353
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15357 (Index Scan) - LEAF
...
### Step 8: Node 15359 (Seq Scan) - LEAF
...
### Step 12: Node 15360 (Index Scan) - LEAF
...
### Step 13: Node 15362 (Seq Scan) - LEAF
...
### Step 14: Node 15344 (Nested Loop)
...
### Step 18: Node 15340 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15341, 15349, 15350, 15351, 15352, 15353
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2343
  - Aggregate_nt1=2343
