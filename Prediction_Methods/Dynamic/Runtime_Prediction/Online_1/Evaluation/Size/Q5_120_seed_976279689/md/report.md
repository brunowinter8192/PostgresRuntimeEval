...
**Test Query:** Q5_120_seed_976279689
...
```
Node 7341 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7342 (Aggregate) [consumed]
    Node 7343 (Gather Merge) [consumed]
      Node 7344 (Aggregate)
        Node 7345 (Sort) [PATTERN: 3e2d5a00]
          Node 7346 (Hash Join) [consumed]
            Node 7347 (Nested Loop)
              Node 7348 (Hash Join)
                Node 7349 (Seq Scan) - LEAF
                Node 7350 (Hash) [PATTERN: 444761fb]
                  Node 7351 (Hash Join) [consumed]
                    Node 7352 (Seq Scan) [consumed] - LEAF
                    Node 7353 (Hash) [consumed]
                      Node 7354 (Hash Join) [consumed]
                        Node 7355 (Seq Scan) - LEAF
                        Node 7356 (Hash)
                          Node 7357 (Seq Scan) - LEAF
              Node 7358 (Index Scan) - LEAF
            Node 7359 (Hash)
              Node 7360 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7350 | 7341, 7342, 7343, 7345, 7346, 7351, 7352, 7353, 7354 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7341 | 7342, 7343, 7345, 7346, 7350, 7351, 7352, 7353, 7354 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7345 | 7341, 7342, 7343, 7346, 7350, 7351, 7352, 7353, 7354 |
...
|------|------|--------|-----------|-----|--------|
| 7341 | Sort | 1061.08 | 2934.42 | 176.6% | pattern |
| 7344 | Aggregate | 1056.53 | 1128.78 | 6.8% | operator |
| 7345 | Sort | 1056.27 | 1835.91 | 73.8% | pattern |
| 7347 | Nested Loop | 1041.81 | 1188.39 | 14.1% | operator |
| 7359 | Hash | 5.85 | 18.67 | 219.4% | operator |
| 7348 | Hash Join | 218.79 | 253.07 | 15.7% | operator |
| 7358 | Index Scan | 0.07 | -0.22 | 405.6% | operator |
| 7360 | Seq Scan | 5.29 | 12.36 | 133.7% | operator |
| 7349 | Seq Scan | 171.76 | 159.81 | 7.0% | operator |
| 7350 | Hash | 34.25 | 46.85 | 36.8% | pattern |
| 7355 | Seq Scan | 0.01 | 11.81 | 147503.4% | operator |
| 7356 | Hash | 0.06 | 21.05 | 36835.5% | operator |
| 7357 | Seq Scan | 0.05 | 22.93 | 42369.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7357 (Seq Scan) - LEAF
...
### Step 2: Node 7355 (Seq Scan) - LEAF
...
### Step 4: Node 7349 (Seq Scan) - LEAF
...
### Step 5: Node 7350 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7341, 7342, 7343, 7345, 7346, 7351, 7352, 7353, 7354
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7358 (Index Scan) - LEAF
...
### Step 8: Node 7360 (Seq Scan) - LEAF
...
### Step 11: Node 7345 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7341, 7342, 7343, 7346, 7350, 7351, 7352, 7353, 7354
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7341 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7342, 7343, 7345, 7346, 7350, 7351, 7352, 7353, 7354
- **Input Features:**
