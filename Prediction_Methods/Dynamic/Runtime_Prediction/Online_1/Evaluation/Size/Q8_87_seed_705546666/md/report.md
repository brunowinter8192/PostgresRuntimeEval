...
**Test Query:** Q8_87_seed_705546666
...
```
Node 16306 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16307 (Gather Merge) [consumed]
    Node 16308 (Sort)
      Node 16309 (Hash Join)
        Node 16310 (Nested Loop)
          Node 16311 (Hash Join)
            Node 16312 (Nested Loop)
              Node 16313 (Hash Join)
                Node 16314 (Seq Scan) - LEAF
                Node 16315 (Hash) [PATTERN: 444761fb]
                  Node 16316 (Hash Join) [consumed]
                    Node 16317 (Seq Scan) [consumed] - LEAF
                    Node 16318 (Hash) [consumed]
                      Node 16319 (Hash Join) [consumed]
                        Node 16320 (Seq Scan) - LEAF
                        Node 16321 (Hash)
                          Node 16322 (Seq Scan) - LEAF
              Node 16323 (Index Scan) - LEAF
            Node 16324 (Hash)
              Node 16325 (Seq Scan) - LEAF
          Node 16326 (Index Scan) - LEAF
        Node 16327 (Hash)
          Node 16328 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16315 | 16306, 16307, 16316, 16317, 16318, 16319 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16306 | 16307, 16315, 16316, 16317, 16318, 16319 |
...
|------|------|--------|-----------|-----|--------|
| 16306 | Aggregate | 1232.99 | 1132.59 | 8.1% | pattern |
| 16308 | Sort | 1228.57 | 1046.21 | 14.8% | operator |
| 16309 | Hash Join | 1228.31 | 785.61 | 36.0% | operator |
| 16310 | Nested Loop | 1227.99 | 1198.29 | 2.4% | operator |
| 16327 | Hash | 0.01 | 13.72 | 114267.4% | operator |
| 16311 | Hash Join | 1223.33 | 785.60 | 35.8% | operator |
| 16326 | Index Scan | 0.01 | 12.42 | 177378.3% | operator |
| 16328 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 16312 | Nested Loop | 1176.09 | 1198.29 | 1.9% | operator |
| 16324 | Hash | 42.87 | 19.97 | 53.4% | operator |
| 16313 | Hash Join | 212.23 | 251.67 | 18.6% | operator |
| 16323 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 16325 | Seq Scan | 42.81 | 48.87 | 14.2% | operator |
| 16314 | Seq Scan | 157.93 | 159.67 | 1.1% | operator |
| 16315 | Hash | 35.56 | 43.51 | 22.4% | pattern |
| 16320 | Seq Scan | 0.01 | 12.22 | 93938.0% | operator |
| 16321 | Hash | 0.25 | 18.14 | 7100.1% | operator |
| 16322 | Seq Scan | 0.25 | 23.36 | 9320.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16322 (Seq Scan) - LEAF
...
### Step 2: Node 16320 (Seq Scan) - LEAF
...
### Step 4: Node 16314 (Seq Scan) - LEAF
...
### Step 5: Node 16315 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16306, 16307, 16316, 16317, 16318, 16319
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16323 (Index Scan) - LEAF
...
### Step 8: Node 16325 (Seq Scan) - LEAF
...
### Step 12: Node 16326 (Index Scan) - LEAF
...
### Step 13: Node 16328 (Seq Scan) - LEAF
...
### Step 14: Node 16310 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70508.4700
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 16327 (Hash)
...
- **Input Features:**
  ...
  - total_cost=70514.3900
- **Output:** st=113.62, rt=785.61

### Step 17: Node 16308 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70552.0300
  - total_cost=70553.9800
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 16306 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16307, 16315, 16316, 16317, 16318, 16319
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2427
  ...
  - Aggregate_sel=0.9913
  - Aggregate_startup_cost=71552.0700
  - Aggregate_total_cost=71935.8600
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2427
  - GatherMerge_Outer_nt1=783
  ...
  - GatherMerge_Outer_startup_cost=71552.0700
  - GatherMerge_Outer_total_cost=71839.2000
- **Output:** st=1129.61, rt=1132.59
