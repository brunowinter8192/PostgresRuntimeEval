...
**Test Query:** Q8_72_seed_582486201
...
```
Node 15938 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15939 (Gather Merge) [consumed]
    Node 15940 (Sort)
      Node 15941 (Hash Join)
        Node 15942 (Nested Loop)
          Node 15943 (Hash Join)
            Node 15944 (Nested Loop)
              Node 15945 (Hash Join)
                Node 15946 (Seq Scan) - LEAF
                Node 15947 (Hash) [PATTERN: 444761fb]
                  Node 15948 (Hash Join) [consumed]
                    Node 15949 (Seq Scan) [consumed] - LEAF
                    Node 15950 (Hash) [consumed]
                      Node 15951 (Hash Join) [consumed]
                        Node 15952 (Seq Scan) - LEAF
                        Node 15953 (Hash)
                          Node 15954 (Seq Scan) - LEAF
              Node 15955 (Index Scan) - LEAF
            Node 15956 (Hash)
              Node 15957 (Seq Scan) - LEAF
          Node 15958 (Index Scan) - LEAF
        Node 15959 (Hash)
          Node 15960 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15947 | 15938, 15939, 15948, 15949, 15950, 15951 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15938 | 15939, 15947, 15948, 15949, 15950, 15951 |
...
|------|------|--------|-----------|-----|--------|
| 15938 | Aggregate | 1218.51 | 1128.46 | 7.4% | pattern |
| 15940 | Sort | 1213.36 | 1046.21 | 13.8% | operator |
| 15941 | Hash Join | 1213.08 | 785.61 | 35.2% | operator |
| 15942 | Nested Loop | 1212.76 | 1198.29 | 1.2% | operator |
| 15959 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 15943 | Hash Join | 1208.95 | 785.60 | 35.0% | operator |
| 15958 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15960 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 15944 | Nested Loop | 1162.88 | 1198.29 | 3.0% | operator |
| 15956 | Hash | 39.74 | 19.97 | 49.7% | operator |
| 15945 | Hash Join | 209.95 | 251.67 | 19.9% | operator |
| 15955 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15957 | Seq Scan | 39.64 | 48.86 | 23.3% | operator |
| 15946 | Seq Scan | 157.26 | 159.67 | 1.5% | operator |
| 15947 | Hash | 34.82 | 43.51 | 25.0% | pattern |
| 15952 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 15953 | Hash | 0.07 | 18.14 | 25820.2% | operator |
| 15954 | Seq Scan | 0.07 | 23.36 | 34769.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15954 (Seq Scan) - LEAF
...
### Step 2: Node 15952 (Seq Scan) - LEAF
...
### Step 4: Node 15946 (Seq Scan) - LEAF
...
### Step 5: Node 15947 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15938, 15939, 15948, 15949, 15950, 15951
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15955 (Index Scan) - LEAF
...
### Step 8: Node 15957 (Seq Scan) - LEAF
...
### Step 12: Node 15958 (Index Scan) - LEAF
...
### Step 13: Node 15960 (Seq Scan) - LEAF
...
### Step 14: Node 15942 (Nested Loop)
...
### Step 18: Node 15938 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15939, 15947, 15948, 15949, 15950, 15951
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2511
  ...
  - Aggregate_sel=0.9582
  - Aggregate_startup_cost=71562.1200
  - Aggregate_total_cost=71957.5300
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2511
  - GatherMerge_Outer_nt1=810
  ...
  - GatherMerge_Outer_startup_cost=71562.1200
  - GatherMerge_Outer_total_cost=71859.1900
- **Output:** st=1125.50, rt=1128.46
