...
**Test Query:** Q8_91_seed_738362790
**Timestamp:** 2026-01-18 18:53:54
...
```
Node 16421 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16422 (Gather Merge) [consumed]
    Node 16423 (Sort)
      Node 16424 (Hash Join)
        Node 16425 (Nested Loop)
          Node 16426 (Hash Join)
            Node 16427 (Nested Loop)
              Node 16428 (Hash Join)
                Node 16429 (Seq Scan) - LEAF
                Node 16430 (Hash) [PATTERN: 444761fb]
                  Node 16431 (Hash Join) [consumed]
                    Node 16432 (Seq Scan) [consumed] - LEAF
                    Node 16433 (Hash) [consumed]
                      Node 16434 (Hash Join) [consumed]
                        Node 16435 (Seq Scan) - LEAF
                        Node 16436 (Hash)
                          Node 16437 (Seq Scan) - LEAF
              Node 16438 (Index Scan) - LEAF
            Node 16439 (Hash)
              Node 16440 (Seq Scan) - LEAF
          Node 16441 (Index Scan) - LEAF
        Node 16442 (Hash)
          Node 16443 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16430 | 16421, 16422, 16431, 16432, 16433, 16434 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16421 | 16422, 16430, 16431, 16432, 16433, 16434 |
...
|------|------|--------|-----------|-----|--------|
| 16421 | Aggregate | 1202.73 | 1132.74 | 5.8% | pattern |
| 16423 | Sort | 1197.36 | 1046.21 | 12.6% | operator |
| 16424 | Hash Join | 1197.11 | 785.61 | 34.4% | operator |
| 16425 | Nested Loop | 1196.79 | 1198.29 | 0.1% | operator |
| 16442 | Hash | 0.02 | 13.72 | 80629.9% | operator |
| 16426 | Hash Join | 1192.85 | 785.60 | 34.1% | operator |
| 16441 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16443 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 16427 | Nested Loop | 1148.94 | 1198.29 | 4.3% | operator |
| 16439 | Hash | 39.51 | 19.97 | 49.5% | operator |
| 16428 | Hash Join | 218.05 | 251.67 | 15.4% | operator |
| 16438 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 16440 | Seq Scan | 39.46 | 48.87 | 23.9% | operator |
| 16429 | Seq Scan | 162.94 | 159.67 | 2.0% | operator |
| 16430 | Hash | 37.37 | 43.51 | 16.4% | pattern |
| 16435 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 16436 | Hash | 0.23 | 18.14 | 7928.4% | operator |
| 16437 | Seq Scan | 0.22 | 23.36 | 10376.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16437 (Seq Scan) - LEAF
...
### Step 2: Node 16435 (Seq Scan) - LEAF
...
### Step 4: Node 16429 (Seq Scan) - LEAF
...
### Step 5: Node 16430 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16421, 16422, 16431, 16432, 16433, 16434
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16438 (Index Scan) - LEAF
...
### Step 8: Node 16440 (Seq Scan) - LEAF
...
### Step 12: Node 16441 (Index Scan) - LEAF
...
### Step 13: Node 16443 (Seq Scan) - LEAF
...
### Step 14: Node 16425 (Nested Loop)
...
### Step 18: Node 16421 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16422, 16430, 16431, 16432, 16433, 16434
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2424
