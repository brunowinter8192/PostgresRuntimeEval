...
```
Node 25622 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 25623 (Aggregate) [consumed]
    Node 25624 (Aggregate)
      Node 25625 (Hash Join) [PATTERN: 895c6e8c]
        Node 25626 (Seq Scan) [consumed] - LEAF
        Node 25627 (Hash) [consumed]
          Node 25628 (Index Only Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Sort -> Aggregate (Outer) | 1d35fb97 | 25622 | 25623, 25625, 25626, 25627 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 25625 | 25622, 25623, 25626, 25627 |
...
|------|------|--------|-----------|-----|--------|
| 25622 | Sort | 924.68 | 1079.93 | 16.8% | pattern |
| 25624 | Aggregate | 918.59 | 999.55 | 8.8% | operator |
| 25625 | Hash Join | 711.27 | 470.31 | 33.9% | pattern |
| 25628 | Index Only Scan | 10.18 | 0.00 | 100.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 25628 (Index Only Scan) - LEAF

- **Source:** operator
- **Output:** st=0.00, rt=0.00

### Step 2: Node 25625 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 25622, 25623, 25626, 25627
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=1484970
...
### Step 4: Node 25622 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 25623, 25625, 25626, 25627
- **Input Features:**
