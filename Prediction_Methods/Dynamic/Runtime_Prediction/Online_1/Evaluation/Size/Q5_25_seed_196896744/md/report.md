...
**Test Query:** Q5_25_seed_196896744
...
```
Node 8241 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8242 (Aggregate) [consumed]
    Node 8243 (Gather Merge) [consumed]
      Node 8244 (Aggregate)
        Node 8245 (Sort) [PATTERN: 3e2d5a00]
          Node 8246 (Hash Join) [consumed]
            Node 8247 (Nested Loop)
              Node 8248 (Hash Join)
                Node 8249 (Seq Scan) - LEAF
                Node 8250 (Hash) [PATTERN: 444761fb]
                  Node 8251 (Hash Join) [consumed]
                    Node 8252 (Seq Scan) [consumed] - LEAF
                    Node 8253 (Hash) [consumed]
                      Node 8254 (Hash Join) [consumed]
                        Node 8255 (Seq Scan) - LEAF
                        Node 8256 (Hash)
                          Node 8257 (Seq Scan) - LEAF
              Node 8258 (Index Scan) - LEAF
            Node 8259 (Hash)
              Node 8260 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8250 | 8241, 8242, 8243, 8245, 8246, 8251, 8252, 8253, 8254 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8241 | 8242, 8243, 8245, 8246, 8250, 8251, 8252, 8253, 8254 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8245 | 8241, 8242, 8243, 8246, 8250, 8251, 8252, 8253, 8254 |
...
|------|------|--------|-----------|-----|--------|
| 8241 | Sort | 1073.39 | 2934.42 | 173.4% | pattern |
| 8244 | Aggregate | 1067.33 | 1128.74 | 5.8% | operator |
| 8245 | Sort | 1067.07 | 1835.96 | 72.1% | pattern |
| 8247 | Nested Loop | 1053.38 | 1188.57 | 12.8% | operator |
| 8259 | Hash | 5.18 | 18.67 | 260.7% | operator |
| 8248 | Hash Join | 213.50 | 253.07 | 18.5% | operator |
| 8258 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 8260 | Seq Scan | 4.68 | 12.36 | 163.9% | operator |
| 8249 | Seq Scan | 166.59 | 159.81 | 4.1% | operator |
| 8250 | Hash | 33.80 | 46.85 | 38.6% | pattern |
| 8255 | Seq Scan | 0.01 | 11.81 | 107247.9% | operator |
| 8256 | Hash | 0.06 | 21.05 | 37495.1% | operator |
| 8257 | Seq Scan | 0.05 | 22.93 | 42369.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8257 (Seq Scan) - LEAF
...
### Step 2: Node 8255 (Seq Scan) - LEAF
...
### Step 4: Node 8249 (Seq Scan) - LEAF
...
### Step 5: Node 8250 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8241, 8242, 8243, 8245, 8246, 8251, 8252, 8253, 8254
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8258 (Index Scan) - LEAF
...
### Step 8: Node 8260 (Seq Scan) - LEAF
...
### Step 11: Node 8245 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8241, 8242, 8243, 8246, 8250, 8251, 8252, 8253, 8254
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8241 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8242, 8243, 8245, 8246, 8250, 8251, 8252, 8253, 8254
- **Input Features:**
