...
**Test Query:** Q10_143_seed_1164972402
...
```
Node 19981 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19982 (Sort) [consumed]
    Node 19983 (Aggregate) [consumed]
      Node 19984 (Gather) [consumed]
        Node 19985 (Hash Join)
          Node 19986 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19987 (Nested Loop) [consumed]
              Node 19988 (Seq Scan) - LEAF
              Node 19989 (Index Scan) - LEAF
            Node 19990 (Hash) [consumed]
              Node 19991 (Seq Scan) - LEAF
          Node 19992 (Hash)
            Node 19993 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19981 | 19982, 19983, 19984, 19986, 19987, 19990 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19986 | 19981, 19982, 19983, 19984, 19987, 19990 |
...
|------|------|--------|-----------|-----|--------|
| 19981 | Limit | 1167.82 | 1199.47 | 2.7% | pattern |
| 19985 | Hash Join | 1108.83 | 1095.60 | 1.2% | operator |
| 19986 | Hash Join | 1105.94 | 1120.19 | 1.3% | pattern |
| 19992 | Hash | 0.01 | 15.71 | 130823.1% | operator |
| 19993 | Seq Scan | 0.01 | 6.98 | 77441.2% | operator |
| 19988 | Seq Scan | 162.01 | 159.74 | 1.4% | operator |
| 19989 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 19991 | Seq Scan | 24.74 | 26.64 | 7.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19988 (Seq Scan) - LEAF
...
### Step 2: Node 19989 (Index Scan) - LEAF
...
### Step 3: Node 19991 (Seq Scan) - LEAF
...
### Step 4: Node 19993 (Seq Scan) - LEAF
...
### Step 5: Node 19986 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19981, 19982, 19983, 19984, 19987, 19990
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18384
  - HashJoin_nt1=18384
...
- **Output:** st=54.23, rt=1095.60

### Step 8: Node 19981 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19982, 19983, 19984, 19986, 19987, 19990
- **Input Features:**
