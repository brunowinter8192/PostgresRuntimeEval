...
**Test Query:** Q9_90_seed_730158759
...
```
Node 19171 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 19172 (Aggregate) [consumed]
    Node 19173 (Gather)
      Node 19174 (Aggregate)
        Node 19175 (Nested Loop)
          Node 19176 (Hash Join) [PATTERN: bb930825]
            Node 19177 (Seq Scan) [consumed] - LEAF
            Node 19178 (Hash) [consumed]
              Node 19179 (Hash Join) [consumed]
                Node 19180 (Hash Join)
                  Node 19181 (Nested Loop)
                    Node 19182 (Seq Scan) - LEAF
                    Node 19183 (Index Scan) - LEAF
                  Node 19184 (Hash)
                    Node 19185 (Seq Scan) - LEAF
                Node 19186 (Hash)
                  Node 19187 (Seq Scan) - LEAF
          Node 19188 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | bb930825 | 19176 | 19171, 19172, 19177, 19178, 19179 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 19171 | 19172, 19176, 19177, 19178, 19179 |
...
|------|------|--------|-----------|-----|--------|
| 19171 | Sort | 1229.63 | 1045.06 | 15.0% | pattern |
| 19173 | Gather | 1228.41 | 964.74 | 21.5% | operator |
| 19174 | Aggregate | 1209.07 | 1106.30 | 8.5% | operator |
| 19175 | Nested Loop | 1184.45 | 1252.17 | 5.7% | operator |
| 19176 | Hash Join | 1011.20 | 134.80 | 86.7% | pattern |
| 19188 | Index Scan | 0.00 | -5.54 | 184812.7% | operator |
| 19180 | Hash Join | 168.90 | 837.58 | 395.9% | operator |
| 19186 | Hash | 12.78 | 14.95 | 16.9% | operator |
| 19181 | Nested Loop | 162.88 | 1251.77 | 668.5% | operator |
| 19184 | Hash | 3.89 | 15.94 | 310.1% | operator |
| 19187 | Seq Scan | 12.77 | 2.94 | 77.0% | operator |
| 19182 | Seq Scan | 19.78 | 44.10 | 122.9% | operator |
| 19183 | Index Scan | 0.08 | -1.39 | 1880.1% | operator |
| 19185 | Seq Scan | 3.35 | 9.45 | 181.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19182 (Seq Scan) - LEAF
...
### Step 2: Node 19183 (Index Scan) - LEAF
...
### Step 3: Node 19185 (Seq Scan) - LEAF
...
### Step 6: Node 19187 (Seq Scan) - LEAF
...
### Step 9: Node 19176 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** bb930825 (Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)])
- **Consumes:** Nodes 19171, 19172, 19177, 19178, 19179
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 19188 (Index Scan) - LEAF
...
### Step 11: Node 19175 (Nested Loop)
...
### Step 14: Node 19171 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 19172, 19176, 19177, 19178, 19179
- **Input Features:**
