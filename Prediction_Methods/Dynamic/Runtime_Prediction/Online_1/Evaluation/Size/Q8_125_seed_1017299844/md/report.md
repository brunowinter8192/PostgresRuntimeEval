...
**Test Query:** Q8_125_seed_1017299844
**Timestamp:** 2026-01-18 18:19:12
...
```
Node 13822 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13823 (Gather Merge) [consumed]
    Node 13824 (Sort)
      Node 13825 (Hash Join)
        Node 13826 (Nested Loop)
          Node 13827 (Hash Join)
            Node 13828 (Nested Loop)
              Node 13829 (Hash Join)
                Node 13830 (Seq Scan) - LEAF
                Node 13831 (Hash) [PATTERN: 444761fb]
                  Node 13832 (Hash Join) [consumed]
                    Node 13833 (Seq Scan) [consumed] - LEAF
                    Node 13834 (Hash) [consumed]
                      Node 13835 (Hash Join) [consumed]
                        Node 13836 (Seq Scan) - LEAF
                        Node 13837 (Hash)
                          Node 13838 (Seq Scan) - LEAF
              Node 13839 (Index Scan) - LEAF
            Node 13840 (Hash)
              Node 13841 (Seq Scan) - LEAF
          Node 13842 (Index Scan) - LEAF
        Node 13843 (Hash)
          Node 13844 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13831 | 13822, 13823, 13832, 13833, 13834, 13835 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13822 | 13823, 13831, 13832, 13833, 13834, 13835 |
...
|------|------|--------|-----------|-----|--------|
| 13822 | Aggregate | 1277.23 | 1132.74 | 11.3% | pattern |
| 13824 | Sort | 1271.23 | 1046.21 | 17.7% | operator |
| 13825 | Hash Join | 1270.97 | 785.61 | 38.2% | operator |
| 13826 | Nested Loop | 1270.63 | 1198.29 | 5.7% | operator |
| 13843 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 13827 | Hash Join | 1266.77 | 785.60 | 38.0% | operator |
| 13842 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13844 | Seq Scan | 0.01 | 6.51 | 49992.0% | operator |
| 13828 | Nested Loop | 1220.36 | 1198.29 | 1.8% | operator |
| 13840 | Hash | 41.90 | 19.97 | 52.3% | operator |
| 13829 | Hash Join | 215.88 | 251.67 | 16.6% | operator |
| 13839 | Index Scan | 0.04 | 2.88 | 6447.4% | operator |
| 13841 | Seq Scan | 41.81 | 48.87 | 16.9% | operator |
| 13830 | Seq Scan | 159.57 | 159.67 | 0.1% | operator |
| 13831 | Hash | 38.26 | 43.51 | 13.7% | pattern |
| 13836 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13837 | Hash | 0.21 | 18.14 | 8339.1% | operator |
| 13838 | Seq Scan | 0.21 | 23.36 | 10972.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13838 (Seq Scan) - LEAF
...
### Step 2: Node 13836 (Seq Scan) - LEAF
...
### Step 4: Node 13830 (Seq Scan) - LEAF
...
### Step 5: Node 13831 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13822, 13823, 13832, 13833, 13834, 13835
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13839 (Index Scan) - LEAF
...
### Step 8: Node 13841 (Seq Scan) - LEAF
...
### Step 12: Node 13842 (Index Scan) - LEAF
...
### Step 13: Node 13844 (Seq Scan) - LEAF
...
### Step 14: Node 13826 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70508.1600
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 13843 (Hash)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70551.6500
  - total_cost=70553.6100
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 13822 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13823, 13831, 13832, 13833, 13834, 13835
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2424
  ...
  - Aggregate_sel=0.9926
  - Aggregate_startup_cost=71551.6900
  - Aggregate_total_cost=71935.0700
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2424
  - GatherMerge_Outer_nt1=782
  ...
  - GatherMerge_Outer_startup_cost=71551.6900
  - GatherMerge_Outer_total_cost=71838.4700
- **Output:** st=1129.76, rt=1132.74
