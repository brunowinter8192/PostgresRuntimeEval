...
**Test Query:** Q5_139_seed_1132156278
...
```
Node 7741 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7742 (Aggregate) [consumed]
    Node 7743 (Gather Merge) [consumed]
      Node 7744 (Aggregate)
        Node 7745 (Sort) [PATTERN: 3e2d5a00]
          Node 7746 (Hash Join) [consumed]
            Node 7747 (Nested Loop)
              Node 7748 (Hash Join)
                Node 7749 (Seq Scan) - LEAF
                Node 7750 (Hash) [PATTERN: 444761fb]
                  Node 7751 (Hash Join) [consumed]
                    Node 7752 (Seq Scan) [consumed] - LEAF
                    Node 7753 (Hash) [consumed]
                      Node 7754 (Hash Join) [consumed]
                        Node 7755 (Seq Scan) - LEAF
                        Node 7756 (Hash)
                          Node 7757 (Seq Scan) - LEAF
              Node 7758 (Index Scan) - LEAF
            Node 7759 (Hash)
              Node 7760 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7750 | 7741, 7742, 7743, 7745, 7746, 7751, 7752, 7753, 7754 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7741 | 7742, 7743, 7745, 7746, 7750, 7751, 7752, 7753, 7754 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7745 | 7741, 7742, 7743, 7746, 7750, 7751, 7752, 7753, 7754 |
...
|------|------|--------|-----------|-----|--------|
| 7741 | Sort | 1069.60 | 2934.42 | 174.3% | pattern |
| 7744 | Aggregate | 1064.65 | 1128.79 | 6.0% | operator |
| 7745 | Sort | 1064.38 | 1835.90 | 72.5% | pattern |
| 7747 | Nested Loop | 1051.99 | 1188.37 | 13.0% | operator |
| 7759 | Hash | 3.63 | 18.67 | 414.4% | operator |
| 7748 | Hash Join | 220.82 | 253.07 | 14.6% | operator |
| 7758 | Index Scan | 0.07 | -0.22 | 405.6% | operator |
| 7760 | Seq Scan | 3.12 | 12.36 | 296.8% | operator |
| 7749 | Seq Scan | 169.27 | 159.81 | 5.6% | operator |
| 7750 | Hash | 37.00 | 46.85 | 26.6% | pattern |
| 7755 | Seq Scan | 0.01 | 11.81 | 117982.7% | operator |
| 7756 | Hash | 0.05 | 21.05 | 38887.5% | operator |
| 7757 | Seq Scan | 0.05 | 22.93 | 43170.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7757 (Seq Scan) - LEAF
...
### Step 2: Node 7755 (Seq Scan) - LEAF
...
### Step 4: Node 7749 (Seq Scan) - LEAF
...
### Step 5: Node 7750 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7741, 7742, 7743, 7745, 7746, 7751, 7752, 7753, 7754
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7758 (Index Scan) - LEAF
...
### Step 8: Node 7760 (Seq Scan) - LEAF
...
### Step 11: Node 7745 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7741, 7742, 7743, 7746, 7750, 7751, 7752, 7753, 7754
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7741 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7742, 7743, 7745, 7746, 7750, 7751, 7752, 7753, 7754
- **Input Features:**
