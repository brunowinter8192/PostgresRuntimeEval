...
**Test Query:** Q10_132_seed_1074728061
...
```
Node 19825 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19826 (Sort) [consumed]
    Node 19827 (Aggregate) [consumed]
      Node 19828 (Gather) [consumed]
        Node 19829 (Hash Join)
          Node 19830 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19831 (Nested Loop) [consumed]
              Node 19832 (Seq Scan) - LEAF
              Node 19833 (Index Scan) - LEAF
            Node 19834 (Hash) [consumed]
              Node 19835 (Seq Scan) - LEAF
          Node 19836 (Hash)
            Node 19837 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19825 | 19826, 19827, 19828, 19830, 19831, 19834 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19830 | 19825, 19826, 19827, 19828, 19831, 19834 |
...
|------|------|--------|-----------|-----|--------|
| 19825 | Limit | 1129.84 | 1199.47 | 6.2% | pattern |
| 19829 | Hash Join | 1082.21 | 1095.49 | 1.2% | operator |
| 19830 | Hash Join | 1079.35 | 1120.19 | 3.8% | pattern |
| 19836 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 19837 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 19832 | Seq Scan | 164.97 | 159.74 | 3.2% | operator |
| 19833 | Index Scan | 0.06 | -0.52 | 949.6% | operator |
| 19835 | Seq Scan | 28.60 | 26.64 | 6.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19832 (Seq Scan) - LEAF
...
### Step 2: Node 19833 (Index Scan) - LEAF
...
### Step 3: Node 19835 (Seq Scan) - LEAF
...
### Step 4: Node 19837 (Seq Scan) - LEAF
...
### Step 5: Node 19830 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19825, 19826, 19827, 19828, 19831, 19834
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17964
  - HashJoin_nt1=17964
...
### Step 8: Node 19825 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19826, 19827, 19828, 19830, 19831, 19834
- **Input Features:**
