...
**Test Query:** Q8_12_seed_90244341
...
```
Node 13937 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13938 (Gather Merge) [consumed]
    Node 13939 (Sort)
      Node 13940 (Hash Join)
        Node 13941 (Nested Loop)
          Node 13942 (Hash Join)
            Node 13943 (Nested Loop)
              Node 13944 (Hash Join)
                Node 13945 (Seq Scan) - LEAF
                Node 13946 (Hash) [PATTERN: 444761fb]
                  Node 13947 (Hash Join) [consumed]
                    Node 13948 (Seq Scan) [consumed] - LEAF
                    Node 13949 (Hash) [consumed]
                      Node 13950 (Hash Join) [consumed]
                        Node 13951 (Seq Scan) - LEAF
                        Node 13952 (Hash)
                          Node 13953 (Seq Scan) - LEAF
              Node 13954 (Index Scan) - LEAF
            Node 13955 (Hash)
              Node 13956 (Seq Scan) - LEAF
          Node 13957 (Index Scan) - LEAF
        Node 13958 (Hash)
          Node 13959 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13946 | 13937, 13938, 13947, 13948, 13949, 13950 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13937 | 13938, 13946, 13947, 13948, 13949, 13950 |
...
|------|------|--------|-----------|-----|--------|
| 13937 | Aggregate | 1239.85 | 1133.20 | 8.6% | pattern |
| 13939 | Sort | 1234.48 | 1046.21 | 15.3% | operator |
| 13940 | Hash Join | 1234.24 | 785.61 | 36.3% | operator |
| 13941 | Nested Loop | 1233.93 | 1198.29 | 2.9% | operator |
| 13958 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 13942 | Hash Join | 1229.47 | 785.60 | 36.1% | operator |
| 13957 | Index Scan | 0.01 | 12.42 | 177378.3% | operator |
| 13959 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13943 | Nested Loop | 1183.50 | 1198.29 | 1.2% | operator |
| 13955 | Hash | 41.50 | 19.97 | 51.9% | operator |
| 13944 | Hash Join | 217.44 | 251.67 | 15.7% | operator |
| 13954 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 13956 | Seq Scan | 41.40 | 48.88 | 18.1% | operator |
| 13945 | Seq Scan | 161.37 | 159.67 | 1.1% | operator |
| 13946 | Hash | 37.34 | 43.51 | 16.5% | pattern |
| 13951 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13952 | Hash | 1.38 | 18.14 | 1212.9% | operator |
| 13953 | Seq Scan | 1.38 | 23.36 | 1594.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13953 (Seq Scan) - LEAF
...
### Step 2: Node 13951 (Seq Scan) - LEAF
...
### Step 4: Node 13945 (Seq Scan) - LEAF
...
### Step 5: Node 13946 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13937, 13938, 13947, 13948, 13949, 13950
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13954 (Index Scan) - LEAF
...
### Step 8: Node 13956 (Seq Scan) - LEAF
...
### Step 12: Node 13957 (Index Scan) - LEAF
...
### Step 13: Node 13959 (Seq Scan) - LEAF
...
### Step 14: Node 13941 (Nested Loop)
...
### Step 18: Node 13937 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13938, 13946, 13947, 13948, 13949, 13950
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2415
