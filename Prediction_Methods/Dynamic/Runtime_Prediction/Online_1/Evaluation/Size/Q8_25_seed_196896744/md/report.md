...
**Test Query:** Q8_25_seed_196896744
...
```
Node 14742 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14743 (Gather Merge) [consumed]
    Node 14744 (Sort)
      Node 14745 (Hash Join)
        Node 14746 (Nested Loop)
          Node 14747 (Hash Join)
            Node 14748 (Nested Loop)
              Node 14749 (Hash Join)
                Node 14750 (Seq Scan) - LEAF
                Node 14751 (Hash) [PATTERN: 444761fb]
                  Node 14752 (Hash Join) [consumed]
                    Node 14753 (Seq Scan) [consumed] - LEAF
                    Node 14754 (Hash) [consumed]
                      Node 14755 (Hash Join) [consumed]
                        Node 14756 (Seq Scan) - LEAF
                        Node 14757 (Hash)
                          Node 14758 (Seq Scan) - LEAF
              Node 14759 (Index Scan) - LEAF
            Node 14760 (Hash)
              Node 14761 (Seq Scan) - LEAF
          Node 14762 (Index Scan) - LEAF
        Node 14763 (Hash)
          Node 14764 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14751 | 14742, 14743, 14752, 14753, 14754, 14755 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14742 | 14743, 14751, 14752, 14753, 14754, 14755 |
...
|------|------|--------|-----------|-----|--------|
| 14742 | Aggregate | 1232.70 | 1133.65 | 8.0% | pattern |
| 14744 | Sort | 1227.29 | 1046.21 | 14.8% | operator |
| 14745 | Hash Join | 1227.03 | 785.61 | 36.0% | operator |
| 14746 | Nested Loop | 1226.71 | 1198.29 | 2.3% | operator |
| 14763 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 14747 | Hash Join | 1222.83 | 785.60 | 35.8% | operator |
| 14762 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14764 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 14748 | Nested Loop | 1178.97 | 1198.29 | 1.6% | operator |
| 14760 | Hash | 37.00 | 19.97 | 46.0% | operator |
| 14749 | Hash Join | 213.11 | 251.67 | 18.1% | operator |
| 14759 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14761 | Seq Scan | 36.88 | 48.89 | 32.6% | operator |
| 14750 | Seq Scan | 159.13 | 159.67 | 0.3% | operator |
| 14751 | Hash | 35.09 | 43.51 | 24.0% | pattern |
| 14756 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 14757 | Hash | 0.06 | 18.14 | 28700.2% | operator |
| 14758 | Seq Scan | 0.06 | 23.36 | 38837.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14758 (Seq Scan) - LEAF
...
### Step 2: Node 14756 (Seq Scan) - LEAF
...
### Step 4: Node 14750 (Seq Scan) - LEAF
...
### Step 5: Node 14751 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14742, 14743, 14752, 14753, 14754, 14755
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14759 (Index Scan) - LEAF
...
### Step 8: Node 14761 (Seq Scan) - LEAF
...
### Step 12: Node 14762 (Index Scan) - LEAF
...
### Step 13: Node 14764 (Seq Scan) - LEAF
...
### Step 14: Node 14746 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70498.5000
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14763 (Hash)
...
- **Input Features:**
  ...
  - total_cost=70504.2500
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14744 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70540.1200
  - total_cost=70542.0000
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14742 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14743, 14751, 14752, 14753, 14754, 14755
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2329
  - Aggregate_nt1=2329
  ...
  - Aggregate_startup_cost=71540.1600
  - Aggregate_total_cost=71908.8500
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2329
  - GatherMerge_Outer_nt1=751
  ...
  - GatherMerge_Outer_startup_cost=71540.1600
  - GatherMerge_Outer_total_cost=71815.6900
- **Output:** st=1130.66, rt=1133.65
