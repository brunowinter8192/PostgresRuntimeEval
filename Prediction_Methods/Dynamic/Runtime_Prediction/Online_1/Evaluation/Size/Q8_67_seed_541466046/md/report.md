...
**Test Query:** Q8_67_seed_541466046
**Timestamp:** 2026-01-18 18:45:16
...
Node 15800 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15801 (Gather Merge) [consumed]
    Node 15802 (Sort)
      Node 15803 (Hash Join)
        Node 15804 (Nested Loop)
          Node 15805 (Hash Join)
            Node 15806 (Nested Loop)
              Node 15807 (Hash Join)
                Node 15808 (Seq Scan) - LEAF
                Node 15809 (Hash) [PATTERN: 444761fb]
                  Node 15810 (Hash Join) [consumed]
                    Node 15811 (Seq Scan) [consumed] - LEAF
                    Node 15812 (Hash) [consumed]
                      Node 15813 (Hash Join) [consumed]
                        Node 15814 (Seq Scan) - LEAF
                        Node 15815 (Hash)
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15809 | 15800, 15801, 15810, 15811, 15812, 15813 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15800 | 15801, 15809, 15810, 15811, 15812, 15813 |
...
|------|------|--------|-----------|-----|--------|
| 15800 | Aggregate | 1206.17 | 1122.26 | 7.0% | pattern |
| 15802 | Sort | 1200.23 | 1046.22 | 12.8% | operator |
| 15803 | Hash Join | 1199.97 | 785.61 | 34.5% | operator |
| 15804 | Nested Loop | 1199.66 | 1198.29 | 0.1% | operator |
| 15821 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 15805 | Hash Join | 1195.73 | 785.60 | 34.3% | operator |
| 15820 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15822 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 15806 | Nested Loop | 1150.17 | 1198.29 | 4.2% | operator |
| 15818 | Hash | 40.91 | 19.97 | 51.2% | operator |
| 15807 | Hash Join | 214.21 | 251.67 | 17.5% | operator |
| 15817 | Index Scan | 0.04 | 2.88 | 7102.1% | operator |
| 15819 | Seq Scan | 40.84 | 48.85 | 19.6% | operator |
| 15808 | Seq Scan | 158.48 | 159.67 | 0.7% | operator |
| 15809 | Hash | 36.88 | 43.51 | 18.0% | pattern |
| 15814 | Seq Scan | 0.01 | 12.22 | 111035.8% | operator |
| 15815 | Hash | 0.24 | 18.14 | 7428.7% | operator |
| 15816 | Seq Scan | 0.24 | 23.36 | 9757.5% | operator |
...
### Step 2: Node 15814 (Seq Scan) - LEAF
...
### Step 4: Node 15808 (Seq Scan) - LEAF
...
### Step 5: Node 15809 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15800, 15801, 15810, 15811, 15812, 15813
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15817 (Index Scan) - LEAF
...
### Step 8: Node 15819 (Seq Scan) - LEAF
...
- **Output:** st=1.66, rt=48.85

### Step 9: Node 15806 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9659
  - startup_cost=9920.7300
  - total_cost=70272.2000
- **Output:** st=113.62, rt=785.60

### Step 12: Node 15820 (Index Scan) - LEAF
...
### Step 13: Node 15822 (Seq Scan) - LEAF
...
### Step 14: Node 15804 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9921.0100
  - total_cost=70530.8800
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15821 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.5700
  - total_cost=70537.2000
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15802 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70578.8400
  - total_cost=70580.9800
- **Output:** st=1044.95, rt=1046.22

### Step 18: Node 15800 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15801, 15809, 15810, 15811, 15812, 15813
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2649
  ...
  - Aggregate_total_cost=71993.3800
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2649
  - GatherMerge_Outer_nt1=855
  ...
  - GatherMerge_Outer_total_cost=71892.2800
- **Output:** st=1119.37, rt=1122.26
