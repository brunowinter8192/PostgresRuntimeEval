...
**Test Query:** Q7_47_seed_377385426
...
```
Node 12139 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12140 (Gather Merge) [consumed]
    Node 12141 (Sort)
      Node 12142 (Hash Join)
        Node 12143 (Nested Loop)
          Node 12144 (Hash Join) [PATTERN: 895c6e8c]
            Node 12145 (Seq Scan) [consumed] - LEAF
            Node 12146 (Hash) [consumed]
              Node 12147 (Hash Join) [PATTERN: f4cb205a]
                Node 12148 (Seq Scan) [consumed] - LEAF
                Node 12149 (Hash) [consumed]
                  Node 12150 (Seq Scan) [consumed] - LEAF
          Node 12151 (Index Scan) - LEAF
        Node 12152 (Hash)
          Node 12153 (Hash Join) [PATTERN: f4cb205a]
            Node 12154 (Seq Scan) [consumed] - LEAF
            Node 12155 (Hash) [consumed]
              Node 12156 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12147 | 12139, 12140, 12144, 12145, 12146, 12148, 12149, 12150, 12153, 12154, 12155, 12156 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12153 | 12139, 12140, 12144, 12145, 12146, 12147, 12148, 12149, 12150, 12154, 12155, 12156 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12139 | 12140, 12144, 12145, 12146, 12147, 12148, 12149, 12150, 12153, 12154, 12155, 12156 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12144 | 12139, 12140, 12145, 12146, 12147, 12148, 12149, 12150, 12153, 12154, 12155, 12156 |
...
|------|------|--------|-----------|-----|--------|
| 12139 | Aggregate | 1202.46 | 1163.13 | 3.3% | pattern |
| 12141 | Sort | 1196.15 | 1109.16 | 7.3% | operator |
| 12142 | Hash Join | 1195.52 | 843.11 | 29.5% | operator |
| 12143 | Nested Loop | 1187.25 | 1215.99 | 2.4% | operator |
| 12152 | Hash | 3.45 | 27.74 | 703.9% | operator |
| 12144 | Hash Join | 209.83 | 304.89 | 45.3% | pattern |
| 12151 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12153 | Hash Join | 3.41 | 429.36 | 12506.1% | pattern |
| 12147 | Hash Join | 35.40 | 429.38 | 1113.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12147 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12139, 12140, 12144, 12145, 12146, 12148, 12149, 12150, 12153, 12154, 12155, 12156
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12144 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12139, 12140, 12145, 12146, 12147, 12148, 12149, 12150, 12153, 12154, 12155, 12156
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12151 (Index Scan) - LEAF
...
### Step 4: Node 12153 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12139, 12140, 12144, 12145, 12146, 12147, 12148, 12149, 12150, 12154, 12155, 12156
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12139 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12140, 12144, 12145, 12146, 12147, 12148, 12149, 12150, 12153, 12154, 12155, 12156
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
