...
**Test Query:** Q10_139_seed_1132156278
...
```
Node 19916 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19917 (Sort) [consumed]
    Node 19918 (Aggregate) [consumed]
      Node 19919 (Gather) [consumed]
        Node 19920 (Hash Join)
          Node 19921 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19922 (Nested Loop) [consumed]
              Node 19923 (Seq Scan) - LEAF
              Node 19924 (Index Scan) - LEAF
            Node 19925 (Hash) [consumed]
              Node 19926 (Seq Scan) - LEAF
          Node 19927 (Hash)
            Node 19928 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19916 | 19917, 19918, 19919, 19921, 19922, 19925 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19921 | 19916, 19917, 19918, 19919, 19922, 19925 |
...
|------|------|--------|-----------|-----|--------|
| 19916 | Limit | 1158.99 | 1199.47 | 3.5% | pattern |
| 19920 | Hash Join | 1105.69 | 1095.57 | 0.9% | operator |
| 19921 | Hash Join | 1102.78 | 1120.19 | 1.6% | pattern |
| 19927 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 19928 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 19923 | Seq Scan | 159.59 | 159.74 | 0.1% | operator |
| 19924 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 19926 | Seq Scan | 27.37 | 26.64 | 2.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19923 (Seq Scan) - LEAF
...
### Step 2: Node 19924 (Index Scan) - LEAF
...
### Step 3: Node 19926 (Seq Scan) - LEAF
...
### Step 4: Node 19928 (Seq Scan) - LEAF
...
### Step 5: Node 19921 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19916, 19917, 19918, 19919, 19922, 19925
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18240
  - HashJoin_nt1=18240
...
### Step 8: Node 19916 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19917, 19918, 19919, 19921, 19922, 19925
- **Input Features:**
