...
**Test Query:** Q10_48_seed_385589457
...
```
Node 20566 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20567 (Sort) [consumed]
    Node 20568 (Aggregate) [consumed]
      Node 20569 (Gather) [consumed]
        Node 20570 (Hash Join)
          Node 20571 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20572 (Nested Loop) [consumed]
              Node 20573 (Seq Scan) - LEAF
              Node 20574 (Index Scan) - LEAF
            Node 20575 (Hash) [consumed]
              Node 20576 (Seq Scan) - LEAF
          Node 20577 (Hash)
            Node 20578 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20566 | 20567, 20568, 20569, 20571, 20572, 20575 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20571 | 20566, 20567, 20568, 20569, 20572, 20575 |
...
|------|------|--------|-----------|-----|--------|
| 20566 | Limit | 1162.11 | 1199.47 | 3.2% | pattern |
| 20570 | Hash Join | 1109.95 | 1095.48 | 1.3% | operator |
| 20571 | Hash Join | 1107.05 | 1120.19 | 1.2% | pattern |
| 20577 | Hash | 0.02 | 15.71 | 78453.9% | operator |
| 20578 | Seq Scan | 0.01 | 6.98 | 49747.9% | operator |
| 20573 | Seq Scan | 164.40 | 159.74 | 2.8% | operator |
| 20574 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20576 | Seq Scan | 29.05 | 26.64 | 8.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20573 (Seq Scan) - LEAF
...
### Step 2: Node 20574 (Index Scan) - LEAF
...
### Step 3: Node 20576 (Seq Scan) - LEAF
...
### Step 4: Node 20578 (Seq Scan) - LEAF
...
### Step 5: Node 20571 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20566, 20567, 20568, 20569, 20572, 20575
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17901
  - HashJoin_nt1=17901
...
### Step 8: Node 20566 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20567, 20568, 20569, 20571, 20572, 20575
- **Input Features:**
