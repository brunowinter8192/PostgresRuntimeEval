...
**Test Query:** Q9_113_seed_918851472
...
```
Node 16905 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 16906 (Aggregate) [consumed]
    Node 16907 (Gather)
      Node 16908 (Aggregate)
        Node 16909 (Nested Loop)
          Node 16910 (Hash Join) [PATTERN: bb930825]
            Node 16911 (Seq Scan) [consumed] - LEAF
            Node 16912 (Hash) [consumed]
              Node 16913 (Hash Join) [consumed]
                Node 16914 (Hash Join)
                  Node 16915 (Nested Loop)
                    Node 16916 (Seq Scan) - LEAF
                    Node 16917 (Index Scan) - LEAF
                  Node 16918 (Hash)
                    Node 16919 (Seq Scan) - LEAF
                Node 16920 (Hash)
                  Node 16921 (Seq Scan) - LEAF
          Node 16922 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | bb930825 | 16910 | 16905, 16906, 16911, 16912, 16913 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 16905 | 16906, 16910, 16911, 16912, 16913 |
...
|------|------|--------|-----------|-----|--------|
| 16905 | Sort | 1192.34 | 1044.54 | 12.4% | pattern |
| 16907 | Gather | 1191.41 | 965.01 | 19.0% | operator |
| 16908 | Aggregate | 1170.71 | 1129.15 | 3.6% | operator |
| 16909 | Nested Loop | 1145.96 | 1252.17 | 9.3% | operator |
| 16910 | Hash Join | 976.29 | 134.80 | 86.2% | pattern |
| 16922 | Index Scan | 0.00 | -5.54 | 184812.7% | operator |
| 16914 | Hash Join | 166.20 | 837.52 | 403.9% | operator |
| 16920 | Hash | 13.76 | 14.95 | 8.6% | operator |
| 16915 | Nested Loop | 161.47 | 1251.76 | 675.2% | operator |
| 16918 | Hash | 2.53 | 15.94 | 530.3% | operator |
| 16921 | Seq Scan | 13.75 | 2.94 | 78.6% | operator |
| 16916 | Seq Scan | 19.18 | 44.23 | 130.6% | operator |
| 16917 | Index Scan | 0.08 | -1.39 | 1857.6% | operator |
| 16919 | Seq Scan | 2.02 | 9.45 | 367.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16916 (Seq Scan) - LEAF
...
### Step 2: Node 16917 (Index Scan) - LEAF
...
### Step 3: Node 16919 (Seq Scan) - LEAF
...
### Step 6: Node 16921 (Seq Scan) - LEAF
...
### Step 9: Node 16910 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** bb930825 (Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)])
- **Consumes:** Nodes 16905, 16906, 16911, 16912, 16913
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 16922 (Index Scan) - LEAF
...
### Step 11: Node 16909 (Nested Loop)
...
### Step 14: Node 16905 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 16906, 16910, 16911, 16912, 16913
- **Input Features:**
