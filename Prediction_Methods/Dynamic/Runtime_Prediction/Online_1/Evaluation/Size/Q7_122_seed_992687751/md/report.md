...
**Test Query:** Q7_122_seed_992687751
...
```
Node 10933 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10934 (Gather Merge) [consumed]
    Node 10935 (Sort)
      Node 10936 (Hash Join)
        Node 10937 (Nested Loop)
          Node 10938 (Hash Join) [PATTERN: 895c6e8c]
            Node 10939 (Seq Scan) [consumed] - LEAF
            Node 10940 (Hash) [consumed]
              Node 10941 (Hash Join) [PATTERN: f4cb205a]
                Node 10942 (Seq Scan) [consumed] - LEAF
                Node 10943 (Hash) [consumed]
                  Node 10944 (Seq Scan) [consumed] - LEAF
          Node 10945 (Index Scan) - LEAF
        Node 10946 (Hash)
          Node 10947 (Hash Join) [PATTERN: f4cb205a]
            Node 10948 (Seq Scan) [consumed] - LEAF
            Node 10949 (Hash) [consumed]
              Node 10950 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10941 | 10933, 10934, 10938, 10939, 10940, 10942, 10943, 10944, 10947, 10948, 10949, 10950 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10947 | 10933, 10934, 10938, 10939, 10940, 10941, 10942, 10943, 10944, 10948, 10949, 10950 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10933 | 10934, 10938, 10939, 10940, 10941, 10942, 10943, 10944, 10947, 10948, 10949, 10950 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10938 | 10933, 10934, 10939, 10940, 10941, 10942, 10943, 10944, 10947, 10948, 10949, 10950 |
...
|------|------|--------|-----------|-----|--------|
| 10933 | Aggregate | 1193.11 | 1163.13 | 2.5% | pattern |
| 10935 | Sort | 1185.88 | 1109.16 | 6.5% | operator |
| 10936 | Hash Join | 1185.22 | 843.11 | 28.9% | operator |
| 10937 | Nested Loop | 1178.89 | 1215.99 | 3.1% | operator |
| 10946 | Hash | 2.44 | 27.74 | 1036.1% | operator |
| 10938 | Hash Join | 214.12 | 304.89 | 42.4% | pattern |
| 10945 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10947 | Hash Join | 2.39 | 429.36 | 17835.0% | pattern |
| 10941 | Hash Join | 36.36 | 429.38 | 1080.9% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10941 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10933, 10934, 10938, 10939, 10940, 10942, 10943, 10944, 10947, 10948, 10949, 10950
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10938 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10933, 10934, 10939, 10940, 10941, 10942, 10943, 10944, 10947, 10948, 10949, 10950
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10945 (Index Scan) - LEAF
...
### Step 4: Node 10947 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10933, 10934, 10938, 10939, 10940, 10941, 10942, 10943, 10944, 10948, 10949, 10950
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10933 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10934, 10938, 10939, 10940, 10941, 10942, 10943, 10944, 10947, 10948, 10949, 10950
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
