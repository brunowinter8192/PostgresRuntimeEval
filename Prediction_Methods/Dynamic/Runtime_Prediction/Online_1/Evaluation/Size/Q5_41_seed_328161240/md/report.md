...
**Test Query:** Q5_41_seed_328161240
...
```
Node 8601 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8602 (Aggregate) [consumed]
    Node 8603 (Gather Merge) [consumed]
      Node 8604 (Aggregate)
        Node 8605 (Sort) [PATTERN: 3e2d5a00]
          Node 8606 (Hash Join) [consumed]
            Node 8607 (Nested Loop)
              Node 8608 (Hash Join)
                Node 8609 (Seq Scan) - LEAF
                Node 8610 (Hash) [PATTERN: 444761fb]
                  Node 8611 (Hash Join) [consumed]
                    Node 8612 (Seq Scan) [consumed] - LEAF
                    Node 8613 (Hash) [consumed]
                      Node 8614 (Hash Join) [consumed]
                        Node 8615 (Seq Scan) - LEAF
                        Node 8616 (Hash)
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8610 | 8601, 8602, 8603, 8605, 8606, 8611, 8612, 8613, 8614 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8601 | 8602, 8603, 8605, 8606, 8610, 8611, 8612, 8613, 8614 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8605 | 8601, 8602, 8603, 8606, 8610, 8611, 8612, 8613, 8614 |
...
|------|------|--------|-----------|-----|--------|
| 8601 | Sort | 1062.28 | 2934.42 | 176.2% | pattern |
| 8604 | Aggregate | 1058.18 | 1128.79 | 6.7% | operator |
| 8605 | Sort | 1057.91 | 1835.90 | 73.5% | pattern |
| 8607 | Nested Loop | 1045.67 | 1188.37 | 13.6% | operator |
| 8619 | Hash | 3.61 | 18.67 | 417.4% | operator |
| 8608 | Hash Join | 218.86 | 253.07 | 15.6% | operator |
| 8618 | Index Scan | 0.07 | -0.22 | 405.6% | operator |
| 8620 | Seq Scan | 3.10 | 12.36 | 299.3% | operator |
| 8609 | Seq Scan | 167.37 | 159.81 | 4.5% | operator |
| 8610 | Hash | 38.29 | 46.85 | 22.4% | pattern |
| 8615 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 8616 | Hash | 0.07 | 21.05 | 32289.6% | operator |
| 8617 | Seq Scan | 0.06 | 22.93 | 37495.6% | operator |
...
### Step 2: Node 8615 (Seq Scan) - LEAF
...
### Step 4: Node 8609 (Seq Scan) - LEAF
...
### Step 5: Node 8610 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8601, 8602, 8603, 8605, 8606, 8611, 8612, 8613, 8614
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8618 (Index Scan) - LEAF
...
### Step 8: Node 8620 (Seq Scan) - LEAF
...
### Step 11: Node 8605 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8601, 8602, 8603, 8606, 8610, 8611, 8612, 8613, 8614
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8601 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8602, 8603, 8605, 8606, 8610, 8611, 8612, 8613, 8614
- **Input Features:**
