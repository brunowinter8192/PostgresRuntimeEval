...
**Test Query:** Q10_138_seed_1123952247
...
```
Node 19903 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19904 (Sort) [consumed]
    Node 19905 (Aggregate) [consumed]
      Node 19906 (Gather) [consumed]
        Node 19907 (Hash Join)
          Node 19908 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19909 (Nested Loop) [consumed]
              Node 19910 (Seq Scan) - LEAF
              Node 19911 (Index Scan) - LEAF
            Node 19912 (Hash) [consumed]
              Node 19913 (Seq Scan) - LEAF
          Node 19914 (Hash)
            Node 19915 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19903 | 19904, 19905, 19906, 19908, 19909, 19912 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19908 | 19903, 19904, 19905, 19906, 19909, 19912 |
...
|------|------|--------|-----------|-----|--------|
| 19903 | Limit | 1180.72 | 1199.47 | 1.6% | pattern |
| 19907 | Hash Join | 1132.13 | 1095.51 | 3.2% | operator |
| 19908 | Hash Join | 1129.22 | 1120.19 | 0.8% | pattern |
| 19914 | Hash | 0.01 | 15.71 | 142725.2% | operator |
| 19915 | Seq Scan | 0.01 | 6.98 | 87133.8% | operator |
| 19910 | Seq Scan | 158.41 | 159.74 | 0.8% | operator |
| 19911 | Index Scan | 0.06 | -0.52 | 909.7% | operator |
| 19913 | Seq Scan | 25.36 | 26.64 | 5.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19910 (Seq Scan) - LEAF
...
### Step 2: Node 19911 (Index Scan) - LEAF
...
### Step 3: Node 19913 (Seq Scan) - LEAF
...
### Step 4: Node 19915 (Seq Scan) - LEAF
...
### Step 5: Node 19908 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19903, 19904, 19905, 19906, 19909, 19912
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18015
  - HashJoin_nt1=18015
...
### Step 8: Node 19903 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19904, 19905, 19906, 19908, 19909, 19912
- **Input Features:**
