...
**Test Query:** Q8_34_seed_270733023
...
```
Node 14972 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14973 (Gather Merge) [consumed]
    Node 14974 (Sort)
      Node 14975 (Hash Join)
        Node 14976 (Nested Loop)
          Node 14977 (Hash Join)
            Node 14978 (Nested Loop)
              Node 14979 (Hash Join)
                Node 14980 (Seq Scan) - LEAF
                Node 14981 (Hash) [PATTERN: 444761fb]
                  Node 14982 (Hash Join) [consumed]
                    Node 14983 (Seq Scan) [consumed] - LEAF
                    Node 14984 (Hash) [consumed]
                      Node 14985 (Hash Join) [consumed]
                        Node 14986 (Seq Scan) - LEAF
                        Node 14987 (Hash)
                          Node 14988 (Seq Scan) - LEAF
              Node 14989 (Index Scan) - LEAF
            Node 14990 (Hash)
              Node 14991 (Seq Scan) - LEAF
          Node 14992 (Index Scan) - LEAF
        Node 14993 (Hash)
          Node 14994 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14981 | 14972, 14973, 14982, 14983, 14984, 14985 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14972 | 14973, 14981, 14982, 14983, 14984, 14985 |
...
|------|------|--------|-----------|-----|--------|
| 14972 | Aggregate | 1180.21 | 1132.74 | 4.0% | pattern |
| 14974 | Sort | 1173.09 | 1046.21 | 10.8% | operator |
| 14975 | Hash Join | 1172.85 | 785.61 | 33.0% | operator |
| 14976 | Nested Loop | 1172.55 | 1198.29 | 2.2% | operator |
| 14993 | Hash | 0.02 | 13.72 | 72132.0% | operator |
| 14977 | Hash Join | 1168.89 | 785.60 | 32.8% | operator |
| 14992 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14994 | Seq Scan | 0.01 | 6.51 | 46414.0% | operator |
| 14978 | Nested Loop | 1130.54 | 1198.29 | 6.0% | operator |
| 14990 | Hash | 34.04 | 19.97 | 41.3% | operator |
| 14979 | Hash Join | 207.29 | 251.67 | 21.4% | operator |
| 14989 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14991 | Seq Scan | 33.98 | 48.87 | 43.8% | operator |
| 14980 | Seq Scan | 155.73 | 159.67 | 2.5% | operator |
| 14981 | Hash | 34.02 | 43.51 | 27.9% | pattern |
| 14986 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14987 | Hash | 0.15 | 18.14 | 12159.6% | operator |
| 14988 | Seq Scan | 0.14 | 23.36 | 16237.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14988 (Seq Scan) - LEAF
...
### Step 2: Node 14986 (Seq Scan) - LEAF
...
### Step 4: Node 14980 (Seq Scan) - LEAF
...
### Step 5: Node 14981 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14972, 14973, 14982, 14983, 14984, 14985
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14989 (Index Scan) - LEAF
...
### Step 8: Node 14991 (Seq Scan) - LEAF
...
### Step 12: Node 14992 (Index Scan) - LEAF
...
### Step 13: Node 14994 (Seq Scan) - LEAF
...
### Step 14: Node 14976 (Nested Loop)
...
### Step 18: Node 14972 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14973, 14981, 14982, 14983, 14984, 14985
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2424
