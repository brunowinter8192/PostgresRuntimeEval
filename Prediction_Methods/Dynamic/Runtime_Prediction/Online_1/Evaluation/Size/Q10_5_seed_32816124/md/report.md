...
**Test Query:** Q10_5_seed_32816124
...
```
Node 20735 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20736 (Sort) [consumed]
    Node 20737 (Aggregate) [consumed]
      Node 20738 (Gather) [consumed]
        Node 20739 (Hash Join)
          Node 20740 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20741 (Nested Loop) [consumed]
              Node 20742 (Seq Scan) - LEAF
              Node 20743 (Index Scan) - LEAF
            Node 20744 (Hash) [consumed]
              Node 20745 (Seq Scan) - LEAF
          Node 20746 (Hash)
            Node 20747 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20735 | 20736, 20737, 20738, 20740, 20741, 20744 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20740 | 20735, 20736, 20737, 20738, 20741, 20744 |
...
|------|------|--------|-----------|-----|--------|
| 20735 | Limit | 1176.18 | 1199.47 | 2.0% | pattern |
| 20739 | Hash Join | 1130.46 | 1095.51 | 3.1% | operator |
| 20740 | Hash Join | 1127.59 | 1120.19 | 0.7% | pattern |
| 20746 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20747 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20742 | Seq Scan | 165.38 | 159.74 | 3.4% | operator |
| 20743 | Index Scan | 0.06 | -0.52 | 909.7% | operator |
| 20745 | Seq Scan | 26.09 | 26.64 | 2.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20742 (Seq Scan) - LEAF
...
### Step 2: Node 20743 (Index Scan) - LEAF
...
### Step 3: Node 20745 (Seq Scan) - LEAF
...
### Step 4: Node 20747 (Seq Scan) - LEAF
...
### Step 5: Node 20740 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20735, 20736, 20737, 20738, 20741, 20744
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18015
  - HashJoin_nt1=18015
...
### Step 8: Node 20735 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20736, 20737, 20738, 20740, 20741, 20744
- **Input Features:**
