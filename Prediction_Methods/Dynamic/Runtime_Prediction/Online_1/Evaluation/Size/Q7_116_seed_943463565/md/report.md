        ...
        Node 10811 (Nested Loop)
          Node 10812 (Hash Join) [PATTERN: 895c6e8c]
            ...
            Node 10814 (Hash) [consumed]
              Node 10815 (Hash Join) [PATTERN: f4cb205a]
                Node 10816 (Seq Scan) [consumed] - LEAF
                Node 10817 (Hash) [consumed]
                  Node 10818 (Seq Scan) [consumed] - LEAF
          Node 10819 (Index Scan) - LEAF
        Node 10820 (Hash)
          Node 10821 (Hash Join) [PATTERN: f4cb205a]
            Node 10822 (Seq Scan) [consumed] - LEAF
            Node 10823 (Hash) [consumed]
              Node 10824 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10815 | 10807, 10808, 10812, 10813, 10814, 10816, 10817, 10818, 10821, 10822, 10823, 10824 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10821 | 10807, 10808, 10812, 10813, 10814, 10815, 10816, 10817, 10818, 10822, 10823, 10824 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10807 | 10808, 10812, 10813, 10814, 10815, 10816, 10817, 10818, 10821, 10822, 10823, 10824 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10812 | 10807, 10808, 10813, 10814, 10815, 10816, 10817, 10818, 10821, 10822, 10823, 10824 |
...
|------|------|--------|-----------|-----|--------|
| 10807 | Aggregate | 1199.38 | 1163.13 | 3.0% | pattern |
| 10809 | Sort | 1192.59 | 1109.16 | 7.0% | operator |
| 10810 | Hash Join | 1191.96 | 843.11 | 29.3% | operator |
| 10811 | Nested Loop | 1185.04 | 1215.99 | 2.6% | operator |
| 10820 | Hash | 3.16 | 27.74 | 778.0% | operator |
| 10812 | Hash Join | 215.56 | 304.89 | 41.4% | pattern |
| 10819 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10821 | Hash Join | 3.12 | 429.36 | 13683.7% | pattern |
| 10815 | Hash Join | 36.71 | 429.38 | 1069.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10815 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10807, 10808, 10812, 10813, 10814, 10816, 10817, 10818, 10821, 10822, 10823, 10824
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10812 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10807, 10808, 10813, 10814, 10815, 10816, 10817, 10818, 10821, 10822, 10823, 10824
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10819 (Index Scan) - LEAF
...
### Step 4: Node 10821 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10807, 10808, 10812, 10813, 10814, 10815, 10816, 10817, 10818, 10822, 10823, 10824
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10807 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10808, 10812, 10813, 10814, 10815, 10816, 10817, 10818, 10821, 10822, 10823, 10824
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
