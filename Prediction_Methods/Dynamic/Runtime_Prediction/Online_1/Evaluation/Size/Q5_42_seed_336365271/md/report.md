...
**Test Query:** Q5_42_seed_336365271
...
```
Node 8621 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8622 (Aggregate) [consumed]
    Node 8623 (Gather Merge) [consumed]
      Node 8624 (Aggregate)
        Node 8625 (Sort) [PATTERN: 3e2d5a00]
          Node 8626 (Hash Join) [consumed]
            Node 8627 (Nested Loop)
              Node 8628 (Hash Join)
                Node 8629 (Seq Scan) - LEAF
                Node 8630 (Hash) [PATTERN: 444761fb]
                  Node 8631 (Hash Join) [consumed]
                    Node 8632 (Seq Scan) [consumed] - LEAF
                    Node 8633 (Hash) [consumed]
                      Node 8634 (Hash Join) [consumed]
                        Node 8635 (Seq Scan) - LEAF
                        Node 8636 (Hash)
                          Node 8637 (Seq Scan) - LEAF
              Node 8638 (Index Scan) - LEAF
            Node 8639 (Hash)
              Node 8640 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8630 | 8621, 8622, 8623, 8625, 8626, 8631, 8632, 8633, 8634 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8621 | 8622, 8623, 8625, 8626, 8630, 8631, 8632, 8633, 8634 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8625 | 8621, 8622, 8623, 8626, 8630, 8631, 8632, 8633, 8634 |
...
|------|------|--------|-----------|-----|--------|
| 8621 | Sort | 1062.94 | 2934.42 | 176.1% | pattern |
| 8624 | Aggregate | 1058.07 | 1128.78 | 6.7% | operator |
| 8625 | Sort | 1057.82 | 1835.92 | 73.6% | pattern |
| 8627 | Nested Loop | 1044.83 | 1188.42 | 13.7% | operator |
| 8639 | Hash | 4.39 | 18.67 | 324.8% | operator |
| 8628 | Hash Join | 214.27 | 253.07 | 18.1% | operator |
| 8638 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 8640 | Seq Scan | 3.82 | 12.36 | 223.9% | operator |
| 8629 | Seq Scan | 165.73 | 159.81 | 3.6% | operator |
| 8630 | Hash | 35.90 | 46.85 | 30.5% | pattern |
| 8635 | Seq Scan | 0.01 | 11.81 | 90732.9% | operator |
| 8636 | Hash | 0.06 | 21.05 | 38178.6% | operator |
| 8637 | Seq Scan | 0.05 | 22.93 | 44867.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8637 (Seq Scan) - LEAF
...
### Step 2: Node 8635 (Seq Scan) - LEAF
...
### Step 4: Node 8629 (Seq Scan) - LEAF
...
### Step 5: Node 8630 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8621, 8622, 8623, 8625, 8626, 8631, 8632, 8633, 8634
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8638 (Index Scan) - LEAF
...
### Step 8: Node 8640 (Seq Scan) - LEAF
...
### Step 11: Node 8625 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8621, 8622, 8623, 8626, 8630, 8631, 8632, 8633, 8634
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8621 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8622, 8623, 8625, 8626, 8630, 8631, 8632, 8633, 8634
- **Input Features:**
