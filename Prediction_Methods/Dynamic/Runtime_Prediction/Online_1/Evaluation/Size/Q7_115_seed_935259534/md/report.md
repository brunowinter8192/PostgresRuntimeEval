...
**Test Query:** Q7_115_seed_935259534
...
```
Node 10789 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10790 (Gather Merge) [consumed]
    Node 10791 (Sort)
      Node 10792 (Hash Join)
        Node 10793 (Nested Loop)
          Node 10794 (Hash Join) [PATTERN: 895c6e8c]
            Node 10795 (Seq Scan) [consumed] - LEAF
            Node 10796 (Hash) [consumed]
              Node 10797 (Hash Join) [PATTERN: f4cb205a]
                Node 10798 (Seq Scan) [consumed] - LEAF
                Node 10799 (Hash) [consumed]
                  Node 10800 (Seq Scan) [consumed] - LEAF
          Node 10801 (Index Scan) - LEAF
        Node 10802 (Hash)
          Node 10803 (Hash Join) [PATTERN: f4cb205a]
            Node 10804 (Seq Scan) [consumed] - LEAF
            Node 10805 (Hash) [consumed]
              Node 10806 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10797 | 10789, 10790, 10794, 10795, 10796, 10798, 10799, 10800, 10803, 10804, 10805, 10806 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10803 | 10789, 10790, 10794, 10795, 10796, 10797, 10798, 10799, 10800, 10804, 10805, 10806 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10789 | 10790, 10794, 10795, 10796, 10797, 10798, 10799, 10800, 10803, 10804, 10805, 10806 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10794 | 10789, 10790, 10795, 10796, 10797, 10798, 10799, 10800, 10803, 10804, 10805, 10806 |
...
|------|------|--------|-----------|-----|--------|
| 10789 | Aggregate | 1198.13 | 1163.13 | 2.9% | pattern |
| 10791 | Sort | 1191.21 | 1109.16 | 6.9% | operator |
| 10792 | Hash Join | 1190.58 | 843.11 | 29.2% | operator |
| 10793 | Nested Loop | 1181.95 | 1215.99 | 2.9% | operator |
| 10802 | Hash | 4.82 | 27.74 | 475.7% | operator |
| 10794 | Hash Join | 213.55 | 304.89 | 42.8% | pattern |
| 10801 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10803 | Hash Join | 4.77 | 429.36 | 8901.3% | pattern |
| 10797 | Hash Join | 34.37 | 429.38 | 1149.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10797 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10789, 10790, 10794, 10795, 10796, 10798, 10799, 10800, 10803, 10804, 10805, 10806
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10794 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10789, 10790, 10795, 10796, 10797, 10798, 10799, 10800, 10803, 10804, 10805, 10806
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10801 (Index Scan) - LEAF
...
### Step 4: Node 10803 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10789, 10790, 10794, 10795, 10796, 10797, 10798, 10799, 10800, 10804, 10805, 10806
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10789 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10790, 10794, 10795, 10796, 10797, 10798, 10799, 10800, 10803, 10804, 10805, 10806
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
