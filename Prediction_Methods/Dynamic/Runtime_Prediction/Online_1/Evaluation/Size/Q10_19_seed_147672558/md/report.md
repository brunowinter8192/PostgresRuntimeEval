...
**Test Query:** Q10_19_seed_147672558
...
```
Node 20150 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20151 (Sort) [consumed]
    Node 20152 (Aggregate) [consumed]
      Node 20153 (Gather) [consumed]
        Node 20154 (Hash Join)
          Node 20155 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20156 (Nested Loop) [consumed]
              Node 20157 (Seq Scan) - LEAF
              Node 20158 (Index Scan) - LEAF
            Node 20159 (Hash) [consumed]
              Node 20160 (Seq Scan) - LEAF
          Node 20161 (Hash)
            Node 20162 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20150 | 20151, 20152, 20153, 20155, 20156, 20159 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20155 | 20150, 20151, 20152, 20153, 20156, 20159 |
...
|------|------|--------|-----------|-----|--------|
| 20150 | Limit | 1146.76 | 1199.47 | 4.6% | pattern |
| 20154 | Hash Join | 1096.32 | 1095.46 | 0.1% | operator |
| 20155 | Hash Join | 1093.43 | 1120.19 | 2.4% | pattern |
| 20161 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 20162 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 20157 | Seq Scan | 161.73 | 159.74 | 1.2% | operator |
| 20158 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20160 | Seq Scan | 25.53 | 26.64 | 4.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20157 (Seq Scan) - LEAF
...
### Step 2: Node 20158 (Index Scan) - LEAF
...
### Step 3: Node 20160 (Seq Scan) - LEAF
...
### Step 4: Node 20162 (Seq Scan) - LEAF
...
### Step 5: Node 20155 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20150, 20151, 20152, 20153, 20156, 20159
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17827
  - HashJoin_nt1=17827
...
### Step 8: Node 20150 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20151, 20152, 20153, 20155, 20156, 20159
- **Input Features:**
