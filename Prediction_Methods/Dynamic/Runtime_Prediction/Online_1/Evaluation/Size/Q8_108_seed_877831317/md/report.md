...
**Test Query:** Q8_108_seed_877831317
...
```
Node 13385 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13386 (Gather Merge) [consumed]
    Node 13387 (Sort)
      Node 13388 (Hash Join)
        Node 13389 (Nested Loop)
          Node 13390 (Hash Join)
            Node 13391 (Nested Loop)
              Node 13392 (Hash Join)
                Node 13393 (Seq Scan) - LEAF
                Node 13394 (Hash) [PATTERN: 444761fb]
                  Node 13395 (Hash Join) [consumed]
                    Node 13396 (Seq Scan) [consumed] - LEAF
                    Node 13397 (Hash) [consumed]
                      Node 13398 (Hash Join) [consumed]
                        Node 13399 (Seq Scan) - LEAF
                        Node 13400 (Hash)
                          Node 13401 (Seq Scan) - LEAF
              Node 13402 (Index Scan) - LEAF
            Node 13403 (Hash)
              Node 13404 (Seq Scan) - LEAF
          Node 13405 (Index Scan) - LEAF
        Node 13406 (Hash)
          Node 13407 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13394 | 13385, 13386, 13395, 13396, 13397, 13398 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13385 | 13386, 13394, 13395, 13396, 13397, 13398 |
...
|------|------|--------|-----------|-----|--------|
| 13385 | Aggregate | 1239.03 | 1132.51 | 8.6% | pattern |
| 13387 | Sort | 1232.89 | 1046.21 | 15.1% | operator |
| 13388 | Hash Join | 1232.64 | 785.61 | 36.3% | operator |
| 13389 | Nested Loop | 1232.34 | 1198.29 | 2.8% | operator |
| 13406 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 13390 | Hash Join | 1228.68 | 785.60 | 36.1% | operator |
| 13405 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13407 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13391 | Nested Loop | 1182.76 | 1198.29 | 1.3% | operator |
| 13403 | Hash | 41.55 | 19.97 | 51.9% | operator |
| 13392 | Hash Join | 219.29 | 251.67 | 14.8% | operator |
| 13402 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 13404 | Seq Scan | 41.44 | 48.87 | 18.0% | operator |
| 13393 | Seq Scan | 161.94 | 159.67 | 1.4% | operator |
| 13394 | Hash | 39.40 | 43.51 | 10.4% | pattern |
| 13399 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13400 | Hash | 0.39 | 18.14 | 4612.8% | operator |
| 13401 | Seq Scan | 0.38 | 23.36 | 6031.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13401 (Seq Scan) - LEAF
...
### Step 2: Node 13399 (Seq Scan) - LEAF
...
### Step 4: Node 13393 (Seq Scan) - LEAF
...
### Step 5: Node 13394 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13385, 13386, 13395, 13396, 13397, 13398
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13402 (Index Scan) - LEAF
...
### Step 8: Node 13404 (Seq Scan) - LEAF
...
### Step 12: Node 13405 (Index Scan) - LEAF
...
### Step 13: Node 13407 (Seq Scan) - LEAF
...
### Step 14: Node 13389 (Nested Loop)
...
### Step 18: Node 13385 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13386, 13394, 13395, 13396, 13397, 13398
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2429
