...
**Test Query:** Q10_16_seed_123060465
...
```
Node 20111 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20112 (Sort) [consumed]
    Node 20113 (Aggregate) [consumed]
      Node 20114 (Gather) [consumed]
        Node 20115 (Hash Join)
          Node 20116 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20117 (Nested Loop) [consumed]
              Node 20118 (Seq Scan) - LEAF
              Node 20119 (Index Scan) - LEAF
            Node 20120 (Hash) [consumed]
              Node 20121 (Seq Scan) - LEAF
          Node 20122 (Hash)
            Node 20123 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20111 | 20112, 20113, 20114, 20116, 20117, 20120 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20116 | 20111, 20112, 20113, 20114, 20117, 20120 |
...
|------|------|--------|-----------|-----|--------|
| 20111 | Limit | 1141.09 | 1199.47 | 5.1% | pattern |
| 20115 | Hash Join | 1099.74 | 1095.39 | 0.4% | operator |
| 20116 | Hash Join | 1097.21 | 1120.19 | 2.1% | pattern |
| 20122 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20123 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20118 | Seq Scan | 165.90 | 159.75 | 3.7% | operator |
| 20119 | Index Scan | 0.06 | -0.52 | 935.8% | operator |
| 20121 | Seq Scan | 28.00 | 26.64 | 4.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20118 (Seq Scan) - LEAF
...
### Step 2: Node 20119 (Index Scan) - LEAF
...
### Step 3: Node 20121 (Seq Scan) - LEAF
...
### Step 4: Node 20123 (Seq Scan) - LEAF
...
### Step 5: Node 20116 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20111, 20112, 20113, 20114, 20117, 20120
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17592
  - HashJoin_nt1=17592
...
### Step 8: Node 20111 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20112, 20113, 20114, 20116, 20117, 20120
- **Input Features:**
