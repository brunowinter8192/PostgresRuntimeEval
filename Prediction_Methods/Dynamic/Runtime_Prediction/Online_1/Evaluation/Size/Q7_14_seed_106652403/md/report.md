...
**Test Query:** Q7_14_seed_106652403
...
```
Node 11473 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11474 (Gather Merge) [consumed]
    Node 11475 (Sort)
      Node 11476 (Hash Join)
        Node 11477 (Nested Loop)
          Node 11478 (Hash Join) [PATTERN: 895c6e8c]
            Node 11479 (Seq Scan) [consumed] - LEAF
            Node 11480 (Hash) [consumed]
              Node 11481 (Hash Join) [PATTERN: f4cb205a]
                Node 11482 (Seq Scan) [consumed] - LEAF
                Node 11483 (Hash) [consumed]
                  Node 11484 (Seq Scan) [consumed] - LEAF
          Node 11485 (Index Scan) - LEAF
        Node 11486 (Hash)
          Node 11487 (Hash Join) [PATTERN: f4cb205a]
            Node 11488 (Seq Scan) [consumed] - LEAF
            Node 11489 (Hash) [consumed]
              Node 11490 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11481 | 11473, 11474, 11478, 11479, 11480, 11482, 11483, 11484, 11487, 11488, 11489, 11490 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11487 | 11473, 11474, 11478, 11479, 11480, 11481, 11482, 11483, 11484, 11488, 11489, 11490 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11473 | 11474, 11478, 11479, 11480, 11481, 11482, 11483, 11484, 11487, 11488, 11489, 11490 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11478 | 11473, 11474, 11479, 11480, 11481, 11482, 11483, 11484, 11487, 11488, 11489, 11490 |
...
|------|------|--------|-----------|-----|--------|
| 11473 | Aggregate | 1193.52 | 1163.13 | 2.5% | pattern |
| 11475 | Sort | 1185.90 | 1109.16 | 6.5% | operator |
| 11476 | Hash Join | 1185.25 | 843.11 | 28.9% | operator |
| 11477 | Nested Loop | 1178.17 | 1215.99 | 3.2% | operator |
| 11486 | Hash | 2.39 | 27.74 | 1061.3% | operator |
| 11478 | Hash Join | 214.37 | 304.89 | 42.2% | pattern |
| 11485 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11487 | Hash Join | 2.34 | 429.36 | 18264.5% | pattern |
| 11481 | Hash Join | 36.90 | 429.38 | 1063.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11481 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11473, 11474, 11478, 11479, 11480, 11482, 11483, 11484, 11487, 11488, 11489, 11490
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11478 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11473, 11474, 11479, 11480, 11481, 11482, 11483, 11484, 11487, 11488, 11489, 11490
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11485 (Index Scan) - LEAF
...
### Step 4: Node 11487 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11473, 11474, 11478, 11479, 11480, 11481, 11482, 11483, 11484, 11488, 11489, 11490
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11473 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11474, 11478, 11479, 11480, 11481, 11482, 11483, 11484, 11487, 11488, 11489, 11490
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
