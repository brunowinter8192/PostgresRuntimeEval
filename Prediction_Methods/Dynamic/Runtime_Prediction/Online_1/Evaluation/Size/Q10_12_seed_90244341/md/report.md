...
**Test Query:** Q10_12_seed_90244341
...
```
Node 19786 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19787 (Sort) [consumed]
    Node 19788 (Aggregate) [consumed]
      Node 19789 (Gather) [consumed]
        Node 19790 (Hash Join)
          Node 19791 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19792 (Nested Loop) [consumed]
              Node 19793 (Seq Scan) - LEAF
              Node 19794 (Index Scan) - LEAF
            Node 19795 (Hash) [consumed]
              Node 19796 (Seq Scan) - LEAF
          Node 19797 (Hash)
            Node 19798 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19786 | 19787, 19788, 19789, 19791, 19792, 19795 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19791 | 19786, 19787, 19788, 19789, 19792, 19795 |
...
|------|------|--------|-----------|-----|--------|
| 19786 | Limit | 1159.52 | 1199.47 | 3.4% | pattern |
| 19790 | Hash Join | 1108.27 | 1095.55 | 1.1% | operator |
| 19791 | Hash Join | 1105.39 | 1120.19 | 1.3% | pattern |
| 19797 | Hash | 0.01 | 15.71 | 120752.1% | operator |
| 19798 | Seq Scan | 0.01 | 6.98 | 77441.2% | operator |
| 19793 | Seq Scan | 163.29 | 159.74 | 2.2% | operator |
| 19794 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 19796 | Seq Scan | 27.52 | 26.64 | 3.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19793 (Seq Scan) - LEAF
...
### Step 2: Node 19794 (Index Scan) - LEAF
...
### Step 3: Node 19796 (Seq Scan) - LEAF
...
### Step 4: Node 19798 (Seq Scan) - LEAF
...
### Step 5: Node 19791 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19786, 19787, 19788, 19789, 19792, 19795
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18184
  - HashJoin_nt1=18184
...
### Step 8: Node 19786 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19787, 19788, 19789, 19791, 19792, 19795
- **Input Features:**
