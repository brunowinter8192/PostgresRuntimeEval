...
**Test Query:** Q10_67_seed_541466046
...
```
Node 20839 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20840 (Sort) [consumed]
    Node 20841 (Aggregate) [consumed]
      Node 20842 (Gather) [consumed]
        Node 20843 (Hash Join)
          Node 20844 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20845 (Nested Loop) [consumed]
              Node 20846 (Seq Scan) - LEAF
              Node 20847 (Index Scan) - LEAF
            Node 20848 (Hash) [consumed]
              Node 20849 (Seq Scan) - LEAF
          Node 20850 (Hash)
            Node 20851 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20839 | 20840, 20841, 20842, 20844, 20845, 20848 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20844 | 20839, 20840, 20841, 20842, 20845, 20848 |
...
|------|------|--------|-----------|-----|--------|
| 20839 | Limit | 1144.89 | 1199.47 | 4.8% | pattern |
| 20843 | Hash Join | 1095.36 | 1095.47 | 0.0% | operator |
| 20844 | Hash Join | 1092.35 | 1120.19 | 2.5% | pattern |
| 20850 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 20851 | Seq Scan | 0.01 | 6.98 | 53582.3% | operator |
| 20846 | Seq Scan | 163.99 | 159.74 | 2.6% | operator |
| 20847 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20849 | Seq Scan | 26.14 | 26.64 | 1.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20846 (Seq Scan) - LEAF
...
### Step 2: Node 20847 (Index Scan) - LEAF
...
### Step 3: Node 20849 (Seq Scan) - LEAF
...
### Step 4: Node 20851 (Seq Scan) - LEAF
...
### Step 5: Node 20844 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20839, 20840, 20841, 20842, 20845, 20848
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17881
  - HashJoin_nt1=17881
...
### Step 8: Node 20839 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20840, 20841, 20842, 20844, 20845, 20848
- **Input Features:**
