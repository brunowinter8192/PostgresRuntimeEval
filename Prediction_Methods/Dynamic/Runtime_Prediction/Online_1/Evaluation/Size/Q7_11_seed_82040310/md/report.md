...
**Test Query:** Q7_11_seed_82040310
...
```
Node 10879 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10880 (Gather Merge) [consumed]
    Node 10881 (Sort)
      Node 10882 (Hash Join)
        Node 10883 (Nested Loop)
          Node 10884 (Hash Join) [PATTERN: 895c6e8c]
            Node 10885 (Seq Scan) [consumed] - LEAF
            Node 10886 (Hash) [consumed]
              Node 10887 (Hash Join) [PATTERN: f4cb205a]
                Node 10888 (Seq Scan) [consumed] - LEAF
                Node 10889 (Hash) [consumed]
                  Node 10890 (Seq Scan) [consumed] - LEAF
          Node 10891 (Index Scan) - LEAF
        Node 10892 (Hash)
          Node 10893 (Hash Join) [PATTERN: f4cb205a]
            Node 10894 (Seq Scan) [consumed] - LEAF
            Node 10895 (Hash) [consumed]
              Node 10896 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10887 | 10879, 10880, 10884, 10885, 10886, 10888, 10889, 10890, 10893, 10894, 10895, 10896 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10893 | 10879, 10880, 10884, 10885, 10886, 10887, 10888, 10889, 10890, 10894, 10895, 10896 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10879 | 10880, 10884, 10885, 10886, 10887, 10888, 10889, 10890, 10893, 10894, 10895, 10896 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10884 | 10879, 10880, 10885, 10886, 10887, 10888, 10889, 10890, 10893, 10894, 10895, 10896 |
...
|------|------|--------|-----------|-----|--------|
| 10879 | Aggregate | 1217.51 | 1163.13 | 4.5% | pattern |
| 10881 | Sort | 1210.73 | 1109.16 | 8.4% | operator |
| 10882 | Hash Join | 1210.09 | 843.11 | 30.3% | operator |
| 10883 | Nested Loop | 1202.29 | 1215.99 | 1.1% | operator |
| 10892 | Hash | 3.85 | 27.74 | 620.8% | operator |
| 10884 | Hash Join | 212.53 | 304.89 | 43.5% | pattern |
| 10891 | Index Scan | 0.03 | 0.96 | 2731.9% | operator |
| 10893 | Hash Join | 3.80 | 429.36 | 11196.0% | pattern |
| 10887 | Hash Join | 34.65 | 429.38 | 1139.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10887 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10879, 10880, 10884, 10885, 10886, 10888, 10889, 10890, 10893, 10894, 10895, 10896
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10884 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10879, 10880, 10885, 10886, 10887, 10888, 10889, 10890, 10893, 10894, 10895, 10896
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10891 (Index Scan) - LEAF
...
### Step 4: Node 10893 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10879, 10880, 10884, 10885, 10886, 10887, 10888, 10889, 10890, 10894, 10895, 10896
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10879 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10880, 10884, 10885, 10886, 10887, 10888, 10889, 10890, 10893, 10894, 10895, 10896
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
