...
**Test Query:** Q5_61_seed_492241860
...
```
Node 9041 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9042 (Aggregate) [consumed]
    Node 9043 (Gather Merge) [consumed]
      Node 9044 (Aggregate)
        Node 9045 (Sort) [PATTERN: 3e2d5a00]
          Node 9046 (Hash Join) [consumed]
            Node 9047 (Nested Loop)
              Node 9048 (Hash Join)
                Node 9049 (Seq Scan) - LEAF
                Node 9050 (Hash) [PATTERN: 444761fb]
                  Node 9051 (Hash Join) [consumed]
                    Node 9052 (Seq Scan) [consumed] - LEAF
                    Node 9053 (Hash) [consumed]
                      Node 9054 (Hash Join) [consumed]
                        Node 9055 (Seq Scan) - LEAF
                        Node 9056 (Hash)
                          Node 9057 (Seq Scan) - LEAF
              Node 9058 (Index Scan) - LEAF
            Node 9059 (Hash)
              Node 9060 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9050 | 9041, 9042, 9043, 9045, 9046, 9051, 9052, 9053, 9054 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9041 | 9042, 9043, 9045, 9046, 9050, 9051, 9052, 9053, 9054 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9045 | 9041, 9042, 9043, 9046, 9050, 9051, 9052, 9053, 9054 |
...
|------|------|--------|-----------|-----|--------|
| 9041 | Sort | 1049.28 | 2934.42 | 179.7% | pattern |
| 9044 | Aggregate | 1043.89 | 1128.78 | 8.1% | operator |
| 9045 | Sort | 1043.65 | 1835.91 | 75.9% | pattern |
| 9047 | Nested Loop | 1030.35 | 1188.39 | 15.3% | operator |
| 9059 | Hash | 4.64 | 18.67 | 302.4% | operator |
| 9048 | Hash Join | 219.78 | 253.07 | 15.1% | operator |
| 9058 | Index Scan | 0.07 | -0.22 | 405.6% | operator |
| 9060 | Seq Scan | 4.11 | 12.36 | 200.4% | operator |
| 9049 | Seq Scan | 172.46 | 159.81 | 7.3% | operator |
| 9050 | Hash | 33.50 | 46.85 | 39.8% | pattern |
| 9055 | Seq Scan | 0.01 | 11.81 | 98302.3% | operator |
| 9056 | Hash | 0.06 | 21.05 | 34988.7% | operator |
| 9057 | Seq Scan | 0.06 | 22.93 | 40133.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9057 (Seq Scan) - LEAF
...
### Step 2: Node 9055 (Seq Scan) - LEAF
...
### Step 4: Node 9049 (Seq Scan) - LEAF
...
### Step 5: Node 9050 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9041, 9042, 9043, 9045, 9046, 9051, 9052, 9053, 9054
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9058 (Index Scan) - LEAF
...
### Step 8: Node 9060 (Seq Scan) - LEAF
...
### Step 11: Node 9045 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9041, 9042, 9043, 9046, 9050, 9051, 9052, 9053, 9054
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9041 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9042, 9043, 9045, 9046, 9050, 9051, 9052, 9053, 9054
- **Input Features:**
