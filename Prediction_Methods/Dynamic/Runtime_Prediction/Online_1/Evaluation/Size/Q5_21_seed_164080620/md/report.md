...
**Test Query:** Q5_21_seed_164080620
...
```
Node 8161 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8162 (Aggregate) [consumed]
    Node 8163 (Gather Merge) [consumed]
      Node 8164 (Aggregate)
        Node 8165 (Sort) [PATTERN: 3e2d5a00]
          Node 8166 (Hash Join) [consumed]
            Node 8167 (Nested Loop)
              Node 8168 (Hash Join)
                Node 8169 (Seq Scan) - LEAF
                Node 8170 (Hash) [PATTERN: 444761fb]
                  Node 8171 (Hash Join) [consumed]
                    Node 8172 (Seq Scan) [consumed] - LEAF
                    Node 8173 (Hash) [consumed]
                      Node 8174 (Hash Join) [consumed]
                        Node 8175 (Seq Scan) - LEAF
                        Node 8176 (Hash)
                          Node 8177 (Seq Scan) - LEAF
              Node 8178 (Index Scan) - LEAF
            Node 8179 (Hash)
              Node 8180 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8170 | 8161, 8162, 8163, 8165, 8166, 8171, 8172, 8173, 8174 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8161 | 8162, 8163, 8165, 8166, 8170, 8171, 8172, 8173, 8174 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8165 | 8161, 8162, 8163, 8166, 8170, 8171, 8172, 8173, 8174 |
...
|------|------|--------|-----------|-----|--------|
| 8161 | Sort | 1049.49 | 2934.42 | 179.6% | pattern |
| 8164 | Aggregate | 1044.44 | 1128.74 | 8.1% | operator |
| 8165 | Sort | 1044.18 | 1835.96 | 75.8% | pattern |
| 8167 | Nested Loop | 1030.01 | 1188.57 | 15.4% | operator |
| 8179 | Hash | 4.92 | 18.67 | 279.3% | operator |
| 8168 | Hash Join | 216.28 | 253.07 | 17.0% | operator |
| 8178 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 8180 | Seq Scan | 3.65 | 12.36 | 238.9% | operator |
| 8169 | Seq Scan | 166.93 | 159.81 | 4.3% | operator |
| 8170 | Hash | 35.98 | 46.85 | 30.2% | pattern |
| 8175 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 8176 | Hash | 0.10 | 21.05 | 19950.7% | operator |
| 8177 | Seq Scan | 0.10 | 22.93 | 22383.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8177 (Seq Scan) - LEAF
...
### Step 2: Node 8175 (Seq Scan) - LEAF
...
### Step 4: Node 8169 (Seq Scan) - LEAF
...
### Step 5: Node 8170 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8161, 8162, 8163, 8165, 8166, 8171, 8172, 8173, 8174
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8178 (Index Scan) - LEAF
...
### Step 8: Node 8180 (Seq Scan) - LEAF
...
### Step 11: Node 8165 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8161, 8162, 8163, 8166, 8170, 8171, 8172, 8173, 8174
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8161 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8162, 8163, 8165, 8166, 8170, 8171, 8172, 8173, 8174
- **Input Features:**
