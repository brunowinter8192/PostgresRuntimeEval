...
**Test Query:** Q8_95_seed_771178914
...
```
Node 16513 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16514 (Gather Merge) [consumed]
    Node 16515 (Sort)
      Node 16516 (Hash Join)
        Node 16517 (Nested Loop)
          Node 16518 (Hash Join)
            Node 16519 (Nested Loop)
              Node 16520 (Hash Join)
                Node 16521 (Seq Scan) - LEAF
                Node 16522 (Hash) [PATTERN: 444761fb]
                  Node 16523 (Hash Join) [consumed]
                    Node 16524 (Seq Scan) [consumed] - LEAF
                    Node 16525 (Hash) [consumed]
                      Node 16526 (Hash Join) [consumed]
                        Node 16527 (Seq Scan) - LEAF
                        Node 16528 (Hash)
                          Node 16529 (Seq Scan) - LEAF
              Node 16530 (Index Scan) - LEAF
            Node 16531 (Hash)
              Node 16532 (Seq Scan) - LEAF
          Node 16533 (Index Scan) - LEAF
        Node 16534 (Hash)
          Node 16535 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16522 | 16513, 16514, 16523, 16524, 16525, 16526 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16513 | 16514, 16522, 16523, 16524, 16525, 16526 |
...
|------|------|--------|-----------|-----|--------|
| 16513 | Aggregate | 1205.17 | 1131.71 | 6.1% | pattern |
| 16515 | Sort | 1198.26 | 1046.21 | 12.7% | operator |
| 16516 | Hash Join | 1198.02 | 785.61 | 34.4% | operator |
| 16517 | Nested Loop | 1197.70 | 1198.29 | 0.0% | operator |
| 16534 | Hash | 0.02 | 13.72 | 80629.9% | operator |
| 16518 | Hash Join | 1193.70 | 785.60 | 34.2% | operator |
| 16533 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16535 | Seq Scan | 0.01 | 6.51 | 49992.0% | operator |
| 16519 | Nested Loop | 1152.77 | 1198.29 | 3.9% | operator |
| 16531 | Hash | 36.36 | 19.97 | 45.1% | operator |
| 16520 | Hash Join | 216.66 | 251.67 | 16.2% | operator |
| 16530 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 16532 | Seq Scan | 36.26 | 48.87 | 34.8% | operator |
| 16521 | Seq Scan | 161.83 | 159.67 | 1.3% | operator |
| 16522 | Hash | 37.10 | 43.51 | 17.3% | pattern |
| 16527 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 16528 | Hash | 0.20 | 18.14 | 8926.9% | operator |
| 16529 | Seq Scan | 0.20 | 23.36 | 11759.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16529 (Seq Scan) - LEAF
...
### Step 2: Node 16527 (Seq Scan) - LEAF
...
### Step 4: Node 16521 (Seq Scan) - LEAF
...
### Step 5: Node 16522 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16513, 16514, 16523, 16524, 16525, 16526
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16530 (Index Scan) - LEAF
...
### Step 8: Node 16532 (Seq Scan) - LEAF
...
### Step 12: Node 16533 (Index Scan) - LEAF
...
### Step 13: Node 16535 (Seq Scan) - LEAF
...
### Step 14: Node 16517 (Nested Loop)
...
### Step 18: Node 16513 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16514, 16522, 16523, 16524, 16525, 16526
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2444
