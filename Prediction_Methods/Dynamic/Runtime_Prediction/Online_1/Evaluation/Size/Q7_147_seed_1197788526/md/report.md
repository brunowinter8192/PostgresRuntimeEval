...
**Test Query:** Q7_147_seed_1197788526
...
```
Node 11419 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11420 (Gather Merge) [consumed]
    Node 11421 (Sort)
      Node 11422 (Hash Join)
        Node 11423 (Nested Loop)
          Node 11424 (Hash Join) [PATTERN: 895c6e8c]
            Node 11425 (Seq Scan) [consumed] - LEAF
            Node 11426 (Hash) [consumed]
              Node 11427 (Hash Join) [PATTERN: f4cb205a]
                Node 11428 (Seq Scan) [consumed] - LEAF
                Node 11429 (Hash) [consumed]
                  Node 11430 (Seq Scan) [consumed] - LEAF
          Node 11431 (Index Scan) - LEAF
        Node 11432 (Hash)
          Node 11433 (Hash Join) [PATTERN: f4cb205a]
            Node 11434 (Seq Scan) [consumed] - LEAF
            Node 11435 (Hash) [consumed]
              Node 11436 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11427 | 11419, 11420, 11424, 11425, 11426, 11428, 11429, 11430, 11433, 11434, 11435, 11436 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11433 | 11419, 11420, 11424, 11425, 11426, 11427, 11428, 11429, 11430, 11434, 11435, 11436 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11419 | 11420, 11424, 11425, 11426, 11427, 11428, 11429, 11430, 11433, 11434, 11435, 11436 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11424 | 11419, 11420, 11425, 11426, 11427, 11428, 11429, 11430, 11433, 11434, 11435, 11436 |
...
|------|------|--------|-----------|-----|--------|
| 11419 | Aggregate | 1186.84 | 1163.13 | 2.0% | pattern |
| 11421 | Sort | 1179.63 | 1109.16 | 6.0% | operator |
| 11422 | Hash Join | 1179.00 | 843.11 | 28.5% | operator |
| 11423 | Nested Loop | 1170.75 | 1215.99 | 3.9% | operator |
| 11432 | Hash | 4.61 | 27.74 | 502.5% | operator |
| 11424 | Hash Join | 214.13 | 304.89 | 42.4% | pattern |
| 11431 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11433 | Hash Join | 4.55 | 429.36 | 9328.3% | pattern |
| 11427 | Hash Join | 35.11 | 429.38 | 1123.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11427 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11419, 11420, 11424, 11425, 11426, 11428, 11429, 11430, 11433, 11434, 11435, 11436
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11424 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11419, 11420, 11425, 11426, 11427, 11428, 11429, 11430, 11433, 11434, 11435, 11436
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11431 (Index Scan) - LEAF
...
### Step 4: Node 11433 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11419, 11420, 11424, 11425, 11426, 11427, 11428, 11429, 11430, 11434, 11435, 11436
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11419 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11420, 11424, 11425, 11426, 11427, 11428, 11429, 11430, 11433, 11434, 11435, 11436
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
