...
**Test Query:** Q10_10_seed_73836279
...
```
Node 19500 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19501 (Sort) [consumed]
    Node 19502 (Aggregate) [consumed]
      Node 19503 (Gather) [consumed]
        Node 19504 (Hash Join)
          Node 19505 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19506 (Nested Loop) [consumed]
              Node 19507 (Seq Scan) - LEAF
              Node 19508 (Index Scan) - LEAF
            Node 19509 (Hash) [consumed]
              Node 19510 (Seq Scan) - LEAF
          Node 19511 (Hash)
            Node 19512 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19500 | 19501, 19502, 19503, 19505, 19506, 19509 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19505 | 19500, 19501, 19502, 19503, 19506, 19509 |
...
|------|------|--------|-----------|-----|--------|
| 19500 | Limit | 1172.31 | 1199.47 | 2.3% | pattern |
| 19504 | Hash Join | 1108.25 | 1095.60 | 1.1% | operator |
| 19505 | Hash Join | 1105.31 | 1120.19 | 1.3% | pattern |
| 19511 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 19512 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 19507 | Seq Scan | 164.31 | 159.74 | 2.8% | operator |
| 19508 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 19510 | Seq Scan | 26.01 | 26.64 | 2.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19507 (Seq Scan) - LEAF
...
### Step 2: Node 19508 (Index Scan) - LEAF
...
### Step 3: Node 19510 (Seq Scan) - LEAF
...
### Step 4: Node 19512 (Seq Scan) - LEAF
...
### Step 5: Node 19505 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19500, 19501, 19502, 19503, 19506, 19509
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18384
  - HashJoin_nt1=18384
...
### Step 8: Node 19500 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19501, 19502, 19503, 19505, 19506, 19509
- **Input Features:**
