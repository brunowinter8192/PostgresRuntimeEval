...
**Test Query:** Q10_42_seed_336365271
...
```
Node 20488 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20489 (Sort) [consumed]
    Node 20490 (Aggregate) [consumed]
      Node 20491 (Gather) [consumed]
        Node 20492 (Hash Join)
          Node 20493 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20494 (Nested Loop) [consumed]
              Node 20495 (Seq Scan) - LEAF
              Node 20496 (Index Scan) - LEAF
            Node 20497 (Hash) [consumed]
              Node 20498 (Seq Scan) - LEAF
          Node 20499 (Hash)
            Node 20500 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20488 | 20489, 20490, 20491, 20493, 20494, 20497 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20493 | 20488, 20489, 20490, 20491, 20494, 20497 |
...
|------|------|--------|-----------|-----|--------|
| 20488 | Limit | 1184.26 | 1199.47 | 1.3% | pattern |
| 20492 | Hash Join | 1128.05 | 1095.57 | 2.9% | operator |
| 20493 | Hash Join | 1125.03 | 1120.19 | 0.4% | pattern |
| 20499 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20500 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20495 | Seq Scan | 161.35 | 159.74 | 1.0% | operator |
| 20496 | Index Scan | 0.06 | -0.52 | 909.7% | operator |
| 20498 | Seq Scan | 23.86 | 26.64 | 11.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20495 (Seq Scan) - LEAF
...
### Step 2: Node 20496 (Index Scan) - LEAF
...
### Step 3: Node 20498 (Seq Scan) - LEAF
...
### Step 4: Node 20500 (Seq Scan) - LEAF
...
### Step 5: Node 20493 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20488, 20489, 20490, 20491, 20494, 20497
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18257
  - HashJoin_nt1=18257
...
### Step 8: Node 20488 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20489, 20490, 20491, 20493, 20494, 20497
- **Input Features:**
