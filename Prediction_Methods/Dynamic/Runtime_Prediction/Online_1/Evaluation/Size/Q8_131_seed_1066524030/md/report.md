...
**Test Query:** Q8_131_seed_1066524030
...
```
Node 13983 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13984 (Gather Merge) [consumed]
    Node 13985 (Sort)
      Node 13986 (Hash Join)
        Node 13987 (Nested Loop)
          Node 13988 (Hash Join)
            Node 13989 (Nested Loop)
              Node 13990 (Hash Join)
                Node 13991 (Seq Scan) - LEAF
                Node 13992 (Hash) [PATTERN: 444761fb]
                  Node 13993 (Hash Join) [consumed]
                    Node 13994 (Seq Scan) [consumed] - LEAF
                    Node 13995 (Hash) [consumed]
                      Node 13996 (Hash Join) [consumed]
                        Node 13997 (Seq Scan) - LEAF
                        Node 13998 (Hash)
                          Node 13999 (Seq Scan) - LEAF
              Node 14000 (Index Scan) - LEAF
            Node 14001 (Hash)
              Node 14002 (Seq Scan) - LEAF
          Node 14003 (Index Scan) - LEAF
        Node 14004 (Hash)
          Node 14005 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13992 | 13983, 13984, 13993, 13994, 13995, 13996 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13983 | 13984, 13992, 13993, 13994, 13995, 13996 |
...
|------|------|--------|-----------|-----|--------|
| 13983 | Aggregate | 1179.06 | 1131.84 | 4.0% | pattern |
| 13985 | Sort | 1174.21 | 1046.21 | 10.9% | operator |
| 13986 | Hash Join | 1173.95 | 785.61 | 33.1% | operator |
| 13987 | Nested Loop | 1173.62 | 1198.29 | 2.1% | operator |
| 14004 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 13988 | Hash Join | 1169.80 | 785.60 | 32.8% | operator |
| 14003 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14005 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13989 | Nested Loop | 1126.76 | 1198.29 | 6.3% | operator |
| 14001 | Hash | 38.51 | 19.97 | 48.1% | operator |
| 13990 | Hash Join | 207.52 | 251.67 | 21.3% | operator |
| 14000 | Index Scan | 0.04 | 2.88 | 7286.8% | operator |
| 14002 | Seq Scan | 38.46 | 48.87 | 27.1% | operator |
| 13991 | Seq Scan | 158.20 | 159.67 | 0.9% | operator |
| 13992 | Hash | 31.57 | 43.51 | 37.9% | pattern |
| 13997 | Seq Scan | 0.01 | 12.22 | 101774.5% | operator |
| 13998 | Hash | 0.17 | 18.14 | 10448.9% | operator |
| 13999 | Seq Scan | 0.17 | 23.36 | 13806.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13999 (Seq Scan) - LEAF
...
### Step 2: Node 13997 (Seq Scan) - LEAF
...
### Step 4: Node 13991 (Seq Scan) - LEAF
...
### Step 5: Node 13992 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13983, 13984, 13993, 13994, 13995, 13996
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14000 (Index Scan) - LEAF
...
### Step 8: Node 14002 (Seq Scan) - LEAF
...
### Step 12: Node 14003 (Index Scan) - LEAF
...
### Step 13: Node 14005 (Seq Scan) - LEAF
...
### Step 14: Node 13987 (Nested Loop)
...
### Step 18: Node 13983 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13984, 13992, 13993, 13994, 13995, 13996
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2442
