...
**Test Query:** Q13_107_seed_869627286
**Timestamp:** 2026-01-18 19:54:19
...
```
Node 25419 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 25420 (Aggregate) [consumed]
    Node 25421 (Aggregate)
      Node 25422 (Hash Join) [PATTERN: 895c6e8c]
        Node 25423 (Seq Scan) [consumed] - LEAF
        Node 25424 (Hash) [consumed]
          Node 25425 (Index Only Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Sort -> Aggregate (Outer) | 1d35fb97 | 25419 | 25420, 25422, 25423, 25424 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 25422 | 25419, 25420, 25423, 25424 |
...
|------|------|--------|-----------|-----|--------|
| 25419 | Sort | 894.71 | 1079.93 | 20.7% | pattern |
| 25421 | Aggregate | 888.64 | 999.35 | 12.5% | operator |
| 25422 | Hash Join | 684.62 | 470.31 | 31.3% | pattern |
| 25425 | Index Only Scan | 10.21 | 0.00 | 100.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 25425 (Index Only Scan) - LEAF

- **Source:** operator
- **Output:** st=0.00, rt=0.00

### Step 2: Node 25422 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 25419, 25420, 25423, 25424
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=1481964
  - HashJoin_nt1=1481964
  - HashJoin_nt2=150000
  ...
  - HashJoin_total_cost=53364.1900
  - Hash_Inner_np=0
...
- **Output:** st=908.29, rt=999.35

### Step 4: Node 25419 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 25420, 25422, 25423, 25424
- **Input Features:**
