...
**Test Query:** Q5_54_seed_434813643
...
```
Node 8881 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8882 (Aggregate) [consumed]
    Node 8883 (Gather Merge) [consumed]
      Node 8884 (Aggregate)
        Node 8885 (Sort) [PATTERN: 3e2d5a00]
          Node 8886 (Hash Join) [consumed]
            Node 8887 (Nested Loop)
              Node 8888 (Hash Join)
                Node 8889 (Seq Scan) - LEAF
                Node 8890 (Hash) [PATTERN: 444761fb]
                  Node 8891 (Hash Join) [consumed]
                    Node 8892 (Seq Scan) [consumed] - LEAF
                    Node 8893 (Hash) [consumed]
                      Node 8894 (Hash Join) [consumed]
                        Node 8895 (Seq Scan) - LEAF
                        Node 8896 (Hash)
                          Node 8897 (Seq Scan) - LEAF
              Node 8898 (Index Scan) - LEAF
            Node 8899 (Hash)
              Node 8900 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8890 | 8881, 8882, 8883, 8885, 8886, 8891, 8892, 8893, 8894 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8881 | 8882, 8883, 8885, 8886, 8890, 8891, 8892, 8893, 8894 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8885 | 8881, 8882, 8883, 8886, 8890, 8891, 8892, 8893, 8894 |
...
|------|------|--------|-----------|-----|--------|
| 8881 | Sort | 1054.77 | 2934.42 | 178.2% | pattern |
| 8884 | Aggregate | 1049.65 | 1128.76 | 7.5% | operator |
| 8885 | Sort | 1049.39 | 1835.93 | 75.0% | pattern |
| 8887 | Nested Loop | 1037.80 | 1188.47 | 14.5% | operator |
| 8899 | Hash | 2.93 | 18.67 | 537.2% | operator |
| 8888 | Hash Join | 214.62 | 253.07 | 17.9% | operator |
| 8898 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 8900 | Seq Scan | 2.41 | 12.36 | 412.2% | operator |
| 8889 | Seq Scan | 166.94 | 159.81 | 4.3% | operator |
| 8890 | Hash | 34.87 | 46.85 | 34.4% | pattern |
| 8895 | Seq Scan | 0.01 | 11.81 | 84244.8% | operator |
| 8896 | Hash | 0.07 | 21.05 | 28740.1% | operator |
| 8897 | Seq Scan | 0.07 | 22.93 | 32661.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8897 (Seq Scan) - LEAF
...
### Step 2: Node 8895 (Seq Scan) - LEAF
...
### Step 4: Node 8889 (Seq Scan) - LEAF
...
### Step 5: Node 8890 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8881, 8882, 8883, 8885, 8886, 8891, 8892, 8893, 8894
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8898 (Index Scan) - LEAF
...
### Step 8: Node 8900 (Seq Scan) - LEAF
...
### Step 11: Node 8885 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8881, 8882, 8883, 8886, 8890, 8891, 8892, 8893, 8894
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8881 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8882, 8883, 8885, 8886, 8890, 8891, 8892, 8893, 8894
- **Input Features:**
