...
**Test Query:** Q8_126_seed_1025503875
...
```
Node 13845 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13846 (Gather Merge) [consumed]
    Node 13847 (Sort)
      Node 13848 (Hash Join)
        Node 13849 (Nested Loop)
          Node 13850 (Hash Join)
            Node 13851 (Nested Loop)
              Node 13852 (Hash Join)
                Node 13853 (Seq Scan) - LEAF
                Node 13854 (Hash) [PATTERN: 444761fb]
                  Node 13855 (Hash Join) [consumed]
                    Node 13856 (Seq Scan) [consumed] - LEAF
                    Node 13857 (Hash) [consumed]
                      Node 13858 (Hash Join) [consumed]
                        Node 13859 (Seq Scan) - LEAF
                        Node 13860 (Hash)
                          Node 13861 (Seq Scan) - LEAF
              Node 13862 (Index Scan) - LEAF
            Node 13863 (Hash)
              Node 13864 (Seq Scan) - LEAF
          Node 13865 (Index Scan) - LEAF
        Node 13866 (Hash)
          Node 13867 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13854 | 13845, 13846, 13855, 13856, 13857, 13858 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13845 | 13846, 13854, 13855, 13856, 13857, 13858 |
...
|------|------|--------|-----------|-----|--------|
| 13845 | Aggregate | 1233.70 | 1132.96 | 8.2% | pattern |
| 13847 | Sort | 1228.42 | 1046.21 | 14.8% | operator |
| 13848 | Hash Join | 1228.18 | 785.61 | 36.0% | operator |
| 13849 | Nested Loop | 1227.86 | 1198.29 | 2.4% | operator |
| 13866 | Hash | 0.01 | 13.72 | 114267.4% | operator |
| 13850 | Hash Join | 1223.55 | 785.60 | 35.8% | operator |
| 13865 | Index Scan | 0.01 | 12.42 | 177378.3% | operator |
| 13867 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 13851 | Nested Loop | 1172.93 | 1198.29 | 2.2% | operator |
| 13863 | Hash | 41.43 | 19.97 | 51.8% | operator |
| 13852 | Hash Join | 216.22 | 251.67 | 16.4% | operator |
| 13862 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 13864 | Seq Scan | 41.29 | 48.87 | 18.4% | operator |
| 13853 | Seq Scan | 161.64 | 159.67 | 1.2% | operator |
| 13854 | Hash | 36.20 | 43.51 | 20.2% | pattern |
| 13859 | Seq Scan | 0.01 | 12.22 | 87221.0% | operator |
| 13860 | Hash | 0.07 | 18.14 | 25820.2% | operator |
| 13861 | Seq Scan | 0.07 | 23.36 | 35297.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13861 (Seq Scan) - LEAF
...
### Step 2: Node 13859 (Seq Scan) - LEAF
...
### Step 4: Node 13853 (Seq Scan) - LEAF
...
### Step 5: Node 13854 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13845, 13846, 13855, 13856, 13857, 13858
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13862 (Index Scan) - LEAF
...
### Step 8: Node 13864 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9703
  - startup_cost=9920.0800
  - total_cost=70271.5500
- **Output:** st=113.62, rt=785.60

### Step 12: Node 13865 (Index Scan) - LEAF
...
### Step 13: Node 13867 (Seq Scan) - LEAF
...
### Step 14: Node 13849 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.3600
  - total_cost=70507.8400
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 13866 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.9200
  - total_cost=70513.7500
- **Output:** st=113.62, rt=785.61

### Step 17: Node 13847 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70551.2800
  - total_cost=70553.2300
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 13845 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13846, 13854, 13855, 13856, 13857, 13858
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2420
  ...
  - Aggregate_sel=0.9942
  - Aggregate_startup_cost=71551.3200
  - Aggregate_total_cost=71934.1400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2420
  ...
  - GatherMerge_Outer_sel=3.0986
  - GatherMerge_Outer_startup_cost=71551.3200
  - GatherMerge_Outer_total_cost=71837.6200
- **Output:** st=1129.97, rt=1132.96
