...
**Test Query:** Q7_32_seed_254324961
...
```
Node 11851 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11852 (Gather Merge) [consumed]
    Node 11853 (Sort)
      Node 11854 (Hash Join)
        Node 11855 (Nested Loop)
          Node 11856 (Hash Join) [PATTERN: 895c6e8c]
            Node 11857 (Seq Scan) [consumed] - LEAF
            Node 11858 (Hash) [consumed]
              Node 11859 (Hash Join) [PATTERN: f4cb205a]
                Node 11860 (Seq Scan) [consumed] - LEAF
                Node 11861 (Hash) [consumed]
                  Node 11862 (Seq Scan) [consumed] - LEAF
          Node 11863 (Index Scan) - LEAF
        Node 11864 (Hash)
          Node 11865 (Hash Join) [PATTERN: f4cb205a]
            Node 11866 (Seq Scan) [consumed] - LEAF
            Node 11867 (Hash) [consumed]
              Node 11868 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11859 | 11851, 11852, 11856, 11857, 11858, 11860, 11861, 11862, 11865, 11866, 11867, 11868 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11865 | 11851, 11852, 11856, 11857, 11858, 11859, 11860, 11861, 11862, 11866, 11867, 11868 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11851 | 11852, 11856, 11857, 11858, 11859, 11860, 11861, 11862, 11865, 11866, 11867, 11868 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11856 | 11851, 11852, 11857, 11858, 11859, 11860, 11861, 11862, 11865, 11866, 11867, 11868 |
...
|------|------|--------|-----------|-----|--------|
| 11851 | Aggregate | 1178.88 | 1163.13 | 1.3% | pattern |
| 11853 | Sort | 1169.68 | 1109.16 | 5.2% | operator |
| 11854 | Hash Join | 1169.06 | 843.11 | 27.9% | operator |
| 11855 | Nested Loop | 1162.63 | 1215.99 | 4.6% | operator |
| 11864 | Hash | 2.72 | 27.74 | 921.5% | operator |
| 11856 | Hash Join | 208.73 | 304.89 | 46.1% | pattern |
| 11863 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 11865 | Hash Join | 2.68 | 429.36 | 15939.0% | pattern |
| 11859 | Hash Join | 32.90 | 429.38 | 1205.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11859 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11851, 11852, 11856, 11857, 11858, 11860, 11861, 11862, 11865, 11866, 11867, 11868
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11856 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11851, 11852, 11857, 11858, 11859, 11860, 11861, 11862, 11865, 11866, 11867, 11868
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11863 (Index Scan) - LEAF
...
### Step 4: Node 11865 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11851, 11852, 11856, 11857, 11858, 11859, 11860, 11861, 11862, 11866, 11867, 11868
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11851 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11852, 11856, 11857, 11858, 11859, 11860, 11861, 11862, 11865, 11866, 11867, 11868
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
