...
**Test Query:** Q5_136_seed_1107544185
...
```
Node 7681 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7682 (Aggregate) [consumed]
    Node 7683 (Gather Merge) [consumed]
      Node 7684 (Aggregate)
        Node 7685 (Sort) [PATTERN: 3e2d5a00]
          Node 7686 (Hash Join) [consumed]
            Node 7687 (Nested Loop)
              Node 7688 (Hash Join)
                Node 7689 (Seq Scan) - LEAF
                Node 7690 (Hash) [PATTERN: 444761fb]
                  Node 7691 (Hash Join) [consumed]
                    Node 7692 (Seq Scan) [consumed] - LEAF
                    Node 7693 (Hash) [consumed]
                      Node 7694 (Hash Join) [consumed]
                        Node 7695 (Seq Scan) - LEAF
                        Node 7696 (Hash)
                          Node 7697 (Seq Scan) - LEAF
              Node 7698 (Index Scan) - LEAF
            Node 7699 (Hash)
              Node 7700 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7690 | 7681, 7682, 7683, 7685, 7686, 7691, 7692, 7693, 7694 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7681 | 7682, 7683, 7685, 7686, 7690, 7691, 7692, 7693, 7694 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7685 | 7681, 7682, 7683, 7686, 7690, 7691, 7692, 7693, 7694 |
...
|------|------|--------|-----------|-----|--------|
| 7681 | Sort | 1066.37 | 2934.42 | 175.2% | pattern |
| 7684 | Aggregate | 1060.68 | 1128.78 | 6.4% | operator |
| 7685 | Sort | 1060.44 | 1835.92 | 73.1% | pattern |
| 7687 | Nested Loop | 1046.62 | 1188.42 | 13.5% | operator |
| 7699 | Hash | 5.19 | 18.67 | 259.7% | operator |
| 7688 | Hash Join | 215.93 | 253.07 | 17.2% | operator |
| 7698 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 7700 | Seq Scan | 4.61 | 12.36 | 168.3% | operator |
| 7689 | Seq Scan | 168.35 | 159.81 | 5.1% | operator |
| 7690 | Hash | 34.22 | 46.85 | 36.9% | pattern |
| 7695 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 7696 | Hash | 0.06 | 21.05 | 35583.5% | operator |
| 7697 | Seq Scan | 0.06 | 22.93 | 41597.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7697 (Seq Scan) - LEAF
...
### Step 2: Node 7695 (Seq Scan) - LEAF
...
### Step 4: Node 7689 (Seq Scan) - LEAF
...
### Step 5: Node 7690 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7681, 7682, 7683, 7685, 7686, 7691, 7692, 7693, 7694
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7698 (Index Scan) - LEAF
...
### Step 8: Node 7700 (Seq Scan) - LEAF
...
### Step 11: Node 7685 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7681, 7682, 7683, 7686, 7690, 7691, 7692, 7693, 7694
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7681 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7682, 7683, 7685, 7686, 7690, 7691, 7692, 7693, 7694
- **Input Features:**
