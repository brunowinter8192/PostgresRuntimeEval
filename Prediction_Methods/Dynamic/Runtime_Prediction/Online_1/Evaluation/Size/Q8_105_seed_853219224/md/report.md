...
**Test Query:** Q8_105_seed_853219224
...
```
Node 13316 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13317 (Gather Merge) [consumed]
    Node 13318 (Sort)
      Node 13319 (Hash Join)
        Node 13320 (Nested Loop)
          Node 13321 (Hash Join)
            Node 13322 (Nested Loop)
              Node 13323 (Hash Join)
                Node 13324 (Seq Scan) - LEAF
                Node 13325 (Hash) [PATTERN: 444761fb]
                  Node 13326 (Hash Join) [consumed]
                    Node 13327 (Seq Scan) [consumed] - LEAF
                    Node 13328 (Hash) [consumed]
                      Node 13329 (Hash Join) [consumed]
                        Node 13330 (Seq Scan) - LEAF
                        Node 13331 (Hash)
                          Node 13332 (Seq Scan) - LEAF
              Node 13333 (Index Scan) - LEAF
            Node 13334 (Hash)
              Node 13335 (Seq Scan) - LEAF
          Node 13336 (Index Scan) - LEAF
        Node 13337 (Hash)
          Node 13338 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13325 | 13316, 13317, 13326, 13327, 13328, 13329 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13316 | 13317, 13325, 13326, 13327, 13328, 13329 |
...
|------|------|--------|-----------|-----|--------|
| 13316 | Aggregate | 1239.36 | 1132.74 | 8.6% | pattern |
| 13318 | Sort | 1234.06 | 1046.21 | 15.2% | operator |
| 13319 | Hash Join | 1233.83 | 785.61 | 36.3% | operator |
| 13320 | Nested Loop | 1233.52 | 1198.29 | 2.9% | operator |
| 13337 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 13321 | Hash Join | 1229.72 | 785.60 | 36.1% | operator |
| 13336 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13338 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 13322 | Nested Loop | 1181.93 | 1198.29 | 1.4% | operator |
| 13334 | Hash | 43.10 | 19.97 | 53.7% | operator |
| 13323 | Hash Join | 214.19 | 251.67 | 17.5% | operator |
| 13333 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 13335 | Seq Scan | 42.99 | 48.87 | 13.7% | operator |
| 13324 | Seq Scan | 159.59 | 159.67 | 0.0% | operator |
| 13325 | Hash | 36.79 | 43.51 | 18.3% | pattern |
| 13330 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 13331 | Hash | 0.16 | 18.14 | 11169.7% | operator |
| 13332 | Seq Scan | 0.16 | 23.36 | 14686.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13332 (Seq Scan) - LEAF
...
### Step 2: Node 13330 (Seq Scan) - LEAF
...
### Step 4: Node 13324 (Seq Scan) - LEAF
...
### Step 5: Node 13325 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13316, 13317, 13326, 13327, 13328, 13329
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13333 (Index Scan) - LEAF
...
### Step 8: Node 13335 (Seq Scan) - LEAF
...
### Step 12: Node 13336 (Index Scan) - LEAF
...
### Step 13: Node 13338 (Seq Scan) - LEAF
...
### Step 14: Node 13320 (Nested Loop)
...
### Step 18: Node 13316 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13317, 13325, 13326, 13327, 13328, 13329
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2424
