...
**Test Query:** Q8_76_seed_615302325
...
```
Node 16030 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16031 (Gather Merge) [consumed]
    Node 16032 (Sort)
      Node 16033 (Hash Join)
        Node 16034 (Nested Loop)
          Node 16035 (Hash Join)
            Node 16036 (Nested Loop)
              Node 16037 (Hash Join)
                Node 16038 (Seq Scan) - LEAF
                Node 16039 (Hash) [PATTERN: 444761fb]
                  Node 16040 (Hash Join) [consumed]
                    Node 16041 (Seq Scan) [consumed] - LEAF
                    Node 16042 (Hash) [consumed]
                      Node 16043 (Hash Join) [consumed]
                        Node 16044 (Seq Scan) - LEAF
                        Node 16045 (Hash)
                          Node 16046 (Seq Scan) - LEAF
              Node 16047 (Index Scan) - LEAF
            Node 16048 (Hash)
              Node 16049 (Seq Scan) - LEAF
          Node 16050 (Index Scan) - LEAF
        Node 16051 (Hash)
          Node 16052 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16039 | 16030, 16031, 16040, 16041, 16042, 16043 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16030 | 16031, 16039, 16040, 16041, 16042, 16043 |
...
|------|------|--------|-----------|-----|--------|
| 16030 | Aggregate | 1247.57 | 1133.68 | 9.1% | pattern |
| 16032 | Sort | 1240.70 | 1046.21 | 15.7% | operator |
| 16033 | Hash Join | 1240.44 | 785.61 | 36.7% | operator |
| 16034 | Nested Loop | 1240.11 | 1198.29 | 3.4% | operator |
| 16051 | Hash | 0.02 | 13.72 | 76144.9% | operator |
| 16035 | Hash Join | 1236.05 | 785.60 | 36.4% | operator |
| 16050 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16052 | Seq Scan | 0.01 | 6.51 | 49992.0% | operator |
| 16036 | Nested Loop | 1189.77 | 1198.29 | 0.7% | operator |
| 16048 | Hash | 41.68 | 19.97 | 52.1% | operator |
| 16037 | Hash Join | 218.57 | 251.67 | 15.1% | operator |
| 16047 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 16049 | Seq Scan | 41.58 | 48.88 | 17.6% | operator |
| 16038 | Seq Scan | 161.39 | 159.67 | 1.1% | operator |
| 16039 | Hash | 39.03 | 43.51 | 11.5% | pattern |
| 16044 | Seq Scan | 0.01 | 12.22 | 101774.5% | operator |
| 16045 | Hash | 0.11 | 18.14 | 16100.1% | operator |
| 16046 | Seq Scan | 0.11 | 23.36 | 21333.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16046 (Seq Scan) - LEAF
...
### Step 2: Node 16044 (Seq Scan) - LEAF
...
### Step 4: Node 16038 (Seq Scan) - LEAF
...
### Step 5: Node 16039 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16030, 16031, 16040, 16041, 16042, 16043
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16047 (Index Scan) - LEAF
...
### Step 8: Node 16049 (Seq Scan) - LEAF
...
### Step 12: Node 16050 (Index Scan) - LEAF
...
### Step 13: Node 16052 (Seq Scan) - LEAF
...
### Step 14: Node 16034 (Nested Loop)
...
### Step 18: Node 16030 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16031, 16039, 16040, 16041, 16042, 16043
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2383
  - Aggregate_nt1=2383
