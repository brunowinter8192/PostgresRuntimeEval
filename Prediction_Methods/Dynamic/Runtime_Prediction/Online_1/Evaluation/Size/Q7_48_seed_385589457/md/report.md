...
**Test Query:** Q7_48_seed_385589457
...
```
Node 12157 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12158 (Gather Merge) [consumed]
    Node 12159 (Sort)
      Node 12160 (Hash Join)
        Node 12161 (Nested Loop)
          Node 12162 (Hash Join) [PATTERN: 895c6e8c]
            Node 12163 (Seq Scan) [consumed] - LEAF
            Node 12164 (Hash) [consumed]
              Node 12165 (Hash Join) [PATTERN: f4cb205a]
                Node 12166 (Seq Scan) [consumed] - LEAF
                Node 12167 (Hash) [consumed]
                  Node 12168 (Seq Scan) [consumed] - LEAF
          Node 12169 (Index Scan) - LEAF
        Node 12170 (Hash)
          Node 12171 (Hash Join) [PATTERN: f4cb205a]
            Node 12172 (Seq Scan) [consumed] - LEAF
            Node 12173 (Hash) [consumed]
              Node 12174 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12165 | 12157, 12158, 12162, 12163, 12164, 12166, 12167, 12168, 12171, 12172, 12173, 12174 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12171 | 12157, 12158, 12162, 12163, 12164, 12165, 12166, 12167, 12168, 12172, 12173, 12174 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12157 | 12158, 12162, 12163, 12164, 12165, 12166, 12167, 12168, 12171, 12172, 12173, 12174 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12162 | 12157, 12158, 12163, 12164, 12165, 12166, 12167, 12168, 12171, 12172, 12173, 12174 |
...
|------|------|--------|-----------|-----|--------|
| 12157 | Aggregate | 1217.62 | 1163.13 | 4.5% | pattern |
| 12159 | Sort | 1210.11 | 1109.16 | 8.3% | operator |
| 12160 | Hash Join | 1209.44 | 843.11 | 30.3% | operator |
| 12161 | Nested Loop | 1201.86 | 1215.99 | 1.2% | operator |
| 12170 | Hash | 3.80 | 27.74 | 629.5% | operator |
| 12162 | Hash Join | 215.40 | 304.89 | 41.5% | pattern |
| 12169 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12171 | Hash Join | 3.76 | 429.36 | 11325.3% | pattern |
| 12165 | Hash Join | 34.84 | 429.38 | 1132.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12165 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12157, 12158, 12162, 12163, 12164, 12166, 12167, 12168, 12171, 12172, 12173, 12174
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12162 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12157, 12158, 12163, 12164, 12165, 12166, 12167, 12168, 12171, 12172, 12173, 12174
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12169 (Index Scan) - LEAF
...
### Step 4: Node 12171 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12157, 12158, 12162, 12163, 12164, 12165, 12166, 12167, 12168, 12172, 12173, 12174
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12157 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12158, 12162, 12163, 12164, 12165, 12166, 12167, 12168, 12171, 12172, 12173, 12174
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
