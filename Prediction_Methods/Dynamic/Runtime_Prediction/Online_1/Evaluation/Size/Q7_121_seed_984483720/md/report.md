...
**Test Query:** Q7_121_seed_984483720
...
```
Node 10915 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10916 (Gather Merge) [consumed]
    Node 10917 (Sort)
      Node 10918 (Hash Join)
        Node 10919 (Nested Loop)
          Node 10920 (Hash Join) [PATTERN: 895c6e8c]
            Node 10921 (Seq Scan) [consumed] - LEAF
            Node 10922 (Hash) [consumed]
              Node 10923 (Hash Join) [PATTERN: f4cb205a]
                Node 10924 (Seq Scan) [consumed] - LEAF
                Node 10925 (Hash) [consumed]
                  Node 10926 (Seq Scan) [consumed] - LEAF
          Node 10927 (Index Scan) - LEAF
        Node 10928 (Hash)
          Node 10929 (Hash Join) [PATTERN: f4cb205a]
            Node 10930 (Seq Scan) [consumed] - LEAF
            Node 10931 (Hash) [consumed]
              Node 10932 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10923 | 10915, 10916, 10920, 10921, 10922, 10924, 10925, 10926, 10929, 10930, 10931, 10932 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10929 | 10915, 10916, 10920, 10921, 10922, 10923, 10924, 10925, 10926, 10930, 10931, 10932 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10915 | 10916, 10920, 10921, 10922, 10923, 10924, 10925, 10926, 10929, 10930, 10931, 10932 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10920 | 10915, 10916, 10921, 10922, 10923, 10924, 10925, 10926, 10929, 10930, 10931, 10932 |
...
|------|------|--------|-----------|-----|--------|
| 10915 | Aggregate | 1198.38 | 1163.13 | 2.9% | pattern |
| 10917 | Sort | 1191.52 | 1109.16 | 6.9% | operator |
| 10918 | Hash Join | 1190.92 | 843.11 | 29.2% | operator |
| 10919 | Nested Loop | 1184.19 | 1215.99 | 2.7% | operator |
| 10928 | Hash | 3.13 | 27.74 | 787.5% | operator |
| 10920 | Hash Join | 212.97 | 304.89 | 43.2% | pattern |
| 10927 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 10929 | Hash Join | 3.08 | 429.36 | 13835.8% | pattern |
| 10923 | Hash Join | 35.12 | 429.38 | 1122.5% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10923 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10915, 10916, 10920, 10921, 10922, 10924, 10925, 10926, 10929, 10930, 10931, 10932
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10920 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10915, 10916, 10921, 10922, 10923, 10924, 10925, 10926, 10929, 10930, 10931, 10932
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10927 (Index Scan) - LEAF
...
### Step 4: Node 10929 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10915, 10916, 10920, 10921, 10922, 10923, 10924, 10925, 10926, 10930, 10931, 10932
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10915 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10916, 10920, 10921, 10922, 10923, 10924, 10925, 10926, 10929, 10930, 10931, 10932
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
