...
**Test Query:** Q9_138_seed_1123952247
...
```
Node 17393 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 17394 (Aggregate) [consumed]
    Node 17395 (Gather)
      Node 17396 (Aggregate)
        Node 17397 (Nested Loop)
          Node 17398 (Hash Join) [PATTERN: bb930825]
            Node 17399 (Seq Scan) [consumed] - LEAF
            Node 17400 (Hash) [consumed]
              Node 17401 (Hash Join) [consumed]
                Node 17402 (Hash Join)
                  Node 17403 (Nested Loop)
                    Node 17404 (Seq Scan) - LEAF
                    Node 17405 (Index Scan) - LEAF
                  Node 17406 (Hash)
                    Node 17407 (Seq Scan) - LEAF
                Node 17408 (Hash)
                  Node 17409 (Seq Scan) - LEAF
          Node 17410 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | bb930825 | 17398 | 17393, 17394, 17399, 17400, 17401 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 17393 | 17394, 17398, 17399, 17400, 17401 |
...
|------|------|--------|-----------|-----|--------|
| 17393 | Sort | 1186.13 | 1045.59 | 11.8% | pattern |
| 17395 | Gather | 1184.76 | 964.55 | 18.6% | operator |
| 17396 | Aggregate | 1166.52 | 1085.45 | 6.9% | operator |
| 17397 | Nested Loop | 1141.33 | 1252.17 | 9.7% | operator |
| 17398 | Hash Join | 964.68 | 134.80 | 86.0% | pattern |
| 17410 | Index Scan | 0.00 | -5.54 | 184812.7% | operator |
| 17402 | Hash Join | 156.39 | 837.63 | 435.6% | operator |
| 17408 | Hash | 13.49 | 14.95 | 10.8% | operator |
| 17403 | Nested Loop | 149.78 | 1251.77 | 735.7% | operator |
| 17406 | Hash | 4.32 | 15.94 | 269.0% | operator |
| 17409 | Seq Scan | 13.47 | 2.94 | 78.2% | operator |
| 17404 | Seq Scan | 19.66 | 43.96 | 123.6% | operator |
| 17405 | Index Scan | 0.07 | -1.39 | 2055.6% | operator |
| 17407 | Seq Scan | 3.79 | 9.45 | 149.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 17404 (Seq Scan) - LEAF
...
### Step 2: Node 17405 (Index Scan) - LEAF
...
### Step 3: Node 17407 (Seq Scan) - LEAF
...
### Step 6: Node 17409 (Seq Scan) - LEAF
...
### Step 9: Node 17398 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** bb930825 (Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)])
- **Consumes:** Nodes 17393, 17394, 17399, 17400, 17401
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 17410 (Index Scan) - LEAF
...
### Step 11: Node 17397 (Nested Loop)
...
### Step 14: Node 17393 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 17394, 17398, 17399, 17400, 17401
- **Input Features:**
