...
**Test Query:** Q9_105_seed_853219224
...
```
Node 16742 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 16743 (Aggregate) [consumed]
    Node 16744 (Gather)
      Node 16745 (Aggregate)
        Node 16746 (Nested Loop)
          Node 16747 (Hash Join) [PATTERN: bb930825]
            Node 16748 (Seq Scan) [consumed] - LEAF
            Node 16749 (Hash) [consumed]
              Node 16750 (Hash Join) [consumed]
                Node 16751 (Hash Join)
                  Node 16752 (Nested Loop)
                    Node 16753 (Seq Scan) - LEAF
                    Node 16754 (Index Scan) - LEAF
                  Node 16755 (Hash)
                    Node 16756 (Seq Scan) - LEAF
                Node 16757 (Hash)
                  Node 16758 (Seq Scan) - LEAF
          Node 16759 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | bb930825 | 16747 | 16742, 16743, 16748, 16749, 16750 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 16742 | 16743, 16747, 16748, 16749, 16750 |
...
|------|------|--------|-----------|-----|--------|
| 16742 | Sort | 1186.90 | 1045.10 | 11.9% | pattern |
| 16744 | Gather | 1185.78 | 960.51 | 19.0% | operator |
| 16745 | Aggregate | 1167.04 | 1141.32 | 2.2% | operator |
| 16746 | Nested Loop | 1143.19 | 1252.17 | 9.5% | operator |
| 16747 | Hash Join | 967.36 | 134.80 | 86.1% | pattern |
| 16759 | Index Scan | 0.00 | -5.54 | 184812.7% | operator |
| 16751 | Hash Join | 161.21 | 837.46 | 419.5% | operator |
| 16757 | Hash | 13.04 | 14.95 | 14.6% | operator |
| 16752 | Nested Loop | 154.69 | 1251.76 | 709.2% | operator |
| 16755 | Hash | 4.20 | 15.94 | 279.9% | operator |
| 16758 | Seq Scan | 13.03 | 2.94 | 77.5% | operator |
| 16753 | Seq Scan | 19.47 | 44.37 | 127.9% | operator |
| 16754 | Index Scan | 0.07 | -1.39 | 1951.3% | operator |
| 16756 | Seq Scan | 3.64 | 9.45 | 159.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16753 (Seq Scan) - LEAF
...
### Step 2: Node 16754 (Index Scan) - LEAF
...
### Step 3: Node 16756 (Seq Scan) - LEAF
...
### Step 6: Node 16758 (Seq Scan) - LEAF
...
### Step 9: Node 16747 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** bb930825 (Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)])
- **Consumes:** Nodes 16742, 16743, 16748, 16749, 16750
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 16759 (Index Scan) - LEAF
...
### Step 11: Node 16746 (Nested Loop)
...
### Step 14: Node 16742 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 16743, 16747, 16748, 16749, 16750
- **Input Features:**
