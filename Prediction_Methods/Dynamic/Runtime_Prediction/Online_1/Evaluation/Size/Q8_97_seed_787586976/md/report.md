...
**Test Query:** Q8_97_seed_787586976
...
```
Node 16559 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16560 (Gather Merge) [consumed]
    Node 16561 (Sort)
      Node 16562 (Hash Join)
        Node 16563 (Nested Loop)
          Node 16564 (Hash Join)
            Node 16565 (Nested Loop)
              Node 16566 (Hash Join)
                Node 16567 (Seq Scan) - LEAF
                Node 16568 (Hash) [PATTERN: 444761fb]
                  Node 16569 (Hash Join) [consumed]
                    Node 16570 (Seq Scan) [consumed] - LEAF
                    Node 16571 (Hash) [consumed]
                      Node 16572 (Hash Join) [consumed]
                        Node 16573 (Seq Scan) - LEAF
                        Node 16574 (Hash)
                          Node 16575 (Seq Scan) - LEAF
              Node 16576 (Index Scan) - LEAF
            Node 16577 (Hash)
              Node 16578 (Seq Scan) - LEAF
          Node 16579 (Index Scan) - LEAF
        Node 16580 (Hash)
          Node 16581 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16568 | 16559, 16560, 16569, 16570, 16571, 16572 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16559 | 16560, 16568, 16569, 16570, 16571, 16572 |
...
|------|------|--------|-----------|-----|--------|
| 16559 | Aggregate | 1259.58 | 1133.67 | 10.0% | pattern |
| 16561 | Sort | 1254.22 | 1046.21 | 16.6% | operator |
| 16562 | Hash Join | 1253.98 | 785.61 | 37.4% | operator |
| 16563 | Nested Loop | 1253.68 | 1198.29 | 4.4% | operator |
| 16580 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 16564 | Hash Join | 1249.99 | 785.60 | 37.2% | operator |
| 16579 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16581 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 16565 | Nested Loop | 1203.66 | 1198.29 | 0.4% | operator |
| 16577 | Hash | 41.91 | 19.97 | 52.3% | operator |
| 16566 | Hash Join | 216.15 | 251.67 | 16.4% | operator |
| 16576 | Index Scan | 0.04 | 2.88 | 6599.7% | operator |
| 16578 | Seq Scan | 41.73 | 48.88 | 17.1% | operator |
| 16567 | Seq Scan | 156.94 | 159.67 | 1.7% | operator |
| 16568 | Hash | 41.70 | 43.51 | 4.3% | pattern |
| 16573 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 16574 | Hash | 0.35 | 18.14 | 5040.0% | operator |
| 16575 | Seq Scan | 0.35 | 23.36 | 6594.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16575 (Seq Scan) - LEAF
...
### Step 2: Node 16573 (Seq Scan) - LEAF
...
### Step 4: Node 16567 (Seq Scan) - LEAF
...
### Step 5: Node 16568 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16559, 16560, 16569, 16570, 16571, 16572
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16576 (Index Scan) - LEAF
...
### Step 8: Node 16578 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9713
  - startup_cost=9919.9300
  - total_cost=70271.4000
- **Output:** st=113.62, rt=785.60

### Step 12: Node 16579 (Index Scan) - LEAF
...
### Step 13: Node 16581 (Seq Scan) - LEAF
...
### Step 14: Node 16563 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.2100
  - total_cost=70502.2500
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 16580 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.7700
  - total_cost=70508.0600
- **Output:** st=113.62, rt=785.61

### Step 17: Node 16561 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70544.5900
  - total_cost=70546.5000
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 16559 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16560, 16568, 16569, 16570, 16571, 16572
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2365
  - Aggregate_nt1=2365
  ...
  - Aggregate_startup_cost=71544.6300
  - Aggregate_total_cost=71919.0300
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2365
  - GatherMerge_Outer_nt1=763
  ...
  - GatherMerge_Outer_startup_cost=71544.6300
  - GatherMerge_Outer_total_cost=71824.4300
