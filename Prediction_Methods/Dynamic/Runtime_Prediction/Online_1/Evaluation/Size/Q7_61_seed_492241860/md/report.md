...
**Test Query:** Q7_61_seed_492241860
...
```
Node 12427 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12428 (Gather Merge) [consumed]
    Node 12429 (Sort)
      Node 12430 (Hash Join)
        Node 12431 (Nested Loop)
          Node 12432 (Hash Join) [PATTERN: 895c6e8c]
            Node 12433 (Seq Scan) [consumed] - LEAF
            Node 12434 (Hash) [consumed]
              Node 12435 (Hash Join) [PATTERN: f4cb205a]
                Node 12436 (Seq Scan) [consumed] - LEAF
                Node 12437 (Hash) [consumed]
                  Node 12438 (Seq Scan) [consumed] - LEAF
          Node 12439 (Index Scan) - LEAF
        Node 12440 (Hash)
          Node 12441 (Hash Join) [PATTERN: f4cb205a]
            Node 12442 (Seq Scan) [consumed] - LEAF
            Node 12443 (Hash) [consumed]
              Node 12444 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12435 | 12427, 12428, 12432, 12433, 12434, 12436, 12437, 12438, 12441, 12442, 12443, 12444 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12441 | 12427, 12428, 12432, 12433, 12434, 12435, 12436, 12437, 12438, 12442, 12443, 12444 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12427 | 12428, 12432, 12433, 12434, 12435, 12436, 12437, 12438, 12441, 12442, 12443, 12444 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12432 | 12427, 12428, 12433, 12434, 12435, 12436, 12437, 12438, 12441, 12442, 12443, 12444 |
...
|------|------|--------|-----------|-----|--------|
| 12427 | Aggregate | 1196.51 | 1163.13 | 2.8% | pattern |
| 12429 | Sort | 1189.20 | 1109.16 | 6.7% | operator |
| 12430 | Hash Join | 1188.58 | 843.11 | 29.1% | operator |
| 12431 | Nested Loop | 1181.99 | 1215.99 | 2.9% | operator |
| 12440 | Hash | 2.86 | 27.74 | 869.0% | operator |
| 12432 | Hash Join | 211.32 | 304.89 | 44.3% | pattern |
| 12439 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12441 | Hash Join | 2.82 | 429.36 | 15114.8% | pattern |
| 12435 | Hash Join | 35.32 | 429.38 | 1115.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12435 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12427, 12428, 12432, 12433, 12434, 12436, 12437, 12438, 12441, 12442, 12443, 12444
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12432 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12427, 12428, 12433, 12434, 12435, 12436, 12437, 12438, 12441, 12442, 12443, 12444
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12439 (Index Scan) - LEAF
...
### Step 4: Node 12441 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12427, 12428, 12432, 12433, 12434, 12435, 12436, 12437, 12438, 12442, 12443, 12444
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12427 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12428, 12432, 12433, 12434, 12435, 12436, 12437, 12438, 12441, 12442, 12443, 12444
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
