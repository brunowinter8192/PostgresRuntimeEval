...
**Test Query:** Q8_23_seed_180488682
...
```
Node 14696 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14697 (Gather Merge) [consumed]
    Node 14698 (Sort)
      Node 14699 (Hash Join)
        Node 14700 (Nested Loop)
          Node 14701 (Hash Join)
            Node 14702 (Nested Loop)
              Node 14703 (Hash Join)
                Node 14704 (Seq Scan) - LEAF
                Node 14705 (Hash) [PATTERN: 444761fb]
                  Node 14706 (Hash Join) [consumed]
                    Node 14707 (Seq Scan) [consumed] - LEAF
                    Node 14708 (Hash) [consumed]
                      Node 14709 (Hash Join) [consumed]
                        Node 14710 (Seq Scan) - LEAF
                        Node 14711 (Hash)
                          Node 14712 (Seq Scan) - LEAF
              Node 14713 (Index Scan) - LEAF
            Node 14714 (Hash)
              Node 14715 (Seq Scan) - LEAF
          Node 14716 (Index Scan) - LEAF
        Node 14717 (Hash)
          Node 14718 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14705 | 14696, 14697, 14706, 14707, 14708, 14709 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14696 | 14697, 14705, 14706, 14707, 14708, 14709 |
...
|------|------|--------|-----------|-----|--------|
| 14696 | Aggregate | 1208.48 | 1131.04 | 6.4% | pattern |
| 14698 | Sort | 1203.07 | 1046.21 | 13.0% | operator |
| 14699 | Hash Join | 1202.84 | 785.61 | 34.7% | operator |
| 14700 | Nested Loop | 1202.53 | 1198.29 | 0.4% | operator |
| 14717 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 14701 | Hash Join | 1198.80 | 785.60 | 34.5% | operator |
| 14716 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14718 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 14702 | Nested Loop | 1156.44 | 1198.29 | 3.6% | operator |
| 14714 | Hash | 37.87 | 19.97 | 47.3% | operator |
| 14703 | Hash Join | 213.68 | 251.67 | 17.8% | operator |
| 14713 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14715 | Seq Scan | 37.80 | 48.87 | 29.3% | operator |
| 14704 | Seq Scan | 156.84 | 159.67 | 1.8% | operator |
| 14705 | Hash | 37.74 | 43.51 | 15.3% | pattern |
| 14710 | Seq Scan | 0.01 | 12.22 | 93938.0% | operator |
| 14711 | Hash | 0.35 | 18.14 | 5011.0% | operator |
| 14712 | Seq Scan | 0.35 | 23.36 | 6574.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14712 (Seq Scan) - LEAF
...
### Step 2: Node 14710 (Seq Scan) - LEAF
...
### Step 4: Node 14704 (Seq Scan) - LEAF
...
### Step 5: Node 14705 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14696, 14697, 14706, 14707, 14708, 14709
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14713 (Index Scan) - LEAF
...
### Step 8: Node 14715 (Seq Scan) - LEAF
...
### Step 12: Node 14716 (Index Scan) - LEAF
...
### Step 13: Node 14718 (Seq Scan) - LEAF
...
### Step 14: Node 14700 (Nested Loop)
...
### Step 18: Node 14696 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14697, 14705, 14706, 14707, 14708, 14709
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2458
