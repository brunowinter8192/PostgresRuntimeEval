        ...
        Node 12215 (Nested Loop)
          Node 12216 (Hash Join) [PATTERN: 895c6e8c]
            ...
            Node 12218 (Hash) [consumed]
              Node 12219 (Hash Join) [PATTERN: f4cb205a]
                Node 12220 (Seq Scan) [consumed] - LEAF
                Node 12221 (Hash) [consumed]
                  Node 12222 (Seq Scan) [consumed] - LEAF
          Node 12223 (Index Scan) - LEAF
        Node 12224 (Hash)
          Node 12225 (Hash Join) [PATTERN: f4cb205a]
            Node 12226 (Seq Scan) [consumed] - LEAF
            Node 12227 (Hash) [consumed]
              Node 12228 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12219 | 12211, 12212, 12216, 12217, 12218, 12220, 12221, 12222, 12225, 12226, 12227, 12228 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12225 | 12211, 12212, 12216, 12217, 12218, 12219, 12220, 12221, 12222, 12226, 12227, 12228 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12211 | 12212, 12216, 12217, 12218, 12219, 12220, 12221, 12222, 12225, 12226, 12227, 12228 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12216 | 12211, 12212, 12217, 12218, 12219, 12220, 12221, 12222, 12225, 12226, 12227, 12228 |
...
|------|------|--------|-----------|-----|--------|
| 12211 | Aggregate | 1222.75 | 1163.13 | 4.9% | pattern |
| 12213 | Sort | 1216.05 | 1109.16 | 8.8% | operator |
| 12214 | Hash Join | 1215.40 | 843.11 | 30.6% | operator |
| 12215 | Nested Loop | 1208.68 | 1215.99 | 0.6% | operator |
| 12224 | Hash | 2.93 | 27.74 | 845.9% | operator |
| 12216 | Hash Join | 205.33 | 304.89 | 48.5% | pattern |
| 12223 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12225 | Hash Join | 2.86 | 429.36 | 14897.0% | pattern |
| 12219 | Hash Join | 31.40 | 429.38 | 1267.5% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12219 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12211, 12212, 12216, 12217, 12218, 12220, 12221, 12222, 12225, 12226, 12227, 12228
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12216 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12211, 12212, 12217, 12218, 12219, 12220, 12221, 12222, 12225, 12226, 12227, 12228
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12223 (Index Scan) - LEAF
...
### Step 4: Node 12225 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12211, 12212, 12216, 12217, 12218, 12219, 12220, 12221, 12222, 12226, 12227, 12228
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12211 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12212, 12216, 12217, 12218, 12219, 12220, 12221, 12222, 12225, 12226, 12227, 12228
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
