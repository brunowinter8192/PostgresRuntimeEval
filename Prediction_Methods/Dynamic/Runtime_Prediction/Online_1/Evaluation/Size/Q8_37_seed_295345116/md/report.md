...
**Test Query:** Q8_37_seed_295345116
**Timestamp:** 2026-01-18 18:32:10
...
```
Node 15041 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15042 (Gather Merge) [consumed]
    Node 15043 (Sort)
      Node 15044 (Hash Join)
        Node 15045 (Nested Loop)
          Node 15046 (Hash Join)
            Node 15047 (Nested Loop)
              Node 15048 (Hash Join)
                Node 15049 (Seq Scan) - LEAF
                Node 15050 (Hash) [PATTERN: 444761fb]
                  Node 15051 (Hash Join) [consumed]
                    Node 15052 (Seq Scan) [consumed] - LEAF
                    Node 15053 (Hash) [consumed]
                      Node 15054 (Hash Join) [consumed]
                        Node 15055 (Seq Scan) - LEAF
                        Node 15056 (Hash)
                          Node 15057 (Seq Scan) - LEAF
              Node 15058 (Index Scan) - LEAF
            Node 15059 (Hash)
              Node 15060 (Seq Scan) - LEAF
          Node 15061 (Index Scan) - LEAF
        Node 15062 (Hash)
          Node 15063 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15050 | 15041, 15042, 15051, 15052, 15053, 15054 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15041 | 15042, 15050, 15051, 15052, 15053, 15054 |
...
|------|------|--------|-----------|-----|--------|
| 15041 | Aggregate | 1269.18 | 1130.39 | 10.9% | pattern |
| 15043 | Sort | 1263.36 | 1046.21 | 17.2% | operator |
| 15044 | Hash Join | 1263.12 | 785.61 | 37.8% | operator |
| 15045 | Nested Loop | 1262.82 | 1198.29 | 5.1% | operator |
| 15062 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 15046 | Hash Join | 1258.95 | 785.60 | 37.6% | operator |
| 15061 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15063 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 15047 | Nested Loop | 1213.45 | 1198.29 | 1.2% | operator |
| 15059 | Hash | 41.02 | 19.97 | 51.3% | operator |
| 15048 | Hash Join | 216.40 | 251.67 | 16.3% | operator |
| 15058 | Index Scan | 0.04 | 2.88 | 6599.7% | operator |
| 15060 | Seq Scan | 40.93 | 48.87 | 19.4% | operator |
| 15049 | Seq Scan | 162.03 | 159.67 | 1.5% | operator |
| 15050 | Hash | 35.18 | 43.51 | 23.7% | pattern |
| 15055 | Seq Scan | 0.01 | 12.22 | 111035.8% | operator |
| 15056 | Hash | 0.15 | 18.14 | 12243.0% | operator |
| 15057 | Seq Scan | 0.14 | 23.36 | 16123.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15057 (Seq Scan) - LEAF
...
### Step 2: Node 15055 (Seq Scan) - LEAF
...
### Step 4: Node 15049 (Seq Scan) - LEAF
...
### Step 5: Node 15050 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15041, 15042, 15051, 15052, 15053, 15054
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15058 (Index Scan) - LEAF
...
### Step 8: Node 15060 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9693
  - startup_cost=9920.2300
  - total_cost=70271.7000
- **Output:** st=113.62, rt=785.60

### Step 12: Node 15061 (Index Scan) - LEAF
...
### Step 13: Node 15063 (Seq Scan) - LEAF
...
### Step 14: Node 15045 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.5100
  - total_cost=70512.8300
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15062 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.0700
  - total_cost=70518.8400
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15043 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70557.2400
  - total_cost=70559.2400
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 15041 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15042, 15050, 15051, 15052, 15053, 15054
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2471
  ...
  - Aggregate_sel=0.9737
  - Aggregate_startup_cost=71557.2800
  - Aggregate_total_cost=71947.1600
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2471
  - GatherMerge_Outer_nt1=797
  ...
  - GatherMerge_Outer_startup_cost=71557.2800
  - GatherMerge_Outer_total_cost=71849.6200
- **Output:** st=1127.42, rt=1130.39
