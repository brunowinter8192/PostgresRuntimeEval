...
**Test Query:** Q5_49_seed_393793488
...
```
Node 8761 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8762 (Aggregate) [consumed]
    Node 8763 (Gather Merge) [consumed]
      Node 8764 (Aggregate)
        Node 8765 (Sort) [PATTERN: 3e2d5a00]
          Node 8766 (Hash Join) [consumed]
            Node 8767 (Nested Loop)
              Node 8768 (Hash Join)
                Node 8769 (Seq Scan) - LEAF
                Node 8770 (Hash) [PATTERN: 444761fb]
                  Node 8771 (Hash Join) [consumed]
                    Node 8772 (Seq Scan) [consumed] - LEAF
                    Node 8773 (Hash) [consumed]
                      Node 8774 (Hash Join) [consumed]
                        Node 8775 (Seq Scan) - LEAF
                        Node 8776 (Hash)
                          Node 8777 (Seq Scan) - LEAF
              Node 8778 (Index Scan) - LEAF
            Node 8779 (Hash)
              Node 8780 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8770 | 8761, 8762, 8763, 8765, 8766, 8771, 8772, 8773, 8774 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8761 | 8762, 8763, 8765, 8766, 8770, 8771, 8772, 8773, 8774 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8765 | 8761, 8762, 8763, 8766, 8770, 8771, 8772, 8773, 8774 |
...
|------|------|--------|-----------|-----|--------|
| 8761 | Sort | 1059.63 | 2934.42 | 176.9% | pattern |
| 8764 | Aggregate | 1054.08 | 1128.79 | 7.1% | operator |
| 8765 | Sort | 1053.81 | 1835.90 | 74.2% | pattern |
| 8767 | Nested Loop | 1041.11 | 1188.37 | 14.1% | operator |
| 8779 | Hash | 3.71 | 18.67 | 403.3% | operator |
| 8768 | Hash Join | 218.07 | 253.07 | 16.0% | operator |
| 8778 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 8780 | Seq Scan | 3.14 | 12.36 | 293.4% | operator |
| 8769 | Seq Scan | 169.08 | 159.81 | 5.5% | operator |
| 8770 | Hash | 35.52 | 46.85 | 31.9% | pattern |
| 8775 | Seq Scan | 0.01 | 11.81 | 107247.9% | operator |
| 8776 | Hash | 0.05 | 21.05 | 43760.9% | operator |
| 8777 | Seq Scan | 0.05 | 22.93 | 49755.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8777 (Seq Scan) - LEAF
...
### Step 2: Node 8775 (Seq Scan) - LEAF
...
### Step 4: Node 8769 (Seq Scan) - LEAF
...
### Step 5: Node 8770 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8761, 8762, 8763, 8765, 8766, 8771, 8772, 8773, 8774
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8778 (Index Scan) - LEAF
...
### Step 8: Node 8780 (Seq Scan) - LEAF
...
### Step 11: Node 8765 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8761, 8762, 8763, 8766, 8770, 8771, 8772, 8773, 8774
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8761 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8762, 8763, 8765, 8766, 8770, 8771, 8772, 8773, 8774
- **Input Features:**
