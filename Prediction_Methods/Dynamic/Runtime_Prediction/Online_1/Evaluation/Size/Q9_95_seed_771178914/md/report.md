...
**Test Query:** Q9_95_seed_771178914
...
```
Node 19262 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 19263 (Aggregate) [consumed]
    Node 19264 (Gather)
      Node 19265 (Aggregate)
        Node 19266 (Nested Loop)
          Node 19267 (Hash Join) [PATTERN: bb930825]
            Node 19268 (Seq Scan) [consumed] - LEAF
            Node 19269 (Hash) [consumed]
              Node 19270 (Hash Join) [consumed]
                Node 19271 (Hash Join)
                  Node 19272 (Nested Loop)
                    Node 19273 (Seq Scan) - LEAF
                    Node 19274 (Index Scan) - LEAF
                  Node 19275 (Hash)
                    Node 19276 (Seq Scan) - LEAF
                Node 19277 (Hash)
                  Node 19278 (Seq Scan) - LEAF
          Node 19279 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | bb930825 | 19267 | 19262, 19263, 19268, 19269, 19270 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 19262 | 19263, 19267, 19268, 19269, 19270 |
...
|------|------|--------|-----------|-----|--------|
| 19262 | Sort | 1170.03 | 1049.52 | 10.3% | pattern |
| 19264 | Gather | 1168.65 | 941.00 | 19.5% | operator |
| 19265 | Aggregate | 1150.34 | 1141.96 | 0.7% | operator |
| 19266 | Nested Loop | 1126.49 | 1252.17 | 11.2% | operator |
| 19267 | Hash Join | 957.44 | 134.80 | 85.9% | pattern |
| 19279 | Index Scan | 0.00 | -5.54 | 184812.7% | operator |
| 19271 | Hash Join | 158.76 | 837.31 | 427.4% | operator |
| 19277 | Hash | 15.39 | 14.95 | 2.8% | operator |
| 19272 | Nested Loop | 152.81 | 1251.76 | 719.2% | operator |
| 19275 | Hash | 3.69 | 15.94 | 331.8% | operator |
| 19278 | Seq Scan | 15.38 | 2.94 | 80.9% | operator |
| 19273 | Seq Scan | 19.53 | 44.65 | 128.6% | operator |
| 19274 | Index Scan | 0.07 | -1.39 | 1951.2% | operator |
| 19276 | Seq Scan | 3.12 | 9.45 | 202.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19273 (Seq Scan) - LEAF
...
### Step 2: Node 19274 (Index Scan) - LEAF
...
### Step 3: Node 19276 (Seq Scan) - LEAF
...
### Step 6: Node 19278 (Seq Scan) - LEAF
...
### Step 9: Node 19267 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** bb930825 (Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)])
- **Consumes:** Nodes 19262, 19263, 19268, 19269, 19270
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 19279 (Index Scan) - LEAF
...
### Step 11: Node 19266 (Nested Loop)
...
### Step 14: Node 19262 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 19263, 19267, 19268, 19269, 19270
- **Input Features:**
