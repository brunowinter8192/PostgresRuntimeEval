...
**Test Query:** Q7_45_seed_360977364
...
```
Node 12103 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12104 (Gather Merge) [consumed]
    Node 12105 (Sort)
      Node 12106 (Hash Join)
        Node 12107 (Nested Loop)
          Node 12108 (Hash Join) [PATTERN: 895c6e8c]
            Node 12109 (Seq Scan) [consumed] - LEAF
            Node 12110 (Hash) [consumed]
              Node 12111 (Hash Join) [PATTERN: f4cb205a]
                Node 12112 (Seq Scan) [consumed] - LEAF
                Node 12113 (Hash) [consumed]
                  Node 12114 (Seq Scan) [consumed] - LEAF
          Node 12115 (Index Scan) - LEAF
        Node 12116 (Hash)
          Node 12117 (Hash Join) [PATTERN: f4cb205a]
            Node 12118 (Seq Scan) [consumed] - LEAF
            Node 12119 (Hash) [consumed]
              Node 12120 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12111 | 12103, 12104, 12108, 12109, 12110, 12112, 12113, 12114, 12117, 12118, 12119, 12120 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12117 | 12103, 12104, 12108, 12109, 12110, 12111, 12112, 12113, 12114, 12118, 12119, 12120 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12103 | 12104, 12108, 12109, 12110, 12111, 12112, 12113, 12114, 12117, 12118, 12119, 12120 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12108 | 12103, 12104, 12109, 12110, 12111, 12112, 12113, 12114, 12117, 12118, 12119, 12120 |
...
|------|------|--------|-----------|-----|--------|
| 12103 | Aggregate | 1223.46 | 1163.13 | 4.9% | pattern |
| 12105 | Sort | 1216.40 | 1109.16 | 8.8% | operator |
| 12106 | Hash Join | 1215.76 | 843.11 | 30.7% | operator |
| 12107 | Nested Loop | 1206.94 | 1215.99 | 0.8% | operator |
| 12116 | Hash | 5.03 | 27.74 | 452.1% | operator |
| 12108 | Hash Join | 216.15 | 304.89 | 41.1% | pattern |
| 12115 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12117 | Hash Join | 4.98 | 429.36 | 8521.7% | pattern |
| 12111 | Hash Join | 38.36 | 429.38 | 1019.4% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12111 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12103, 12104, 12108, 12109, 12110, 12112, 12113, 12114, 12117, 12118, 12119, 12120
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12108 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12103, 12104, 12109, 12110, 12111, 12112, 12113, 12114, 12117, 12118, 12119, 12120
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12115 (Index Scan) - LEAF
...
### Step 4: Node 12117 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12103, 12104, 12108, 12109, 12110, 12111, 12112, 12113, 12114, 12118, 12119, 12120
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12103 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12104, 12108, 12109, 12110, 12111, 12112, 12113, 12114, 12117, 12118, 12119, 12120
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
