...
**Test Query:** Q8_132_seed_1074728061
...
```
Node 14006 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14007 (Gather Merge) [consumed]
    Node 14008 (Sort)
      Node 14009 (Hash Join)
        Node 14010 (Nested Loop)
          Node 14011 (Hash Join)
            Node 14012 (Nested Loop)
              Node 14013 (Hash Join)
                Node 14014 (Seq Scan) - LEAF
                Node 14015 (Hash) [PATTERN: 444761fb]
                  Node 14016 (Hash Join) [consumed]
                    Node 14017 (Seq Scan) [consumed] - LEAF
                    Node 14018 (Hash) [consumed]
                      Node 14019 (Hash Join) [consumed]
                        Node 14020 (Seq Scan) - LEAF
                        Node 14021 (Hash)
                          Node 14022 (Seq Scan) - LEAF
              Node 14023 (Index Scan) - LEAF
            Node 14024 (Hash)
              Node 14025 (Seq Scan) - LEAF
          Node 14026 (Index Scan) - LEAF
        Node 14027 (Hash)
          Node 14028 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14015 | 14006, 14007, 14016, 14017, 14018, 14019 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14006 | 14007, 14015, 14016, 14017, 14018, 14019 |
...
|------|------|--------|-----------|-----|--------|
| 14006 | Aggregate | 1204.22 | 1133.65 | 5.9% | pattern |
| 14008 | Sort | 1199.27 | 1046.21 | 12.8% | operator |
| 14009 | Hash Join | 1199.01 | 785.61 | 34.5% | operator |
| 14010 | Nested Loop | 1198.69 | 1198.29 | 0.0% | operator |
| 14027 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 14011 | Hash Join | 1194.78 | 785.60 | 34.2% | operator |
| 14026 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14028 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 14012 | Nested Loop | 1149.29 | 1198.29 | 4.3% | operator |
| 14024 | Hash | 41.05 | 19.97 | 51.4% | operator |
| 14013 | Hash Join | 207.11 | 251.67 | 21.5% | operator |
| 14023 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14025 | Seq Scan | 40.93 | 48.88 | 19.4% | operator |
| 14014 | Seq Scan | 156.38 | 159.67 | 2.1% | operator |
| 14015 | Hash | 32.86 | 43.51 | 32.4% | pattern |
| 14020 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14021 | Hash | 0.13 | 18.14 | 13750.5% | operator |
| 14022 | Seq Scan | 0.13 | 23.36 | 18151.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14022 (Seq Scan) - LEAF
...
### Step 2: Node 14020 (Seq Scan) - LEAF
...
### Step 4: Node 14014 (Seq Scan) - LEAF
...
### Step 5: Node 14015 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14006, 14007, 14016, 14017, 14018, 14019
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14023 (Index Scan) - LEAF
...
### Step 8: Node 14025 (Seq Scan) - LEAF
...
### Step 12: Node 14026 (Index Scan) - LEAF
...
### Step 13: Node 14028 (Seq Scan) - LEAF
...
### Step 14: Node 14010 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70500.6800
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14027 (Hash)
...
- **Input Features:**
  ...
  - total_cost=70506.4600
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14008 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70542.7200
  - total_cost=70544.6100
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14006 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14007, 14015, 14016, 14017, 14018, 14019
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2351
  - Aggregate_nt1=2351
  ...
  - Aggregate_startup_cost=71542.7600
  - Aggregate_total_cost=71914.9400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2351
  - GatherMerge_Outer_nt1=758
  ...
  - GatherMerge_Outer_sel=3.1016
  - GatherMerge_Outer_startup_cost=71542.7600
  - GatherMerge_Outer_total_cost=71820.9000
