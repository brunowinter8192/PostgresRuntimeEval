...
**Test Query:** Q5_103_seed_836811162
...
```
Node 6961 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 6962 (Aggregate) [consumed]
    Node 6963 (Gather Merge) [consumed]
      Node 6964 (Aggregate)
        Node 6965 (Sort) [PATTERN: 3e2d5a00]
          Node 6966 (Hash Join) [consumed]
            Node 6967 (Nested Loop)
              Node 6968 (Hash Join)
                Node 6969 (Seq Scan) - LEAF
                Node 6970 (Hash) [PATTERN: 444761fb]
                  Node 6971 (Hash Join) [consumed]
                    Node 6972 (Seq Scan) [consumed] - LEAF
                    Node 6973 (Hash) [consumed]
                      Node 6974 (Hash Join) [consumed]
                        Node 6975 (Seq Scan) - LEAF
                        Node 6976 (Hash)
                          Node 6977 (Seq Scan) - LEAF
              Node 6978 (Index Scan) - LEAF
            Node 6979 (Hash)
              Node 6980 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 6970 | 6961, 6962, 6963, 6965, 6966, 6971, 6972, 6973, 6974 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 6961 | 6962, 6963, 6965, 6966, 6970, 6971, 6972, 6973, 6974 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 6965 | 6961, 6962, 6963, 6966, 6970, 6971, 6972, 6973, 6974 |
...
|------|------|--------|-----------|-----|--------|
| 6961 | Sort | 1083.33 | 2934.42 | 170.9% | pattern |
| 6964 | Aggregate | 1078.22 | 1128.76 | 4.7% | operator |
| 6965 | Sort | 1077.97 | 1835.93 | 70.3% | pattern |
| 6967 | Nested Loop | 1066.69 | 1188.47 | 11.4% | operator |
| 6979 | Hash | 2.75 | 18.67 | 578.2% | operator |
| 6968 | Hash Join | 221.91 | 253.07 | 14.0% | operator |
| 6978 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 6980 | Seq Scan | 2.17 | 12.36 | 469.3% | operator |
| 6969 | Seq Scan | 171.56 | 159.81 | 6.8% | operator |
| 6970 | Hash | 37.83 | 46.85 | 23.8% | pattern |
| 6975 | Seq Scan | 0.02 | 11.81 | 53574.0% | operator |
| 6976 | Hash | 0.15 | 21.05 | 14320.0% | operator |
| 6977 | Seq Scan | 0.14 | 22.93 | 16050.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 6977 (Seq Scan) - LEAF
...
### Step 2: Node 6975 (Seq Scan) - LEAF
...
### Step 4: Node 6969 (Seq Scan) - LEAF
...
### Step 5: Node 6970 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 6961, 6962, 6963, 6965, 6966, 6971, 6972, 6973, 6974
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 6978 (Index Scan) - LEAF
...
### Step 8: Node 6980 (Seq Scan) - LEAF
...
### Step 11: Node 6965 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 6961, 6962, 6963, 6966, 6970, 6971, 6972, 6973, 6974
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 6961 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 6962, 6963, 6965, 6966, 6970, 6971, 6972, 6973, 6974
- **Input Features:**
