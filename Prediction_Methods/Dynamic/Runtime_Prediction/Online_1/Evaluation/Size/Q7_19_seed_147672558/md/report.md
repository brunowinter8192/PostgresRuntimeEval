...
**Test Query:** Q7_19_seed_147672558
...
```
Node 11581 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11582 (Gather Merge) [consumed]
    Node 11583 (Sort)
      Node 11584 (Hash Join)
        Node 11585 (Nested Loop)
          Node 11586 (Hash Join) [PATTERN: 895c6e8c]
            Node 11587 (Seq Scan) [consumed] - LEAF
            Node 11588 (Hash) [consumed]
              Node 11589 (Hash Join) [PATTERN: f4cb205a]
                Node 11590 (Seq Scan) [consumed] - LEAF
                Node 11591 (Hash) [consumed]
                  Node 11592 (Seq Scan) [consumed] - LEAF
          Node 11593 (Index Scan) - LEAF
        Node 11594 (Hash)
          Node 11595 (Hash Join) [PATTERN: f4cb205a]
            Node 11596 (Seq Scan) [consumed] - LEAF
            Node 11597 (Hash) [consumed]
              Node 11598 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11589 | 11581, 11582, 11586, 11587, 11588, 11590, 11591, 11592, 11595, 11596, 11597, 11598 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11595 | 11581, 11582, 11586, 11587, 11588, 11589, 11590, 11591, 11592, 11596, 11597, 11598 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11581 | 11582, 11586, 11587, 11588, 11589, 11590, 11591, 11592, 11595, 11596, 11597, 11598 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11586 | 11581, 11582, 11587, 11588, 11589, 11590, 11591, 11592, 11595, 11596, 11597, 11598 |
...
|------|------|--------|-----------|-----|--------|
| 11581 | Aggregate | 1175.27 | 1163.13 | 1.0% | pattern |
| 11583 | Sort | 1168.14 | 1109.16 | 5.0% | operator |
| 11584 | Hash Join | 1167.49 | 843.11 | 27.8% | operator |
| 11585 | Nested Loop | 1160.25 | 1215.99 | 4.8% | operator |
| 11594 | Hash | 3.38 | 27.74 | 721.0% | operator |
| 11586 | Hash Join | 211.05 | 304.89 | 44.5% | pattern |
| 11593 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 11595 | Hash Join | 3.33 | 429.36 | 12774.4% | pattern |
| 11589 | Hash Join | 36.17 | 429.38 | 1087.1% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11589 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11581, 11582, 11586, 11587, 11588, 11590, 11591, 11592, 11595, 11596, 11597, 11598
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11586 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11581, 11582, 11587, 11588, 11589, 11590, 11591, 11592, 11595, 11596, 11597, 11598
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11593 (Index Scan) - LEAF
...
### Step 4: Node 11595 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11581, 11582, 11586, 11587, 11588, 11589, 11590, 11591, 11592, 11596, 11597, 11598
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11581 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11582, 11586, 11587, 11588, 11589, 11590, 11591, 11592, 11595, 11596, 11597, 11598
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
