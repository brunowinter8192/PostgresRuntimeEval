...
**Test Query:** Q10_145_seed_1181380464
...
```
Node 20007 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20008 (Sort) [consumed]
    Node 20009 (Aggregate) [consumed]
      Node 20010 (Gather) [consumed]
        Node 20011 (Hash Join)
          Node 20012 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20013 (Nested Loop) [consumed]
              Node 20014 (Seq Scan) - LEAF
              Node 20015 (Index Scan) - LEAF
            Node 20016 (Hash) [consumed]
              Node 20017 (Seq Scan) - LEAF
          Node 20018 (Hash)
            Node 20019 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20007 | 20008, 20009, 20010, 20012, 20013, 20016 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20012 | 20007, 20008, 20009, 20010, 20013, 20016 |
...
|------|------|--------|-----------|-----|--------|
| 20007 | Limit | 1162.11 | 1199.47 | 3.2% | pattern |
| 20011 | Hash Join | 1111.21 | 1095.57 | 1.4% | operator |
| 20012 | Hash Join | 1107.59 | 1120.19 | 1.1% | pattern |
| 20018 | Hash | 0.01 | 15.71 | 120752.1% | operator |
| 20019 | Seq Scan | 0.01 | 6.98 | 77441.2% | operator |
| 20014 | Seq Scan | 161.87 | 159.74 | 1.3% | operator |
| 20015 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20017 | Seq Scan | 25.13 | 26.64 | 6.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20014 (Seq Scan) - LEAF
...
### Step 2: Node 20015 (Index Scan) - LEAF
...
### Step 3: Node 20017 (Seq Scan) - LEAF
...
### Step 4: Node 20019 (Seq Scan) - LEAF
...
### Step 5: Node 20012 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20007, 20008, 20009, 20010, 20013, 20016
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18257
  - HashJoin_nt1=18257
...
### Step 8: Node 20007 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20008, 20009, 20010, 20012, 20013, 20016
- **Input Features:**
