...
**Test Query:** Q10_142_seed_1156768371
...
```
Node 19968 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19969 (Sort) [consumed]
    Node 19970 (Aggregate) [consumed]
      Node 19971 (Gather) [consumed]
        Node 19972 (Hash Join)
          Node 19973 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19974 (Nested Loop) [consumed]
              Node 19975 (Seq Scan) - LEAF
              Node 19976 (Index Scan) - LEAF
            Node 19977 (Hash) [consumed]
              Node 19978 (Seq Scan) - LEAF
          Node 19979 (Hash)
            Node 19980 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19968 | 19969, 19970, 19971, 19973, 19974, 19977 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19973 | 19968, 19969, 19970, 19971, 19974, 19977 |
...
|------|------|--------|-----------|-----|--------|
| 19968 | Limit | 1166.59 | 1199.47 | 2.8% | pattern |
| 19972 | Hash Join | 1116.33 | 1095.44 | 1.9% | operator |
| 19973 | Hash Join | 1113.53 | 1120.19 | 0.6% | pattern |
| 19979 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 19980 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 19975 | Seq Scan | 158.89 | 159.75 | 0.5% | operator |
| 19976 | Index Scan | 0.07 | -0.52 | 897.3% | operator |
| 19978 | Seq Scan | 29.30 | 26.64 | 9.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19975 (Seq Scan) - LEAF
...
### Step 2: Node 19976 (Index Scan) - LEAF
...
### Step 3: Node 19978 (Seq Scan) - LEAF
...
### Step 4: Node 19980 (Seq Scan) - LEAF
...
### Step 5: Node 19973 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19968, 19969, 19970, 19971, 19974, 19977
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17761
  - HashJoin_nt1=17761
...
- **Output:** st=54.17, rt=1095.44

### Step 8: Node 19968 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19969, 19970, 19971, 19973, 19974, 19977
- **Input Features:**
