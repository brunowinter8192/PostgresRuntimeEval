...
**Test Query:** Q5_46_seed_369181395
**Timestamp:** 2026-01-18 17:13:48
...
```
Node 8701 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8702 (Aggregate) [consumed]
    Node 8703 (Gather Merge) [consumed]
      Node 8704 (Aggregate)
        Node 8705 (Sort) [PATTERN: 3e2d5a00]
          Node 8706 (Hash Join) [consumed]
            Node 8707 (Nested Loop)
              Node 8708 (Hash Join)
                Node 8709 (Seq Scan) - LEAF
                Node 8710 (Hash) [PATTERN: 444761fb]
                  Node 8711 (Hash Join) [consumed]
                    Node 8712 (Seq Scan) [consumed] - LEAF
                    Node 8713 (Hash) [consumed]
                      Node 8714 (Hash Join) [consumed]
                        Node 8715 (Seq Scan) - LEAF
                        Node 8716 (Hash)
                          Node 8717 (Seq Scan) - LEAF
              Node 8718 (Index Scan) - LEAF
            Node 8719 (Hash)
              Node 8720 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8710 | 8701, 8702, 8703, 8705, 8706, 8711, 8712, 8713, 8714 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8701 | 8702, 8703, 8705, 8706, 8710, 8711, 8712, 8713, 8714 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8705 | 8701, 8702, 8703, 8706, 8710, 8711, 8712, 8713, 8714 |
...
|------|------|--------|-----------|-----|--------|
| 8701 | Sort | 1068.38 | 2934.42 | 174.7% | pattern |
| 8704 | Aggregate | 1063.14 | 1128.78 | 6.2% | operator |
| 8705 | Sort | 1062.88 | 1835.92 | 72.7% | pattern |
| 8707 | Nested Loop | 1050.27 | 1188.42 | 13.2% | operator |
| 8719 | Hash | 3.96 | 18.67 | 371.6% | operator |
| 8708 | Hash Join | 219.99 | 253.07 | 15.0% | operator |
| 8718 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 8720 | Seq Scan | 3.41 | 12.36 | 262.2% | operator |
| 8709 | Seq Scan | 170.09 | 159.81 | 6.0% | operator |
| 8710 | Hash | 37.22 | 46.85 | 25.9% | pattern |
| 8715 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 8716 | Hash | 0.06 | 21.05 | 36198.7% | operator |
| 8717 | Seq Scan | 0.06 | 22.93 | 41597.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8717 (Seq Scan) - LEAF
...
### Step 2: Node 8715 (Seq Scan) - LEAF
...
### Step 4: Node 8709 (Seq Scan) - LEAF
...
### Step 5: Node 8710 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8701, 8702, 8703, 8705, 8706, 8711, 8712, 8713, 8714
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8718 (Index Scan) - LEAF
...
### Step 8: Node 8720 (Seq Scan) - LEAF
...
### Step 11: Node 8705 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8701, 8702, 8703, 8706, 8710, 8711, 8712, 8713, 8714
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8701 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8702, 8703, 8705, 8706, 8710, 8711, 8712, 8713, 8714
- **Input Features:**
