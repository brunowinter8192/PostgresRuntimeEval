...
**Test Query:** Q8_69_seed_557874108
...
```
Node 15846 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15847 (Gather Merge) [consumed]
    Node 15848 (Sort)
      Node 15849 (Hash Join)
        Node 15850 (Nested Loop)
          Node 15851 (Hash Join)
            Node 15852 (Nested Loop)
              Node 15853 (Hash Join)
                Node 15854 (Seq Scan) - LEAF
                Node 15855 (Hash) [PATTERN: 444761fb]
                  Node 15856 (Hash Join) [consumed]
                    Node 15857 (Seq Scan) [consumed] - LEAF
                    Node 15858 (Hash) [consumed]
                      Node 15859 (Hash Join) [consumed]
                        Node 15860 (Seq Scan) - LEAF
                        Node 15861 (Hash)
                          Node 15862 (Seq Scan) - LEAF
              Node 15863 (Index Scan) - LEAF
            Node 15864 (Hash)
              Node 15865 (Seq Scan) - LEAF
          Node 15866 (Index Scan) - LEAF
        Node 15867 (Hash)
          Node 15868 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15855 | 15846, 15847, 15856, 15857, 15858, 15859 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15846 | 15847, 15855, 15856, 15857, 15858, 15859 |
...
|------|------|--------|-----------|-----|--------|
| 15846 | Aggregate | 1209.69 | 1127.83 | 6.8% | pattern |
| 15848 | Sort | 1201.82 | 1046.22 | 12.9% | operator |
| 15849 | Hash Join | 1201.57 | 785.61 | 34.6% | operator |
| 15850 | Nested Loop | 1201.26 | 1198.29 | 0.2% | operator |
| 15867 | Hash | 0.02 | 13.72 | 76144.9% | operator |
| 15851 | Hash Join | 1197.61 | 785.60 | 34.4% | operator |
| 15866 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15868 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 15852 | Nested Loop | 1159.51 | 1198.29 | 3.3% | operator |
| 15864 | Hash | 33.17 | 19.97 | 39.8% | operator |
| 15853 | Hash Join | 204.57 | 251.67 | 23.0% | operator |
| 15863 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 15865 | Seq Scan | 33.02 | 48.86 | 48.0% | operator |
| 15854 | Seq Scan | 154.09 | 159.67 | 3.6% | operator |
| 15855 | Hash | 32.33 | 43.51 | 34.6% | pattern |
| 15860 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 15861 | Hash | 0.06 | 18.14 | 30652.8% | operator |
| 15862 | Seq Scan | 0.06 | 23.36 | 41618.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15862 (Seq Scan) - LEAF
...
### Step 2: Node 15860 (Seq Scan) - LEAF
...
### Step 4: Node 15854 (Seq Scan) - LEAF
...
### Step 5: Node 15855 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15846, 15847, 15856, 15857, 15858, 15859
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15863 (Index Scan) - LEAF
...
### Step 8: Node 15865 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9683
  - startup_cost=9920.3800
  - total_cost=70271.8500
- **Output:** st=113.62, rt=785.60

### Step 12: Node 15866 (Index Scan) - LEAF
...
### Step 13: Node 15868 (Seq Scan) - LEAF
...
### Step 14: Node 15850 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.6600
  - total_cost=70518.1300
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15867 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.2200
  - total_cost=70524.2300
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15848 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70563.5800
  - total_cost=70565.6100
- **Output:** st=1044.95, rt=1046.22

### Step 18: Node 15846 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15847, 15855, 15856, 15857, 15858, 15859
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2524
  ...
  - Aggregate_sel=0.9532
  - Aggregate_startup_cost=71563.6200
  - Aggregate_total_cost=71960.8200
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2524
  - GatherMerge_Outer_nt1=814
  ...
  - GatherMerge_Outer_startup_cost=71563.6200
  - GatherMerge_Outer_total_cost=71862.2200
- **Output:** st=1124.89, rt=1127.83
