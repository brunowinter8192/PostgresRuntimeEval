...
**Test Query:** Q8_16_seed_123060465
...
```
Node 14512 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14513 (Gather Merge) [consumed]
    Node 14514 (Sort)
      Node 14515 (Hash Join)
        Node 14516 (Nested Loop)
          Node 14517 (Hash Join)
            Node 14518 (Nested Loop)
              Node 14519 (Hash Join)
                Node 14520 (Seq Scan) - LEAF
                Node 14521 (Hash) [PATTERN: 444761fb]
                  Node 14522 (Hash Join) [consumed]
                    Node 14523 (Seq Scan) [consumed] - LEAF
                    Node 14524 (Hash) [consumed]
                      Node 14525 (Hash Join) [consumed]
                        Node 14526 (Seq Scan) - LEAF
                        Node 14527 (Hash)
                          Node 14528 (Seq Scan) - LEAF
              Node 14529 (Index Scan) - LEAF
            Node 14530 (Hash)
              Node 14531 (Seq Scan) - LEAF
          Node 14532 (Index Scan) - LEAF
        Node 14533 (Hash)
          Node 14534 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14521 | 14512, 14513, 14522, 14523, 14524, 14525 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14512 | 14513, 14521, 14522, 14523, 14524, 14525 |
...
|------|------|--------|-----------|-----|--------|
| 14512 | Aggregate | 1217.10 | 1130.30 | 7.1% | pattern |
| 14514 | Sort | 1209.79 | 1046.21 | 13.5% | operator |
| 14515 | Hash Join | 1209.53 | 785.61 | 35.0% | operator |
| 14516 | Nested Loop | 1207.83 | 1198.29 | 0.8% | operator |
| 14533 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 14517 | Hash Join | 1203.96 | 785.60 | 34.7% | operator |
| 14532 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14534 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 14518 | Nested Loop | 1161.59 | 1198.29 | 3.2% | operator |
| 14530 | Hash | 37.77 | 19.97 | 47.1% | operator |
| 14519 | Hash Join | 213.06 | 251.67 | 18.1% | operator |
| 14529 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14531 | Seq Scan | 37.65 | 48.87 | 29.8% | operator |
| 14520 | Seq Scan | 158.30 | 159.67 | 0.9% | operator |
| 14521 | Hash | 36.98 | 43.51 | 17.7% | pattern |
| 14526 | Seq Scan | 0.01 | 12.22 | 81399.6% | operator |
| 14527 | Hash | 0.43 | 18.14 | 4159.2% | operator |
| 14528 | Seq Scan | 0.42 | 23.36 | 5436.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14528 (Seq Scan) - LEAF
...
### Step 2: Node 14526 (Seq Scan) - LEAF
...
### Step 4: Node 14520 (Seq Scan) - LEAF
...
### Step 5: Node 14521 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14512, 14513, 14522, 14523, 14524, 14525
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14529 (Index Scan) - LEAF
...
### Step 8: Node 14531 (Seq Scan) - LEAF
...
### Step 12: Node 14532 (Index Scan) - LEAF
...
### Step 13: Node 14534 (Seq Scan) - LEAF
...
### Step 14: Node 14516 (Nested Loop)
...
### Step 18: Node 14512 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14513, 14521, 14522, 14523, 14524, 14525
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2473
