...
**Test Query:** Q10_66_seed_533262015
...
```
Node 20826 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20827 (Sort) [consumed]
    Node 20828 (Aggregate) [consumed]
      Node 20829 (Gather) [consumed]
        Node 20830 (Hash Join)
          Node 20831 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20832 (Nested Loop) [consumed]
              Node 20833 (Seq Scan) - LEAF
              Node 20834 (Index Scan) - LEAF
            Node 20835 (Hash) [consumed]
              Node 20836 (Seq Scan) - LEAF
          Node 20837 (Hash)
            Node 20838 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20826 | 20827, 20828, 20829, 20831, 20832, 20835 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20831 | 20826, 20827, 20828, 20829, 20832, 20835 |
...
|------|------|--------|-----------|-----|--------|
| 20826 | Limit | 1150.80 | 1199.47 | 4.2% | pattern |
| 20830 | Hash Join | 1104.68 | 1095.57 | 0.8% | operator |
| 20831 | Hash Join | 1101.68 | 1120.19 | 1.7% | pattern |
| 20837 | Hash | 0.02 | 15.71 | 82588.3% | operator |
| 20838 | Seq Scan | 0.02 | 6.98 | 43516.9% | operator |
| 20833 | Seq Scan | 167.43 | 159.74 | 4.6% | operator |
| 20834 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20836 | Seq Scan | 26.98 | 26.64 | 1.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20833 (Seq Scan) - LEAF
...
### Step 2: Node 20834 (Index Scan) - LEAF
...
### Step 3: Node 20836 (Seq Scan) - LEAF
...
### Step 4: Node 20838 (Seq Scan) - LEAF
...
### Step 5: Node 20831 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20826, 20827, 20828, 20829, 20832, 20835
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18240
  - HashJoin_nt1=18240
...
### Step 8: Node 20826 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20827, 20828, 20829, 20831, 20832, 20835
- **Input Features:**
