...
```
Node 16628 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16629 (Gather Merge) [consumed]
    Node 16630 (Sort)
      Node 16631 (Hash Join)
        Node 16632 (Nested Loop)
          Node 16633 (Hash Join)
            Node 16634 (Nested Loop)
              Node 16635 (Hash Join)
                Node 16636 (Seq Scan) - LEAF
                Node 16637 (Hash) [PATTERN: 444761fb]
                  Node 16638 (Hash Join) [consumed]
                    Node 16639 (Seq Scan) [consumed] - LEAF
                    Node 16640 (Hash) [consumed]
                      Node 16641 (Hash Join) [consumed]
                        Node 16642 (Seq Scan) - LEAF
                        Node 16643 (Hash)
                          Node 16644 (Seq Scan) - LEAF
              Node 16645 (Index Scan) - LEAF
            Node 16646 (Hash)
              Node 16647 (Seq Scan) - LEAF
          Node 16648 (Index Scan) - LEAF
        Node 16649 (Hash)
          Node 16650 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16637 | 16628, 16629, 16638, 16639, 16640, 16641 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16628 | 16629, 16637, 16638, 16639, 16640, 16641 |
...
|------|------|--------|-----------|-----|--------|
| 16628 | Aggregate | 1237.91 | 1128.88 | 8.8% | pattern |
| 16630 | Sort | 1230.99 | 1046.21 | 15.0% | operator |
| 16631 | Hash Join | 1230.72 | 785.61 | 36.2% | operator |
| 16632 | Nested Loop | 1230.41 | 1198.29 | 2.6% | operator |
| 16649 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 16633 | Hash Join | 1226.17 | 785.60 | 35.9% | operator |
| 16648 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16650 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 16634 | Nested Loop | 1182.58 | 1198.29 | 1.3% | operator |
| 16646 | Hash | 38.64 | 19.97 | 48.3% | operator |
| 16635 | Hash Join | 217.50 | 251.67 | 15.7% | operator |
| 16645 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 16647 | Seq Scan | 38.47 | 48.87 | 27.0% | operator |
| 16636 | Seq Scan | 161.18 | 159.67 | 0.9% | operator |
| 16637 | Hash | 37.82 | 43.51 | 15.0% | pattern |
| 16642 | Seq Scan | 0.01 | 12.22 | 111035.8% | operator |
| 16643 | Hash | 0.60 | 18.14 | 2929.1% | operator |
| 16644 | Seq Scan | 0.59 | 23.36 | 3826.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16644 (Seq Scan) - LEAF
...
### Step 2: Node 16642 (Seq Scan) - LEAF
...
### Step 4: Node 16636 (Seq Scan) - LEAF
...
### Step 5: Node 16637 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16628, 16629, 16638, 16639, 16640, 16641
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16645 (Index Scan) - LEAF
...
### Step 8: Node 16647 (Seq Scan) - LEAF
...
### Step 12: Node 16648 (Index Scan) - LEAF
...
### Step 13: Node 16650 (Seq Scan) - LEAF
...
### Step 14: Node 16632 (Nested Loop)
...
### Step 18: Node 16628 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16629, 16637, 16638, 16639, 16640, 16641
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2502
