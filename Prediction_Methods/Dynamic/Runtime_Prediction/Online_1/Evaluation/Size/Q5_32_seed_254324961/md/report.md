...
**Test Query:** Q5_32_seed_254324961
...
```
Node 8401 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8402 (Aggregate) [consumed]
    Node 8403 (Gather Merge) [consumed]
      Node 8404 (Aggregate)
        Node 8405 (Sort) [PATTERN: 3e2d5a00]
          Node 8406 (Hash Join) [consumed]
            Node 8407 (Nested Loop)
              Node 8408 (Hash Join)
                Node 8409 (Seq Scan) - LEAF
                Node 8410 (Hash) [PATTERN: 444761fb]
                  Node 8411 (Hash Join) [consumed]
                    Node 8412 (Seq Scan) [consumed] - LEAF
                    Node 8413 (Hash) [consumed]
                      Node 8414 (Hash Join) [consumed]
                        Node 8415 (Seq Scan) - LEAF
                        Node 8416 (Hash)
                          Node 8417 (Seq Scan) - LEAF
              Node 8418 (Index Scan) - LEAF
            Node 8419 (Hash)
              Node 8420 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8410 | 8401, 8402, 8403, 8405, 8406, 8411, 8412, 8413, 8414 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8401 | 8402, 8403, 8405, 8406, 8410, 8411, 8412, 8413, 8414 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8405 | 8401, 8402, 8403, 8406, 8410, 8411, 8412, 8413, 8414 |
...
|------|------|--------|-----------|-----|--------|
| 8401 | Sort | 1078.57 | 2934.42 | 172.1% | pattern |
| 8404 | Aggregate | 1073.43 | 1128.78 | 5.2% | operator |
| 8405 | Sort | 1073.17 | 1835.92 | 71.1% | pattern |
| 8407 | Nested Loop | 1060.21 | 1188.42 | 12.1% | operator |
| 8419 | Hash | 4.45 | 18.67 | 319.9% | operator |
| 8408 | Hash Join | 223.67 | 253.07 | 13.1% | operator |
| 8418 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 8420 | Seq Scan | 3.94 | 12.36 | 214.1% | operator |
| 8409 | Seq Scan | 171.24 | 159.81 | 6.7% | operator |
| 8410 | Hash | 39.11 | 46.85 | 19.8% | pattern |
| 8415 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 8416 | Hash | 0.05 | 21.05 | 38887.5% | operator |
| 8417 | Seq Scan | 0.05 | 22.93 | 44867.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8417 (Seq Scan) - LEAF
...
### Step 2: Node 8415 (Seq Scan) - LEAF
...
### Step 4: Node 8409 (Seq Scan) - LEAF
...
### Step 5: Node 8410 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8401, 8402, 8403, 8405, 8406, 8411, 8412, 8413, 8414
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8418 (Index Scan) - LEAF
...
### Step 8: Node 8420 (Seq Scan) - LEAF
...
### Step 11: Node 8405 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8401, 8402, 8403, 8406, 8410, 8411, 8412, 8413, 8414
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8401 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8402, 8403, 8405, 8406, 8410, 8411, 8412, 8413, 8414
- **Input Features:**
