...
**Test Query:** Q7_105_seed_853219224
...
```
Node 10591 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10592 (Gather Merge) [consumed]
    Node 10593 (Sort)
      Node 10594 (Hash Join)
        Node 10595 (Nested Loop)
          Node 10596 (Hash Join) [PATTERN: 895c6e8c]
            Node 10597 (Seq Scan) [consumed] - LEAF
            Node 10598 (Hash) [consumed]
              Node 10599 (Hash Join) [PATTERN: f4cb205a]
                Node 10600 (Seq Scan) [consumed] - LEAF
                Node 10601 (Hash) [consumed]
                  Node 10602 (Seq Scan) [consumed] - LEAF
          Node 10603 (Index Scan) - LEAF
        Node 10604 (Hash)
          Node 10605 (Hash Join) [PATTERN: f4cb205a]
            Node 10606 (Seq Scan) [consumed] - LEAF
            Node 10607 (Hash) [consumed]
              Node 10608 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10599 | 10591, 10592, 10596, 10597, 10598, 10600, 10601, 10602, 10605, 10606, 10607, 10608 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10605 | 10591, 10592, 10596, 10597, 10598, 10599, 10600, 10601, 10602, 10606, 10607, 10608 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10591 | 10592, 10596, 10597, 10598, 10599, 10600, 10601, 10602, 10605, 10606, 10607, 10608 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10596 | 10591, 10592, 10597, 10598, 10599, 10600, 10601, 10602, 10605, 10606, 10607, 10608 |
...
|------|------|--------|-----------|-----|--------|
| 10591 | Aggregate | 1213.90 | 1163.13 | 4.2% | pattern |
| 10593 | Sort | 1207.03 | 1109.16 | 8.1% | operator |
| 10594 | Hash Join | 1206.41 | 843.11 | 30.1% | operator |
| 10595 | Nested Loop | 1199.89 | 1215.99 | 1.3% | operator |
| 10604 | Hash | 2.70 | 27.74 | 927.9% | operator |
| 10596 | Hash Join | 217.07 | 304.89 | 40.5% | pattern |
| 10603 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10605 | Hash Join | 2.66 | 429.36 | 16053.6% | pattern |
| 10599 | Hash Join | 34.91 | 429.38 | 1129.9% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10599 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10591, 10592, 10596, 10597, 10598, 10600, 10601, 10602, 10605, 10606, 10607, 10608
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10596 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10591, 10592, 10597, 10598, 10599, 10600, 10601, 10602, 10605, 10606, 10607, 10608
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10603 (Index Scan) - LEAF
...
### Step 4: Node 10605 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10591, 10592, 10596, 10597, 10598, 10599, 10600, 10601, 10602, 10606, 10607, 10608
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10591 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10592, 10596, 10597, 10598, 10599, 10600, 10601, 10602, 10605, 10606, 10607, 10608
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
