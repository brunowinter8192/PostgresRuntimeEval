...
**Test Query:** Q5_1_seed_0101000000
...
```
Node 8121 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8122 (Aggregate) [consumed]
    Node 8123 (Gather Merge) [consumed]
      Node 8124 (Aggregate)
        Node 8125 (Sort) [PATTERN: 3e2d5a00]
          Node 8126 (Hash Join) [consumed]
            Node 8127 (Nested Loop)
              Node 8128 (Hash Join)
                Node 8129 (Seq Scan) - LEAF
                Node 8130 (Hash) [PATTERN: 444761fb]
                  Node 8131 (Hash Join) [consumed]
                    Node 8132 (Seq Scan) [consumed] - LEAF
                    Node 8133 (Hash) [consumed]
                      Node 8134 (Hash Join) [consumed]
                        Node 8135 (Seq Scan) - LEAF
                        Node 8136 (Hash)
                          Node 8137 (Seq Scan) - LEAF
              Node 8138 (Index Scan) - LEAF
            Node 8139 (Hash)
              Node 8140 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8130 | 8121, 8122, 8123, 8125, 8126, 8131, 8132, 8133, 8134 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8121 | 8122, 8123, 8125, 8126, 8130, 8131, 8132, 8133, 8134 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8125 | 8121, 8122, 8123, 8126, 8130, 8131, 8132, 8133, 8134 |
...
|------|------|--------|-----------|-----|--------|
| 8121 | Sort | 1082.07 | 2934.42 | 171.2% | pattern |
| 8124 | Aggregate | 1076.87 | 1128.78 | 4.8% | operator |
| 8125 | Sort | 1076.62 | 1835.91 | 70.5% | pattern |
| 8127 | Nested Loop | 1062.21 | 1188.39 | 11.9% | operator |
| 8139 | Hash | 5.90 | 18.67 | 216.7% | operator |
| 8128 | Hash Join | 217.29 | 253.07 | 16.5% | operator |
| 8138 | Index Scan | 0.07 | -0.22 | 393.4% | operator |
| 8140 | Seq Scan | 5.37 | 12.36 | 130.3% | operator |
| 8129 | Seq Scan | 168.04 | 159.81 | 4.9% | operator |
| 8130 | Hash | 36.50 | 46.85 | 28.3% | pattern |
| 8135 | Seq Scan | 0.01 | 11.81 | 168589.6% | operator |
| 8136 | Hash | 0.06 | 21.05 | 37495.1% | operator |
| 8137 | Seq Scan | 0.05 | 22.93 | 43170.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8137 (Seq Scan) - LEAF
...
### Step 2: Node 8135 (Seq Scan) - LEAF
...
### Step 4: Node 8129 (Seq Scan) - LEAF
...
### Step 5: Node 8130 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8121, 8122, 8123, 8125, 8126, 8131, 8132, 8133, 8134
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8138 (Index Scan) - LEAF
...
### Step 8: Node 8140 (Seq Scan) - LEAF
...
### Step 11: Node 8125 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8121, 8122, 8123, 8126, 8130, 8131, 8132, 8133, 8134
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8121 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8122, 8123, 8125, 8126, 8130, 8131, 8132, 8133, 8134
- **Input Features:**
