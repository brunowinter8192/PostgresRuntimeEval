...
```
Node 14857 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14858 (Gather Merge) [consumed]
    Node 14859 (Sort)
      Node 14860 (Hash Join)
        Node 14861 (Nested Loop)
          Node 14862 (Hash Join)
            Node 14863 (Nested Loop)
              Node 14864 (Hash Join)
                Node 14865 (Seq Scan) - LEAF
                Node 14866 (Hash) [PATTERN: 444761fb]
                  Node 14867 (Hash Join) [consumed]
                    Node 14868 (Seq Scan) [consumed] - LEAF
                    Node 14869 (Hash) [consumed]
                      Node 14870 (Hash Join) [consumed]
                        Node 14871 (Seq Scan) - LEAF
                        Node 14872 (Hash)
                          Node 14873 (Seq Scan) - LEAF
              Node 14874 (Index Scan) - LEAF
            Node 14875 (Hash)
              Node 14876 (Seq Scan) - LEAF
          Node 14877 (Index Scan) - LEAF
        Node 14878 (Hash)
          Node 14879 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14866 | 14857, 14858, 14867, 14868, 14869, 14870 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14857 | 14858, 14866, 14867, 14868, 14869, 14870 |
...
|------|------|--------|-----------|-----|--------|
| 14857 | Aggregate | 1233.78 | 1133.67 | 8.1% | pattern |
| 14859 | Sort | 1227.21 | 1046.21 | 14.7% | operator |
| 14860 | Hash Join | 1226.95 | 785.61 | 36.0% | operator |
| 14861 | Nested Loop | 1226.64 | 1198.29 | 2.3% | operator |
| 14878 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 14862 | Hash Join | 1222.86 | 785.60 | 35.8% | operator |
| 14877 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14879 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 14863 | Nested Loop | 1178.01 | 1198.29 | 1.7% | operator |
| 14875 | Hash | 40.45 | 19.97 | 50.6% | operator |
| 14864 | Hash Join | 216.95 | 251.67 | 16.0% | operator |
| 14874 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14876 | Seq Scan | 40.39 | 48.88 | 21.0% | operator |
| 14865 | Seq Scan | 161.89 | 159.67 | 1.4% | operator |
| 14866 | Hash | 36.81 | 43.51 | 18.2% | pattern |
| 14871 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 14872 | Hash | 0.20 | 18.14 | 8750.8% | operator |
| 14873 | Seq Scan | 0.20 | 23.36 | 11639.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14873 (Seq Scan) - LEAF
...
### Step 2: Node 14871 (Seq Scan) - LEAF
...
### Step 4: Node 14865 (Seq Scan) - LEAF
...
### Step 5: Node 14866 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14857, 14858, 14867, 14868, 14869, 14870
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14874 (Index Scan) - LEAF
...
### Step 8: Node 14876 (Seq Scan) - LEAF
...
### Step 12: Node 14877 (Index Scan) - LEAF
...
### Step 13: Node 14879 (Seq Scan) - LEAF
...
### Step 14: Node 14861 (Nested Loop)
...
### Step 18: Node 14857 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14858, 14866, 14867, 14868, 14869, 14870
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2362
  - Aggregate_nt1=2362
