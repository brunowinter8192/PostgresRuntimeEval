...
**Test Query:** Q7_76_seed_615302325
**Timestamp:** 2026-01-18 18:03:39
...
```
Node 12715 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12716 (Gather Merge) [consumed]
    Node 12717 (Sort)
      Node 12718 (Hash Join)
        Node 12719 (Nested Loop)
          Node 12720 (Hash Join) [PATTERN: 895c6e8c]
            Node 12721 (Seq Scan) [consumed] - LEAF
            Node 12722 (Hash) [consumed]
              Node 12723 (Hash Join) [PATTERN: f4cb205a]
                Node 12724 (Seq Scan) [consumed] - LEAF
                Node 12725 (Hash) [consumed]
                  Node 12726 (Seq Scan) [consumed] - LEAF
          Node 12727 (Index Scan) - LEAF
        Node 12728 (Hash)
          Node 12729 (Hash Join) [PATTERN: f4cb205a]
            Node 12730 (Seq Scan) [consumed] - LEAF
            Node 12731 (Hash) [consumed]
              Node 12732 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12723 | 12715, 12716, 12720, 12721, 12722, 12724, 12725, 12726, 12729, 12730, 12731, 12732 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12729 | 12715, 12716, 12720, 12721, 12722, 12723, 12724, 12725, 12726, 12730, 12731, 12732 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12715 | 12716, 12720, 12721, 12722, 12723, 12724, 12725, 12726, 12729, 12730, 12731, 12732 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12720 | 12715, 12716, 12721, 12722, 12723, 12724, 12725, 12726, 12729, 12730, 12731, 12732 |
...
|------|------|--------|-----------|-----|--------|
| 12715 | Aggregate | 1254.17 | 1163.13 | 7.3% | pattern |
| 12717 | Sort | 1247.45 | 1109.16 | 11.1% | operator |
| 12718 | Hash Join | 1246.81 | 843.11 | 32.4% | operator |
| 12719 | Nested Loop | 1239.60 | 1215.99 | 1.9% | operator |
| 12728 | Hash | 3.31 | 27.74 | 738.9% | operator |
| 12720 | Hash Join | 217.80 | 304.89 | 40.0% | pattern |
| 12727 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12729 | Hash Join | 3.26 | 429.36 | 13078.7% | pattern |
| 12723 | Hash Join | 34.50 | 429.38 | 1144.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12723 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12715, 12716, 12720, 12721, 12722, 12724, 12725, 12726, 12729, 12730, 12731, 12732
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12720 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12715, 12716, 12721, 12722, 12723, 12724, 12725, 12726, 12729, 12730, 12731, 12732
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12727 (Index Scan) - LEAF
...
### Step 4: Node 12729 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12715, 12716, 12720, 12721, 12722, 12723, 12724, 12725, 12726, 12730, 12731, 12732
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12715 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12716, 12720, 12721, 12722, 12723, 12724, 12725, 12726, 12729, 12730, 12731, 12732
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
