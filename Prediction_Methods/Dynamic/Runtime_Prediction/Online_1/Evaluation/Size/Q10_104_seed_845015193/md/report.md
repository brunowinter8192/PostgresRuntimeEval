...
**Test Query:** Q10_104_seed_845015193
...
```
Node 19422 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19423 (Sort) [consumed]
    Node 19424 (Aggregate) [consumed]
      Node 19425 (Gather) [consumed]
        Node 19426 (Hash Join)
          Node 19427 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19428 (Nested Loop) [consumed]
              Node 19429 (Seq Scan) - LEAF
              Node 19430 (Index Scan) - LEAF
            Node 19431 (Hash) [consumed]
              Node 19432 (Seq Scan) - LEAF
          Node 19433 (Hash)
            Node 19434 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19422 | 19423, 19424, 19425, 19427, 19428, 19431 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19427 | 19422, 19423, 19424, 19425, 19428, 19431 |
...
|------|------|--------|-----------|-----|--------|
| 19422 | Limit | 1136.56 | 1199.47 | 5.5% | pattern |
| 19426 | Hash Join | 1087.37 | 1095.48 | 0.7% | operator |
| 19427 | Hash Join | 1084.59 | 1120.19 | 3.3% | pattern |
| 19433 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 19434 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 19429 | Seq Scan | 159.49 | 159.74 | 0.2% | operator |
| 19430 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 19432 | Seq Scan | 29.47 | 26.64 | 9.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19429 (Seq Scan) - LEAF
...
### Step 2: Node 19430 (Index Scan) - LEAF
...
### Step 3: Node 19432 (Seq Scan) - LEAF
...
### Step 4: Node 19434 (Seq Scan) - LEAF
...
### Step 5: Node 19427 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19422, 19423, 19424, 19425, 19428, 19431
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17935
  - HashJoin_nt1=17935
...
### Step 8: Node 19422 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19423, 19424, 19425, 19427, 19428, 19431
- **Input Features:**
