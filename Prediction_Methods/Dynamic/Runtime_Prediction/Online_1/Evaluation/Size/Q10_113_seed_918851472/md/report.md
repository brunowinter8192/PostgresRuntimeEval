...
**Test Query:** Q10_113_seed_918851472
...
```
Node 19552 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19553 (Sort) [consumed]
    Node 19554 (Aggregate) [consumed]
      Node 19555 (Gather) [consumed]
        Node 19556 (Hash Join)
          Node 19557 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19558 (Nested Loop) [consumed]
              Node 19559 (Seq Scan) - LEAF
              Node 19560 (Index Scan) - LEAF
            Node 19561 (Hash) [consumed]
              Node 19562 (Seq Scan) - LEAF
          Node 19563 (Hash)
            Node 19564 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19552 | 19553, 19554, 19555, 19557, 19558, 19561 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19557 | 19552, 19553, 19554, 19555, 19558, 19561 |
...
|------|------|--------|-----------|-----|--------|
| 19552 | Limit | 1158.76 | 1199.47 | 3.5% | pattern |
| 19556 | Hash Join | 1107.46 | 1095.60 | 1.1% | operator |
| 19557 | Hash Join | 1104.52 | 1120.19 | 1.4% | pattern |
| 19563 | Hash | 0.02 | 15.71 | 92316.3% | operator |
| 19564 | Seq Scan | 0.01 | 6.98 | 53582.3% | operator |
| 19559 | Seq Scan | 158.49 | 159.74 | 0.8% | operator |
| 19560 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 19562 | Seq Scan | 27.91 | 26.64 | 4.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19559 (Seq Scan) - LEAF
...
### Step 2: Node 19560 (Index Scan) - LEAF
...
### Step 3: Node 19562 (Seq Scan) - LEAF
...
### Step 4: Node 19564 (Seq Scan) - LEAF
...
### Step 5: Node 19557 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19552, 19553, 19554, 19555, 19558, 19561
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18384
  - HashJoin_nt1=18384
...
### Step 8: Node 19552 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19553, 19554, 19555, 19557, 19558, 19561
- **Input Features:**
