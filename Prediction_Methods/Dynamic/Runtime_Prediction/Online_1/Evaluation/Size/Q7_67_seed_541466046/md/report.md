...
**Test Query:** Q7_67_seed_541466046
...
```
Node 12535 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12536 (Gather Merge) [consumed]
    Node 12537 (Sort)
      Node 12538 (Hash Join)
        Node 12539 (Nested Loop)
          Node 12540 (Hash Join) [PATTERN: 895c6e8c]
            Node 12541 (Seq Scan) [consumed] - LEAF
            Node 12542 (Hash) [consumed]
              Node 12543 (Hash Join) [PATTERN: f4cb205a]
                Node 12544 (Seq Scan) [consumed] - LEAF
                Node 12545 (Hash) [consumed]
                  Node 12546 (Seq Scan) [consumed] - LEAF
          Node 12547 (Index Scan) - LEAF
        Node 12548 (Hash)
          Node 12549 (Hash Join) [PATTERN: f4cb205a]
            Node 12550 (Seq Scan) [consumed] - LEAF
            Node 12551 (Hash) [consumed]
              Node 12552 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12543 | 12535, 12536, 12540, 12541, 12542, 12544, 12545, 12546, 12549, 12550, 12551, 12552 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12549 | 12535, 12536, 12540, 12541, 12542, 12543, 12544, 12545, 12546, 12550, 12551, 12552 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12535 | 12536, 12540, 12541, 12542, 12543, 12544, 12545, 12546, 12549, 12550, 12551, 12552 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12540 | 12535, 12536, 12541, 12542, 12543, 12544, 12545, 12546, 12549, 12550, 12551, 12552 |
...
|------|------|--------|-----------|-----|--------|
| 12535 | Aggregate | 1194.57 | 1163.13 | 2.6% | pattern |
| 12537 | Sort | 1185.03 | 1109.16 | 6.4% | operator |
| 12538 | Hash Join | 1184.38 | 843.11 | 28.8% | operator |
| 12539 | Nested Loop | 1177.63 | 1215.99 | 3.3% | operator |
| 12548 | Hash | 2.94 | 27.74 | 843.0% | operator |
| 12540 | Hash Join | 210.52 | 304.89 | 44.8% | pattern |
| 12547 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12549 | Hash Join | 2.89 | 429.36 | 14736.3% | pattern |
| 12543 | Hash Join | 32.71 | 429.38 | 1212.5% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12543 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12535, 12536, 12540, 12541, 12542, 12544, 12545, 12546, 12549, 12550, 12551, 12552
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12540 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12535, 12536, 12541, 12542, 12543, 12544, 12545, 12546, 12549, 12550, 12551, 12552
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12547 (Index Scan) - LEAF
...
### Step 4: Node 12549 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12535, 12536, 12540, 12541, 12542, 12543, 12544, 12545, 12546, 12550, 12551, 12552
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12535 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12536, 12540, 12541, 12542, 12543, 12544, 12545, 12546, 12549, 12550, 12551, 12552
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
