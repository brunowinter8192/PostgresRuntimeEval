...
**Test Query:** Q7_110_seed_894239379
...
```
Node 10699 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10700 (Gather Merge) [consumed]
    Node 10701 (Sort)
      Node 10702 (Hash Join)
        Node 10703 (Nested Loop)
          Node 10704 (Hash Join) [PATTERN: 895c6e8c]
            Node 10705 (Seq Scan) [consumed] - LEAF
            Node 10706 (Hash) [consumed]
              Node 10707 (Hash Join) [PATTERN: f4cb205a]
                Node 10708 (Seq Scan) [consumed] - LEAF
                Node 10709 (Hash) [consumed]
                  Node 10710 (Seq Scan) [consumed] - LEAF
          Node 10711 (Index Scan) - LEAF
        Node 10712 (Hash)
          Node 10713 (Hash Join) [PATTERN: f4cb205a]
            Node 10714 (Seq Scan) [consumed] - LEAF
            Node 10715 (Hash) [consumed]
              Node 10716 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10707 | 10699, 10700, 10704, 10705, 10706, 10708, 10709, 10710, 10713, 10714, 10715, 10716 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10713 | 10699, 10700, 10704, 10705, 10706, 10707, 10708, 10709, 10710, 10714, 10715, 10716 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10699 | 10700, 10704, 10705, 10706, 10707, 10708, 10709, 10710, 10713, 10714, 10715, 10716 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10704 | 10699, 10700, 10705, 10706, 10707, 10708, 10709, 10710, 10713, 10714, 10715, 10716 |
...
|------|------|--------|-----------|-----|--------|
| 10699 | Aggregate | 1218.75 | 1163.13 | 4.6% | pattern |
| 10701 | Sort | 1212.23 | 1109.16 | 8.5% | operator |
| 10702 | Hash Join | 1211.59 | 843.11 | 30.4% | operator |
| 10703 | Nested Loop | 1204.72 | 1215.99 | 0.9% | operator |
| 10712 | Hash | 3.12 | 27.74 | 790.3% | operator |
| 10704 | Hash Join | 217.47 | 304.89 | 40.2% | pattern |
| 10711 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 10713 | Hash Join | 3.07 | 429.36 | 13881.2% | pattern |
| 10707 | Hash Join | 37.18 | 429.38 | 1054.8% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10707 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10699, 10700, 10704, 10705, 10706, 10708, 10709, 10710, 10713, 10714, 10715, 10716
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10704 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10699, 10700, 10705, 10706, 10707, 10708, 10709, 10710, 10713, 10714, 10715, 10716
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10711 (Index Scan) - LEAF
...
### Step 4: Node 10713 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10699, 10700, 10704, 10705, 10706, 10707, 10708, 10709, 10710, 10714, 10715, 10716
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10699 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10700, 10704, 10705, 10706, 10707, 10708, 10709, 10710, 10713, 10714, 10715, 10716
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
