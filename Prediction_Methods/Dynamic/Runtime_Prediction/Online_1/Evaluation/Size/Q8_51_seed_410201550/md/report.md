...
**Test Query:** Q8_51_seed_410201550
...
```
Node 15409 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15410 (Gather Merge) [consumed]
    Node 15411 (Sort)
      Node 15412 (Hash Join)
        Node 15413 (Nested Loop)
          Node 15414 (Hash Join)
            Node 15415 (Nested Loop)
              Node 15416 (Hash Join)
                Node 15417 (Seq Scan) - LEAF
                Node 15418 (Hash) [PATTERN: 444761fb]
                  Node 15419 (Hash Join) [consumed]
                    Node 15420 (Seq Scan) [consumed] - LEAF
                    Node 15421 (Hash) [consumed]
                      Node 15422 (Hash Join) [consumed]
                        Node 15423 (Seq Scan) - LEAF
                        Node 15424 (Hash)
                          Node 15425 (Seq Scan) - LEAF
              Node 15426 (Index Scan) - LEAF
            Node 15427 (Hash)
              Node 15428 (Seq Scan) - LEAF
          Node 15429 (Index Scan) - LEAF
        Node 15430 (Hash)
          Node 15431 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15418 | 15409, 15410, 15419, 15420, 15421, 15422 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15409 | 15410, 15418, 15419, 15420, 15421, 15422 |
...
|------|------|--------|-----------|-----|--------|
| 15409 | Aggregate | 1214.49 | 1124.52 | 7.4% | pattern |
| 15411 | Sort | 1209.83 | 1046.22 | 13.5% | operator |
| 15412 | Hash Join | 1209.54 | 785.61 | 35.0% | operator |
| 15413 | Nested Loop | 1209.20 | 1198.29 | 0.9% | operator |
| 15430 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 15414 | Hash Join | 1205.23 | 785.60 | 34.8% | operator |
| 15429 | Index Scan | 0.01 | 12.42 | 248369.7% | operator |
| 15431 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 15415 | Nested Loop | 1158.50 | 1198.29 | 3.4% | operator |
| 15427 | Hash | 42.16 | 19.97 | 52.6% | operator |
| 15416 | Hash Join | 207.50 | 251.67 | 21.3% | operator |
| 15426 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15428 | Seq Scan | 42.09 | 48.86 | 16.1% | operator |
| 15417 | Seq Scan | 158.35 | 159.67 | 0.8% | operator |
| 15418 | Hash | 30.34 | 43.51 | 43.4% | pattern |
| 15423 | Seq Scan | 0.01 | 12.22 | 111035.8% | operator |
| 15424 | Hash | 0.19 | 18.14 | 9654.9% | operator |
| 15425 | Seq Scan | 0.18 | 23.36 | 12666.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15425 (Seq Scan) - LEAF
...
### Step 2: Node 15423 (Seq Scan) - LEAF
...
### Step 4: Node 15417 (Seq Scan) - LEAF
...
### Step 5: Node 15418 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15409, 15410, 15419, 15420, 15421, 15422
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15426 (Index Scan) - LEAF
...
### Step 8: Node 15428 (Seq Scan) - LEAF
...
### Step 12: Node 15429 (Index Scan) - LEAF
...
### Step 13: Node 15431 (Seq Scan) - LEAF
...
### Step 14: Node 15413 (Nested Loop)
...
### Step 18: Node 15409 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15410, 15418, 15419, 15420, 15421, 15422
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2597
