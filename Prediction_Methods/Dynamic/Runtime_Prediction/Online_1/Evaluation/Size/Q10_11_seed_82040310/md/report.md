...
**Test Query:** Q10_11_seed_82040310
...
```
Node 19643 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19644 (Sort) [consumed]
    Node 19645 (Aggregate) [consumed]
      Node 19646 (Gather) [consumed]
        Node 19647 (Hash Join)
          Node 19648 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19649 (Nested Loop) [consumed]
              Node 19650 (Seq Scan) - LEAF
              Node 19651 (Index Scan) - LEAF
            Node 19652 (Hash) [consumed]
              Node 19653 (Seq Scan) - LEAF
          Node 19654 (Hash)
            Node 19655 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19643 | 19644, 19645, 19646, 19648, 19649, 19652 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19648 | 19643, 19644, 19645, 19646, 19649, 19652 |
...
|------|------|--------|-----------|-----|--------|
| 19643 | Limit | 1141.96 | 1199.47 | 5.0% | pattern |
| 19647 | Hash Join | 1097.63 | 1095.56 | 0.2% | operator |
| 19648 | Hash Join | 1094.78 | 1120.19 | 2.3% | pattern |
| 19654 | Hash | 0.01 | 15.71 | 120752.1% | operator |
| 19655 | Seq Scan | 0.01 | 6.98 | 77441.2% | operator |
| 19650 | Seq Scan | 159.76 | 159.74 | 0.0% | operator |
| 19651 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 19653 | Seq Scan | 25.05 | 26.64 | 6.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19650 (Seq Scan) - LEAF
...
### Step 2: Node 19651 (Index Scan) - LEAF
...
### Step 3: Node 19653 (Seq Scan) - LEAF
...
### Step 4: Node 19655 (Seq Scan) - LEAF
...
### Step 5: Node 19648 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19643, 19644, 19645, 19646, 19649, 19652
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18231
  - HashJoin_nt1=18231
...
### Step 8: Node 19643 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19644, 19645, 19646, 19648, 19649, 19652
- **Input Features:**
