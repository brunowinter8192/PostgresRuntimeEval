...
**Test Query:** Q10_63_seed_508649922
...
```
Node 20787 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20788 (Sort) [consumed]
    Node 20789 (Aggregate) [consumed]
      Node 20790 (Gather) [consumed]
        Node 20791 (Hash Join)
          Node 20792 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20793 (Nested Loop) [consumed]
              Node 20794 (Seq Scan) - LEAF
              Node 20795 (Index Scan) - LEAF
            Node 20796 (Hash) [consumed]
              Node 20797 (Seq Scan) - LEAF
          Node 20798 (Hash)
            Node 20799 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20787 | 20788, 20789, 20790, 20792, 20793, 20796 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20792 | 20787, 20788, 20789, 20790, 20793, 20796 |
...
|------|------|--------|-----------|-----|--------|
| 20787 | Limit | 1159.30 | 1199.47 | 3.5% | pattern |
| 20791 | Hash Join | 1110.18 | 1095.48 | 1.3% | operator |
| 20792 | Hash Join | 1107.25 | 1120.19 | 1.2% | pattern |
| 20798 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 20799 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 20794 | Seq Scan | 165.32 | 159.74 | 3.4% | operator |
| 20795 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20797 | Seq Scan | 28.07 | 26.64 | 5.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20794 (Seq Scan) - LEAF
...
### Step 2: Node 20795 (Index Scan) - LEAF
...
### Step 3: Node 20797 (Seq Scan) - LEAF
...
### Step 4: Node 20799 (Seq Scan) - LEAF
...
### Step 5: Node 20792 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20787, 20788, 20789, 20790, 20793, 20796
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17901
  - HashJoin_nt1=17901
...
### Step 8: Node 20787 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20788, 20789, 20790, 20792, 20793, 20796
- **Input Features:**
