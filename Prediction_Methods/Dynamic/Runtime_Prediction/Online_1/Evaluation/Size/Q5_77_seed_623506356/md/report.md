...
**Test Query:** Q5_77_seed_623506356
...
```
Node 9381 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9382 (Aggregate) [consumed]
    Node 9383 (Gather Merge) [consumed]
      Node 9384 (Aggregate)
        Node 9385 (Sort) [PATTERN: 3e2d5a00]
          Node 9386 (Hash Join) [consumed]
            Node 9387 (Nested Loop)
              Node 9388 (Hash Join)
                Node 9389 (Seq Scan) - LEAF
                Node 9390 (Hash) [PATTERN: 444761fb]
                  Node 9391 (Hash Join) [consumed]
                    Node 9392 (Seq Scan) [consumed] - LEAF
                    Node 9393 (Hash) [consumed]
                      Node 9394 (Hash Join) [consumed]
                        Node 9395 (Seq Scan) - LEAF
                        Node 9396 (Hash)
                          Node 9397 (Seq Scan) - LEAF
              Node 9398 (Index Scan) - LEAF
            Node 9399 (Hash)
              Node 9400 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9390 | 9381, 9382, 9383, 9385, 9386, 9391, 9392, 9393, 9394 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9381 | 9382, 9383, 9385, 9386, 9390, 9391, 9392, 9393, 9394 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9385 | 9381, 9382, 9383, 9386, 9390, 9391, 9392, 9393, 9394 |
...
|------|------|--------|-----------|-----|--------|
| 9381 | Sort | 1098.25 | 2934.42 | 167.2% | pattern |
| 9384 | Aggregate | 1093.42 | 1128.78 | 3.2% | operator |
| 9385 | Sort | 1093.15 | 1835.91 | 67.9% | pattern |
| 9387 | Nested Loop | 1080.66 | 1188.39 | 10.0% | operator |
| 9399 | Hash | 3.94 | 18.67 | 373.5% | operator |
| 9388 | Hash Join | 228.19 | 253.07 | 10.9% | operator |
| 9398 | Index Scan | 0.07 | -0.22 | 393.4% | operator |
| 9400 | Seq Scan | 3.41 | 12.36 | 262.1% | operator |
| 9389 | Seq Scan | 173.48 | 159.81 | 7.9% | operator |
| 9390 | Hash | 41.53 | 46.85 | 12.8% | pattern |
| 9395 | Seq Scan | 0.01 | 11.81 | 98302.3% | operator |
| 9396 | Hash | 0.07 | 21.05 | 32289.6% | operator |
| 9397 | Seq Scan | 0.06 | 22.93 | 36302.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9397 (Seq Scan) - LEAF
...
### Step 2: Node 9395 (Seq Scan) - LEAF
...
### Step 4: Node 9389 (Seq Scan) - LEAF
...
### Step 5: Node 9390 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9381, 9382, 9383, 9385, 9386, 9391, 9392, 9393, 9394
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9398 (Index Scan) - LEAF
...
### Step 8: Node 9400 (Seq Scan) - LEAF
...
### Step 11: Node 9385 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9381, 9382, 9383, 9386, 9390, 9391, 9392, 9393, 9394
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9381 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9382, 9383, 9385, 9386, 9390, 9391, 9392, 9393, 9394
- **Input Features:**
