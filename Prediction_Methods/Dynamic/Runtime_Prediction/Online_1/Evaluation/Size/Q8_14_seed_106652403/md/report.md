...
**Test Query:** Q8_14_seed_106652403
...
```
Node 14443 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14444 (Gather Merge) [consumed]
    Node 14445 (Sort)
      Node 14446 (Hash Join)
        Node 14447 (Nested Loop)
          Node 14448 (Hash Join)
            Node 14449 (Nested Loop)
              Node 14450 (Hash Join)
                Node 14451 (Seq Scan) - LEAF
                Node 14452 (Hash) [PATTERN: 444761fb]
                  Node 14453 (Hash Join) [consumed]
                    Node 14454 (Seq Scan) [consumed] - LEAF
                    Node 14455 (Hash) [consumed]
                      Node 14456 (Hash Join) [consumed]
                        Node 14457 (Seq Scan) - LEAF
                        Node 14458 (Hash)
                          Node 14459 (Seq Scan) - LEAF
              Node 14460 (Index Scan) - LEAF
            Node 14461 (Hash)
              Node 14462 (Seq Scan) - LEAF
          Node 14463 (Index Scan) - LEAF
        Node 14464 (Hash)
          Node 14465 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14452 | 14443, 14444, 14453, 14454, 14455, 14456 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14443 | 14444, 14452, 14453, 14454, 14455, 14456 |
...
|------|------|--------|-----------|-----|--------|
| 14443 | Aggregate | 1203.09 | 1131.71 | 5.9% | pattern |
| 14445 | Sort | 1196.51 | 1046.21 | 12.6% | operator |
| 14446 | Hash Join | 1196.26 | 785.61 | 34.3% | operator |
| 14447 | Nested Loop | 1195.95 | 1198.29 | 0.2% | operator |
| 14464 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 14448 | Hash Join | 1192.22 | 785.60 | 34.1% | operator |
| 14463 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14465 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 14449 | Nested Loop | 1151.94 | 1198.29 | 4.0% | operator |
| 14461 | Hash | 35.51 | 19.97 | 43.8% | operator |
| 14450 | Hash Join | 206.49 | 251.67 | 21.9% | operator |
| 14460 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14462 | Seq Scan | 35.40 | 48.87 | 38.1% | operator |
| 14451 | Seq Scan | 156.27 | 159.67 | 2.2% | operator |
| 14452 | Hash | 32.85 | 43.51 | 32.4% | pattern |
| 14457 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14458 | Hash | 0.13 | 18.14 | 14186.7% | operator |
| 14459 | Seq Scan | 0.12 | 23.36 | 18740.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14459 (Seq Scan) - LEAF
...
### Step 2: Node 14457 (Seq Scan) - LEAF
...
### Step 4: Node 14451 (Seq Scan) - LEAF
...
### Step 5: Node 14452 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14443, 14444, 14453, 14454, 14455, 14456
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14460 (Index Scan) - LEAF
...
### Step 8: Node 14462 (Seq Scan) - LEAF
...
### Step 12: Node 14463 (Index Scan) - LEAF
...
### Step 13: Node 14465 (Seq Scan) - LEAF
...
### Step 14: Node 14447 (Nested Loop)
...
### Step 18: Node 14443 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14444, 14452, 14453, 14454, 14455, 14456
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2444
  ...
  - Aggregate_sel=0.9845
  - Aggregate_startup_cost=71553.9400
  - Aggregate_total_cost=71940.0800
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2444
  ...
  - GatherMerge_Outer_startup_cost=71553.9400
  - GatherMerge_Outer_total_cost=71843.0800
- **Output:** st=1128.73, rt=1131.71
