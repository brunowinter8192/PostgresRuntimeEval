...
**Test Query:** Q7_101_seed_820403100
...
```
Node 10519 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10520 (Gather Merge) [consumed]
    Node 10521 (Sort)
      Node 10522 (Hash Join)
        Node 10523 (Nested Loop)
          Node 10524 (Hash Join) [PATTERN: 895c6e8c]
            Node 10525 (Seq Scan) [consumed] - LEAF
            Node 10526 (Hash) [consumed]
              Node 10527 (Hash Join) [PATTERN: f4cb205a]
                Node 10528 (Seq Scan) [consumed] - LEAF
                Node 10529 (Hash) [consumed]
                  Node 10530 (Seq Scan) [consumed] - LEAF
          Node 10531 (Index Scan) - LEAF
        Node 10532 (Hash)
          Node 10533 (Hash Join) [PATTERN: f4cb205a]
            Node 10534 (Seq Scan) [consumed] - LEAF
            Node 10535 (Hash) [consumed]
              Node 10536 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10527 | 10519, 10520, 10524, 10525, 10526, 10528, 10529, 10530, 10533, 10534, 10535, 10536 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10533 | 10519, 10520, 10524, 10525, 10526, 10527, 10528, 10529, 10530, 10534, 10535, 10536 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10519 | 10520, 10524, 10525, 10526, 10527, 10528, 10529, 10530, 10533, 10534, 10535, 10536 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10524 | 10519, 10520, 10525, 10526, 10527, 10528, 10529, 10530, 10533, 10534, 10535, 10536 |
...
|------|------|--------|-----------|-----|--------|
| 10519 | Aggregate | 1208.67 | 1163.13 | 3.8% | pattern |
| 10521 | Sort | 1201.04 | 1109.16 | 7.7% | operator |
| 10522 | Hash Join | 1200.39 | 843.11 | 29.8% | operator |
| 10523 | Nested Loop | 1191.05 | 1215.99 | 2.1% | operator |
| 10532 | Hash | 5.33 | 27.74 | 420.8% | operator |
| 10524 | Hash Join | 211.72 | 304.89 | 44.0% | pattern |
| 10531 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 10533 | Hash Join | 5.29 | 429.36 | 8022.6% | pattern |
| 10527 | Hash Join | 31.97 | 429.38 | 1242.9% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10527 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10519, 10520, 10524, 10525, 10526, 10528, 10529, 10530, 10533, 10534, 10535, 10536
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10524 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10519, 10520, 10525, 10526, 10527, 10528, 10529, 10530, 10533, 10534, 10535, 10536
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10531 (Index Scan) - LEAF
...
### Step 4: Node 10533 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10519, 10520, 10524, 10525, 10526, 10527, 10528, 10529, 10530, 10534, 10535, 10536
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10519 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10520, 10524, 10525, 10526, 10527, 10528, 10529, 10530, 10533, 10534, 10535, 10536
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
