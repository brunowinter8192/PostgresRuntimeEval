...
**Test Query:** Q8_136_seed_1107544185
...
```
Node 14098 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14099 (Gather Merge) [consumed]
    Node 14100 (Sort)
      Node 14101 (Hash Join)
        Node 14102 (Nested Loop)
          Node 14103 (Hash Join)
            Node 14104 (Nested Loop)
              Node 14105 (Hash Join)
                Node 14106 (Seq Scan) - LEAF
                Node 14107 (Hash) [PATTERN: 444761fb]
                  Node 14108 (Hash Join) [consumed]
                    Node 14109 (Seq Scan) [consumed] - LEAF
                    Node 14110 (Hash) [consumed]
                      Node 14111 (Hash Join) [consumed]
                        Node 14112 (Seq Scan) - LEAF
                        Node 14113 (Hash)
                          Node 14114 (Seq Scan) - LEAF
              Node 14115 (Index Scan) - LEAF
            Node 14116 (Hash)
              Node 14117 (Seq Scan) - LEAF
          Node 14118 (Index Scan) - LEAF
        Node 14119 (Hash)
          Node 14120 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14107 | 14098, 14099, 14108, 14109, 14110, 14111 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14098 | 14099, 14107, 14108, 14109, 14110, 14111 |
...
|------|------|--------|-----------|-----|--------|
| 14098 | Aggregate | 1210.06 | 1133.67 | 6.3% | pattern |
| 14100 | Sort | 1204.10 | 1046.21 | 13.1% | operator |
| 14101 | Hash Join | 1203.85 | 785.61 | 34.7% | operator |
| 14102 | Nested Loop | 1203.53 | 1198.29 | 0.4% | operator |
| 14119 | Hash | 0.02 | 13.72 | 76144.9% | operator |
| 14103 | Hash Join | 1199.70 | 785.60 | 34.5% | operator |
| 14118 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14120 | Seq Scan | 0.01 | 6.51 | 49992.0% | operator |
| 14104 | Nested Loop | 1152.88 | 1198.29 | 3.9% | operator |
| 14116 | Hash | 42.32 | 19.97 | 52.8% | operator |
| 14105 | Hash Join | 212.07 | 251.67 | 18.7% | operator |
| 14115 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14117 | Seq Scan | 42.17 | 48.89 | 15.9% | operator |
| 14106 | Seq Scan | 159.16 | 159.67 | 0.3% | operator |
| 14107 | Hash | 34.98 | 43.51 | 24.4% | pattern |
| 14112 | Seq Scan | 0.01 | 12.22 | 81399.6% | operator |
| 14113 | Hash | 0.08 | 18.14 | 22867.3% | operator |
| 14114 | Seq Scan | 0.08 | 23.36 | 30639.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14114 (Seq Scan) - LEAF
...
### Step 2: Node 14112 (Seq Scan) - LEAF
...
### Step 4: Node 14106 (Seq Scan) - LEAF
...
### Step 5: Node 14107 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14098, 14099, 14108, 14109, 14110, 14111
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14115 (Index Scan) - LEAF
...
### Step 8: Node 14117 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9720
  - startup_cost=9919.8100
  - total_cost=70271.2900
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14118 (Index Scan) - LEAF
...
### Step 13: Node 14120 (Seq Scan) - LEAF
...
### Step 14: Node 14102 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70498.2000
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14119 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.6600
  - total_cost=70503.9400
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14100 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70539.7600
  - total_cost=70541.6300
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14098 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14099, 14107, 14108, 14109, 14110, 14111
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2325
  - Aggregate_nt1=2325
  ...
  - Aggregate_total_cost=71907.8600
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2325
  - GatherMerge_Outer_nt1=750
  ...
  - GatherMerge_Outer_total_cost=71814.8600
