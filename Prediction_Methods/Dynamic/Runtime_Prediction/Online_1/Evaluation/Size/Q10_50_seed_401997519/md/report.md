...
**Test Query:** Q10_50_seed_401997519
...
```
Node 20605 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20606 (Sort) [consumed]
    Node 20607 (Aggregate) [consumed]
      Node 20608 (Gather) [consumed]
        Node 20609 (Hash Join)
          Node 20610 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20611 (Nested Loop) [consumed]
              Node 20612 (Seq Scan) - LEAF
              Node 20613 (Index Scan) - LEAF
            Node 20614 (Hash) [consumed]
              Node 20615 (Seq Scan) - LEAF
          Node 20616 (Hash)
            Node 20617 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20605 | 20606, 20607, 20608, 20610, 20611, 20614 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20610 | 20605, 20606, 20607, 20608, 20611, 20614 |
...
|------|------|--------|-----------|-----|--------|
| 20605 | Limit | 1121.90 | 1199.47 | 6.9% | pattern |
| 20609 | Hash Join | 1073.38 | 1095.54 | 2.1% | operator |
| 20610 | Hash Join | 1070.48 | 1120.19 | 4.6% | pattern |
| 20616 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 20617 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 20612 | Seq Scan | 159.90 | 159.74 | 0.1% | operator |
| 20613 | Index Scan | 0.06 | -0.52 | 963.7% | operator |
| 20615 | Seq Scan | 24.98 | 26.64 | 6.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20612 (Seq Scan) - LEAF
...
### Step 2: Node 20613 (Index Scan) - LEAF
...
### Step 3: Node 20615 (Seq Scan) - LEAF
...
### Step 4: Node 20617 (Seq Scan) - LEAF
...
### Step 5: Node 20610 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20605, 20606, 20607, 20608, 20611, 20614
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18130
  - HashJoin_nt1=18130
...
### Step 8: Node 20605 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20606, 20607, 20608, 20610, 20611, 20614
- **Input Features:**
