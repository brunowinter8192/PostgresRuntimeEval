...
**Test Query:** Q5_78_seed_631710387
...
```
Node 9401 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9402 (Aggregate) [consumed]
    Node 9403 (Gather Merge) [consumed]
      Node 9404 (Aggregate)
        Node 9405 (Sort) [PATTERN: 3e2d5a00]
          Node 9406 (Hash Join) [consumed]
            Node 9407 (Nested Loop)
              Node 9408 (Hash Join)
                Node 9409 (Seq Scan) - LEAF
                Node 9410 (Hash) [PATTERN: 444761fb]
                  Node 9411 (Hash Join) [consumed]
                    Node 9412 (Seq Scan) [consumed] - LEAF
                    Node 9413 (Hash) [consumed]
                      Node 9414 (Hash Join) [consumed]
                        Node 9415 (Seq Scan) - LEAF
                        Node 9416 (Hash)
                          Node 9417 (Seq Scan) - LEAF
              Node 9418 (Index Scan) - LEAF
            Node 9419 (Hash)
              Node 9420 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9410 | 9401, 9402, 9403, 9405, 9406, 9411, 9412, 9413, 9414 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9401 | 9402, 9403, 9405, 9406, 9410, 9411, 9412, 9413, 9414 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9405 | 9401, 9402, 9403, 9406, 9410, 9411, 9412, 9413, 9414 |
...
|------|------|--------|-----------|-----|--------|
| 9401 | Sort | 1052.49 | 2934.42 | 178.8% | pattern |
| 9404 | Aggregate | 1047.35 | 1128.74 | 7.8% | operator |
| 9405 | Sort | 1047.10 | 1835.96 | 75.3% | pattern |
| 9407 | Nested Loop | 1035.63 | 1188.57 | 14.8% | operator |
| 9419 | Hash | 3.06 | 18.67 | 510.2% | operator |
| 9408 | Hash Join | 213.58 | 253.07 | 18.5% | operator |
| 9418 | Index Scan | 0.07 | -0.22 | 405.6% | operator |
| 9420 | Seq Scan | 2.55 | 12.36 | 383.9% | operator |
| 9409 | Seq Scan | 165.33 | 159.81 | 3.3% | operator |
| 9410 | Hash | 34.89 | 46.85 | 34.3% | pattern |
| 9415 | Seq Scan | 0.01 | 11.81 | 84244.8% | operator |
| 9416 | Hash | 0.10 | 21.05 | 22061.3% | operator |
| 9417 | Seq Scan | 0.09 | 22.93 | 24827.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9417 (Seq Scan) - LEAF
...
### Step 2: Node 9415 (Seq Scan) - LEAF
...
### Step 4: Node 9409 (Seq Scan) - LEAF
...
### Step 5: Node 9410 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9401, 9402, 9403, 9405, 9406, 9411, 9412, 9413, 9414
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9418 (Index Scan) - LEAF
...
### Step 8: Node 9420 (Seq Scan) - LEAF
...
### Step 11: Node 9405 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9401, 9402, 9403, 9406, 9410, 9411, 9412, 9413, 9414
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9401 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9402, 9403, 9405, 9406, 9410, 9411, 9412, 9413, 9414
- **Input Features:**
