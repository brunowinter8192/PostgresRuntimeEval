...
**Test Query:** Q10_148_seed_1205992557
...
```
Node 20046 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20047 (Sort) [consumed]
    Node 20048 (Aggregate) [consumed]
      Node 20049 (Gather) [consumed]
        Node 20050 (Hash Join)
          Node 20051 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20052 (Nested Loop) [consumed]
              Node 20053 (Seq Scan) - LEAF
              Node 20054 (Index Scan) - LEAF
            Node 20055 (Hash) [consumed]
              Node 20056 (Seq Scan) - LEAF
          Node 20057 (Hash)
            Node 20058 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20046 | 20047, 20048, 20049, 20051, 20052, 20055 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20051 | 20046, 20047, 20048, 20049, 20052, 20055 |
...
|------|------|--------|-----------|-----|--------|
| 20046 | Limit | 1144.40 | 1199.47 | 4.8% | pattern |
| 20050 | Hash Join | 1092.33 | 1095.55 | 0.3% | operator |
| 20051 | Hash Join | 1089.43 | 1120.19 | 2.8% | pattern |
| 20057 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 20058 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 20053 | Seq Scan | 167.01 | 159.74 | 4.4% | operator |
| 20054 | Index Scan | 0.06 | -0.52 | 949.6% | operator |
| 20056 | Seq Scan | 26.41 | 26.64 | 0.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20053 (Seq Scan) - LEAF
...
### Step 2: Node 20054 (Index Scan) - LEAF
...
### Step 3: Node 20056 (Seq Scan) - LEAF
...
### Step 4: Node 20058 (Seq Scan) - LEAF
...
### Step 5: Node 20051 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20046, 20047, 20048, 20049, 20052, 20055
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18186
  - HashJoin_nt1=18186
...
### Step 8: Node 20046 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20047, 20048, 20049, 20051, 20052, 20055
- **Input Features:**
