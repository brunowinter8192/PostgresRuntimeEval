...
**Test Query:** Q8_58_seed_467629767
...
```
Node 15570 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15571 (Gather Merge) [consumed]
    Node 15572 (Sort)
      Node 15573 (Hash Join)
        Node 15574 (Nested Loop)
          Node 15575 (Hash Join)
            Node 15576 (Nested Loop)
              Node 15577 (Hash Join)
                Node 15578 (Seq Scan) - LEAF
                Node 15579 (Hash) [PATTERN: 444761fb]
                  Node 15580 (Hash Join) [consumed]
                    Node 15581 (Seq Scan) [consumed] - LEAF
                    Node 15582 (Hash) [consumed]
                      Node 15583 (Hash Join) [consumed]
                        Node 15584 (Seq Scan) - LEAF
                        Node 15585 (Hash)
                          Node 15586 (Seq Scan) - LEAF
              Node 15587 (Index Scan) - LEAF
            Node 15588 (Hash)
              Node 15589 (Seq Scan) - LEAF
          Node 15590 (Index Scan) - LEAF
        Node 15591 (Hash)
          Node 15592 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15579 | 15570, 15571, 15580, 15581, 15582, 15583 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15570 | 15571, 15579, 15580, 15581, 15582, 15583 |
...
|------|------|--------|-----------|-----|--------|
| 15570 | Aggregate | 1208.42 | 1133.60 | 6.2% | pattern |
| 15572 | Sort | 1200.82 | 1046.21 | 12.9% | operator |
| 15573 | Hash Join | 1200.57 | 785.61 | 34.6% | operator |
| 15574 | Nested Loop | 1200.26 | 1198.29 | 0.2% | operator |
| 15591 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 15575 | Hash Join | 1196.55 | 785.60 | 34.3% | operator |
| 15590 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15592 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 15576 | Nested Loop | 1152.78 | 1198.29 | 3.9% | operator |
| 15588 | Hash | 39.47 | 19.97 | 49.4% | operator |
| 15577 | Hash Join | 223.44 | 251.67 | 12.6% | operator |
| 15587 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 15589 | Seq Scan | 39.41 | 48.88 | 24.0% | operator |
| 15578 | Seq Scan | 161.83 | 159.67 | 1.3% | operator |
| 15579 | Hash | 43.49 | 43.51 | 0.1% | pattern |
| 15584 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 15585 | Hash | 0.24 | 18.14 | 7523.6% | operator |
| 15586 | Seq Scan | 0.23 | 23.36 | 9841.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15586 (Seq Scan) - LEAF
...
### Step 2: Node 15584 (Seq Scan) - LEAF
...
### Step 4: Node 15578 (Seq Scan) - LEAF
...
### Step 5: Node 15579 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15570, 15571, 15580, 15581, 15582, 15583
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15587 (Index Scan) - LEAF
...
### Step 8: Node 15589 (Seq Scan) - LEAF
...
### Step 12: Node 15590 (Index Scan) - LEAF
...
### Step 13: Node 15592 (Seq Scan) - LEAF
...
### Step 14: Node 15574 (Nested Loop)
...
### Step 18: Node 15570 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15571, 15579, 15580, 15581, 15582, 15583
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2407
