...
**Test Query:** Q7_87_seed_705546666
...
```
Node 12931 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12932 (Gather Merge) [consumed]
    Node 12933 (Sort)
      Node 12934 (Hash Join)
        Node 12935 (Nested Loop)
          Node 12936 (Hash Join) [PATTERN: 895c6e8c]
            Node 12937 (Seq Scan) [consumed] - LEAF
            Node 12938 (Hash) [consumed]
              Node 12939 (Hash Join) [PATTERN: f4cb205a]
                Node 12940 (Seq Scan) [consumed] - LEAF
                Node 12941 (Hash) [consumed]
                  Node 12942 (Seq Scan) [consumed] - LEAF
          Node 12943 (Index Scan) - LEAF
        Node 12944 (Hash)
          Node 12945 (Hash Join) [PATTERN: f4cb205a]
            Node 12946 (Seq Scan) [consumed] - LEAF
            Node 12947 (Hash) [consumed]
              Node 12948 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12939 | 12931, 12932, 12936, 12937, 12938, 12940, 12941, 12942, 12945, 12946, 12947, 12948 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12945 | 12931, 12932, 12936, 12937, 12938, 12939, 12940, 12941, 12942, 12946, 12947, 12948 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12931 | 12932, 12936, 12937, 12938, 12939, 12940, 12941, 12942, 12945, 12946, 12947, 12948 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12936 | 12931, 12932, 12937, 12938, 12939, 12940, 12941, 12942, 12945, 12946, 12947, 12948 |
...
|------|------|--------|-----------|-----|--------|
| 12931 | Aggregate | 1183.44 | 1163.13 | 1.7% | pattern |
| 12933 | Sort | 1177.08 | 1109.16 | 5.8% | operator |
| 12934 | Hash Join | 1176.44 | 843.11 | 28.3% | operator |
| 12935 | Nested Loop | 1169.51 | 1215.99 | 4.0% | operator |
| 12944 | Hash | 3.11 | 27.74 | 793.2% | operator |
| 12936 | Hash Join | 205.69 | 304.89 | 48.2% | pattern |
| 12943 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 12945 | Hash Join | 3.06 | 429.36 | 13940.6% | pattern |
| 12939 | Hash Join | 31.05 | 429.38 | 1282.9% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12939 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12931, 12932, 12936, 12937, 12938, 12940, 12941, 12942, 12945, 12946, 12947, 12948
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12936 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12931, 12932, 12937, 12938, 12939, 12940, 12941, 12942, 12945, 12946, 12947, 12948
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12943 (Index Scan) - LEAF
...
### Step 4: Node 12945 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12931, 12932, 12936, 12937, 12938, 12939, 12940, 12941, 12942, 12946, 12947, 12948
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12931 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12932, 12936, 12937, 12938, 12939, 12940, 12941, 12942, 12945, 12946, 12947, 12948
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
