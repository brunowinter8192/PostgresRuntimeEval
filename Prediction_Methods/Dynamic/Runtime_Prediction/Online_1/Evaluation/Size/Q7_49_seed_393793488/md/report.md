...
**Test Query:** Q7_49_seed_393793488
...
```
Node 12175 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12176 (Gather Merge) [consumed]
    Node 12177 (Sort)
      Node 12178 (Hash Join)
        Node 12179 (Nested Loop)
          Node 12180 (Hash Join) [PATTERN: 895c6e8c]
            Node 12181 (Seq Scan) [consumed] - LEAF
            Node 12182 (Hash) [consumed]
              Node 12183 (Hash Join) [PATTERN: f4cb205a]
                Node 12184 (Seq Scan) [consumed] - LEAF
                Node 12185 (Hash) [consumed]
                  Node 12186 (Seq Scan) [consumed] - LEAF
          Node 12187 (Index Scan) - LEAF
        Node 12188 (Hash)
          Node 12189 (Hash Join) [PATTERN: f4cb205a]
            Node 12190 (Seq Scan) [consumed] - LEAF
            Node 12191 (Hash) [consumed]
              Node 12192 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12183 | 12175, 12176, 12180, 12181, 12182, 12184, 12185, 12186, 12189, 12190, 12191, 12192 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12189 | 12175, 12176, 12180, 12181, 12182, 12183, 12184, 12185, 12186, 12190, 12191, 12192 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12175 | 12176, 12180, 12181, 12182, 12183, 12184, 12185, 12186, 12189, 12190, 12191, 12192 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12180 | 12175, 12176, 12181, 12182, 12183, 12184, 12185, 12186, 12189, 12190, 12191, 12192 |
...
|------|------|--------|-----------|-----|--------|
| 12175 | Aggregate | 1190.70 | 1163.13 | 2.3% | pattern |
| 12177 | Sort | 1183.30 | 1109.16 | 6.3% | operator |
| 12178 | Hash Join | 1182.63 | 843.11 | 28.7% | operator |
| 12179 | Nested Loop | 1172.34 | 1215.99 | 3.7% | operator |
| 12188 | Hash | 6.43 | 27.74 | 331.5% | operator |
| 12180 | Hash Join | 210.92 | 304.89 | 44.6% | pattern |
| 12187 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12189 | Hash Join | 6.39 | 429.36 | 6622.4% | pattern |
| 12183 | Hash Join | 34.44 | 429.38 | 1146.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12183 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12175, 12176, 12180, 12181, 12182, 12184, 12185, 12186, 12189, 12190, 12191, 12192
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12180 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12175, 12176, 12181, 12182, 12183, 12184, 12185, 12186, 12189, 12190, 12191, 12192
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12187 (Index Scan) - LEAF
...
### Step 4: Node 12189 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12175, 12176, 12180, 12181, 12182, 12183, 12184, 12185, 12186, 12190, 12191, 12192
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12175 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12176, 12180, 12181, 12182, 12183, 12184, 12185, 12186, 12189, 12190, 12191, 12192
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
