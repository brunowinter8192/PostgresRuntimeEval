...
**Test Query:** Q5_138_seed_1123952247
...
```
Node 7721 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7722 (Aggregate) [consumed]
    Node 7723 (Gather Merge) [consumed]
      Node 7724 (Aggregate)
        Node 7725 (Sort) [PATTERN: 3e2d5a00]
          Node 7726 (Hash Join) [consumed]
            Node 7727 (Nested Loop)
              Node 7728 (Hash Join)
                Node 7729 (Seq Scan) - LEAF
                Node 7730 (Hash) [PATTERN: 444761fb]
                  Node 7731 (Hash Join) [consumed]
                    Node 7732 (Seq Scan) [consumed] - LEAF
                    Node 7733 (Hash) [consumed]
                      Node 7734 (Hash Join) [consumed]
                        Node 7735 (Seq Scan) - LEAF
                        Node 7736 (Hash)
                          Node 7737 (Seq Scan) - LEAF
              Node 7738 (Index Scan) - LEAF
            Node 7739 (Hash)
              Node 7740 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7730 | 7721, 7722, 7723, 7725, 7726, 7731, 7732, 7733, 7734 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7721 | 7722, 7723, 7725, 7726, 7730, 7731, 7732, 7733, 7734 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7725 | 7721, 7722, 7723, 7726, 7730, 7731, 7732, 7733, 7734 |
...
|------|------|--------|-----------|-----|--------|
| 7721 | Sort | 1042.51 | 2934.42 | 181.5% | pattern |
| 7724 | Aggregate | 1037.23 | 1128.78 | 8.8% | operator |
| 7725 | Sort | 1036.97 | 1835.92 | 77.0% | pattern |
| 7727 | Nested Loop | 1024.19 | 1188.42 | 16.0% | operator |
| 7739 | Hash | 4.18 | 18.67 | 346.9% | operator |
| 7728 | Hash Join | 213.34 | 253.07 | 18.6% | operator |
| 7738 | Index Scan | 0.07 | -0.22 | 409.9% | operator |
| 7740 | Seq Scan | 3.67 | 12.36 | 237.2% | operator |
| 7729 | Seq Scan | 167.13 | 159.81 | 4.4% | operator |
| 7730 | Hash | 32.72 | 46.85 | 43.2% | pattern |
| 7735 | Seq Scan | 0.01 | 11.81 | 168589.6% | operator |
| 7736 | Hash | 1.13 | 21.05 | 1769.7% | operator |
| 7737 | Seq Scan | 1.12 | 22.93 | 1942.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7737 (Seq Scan) - LEAF
...
### Step 2: Node 7735 (Seq Scan) - LEAF
...
### Step 4: Node 7729 (Seq Scan) - LEAF
...
### Step 5: Node 7730 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7721, 7722, 7723, 7725, 7726, 7731, 7732, 7733, 7734
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7738 (Index Scan) - LEAF
...
### Step 8: Node 7740 (Seq Scan) - LEAF
...
### Step 11: Node 7725 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7721, 7722, 7723, 7726, 7730, 7731, 7732, 7733, 7734
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7721 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7722, 7723, 7725, 7726, 7730, 7731, 7732, 7733, 7734
- **Input Features:**
