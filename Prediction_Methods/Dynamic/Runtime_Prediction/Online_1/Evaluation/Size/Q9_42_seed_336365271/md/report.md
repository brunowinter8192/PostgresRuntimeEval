...
**Test Query:** Q9_42_seed_336365271
...
```
Node 18210 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 18211 (Aggregate) [consumed]
    Node 18212 (Gather)
      Node 18213 (Aggregate)
        Node 18214 (Nested Loop)
          Node 18215 (Hash Join) [PATTERN: bb930825]
            Node 18216 (Seq Scan) [consumed] - LEAF
            Node 18217 (Hash) [consumed]
              Node 18218 (Hash Join) [consumed]
                Node 18219 (Hash Join)
                  Node 18220 (Nested Loop)
                    Node 18221 (Seq Scan) - LEAF
                    Node 18222 (Index Scan) - LEAF
                  Node 18223 (Hash)
                    Node 18224 (Seq Scan) - LEAF
                Node 18225 (Hash)
                  Node 18226 (Seq Scan) - LEAF
          Node 18227 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | bb930825 | 18215 | 18210, 18211, 18216, 18217, 18218 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 18210 | 18211, 18215, 18216, 18217, 18218 |
...
|------|------|--------|-----------|-----|--------|
| 18210 | Sort | 1219.15 | 1045.59 | 14.2% | pattern |
| 18212 | Gather | 1217.91 | 964.55 | 20.8% | operator |
| 18213 | Aggregate | 1196.55 | 1085.45 | 9.3% | operator |
| 18214 | Nested Loop | 1172.10 | 1252.17 | 6.8% | operator |
| 18215 | Hash Join | 999.49 | 134.80 | 86.5% | pattern |
| 18227 | Index Scan | 0.00 | -5.54 | 184812.7% | operator |
| 18219 | Hash Join | 165.48 | 837.63 | 406.2% | operator |
| 18225 | Hash | 14.61 | 14.95 | 2.3% | operator |
| 18220 | Nested Loop | 159.95 | 1251.77 | 682.6% | operator |
| 18223 | Hash | 3.07 | 15.94 | 419.2% | operator |
| 18226 | Seq Scan | 14.60 | 2.94 | 79.9% | operator |
| 18221 | Seq Scan | 19.03 | 43.96 | 131.0% | operator |
| 18222 | Index Scan | 0.08 | -1.39 | 1880.1% | operator |
| 18224 | Seq Scan | 2.53 | 9.45 | 273.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 18221 (Seq Scan) - LEAF
...
### Step 2: Node 18222 (Index Scan) - LEAF
...
### Step 3: Node 18224 (Seq Scan) - LEAF
...
### Step 6: Node 18226 (Seq Scan) - LEAF
...
### Step 9: Node 18215 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** bb930825 (Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)])
- **Consumes:** Nodes 18210, 18211, 18216, 18217, 18218
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 18227 (Index Scan) - LEAF
...
### Step 11: Node 18214 (Nested Loop)
...
### Step 14: Node 18210 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 18211, 18215, 18216, 18217, 18218
- **Input Features:**
