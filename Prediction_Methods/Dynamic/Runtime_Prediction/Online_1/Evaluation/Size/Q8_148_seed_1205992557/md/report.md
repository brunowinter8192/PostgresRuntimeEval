...
**Test Query:** Q8_148_seed_1205992557
**Timestamp:** 2026-01-18 18:23:32
...
```
Node 14397 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14398 (Gather Merge) [consumed]
    Node 14399 (Sort)
      Node 14400 (Hash Join)
        Node 14401 (Nested Loop)
          Node 14402 (Hash Join)
            Node 14403 (Nested Loop)
              Node 14404 (Hash Join)
                Node 14405 (Seq Scan) - LEAF
                Node 14406 (Hash) [PATTERN: 444761fb]
                  Node 14407 (Hash Join) [consumed]
                    Node 14408 (Seq Scan) [consumed] - LEAF
                    Node 14409 (Hash) [consumed]
                      Node 14410 (Hash Join) [consumed]
                        Node 14411 (Seq Scan) - LEAF
                        Node 14412 (Hash)
                          Node 14413 (Seq Scan) - LEAF
              Node 14414 (Index Scan) - LEAF
            Node 14415 (Hash)
              Node 14416 (Seq Scan) - LEAF
          Node 14417 (Index Scan) - LEAF
        Node 14418 (Hash)
          Node 14419 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14406 | 14397, 14398, 14407, 14408, 14409, 14410 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14397 | 14398, 14406, 14407, 14408, 14409, 14410 |
...
|------|------|--------|-----------|-----|--------|
| 14397 | Aggregate | 1225.59 | 1124.52 | 8.2% | pattern |
| 14399 | Sort | 1219.29 | 1046.22 | 14.2% | operator |
| 14400 | Hash Join | 1219.03 | 785.61 | 35.6% | operator |
| 14401 | Nested Loop | 1218.71 | 1198.29 | 1.7% | operator |
| 14418 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 14402 | Hash Join | 1214.90 | 785.60 | 35.3% | operator |
| 14417 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14419 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 14403 | Nested Loop | 1170.08 | 1198.29 | 2.4% | operator |
| 14415 | Hash | 39.96 | 19.97 | 50.0% | operator |
| 14404 | Hash Join | 217.18 | 251.67 | 15.9% | operator |
| 14414 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14416 | Seq Scan | 39.86 | 48.86 | 22.6% | operator |
| 14405 | Seq Scan | 162.13 | 159.67 | 1.5% | operator |
| 14406 | Hash | 37.38 | 43.51 | 16.4% | pattern |
| 14411 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14412 | Hash | 0.12 | 18.14 | 15020.1% | operator |
| 14413 | Seq Scan | 0.12 | 23.36 | 19698.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14413 (Seq Scan) - LEAF
...
### Step 2: Node 14411 (Seq Scan) - LEAF
...
### Step 4: Node 14405 (Seq Scan) - LEAF
...
### Step 5: Node 14406 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14397, 14398, 14407, 14408, 14409, 14410
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14414 (Index Scan) - LEAF
...
### Step 8: Node 14416 (Seq Scan) - LEAF
...
### Step 12: Node 14417 (Index Scan) - LEAF
...
### Step 13: Node 14419 (Seq Scan) - LEAF
...
### Step 14: Node 14401 (Nested Loop)
...
### Step 18: Node 14397 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14398, 14406, 14407, 14408, 14409, 14410
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2597
  ...
  - Aggregate_sel=0.9265
  - Aggregate_startup_cost=71572.5600
  - Aggregate_total_cost=71979.8700
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2597
  ...
  - GatherMerge_Outer_total_cost=71879.8100
- **Output:** st=1121.60, rt=1124.52
