        ...
        Node 12737 (Nested Loop)
          Node 12738 (Hash Join) [PATTERN: 895c6e8c]
            ...
            Node 12740 (Hash) [consumed]
              Node 12741 (Hash Join) [PATTERN: f4cb205a]
                Node 12742 (Seq Scan) [consumed] - LEAF
                Node 12743 (Hash) [consumed]
                  Node 12744 (Seq Scan) [consumed] - LEAF
          Node 12745 (Index Scan) - LEAF
        Node 12746 (Hash)
          Node 12747 (Hash Join) [PATTERN: f4cb205a]
            Node 12748 (Seq Scan) [consumed] - LEAF
            Node 12749 (Hash) [consumed]
              Node 12750 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12741 | 12733, 12734, 12738, 12739, 12740, 12742, 12743, 12744, 12747, 12748, 12749, 12750 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12747 | 12733, 12734, 12738, 12739, 12740, 12741, 12742, 12743, 12744, 12748, 12749, 12750 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12733 | 12734, 12738, 12739, 12740, 12741, 12742, 12743, 12744, 12747, 12748, 12749, 12750 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12738 | 12733, 12734, 12739, 12740, 12741, 12742, 12743, 12744, 12747, 12748, 12749, 12750 |
...
|------|------|--------|-----------|-----|--------|
| 12733 | Aggregate | 1179.00 | 1163.13 | 1.3% | pattern |
| 12735 | Sort | 1171.95 | 1109.16 | 5.4% | operator |
| 12736 | Hash Join | 1171.29 | 843.11 | 28.0% | operator |
| 12737 | Nested Loop | 1164.91 | 1215.99 | 4.4% | operator |
| 12746 | Hash | 2.65 | 27.74 | 948.1% | operator |
| 12738 | Hash Join | 203.78 | 304.89 | 49.6% | pattern |
| 12745 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12747 | Hash Join | 2.60 | 429.36 | 16401.3% | pattern |
| 12741 | Hash Join | 28.71 | 429.38 | 1395.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12741 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12733, 12734, 12738, 12739, 12740, 12742, 12743, 12744, 12747, 12748, 12749, 12750
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12738 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12733, 12734, 12739, 12740, 12741, 12742, 12743, 12744, 12747, 12748, 12749, 12750
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12745 (Index Scan) - LEAF
...
### Step 4: Node 12747 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12733, 12734, 12738, 12739, 12740, 12741, 12742, 12743, 12744, 12748, 12749, 12750
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12733 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12734, 12738, 12739, 12740, 12741, 12742, 12743, 12744, 12747, 12748, 12749, 12750
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
