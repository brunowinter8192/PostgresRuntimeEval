...
**Test Query:** Q5_73_seed_590690232
...
```
Node 9301 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9302 (Aggregate) [consumed]
    Node 9303 (Gather Merge) [consumed]
      Node 9304 (Aggregate)
        Node 9305 (Sort) [PATTERN: 3e2d5a00]
          Node 9306 (Hash Join) [consumed]
            Node 9307 (Nested Loop)
              Node 9308 (Hash Join)
                Node 9309 (Seq Scan) - LEAF
                Node 9310 (Hash) [PATTERN: 444761fb]
                  Node 9311 (Hash Join) [consumed]
                    Node 9312 (Seq Scan) [consumed] - LEAF
                    Node 9313 (Hash) [consumed]
                      Node 9314 (Hash Join) [consumed]
                        Node 9315 (Seq Scan) - LEAF
                        Node 9316 (Hash)
                          Node 9317 (Seq Scan) - LEAF
              Node 9318 (Index Scan) - LEAF
            Node 9319 (Hash)
              Node 9320 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9310 | 9301, 9302, 9303, 9305, 9306, 9311, 9312, 9313, 9314 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9301 | 9302, 9303, 9305, 9306, 9310, 9311, 9312, 9313, 9314 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9305 | 9301, 9302, 9303, 9306, 9310, 9311, 9312, 9313, 9314 |
...
|------|------|--------|-----------|-----|--------|
| 9301 | Sort | 1076.05 | 2934.42 | 172.7% | pattern |
| 9304 | Aggregate | 1071.61 | 1128.78 | 5.3% | operator |
| 9305 | Sort | 1071.37 | 1835.91 | 71.4% | pattern |
| 9307 | Nested Loop | 1059.09 | 1188.39 | 12.2% | operator |
| 9319 | Hash | 3.65 | 18.67 | 411.7% | operator |
| 9308 | Hash Join | 210.70 | 253.07 | 20.1% | operator |
| 9318 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 9320 | Seq Scan | 3.08 | 12.36 | 301.0% | operator |
| 9309 | Seq Scan | 163.12 | 159.81 | 2.0% | operator |
| 9310 | Hash | 34.58 | 46.85 | 35.5% | pattern |
| 9315 | Seq Scan | 0.14 | 11.81 | 8334.5% | operator |
| 9316 | Hash | 0.12 | 21.05 | 17444.4% | operator |
| 9317 | Seq Scan | 0.12 | 22.93 | 19501.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9317 (Seq Scan) - LEAF
...
### Step 2: Node 9315 (Seq Scan) - LEAF
...
### Step 4: Node 9309 (Seq Scan) - LEAF
...
### Step 5: Node 9310 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9301, 9302, 9303, 9305, 9306, 9311, 9312, 9313, 9314
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9318 (Index Scan) - LEAF
...
### Step 8: Node 9320 (Seq Scan) - LEAF
...
### Step 11: Node 9305 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9301, 9302, 9303, 9306, 9310, 9311, 9312, 9313, 9314
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9301 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9302, 9303, 9305, 9306, 9310, 9311, 9312, 9313, 9314
- **Input Features:**
