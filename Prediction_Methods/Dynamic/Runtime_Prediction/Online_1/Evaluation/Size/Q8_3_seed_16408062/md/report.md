...
```
Node 15110 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15111 (Gather Merge) [consumed]
    Node 15112 (Sort)
      Node 15113 (Hash Join)
        Node 15114 (Nested Loop)
          Node 15115 (Hash Join)
            Node 15116 (Nested Loop)
              Node 15117 (Hash Join)
                Node 15118 (Seq Scan) - LEAF
                Node 15119 (Hash) [PATTERN: 444761fb]
                  Node 15120 (Hash Join) [consumed]
                    Node 15121 (Seq Scan) [consumed] - LEAF
                    Node 15122 (Hash) [consumed]
                      Node 15123 (Hash Join) [consumed]
                        Node 15124 (Seq Scan) - LEAF
                        Node 15125 (Hash)
                          Node 15126 (Seq Scan) - LEAF
              Node 15127 (Index Scan) - LEAF
            Node 15128 (Hash)
              Node 15129 (Seq Scan) - LEAF
          Node 15130 (Index Scan) - LEAF
        Node 15131 (Hash)
          Node 15132 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15119 | 15110, 15111, 15120, 15121, 15122, 15123 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15110 | 15111, 15119, 15120, 15121, 15122, 15123 |
...
|------|------|--------|-----------|-----|--------|
| 15110 | Aggregate | 1237.06 | 1133.14 | 8.4% | pattern |
| 15112 | Sort | 1227.63 | 1046.21 | 14.8% | operator |
| 15113 | Hash Join | 1227.38 | 785.61 | 36.0% | operator |
| 15114 | Nested Loop | 1227.02 | 1198.29 | 2.3% | operator |
| 15131 | Hash | 0.02 | 13.72 | 80629.9% | operator |
| 15115 | Hash Join | 1223.35 | 785.60 | 35.8% | operator |
| 15130 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15132 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 15116 | Nested Loop | 1181.09 | 1198.29 | 1.5% | operator |
| 15128 | Hash | 37.83 | 19.97 | 47.2% | operator |
| 15117 | Hash Join | 219.51 | 251.67 | 14.7% | operator |
| 15127 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 15129 | Seq Scan | 37.75 | 48.88 | 29.5% | operator |
| 15118 | Seq Scan | 158.08 | 159.67 | 1.0% | operator |
| 15119 | Hash | 43.81 | 43.51 | 0.7% | pattern |
| 15124 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 15125 | Hash | 0.14 | 18.14 | 13047.9% | operator |
| 15126 | Seq Scan | 0.13 | 23.36 | 17334.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15126 (Seq Scan) - LEAF
...
### Step 2: Node 15124 (Seq Scan) - LEAF
...
### Step 4: Node 15118 (Seq Scan) - LEAF
...
### Step 5: Node 15119 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15110, 15111, 15120, 15121, 15122, 15123
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15127 (Index Scan) - LEAF
...
### Step 8: Node 15129 (Seq Scan) - LEAF
...
### Step 12: Node 15130 (Index Scan) - LEAF
...
### Step 13: Node 15132 (Seq Scan) - LEAF
...
### Step 14: Node 15114 (Nested Loop)
...
### Step 18: Node 15110 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15111, 15119, 15120, 15121, 15122, 15123
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2416
  ...
  - Aggregate_sel=0.9959
  - Aggregate_startup_cost=71550.5800
  - Aggregate_total_cost=71932.8400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2416
  ...
  - GatherMerge_Outer_total_cost=71836.4000
- **Output:** st=1130.14, rt=1133.14
