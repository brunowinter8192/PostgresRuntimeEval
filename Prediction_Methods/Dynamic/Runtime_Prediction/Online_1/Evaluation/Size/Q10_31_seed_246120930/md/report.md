...
**Test Query:** Q10_31_seed_246120930
...
```
Node 20332 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20333 (Sort) [consumed]
    Node 20334 (Aggregate) [consumed]
      Node 20335 (Gather) [consumed]
        Node 20336 (Hash Join)
          Node 20337 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20338 (Nested Loop) [consumed]
              Node 20339 (Seq Scan) - LEAF
              Node 20340 (Index Scan) - LEAF
            Node 20341 (Hash) [consumed]
              Node 20342 (Seq Scan) - LEAF
          Node 20343 (Hash)
            Node 20344 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20332 | 20333, 20334, 20335, 20337, 20338, 20341 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20337 | 20332, 20333, 20334, 20335, 20338, 20341 |
...
|------|------|--------|-----------|-----|--------|
| 20332 | Limit | 1135.95 | 1199.47 | 5.6% | pattern |
| 20336 | Hash Join | 1095.58 | 1095.39 | 0.0% | operator |
| 20337 | Hash Join | 1093.01 | 1120.19 | 2.5% | pattern |
| 20343 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 20344 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20339 | Seq Scan | 163.59 | 159.75 | 2.4% | operator |
| 20340 | Index Scan | 0.06 | -0.52 | 935.8% | operator |
| 20342 | Seq Scan | 27.99 | 26.64 | 4.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20339 (Seq Scan) - LEAF
...
### Step 2: Node 20340 (Index Scan) - LEAF
...
### Step 3: Node 20342 (Seq Scan) - LEAF
...
### Step 4: Node 20344 (Seq Scan) - LEAF
...
### Step 5: Node 20337 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20332, 20333, 20334, 20335, 20338, 20341
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17592
  - HashJoin_nt1=17592
...
### Step 8: Node 20332 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20333, 20334, 20335, 20337, 20338, 20341
- **Input Features:**
