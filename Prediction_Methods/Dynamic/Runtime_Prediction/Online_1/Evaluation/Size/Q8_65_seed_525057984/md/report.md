...
**Test Query:** Q8_65_seed_525057984
...
```
Node 15754 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15755 (Gather Merge) [consumed]
    Node 15756 (Sort)
      Node 15757 (Hash Join)
        Node 15758 (Nested Loop)
          Node 15759 (Hash Join)
            Node 15760 (Nested Loop)
              Node 15761 (Hash Join)
                Node 15762 (Seq Scan) - LEAF
                Node 15763 (Hash) [PATTERN: 444761fb]
                  Node 15764 (Hash Join) [consumed]
                    Node 15765 (Seq Scan) [consumed] - LEAF
                    Node 15766 (Hash) [consumed]
                      Node 15767 (Hash Join) [consumed]
                        Node 15768 (Seq Scan) - LEAF
                        Node 15769 (Hash)
                          Node 15770 (Seq Scan) - LEAF
              Node 15771 (Index Scan) - LEAF
            Node 15772 (Hash)
              Node 15773 (Seq Scan) - LEAF
          Node 15774 (Index Scan) - LEAF
        Node 15775 (Hash)
          Node 15776 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15763 | 15754, 15755, 15764, 15765, 15766, 15767 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15754 | 15755, 15763, 15764, 15765, 15766, 15767 |
...
|------|------|--------|-----------|-----|--------|
| 15754 | Aggregate | 1245.53 | 1133.65 | 9.0% | pattern |
| 15756 | Sort | 1239.01 | 1046.21 | 15.6% | operator |
| 15757 | Hash Join | 1238.79 | 785.61 | 36.6% | operator |
| 15758 | Nested Loop | 1238.50 | 1198.29 | 3.2% | operator |
| 15775 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 15759 | Hash Join | 1234.80 | 785.60 | 36.4% | operator |
| 15774 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 15776 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 15760 | Nested Loop | 1189.32 | 1198.29 | 0.8% | operator |
| 15772 | Hash | 41.29 | 19.97 | 51.6% | operator |
| 15761 | Hash Join | 215.27 | 251.67 | 16.9% | operator |
| 15771 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 15773 | Seq Scan | 41.23 | 48.90 | 18.6% | operator |
| 15762 | Seq Scan | 159.08 | 159.67 | 0.4% | operator |
| 15763 | Hash | 38.19 | 43.51 | 13.9% | pattern |
| 15768 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 15769 | Hash | 0.23 | 18.14 | 7964.1% | operator |
| 15770 | Seq Scan | 0.22 | 23.36 | 10519.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15770 (Seq Scan) - LEAF
...
### Step 2: Node 15768 (Seq Scan) - LEAF
...
### Step 4: Node 15762 (Seq Scan) - LEAF
...
### Step 5: Node 15763 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15754, 15755, 15764, 15765, 15766, 15767
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15771 (Index Scan) - LEAF
...
### Step 8: Node 15773 (Seq Scan) - LEAF
...
- **Output:** st=1.66, rt=48.90

### Step 9: Node 15760 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9739
  - startup_cost=9919.5400
  - total_cost=70271.0200
- **Output:** st=113.62, rt=785.60

### Step 12: Node 15774 (Index Scan) - LEAF
...
### Step 13: Node 15776 (Seq Scan) - LEAF
...
### Step 14: Node 15758 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9919.8200
  - total_cost=70488.5500
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15775 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.3900
  - total_cost=70494.1200
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15756 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70528.2300
  - total_cost=70530.0300
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 15754 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15755, 15763, 15764, 15765, 15766, 15767
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2230
  - Aggregate_nt1=2230
  ...
  - Aggregate_startup_cost=71528.2700
  - Aggregate_total_cost=71881.2900
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2230
  - GatherMerge_Outer_nt1=719
  ...
  - GatherMerge_Outer_sel=3.1015
  - GatherMerge_Outer_startup_cost=71528.2700
  - GatherMerge_Outer_total_cost=71792.0900
- **Output:** st=1130.65, rt=1133.65
