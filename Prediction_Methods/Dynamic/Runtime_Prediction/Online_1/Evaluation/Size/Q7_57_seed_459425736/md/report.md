...
**Test Query:** Q7_57_seed_459425736
...
```
Node 12337 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12338 (Gather Merge) [consumed]
    Node 12339 (Sort)
      Node 12340 (Hash Join)
        Node 12341 (Nested Loop)
          Node 12342 (Hash Join) [PATTERN: 895c6e8c]
            Node 12343 (Seq Scan) [consumed] - LEAF
            Node 12344 (Hash) [consumed]
              Node 12345 (Hash Join) [PATTERN: f4cb205a]
                Node 12346 (Seq Scan) [consumed] - LEAF
                Node 12347 (Hash) [consumed]
                  Node 12348 (Seq Scan) [consumed] - LEAF
          Node 12349 (Index Scan) - LEAF
        Node 12350 (Hash)
          Node 12351 (Hash Join) [PATTERN: f4cb205a]
            Node 12352 (Seq Scan) [consumed] - LEAF
            Node 12353 (Hash) [consumed]
              Node 12354 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12345 | 12337, 12338, 12342, 12343, 12344, 12346, 12347, 12348, 12351, 12352, 12353, 12354 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12351 | 12337, 12338, 12342, 12343, 12344, 12345, 12346, 12347, 12348, 12352, 12353, 12354 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12337 | 12338, 12342, 12343, 12344, 12345, 12346, 12347, 12348, 12351, 12352, 12353, 12354 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12342 | 12337, 12338, 12343, 12344, 12345, 12346, 12347, 12348, 12351, 12352, 12353, 12354 |
...
|------|------|--------|-----------|-----|--------|
| 12337 | Aggregate | 1215.22 | 1163.13 | 4.3% | pattern |
| 12339 | Sort | 1209.25 | 1109.16 | 8.3% | operator |
| 12340 | Hash Join | 1208.64 | 843.11 | 30.2% | operator |
| 12341 | Nested Loop | 1201.35 | 1215.99 | 1.2% | operator |
| 12350 | Hash | 3.62 | 27.74 | 666.4% | operator |
| 12342 | Hash Join | 222.58 | 304.89 | 37.0% | pattern |
| 12349 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12351 | Hash Join | 3.58 | 429.36 | 11896.7% | pattern |
| 12345 | Hash Join | 36.26 | 429.38 | 1084.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12345 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12337, 12338, 12342, 12343, 12344, 12346, 12347, 12348, 12351, 12352, 12353, 12354
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12342 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12337, 12338, 12343, 12344, 12345, 12346, 12347, 12348, 12351, 12352, 12353, 12354
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12349 (Index Scan) - LEAF
...
### Step 4: Node 12351 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12337, 12338, 12342, 12343, 12344, 12345, 12346, 12347, 12348, 12352, 12353, 12354
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12337 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12338, 12342, 12343, 12344, 12345, 12346, 12347, 12348, 12351, 12352, 12353, 12354
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
