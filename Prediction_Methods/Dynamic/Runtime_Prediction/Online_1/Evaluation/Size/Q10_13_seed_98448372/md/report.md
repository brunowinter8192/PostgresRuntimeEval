...
**Test Query:** Q10_13_seed_98448372
...
```
Node 19929 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19930 (Sort) [consumed]
    Node 19931 (Aggregate) [consumed]
      Node 19932 (Gather) [consumed]
        Node 19933 (Hash Join)
          Node 19934 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19935 (Nested Loop) [consumed]
              Node 19936 (Seq Scan) - LEAF
              Node 19937 (Index Scan) - LEAF
            Node 19938 (Hash) [consumed]
              Node 19939 (Seq Scan) - LEAF
          Node 19940 (Hash)
            Node 19941 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19929 | 19930, 19931, 19932, 19934, 19935, 19938 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19934 | 19929, 19930, 19931, 19932, 19935, 19938 |
...
|------|------|--------|-----------|-----|--------|
| 19929 | Limit | 1160.96 | 1199.47 | 3.3% | pattern |
| 19933 | Hash Join | 1109.10 | 1095.53 | 1.2% | operator |
| 19934 | Hash Join | 1106.18 | 1120.19 | 1.3% | pattern |
| 19940 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 19941 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 19936 | Seq Scan | 162.91 | 159.74 | 1.9% | operator |
| 19937 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 19939 | Seq Scan | 29.14 | 26.64 | 8.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19936 (Seq Scan) - LEAF
...
### Step 2: Node 19937 (Index Scan) - LEAF
...
### Step 3: Node 19939 (Seq Scan) - LEAF
...
### Step 4: Node 19941 (Seq Scan) - LEAF
...
### Step 5: Node 19934 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19929, 19930, 19931, 19932, 19935, 19938
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18089
  - HashJoin_nt1=18089
...
### Step 8: Node 19929 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19930, 19931, 19932, 19934, 19935, 19938
- **Input Features:**
