...
**Test Query:** Q8_88_seed_713750697
...
Node 16329 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16330 (Gather Merge) [consumed]
    Node 16331 (Sort)
      Node 16332 (Hash Join)
        Node 16333 (Nested Loop)
          Node 16334 (Hash Join)
            Node 16335 (Nested Loop)
              Node 16336 (Hash Join)
                Node 16337 (Seq Scan) - LEAF
                Node 16338 (Hash) [PATTERN: 444761fb]
                  Node 16339 (Hash Join) [consumed]
                    Node 16340 (Seq Scan) [consumed] - LEAF
                    Node 16341 (Hash) [consumed]
                      Node 16342 (Hash Join) [consumed]
                        Node 16343 (Seq Scan) - LEAF
                        Node 16344 (Hash)
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16338 | 16329, 16330, 16339, 16340, 16341, 16342 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16329 | 16330, 16338, 16339, 16340, 16341, 16342 |
...
|------|------|--------|-----------|-----|--------|
| 16329 | Aggregate | 1212.45 | 1130.18 | 6.8% | pattern |
| 16331 | Sort | 1208.16 | 1046.21 | 13.4% | operator |
| 16332 | Hash Join | 1207.88 | 785.61 | 35.0% | operator |
| 16333 | Nested Loop | 1206.98 | 1198.29 | 0.7% | operator |
| 16350 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 16334 | Hash Join | 1203.27 | 785.60 | 34.7% | operator |
| 16349 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16351 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 16335 | Nested Loop | 1157.25 | 1198.29 | 3.5% | operator |
| 16347 | Hash | 41.49 | 19.97 | 51.9% | operator |
| 16336 | Hash Join | 213.21 | 251.67 | 18.0% | operator |
| 16346 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 16348 | Seq Scan | 41.38 | 48.87 | 18.1% | operator |
| 16337 | Seq Scan | 158.64 | 159.67 | 0.6% | operator |
| 16338 | Hash | 36.07 | 43.51 | 20.6% | pattern |
| 16343 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 16344 | Hash | 0.16 | 18.14 | 10963.5% | operator |
| 16345 | Seq Scan | 0.16 | 23.36 | 14501.4% | operator |
...
### Step 2: Node 16343 (Seq Scan) - LEAF
...
### Step 4: Node 16337 (Seq Scan) - LEAF
...
### Step 5: Node 16338 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16329, 16330, 16339, 16340, 16341, 16342
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16346 (Index Scan) - LEAF
...
### Step 8: Node 16348 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - total_cost=70271.7200
- **Output:** st=113.62, rt=785.60

### Step 12: Node 16349 (Index Scan) - LEAF
...
### Step 13: Node 16351 (Seq Scan) - LEAF
...
### Step 14: Node 16333 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.5200
  - total_cost=70513.1500
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 16350 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.0900
  - total_cost=70519.1600
- **Output:** st=113.62, rt=785.61

### Step 17: Node 16331 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70557.6200
  - total_cost=70559.6200
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 16329 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16330, 16338, 16339, 16340, 16341, 16342
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2475
  ...
  - Aggregate_total_cost=71948.0900
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2475
  ...
  - GatherMerge_Outer_total_cost=71850.4700
- **Output:** st=1127.21, rt=1130.18
