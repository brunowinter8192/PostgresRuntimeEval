...
**Test Query:** Q5_149_seed_1214196588
...
```
Node 7961 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7962 (Aggregate) [consumed]
    Node 7963 (Gather Merge) [consumed]
      Node 7964 (Aggregate)
        Node 7965 (Sort) [PATTERN: 3e2d5a00]
          Node 7966 (Hash Join) [consumed]
            Node 7967 (Nested Loop)
              Node 7968 (Hash Join)
                Node 7969 (Seq Scan) - LEAF
                Node 7970 (Hash) [PATTERN: 444761fb]
                  Node 7971 (Hash Join) [consumed]
                    Node 7972 (Seq Scan) [consumed] - LEAF
                    Node 7973 (Hash) [consumed]
                      Node 7974 (Hash Join) [consumed]
                        Node 7975 (Seq Scan) - LEAF
                        Node 7976 (Hash)
                          Node 7977 (Seq Scan) - LEAF
              Node 7978 (Index Scan) - LEAF
            Node 7979 (Hash)
              Node 7980 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7970 | 7961, 7962, 7963, 7965, 7966, 7971, 7972, 7973, 7974 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7961 | 7962, 7963, 7965, 7966, 7970, 7971, 7972, 7973, 7974 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7965 | 7961, 7962, 7963, 7966, 7970, 7971, 7972, 7973, 7974 |
...
|------|------|--------|-----------|-----|--------|
| 7961 | Sort | 1079.87 | 2934.42 | 171.7% | pattern |
| 7964 | Aggregate | 1074.49 | 1128.79 | 5.1% | operator |
| 7965 | Sort | 1074.22 | 1835.90 | 70.9% | pattern |
| 7967 | Nested Loop | 1062.24 | 1188.37 | 11.9% | operator |
| 7979 | Hash | 3.31 | 18.67 | 463.2% | operator |
| 7968 | Hash Join | 220.24 | 253.07 | 14.9% | operator |
| 7978 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 7980 | Seq Scan | 2.76 | 12.36 | 347.2% | operator |
| 7969 | Seq Scan | 167.28 | 159.81 | 4.5% | operator |
| 7970 | Hash | 40.08 | 46.85 | 16.9% | pattern |
| 7975 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 7976 | Hash | 0.54 | 21.05 | 3791.5% | operator |
| 7977 | Seq Scan | 0.54 | 22.93 | 4162.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7977 (Seq Scan) - LEAF
...
### Step 2: Node 7975 (Seq Scan) - LEAF
...
### Step 4: Node 7969 (Seq Scan) - LEAF
...
### Step 5: Node 7970 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7961, 7962, 7963, 7965, 7966, 7971, 7972, 7973, 7974
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7978 (Index Scan) - LEAF
...
### Step 8: Node 7980 (Seq Scan) - LEAF
...
### Step 11: Node 7965 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7961, 7962, 7963, 7966, 7970, 7971, 7972, 7973, 7974
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7961 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7962, 7963, 7965, 7966, 7970, 7971, 7972, 7973, 7974
- **Input Features:**
