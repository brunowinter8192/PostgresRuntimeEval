...
```
Node 12589 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12590 (Gather Merge) [consumed]
    Node 12591 (Sort)
      Node 12592 (Hash Join)
        Node 12593 (Nested Loop)
          Node 12594 (Hash Join) [PATTERN: 895c6e8c]
            Node 12595 (Seq Scan) [consumed] - LEAF
            Node 12596 (Hash) [consumed]
              Node 12597 (Hash Join) [PATTERN: f4cb205a]
                Node 12598 (Seq Scan) [consumed] - LEAF
                Node 12599 (Hash) [consumed]
                  Node 12600 (Seq Scan) [consumed] - LEAF
          Node 12601 (Index Scan) - LEAF
        Node 12602 (Hash)
          Node 12603 (Hash Join) [PATTERN: f4cb205a]
            Node 12604 (Seq Scan) [consumed] - LEAF
            Node 12605 (Hash) [consumed]
              Node 12606 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12597 | 12589, 12590, 12594, 12595, 12596, 12598, 12599, 12600, 12603, 12604, 12605, 12606 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12603 | 12589, 12590, 12594, 12595, 12596, 12597, 12598, 12599, 12600, 12604, 12605, 12606 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12589 | 12590, 12594, 12595, 12596, 12597, 12598, 12599, 12600, 12603, 12604, 12605, 12606 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12594 | 12589, 12590, 12595, 12596, 12597, 12598, 12599, 12600, 12603, 12604, 12605, 12606 |
...
|------|------|--------|-----------|-----|--------|
| 12589 | Aggregate | 1195.50 | 1163.13 | 2.7% | pattern |
| 12591 | Sort | 1188.39 | 1109.16 | 6.7% | operator |
| 12592 | Hash Join | 1187.77 | 843.11 | 29.0% | operator |
| 12593 | Nested Loop | 1180.73 | 1215.99 | 3.0% | operator |
| 12602 | Hash | 3.23 | 27.74 | 760.3% | operator |
| 12594 | Hash Join | 216.69 | 304.89 | 40.7% | pattern |
| 12601 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 12603 | Hash Join | 3.17 | 429.36 | 13440.3% | pattern |
| 12597 | Hash Join | 36.00 | 429.38 | 1092.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12597 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12589, 12590, 12594, 12595, 12596, 12598, 12599, 12600, 12603, 12604, 12605, 12606
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12594 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12589, 12590, 12595, 12596, 12597, 12598, 12599, 12600, 12603, 12604, 12605, 12606
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12601 (Index Scan) - LEAF
...
### Step 4: Node 12603 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12589, 12590, 12594, 12595, 12596, 12597, 12598, 12599, 12600, 12604, 12605, 12606
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12589 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12590, 12594, 12595, 12596, 12597, 12598, 12599, 12600, 12603, 12604, 12605, 12606
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
