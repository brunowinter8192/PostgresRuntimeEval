...
**Test Query:** Q5_146_seed_1189584495
...
```
Node 7901 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 7902 (Aggregate) [consumed]
    Node 7903 (Gather Merge) [consumed]
      Node 7904 (Aggregate)
        Node 7905 (Sort) [PATTERN: 3e2d5a00]
          Node 7906 (Hash Join) [consumed]
            Node 7907 (Nested Loop)
              Node 7908 (Hash Join)
                Node 7909 (Seq Scan) - LEAF
                Node 7910 (Hash) [PATTERN: 444761fb]
                  Node 7911 (Hash Join) [consumed]
                    Node 7912 (Seq Scan) [consumed] - LEAF
                    Node 7913 (Hash) [consumed]
                      Node 7914 (Hash Join) [consumed]
                        Node 7915 (Seq Scan) - LEAF
                        Node 7916 (Hash)
                          Node 7917 (Seq Scan) - LEAF
              Node 7918 (Index Scan) - LEAF
            Node 7919 (Hash)
              Node 7920 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 7910 | 7901, 7902, 7903, 7905, 7906, 7911, 7912, 7913, 7914 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 7901 | 7902, 7903, 7905, 7906, 7910, 7911, 7912, 7913, 7914 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 7905 | 7901, 7902, 7903, 7906, 7910, 7911, 7912, 7913, 7914 |
...
|------|------|--------|-----------|-----|--------|
| 7901 | Sort | 1062.00 | 2934.42 | 176.3% | pattern |
| 7904 | Aggregate | 1057.71 | 1128.78 | 6.7% | operator |
| 7905 | Sort | 1057.46 | 1835.92 | 73.6% | pattern |
| 7907 | Nested Loop | 1045.00 | 1188.42 | 13.7% | operator |
| 7919 | Hash | 3.89 | 18.67 | 380.1% | operator |
| 7908 | Hash Join | 218.06 | 253.07 | 16.1% | operator |
| 7918 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 7920 | Seq Scan | 3.33 | 12.36 | 271.3% | operator |
| 7909 | Seq Scan | 168.96 | 159.81 | 5.4% | operator |
| 7910 | Hash | 34.42 | 46.85 | 36.1% | pattern |
| 7915 | Seq Scan | 0.17 | 11.81 | 6765.3% | operator |
| 7916 | Hash | 0.38 | 21.05 | 5440.3% | operator |
| 7917 | Seq Scan | 0.05 | 22.93 | 45766.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 7917 (Seq Scan) - LEAF
...
### Step 2: Node 7915 (Seq Scan) - LEAF
...
### Step 4: Node 7909 (Seq Scan) - LEAF
...
### Step 5: Node 7910 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 7901, 7902, 7903, 7905, 7906, 7911, 7912, 7913, 7914
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 7918 (Index Scan) - LEAF
...
### Step 8: Node 7920 (Seq Scan) - LEAF
...
### Step 11: Node 7905 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 7901, 7902, 7903, 7906, 7910, 7911, 7912, 7913, 7914
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 7901 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 7902, 7903, 7905, 7906, 7910, 7911, 7912, 7913, 7914
- **Input Features:**
