...
**Test Query:** Q7_85_seed_689138604
...
```
Node 12895 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 12896 (Gather Merge) [consumed]
    Node 12897 (Sort)
      Node 12898 (Hash Join)
        Node 12899 (Nested Loop)
          Node 12900 (Hash Join) [PATTERN: 895c6e8c]
            Node 12901 (Seq Scan) [consumed] - LEAF
            Node 12902 (Hash) [consumed]
              Node 12903 (Hash Join) [PATTERN: f4cb205a]
                Node 12904 (Seq Scan) [consumed] - LEAF
                Node 12905 (Hash) [consumed]
                  Node 12906 (Seq Scan) [consumed] - LEAF
          Node 12907 (Index Scan) - LEAF
        Node 12908 (Hash)
          Node 12909 (Hash Join) [PATTERN: f4cb205a]
            Node 12910 (Seq Scan) [consumed] - LEAF
            Node 12911 (Hash) [consumed]
              Node 12912 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12903 | 12895, 12896, 12900, 12901, 12902, 12904, 12905, 12906, 12909, 12910, 12911, 12912 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 12909 | 12895, 12896, 12900, 12901, 12902, 12903, 12904, 12905, 12906, 12910, 12911, 12912 |
| Aggregate -> Gather Merge (Out | 2724c080 | 12895 | 12896, 12900, 12901, 12902, 12903, 12904, 12905, 12906, 12909, 12910, 12911, 12912 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 12900 | 12895, 12896, 12901, 12902, 12903, 12904, 12905, 12906, 12909, 12910, 12911, 12912 |
...
|------|------|--------|-----------|-----|--------|
| 12895 | Aggregate | 1212.39 | 1163.13 | 4.1% | pattern |
| 12897 | Sort | 1205.93 | 1109.16 | 8.0% | operator |
| 12898 | Hash Join | 1205.28 | 843.11 | 30.0% | operator |
| 12899 | Nested Loop | 1198.23 | 1215.99 | 1.5% | operator |
| 12908 | Hash | 3.33 | 27.74 | 732.1% | operator |
| 12900 | Hash Join | 212.72 | 304.89 | 43.3% | pattern |
| 12907 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 12909 | Hash Join | 3.29 | 429.36 | 12958.5% | pattern |
| 12903 | Hash Join | 33.80 | 429.38 | 1170.4% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 12903 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12895, 12896, 12900, 12901, 12902, 12904, 12905, 12906, 12909, 12910, 12911, 12912
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 12900 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 12895, 12896, 12901, 12902, 12903, 12904, 12905, 12906, 12909, 12910, 12911, 12912
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 12907 (Index Scan) - LEAF
...
### Step 4: Node 12909 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 12895, 12896, 12900, 12901, 12902, 12903, 12904, 12905, 12906, 12910, 12911, 12912
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 12895 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 12896, 12900, 12901, 12902, 12903, 12904, 12905, 12906, 12909, 12910, 12911, 12912
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
