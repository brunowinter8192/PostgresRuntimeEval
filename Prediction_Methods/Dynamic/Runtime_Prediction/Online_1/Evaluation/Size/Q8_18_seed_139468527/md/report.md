...
**Test Query:** Q8_18_seed_139468527
**Timestamp:** 2026-01-18 18:27:53
...
```
Node 14558 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14559 (Gather Merge) [consumed]
    Node 14560 (Sort)
      Node 14561 (Hash Join)
        Node 14562 (Nested Loop)
          Node 14563 (Hash Join)
            Node 14564 (Nested Loop)
              Node 14565 (Hash Join)
                Node 14566 (Seq Scan) - LEAF
                Node 14567 (Hash) [PATTERN: 444761fb]
                  Node 14568 (Hash Join) [consumed]
                    Node 14569 (Seq Scan) [consumed] - LEAF
                    Node 14570 (Hash) [consumed]
                      Node 14571 (Hash Join) [consumed]
                        Node 14572 (Seq Scan) - LEAF
                        Node 14573 (Hash)
                          Node 14574 (Seq Scan) - LEAF
              Node 14575 (Index Scan) - LEAF
            Node 14576 (Hash)
              Node 14577 (Seq Scan) - LEAF
          Node 14578 (Index Scan) - LEAF
        Node 14579 (Hash)
          Node 14580 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14567 | 14558, 14559, 14568, 14569, 14570, 14571 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14558 | 14559, 14567, 14568, 14569, 14570, 14571 |
...
|------|------|--------|-----------|-----|--------|
| 14558 | Aggregate | 1229.54 | 1120.48 | 8.9% | pattern |
| 14560 | Sort | 1224.25 | 1046.22 | 14.5% | operator |
| 14561 | Hash Join | 1223.98 | 785.61 | 35.8% | operator |
| 14562 | Nested Loop | 1223.66 | 1198.29 | 2.1% | operator |
| 14579 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 14563 | Hash Join | 1219.75 | 785.60 | 35.6% | operator |
| 14578 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14580 | Seq Scan | 0.01 | 6.51 | 72255.1% | operator |
| 14564 | Nested Loop | 1176.41 | 1198.29 | 1.9% | operator |
| 14576 | Hash | 38.79 | 19.96 | 48.5% | operator |
| 14565 | Hash Join | 214.84 | 251.67 | 17.1% | operator |
| 14575 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14577 | Seq Scan | 38.70 | 48.84 | 26.2% | operator |
| 14566 | Seq Scan | 161.85 | 159.67 | 1.4% | operator |
| 14567 | Hash | 34.45 | 43.51 | 26.3% | pattern |
| 14572 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 14573 | Hash | 0.15 | 18.14 | 11605.9% | operator |
| 14574 | Seq Scan | 0.15 | 23.36 | 15269.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14574 (Seq Scan) - LEAF
...
### Step 2: Node 14572 (Seq Scan) - LEAF
...
### Step 4: Node 14566 (Seq Scan) - LEAF
...
### Step 5: Node 14567 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14558, 14559, 14568, 14569, 14570, 14571
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14575 (Index Scan) - LEAF
...
### Step 8: Node 14577 (Seq Scan) - LEAF
...
- **Output:** st=1.66, rt=48.84

### Step 9: Node 14564 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=5169.6700
- **Output:** st=19.97, rt=19.96

### Step 11: Node 14563 (Hash Join)
...
- **Input Features:**
  ...
  - total_cost=70272.3300
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14578 (Index Scan) - LEAF
...
### Step 13: Node 14580 (Seq Scan) - LEAF
...
### Step 14: Node 14562 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9921.1400
  - total_cost=70534.9400
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14579 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.7000
  - total_cost=70541.3400
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14560 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70583.7000
  - total_cost=70585.8700
- **Output:** st=1044.95, rt=1046.22

### Step 18: Node 14558 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14559, 14567, 14568, 14569, 14570, 14571
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2691
  ...
  - Aggregate_sel=0.8941
  - Aggregate_startup_cost=71583.7400
  - Aggregate_total_cost=72004.0500
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2691
  - GatherMerge_Outer_nt1=868
  ...
  - GatherMerge_Outer_sel=3.1002
  - GatherMerge_Outer_startup_cost=71583.7400
  - GatherMerge_Outer_total_cost=71902.1100
- **Output:** st=1117.60, rt=1120.48
