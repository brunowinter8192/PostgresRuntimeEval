...
**Test Query:** Q10_53_seed_426609612
...
```
Node 20644 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20645 (Sort) [consumed]
    Node 20646 (Aggregate) [consumed]
      Node 20647 (Gather) [consumed]
        Node 20648 (Hash Join)
          Node 20649 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20650 (Nested Loop) [consumed]
              Node 20651 (Seq Scan) - LEAF
              Node 20652 (Index Scan) - LEAF
            Node 20653 (Hash) [consumed]
              Node 20654 (Seq Scan) - LEAF
          Node 20655 (Hash)
            Node 20656 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20644 | 20645, 20646, 20647, 20649, 20650, 20653 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20649 | 20644, 20645, 20646, 20647, 20650, 20653 |
...
|------|------|--------|-----------|-----|--------|
| 20644 | Limit | 1160.49 | 1199.47 | 3.4% | pattern |
| 20648 | Hash Join | 1107.26 | 1095.51 | 1.1% | operator |
| 20649 | Hash Join | 1104.43 | 1120.19 | 1.4% | pattern |
| 20655 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 20656 | Seq Scan | 0.01 | 6.98 | 77441.2% | operator |
| 20651 | Seq Scan | 162.22 | 159.74 | 1.5% | operator |
| 20652 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20654 | Seq Scan | 29.27 | 26.64 | 9.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20651 (Seq Scan) - LEAF
...
### Step 2: Node 20652 (Index Scan) - LEAF
...
### Step 3: Node 20654 (Seq Scan) - LEAF
...
### Step 4: Node 20656 (Seq Scan) - LEAF
...
### Step 5: Node 20649 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20644, 20645, 20646, 20647, 20650, 20653
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18043
  - HashJoin_nt1=18043
...
- **Output:** st=54.20, rt=1095.51

### Step 8: Node 20644 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20645, 20646, 20647, 20649, 20650, 20653
- **Input Features:**
