...
**Test Query:** Q8_138_seed_1123952247
...
```
Node 14144 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14145 (Gather Merge) [consumed]
    Node 14146 (Sort)
      Node 14147 (Hash Join)
        Node 14148 (Nested Loop)
          Node 14149 (Hash Join)
            Node 14150 (Nested Loop)
              Node 14151 (Hash Join)
                Node 14152 (Seq Scan) - LEAF
                Node 14153 (Hash) [PATTERN: 444761fb]
                  Node 14154 (Hash Join) [consumed]
                    Node 14155 (Seq Scan) [consumed] - LEAF
                    Node 14156 (Hash) [consumed]
                      Node 14157 (Hash Join) [consumed]
                        Node 14158 (Seq Scan) - LEAF
                        Node 14159 (Hash)
                          Node 14160 (Seq Scan) - LEAF
              Node 14161 (Index Scan) - LEAF
            Node 14162 (Hash)
              Node 14163 (Seq Scan) - LEAF
          Node 14164 (Index Scan) - LEAF
        Node 14165 (Hash)
          Node 14166 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14153 | 14144, 14145, 14154, 14155, 14156, 14157 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14144 | 14145, 14153, 14154, 14155, 14156, 14157 |
...
|------|------|--------|-----------|-----|--------|
| 14144 | Aggregate | 1204.71 | 1131.48 | 6.1% | pattern |
| 14146 | Sort | 1199.00 | 1046.21 | 12.7% | operator |
| 14147 | Hash Join | 1198.72 | 785.61 | 34.5% | operator |
| 14148 | Nested Loop | 1198.32 | 1198.29 | 0.0% | operator |
| 14165 | Hash | 0.02 | 13.72 | 80629.9% | operator |
| 14149 | Hash Join | 1194.46 | 785.60 | 34.2% | operator |
| 14164 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14166 | Seq Scan | 0.01 | 6.51 | 49992.0% | operator |
| 14150 | Nested Loop | 1149.19 | 1198.29 | 4.3% | operator |
| 14162 | Hash | 40.78 | 19.97 | 51.0% | operator |
| 14151 | Hash Join | 216.18 | 251.67 | 16.4% | operator |
| 14161 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14163 | Seq Scan | 40.70 | 48.87 | 20.1% | operator |
| 14152 | Seq Scan | 159.69 | 159.67 | 0.0% | operator |
| 14153 | Hash | 38.91 | 43.51 | 11.8% | pattern |
| 14158 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 14159 | Hash | 0.17 | 18.14 | 10764.8% | operator |
| 14160 | Seq Scan | 0.16 | 23.36 | 14593.2% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14160 (Seq Scan) - LEAF
...
### Step 2: Node 14158 (Seq Scan) - LEAF
...
### Step 4: Node 14152 (Seq Scan) - LEAF
...
### Step 5: Node 14153 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14144, 14145, 14154, 14155, 14156, 14157
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14161 (Index Scan) - LEAF
...
### Step 8: Node 14163 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - total_cost=70271.6400
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14164 (Index Scan) - LEAF
...
### Step 13: Node 14166 (Seq Scan) - LEAF
...
### Step 14: Node 14148 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.4500
  - total_cost=70510.6500
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14165 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.0100
  - total_cost=70516.6200
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14146 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70554.6400
  - total_cost=70556.6200
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14144 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14145, 14153, 14154, 14155, 14156, 14157
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2449
  ...
  - Aggregate_total_cost=71941.5100
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2449
  - GatherMerge_Outer_nt1=790
  ...
  - GatherMerge_Outer_total_cost=71844.4100
- **Output:** st=1128.50, rt=1131.48
