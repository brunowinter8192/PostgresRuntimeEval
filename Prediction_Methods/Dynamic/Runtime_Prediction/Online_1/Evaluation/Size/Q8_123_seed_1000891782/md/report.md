...
**Test Query:** Q8_123_seed_1000891782
...
```
Node 13776 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13777 (Gather Merge) [consumed]
    Node 13778 (Sort)
      Node 13779 (Hash Join)
        Node 13780 (Nested Loop)
          Node 13781 (Hash Join)
            Node 13782 (Nested Loop)
              Node 13783 (Hash Join)
                Node 13784 (Seq Scan) - LEAF
                Node 13785 (Hash) [PATTERN: 444761fb]
                  Node 13786 (Hash Join) [consumed]
                    Node 13787 (Seq Scan) [consumed] - LEAF
                    Node 13788 (Hash) [consumed]
                      Node 13789 (Hash Join) [consumed]
                        Node 13790 (Seq Scan) - LEAF
                        Node 13791 (Hash)
                          Node 13792 (Seq Scan) - LEAF
              Node 13793 (Index Scan) - LEAF
            Node 13794 (Hash)
              Node 13795 (Seq Scan) - LEAF
          Node 13796 (Index Scan) - LEAF
        Node 13797 (Hash)
          Node 13798 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13785 | 13776, 13777, 13786, 13787, 13788, 13789 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13776 | 13777, 13785, 13786, 13787, 13788, 13789 |
...
|------|------|--------|-----------|-----|--------|
| 13776 | Aggregate | 1212.34 | 1131.04 | 6.7% | pattern |
| 13778 | Sort | 1207.50 | 1046.21 | 13.4% | operator |
| 13779 | Hash Join | 1207.23 | 785.61 | 34.9% | operator |
| 13780 | Nested Loop | 1206.90 | 1198.29 | 0.7% | operator |
| 13797 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 13781 | Hash Join | 1202.98 | 785.60 | 34.7% | operator |
| 13796 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13798 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13782 | Nested Loop | 1156.17 | 1198.29 | 3.6% | operator |
| 13794 | Hash | 41.51 | 19.97 | 51.9% | operator |
| 13783 | Hash Join | 214.01 | 251.67 | 17.6% | operator |
| 13793 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 13795 | Seq Scan | 41.41 | 48.87 | 18.0% | operator |
| 13784 | Seq Scan | 160.03 | 159.67 | 0.2% | operator |
| 13785 | Hash | 35.77 | 43.51 | 21.7% | pattern |
| 13790 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13791 | Hash | 0.08 | 18.14 | 23161.7% | operator |
| 13792 | Seq Scan | 0.07 | 23.36 | 31049.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13792 (Seq Scan) - LEAF
...
### Step 2: Node 13790 (Seq Scan) - LEAF
...
### Step 4: Node 13784 (Seq Scan) - LEAF
...
### Step 5: Node 13785 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13776, 13777, 13786, 13787, 13788, 13789
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13793 (Index Scan) - LEAF
...
### Step 8: Node 13795 (Seq Scan) - LEAF
...
### Step 12: Node 13796 (Index Scan) - LEAF
...
### Step 13: Node 13798 (Seq Scan) - LEAF
...
### Step 14: Node 13780 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70511.5800
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 13797 (Hash)
...
- **Input Features:**
  ...
  - total_cost=70517.5700
- **Output:** st=113.62, rt=785.61

### Step 17: Node 13778 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70555.7600
  - total_cost=70557.7400
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 13776 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13777, 13785, 13786, 13787, 13788, 13789
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2458
  ...
  - Aggregate_total_cost=71943.8700
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2458
  - GatherMerge_Outer_nt1=793
  ...
  - GatherMerge_Outer_total_cost=71846.5900
- **Output:** st=1128.06, rt=1131.04
