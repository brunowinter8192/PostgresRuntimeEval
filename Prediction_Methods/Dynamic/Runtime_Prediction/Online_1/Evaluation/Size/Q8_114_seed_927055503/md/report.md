...
**Test Query:** Q8_114_seed_927055503
...
```
Node 13546 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13547 (Gather Merge) [consumed]
    Node 13548 (Sort)
      Node 13549 (Hash Join)
        Node 13550 (Nested Loop)
          Node 13551 (Hash Join)
            Node 13552 (Nested Loop)
              Node 13553 (Hash Join)
                Node 13554 (Seq Scan) - LEAF
                Node 13555 (Hash) [PATTERN: 444761fb]
                  Node 13556 (Hash Join) [consumed]
                    Node 13557 (Seq Scan) [consumed] - LEAF
                    Node 13558 (Hash) [consumed]
                      Node 13559 (Hash Join) [consumed]
                        Node 13560 (Seq Scan) - LEAF
                        Node 13561 (Hash)
                          Node 13562 (Seq Scan) - LEAF
              Node 13563 (Index Scan) - LEAF
            Node 13564 (Hash)
              Node 13565 (Seq Scan) - LEAF
          Node 13566 (Index Scan) - LEAF
        Node 13567 (Hash)
          Node 13568 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13555 | 13546, 13547, 13556, 13557, 13558, 13559 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13546 | 13547, 13555, 13556, 13557, 13558, 13559 |
...
|------|------|--------|-----------|-----|--------|
| 13546 | Aggregate | 1201.18 | 1129.95 | 5.9% | pattern |
| 13548 | Sort | 1194.87 | 1046.21 | 12.4% | operator |
| 13549 | Hash Join | 1194.63 | 785.61 | 34.2% | operator |
| 13550 | Nested Loop | 1194.33 | 1198.29 | 0.3% | operator |
| 13567 | Hash | 0.01 | 13.72 | 91393.9% | operator |
| 13551 | Hash Join | 1190.58 | 785.60 | 34.0% | operator |
| 13566 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13568 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 13552 | Nested Loop | 1147.43 | 1198.29 | 4.4% | operator |
| 13564 | Hash | 38.80 | 19.97 | 48.5% | operator |
| 13553 | Hash Join | 213.24 | 251.67 | 18.0% | operator |
| 13563 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 13565 | Seq Scan | 38.74 | 48.87 | 26.2% | operator |
| 13554 | Seq Scan | 156.41 | 159.67 | 2.1% | operator |
| 13555 | Hash | 38.17 | 43.51 | 14.0% | pattern |
| 13560 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 13561 | Hash | 0.29 | 18.14 | 6113.7% | operator |
| 13562 | Seq Scan | 0.29 | 23.36 | 7983.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13562 (Seq Scan) - LEAF
...
### Step 2: Node 13560 (Seq Scan) - LEAF
...
### Step 4: Node 13554 (Seq Scan) - LEAF
...
### Step 5: Node 13555 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13546, 13547, 13556, 13557, 13558, 13559
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13563 (Index Scan) - LEAF
...
### Step 8: Node 13565 (Seq Scan) - LEAF
...
### Step 12: Node 13566 (Index Scan) - LEAF
...
### Step 13: Node 13568 (Seq Scan) - LEAF
...
### Step 14: Node 13550 (Nested Loop)
...
### Step 18: Node 13546 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13547, 13555, 13556, 13557, 13558, 13559
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2480
