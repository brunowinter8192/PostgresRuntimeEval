...
**Test Query:** Q7_137_seed_1115748216
...
```
Node 11221 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11222 (Gather Merge) [consumed]
    Node 11223 (Sort)
      Node 11224 (Hash Join)
        Node 11225 (Nested Loop)
          Node 11226 (Hash Join) [PATTERN: 895c6e8c]
            Node 11227 (Seq Scan) [consumed] - LEAF
            Node 11228 (Hash) [consumed]
              Node 11229 (Hash Join) [PATTERN: f4cb205a]
                Node 11230 (Seq Scan) [consumed] - LEAF
                Node 11231 (Hash) [consumed]
                  Node 11232 (Seq Scan) [consumed] - LEAF
          Node 11233 (Index Scan) - LEAF
        Node 11234 (Hash)
          Node 11235 (Hash Join) [PATTERN: f4cb205a]
            Node 11236 (Seq Scan) [consumed] - LEAF
            Node 11237 (Hash) [consumed]
              Node 11238 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11229 | 11221, 11222, 11226, 11227, 11228, 11230, 11231, 11232, 11235, 11236, 11237, 11238 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11235 | 11221, 11222, 11226, 11227, 11228, 11229, 11230, 11231, 11232, 11236, 11237, 11238 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11221 | 11222, 11226, 11227, 11228, 11229, 11230, 11231, 11232, 11235, 11236, 11237, 11238 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11226 | 11221, 11222, 11227, 11228, 11229, 11230, 11231, 11232, 11235, 11236, 11237, 11238 |
...
|------|------|--------|-----------|-----|--------|
| 11221 | Aggregate | 1169.79 | 1163.13 | 0.6% | pattern |
| 11223 | Sort | 1163.13 | 1109.16 | 4.6% | operator |
| 11224 | Hash Join | 1162.52 | 843.11 | 27.5% | operator |
| 11225 | Nested Loop | 1156.10 | 1215.99 | 5.2% | operator |
| 11234 | Hash | 2.80 | 27.74 | 891.5% | operator |
| 11226 | Hash Join | 213.54 | 304.89 | 42.8% | pattern |
| 11233 | Index Scan | 0.03 | 0.96 | 3006.0% | operator |
| 11235 | Hash Join | 2.75 | 429.36 | 15501.8% | pattern |
| 11229 | Hash Join | 30.97 | 429.38 | 1286.4% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11229 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11221, 11222, 11226, 11227, 11228, 11230, 11231, 11232, 11235, 11236, 11237, 11238
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11226 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11221, 11222, 11227, 11228, 11229, 11230, 11231, 11232, 11235, 11236, 11237, 11238
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11233 (Index Scan) - LEAF
...
### Step 4: Node 11235 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11221, 11222, 11226, 11227, 11228, 11229, 11230, 11231, 11232, 11236, 11237, 11238
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11221 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11222, 11226, 11227, 11228, 11229, 11230, 11231, 11232, 11235, 11236, 11237, 11238
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
