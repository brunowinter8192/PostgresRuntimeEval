...
**Test Query:** Q8_93_seed_754770852
...
```
Node 16467 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16468 (Gather Merge) [consumed]
    Node 16469 (Sort)
      Node 16470 (Hash Join)
        Node 16471 (Nested Loop)
          Node 16472 (Hash Join)
            Node 16473 (Nested Loop)
              Node 16474 (Hash Join)
                Node 16475 (Seq Scan) - LEAF
                Node 16476 (Hash) [PATTERN: 444761fb]
                  Node 16477 (Hash Join) [consumed]
                    Node 16478 (Seq Scan) [consumed] - LEAF
                    Node 16479 (Hash) [consumed]
                      Node 16480 (Hash Join) [consumed]
                        Node 16481 (Seq Scan) - LEAF
                        Node 16482 (Hash)
                          Node 16483 (Seq Scan) - LEAF
              Node 16484 (Index Scan) - LEAF
            Node 16485 (Hash)
              Node 16486 (Seq Scan) - LEAF
          Node 16487 (Index Scan) - LEAF
        Node 16488 (Hash)
          Node 16489 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16476 | 16467, 16468, 16477, 16478, 16479, 16480 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16467 | 16468, 16476, 16477, 16478, 16479, 16480 |
...
|------|------|--------|-----------|-----|--------|
| 16467 | Aggregate | 1205.65 | 1133.20 | 6.0% | pattern |
| 16469 | Sort | 1200.38 | 1046.21 | 12.8% | operator |
| 16470 | Hash Join | 1200.12 | 785.61 | 34.5% | operator |
| 16471 | Nested Loop | 1199.81 | 1198.29 | 0.1% | operator |
| 16488 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 16472 | Hash Join | 1195.84 | 785.60 | 34.3% | operator |
| 16487 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16489 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 16473 | Nested Loop | 1157.55 | 1198.29 | 3.5% | operator |
| 16485 | Hash | 33.94 | 19.97 | 41.2% | operator |
| 16474 | Hash Join | 209.64 | 251.67 | 20.0% | operator |
| 16484 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 16486 | Seq Scan | 33.88 | 48.88 | 44.3% | operator |
| 16475 | Seq Scan | 159.96 | 159.67 | 0.2% | operator |
| 16476 | Hash | 31.98 | 43.51 | 36.0% | pattern |
| 16481 | Seq Scan | 0.01 | 12.22 | 111035.8% | operator |
| 16482 | Hash | 0.20 | 18.14 | 8794.2% | operator |
| 16483 | Seq Scan | 0.20 | 23.36 | 11581.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16483 (Seq Scan) - LEAF
...
### Step 2: Node 16481 (Seq Scan) - LEAF
...
### Step 4: Node 16475 (Seq Scan) - LEAF
...
### Step 5: Node 16476 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16467, 16468, 16477, 16478, 16479, 16480
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16484 (Index Scan) - LEAF
...
### Step 8: Node 16486 (Seq Scan) - LEAF
...
### Step 12: Node 16487 (Index Scan) - LEAF
...
### Step 13: Node 16489 (Seq Scan) - LEAF
...
### Step 14: Node 16471 (Nested Loop)
...
### Step 18: Node 16467 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16468, 16476, 16477, 16478, 16479, 16480
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2415
