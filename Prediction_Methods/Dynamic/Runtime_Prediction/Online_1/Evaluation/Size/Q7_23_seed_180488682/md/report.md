...
**Test Query:** Q7_23_seed_180488682
...
```
Node 11671 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11672 (Gather Merge) [consumed]
    Node 11673 (Sort)
      Node 11674 (Hash Join)
        Node 11675 (Nested Loop)
          Node 11676 (Hash Join) [PATTERN: 895c6e8c]
            Node 11677 (Seq Scan) [consumed] - LEAF
            Node 11678 (Hash) [consumed]
              Node 11679 (Hash Join) [PATTERN: f4cb205a]
                Node 11680 (Seq Scan) [consumed] - LEAF
                Node 11681 (Hash) [consumed]
                  Node 11682 (Seq Scan) [consumed] - LEAF
          Node 11683 (Index Scan) - LEAF
        Node 11684 (Hash)
          Node 11685 (Hash Join) [PATTERN: f4cb205a]
            Node 11686 (Seq Scan) [consumed] - LEAF
            Node 11687 (Hash) [consumed]
              Node 11688 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11679 | 11671, 11672, 11676, 11677, 11678, 11680, 11681, 11682, 11685, 11686, 11687, 11688 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11685 | 11671, 11672, 11676, 11677, 11678, 11679, 11680, 11681, 11682, 11686, 11687, 11688 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11671 | 11672, 11676, 11677, 11678, 11679, 11680, 11681, 11682, 11685, 11686, 11687, 11688 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11676 | 11671, 11672, 11677, 11678, 11679, 11680, 11681, 11682, 11685, 11686, 11687, 11688 |
...
|------|------|--------|-----------|-----|--------|
| 11671 | Aggregate | 1172.24 | 1163.13 | 0.8% | pattern |
| 11673 | Sort | 1165.32 | 1109.16 | 4.8% | operator |
| 11674 | Hash Join | 1164.69 | 843.11 | 27.6% | operator |
| 11675 | Nested Loop | 1156.69 | 1215.99 | 5.1% | operator |
| 11684 | Hash | 4.01 | 27.74 | 592.4% | operator |
| 11676 | Hash Join | 211.28 | 304.89 | 44.3% | pattern |
| 11683 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11685 | Hash Join | 3.96 | 429.36 | 10734.3% | pattern |
| 11679 | Hash Join | 34.45 | 429.38 | 1146.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11679 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11671, 11672, 11676, 11677, 11678, 11680, 11681, 11682, 11685, 11686, 11687, 11688
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11676 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11671, 11672, 11677, 11678, 11679, 11680, 11681, 11682, 11685, 11686, 11687, 11688
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11683 (Index Scan) - LEAF
...
### Step 4: Node 11685 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11671, 11672, 11676, 11677, 11678, 11679, 11680, 11681, 11682, 11686, 11687, 11688
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11671 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11672, 11676, 11677, 11678, 11679, 11680, 11681, 11682, 11685, 11686, 11687, 11688
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
