...
**Test Query:** Q5_102_seed_828607131
...
```
Node 6941 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 6942 (Aggregate) [consumed]
    Node 6943 (Gather Merge) [consumed]
      Node 6944 (Aggregate)
        Node 6945 (Sort) [PATTERN: 3e2d5a00]
          Node 6946 (Hash Join) [consumed]
            Node 6947 (Nested Loop)
              Node 6948 (Hash Join)
                Node 6949 (Seq Scan) - LEAF
                Node 6950 (Hash) [PATTERN: 444761fb]
                  Node 6951 (Hash Join) [consumed]
                    Node 6952 (Seq Scan) [consumed] - LEAF
                    Node 6953 (Hash) [consumed]
                      Node 6954 (Hash Join) [consumed]
                        Node 6955 (Seq Scan) - LEAF
                        Node 6956 (Hash)
                          Node 6957 (Seq Scan) - LEAF
              Node 6958 (Index Scan) - LEAF
            Node 6959 (Hash)
              Node 6960 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 6950 | 6941, 6942, 6943, 6945, 6946, 6951, 6952, 6953, 6954 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 6941 | 6942, 6943, 6945, 6946, 6950, 6951, 6952, 6953, 6954 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 6945 | 6941, 6942, 6943, 6946, 6950, 6951, 6952, 6953, 6954 |
...
|------|------|--------|-----------|-----|--------|
| 6941 | Sort | 1067.28 | 2934.42 | 174.9% | pattern |
| 6944 | Aggregate | 1062.34 | 1128.79 | 6.3% | operator |
| 6945 | Sort | 1062.08 | 1835.90 | 72.9% | pattern |
| 6947 | Nested Loop | 1050.07 | 1188.37 | 13.2% | operator |
| 6959 | Hash | 3.46 | 18.67 | 440.3% | operator |
| 6948 | Hash Join | 215.20 | 253.07 | 17.6% | operator |
| 6958 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 6960 | Seq Scan | 2.93 | 12.36 | 322.4% | operator |
| 6949 | Seq Scan | 166.58 | 159.81 | 4.1% | operator |
| 6950 | Hash | 35.50 | 46.85 | 32.0% | pattern |
| 6955 | Seq Scan | 0.01 | 11.81 | 131103.0% | operator |
| 6956 | Hash | 0.07 | 21.05 | 30860.6% | operator |
| 6957 | Seq Scan | 0.06 | 22.93 | 35733.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 6957 (Seq Scan) - LEAF
...
### Step 2: Node 6955 (Seq Scan) - LEAF
...
### Step 4: Node 6949 (Seq Scan) - LEAF
...
### Step 5: Node 6950 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 6941, 6942, 6943, 6945, 6946, 6951, 6952, 6953, 6954
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 6958 (Index Scan) - LEAF
...
### Step 8: Node 6960 (Seq Scan) - LEAF
...
### Step 11: Node 6945 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 6941, 6942, 6943, 6946, 6950, 6951, 6952, 6953, 6954
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 6941 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 6942, 6943, 6945, 6946, 6950, 6951, 6952, 6953, 6954
- **Input Features:**
