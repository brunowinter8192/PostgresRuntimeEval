...
**Test Query:** Q8_79_seed_639914418
**Timestamp:** 2026-01-18 18:49:35
...
```
Node 16099 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16100 (Gather Merge) [consumed]
    Node 16101 (Sort)
      Node 16102 (Hash Join)
        Node 16103 (Nested Loop)
          Node 16104 (Hash Join)
            Node 16105 (Nested Loop)
              Node 16106 (Hash Join)
                Node 16107 (Seq Scan) - LEAF
                Node 16108 (Hash) [PATTERN: 444761fb]
                  Node 16109 (Hash Join) [consumed]
                    Node 16110 (Seq Scan) [consumed] - LEAF
                    Node 16111 (Hash) [consumed]
                      Node 16112 (Hash Join) [consumed]
                        Node 16113 (Seq Scan) - LEAF
                        Node 16114 (Hash)
                          Node 16115 (Seq Scan) - LEAF
              Node 16116 (Index Scan) - LEAF
            Node 16117 (Hash)
              Node 16118 (Seq Scan) - LEAF
          Node 16119 (Index Scan) - LEAF
        Node 16120 (Hash)
          Node 16121 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16108 | 16099, 16100, 16109, 16110, 16111, 16112 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16099 | 16100, 16108, 16109, 16110, 16111, 16112 |
...
|------|------|--------|-----------|-----|--------|
| 16099 | Aggregate | 1201.52 | 1128.88 | 6.0% | pattern |
| 16101 | Sort | 1193.27 | 1046.21 | 12.3% | operator |
| 16102 | Hash Join | 1193.04 | 785.61 | 34.2% | operator |
| 16103 | Nested Loop | 1192.71 | 1198.29 | 0.5% | operator |
| 16120 | Hash | 0.02 | 13.72 | 76144.9% | operator |
| 16104 | Hash Join | 1188.88 | 785.60 | 33.9% | operator |
| 16119 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 16121 | Seq Scan | 0.01 | 6.51 | 46414.0% | operator |
| 16105 | Nested Loop | 1142.78 | 1198.29 | 4.9% | operator |
| 16117 | Hash | 41.83 | 19.97 | 52.3% | operator |
| 16106 | Hash Join | 207.09 | 251.67 | 21.5% | operator |
| 16116 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 16118 | Seq Scan | 41.64 | 48.87 | 17.3% | operator |
| 16107 | Seq Scan | 156.31 | 159.67 | 2.1% | operator |
| 16108 | Hash | 33.49 | 43.51 | 29.9% | pattern |
| 16113 | Seq Scan | 0.06 | 12.22 | 19304.7% | operator |
| 16114 | Hash | 0.13 | 18.14 | 13750.5% | operator |
| 16115 | Seq Scan | 0.13 | 23.36 | 18441.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16115 (Seq Scan) - LEAF
...
### Step 2: Node 16113 (Seq Scan) - LEAF
...
### Step 4: Node 16107 (Seq Scan) - LEAF
...
### Step 5: Node 16108 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16099, 16100, 16109, 16110, 16111, 16112
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16116 (Index Scan) - LEAF
...
### Step 8: Node 16118 (Seq Scan) - LEAF
...
### Step 12: Node 16119 (Index Scan) - LEAF
...
### Step 13: Node 16121 (Seq Scan) - LEAF
...
### Step 14: Node 16103 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70515.9500
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 16120 (Hash)
...
- **Input Features:**
  ...
  - total_cost=70522.0000
- **Output:** st=113.62, rt=785.61

### Step 17: Node 16101 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70560.9600
  - total_cost=70562.9800
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 16099 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16100, 16108, 16109, 16110, 16111, 16112
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2502
  ...
  - Aggregate_sel=0.9616
  - Aggregate_startup_cost=71561.0000
  - Aggregate_total_cost=71955.1700
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2502
  - GatherMerge_Outer_nt1=807
  ...
  - GatherMerge_Outer_startup_cost=71561.0000
  - GatherMerge_Outer_total_cost=71857.0100
- **Output:** st=1125.93, rt=1128.88
