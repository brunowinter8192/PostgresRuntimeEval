...
**Test Query:** Q5_28_seed_221508837
...
```
Node 8301 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8302 (Aggregate) [consumed]
    Node 8303 (Gather Merge) [consumed]
      Node 8304 (Aggregate)
        Node 8305 (Sort) [PATTERN: 3e2d5a00]
          Node 8306 (Hash Join) [consumed]
            Node 8307 (Nested Loop)
              Node 8308 (Hash Join)
                Node 8309 (Seq Scan) - LEAF
                Node 8310 (Hash) [PATTERN: 444761fb]
                  Node 8311 (Hash Join) [consumed]
                    Node 8312 (Seq Scan) [consumed] - LEAF
                    Node 8313 (Hash) [consumed]
                      Node 8314 (Hash Join) [consumed]
                        Node 8315 (Seq Scan) - LEAF
                        Node 8316 (Hash)
                          Node 8317 (Seq Scan) - LEAF
              Node 8318 (Index Scan) - LEAF
            Node 8319 (Hash)
              Node 8320 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8310 | 8301, 8302, 8303, 8305, 8306, 8311, 8312, 8313, 8314 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8301 | 8302, 8303, 8305, 8306, 8310, 8311, 8312, 8313, 8314 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8305 | 8301, 8302, 8303, 8306, 8310, 8311, 8312, 8313, 8314 |
...
|------|------|--------|-----------|-----|--------|
| 8301 | Sort | 1124.63 | 2934.42 | 160.9% | pattern |
| 8304 | Aggregate | 1119.70 | 1128.78 | 0.8% | operator |
| 8305 | Sort | 1119.45 | 1835.91 | 64.0% | pattern |
| 8307 | Nested Loop | 1107.26 | 1188.39 | 7.3% | operator |
| 8319 | Hash | 3.43 | 18.67 | 444.8% | operator |
| 8308 | Hash Join | 223.96 | 253.07 | 13.0% | operator |
| 8318 | Index Scan | 0.08 | -0.22 | 389.5% | operator |
| 8320 | Seq Scan | 2.79 | 12.36 | 342.2% | operator |
| 8309 | Seq Scan | 172.15 | 159.81 | 7.2% | operator |
| 8310 | Hash | 38.29 | 46.85 | 22.3% | pattern |
| 8315 | Seq Scan | 0.01 | 11.81 | 107247.9% | operator |
| 8316 | Hash | 0.09 | 21.05 | 24380.5% | operator |
| 8317 | Seq Scan | 0.08 | 22.93 | 27867.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8317 (Seq Scan) - LEAF
...
### Step 2: Node 8315 (Seq Scan) - LEAF
...
### Step 4: Node 8309 (Seq Scan) - LEAF
...
### Step 5: Node 8310 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8301, 8302, 8303, 8305, 8306, 8311, 8312, 8313, 8314
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8318 (Index Scan) - LEAF
...
### Step 8: Node 8320 (Seq Scan) - LEAF
...
### Step 11: Node 8305 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8301, 8302, 8303, 8306, 8310, 8311, 8312, 8313, 8314
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8301 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8302, 8303, 8305, 8306, 8310, 8311, 8312, 8313, 8314
- **Input Features:**
