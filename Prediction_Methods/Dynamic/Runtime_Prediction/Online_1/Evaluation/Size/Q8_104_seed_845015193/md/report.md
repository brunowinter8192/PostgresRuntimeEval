...
**Test Query:** Q8_104_seed_845015193
...
```
Node 13293 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13294 (Gather Merge) [consumed]
    Node 13295 (Sort)
      Node 13296 (Hash Join)
        Node 13297 (Nested Loop)
          Node 13298 (Hash Join)
            Node 13299 (Nested Loop)
              Node 13300 (Hash Join)
                Node 13301 (Seq Scan) - LEAF
                Node 13302 (Hash) [PATTERN: 444761fb]
                  Node 13303 (Hash Join) [consumed]
                    Node 13304 (Seq Scan) [consumed] - LEAF
                    Node 13305 (Hash) [consumed]
                      Node 13306 (Hash Join) [consumed]
                        Node 13307 (Seq Scan) - LEAF
                        Node 13308 (Hash)
                          Node 13309 (Seq Scan) - LEAF
              Node 13310 (Index Scan) - LEAF
            Node 13311 (Hash)
              Node 13312 (Seq Scan) - LEAF
          Node 13313 (Index Scan) - LEAF
        Node 13314 (Hash)
          Node 13315 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13302 | 13293, 13294, 13303, 13304, 13305, 13306 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13293 | 13294, 13302, 13303, 13304, 13305, 13306 |
...
|------|------|--------|-----------|-----|--------|
| 13293 | Aggregate | 1206.31 | 1133.68 | 6.0% | pattern |
| 13295 | Sort | 1198.79 | 1046.21 | 12.7% | operator |
| 13296 | Hash Join | 1198.55 | 785.61 | 34.5% | operator |
| 13297 | Nested Loop | 1198.25 | 1198.29 | 0.0% | operator |
| 13314 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 13298 | Hash Join | 1193.70 | 785.60 | 34.2% | operator |
| 13313 | Index Scan | 0.01 | 12.42 | 177378.3% | operator |
| 13315 | Seq Scan | 0.01 | 6.51 | 54166.4% | operator |
| 13299 | Nested Loop | 1148.73 | 1198.29 | 4.3% | operator |
| 13311 | Hash | 39.02 | 19.97 | 48.8% | operator |
| 13300 | Hash Join | 210.31 | 251.67 | 19.7% | operator |
| 13310 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 13312 | Seq Scan | 38.92 | 48.88 | 25.6% | operator |
| 13301 | Seq Scan | 157.00 | 159.67 | 1.7% | operator |
| 13302 | Hash | 34.03 | 43.51 | 27.9% | pattern |
| 13307 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 13308 | Hash | 0.06 | 18.14 | 30652.8% | operator |
| 13309 | Seq Scan | 0.06 | 23.36 | 42376.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13309 (Seq Scan) - LEAF
...
### Step 2: Node 13307 (Seq Scan) - LEAF
...
### Step 4: Node 13301 (Seq Scan) - LEAF
...
### Step 5: Node 13302 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13293, 13294, 13303, 13304, 13305, 13306
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13310 (Index Scan) - LEAF
...
### Step 8: Node 13312 (Seq Scan) - LEAF
...
### Step 12: Node 13313 (Index Scan) - LEAF
...
### Step 13: Node 13315 (Seq Scan) - LEAF
...
### Step 14: Node 13297 (Nested Loop)
...
### Step 18: Node 13293 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13294, 13302, 13303, 13304, 13305, 13306
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2343
  - Aggregate_nt1=2343
