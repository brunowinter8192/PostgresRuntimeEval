...
**Test Query:** Q7_36_seed_287141085
...
```
Node 11923 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11924 (Gather Merge) [consumed]
    Node 11925 (Sort)
      Node 11926 (Hash Join)
        Node 11927 (Nested Loop)
          Node 11928 (Hash Join) [PATTERN: 895c6e8c]
            Node 11929 (Seq Scan) [consumed] - LEAF
            Node 11930 (Hash) [consumed]
              Node 11931 (Hash Join) [PATTERN: f4cb205a]
                Node 11932 (Seq Scan) [consumed] - LEAF
                Node 11933 (Hash) [consumed]
                  Node 11934 (Seq Scan) [consumed] - LEAF
          Node 11935 (Index Scan) - LEAF
        Node 11936 (Hash)
          Node 11937 (Hash Join) [PATTERN: f4cb205a]
            Node 11938 (Seq Scan) [consumed] - LEAF
            Node 11939 (Hash) [consumed]
              Node 11940 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11931 | 11923, 11924, 11928, 11929, 11930, 11932, 11933, 11934, 11937, 11938, 11939, 11940 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11937 | 11923, 11924, 11928, 11929, 11930, 11931, 11932, 11933, 11934, 11938, 11939, 11940 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11923 | 11924, 11928, 11929, 11930, 11931, 11932, 11933, 11934, 11937, 11938, 11939, 11940 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11928 | 11923, 11924, 11929, 11930, 11931, 11932, 11933, 11934, 11937, 11938, 11939, 11940 |
...
|------|------|--------|-----------|-----|--------|
| 11923 | Aggregate | 1207.97 | 1163.13 | 3.7% | pattern |
| 11925 | Sort | 1200.41 | 1109.16 | 7.6% | operator |
| 11926 | Hash Join | 1199.76 | 843.11 | 29.7% | operator |
| 11927 | Nested Loop | 1192.35 | 1215.99 | 2.0% | operator |
| 11936 | Hash | 3.33 | 27.74 | 732.1% | operator |
| 11928 | Hash Join | 211.75 | 304.89 | 44.0% | pattern |
| 11935 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11937 | Hash Join | 3.29 | 429.36 | 12970.4% | pattern |
| 11931 | Hash Join | 33.32 | 429.38 | 1188.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11931 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11923, 11924, 11928, 11929, 11930, 11932, 11933, 11934, 11937, 11938, 11939, 11940
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11928 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11923, 11924, 11929, 11930, 11931, 11932, 11933, 11934, 11937, 11938, 11939, 11940
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11935 (Index Scan) - LEAF
...
### Step 4: Node 11937 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11923, 11924, 11928, 11929, 11930, 11931, 11932, 11933, 11934, 11938, 11939, 11940
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11923 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11924, 11928, 11929, 11930, 11931, 11932, 11933, 11934, 11937, 11938, 11939, 11940
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
