...
**Test Query:** Q8_19_seed_147672558
...
```
Node 14581 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14582 (Gather Merge) [consumed]
    Node 14583 (Sort)
      Node 14584 (Hash Join)
        Node 14585 (Nested Loop)
          Node 14586 (Hash Join)
            Node 14587 (Nested Loop)
              Node 14588 (Hash Join)
                Node 14589 (Seq Scan) - LEAF
                Node 14590 (Hash) [PATTERN: 444761fb]
                  Node 14591 (Hash Join) [consumed]
                    Node 14592 (Seq Scan) [consumed] - LEAF
                    Node 14593 (Hash) [consumed]
                      Node 14594 (Hash Join) [consumed]
                        Node 14595 (Seq Scan) - LEAF
                        Node 14596 (Hash)
                          Node 14597 (Seq Scan) - LEAF
              Node 14598 (Index Scan) - LEAF
            Node 14599 (Hash)
              Node 14600 (Seq Scan) - LEAF
          Node 14601 (Index Scan) - LEAF
        Node 14602 (Hash)
          Node 14603 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14590 | 14581, 14582, 14591, 14592, 14593, 14594 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14581 | 14582, 14590, 14591, 14592, 14593, 14594 |
...
|------|------|--------|-----------|-----|--------|
| 14581 | Aggregate | 1224.17 | 1133.68 | 7.4% | pattern |
| 14583 | Sort | 1218.70 | 1046.21 | 14.2% | operator |
| 14584 | Hash Join | 1218.42 | 785.61 | 35.5% | operator |
| 14585 | Nested Loop | 1218.09 | 1198.29 | 1.6% | operator |
| 14602 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 14586 | Hash Join | 1214.36 | 785.60 | 35.3% | operator |
| 14601 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14603 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 14587 | Nested Loop | 1172.62 | 1198.29 | 2.2% | operator |
| 14599 | Hash | 37.32 | 19.97 | 46.5% | operator |
| 14588 | Hash Join | 208.74 | 251.67 | 20.6% | operator |
| 14598 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14600 | Seq Scan | 37.21 | 48.89 | 31.4% | operator |
| 14589 | Seq Scan | 155.61 | 159.67 | 2.6% | operator |
| 14590 | Hash | 34.96 | 43.51 | 24.5% | pattern |
| 14595 | Seq Scan | 0.01 | 12.22 | 152711.8% | operator |
| 14596 | Hash | 2.49 | 18.14 | 629.9% | operator |
| 14597 | Seq Scan | 2.48 | 23.36 | 841.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14597 (Seq Scan) - LEAF
...
### Step 2: Node 14595 (Seq Scan) - LEAF
...
### Step 4: Node 14589 (Seq Scan) - LEAF
...
### Step 5: Node 14590 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14581, 14582, 14591, 14592, 14593, 14594
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14598 (Index Scan) - LEAF
...
### Step 8: Node 14600 (Seq Scan) - LEAF
...
### Step 12: Node 14601 (Index Scan) - LEAF
...
### Step 13: Node 14603 (Seq Scan) - LEAF
...
### Step 14: Node 14585 (Nested Loop)
...
### Step 18: Node 14581 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14582, 14590, 14591, 14592, 14593, 14594
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2321
  - Aggregate_nt1=2321
