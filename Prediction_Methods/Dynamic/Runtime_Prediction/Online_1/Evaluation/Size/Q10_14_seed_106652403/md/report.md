...
**Test Query:** Q10_14_seed_106652403
...
```
Node 20072 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20073 (Sort) [consumed]
    Node 20074 (Aggregate) [consumed]
      Node 20075 (Gather) [consumed]
        Node 20076 (Hash Join)
          Node 20077 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20078 (Nested Loop) [consumed]
              Node 20079 (Seq Scan) - LEAF
              Node 20080 (Index Scan) - LEAF
            Node 20081 (Hash) [consumed]
              Node 20082 (Seq Scan) - LEAF
          Node 20083 (Hash)
            Node 20084 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20072 | 20073, 20074, 20075, 20077, 20078, 20081 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20077 | 20072, 20073, 20074, 20075, 20078, 20081 |
...
|------|------|--------|-----------|-----|--------|
| 20072 | Limit | 1157.54 | 1199.47 | 3.6% | pattern |
| 20076 | Hash Join | 1104.81 | 1095.50 | 0.8% | operator |
| 20077 | Hash Join | 1101.90 | 1120.19 | 1.7% | pattern |
| 20083 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 20084 | Seq Scan | 0.01 | 6.98 | 58055.9% | operator |
| 20079 | Seq Scan | 163.61 | 159.74 | 2.4% | operator |
| 20080 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20082 | Seq Scan | 25.17 | 26.64 | 5.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20079 (Seq Scan) - LEAF
...
### Step 2: Node 20080 (Index Scan) - LEAF
...
### Step 3: Node 20082 (Seq Scan) - LEAF
...
### Step 4: Node 20084 (Seq Scan) - LEAF
...
### Step 5: Node 20077 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20072, 20073, 20074, 20075, 20078, 20081
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17978
  - HashJoin_nt1=17978
...
### Step 8: Node 20072 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20073, 20074, 20075, 20077, 20078, 20081
- **Input Features:**
