...
**Test Query:** Q7_18_seed_139468527
...
```
Node 11563 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11564 (Gather Merge) [consumed]
    Node 11565 (Sort)
      Node 11566 (Hash Join)
        Node 11567 (Nested Loop)
          Node 11568 (Hash Join) [PATTERN: 895c6e8c]
            Node 11569 (Seq Scan) [consumed] - LEAF
            Node 11570 (Hash) [consumed]
              Node 11571 (Hash Join) [PATTERN: f4cb205a]
                Node 11572 (Seq Scan) [consumed] - LEAF
                Node 11573 (Hash) [consumed]
                  Node 11574 (Seq Scan) [consumed] - LEAF
          Node 11575 (Index Scan) - LEAF
        Node 11576 (Hash)
          Node 11577 (Hash Join) [PATTERN: f4cb205a]
            Node 11578 (Seq Scan) [consumed] - LEAF
            Node 11579 (Hash) [consumed]
              Node 11580 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11571 | 11563, 11564, 11568, 11569, 11570, 11572, 11573, 11574, 11577, 11578, 11579, 11580 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11577 | 11563, 11564, 11568, 11569, 11570, 11571, 11572, 11573, 11574, 11578, 11579, 11580 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11563 | 11564, 11568, 11569, 11570, 11571, 11572, 11573, 11574, 11577, 11578, 11579, 11580 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11568 | 11563, 11564, 11569, 11570, 11571, 11572, 11573, 11574, 11577, 11578, 11579, 11580 |
...
|------|------|--------|-----------|-----|--------|
| 11563 | Aggregate | 1238.92 | 1163.13 | 6.1% | pattern |
| 11565 | Sort | 1232.30 | 1109.16 | 10.0% | operator |
| 11566 | Hash Join | 1231.61 | 843.11 | 31.5% | operator |
| 11567 | Nested Loop | 1223.83 | 1215.99 | 0.6% | operator |
| 11576 | Hash | 3.71 | 27.74 | 647.8% | operator |
| 11568 | Hash Join | 220.48 | 304.89 | 38.3% | pattern |
| 11575 | Index Scan | 0.03 | 0.96 | 2817.7% | operator |
| 11577 | Hash Join | 3.66 | 429.36 | 11628.0% | pattern |
| 11571 | Hash Join | 41.34 | 429.38 | 938.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11571 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11563, 11564, 11568, 11569, 11570, 11572, 11573, 11574, 11577, 11578, 11579, 11580
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11568 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11563, 11564, 11569, 11570, 11571, 11572, 11573, 11574, 11577, 11578, 11579, 11580
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11575 (Index Scan) - LEAF
...
### Step 4: Node 11577 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11563, 11564, 11568, 11569, 11570, 11571, 11572, 11573, 11574, 11578, 11579, 11580
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11563 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11564, 11568, 11569, 11570, 11571, 11572, 11573, 11574, 11577, 11578, 11579, 11580
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
