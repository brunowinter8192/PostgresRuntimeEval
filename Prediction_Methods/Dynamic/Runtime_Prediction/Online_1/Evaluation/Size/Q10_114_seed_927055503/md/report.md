...
**Test Query:** Q10_114_seed_927055503
...
```
Node 19565 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 19566 (Sort) [consumed]
    Node 19567 (Aggregate) [consumed]
      Node 19568 (Gather) [consumed]
        Node 19569 (Hash Join)
          Node 19570 (Hash Join) [PATTERN: 2e0f44ef]
            Node 19571 (Nested Loop) [consumed]
              Node 19572 (Seq Scan) - LEAF
              Node 19573 (Index Scan) - LEAF
            Node 19574 (Hash) [consumed]
              Node 19575 (Seq Scan) - LEAF
          Node 19576 (Hash)
            Node 19577 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 19565 | 19566, 19567, 19568, 19570, 19571, 19574 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19570 | 19565, 19566, 19567, 19568, 19571, 19574 |
...
|------|------|--------|-----------|-----|--------|
| 19565 | Limit | 1154.32 | 1199.47 | 3.9% | pattern |
| 19569 | Hash Join | 1104.14 | 1095.59 | 0.8% | operator |
| 19570 | Hash Join | 1101.23 | 1120.19 | 1.7% | pattern |
| 19576 | Hash | 0.01 | 15.71 | 112119.8% | operator |
| 19577 | Seq Scan | 0.01 | 6.98 | 69687.0% | operator |
| 19572 | Seq Scan | 158.84 | 159.74 | 0.6% | operator |
| 19573 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 19575 | Seq Scan | 27.33 | 26.64 | 2.5% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19572 (Seq Scan) - LEAF
...
### Step 2: Node 19573 (Index Scan) - LEAF
...
### Step 3: Node 19575 (Seq Scan) - LEAF
...
### Step 4: Node 19577 (Seq Scan) - LEAF
...
### Step 5: Node 19570 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19565, 19566, 19567, 19568, 19571, 19574
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18337
  - HashJoin_nt1=18337
...
### Step 8: Node 19565 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19566, 19567, 19568, 19570, 19571, 19574
- **Input Features:**
