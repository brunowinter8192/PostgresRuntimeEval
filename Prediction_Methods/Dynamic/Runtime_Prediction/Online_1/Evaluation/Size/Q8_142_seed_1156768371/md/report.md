...
**Test Query:** Q8_142_seed_1156768371
...
```
Node 14259 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14260 (Gather Merge) [consumed]
    Node 14261 (Sort)
      Node 14262 (Hash Join)
        Node 14263 (Nested Loop)
          Node 14264 (Hash Join)
            Node 14265 (Nested Loop)
              Node 14266 (Hash Join)
                Node 14267 (Seq Scan) - LEAF
                Node 14268 (Hash) [PATTERN: 444761fb]
                  Node 14269 (Hash Join) [consumed]
                    Node 14270 (Seq Scan) [consumed] - LEAF
                    Node 14271 (Hash) [consumed]
                      Node 14272 (Hash Join) [consumed]
                        Node 14273 (Seq Scan) - LEAF
                        Node 14274 (Hash)
                          Node 14275 (Seq Scan) - LEAF
              Node 14276 (Index Scan) - LEAF
            Node 14277 (Hash)
              Node 14278 (Seq Scan) - LEAF
          Node 14279 (Index Scan) - LEAF
        Node 14280 (Hash)
          Node 14281 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14268 | 14259, 14260, 14269, 14270, 14271, 14272 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14259 | 14260, 14268, 14269, 14270, 14271, 14272 |
...
|------|------|--------|-----------|-----|--------|
| 14259 | Aggregate | 1228.32 | 1133.68 | 7.7% | pattern |
| 14261 | Sort | 1223.51 | 1046.21 | 14.5% | operator |
| 14262 | Hash Join | 1223.27 | 785.61 | 35.8% | operator |
| 14263 | Nested Loop | 1222.96 | 1198.29 | 2.0% | operator |
| 14280 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 14264 | Hash Join | 1219.11 | 785.60 | 35.6% | operator |
| 14279 | Index Scan | 0.01 | 12.42 | 177378.3% | operator |
| 14281 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 14265 | Nested Loop | 1174.37 | 1198.29 | 2.0% | operator |
| 14277 | Hash | 40.43 | 19.97 | 50.6% | operator |
| 14266 | Hash Join | 216.47 | 251.67 | 16.3% | operator |
| 14276 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 14278 | Seq Scan | 40.36 | 48.89 | 21.1% | operator |
| 14267 | Seq Scan | 160.40 | 159.67 | 0.5% | operator |
| 14268 | Hash | 37.68 | 43.51 | 15.5% | pattern |
| 14273 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 14274 | Hash | 0.24 | 18.14 | 7397.6% | operator |
| 14275 | Seq Scan | 0.24 | 23.36 | 9675.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 14275 (Seq Scan) - LEAF
...
### Step 2: Node 14273 (Seq Scan) - LEAF
...
### Step 4: Node 14267 (Seq Scan) - LEAF
...
### Step 5: Node 14268 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14259, 14260, 14269, 14270, 14271, 14272
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14276 (Index Scan) - LEAF
...
### Step 8: Node 14278 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9727
  - startup_cost=9919.7100
  - total_cost=70271.1900
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14279 (Index Scan) - LEAF
...
### Step 13: Node 14281 (Seq Scan) - LEAF
...
### Step 14: Node 14263 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.0000
  - total_cost=70494.7700
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14280 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.5600
  - total_cost=70500.4600
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14261 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70535.6700
  - total_cost=70537.5100
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14259 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14260, 14268, 14269, 14270, 14271, 14272
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2290
  - Aggregate_nt1=2290
  ...
  - Aggregate_startup_cost=71535.7100
  - Aggregate_total_cost=71898.2300
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2290
  - GatherMerge_Outer_nt1=739
  ...
  - GatherMerge_Outer_startup_cost=71535.7100
  - GatherMerge_Outer_total_cost=71806.6300
- **Output:** st=1130.68, rt=1133.68
