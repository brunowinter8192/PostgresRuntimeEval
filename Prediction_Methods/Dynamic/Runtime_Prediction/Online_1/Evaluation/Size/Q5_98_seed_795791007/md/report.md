...
**Test Query:** Q5_98_seed_795791007
...
```
Node 9841 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 9842 (Aggregate) [consumed]
    Node 9843 (Gather Merge) [consumed]
      Node 9844 (Aggregate)
        Node 9845 (Sort) [PATTERN: 3e2d5a00]
          Node 9846 (Hash Join) [consumed]
            Node 9847 (Nested Loop)
              Node 9848 (Hash Join)
                Node 9849 (Seq Scan) - LEAF
                Node 9850 (Hash) [PATTERN: 444761fb]
                  Node 9851 (Hash Join) [consumed]
                    Node 9852 (Seq Scan) [consumed] - LEAF
                    Node 9853 (Hash) [consumed]
                      Node 9854 (Hash Join) [consumed]
                        Node 9855 (Seq Scan) - LEAF
                        Node 9856 (Hash)
                          Node 9857 (Seq Scan) - LEAF
              Node 9858 (Index Scan) - LEAF
            Node 9859 (Hash)
              Node 9860 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 9850 | 9841, 9842, 9843, 9845, 9846, 9851, 9852, 9853, 9854 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 9841 | 9842, 9843, 9845, 9846, 9850, 9851, 9852, 9853, 9854 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 9845 | 9841, 9842, 9843, 9846, 9850, 9851, 9852, 9853, 9854 |
...
|------|------|--------|-----------|-----|--------|
| 9841 | Sort | 1065.06 | 2934.42 | 175.5% | pattern |
| 9844 | Aggregate | 1059.54 | 1128.79 | 6.5% | operator |
| 9845 | Sort | 1059.28 | 1835.90 | 73.3% | pattern |
| 9847 | Nested Loop | 1047.16 | 1188.37 | 13.5% | operator |
| 9859 | Hash | 3.47 | 18.67 | 438.5% | operator |
| 9848 | Hash Join | 215.61 | 253.07 | 17.4% | operator |
| 9858 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 9860 | Seq Scan | 2.97 | 12.36 | 315.9% | operator |
| 9849 | Seq Scan | 167.60 | 159.81 | 4.6% | operator |
| 9850 | Hash | 34.53 | 46.85 | 35.7% | pattern |
| 9855 | Seq Scan | 0.01 | 11.81 | 84244.8% | operator |
| 9856 | Hash | 0.35 | 21.05 | 5830.5% | operator |
| 9857 | Seq Scan | 0.35 | 22.93 | 6452.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9857 (Seq Scan) - LEAF
...
### Step 2: Node 9855 (Seq Scan) - LEAF
...
### Step 4: Node 9849 (Seq Scan) - LEAF
...
### Step 5: Node 9850 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 9841, 9842, 9843, 9845, 9846, 9851, 9852, 9853, 9854
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 9858 (Index Scan) - LEAF
...
### Step 8: Node 9860 (Seq Scan) - LEAF
...
### Step 11: Node 9845 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 9841, 9842, 9843, 9846, 9850, 9851, 9852, 9853, 9854
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 9841 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 9842, 9843, 9845, 9846, 9850, 9851, 9852, 9853, 9854
- **Input Features:**
