...
**Test Query:** Q10_49_seed_393793488
...
```
Node 20579 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20580 (Sort) [consumed]
    Node 20581 (Aggregate) [consumed]
      Node 20582 (Gather) [consumed]
        Node 20583 (Hash Join)
          Node 20584 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20585 (Nested Loop) [consumed]
              Node 20586 (Seq Scan) - LEAF
              Node 20587 (Index Scan) - LEAF
            Node 20588 (Hash) [consumed]
              Node 20589 (Seq Scan) - LEAF
          Node 20590 (Hash)
            Node 20591 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20579 | 20580, 20581, 20582, 20584, 20585, 20588 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20584 | 20579, 20580, 20581, 20582, 20585, 20588 |
...
|------|------|--------|-----------|-----|--------|
| 20579 | Limit | 1163.92 | 1199.47 | 3.1% | pattern |
| 20583 | Hash Join | 1104.66 | 1095.51 | 0.8% | operator |
| 20584 | Hash Join | 1101.72 | 1120.19 | 1.7% | pattern |
| 20590 | Hash | 0.02 | 15.71 | 98092.3% | operator |
| 20591 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20586 | Seq Scan | 161.10 | 159.74 | 0.8% | operator |
| 20587 | Index Scan | 0.06 | -0.52 | 935.9% | operator |
| 20589 | Seq Scan | 25.79 | 26.64 | 3.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20586 (Seq Scan) - LEAF
...
### Step 2: Node 20587 (Index Scan) - LEAF
...
### Step 3: Node 20589 (Seq Scan) - LEAF
...
### Step 4: Node 20591 (Seq Scan) - LEAF
...
### Step 5: Node 20584 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20579, 20580, 20581, 20582, 20585, 20588
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18015
  - HashJoin_nt1=18015
...
### Step 8: Node 20579 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20580, 20581, 20582, 20584, 20585, 20588
- **Input Features:**
