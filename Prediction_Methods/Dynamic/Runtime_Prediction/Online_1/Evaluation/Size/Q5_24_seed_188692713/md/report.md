...
**Test Query:** Q5_24_seed_188692713
...
```
Node 8221 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8222 (Aggregate) [consumed]
    Node 8223 (Gather Merge) [consumed]
      Node 8224 (Aggregate)
        Node 8225 (Sort) [PATTERN: 3e2d5a00]
          Node 8226 (Hash Join) [consumed]
            Node 8227 (Nested Loop)
              Node 8228 (Hash Join)
                Node 8229 (Seq Scan) - LEAF
                Node 8230 (Hash) [PATTERN: 444761fb]
                  Node 8231 (Hash Join) [consumed]
                    Node 8232 (Seq Scan) [consumed] - LEAF
                    Node 8233 (Hash) [consumed]
                      Node 8234 (Hash Join) [consumed]
                        Node 8235 (Seq Scan) - LEAF
                        Node 8236 (Hash)
                          Node 8237 (Seq Scan) - LEAF
              Node 8238 (Index Scan) - LEAF
            Node 8239 (Hash)
              Node 8240 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8230 | 8221, 8222, 8223, 8225, 8226, 8231, 8232, 8233, 8234 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8221 | 8222, 8223, 8225, 8226, 8230, 8231, 8232, 8233, 8234 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8225 | 8221, 8222, 8223, 8226, 8230, 8231, 8232, 8233, 8234 |
...
|------|------|--------|-----------|-----|--------|
| 8221 | Sort | 1078.95 | 2934.42 | 172.0% | pattern |
| 8224 | Aggregate | 1074.02 | 1128.78 | 5.1% | operator |
| 8225 | Sort | 1073.76 | 1835.91 | 71.0% | pattern |
| 8227 | Nested Loop | 1061.99 | 1188.39 | 11.9% | operator |
| 8239 | Hash | 2.95 | 18.67 | 532.7% | operator |
| 8228 | Hash Join | 220.86 | 253.07 | 14.6% | operator |
| 8238 | Index Scan | 0.07 | -0.22 | 397.3% | operator |
| 8240 | Seq Scan | 2.44 | 12.36 | 406.5% | operator |
| 8229 | Seq Scan | 169.92 | 159.81 | 5.9% | operator |
| 8230 | Hash | 37.60 | 46.85 | 24.6% | pattern |
| 8235 | Seq Scan | 0.01 | 11.81 | 117982.7% | operator |
| 8236 | Hash | 0.05 | 21.05 | 39623.1% | operator |
| 8237 | Seq Scan | 0.05 | 22.93 | 44867.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8237 (Seq Scan) - LEAF
...
### Step 2: Node 8235 (Seq Scan) - LEAF
...
### Step 4: Node 8229 (Seq Scan) - LEAF
...
### Step 5: Node 8230 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8221, 8222, 8223, 8225, 8226, 8231, 8232, 8233, 8234
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8238 (Index Scan) - LEAF
...
### Step 8: Node 8240 (Seq Scan) - LEAF
...
### Step 11: Node 8225 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8221, 8222, 8223, 8226, 8230, 8231, 8232, 8233, 8234
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8221 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8222, 8223, 8225, 8226, 8230, 8231, 8232, 8233, 8234
- **Input Features:**
