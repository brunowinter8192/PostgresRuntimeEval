...
**Test Query:** Q10_58_seed_467629767
...
```
Node 20709 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20710 (Sort) [consumed]
    Node 20711 (Aggregate) [consumed]
      Node 20712 (Gather) [consumed]
        Node 20713 (Hash Join)
          Node 20714 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20715 (Nested Loop) [consumed]
              Node 20716 (Seq Scan) - LEAF
              Node 20717 (Index Scan) - LEAF
            Node 20718 (Hash) [consumed]
              Node 20719 (Seq Scan) - LEAF
          Node 20720 (Hash)
            Node 20721 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20709 | 20710, 20711, 20712, 20714, 20715, 20718 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20714 | 20709, 20710, 20711, 20712, 20715, 20718 |
...
|------|------|--------|-----------|-----|--------|
| 20709 | Limit | 1168.68 | 1199.47 | 2.6% | pattern |
| 20713 | Hash Join | 1115.55 | 1095.49 | 1.8% | operator |
| 20714 | Hash Join | 1112.68 | 1120.19 | 0.7% | pattern |
| 20720 | Hash | 0.02 | 15.71 | 87182.1% | operator |
| 20721 | Seq Scan | 0.01 | 6.98 | 53582.3% | operator |
| 20716 | Seq Scan | 162.81 | 159.74 | 1.9% | operator |
| 20717 | Index Scan | 0.06 | -0.52 | 922.6% | operator |
| 20719 | Seq Scan | 28.76 | 26.64 | 7.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20716 (Seq Scan) - LEAF
...
### Step 2: Node 20717 (Index Scan) - LEAF
...
### Step 3: Node 20719 (Seq Scan) - LEAF
...
### Step 4: Node 20721 (Seq Scan) - LEAF
...
### Step 5: Node 20714 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20709, 20710, 20711, 20712, 20715, 20718
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17964
  - HashJoin_nt1=17964
...
### Step 8: Node 20709 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20710, 20711, 20712, 20714, 20715, 20718
- **Input Features:**
