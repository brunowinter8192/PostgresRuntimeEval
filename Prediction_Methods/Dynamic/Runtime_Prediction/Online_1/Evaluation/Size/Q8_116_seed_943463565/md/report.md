...
**Test Query:** Q8_116_seed_943463565
...
```
Node 13592 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13593 (Gather Merge) [consumed]
    Node 13594 (Sort)
      Node 13595 (Hash Join)
        Node 13596 (Nested Loop)
          Node 13597 (Hash Join)
            Node 13598 (Nested Loop)
              Node 13599 (Hash Join)
                Node 13600 (Seq Scan) - LEAF
                Node 13601 (Hash) [PATTERN: 444761fb]
                  Node 13602 (Hash Join) [consumed]
                    Node 13603 (Seq Scan) [consumed] - LEAF
                    Node 13604 (Hash) [consumed]
                      Node 13605 (Hash Join) [consumed]
                        Node 13606 (Seq Scan) - LEAF
                        Node 13607 (Hash)
                          Node 13608 (Seq Scan) - LEAF
              Node 13609 (Index Scan) - LEAF
            Node 13610 (Hash)
              Node 13611 (Seq Scan) - LEAF
          Node 13612 (Index Scan) - LEAF
        Node 13613 (Hash)
          Node 13614 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13601 | 13592, 13593, 13602, 13603, 13604, 13605 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13592 | 13593, 13601, 13602, 13603, 13604, 13605 |
...
|------|------|--------|-----------|-----|--------|
| 13592 | Aggregate | 1256.21 | 1129.23 | 10.1% | pattern |
| 13594 | Sort | 1251.10 | 1046.21 | 16.4% | operator |
| 13595 | Hash Join | 1250.85 | 785.61 | 37.2% | operator |
| 13596 | Nested Loop | 1250.53 | 1198.29 | 4.2% | operator |
| 13613 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 13597 | Hash Join | 1246.93 | 785.60 | 37.0% | operator |
| 13612 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13614 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13598 | Nested Loop | 1204.51 | 1198.29 | 0.5% | operator |
| 13610 | Hash | 37.84 | 19.97 | 47.2% | operator |
| 13599 | Hash Join | 211.42 | 251.67 | 19.0% | operator |
| 13609 | Index Scan | 0.04 | 2.88 | 6599.7% | operator |
| 13611 | Seq Scan | 37.76 | 48.87 | 29.4% | operator |
| 13600 | Seq Scan | 162.34 | 159.67 | 1.7% | operator |
| 13601 | Hash | 31.12 | 43.51 | 39.8% | pattern |
| 13606 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13607 | Hash | 0.20 | 18.14 | 9017.7% | operator |
| 13608 | Seq Scan | 0.20 | 23.36 | 11880.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13608 (Seq Scan) - LEAF
...
### Step 2: Node 13606 (Seq Scan) - LEAF
...
### Step 4: Node 13600 (Seq Scan) - LEAF
...
### Step 5: Node 13601 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13592, 13593, 13602, 13603, 13604, 13605
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13609 (Index Scan) - LEAF
...
### Step 8: Node 13611 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9689
  - startup_cost=9920.2900
  - total_cost=70271.7700
- **Output:** st=113.62, rt=785.60

### Step 12: Node 13612 (Index Scan) - LEAF
...
### Step 13: Node 13614 (Seq Scan) - LEAF
...
### Step 14: Node 13596 (Nested Loop)
...
- **Input Features:**
  ...
  - st1=113.6216
  - st2=0.1628
  - startup_cost=9920.5700
  - total_cost=70515.3200
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 13613 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9922.1400
  - total_cost=70521.3600
- **Output:** st=113.62, rt=785.61

### Step 17: Node 13594 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70560.2100
  - total_cost=70562.2200
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 13592 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13593, 13601, 13602, 13603, 13604, 13605
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2495
  ...
  - Aggregate_sel=0.9643
  - Aggregate_startup_cost=71560.2500
  - Aggregate_total_cost=71953.4500
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2495
  - GatherMerge_Outer_nt1=805
  ...
  - GatherMerge_Outer_startup_cost=71560.2500
  - GatherMerge_Outer_total_cost=71855.4300
- **Output:** st=1126.27, rt=1129.23
