...
**Test Query:** Q9_115_seed_935259534
...
```
Node 16941 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 16942 (Aggregate) [consumed]
    Node 16943 (Gather)
      Node 16944 (Aggregate)
        Node 16945 (Hash Join)
          Node 16946 (Seq Scan) - LEAF
          Node 16947 (Hash) [PATTERN: 444761fb]
            Node 16948 (Hash Join) [consumed]
              Node 16949 (Seq Scan) [consumed] - LEAF
              Node 16950 (Hash) [consumed]
                Node 16951 (Hash Join) [consumed]
                  Node 16952 (Hash Join)
                    Node 16953 (Nested Loop)
                      Node 16954 (Seq Scan) - LEAF
                      Node 16955 (Index Scan) - LEAF
                    Node 16956 (Hash)
                      Node 16957 (Seq Scan) - LEAF
                  Node 16958 (Hash)
                    Node 16959 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16947 | 16941, 16942, 16948, 16949, 16950, 16951 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 16941 | 16942, 16947, 16948, 16949, 16950, 16951 |
...
|------|------|--------|-----------|-----|--------|
| 16941 | Sort | 1269.42 | 1063.03 | 16.3% | pattern |
| 16943 | Gather | 1268.16 | 962.11 | 24.1% | operator |
| 16944 | Aggregate | 1249.78 | 931.07 | 25.5% | operator |
| 16945 | Hash Join | 1221.10 | 568.57 | 53.4% | operator |
| 16946 | Seq Scan | 131.44 | 193.40 | 47.1% | operator |
| 16947 | Hash | 1025.05 | 35.69 | 96.5% | pattern |
| 16952 | Hash Join | 193.50 | 837.69 | 332.9% | operator |
| 16958 | Hash | 15.55 | 14.95 | 3.9% | operator |
| 16953 | Nested Loop | 187.44 | 1251.77 | 567.8% | operator |
| 16956 | Hash | 3.54 | 15.94 | 350.0% | operator |
| 16959 | Seq Scan | 15.54 | 2.94 | 81.1% | operator |
| 16954 | Seq Scan | 26.61 | 43.83 | 64.7% | operator |
| 16955 | Index Scan | 0.06 | -1.39 | 2453.4% | operator |
| 16957 | Seq Scan | 3.11 | 9.45 | 203.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16954 (Seq Scan) - LEAF
...
### Step 2: Node 16955 (Index Scan) - LEAF
...
### Step 3: Node 16957 (Seq Scan) - LEAF
...
### Step 6: Node 16959 (Seq Scan) - LEAF
...
### Step 9: Node 16946 (Seq Scan) - LEAF
...
### Step 10: Node 16947 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16941, 16942, 16948, 16949, 16950, 16951
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 14: Node 16941 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 16942, 16947, 16948, 16949, 16950, 16951
- **Input Features:**
