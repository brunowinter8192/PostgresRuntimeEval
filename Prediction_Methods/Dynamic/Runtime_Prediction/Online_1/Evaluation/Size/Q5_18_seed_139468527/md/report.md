...
**Test Query:** Q5_18_seed_139468527
...
```
Node 8081 (Sort) [PATTERN: ddb1e0ca] - ROOT
  Node 8082 (Aggregate) [consumed]
    Node 8083 (Gather Merge) [consumed]
      Node 8084 (Aggregate)
        Node 8085 (Sort) [PATTERN: 3e2d5a00]
          Node 8086 (Hash Join) [consumed]
            Node 8087 (Nested Loop)
              Node 8088 (Hash Join)
                Node 8089 (Seq Scan) - LEAF
                Node 8090 (Hash) [PATTERN: 444761fb]
                  Node 8091 (Hash Join) [consumed]
                    Node 8092 (Seq Scan) [consumed] - LEAF
                    Node 8093 (Hash) [consumed]
                      Node 8094 (Hash Join) [consumed]
                        Node 8095 (Seq Scan) - LEAF
                        Node 8096 (Hash)
                          Node 8097 (Seq Scan) - LEAF
              Node 8098 (Index Scan) - LEAF
            Node 8099 (Hash)
              Node 8100 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 8090 | 8081, 8082, 8083, 8085, 8086, 8091, 8092, 8093, 8094 |
| Sort -> Aggregate -> Gather Me | ddb1e0ca | 8081 | 8082, 8083, 8085, 8086, 8090, 8091, 8092, 8093, 8094 |
| Sort -> Hash Join (Outer) | 3e2d5a00 | 8085 | 8081, 8082, 8083, 8086, 8090, 8091, 8092, 8093, 8094 |
...
|------|------|--------|-----------|-----|--------|
| 8081 | Sort | 1065.61 | 2934.42 | 175.4% | pattern |
| 8084 | Aggregate | 1060.55 | 1128.78 | 6.4% | operator |
| 8085 | Sort | 1060.30 | 1835.91 | 73.1% | pattern |
| 8087 | Nested Loop | 1048.52 | 1188.39 | 13.3% | operator |
| 8099 | Hash | 3.13 | 18.67 | 496.0% | operator |
| 8088 | Hash Join | 222.19 | 253.07 | 13.9% | operator |
| 8098 | Index Scan | 0.07 | -0.22 | 401.4% | operator |
| 8100 | Seq Scan | 2.44 | 12.36 | 407.0% | operator |
| 8089 | Seq Scan | 175.68 | 159.81 | 9.0% | operator |
| 8090 | Hash | 32.99 | 46.85 | 42.0% | pattern |
| 8095 | Seq Scan | 0.02 | 11.81 | 62048.8% | operator |
| 8096 | Hash | 0.10 | 21.05 | 20953.2% | operator |
| 8097 | Seq Scan | 0.10 | 22.93 | 23542.6% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8097 (Seq Scan) - LEAF
...
### Step 2: Node 8095 (Seq Scan) - LEAF
...
### Step 4: Node 8089 (Seq Scan) - LEAF
...
### Step 5: Node 8090 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 8081, 8082, 8083, 8085, 8086, 8091, 8092, 8093, 8094
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 8098 (Index Scan) - LEAF
...
### Step 8: Node 8100 (Seq Scan) - LEAF
...
### Step 11: Node 8085 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 3e2d5a00 (Sort -> Hash Join (Outer))
- **Consumes:** Nodes 8081, 8082, 8083, 8086, 8090, 8091, 8092, 8093, 8094
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 13: Node 8081 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** ddb1e0ca (Sort -> Aggregate -> Gather Merge (Outer) (Outer))
- **Consumes:** Nodes 8082, 8083, 8085, 8086, 8090, 8091, 8092, 8093, 8094
- **Input Features:**
