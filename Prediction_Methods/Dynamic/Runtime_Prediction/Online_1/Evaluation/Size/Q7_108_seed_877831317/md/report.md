...
**Test Query:** Q7_108_seed_877831317
**Timestamp:** 2026-01-18 17:43:57
...
```
Node 10645 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 10646 (Gather Merge) [consumed]
    Node 10647 (Sort)
      Node 10648 (Hash Join)
        Node 10649 (Nested Loop)
          Node 10650 (Hash Join) [PATTERN: 895c6e8c]
            Node 10651 (Seq Scan) [consumed] - LEAF
            Node 10652 (Hash) [consumed]
              Node 10653 (Hash Join) [PATTERN: f4cb205a]
                Node 10654 (Seq Scan) [consumed] - LEAF
                Node 10655 (Hash) [consumed]
                  Node 10656 (Seq Scan) [consumed] - LEAF
          Node 10657 (Index Scan) - LEAF
        Node 10658 (Hash)
          Node 10659 (Hash Join) [PATTERN: f4cb205a]
            Node 10660 (Seq Scan) [consumed] - LEAF
            Node 10661 (Hash) [consumed]
              Node 10662 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10653 | 10645, 10646, 10650, 10651, 10652, 10654, 10655, 10656, 10659, 10660, 10661, 10662 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 10659 | 10645, 10646, 10650, 10651, 10652, 10653, 10654, 10655, 10656, 10660, 10661, 10662 |
| Aggregate -> Gather Merge (Out | 2724c080 | 10645 | 10646, 10650, 10651, 10652, 10653, 10654, 10655, 10656, 10659, 10660, 10661, 10662 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 10650 | 10645, 10646, 10651, 10652, 10653, 10654, 10655, 10656, 10659, 10660, 10661, 10662 |
...
|------|------|--------|-----------|-----|--------|
| 10645 | Aggregate | 1239.59 | 1163.13 | 6.2% | pattern |
| 10647 | Sort | 1233.03 | 1109.16 | 10.0% | operator |
| 10648 | Hash Join | 1232.42 | 843.11 | 31.6% | operator |
| 10649 | Nested Loop | 1225.62 | 1215.99 | 0.8% | operator |
| 10658 | Hash | 3.06 | 27.74 | 806.3% | operator |
| 10650 | Hash Join | 216.04 | 304.89 | 41.1% | pattern |
| 10657 | Index Scan | 0.03 | 0.96 | 2731.9% | operator |
| 10659 | Hash Join | 3.01 | 429.36 | 14145.6% | pattern |
| 10653 | Hash Join | 36.21 | 429.38 | 1085.7% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 10653 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10645, 10646, 10650, 10651, 10652, 10654, 10655, 10656, 10659, 10660, 10661, 10662
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 10650 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 10645, 10646, 10651, 10652, 10653, 10654, 10655, 10656, 10659, 10660, 10661, 10662
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 10657 (Index Scan) - LEAF
...
### Step 4: Node 10659 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 10645, 10646, 10650, 10651, 10652, 10653, 10654, 10655, 10656, 10660, 10661, 10662
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 10645 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 10646, 10650, 10651, 10652, 10653, 10654, 10655, 10656, 10659, 10660, 10661, 10662
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
