...
```
Node 11203 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 11204 (Gather Merge) [consumed]
    Node 11205 (Sort)
      Node 11206 (Hash Join)
        Node 11207 (Nested Loop)
          Node 11208 (Hash Join) [PATTERN: 895c6e8c]
            Node 11209 (Seq Scan) [consumed] - LEAF
            Node 11210 (Hash) [consumed]
              Node 11211 (Hash Join) [PATTERN: f4cb205a]
                Node 11212 (Seq Scan) [consumed] - LEAF
                Node 11213 (Hash) [consumed]
                  Node 11214 (Seq Scan) [consumed] - LEAF
          Node 11215 (Index Scan) - LEAF
        Node 11216 (Hash)
          Node 11217 (Hash Join) [PATTERN: f4cb205a]
            Node 11218 (Seq Scan) [consumed] - LEAF
            Node 11219 (Hash) [consumed]
              Node 11220 (Seq Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11211 | 11203, 11204, 11208, 11209, 11210, 11212, 11213, 11214, 11217, 11218, 11219, 11220 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 11217 | 11203, 11204, 11208, 11209, 11210, 11211, 11212, 11213, 11214, 11218, 11219, 11220 |
| Aggregate -> Gather Merge (Out | 2724c080 | 11203 | 11204, 11208, 11209, 11210, 11211, 11212, 11213, 11214, 11217, 11218, 11219, 11220 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 11208 | 11203, 11204, 11209, 11210, 11211, 11212, 11213, 11214, 11217, 11218, 11219, 11220 |
...
|------|------|--------|-----------|-----|--------|
| 11203 | Aggregate | 1188.58 | 1163.13 | 2.1% | pattern |
| 11205 | Sort | 1179.79 | 1109.16 | 6.0% | operator |
| 11206 | Hash Join | 1179.20 | 843.11 | 28.5% | operator |
| 11207 | Nested Loop | 1173.22 | 1215.99 | 3.6% | operator |
| 11216 | Hash | 2.16 | 27.74 | 1185.0% | operator |
| 11208 | Hash Join | 213.21 | 304.89 | 43.0% | pattern |
| 11215 | Index Scan | 0.03 | 0.96 | 2908.9% | operator |
| 11217 | Hash Join | 2.12 | 429.36 | 20191.2% | pattern |
| 11211 | Hash Join | 36.12 | 429.38 | 1088.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 11211 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11203, 11204, 11208, 11209, 11210, 11212, 11213, 11214, 11217, 11218, 11219, 11220
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5000
  - HashJoin_nt1=62500
...
### Step 2: Node 11208 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 11203, 11204, 11209, 11210, 11211, 11212, 11213, 11214, 11217, 11218, 11219, 11220
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=38710
  - HashJoin_nt1=483871
...
### Step 3: Node 11215 (Index Scan) - LEAF
...
### Step 4: Node 11217 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 11203, 11204, 11208, 11209, 11210, 11211, 11212, 11213, 11214, 11218, 11219, 11220
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=800
  - HashJoin_nt1=10000
...
### Step 9: Node 11203 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 11204, 11208, 11209, 11210, 11211, 11212, 11213, 11214, 11217, 11218, 11219, 11220
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=6078
  - Aggregate_nt1=6078
