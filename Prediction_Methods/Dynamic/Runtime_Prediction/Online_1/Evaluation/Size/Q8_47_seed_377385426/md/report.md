...
**Test Query:** Q8_47_seed_377385426
...
```
Node 15294 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 15295 (Gather Merge) [consumed]
    Node 15296 (Sort)
      Node 15297 (Hash Join)
        Node 15298 (Nested Loop)
          Node 15299 (Hash Join)
            Node 15300 (Nested Loop)
              Node 15301 (Hash Join)
                Node 15302 (Seq Scan) - LEAF
                Node 15303 (Hash) [PATTERN: 444761fb]
                  Node 15304 (Hash Join) [consumed]
                    Node 15305 (Seq Scan) [consumed] - LEAF
                    Node 15306 (Hash) [consumed]
                      Node 15307 (Hash Join) [consumed]
                        Node 15308 (Seq Scan) - LEAF
                        Node 15309 (Hash)
                          Node 15310 (Seq Scan) - LEAF
              Node 15311 (Index Scan) - LEAF
            Node 15312 (Hash)
              Node 15313 (Seq Scan) - LEAF
          Node 15314 (Index Scan) - LEAF
        Node 15315 (Hash)
          Node 15316 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 15303 | 15294, 15295, 15304, 15305, 15306, 15307 |
| Aggregate -> Gather Merge (Out | 2724c080 | 15294 | 15295, 15303, 15304, 15305, 15306, 15307 |
...
|------|------|--------|-----------|-----|--------|
| 15294 | Aggregate | 1218.65 | 1133.66 | 7.0% | pattern |
| 15296 | Sort | 1213.42 | 1046.21 | 13.8% | operator |
| 15297 | Hash Join | 1213.18 | 785.61 | 35.2% | operator |
| 15298 | Nested Loop | 1212.87 | 1198.29 | 1.2% | operator |
| 15315 | Hash | 0.01 | 13.72 | 105469.9% | operator |
| 15299 | Hash Join | 1208.81 | 785.60 | 35.0% | operator |
| 15314 | Index Scan | 0.01 | 12.42 | 177378.3% | operator |
| 15316 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 15300 | Nested Loop | 1170.59 | 1198.29 | 2.4% | operator |
| 15312 | Hash | 33.84 | 19.97 | 41.0% | operator |
| 15301 | Hash Join | 208.18 | 251.67 | 20.9% | operator |
| 15311 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 15313 | Seq Scan | 33.77 | 48.88 | 44.7% | operator |
| 15302 | Seq Scan | 158.36 | 159.67 | 0.8% | operator |
| 15303 | Hash | 32.03 | 43.51 | 35.8% | pattern |
| 15308 | Seq Scan | 0.01 | 12.22 | 111035.8% | operator |
| 15309 | Hash | 0.23 | 18.14 | 7858.0% | operator |
| 15310 | Seq Scan | 0.22 | 23.36 | 10376.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 15310 (Seq Scan) - LEAF
...
### Step 2: Node 15308 (Seq Scan) - LEAF
...
### Step 4: Node 15302 (Seq Scan) - LEAF
...
### Step 5: Node 15303 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 15294, 15295, 15304, 15305, 15306, 15307
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 15311 (Index Scan) - LEAF
...
### Step 8: Node 15313 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - total_cost=70271.4800
- **Output:** st=113.62, rt=785.60

### Step 12: Node 15314 (Index Scan) - LEAF
...
### Step 13: Node 15316 (Seq Scan) - LEAF
...
### Step 14: Node 15298 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70505.0400
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 15315 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.8500
  - total_cost=70510.9100
- **Output:** st=113.62, rt=785.61

### Step 17: Node 15296 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70547.9300
  - total_cost=70549.8600
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 15294 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 15295, 15303, 15304, 15305, 15306, 15307
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2394
  - Aggregate_nt1=2394
  ...
  - Aggregate_startup_cost=71547.9700
  - Aggregate_total_cost=71926.9600
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2394
  - GatherMerge_Outer_nt1=772
  ...
  - GatherMerge_Outer_sel=3.1010
  - GatherMerge_Outer_startup_cost=71547.9700
  - GatherMerge_Outer_total_cost=71831.2000
