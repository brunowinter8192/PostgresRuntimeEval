...
**Test Query:** Q8_135_seed_1099340154
...
Node 14075 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 14076 (Gather Merge) [consumed]
    Node 14077 (Sort)
      Node 14078 (Hash Join)
        Node 14079 (Nested Loop)
          Node 14080 (Hash Join)
            Node 14081 (Nested Loop)
              Node 14082 (Hash Join)
                Node 14083 (Seq Scan) - LEAF
                Node 14084 (Hash) [PATTERN: 444761fb]
                  Node 14085 (Hash Join) [consumed]
                    Node 14086 (Seq Scan) [consumed] - LEAF
                    Node 14087 (Hash) [consumed]
                      Node 14088 (Hash Join) [consumed]
                        Node 14089 (Seq Scan) - LEAF
                        Node 14090 (Hash)
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 14084 | 14075, 14076, 14085, 14086, 14087, 14088 |
| Aggregate -> Gather Merge (Out | 2724c080 | 14075 | 14076, 14084, 14085, 14086, 14087, 14088 |
...
|------|------|--------|-----------|-----|--------|
| 14075 | Aggregate | 1209.19 | 1133.51 | 6.3% | pattern |
| 14077 | Sort | 1203.99 | 1046.21 | 13.1% | operator |
| 14078 | Hash Join | 1203.75 | 785.61 | 34.7% | operator |
| 14079 | Nested Loop | 1203.43 | 1198.29 | 0.4% | operator |
| 14096 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 14080 | Hash Join | 1199.46 | 785.60 | 34.5% | operator |
| 14095 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 14097 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 14081 | Nested Loop | 1153.57 | 1198.29 | 3.9% | operator |
| 14093 | Hash | 40.74 | 19.97 | 51.0% | operator |
| 14082 | Hash Join | 215.35 | 251.67 | 16.9% | operator |
| 14092 | Index Scan | 0.04 | 2.88 | 6926.5% | operator |
| 14094 | Seq Scan | 40.62 | 48.88 | 20.3% | operator |
| 14083 | Seq Scan | 161.00 | 159.67 | 0.8% | operator |
| 14084 | Hash | 36.60 | 43.51 | 18.9% | pattern |
| 14089 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 14090 | Hash | 0.13 | 18.14 | 13542.2% | operator |
| 14091 | Seq Scan | 0.13 | 23.36 | 17870.9% | operator |
...
### Step 2: Node 14089 (Seq Scan) - LEAF
...
### Step 4: Node 14083 (Seq Scan) - LEAF
...
### Step 5: Node 14084 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 14075, 14076, 14085, 14086, 14087, 14088
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 14092 (Index Scan) - LEAF
...
### Step 8: Node 14094 (Seq Scan) - LEAF
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=19.9704
  - startup_cost=9920.0500
  - total_cost=70271.5300
- **Output:** st=113.62, rt=785.60

### Step 12: Node 14095 (Index Scan) - LEAF
...
### Step 13: Node 14097 (Seq Scan) - LEAF
...
### Step 14: Node 14079 (Nested Loop)
...
- **Input Features:**
  ...
  - total_cost=70506.6100
- **Output:** st=186.04, rt=1198.29

### Step 15: Node 14096 (Hash)
...
- **Input Features:**
  ...
  - st1=186.0414
  - st2=13.7224
  - startup_cost=9921.9000
  - total_cost=70512.5000
- **Output:** st=113.62, rt=785.61

### Step 17: Node 14077 (Sort)
...
- **Input Features:**
  ...
  - st1=113.6207
  - st2=0.0000
  - startup_cost=70549.8000
  - total_cost=70551.7400
- **Output:** st=1044.94, rt=1046.21

### Step 18: Node 14075 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 14076, 14084, 14085, 14086, 14087, 14088
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2409
  ...
  - Aggregate_total_cost=71931.1400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2409
  - GatherMerge_Outer_nt1=777
  ...
  - GatherMerge_Outer_total_cost=71834.8400
- **Output:** st=1130.51, rt=1133.51
