...
**Test Query:** Q8_98_seed_795791007
...
Node 16582 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16583 (Gather Merge) [consumed]
    Node 16584 (Sort)
      Node 16585 (Hash Join)
        Node 16586 (Nested Loop)
          Node 16587 (Hash Join)
            Node 16588 (Nested Loop)
              Node 16589 (Hash Join)
                Node 16590 (Seq Scan) - LEAF
                Node 16591 (Hash) [PATTERN: 444761fb]
                  Node 16592 (Hash Join) [consumed]
                    Node 16593 (Seq Scan) [consumed] - LEAF
                    Node 16594 (Hash) [consumed]
                      Node 16595 (Hash Join) [consumed]
                        Node 16596 (Seq Scan) - LEAF
                        Node 16597 (Hash)
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16591 | 16582, 16583, 16592, 16593, 16594, 16595 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16582 | 16583, 16591, 16592, 16593, 16594, 16595 |
...
|------|------|--------|-----------|-----|--------|
| 16582 | Aggregate | 1229.29 | 1133.68 | 7.8% | pattern |
| 16584 | Sort | 1223.04 | 1046.21 | 14.5% | operator |
| 16585 | Hash Join | 1222.79 | 785.61 | 35.8% | operator |
| 16586 | Nested Loop | 1222.46 | 1198.29 | 2.0% | operator |
| 16603 | Hash | 0.02 | 13.72 | 85675.5% | operator |
| 16587 | Hash Join | 1218.04 | 785.60 | 35.5% | operator |
| 16602 | Index Scan | 0.01 | 12.42 | 177378.3% | operator |
| 16604 | Seq Scan | 0.01 | 6.51 | 59099.7% | operator |
| 16588 | Nested Loop | 1172.35 | 1198.29 | 2.2% | operator |
| 16600 | Hash | 41.26 | 19.97 | 51.6% | operator |
| 16589 | Hash Join | 213.47 | 251.67 | 17.9% | operator |
| 16599 | Index Scan | 0.04 | 2.88 | 6759.2% | operator |
| 16601 | Seq Scan | 41.16 | 48.88 | 18.7% | operator |
| 16590 | Seq Scan | 159.13 | 159.67 | 0.3% | operator |
| 16591 | Hash | 36.69 | 43.51 | 18.6% | pattern |
| 16596 | Seq Scan | 0.01 | 12.22 | 135732.7% | operator |
| 16597 | Hash | 0.14 | 18.14 | 13241.3% | operator |
| 16598 | Seq Scan | 0.13 | 23.36 | 17465.6% | operator |
...
### Step 2: Node 16596 (Seq Scan) - LEAF
...
### Step 4: Node 16590 (Seq Scan) - LEAF
...
### Step 5: Node 16591 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16582, 16583, 16592, 16593, 16594, 16595
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16599 (Index Scan) - LEAF
...
### Step 8: Node 16601 (Seq Scan) - LEAF
...
### Step 12: Node 16602 (Index Scan) - LEAF
...
### Step 13: Node 16604 (Seq Scan) - LEAF
...
### Step 14: Node 16586 (Nested Loop)
...
### Step 18: Node 16582 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16583, 16591, 16592, 16593, 16594, 16595
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2405
  - Aggregate_nt1=2405
  ...
  - Aggregate_total_cost=71930.1900
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2405
  ...
  - GatherMerge_Outer_total_cost=71833.9900
