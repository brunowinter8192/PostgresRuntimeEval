...
**Test Query:** Q8_112_seed_910647441
...
```
Node 13500 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 13501 (Gather Merge) [consumed]
    Node 13502 (Sort)
      Node 13503 (Hash Join)
        Node 13504 (Nested Loop)
          Node 13505 (Hash Join)
            Node 13506 (Nested Loop)
              Node 13507 (Hash Join)
                Node 13508 (Seq Scan) - LEAF
                Node 13509 (Hash) [PATTERN: 444761fb]
                  Node 13510 (Hash Join) [consumed]
                    Node 13511 (Seq Scan) [consumed] - LEAF
                    Node 13512 (Hash) [consumed]
                      Node 13513 (Hash Join) [consumed]
                        Node 13514 (Seq Scan) - LEAF
                        Node 13515 (Hash)
                          Node 13516 (Seq Scan) - LEAF
              Node 13517 (Index Scan) - LEAF
            Node 13518 (Hash)
              Node 13519 (Seq Scan) - LEAF
          Node 13520 (Index Scan) - LEAF
        Node 13521 (Hash)
          Node 13522 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 13509 | 13500, 13501, 13510, 13511, 13512, 13513 |
| Aggregate -> Gather Merge (Out | 2724c080 | 13500 | 13501, 13509, 13510, 13511, 13512, 13513 |
...
|------|------|--------|-----------|-----|--------|
| 13500 | Aggregate | 1260.42 | 1133.68 | 10.1% | pattern |
| 13502 | Sort | 1255.30 | 1046.21 | 16.7% | operator |
| 13503 | Hash Join | 1255.07 | 785.61 | 37.4% | operator |
| 13504 | Nested Loop | 1254.76 | 1198.29 | 4.5% | operator |
| 13521 | Hash | 0.01 | 13.72 | 97929.2% | operator |
| 13505 | Hash Join | 1250.77 | 785.60 | 37.2% | operator |
| 13520 | Index Scan | 0.01 | 12.42 | 206958.1% | operator |
| 13522 | Seq Scan | 0.01 | 6.51 | 65019.6% | operator |
| 13506 | Nested Loop | 1206.03 | 1198.29 | 0.6% | operator |
| 13518 | Hash | 40.37 | 19.97 | 50.5% | operator |
| 13507 | Hash Join | 222.59 | 251.67 | 13.1% | operator |
| 13517 | Index Scan | 0.04 | 2.88 | 6599.7% | operator |
| 13519 | Seq Scan | 40.31 | 48.89 | 21.3% | operator |
| 13508 | Seq Scan | 160.79 | 159.67 | 0.7% | operator |
| 13509 | Hash | 42.23 | 43.51 | 3.0% | pattern |
| 13514 | Seq Scan | 0.01 | 12.22 | 122149.4% | operator |
| 13515 | Hash | 0.21 | 18.14 | 8458.6% | operator |
| 13516 | Seq Scan | 0.21 | 23.36 | 11186.1% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 13516 (Seq Scan) - LEAF
...
### Step 2: Node 13514 (Seq Scan) - LEAF
...
### Step 4: Node 13508 (Seq Scan) - LEAF
...
### Step 5: Node 13509 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 13500, 13501, 13510, 13511, 13512, 13513
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 13517 (Index Scan) - LEAF
...
### Step 8: Node 13519 (Seq Scan) - LEAF
...
### Step 12: Node 13520 (Index Scan) - LEAF
...
### Step 13: Node 13522 (Seq Scan) - LEAF
...
### Step 14: Node 13504 (Nested Loop)
...
### Step 18: Node 13500 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 13501, 13509, 13510, 13511, 13512, 13513
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2327
  - Aggregate_nt1=2327
  ...
  - Aggregate_total_cost=71908.5400
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2327
  ...
  - GatherMerge_Outer_sel=3.0985
  - GatherMerge_Outer_startup_cost=71540.1600
  - GatherMerge_Outer_total_cost=71815.4600
