...
**Test Query:** Q10_73_seed_590690232
...
```
Node 20930 (Limit) [PATTERN: 25df29b5] - ROOT
  Node 20931 (Sort) [consumed]
    Node 20932 (Aggregate) [consumed]
      Node 20933 (Gather) [consumed]
        Node 20934 (Hash Join)
          Node 20935 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20936 (Nested Loop) [consumed]
              Node 20937 (Seq Scan) - LEAF
              Node 20938 (Index Scan) - LEAF
            Node 20939 (Hash) [consumed]
              Node 20940 (Seq Scan) - LEAF
          Node 20941 (Hash)
            Node 20942 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 20930 | 20931, 20932, 20933, 20935, 20936, 20939 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20935 | 20930, 20931, 20932, 20933, 20936, 20939 |
...
|------|------|--------|-----------|-----|--------|
| 20930 | Limit | 1169.48 | 1199.47 | 2.6% | pattern |
| 20934 | Hash Join | 1114.42 | 1095.50 | 1.7% | operator |
| 20935 | Hash Join | 1111.56 | 1120.19 | 0.8% | pattern |
| 20941 | Hash | 0.01 | 15.71 | 104638.5% | operator |
| 20942 | Seq Scan | 0.01 | 6.98 | 63342.8% | operator |
| 20937 | Seq Scan | 162.49 | 159.74 | 1.7% | operator |
| 20938 | Index Scan | 0.06 | -0.52 | 909.7% | operator |
| 20940 | Seq Scan | 29.04 | 26.64 | 8.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20937 (Seq Scan) - LEAF
...
### Step 2: Node 20938 (Index Scan) - LEAF
...
### Step 3: Node 20940 (Seq Scan) - LEAF
...
### Step 4: Node 20942 (Seq Scan) - LEAF
...
### Step 5: Node 20935 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20930, 20931, 20932, 20933, 20936, 20939
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17978
  - HashJoin_nt1=17978
...
### Step 8: Node 20930 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20931, 20932, 20933, 20935, 20936, 20939
- **Input Features:**
