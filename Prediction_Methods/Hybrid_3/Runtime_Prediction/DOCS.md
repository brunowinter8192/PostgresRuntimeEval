...
Performs forward feature selection, trains pattern-level SVM models, executes hybrid bottom-up prediction, and evaluates accuracy.
...
Analysis Scripts (independent, run after 03):
...
**Purpose:** Execute hybrid bottom-up prediction with passthrough inheritance on test queries
...
2. For each query in test set:
   ...
   - Three-phase prediction per depth level:
     - **Phase 1: Pattern Matching** - If parent+children match pattern → use pattern model
     - **Phase 2: Passthrough Inheritance** - If operator is passthrough (Sort, Hash, Limit, etc.) → inherit child prediction
     - **Phase 3: Operator Fallback** - Remaining operators → use operator-level fallback model
   - Propagate child predictions to parent features
3. Export predictions with actual vs predicted times and prediction type
...
- `{output-dir}/predictions.csv`
  - Columns: query_file, node_id, node_type, prediction_type (pattern/operator/passthrough), actual_startup_time, predicted_startup_time, actual_total_time, predicted_total_time

**Usage:**
```bash
python 03_Predict_Queries.py test.csv \
...
**Purpose:** Cross-evaluation split by prediction source (pattern vs operator vs passthrough)

**Workflow:**
1. Load predictions
2. Split by prediction_type column (pattern/operator/passthrough)
3. Calculate MRE per node type for each prediction type
4. Export separate analysis for pattern and operator predictions
5. Passthrough operators inherit predictions perfectly (zero approximation error)
