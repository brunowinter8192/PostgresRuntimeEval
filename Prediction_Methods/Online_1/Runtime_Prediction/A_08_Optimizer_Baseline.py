...
# ORCHESTRATOR
def optimizer_baseline_workflow(train_csv: str, test_csv: str, output_dir: str) -> None:
...
def evaluate_optimizer(model, test_df: pd.DataFrame) -> dict:
...
def export_results(results: dict, output_dir: str) -> None:
    ...
    overall_df.to_csv(output_path / 'A_08_optimizer_overall_mre.csv', sep=';', index=False)

    results['template_stats'].to_csv(output_path / 'A_08_optimizer_template_mre.csv', sep=';')
