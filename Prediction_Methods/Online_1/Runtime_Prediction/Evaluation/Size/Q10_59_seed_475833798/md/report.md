...
```
Node 20722 (Limit) [PATTERN: 04a01b61] - ROOT
  Node 20723 (Sort) [consumed]
    Node 20724 (Aggregate) [consumed]
      Node 20725 (Gather) [consumed]
        Node 20726 (Hash Join) [consumed]
          Node 20727 (Hash Join) [PATTERN: 2e0f44ef]
            Node 20728 (Nested Loop) [consumed]
              Node 20729 (Seq Scan) - LEAF
              Node 20730 (Index Scan) - LEAF
            Node 20731 (Hash) [consumed]
              Node 20732 (Seq Scan) - LEAF
          Node 20733 (Hash)
            Node 20734 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 04a01b61 | 20722 | 20723, 20724, 20725, 20726, 20727, 20728, 20731 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 20727 | 20722, 20723, 20724, 20725, 20726, 20728, 20731 |
...
|------|------|--------|-----------|-----|--------|
| 20722 | Limit | 1141.61 | 1151.16 | 0.8% | pattern |
| 20727 | Hash Join | 1093.27 | 1101.74 | 0.8% | pattern |
| 20733 | Hash | 0.01 | 14.54 | 111744.0% | operator |
| 20734 | Seq Scan | 0.01 | 7.19 | 79838.6% | operator |
| 20729 | Seq Scan | 159.80 | 162.03 | 1.4% | operator |
| 20730 | Index Scan | 0.06 | -0.02 | 129.5% | operator |
| 20732 | Seq Scan | 27.75 | 29.69 | 7.0% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 20729 (Seq Scan) - LEAF
...
### Step 2: Node 20730 (Index Scan) - LEAF
...
### Step 3: Node 20732 (Seq Scan) - LEAF
...
### Step 4: Node 20734 (Seq Scan) - LEAF
...
### Step 5: Node 20727 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 20722, 20723, 20724, 20725, 20726, 20728, 20731
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18186
  - HashJoin_nt1=18186
...
### Step 7: Node 20722 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 04a01b61 (Limit -> Sort -> Aggregate -> Gather -> Hash Join (Outer) (Outer) (Outer) (Outer))
- **Consumes:** Nodes 20723, 20724, 20725, 20726, 20727, 20728, 20731
- **Input Features:**
