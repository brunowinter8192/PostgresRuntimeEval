...
**Test Query:** Q9_92_seed_746566821
...
```
Node 19207 (Sort) - ROOT
  Node 19208 (Aggregate) [PATTERN: 0405d50f]
    Node 19209 (Gather) [consumed]
      Node 19210 (Aggregate) [consumed]
        Node 19211 (Nested Loop) [consumed]
          Node 19212 (Hash Join)
            Node 19213 (Seq Scan) - LEAF
            Node 19214 (Hash) [PATTERN: 7df893ad]
              Node 19215 (Hash Join) [consumed]
                Node 19216 (Hash Join) [PATTERN: 2e0f44ef]
                  Node 19217 (Nested Loop) [consumed]
                    Node 19218 (Seq Scan) - LEAF
                    Node 19219 (Index Scan) - LEAF
                  Node 19220 (Hash) [consumed]
                    Node 19221 (Seq Scan) - LEAF
                Node 19222 (Hash)
                  Node 19223 (Seq Scan) - LEAF
          Node 19224 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather -> Aggrega | 0405d50f | 19208 | 19209, 19210, 19211, 19214, 19215, 19216, 19217, 19220 |
| Hash -> Hash Join (Outer) | 7df893ad | 19214 | 19208, 19209, 19210, 19211, 19215, 19216, 19217, 19220 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19216 | 19208, 19209, 19210, 19211, 19214, 19215, 19217, 19220 |
...
|------|------|--------|-----------|-----|--------|
| 19207 | Sort | 1195.21 | 1159.31 | 3.0% | operator |
| 19208 | Aggregate | 1195.09 | 1194.84 | 0.0% | pattern |
| 19212 | Hash Join | 977.87 | 831.42 | 15.0% | operator |
| 19224 | Index Scan | 0.00 | -0.04 | 1512.7% | operator |
| 19213 | Seq Scan | 701.58 | 565.06 | 19.5% | operator |
| 19214 | Hash | 181.10 | 48.85 | 73.0% | pattern |
| 19216 | Hash Join | 165.13 | 981.22 | 494.2% | pattern |
| 19222 | Hash | 13.30 | 14.54 | 9.3% | operator |
| 19223 | Seq Scan | 13.29 | 7.19 | 45.9% | operator |
| 19218 | Seq Scan | 19.73 | 42.93 | 117.6% | operator |
| 19219 | Index Scan | 0.08 | 0.12 | 54.8% | operator |
| 19221 | Seq Scan | 2.85 | 10.62 | 272.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19218 (Seq Scan) - LEAF
...
### Step 2: Node 19219 (Index Scan) - LEAF
...
### Step 3: Node 19221 (Seq Scan) - LEAF
...
### Step 4: Node 19223 (Seq Scan) - LEAF
...
### Step 5: Node 19216 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19208, 19209, 19210, 19211, 19214, 19215, 19217, 19220
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18037
  - HashJoin_nt1=18037
...
### Step 7: Node 19213 (Seq Scan) - LEAF
...
### Step 8: Node 19214 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 7df893ad (Hash -> Hash Join (Outer))
- **Consumes:** Nodes 19208, 19209, 19210, 19211, 19215, 19216, 19217, 19220
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=18037
  - HashJoin_Outer_nt1=18037
  ...
  - HashJoin_Outer_total_cost=14794.7200
  - Hash_np=0
  - Hash_nt=18037
  - Hash_nt1=18037
  - Hash_nt2=0
  ...
  - Hash_sel=1.0000
  - Hash_startup_cost=14794.7200
  - Hash_total_cost=14794.7200
- **Output:** st=48.85, rt=48.85

### Step 9: Node 19212 (Hash Join)
...
- **Output:** st=168.59, rt=831.42

### Step 10: Node 19224 (Index Scan) - LEAF
...
### Step 11: Node 19208 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 0405d50f (Aggregate -> Gather -> Aggregate -> Nested Loop (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19209, 19210, 19211, 19214, 19215, 19216, 19217, 19220
- **Input Features:**
  ...
  - NestedLoop_Outer_np=0
  - NestedLoop_Outer_nt=64945
  - NestedLoop_Outer_nt1=64945
  - NestedLoop_Outer_nt2=1
  - NestedLoop_Outer_parallel_workers=0
  - NestedLoop_Outer_plan_width=159
  - NestedLoop_Outer_reltuples=0.0000
  - NestedLoop_Outer_sel=1.0000
  - NestedLoop_Outer_startup_cost=15065.7100
  - NestedLoop_Outer_total_cost=178092.6500
- **Output:** st=1188.79, rt=1194.84

### Step 12: Node 19207 (Sort) - ROOT
