...
**Test Query:** Q9_124_seed_1009095813
...
```
Node 17122 (Sort) - ROOT
  Node 17123 (Aggregate) [PATTERN: 0405d50f]
    Node 17124 (Gather) [consumed]
      Node 17125 (Aggregate) [consumed]
        Node 17126 (Nested Loop) [consumed]
          Node 17127 (Hash Join)
            Node 17128 (Seq Scan) - LEAF
            Node 17129 (Hash) [PATTERN: 7df893ad]
              Node 17130 (Hash Join) [consumed]
                Node 17131 (Hash Join) [PATTERN: 2e0f44ef]
                  Node 17132 (Nested Loop) [consumed]
                    Node 17133 (Seq Scan) - LEAF
                    Node 17134 (Index Scan) - LEAF
                  Node 17135 (Hash) [consumed]
                    Node 17136 (Seq Scan) - LEAF
                Node 17137 (Hash)
                  Node 17138 (Seq Scan) - LEAF
          Node 17139 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather -> Aggrega | 0405d50f | 17123 | 17124, 17125, 17126, 17129, 17130, 17131, 17132, 17135 |
| Hash -> Hash Join (Outer) | 7df893ad | 17129 | 17123, 17124, 17125, 17126, 17130, 17131, 17132, 17135 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 17131 | 17123, 17124, 17125, 17126, 17129, 17130, 17132, 17135 |
...
|------|------|--------|-----------|-----|--------|
| 17122 | Sort | 1195.21 | 1157.57 | 3.1% | operator |
| 17123 | Aggregate | 1195.08 | 1194.93 | 0.0% | pattern |
| 17127 | Hash Join | 979.04 | 830.87 | 15.1% | operator |
| 17139 | Index Scan | 0.00 | -0.04 | 1512.7% | operator |
| 17128 | Seq Scan | 707.37 | 565.06 | 20.1% | operator |
| 17129 | Hash | 176.76 | 46.65 | 73.6% | pattern |
| 17131 | Hash Join | 160.34 | 982.13 | 512.5% | pattern |
| 17137 | Hash | 13.16 | 14.54 | 10.5% | operator |
| 17138 | Seq Scan | 13.15 | 7.19 | 45.3% | operator |
| 17133 | Seq Scan | 19.92 | 43.18 | 116.8% | operator |
| 17134 | Index Scan | 0.07 | 0.12 | 63.3% | operator |
| 17136 | Seq Scan | 4.75 | 10.62 | 123.3% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 17133 (Seq Scan) - LEAF
...
### Step 2: Node 17134 (Index Scan) - LEAF
...
### Step 3: Node 17136 (Seq Scan) - LEAF
...
### Step 4: Node 17138 (Seq Scan) - LEAF
...
### Step 5: Node 17131 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 17123, 17124, 17125, 17126, 17129, 17130, 17132, 17135
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=15363
  - HashJoin_nt1=15363
...
### Step 7: Node 17128 (Seq Scan) - LEAF
...
### Step 8: Node 17129 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 7df893ad (Hash -> Hash Join (Outer))
- **Consumes:** Nodes 17123, 17124, 17125, 17126, 17130, 17131, 17132, 17135
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=15363
  - HashJoin_Outer_nt1=15363
  ...
  - HashJoin_Outer_total_cost=14172.3300
  - Hash_np=0
  - Hash_nt=15363
  - Hash_nt1=15363
  - Hash_nt2=0
  ...
  - Hash_sel=1.0000
  - Hash_startup_cost=14172.3300
  - Hash_total_cost=14172.3300
- **Output:** st=46.65, rt=46.65

### Step 9: Node 17127 (Hash Join)
...
- **Output:** st=168.87, rt=830.87

### Step 10: Node 17139 (Index Scan) - LEAF
...
### Step 11: Node 17123 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 0405d50f (Aggregate -> Gather -> Aggregate -> Nested Loop (Outer) (Outer) (Outer))
- **Consumes:** Nodes 17124, 17125, 17126, 17129, 17130, 17131, 17132, 17135
- **Input Features:**
  ...
  - NestedLoop_Outer_np=0
  - NestedLoop_Outer_nt=55319
  - NestedLoop_Outer_nt1=55319
  - NestedLoop_Outer_nt2=1
  - NestedLoop_Outer_parallel_workers=0
  - NestedLoop_Outer_plan_width=159
  - NestedLoop_Outer_reltuples=0.0000
  - NestedLoop_Outer_sel=1.0000
  - NestedLoop_Outer_startup_cost=14403.2000
  - NestedLoop_Outer_total_cost=173069.1200
- **Output:** st=1188.87, rt=1194.93

### Step 12: Node 17122 (Sort) - ROOT
