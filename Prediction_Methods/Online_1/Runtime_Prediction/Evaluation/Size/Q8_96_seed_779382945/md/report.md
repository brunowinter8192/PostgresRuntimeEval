...
**Test Query:** Q8_96_seed_779382945
...
```
Node 16536 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16537 (Gather Merge) [consumed]
    Node 16538 (Sort)
      Node 16539 (Hash Join)
        Node 16540 (Nested Loop)
          Node 16541 (Hash Join)
            Node 16542 (Nested Loop)
              Node 16543 (Hash Join)
                Node 16544 (Seq Scan) - LEAF
                Node 16545 (Hash) [PATTERN: 444761fb]
                  Node 16546 (Hash Join) [consumed]
                    Node 16547 (Seq Scan) [consumed] - LEAF
                    Node 16548 (Hash) [consumed]
                      Node 16549 (Hash Join) [consumed]
                        Node 16550 (Seq Scan) - LEAF
                        Node 16551 (Hash)
                          Node 16552 (Seq Scan) - LEAF
              Node 16553 (Index Scan) - LEAF
            Node 16554 (Hash)
              Node 16555 (Seq Scan) - LEAF
          Node 16556 (Index Scan) - LEAF
        Node 16557 (Hash)
          Node 16558 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16545 | 16536, 16537, 16546, 16547, 16548, 16549 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16536 | 16537, 16545, 16546, 16547, 16548, 16549 |
...
|------|------|--------|-----------|-----|--------|
| 16536 | Aggregate | 1237.33 | 1176.20 | 4.9% | pattern |
| 16538 | Sort | 1232.55 | 1087.88 | 11.7% | operator |
| 16539 | Hash Join | 1232.30 | 1055.48 | 14.3% | operator |
| 16540 | Nested Loop | 1231.99 | 1105.66 | 10.3% | operator |
| 16557 | Hash | 0.01 | 14.54 | 121064.3% | operator |
| 16541 | Hash Join | 1228.08 | 1027.33 | 16.3% | operator |
| 16556 | Index Scan | 0.01 | 0.05 | 753.5% | operator |
| 16558 | Seq Scan | 0.01 | 7.19 | 89830.9% | operator |
| 16542 | Nested Loop | 1181.30 | 1119.20 | 5.3% | operator |
| 16554 | Hash | 42.04 | 17.76 | 57.7% | operator |
| 16543 | Hash Join | 212.82 | 226.59 | 6.5% | operator |
| 16553 | Index Scan | 0.04 | 0.09 | 107.2% | operator |
| 16555 | Seq Scan | 41.93 | 44.48 | 6.1% | operator |
| 16544 | Seq Scan | 159.19 | 161.81 | 1.6% | operator |
| 16545 | Hash | 35.37 | 36.08 | 2.0% | pattern |
| 16550 | Seq Scan | 0.01 | 9.26 | 102795.9% | operator |
| 16551 | Hash | 0.08 | 16.01 | 19671.5% | operator |
| 16552 | Seq Scan | 0.08 | 21.39 | 26975.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 16552 (Seq Scan) - LEAF
...
### Step 2: Node 16550 (Seq Scan) - LEAF
...
### Step 4: Node 16544 (Seq Scan) - LEAF
...
### Step 5: Node 16545 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16536, 16537, 16546, 16547, 16548, 16549
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16553 (Index Scan) - LEAF
...
### Step 8: Node 16555 (Seq Scan) - LEAF
...
### Step 12: Node 16556 (Index Scan) - LEAF
...
### Step 13: Node 16558 (Seq Scan) - LEAF
...
### Step 14: Node 16540 (Nested Loop)
...
### Step 18: Node 16536 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16537, 16545, 16546, 16547, 16548, 16549
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2480
  ...
  - Aggregate_sel=0.9702
  - Aggregate_startup_cost=71558.4000
  - Aggregate_total_cost=71949.5200
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2480
  ...
  - GatherMerge_Outer_total_cost=71851.8000
- **Output:** st=1173.72, rt=1176.20
