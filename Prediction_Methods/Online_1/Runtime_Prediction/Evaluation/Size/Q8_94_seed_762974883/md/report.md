...
```
Node 16490 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16491 (Gather Merge) [consumed]
    Node 16492 (Sort)
          ...
          Node 16495 (Hash Join)
            Node 16496 (Nested Loop)
              Node 16497 (Hash Join)
                ...
                Node 16499 (Hash) [PATTERN: 444761fb]
                  Node 16500 (Hash Join) [consumed]
                    Node 16501 (Seq Scan) [consumed] - LEAF
                    Node 16502 (Hash) [consumed]
                      Node 16503 (Hash Join) [consumed]
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16499 | 16490, 16491, 16500, 16501, 16502, 16503 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16490 | 16491, 16499, 16500, 16501, 16502, 16503 |
...
|------|------|--------|-----------|-----|--------|
| 16490 | Aggregate | 1233.58 | 1180.60 | 4.3% | pattern |
| 16492 | Sort | 1227.88 | 1087.88 | 11.4% | operator |
| 16493 | Hash Join | 1227.64 | 1055.47 | 14.0% | operator |
| 16494 | Nested Loop | 1227.33 | 1105.65 | 9.9% | operator |
| 16511 | Hash | 0.02 | 14.54 | 80676.2% | operator |
| 16495 | Hash Join | 1223.53 | 1027.32 | 16.0% | operator |
...
| 16497 | Hash Join | 214.61 | 226.59 | 5.6% | operator |
...
| 16499 | Hash | 35.44 | 36.08 | 1.8% | pattern |
...
### Step 4: Node 16498 (Seq Scan) - LEAF
...
### Step 5: Node 16499 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16490, 16491, 16500, 16501, 16502, 16503
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16507 (Index Scan) - LEAF
...
### Step 8: Node 16509 (Seq Scan) - LEAF
...
### Step 12: Node 16510 (Index Scan) - LEAF
...
### Step 13: Node 16512 (Seq Scan) - LEAF
...
### Step 14: Node 16494 (Nested Loop)
...
### Step 18: Node 16490 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16491, 16499, 16500, 16501, 16502, 16503
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2343
  - Aggregate_nt1=2343
  ...
  - Aggregate_startup_cost=71542.0200
  - Aggregate_total_cost=71912.9300
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2343
  - GatherMerge_Outer_nt1=756
  ...
  - GatherMerge_Outer_sel=3.0992
  - GatherMerge_Outer_startup_cost=71542.0200
  - GatherMerge_Outer_total_cost=71819.2100
