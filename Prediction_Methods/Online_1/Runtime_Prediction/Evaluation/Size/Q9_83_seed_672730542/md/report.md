...
**Test Query:** Q9_83_seed_672730542
...
```
Node 19025 (Sort) - ROOT
  Node 19026 (Aggregate) [PATTERN: 0405d50f]
    Node 19027 (Gather) [consumed]
      Node 19028 (Aggregate) [consumed]
        Node 19029 (Nested Loop) [consumed]
          Node 19030 (Hash Join)
            Node 19031 (Seq Scan) - LEAF
            Node 19032 (Hash) [PATTERN: 7df893ad]
              Node 19033 (Hash Join) [consumed]
                Node 19034 (Hash Join) [PATTERN: 2e0f44ef]
                  Node 19035 (Nested Loop) [consumed]
                    Node 19036 (Seq Scan) - LEAF
                    Node 19037 (Index Scan) - LEAF
                  Node 19038 (Hash) [consumed]
                    Node 19039 (Seq Scan) - LEAF
                Node 19040 (Hash)
                  Node 19041 (Seq Scan) - LEAF
          Node 19042 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather -> Aggrega | 0405d50f | 19026 | 19027, 19028, 19029, 19032, 19033, 19034, 19035, 19038 |
| Hash -> Hash Join (Outer) | 7df893ad | 19032 | 19026, 19027, 19028, 19029, 19033, 19034, 19035, 19038 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 19034 | 19026, 19027, 19028, 19029, 19032, 19033, 19035, 19038 |
...
|------|------|--------|-----------|-----|--------|
| 19025 | Sort | 1154.17 | 1159.60 | 0.5% | operator |
| 19026 | Aggregate | 1154.04 | 1194.84 | 3.5% | pattern |
| 19030 | Hash Join | 929.37 | 831.54 | 10.5% | operator |
| 19042 | Index Scan | 0.00 | -0.04 | 1512.7% | operator |
| 19031 | Seq Scan | 652.38 | 565.06 | 13.4% | operator |
| 19032 | Hash | 183.22 | 49.39 | 73.0% | pattern |
| 19034 | Hash Join | 167.37 | 981.11 | 486.2% | pattern |
| 19040 | Hash | 13.23 | 14.54 | 9.9% | operator |
| 19041 | Seq Scan | 13.22 | 7.19 | 45.6% | operator |
| 19036 | Seq Scan | 19.31 | 42.86 | 121.9% | operator |
| 19037 | Index Scan | 0.08 | 0.12 | 52.8% | operator |
| 19039 | Seq Scan | 3.76 | 10.62 | 182.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 19036 (Seq Scan) - LEAF
...
### Step 2: Node 19037 (Index Scan) - LEAF
...
### Step 3: Node 19039 (Seq Scan) - LEAF
...
### Step 4: Node 19041 (Seq Scan) - LEAF
...
### Step 5: Node 19034 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 19026, 19027, 19028, 19029, 19032, 19033, 19035, 19038
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=18703
  - HashJoin_nt1=18703
  ...
  - NestedLoop_Outer_nt=18703
  - NestedLoop_Outer_nt1=4676
...
- **Output:** st=5.46, rt=981.11

### Step 6: Node 19040 (Hash)
...
### Step 7: Node 19031 (Seq Scan) - LEAF
...
### Step 8: Node 19032 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 7df893ad (Hash -> Hash Join (Outer))
- **Consumes:** Nodes 19026, 19027, 19028, 19029, 19033, 19034, 19035, 19038
- **Input Features:**
  - HashJoin_Outer_np=0
  ...
  - Hash_sel=1.0000
  - Hash_startup_cost=14948.1600
  - Hash_total_cost=14948.1600
- **Output:** st=49.39, rt=49.39

### Step 9: Node 19030 (Hash Join)
...
- **Output:** st=168.52, rt=831.54

### Step 10: Node 19042 (Index Scan) - LEAF
...
### Step 11: Node 19026 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 0405d50f (Aggregate -> Gather -> Aggregate -> Nested Loop (Outer) (Outer) (Outer))
- **Consumes:** Nodes 19027, 19028, 19029, 19032, 19033, 19034, 19035, 19038
- **Input Features:**
  ...
  - NestedLoop_Outer_np=0
  - NestedLoop_Outer_nt=67346
  - NestedLoop_Outer_nt1=67346
  - NestedLoop_Outer_nt2=1
  - NestedLoop_Outer_parallel_workers=0
  - NestedLoop_Outer_plan_width=159
  - NestedLoop_Outer_reltuples=0.0000
  - NestedLoop_Outer_sel=1.0000
  - NestedLoop_Outer_startup_cost=15229.1300
  - NestedLoop_Outer_total_cost=179343.8400
- **Output:** st=1188.79, rt=1194.84

### Step 12: Node 19025 (Sort) - ROOT
