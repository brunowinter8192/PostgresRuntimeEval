...
**Test Query:** Q9_128_seed_1041911937
...
```
Node 17194 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 17195 (Aggregate) [consumed]
    Node 17196 (Gather)
      Node 17197 (Aggregate)
        Node 17198 (Nested Loop)
          Node 17199 (Hash Join) [PATTERN: 895c6e8c]
            Node 17200 (Seq Scan) [consumed] - LEAF
            Node 17201 (Hash) [consumed]
              Node 17202 (Hash Join)
                Node 17203 (Hash Join) [PATTERN: 2e0f44ef]
                  Node 17204 (Nested Loop) [consumed]
                    Node 17205 (Seq Scan) - LEAF
                    Node 17206 (Index Scan) - LEAF
                  Node 17207 (Hash) [consumed]
                    Node 17208 (Seq Scan) - LEAF
                Node 17209 (Hash)
                  Node 17210 (Seq Scan) - LEAF
          Node 17211 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 17199 | 17194, 17195, 17200, 17201, 17203, 17204, 17207 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 17194 | 17195, 17199, 17200, 17201, 17203, 17204, 17207 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 17203 | 17194, 17195, 17199, 17200, 17201, 17204, 17207 |
...
|------|------|--------|-----------|-----|--------|
| 17194 | Sort | 1168.57 | 1114.71 | 4.6% | pattern |
| 17196 | Gather | 1167.36 | 1098.11 | 5.9% | operator |
| 17197 | Aggregate | 1149.87 | 1169.71 | 1.7% | operator |
| 17198 | Nested Loop | 1126.25 | 1168.16 | 3.7% | operator |
| 17199 | Hash Join | 954.86 | 732.52 | 23.3% | pattern |
| 17211 | Index Scan | 0.00 | -0.04 | 1512.7% | operator |
| 17202 | Hash Join | 167.07 | 824.24 | 393.3% | operator |
| 17203 | Hash Join | 153.76 | 983.27 | 539.5% | pattern |
| 17209 | Hash | 12.70 | 14.54 | 14.5% | operator |
| 17210 | Seq Scan | 12.69 | 7.19 | 43.3% | operator |
| 17205 | Seq Scan | 18.70 | 43.37 | 132.0% | operator |
| 17206 | Index Scan | 0.07 | 0.12 | 67.9% | operator |
| 17208 | Seq Scan | 3.04 | 10.62 | 249.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 17205 (Seq Scan) - LEAF
...
### Step 2: Node 17206 (Index Scan) - LEAF
...
### Step 3: Node 17208 (Seq Scan) - LEAF
...
### Step 4: Node 17210 (Seq Scan) - LEAF
...
### Step 5: Node 17203 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 17194, 17195, 17199, 17200, 17201, 17204, 17207
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=13360
  - HashJoin_nt1=13360
...
- **Output:** st=26.18, rt=824.24

### Step 8: Node 17199 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 17194, 17195, 17200, 17201, 17203, 17204, 17207
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=48106
...
- **Output:** st=176.32, rt=732.52

### Step 9: Node 17211 (Index Scan) - LEAF
...
### Step 10: Node 17198 (Nested Loop)
...
- **Output:** st=105.34, rt=1168.16

### Step 11: Node 17197 (Aggregate)
...
- **Output:** st=1160.25, rt=1169.71

### Step 12: Node 17196 (Gather)
...
- **Output:** st=587.61, rt=1098.11

### Step 13: Node 17194 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 17195, 17199, 17200, 17201, 17203, 17204, 17207
- **Input Features:**
