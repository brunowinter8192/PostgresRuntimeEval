...
```
Node 8541 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 8542 (Aggregate) [consumed]
    Node 8543 (Gather Merge)
      Node 8544 (Aggregate)
        Node 8545 (Sort)
          Node 8546 (Hash Join) [PATTERN: 2e0f44ef]
            Node 8547 (Nested Loop) [consumed]
              Node 8548 (Hash Join) [PATTERN: 895c6e8c]
                Node 8549 (Seq Scan) [consumed] - LEAF
                Node 8550 (Hash) [consumed]
                  Node 8551 (Hash Join) [PATTERN: 895c6e8c]
                    Node 8552 (Seq Scan) [consumed] - LEAF
                    Node 8553 (Hash) [consumed]
                      Node 8554 (Hash Join) [PATTERN: f4cb205a]
                        Node 8555 (Seq Scan) [consumed] - LEAF
                        Node 8556 (Hash) [consumed]
                          Node 8557 (Seq Scan) [consumed] - LEAF
              Node 8558 (Index Scan) - LEAF
            Node 8559 (Hash) [consumed]
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 8554 | 8541, 8542, 8546, 8547, 8548, 8549, 8550, 8551, 8552, 8553, 8555, 8556, 8557, 8559 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 8548 | 8541, 8542, 8546, 8547, 8549, 8550, 8551, 8552, 8553, 8554, 8555, 8556, 8557, 8559 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 8551 | 8541, 8542, 8546, 8547, 8548, 8549, 8550, 8552, 8553, 8554, 8555, 8556, 8557, 8559 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 8541 | 8542, 8546, 8547, 8548, 8549, 8550, 8551, 8552, 8553, 8554, 8555, 8556, 8557, 8559 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 8546 | 8541, 8542, 8547, 8548, 8549, 8550, 8551, 8552, 8553, 8554, 8555, 8556, 8557, 8559 |
...
|------|------|--------|-----------|-----|--------|
| 8541 | Sort | 1062.91 | 1080.15 | 1.6% | pattern |
| 8543 | Gather Merge | 1062.89 | 1124.35 | 5.8% | operator |
| 8544 | Aggregate | 1058.07 | 1019.65 | 3.6% | operator |
| 8545 | Sort | 1057.83 | 1073.13 | 1.4% | operator |
| 8546 | Hash Join | 1057.33 | 1080.65 | 2.2% | pattern |
| 8548 | Hash Join | 217.29 | 259.80 | 19.6% | pattern |
| 8558 | Index Scan | 0.07 | -0.03 | 134.4% | operator |
| 8560 | Seq Scan | 3.22 | 10.62 | 230.1% | operator |
| 8551 | Hash Join | 38.61 | 84.97 | 120.1% | pattern |
| 8554 | Hash Join | 0.16 | 11.62 | 7070.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 8554 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 8541, 8542, 8546, 8547, 8548, 8549, 8550, 8551, 8552, 8553, 8555, 8556, 8557, 8559
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5
  - HashJoin_nt1=25
...
### Step 2: Node 8551 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 8541, 8542, 8546, 8547, 8548, 8549, 8550, 8552, 8553, 8554, 8555, 8556, 8557, 8559
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=12500
  - HashJoin_nt1=62500
...
### Step 3: Node 8548 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 8541, 8542, 8546, 8547, 8549, 8550, 8551, 8552, 8553, 8554, 8555, 8556, 8557, 8559
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=14884
  - HashJoin_nt1=74418
...
### Step 4: Node 8558 (Index Scan) - LEAF
...
### Step 5: Node 8560 (Seq Scan) - LEAF
...
### Step 6: Node 8546 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 8541, 8542, 8547, 8548, 8549, 8550, 8551, 8552, 8553, 8554, 8555, 8556, 8557, 8559
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=2382
  - HashJoin_nt1=59546
...
### Step 10: Node 8541 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 8542, 8546, 8547, 8548, 8549, 8550, 8551, 8552, 8553, 8554, 8555, 8556, 8557, 8559
- **Input Features:**
