...
**Test Query:** Q9_59_seed_475833798
...
```
Node 18534 (Sort) - ROOT
  Node 18535 (Aggregate) [PATTERN: 0405d50f]
    Node 18536 (Gather) [consumed]
      Node 18537 (Aggregate) [consumed]
        Node 18538 (Nested Loop) [consumed]
          Node 18539 (Hash Join)
            Node 18540 (Seq Scan) - LEAF
            Node 18541 (Hash) [PATTERN: 7df893ad]
              Node 18542 (Hash Join) [consumed]
                Node 18543 (Hash Join) [PATTERN: 2e0f44ef]
                  Node 18544 (Nested Loop) [consumed]
                    Node 18545 (Seq Scan) - LEAF
                    Node 18546 (Index Scan) - LEAF
                  Node 18547 (Hash) [consumed]
                    Node 18548 (Seq Scan) - LEAF
                Node 18549 (Hash)
                  Node 18550 (Seq Scan) - LEAF
          Node 18551 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather -> Aggrega | 0405d50f | 18535 | 18536, 18537, 18538, 18541, 18542, 18543, 18544, 18547 |
| Hash -> Hash Join (Outer) | 7df893ad | 18541 | 18535, 18536, 18537, 18538, 18542, 18543, 18544, 18547 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 18543 | 18535, 18536, 18537, 18538, 18541, 18542, 18544, 18547 |
...
|------|------|--------|-----------|-----|--------|
| 18534 | Sort | 1188.18 | 1159.89 | 2.4% | operator |
| 18535 | Aggregate | 1187.72 | 1194.84 | 0.6% | pattern |
| 18539 | Hash Join | 961.09 | 831.67 | 13.5% | operator |
| 18551 | Index Scan | 0.00 | -0.04 | 1512.7% | operator |
| 18540 | Seq Scan | 685.39 | 565.06 | 17.6% | operator |
| 18541 | Hash | 181.23 | 49.93 | 72.4% | pattern |
| 18543 | Hash Join | 164.39 | 981.05 | 496.8% | pattern |
| 18549 | Hash | 13.87 | 14.54 | 4.8% | operator |
| 18550 | Seq Scan | 13.86 | 7.19 | 48.1% | operator |
| 18545 | Seq Scan | 20.48 | 42.80 | 108.9% | operator |
| 18546 | Index Scan | 0.07 | 0.12 | 58.9% | operator |
| 18548 | Seq Scan | 3.10 | 10.62 | 241.9% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 18545 (Seq Scan) - LEAF
...
### Step 2: Node 18546 (Index Scan) - LEAF
...
### Step 3: Node 18548 (Seq Scan) - LEAF
...
### Step 4: Node 18550 (Seq Scan) - LEAF
...
### Step 5: Node 18543 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 18535, 18536, 18537, 18538, 18541, 18542, 18544, 18547
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=19372
  - HashJoin_nt1=19372
  ...
  - HashJoin_total_cost=15038.9400
  - Hash_Inner_np=0
  - Hash_Inner_nt=10000
...
- **Output:** st=5.42, rt=981.05

### Step 6: Node 18549 (Hash)
...
### Step 7: Node 18540 (Seq Scan) - LEAF
...
### Step 8: Node 18541 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 7df893ad (Hash -> Hash Join (Outer))
- **Consumes:** Nodes 18535, 18536, 18537, 18538, 18542, 18543, 18544, 18547
- **Input Features:**
  - HashJoin_Outer_np=0
  ...
  - Hash_sel=1.0000
  - Hash_startup_cost=15099.9700
  - Hash_total_cost=15099.9700
- **Output:** st=49.93, rt=49.93

### Step 9: Node 18539 (Hash Join)
...
- **Output:** st=168.44, rt=831.67

### Step 10: Node 18551 (Index Scan) - LEAF
...
### Step 11: Node 18535 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 0405d50f (Aggregate -> Gather -> Aggregate -> Nested Loop (Outer) (Outer) (Outer))
- **Consumes:** Nodes 18536, 18537, 18538, 18541, 18542, 18543, 18544, 18547
- **Input Features:**
  ...
  - NestedLoop_Outer_np=0
  - NestedLoop_Outer_nt=69752
  - NestedLoop_Outer_nt1=69752
  - NestedLoop_Outer_nt2=1
  - NestedLoop_Outer_parallel_workers=0
  - NestedLoop_Outer_plan_width=159
  - NestedLoop_Outer_reltuples=0.0000
  - NestedLoop_Outer_sel=1.0000
  - NestedLoop_Outer_startup_cost=15390.9800
  - NestedLoop_Outer_total_cost=180595.7300
- **Output:** st=1188.78, rt=1194.84

### Step 12: Node 18534 (Sort) - ROOT
