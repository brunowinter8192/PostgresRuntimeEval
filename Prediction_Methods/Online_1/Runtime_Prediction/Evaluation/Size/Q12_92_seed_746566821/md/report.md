...
```
Node 25307 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 25308 (Gather Merge) [consumed]
    Node 25309 (Aggregate)
      Node 25310 (Sort)
        Node 25311 (Nested Loop) [PATTERN: c53c4396]
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 25307 | 25308, 25311, 25312, 25313 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 25311 | 25307, 25308, 25312, 25313 |
...
|------|------|--------|-----------|-----|--------|
| 25307 | Aggregate | 1003.31 | 1027.05 | 2.4% | pattern |
| 25309 | Aggregate | 983.18 | 940.64 | 4.3% | operator |
| 25310 | Sort | 982.55 | 1050.80 | 6.9% | operator |
| 25311 | Nested Loop | 981.60 | 976.04 | 0.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 25311 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 25307, 25308, 25312, 25313
- **Input Features:**
  ...
  - NestedLoop_total_cost=147171.4400
...
### Step 4: Node 25307 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 25308, 25311, 25312, 25313
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
