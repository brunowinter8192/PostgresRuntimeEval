...
**Test Query:** Q10_83_seed_672730542
...
```
Node 21073 (Limit) [PATTERN: 04a01b61] - ROOT
  Node 21074 (Sort) [consumed]
    Node 21075 (Aggregate) [consumed]
      Node 21076 (Gather) [consumed]
        Node 21077 (Hash Join) [consumed]
          Node 21078 (Hash Join) [PATTERN: 2e0f44ef]
            Node 21079 (Nested Loop) [consumed]
              Node 21080 (Seq Scan) - LEAF
              Node 21081 (Index Scan) - LEAF
            Node 21082 (Hash) [consumed]
              Node 21083 (Seq Scan) - LEAF
          Node 21084 (Hash)
            Node 21085 (Seq Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 04a01b61 | 21073 | 21074, 21075, 21076, 21077, 21078, 21079, 21082 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 21078 | 21073, 21074, 21075, 21076, 21077, 21079, 21082 |
...
|------|------|--------|-----------|-----|--------|
| 21073 | Limit | 1146.44 | 1151.14 | 0.4% | pattern |
| 21078 | Hash Join | 1101.27 | 1101.61 | 0.0% | pattern |
| 21084 | Hash | 0.01 | 14.54 | 96831.4% | operator |
| 21085 | Seq Scan | 0.01 | 7.19 | 71844.7% | operator |
| 21080 | Seq Scan | 164.38 | 162.03 | 1.4% | operator |
| 21081 | Index Scan | 0.06 | -0.02 | 128.6% | operator |
| 21083 | Seq Scan | 23.86 | 29.69 | 24.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 21080 (Seq Scan) - LEAF
...
### Step 2: Node 21081 (Index Scan) - LEAF
...
### Step 3: Node 21083 (Seq Scan) - LEAF
...
### Step 4: Node 21085 (Seq Scan) - LEAF
...
### Step 5: Node 21078 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 21073, 21074, 21075, 21076, 21077, 21079, 21082
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=17761
  - HashJoin_nt1=17761
...
- **Output:** st=37.51, rt=1101.61

### Step 6: Node 21084 (Hash)
...
### Step 7: Node 21073 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 04a01b61 (Limit -> Sort -> Aggregate -> Gather -> Hash Join (Outer) (Outer) (Outer) (Outer))
- **Consumes:** Nodes 21074, 21075, 21076, 21077, 21078, 21079, 21082
- **Input Features:**
