...
```
Node 16145 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16146 (Gather Merge) [consumed]
    Node 16147 (Sort)
          ...
          Node 16150 (Hash Join)
            Node 16151 (Nested Loop)
              Node 16152 (Hash Join)
                ...
                Node 16154 (Hash) [PATTERN: 444761fb]
                  Node 16155 (Hash Join) [consumed]
                    Node 16156 (Seq Scan) [consumed] - LEAF
                    Node 16157 (Hash) [consumed]
                      Node 16158 (Hash Join) [consumed]
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16154 | 16145, 16146, 16155, 16156, 16157, 16158 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16145 | 16146, 16154, 16155, 16156, 16157, 16158 |
...
|------|------|--------|-----------|-----|--------|
| 16145 | Aggregate | 1199.43 | 1180.60 | 1.6% | pattern |
...
| 16150 | Hash Join | 1189.13 | 1027.32 | 13.6% | operator |
...
| 16152 | Hash Join | 208.37 | 226.59 | 8.7% | operator |
...
| 16154 | Hash | 33.26 | 36.08 | 8.5% | pattern |
...
### Step 4: Node 16153 (Seq Scan) - LEAF
...
### Step 5: Node 16154 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16145, 16146, 16155, 16156, 16157, 16158
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16162 (Index Scan) - LEAF
...
### Step 8: Node 16164 (Seq Scan) - LEAF
...
### Step 12: Node 16165 (Index Scan) - LEAF
...
### Step 13: Node 16167 (Seq Scan) - LEAF
...
### Step 14: Node 16149 (Nested Loop)
...
### Step 18: Node 16145 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16146, 16154, 16155, 16156, 16157, 16158
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2383
  - Aggregate_nt1=2383
  ...
  - Aggregate_startup_cost=71546.8600
  - Aggregate_total_cost=71924.1100
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2383
  - GatherMerge_Outer_nt1=769
  ...
  - GatherMerge_Outer_sel=3.0988
  - GatherMerge_Outer_startup_cost=71546.8600
  - GatherMerge_Outer_total_cost=71828.7900
