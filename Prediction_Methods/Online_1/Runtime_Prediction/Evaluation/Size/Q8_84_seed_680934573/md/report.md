...
```
Node 16237 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 16238 (Gather Merge) [consumed]
    Node 16239 (Sort)
          ...
          Node 16242 (Hash Join)
            Node 16243 (Nested Loop)
              Node 16244 (Hash Join)
                ...
                Node 16246 (Hash) [PATTERN: 444761fb]
                  Node 16247 (Hash Join) [consumed]
                    Node 16248 (Seq Scan) [consumed] - LEAF
                    Node 16249 (Hash) [consumed]
                      Node 16250 (Hash Join) [consumed]
...
|---------|------|-----------|----------------|
| Hash -> Hash Join -> [Seq Scan | 444761fb | 16246 | 16237, 16238, 16247, 16248, 16249, 16250 |
| Aggregate -> Gather Merge (Out | 2724c080 | 16237 | 16238, 16246, 16247, 16248, 16249, 16250 |
...
|------|------|--------|-----------|-----|--------|
| 16237 | Aggregate | 1197.68 | 1172.02 | 2.1% | pattern |
| 16239 | Sort | 1191.23 | 1087.88 | 8.7% | operator |
| 16240 | Hash Join | 1190.96 | 1055.49 | 11.4% | operator |
| 16241 | Nested Loop | 1190.62 | 1105.66 | 7.1% | operator |
| 16258 | Hash | 0.02 | 14.54 | 63116.2% | operator |
| 16242 | Hash Join | 1187.04 | 1027.33 | 13.5% | operator |
...
| 16244 | Hash Join | 207.51 | 226.59 | 9.2% | operator |
...
| 16246 | Hash | 32.07 | 36.08 | 12.5% | pattern |
...
### Step 4: Node 16245 (Seq Scan) - LEAF
...
### Step 5: Node 16246 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 444761fb (Hash -> Hash Join -> [Seq Scan (Outer), Hash -> Hash Join (Outer) (Inner)] (Outer))
- **Consumes:** Nodes 16237, 16238, 16247, 16248, 16249, 16250
- **Input Features:**
  - HashJoin_Outer_np=0
  - HashJoin_Outer_nt=5
...
### Step 7: Node 16254 (Index Scan) - LEAF
...
### Step 8: Node 16256 (Seq Scan) - LEAF
...
### Step 12: Node 16257 (Index Scan) - LEAF
...
### Step 13: Node 16259 (Seq Scan) - LEAF
...
### Step 14: Node 16241 (Nested Loop)
...
### Step 18: Node 16237 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 16238, 16246, 16247, 16248, 16249, 16250
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=2406
  - Aggregate_nt1=2554
  ...
  - Aggregate_sel=0.9421
  - Aggregate_startup_cost=71567.3400
  - Aggregate_total_cost=71968.6900
  - GatherMerge_Outer_np=0
  - GatherMerge_Outer_nt=2554
  - GatherMerge_Outer_nt1=824
  ...
  - GatherMerge_Outer_sel=3.0995
  - GatherMerge_Outer_startup_cost=71567.3400
  - GatherMerge_Outer_total_cost=71869.4900
- **Output:** st=1169.55, rt=1172.02
