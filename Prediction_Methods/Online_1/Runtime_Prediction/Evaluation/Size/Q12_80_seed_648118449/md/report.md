...
```
Node 25216 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 25217 (Gather Merge) [consumed]
    Node 25218 (Aggregate)
      Node 25219 (Sort)
        Node 25220 (Nested Loop) [PATTERN: c53c4396]
          Node 25221 (Seq Scan) [consumed] - LEAF
          Node 25222 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 25216 | 25217, 25220, 25221, 25222 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 25220 | 25216, 25217, 25221, 25222 |
...
|------|------|--------|-----------|-----|--------|
| 25216 | Aggregate | 993.21 | 1027.05 | 3.4% | pattern |
| 25218 | Aggregate | 973.75 | 940.64 | 3.4% | operator |
| 25219 | Sort | 973.15 | 1050.80 | 8.0% | operator |
| 25220 | Nested Loop | 972.19 | 976.05 | 0.4% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 25220 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 25216, 25217, 25221, 25222
- **Input Features:**
  ...
  - NestedLoop_total_cost=147175.9100
...
### Step 4: Node 25216 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 25217, 25220, 25221, 25222
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
  ...
  - Aggregate_startup_cost=148537.0100
  - Aggregate_total_cost=148657.1900
  ...
  - GatherMerge_Outer_startup_cost=148537.0100
  - GatherMerge_Outer_total_cost=148656.8500
