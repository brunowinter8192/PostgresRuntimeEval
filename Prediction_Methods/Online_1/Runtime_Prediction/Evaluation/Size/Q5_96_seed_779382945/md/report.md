...
```
Node 9801 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 9802 (Aggregate) [consumed]
    Node 9803 (Gather Merge)
      Node 9804 (Aggregate)
        Node 9805 (Sort)
          Node 9806 (Hash Join) [PATTERN: 2e0f44ef]
            Node 9807 (Nested Loop) [consumed]
              Node 9808 (Hash Join) [PATTERN: 895c6e8c]
                Node 9809 (Seq Scan) [consumed] - LEAF
                Node 9810 (Hash) [consumed]
                  Node 9811 (Hash Join) [PATTERN: 895c6e8c]
                    Node 9812 (Seq Scan) [consumed] - LEAF
                    Node 9813 (Hash) [consumed]
                      Node 9814 (Hash Join) [PATTERN: f4cb205a]
                        Node 9815 (Seq Scan) [consumed] - LEAF
                        Node 9816 (Hash) [consumed]
                          Node 9817 (Seq Scan) [consumed] - LEAF
              Node 9818 (Index Scan) - LEAF
            Node 9819 (Hash) [consumed]
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 9814 | 9801, 9802, 9806, 9807, 9808, 9809, 9810, 9811, 9812, 9813, 9815, 9816, 9817, 9819 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 9808 | 9801, 9802, 9806, 9807, 9809, 9810, 9811, 9812, 9813, 9814, 9815, 9816, 9817, 9819 |
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 9811 | 9801, 9802, 9806, 9807, 9808, 9809, 9810, 9812, 9813, 9814, 9815, 9816, 9817, 9819 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 9801 | 9802, 9806, 9807, 9808, 9809, 9810, 9811, 9812, 9813, 9814, 9815, 9816, 9817, 9819 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 9806 | 9801, 9802, 9807, 9808, 9809, 9810, 9811, 9812, 9813, 9814, 9815, 9816, 9817, 9819 |
...
|------|------|--------|-----------|-----|--------|
| 9801 | Sort | 1089.04 | 1080.12 | 0.8% | pattern |
| 9803 | Gather Merge | 1089.02 | 1124.38 | 3.2% | operator |
| 9804 | Aggregate | 1083.57 | 1019.77 | 5.9% | operator |
| 9805 | Sort | 1083.32 | 1073.09 | 0.9% | operator |
| 9806 | Hash Join | 1082.83 | 1080.06 | 0.3% | pattern |
| 9808 | Hash Join | 223.58 | 259.85 | 16.2% | pattern |
| 9818 | Index Scan | 0.07 | -0.03 | 133.9% | operator |
| 9820 | Seq Scan | 1.93 | 10.62 | 450.1% | operator |
| 9811 | Hash Join | 37.61 | 84.97 | 125.9% | pattern |
| 9814 | Hash Join | 0.07 | 11.62 | 16033.1% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 9814 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 9801, 9802, 9806, 9807, 9808, 9809, 9810, 9811, 9812, 9813, 9815, 9816, 9817, 9819
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=5
  - HashJoin_nt1=25
...
### Step 2: Node 9811 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 9801, 9802, 9806, 9807, 9808, 9809, 9810, 9812, 9813, 9814, 9815, 9816, 9817, 9819
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=12500
  - HashJoin_nt1=62500
...
### Step 3: Node 9808 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 9801, 9802, 9806, 9807, 9809, 9810, 9811, 9812, 9813, 9814, 9815, 9816, 9817, 9819
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=14612
  - HashJoin_nt1=73059
...
### Step 4: Node 9818 (Index Scan) - LEAF
...
### Step 5: Node 9820 (Seq Scan) - LEAF
...
### Step 6: Node 9806 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 9801, 9802, 9807, 9808, 9809, 9810, 9811, 9812, 9813, 9814, 9815, 9816, 9817, 9819
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=2338
  - HashJoin_nt1=58459
...
### Step 10: Node 9801 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 9802, 9806, 9807, 9808, 9809, 9810, 9811, 9812, 9813, 9814, 9815, 9816, 9817, 9819
- **Input Features:**
