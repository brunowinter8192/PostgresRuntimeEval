...
```
Node 24446 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 24447 (Gather Merge) [consumed]
    Node 24448 (Aggregate)
      Node 24449 (Sort)
        Node 24450 (Nested Loop) [PATTERN: c53c4396]
          Node 24451 (Seq Scan) [consumed] - LEAF
          Node 24452 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 24446 | 24447, 24450, 24451, 24452 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 24450 | 24446, 24447, 24451, 24452 |
...
|------|------|--------|-----------|-----|--------|
| 24446 | Aggregate | 984.71 | 1027.05 | 4.3% | pattern |
| 24448 | Aggregate | 965.69 | 940.64 | 2.6% | operator |
| 24449 | Sort | 965.07 | 1050.81 | 8.9% | operator |
| 24450 | Nested Loop | 964.09 | 976.09 | 1.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 24450 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 24446, 24447, 24451, 24452
- **Input Features:**
  ...
  - NestedLoop_total_cost=147195.3000
...
### Step 4: Node 24446 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 24447, 24450, 24451, 24452
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
  ...
  - Aggregate_startup_cost=148557.9300
  - Aggregate_total_cost=148678.5500
