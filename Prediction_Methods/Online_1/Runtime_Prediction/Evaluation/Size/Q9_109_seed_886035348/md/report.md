...
Node 16814 (Sort) - ROOT
  Node 16815 (Aggregate) [PATTERN: 0405d50f]
      ...
      Node 16817 (Aggregate) [consumed]
        Node 16818 (Nested Loop) [consumed]
          Node 16819 (Hash Join)
            ...
            Node 16821 (Hash) [PATTERN: 7df893ad]
              Node 16822 (Hash Join) [consumed]
                Node 16823 (Hash Join) [PATTERN: 2e0f44ef]
...
|---------|------|-----------|----------------|
| Aggregate -> Gather -> Aggrega | 0405d50f | 16815 | 16816, 16817, 16818, 16821, 16822, 16823, 16824, 16827 |
| Hash -> Hash Join (Outer) | 7df893ad | 16821 | 16815, 16816, 16817, 16818, 16822, 16823, 16824, 16827 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 16823 | 16815, 16816, 16817, 16818, 16821, 16822, 16824, 16827 |
...
|------|------|--------|-----------|-----|--------|
| 16814 | Sort | 1253.92 | 1157.00 | 7.7% | operator |
| 16815 | Aggregate | 1253.78 | 1194.97 | 4.7% | pattern |
| 16819 | Hash Join | 1039.55 | 830.73 | 20.1% | operator |
| 16831 | Index Scan | 0.00 | -0.04 | 1512.7% | operator |
| 16820 | Seq Scan | 774.17 | 565.06 | 27.0% | operator |
| 16821 | Hash | 173.63 | 46.09 | 73.5% | pattern |
| 16823 | Hash Join | 156.93 | 982.48 | 526.1% | pattern |
...
### Step 4: Node 16830 (Seq Scan) - LEAF
...
### Step 5: Node 16823 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 16815, 16816, 16817, 16818, 16821, 16822, 16824, 16827
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=14697
  - HashJoin_nt1=14697
...
### Step 7: Node 16820 (Seq Scan) - LEAF
...
### Step 8: Node 16821 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 7df893ad (Hash -> Hash Join (Outer))
- **Consumes:** Nodes 16815, 16816, 16817, 16818, 16822, 16823, 16824, 16827
- **Input Features:**
  - HashJoin_Outer_np=0
  ...
  - Hash_sel=1.0000
  - Hash_startup_cost=14013.3900
  - Hash_total_cost=14013.3900
- **Output:** st=46.09, rt=46.09

### Step 9: Node 16819 (Hash Join)
...
- **Output:** st=168.94, rt=830.73

### Step 10: Node 16831 (Index Scan) - LEAF
...
### Step 11: Node 16815 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 0405d50f (Aggregate -> Gather -> Aggregate -> Nested Loop (Outer) (Outer) (Outer))
- **Consumes:** Nodes 16816, 16817, 16818, 16821, 16822, 16823, 16824, 16827
- **Input Features:**
  ...
  - NestedLoop_Outer_np=0
  - NestedLoop_Outer_nt=52919
  - NestedLoop_Outer_nt1=52919
  - NestedLoop_Outer_nt2=1
  - NestedLoop_Outer_parallel_workers=0
  - NestedLoop_Outer_plan_width=159
  - NestedLoop_Outer_reltuples=0.0000
  - NestedLoop_Outer_sel=1.0000
  - NestedLoop_Outer_startup_cost=14234.2700
  - NestedLoop_Outer_total_cost=171812.8800
- **Output:** st=1188.90, rt=1194.97

### Step 12: Node 16814 (Sort) - ROOT
