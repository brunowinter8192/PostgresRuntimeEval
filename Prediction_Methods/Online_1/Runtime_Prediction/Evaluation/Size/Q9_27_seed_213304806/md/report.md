...
**Test Query:** Q9_27_seed_213304806
...
```
Node 17901 (Sort) - ROOT
  Node 17902 (Aggregate) [PATTERN: 0405d50f]
    Node 17903 (Gather) [consumed]
      Node 17904 (Aggregate) [consumed]
        Node 17905 (Nested Loop) [consumed]
          Node 17906 (Hash Join)
            Node 17907 (Seq Scan) - LEAF
            Node 17908 (Hash) [PATTERN: 7df893ad]
              Node 17909 (Hash Join) [consumed]
                Node 17910 (Hash Join) [PATTERN: 2e0f44ef]
                  Node 17911 (Nested Loop) [consumed]
                    Node 17912 (Seq Scan) - LEAF
                    Node 17913 (Index Scan) - LEAF
                  Node 17914 (Hash) [consumed]
                    Node 17915 (Seq Scan) - LEAF
                Node 17916 (Hash)
                  Node 17917 (Seq Scan) - LEAF
          Node 17918 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather -> Aggrega | 0405d50f | 17902 | 17903, 17904, 17905, 17908, 17909, 17910, 17911, 17914 |
| Hash -> Hash Join (Outer) | 7df893ad | 17908 | 17902, 17903, 17904, 17905, 17909, 17910, 17911, 17914 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 17910 | 17902, 17903, 17904, 17905, 17908, 17909, 17911, 17914 |
...
|------|------|--------|-----------|-----|--------|
| 17901 | Sort | 1183.07 | 1159.89 | 2.0% | operator |
| 17902 | Aggregate | 1182.95 | 1194.84 | 1.0% | pattern |
| 17906 | Hash Join | 968.30 | 831.67 | 14.1% | operator |
| 17918 | Index Scan | 0.00 | -0.04 | 1512.7% | operator |
| 17907 | Seq Scan | 697.32 | 565.06 | 19.0% | operator |
| 17908 | Hash | 175.36 | 49.93 | 71.5% | pattern |
| 17910 | Hash Join | 159.52 | 981.05 | 515.0% | pattern |
| 17916 | Hash | 13.30 | 14.54 | 9.3% | operator |
| 17917 | Seq Scan | 13.29 | 7.19 | 45.8% | operator |
| 17912 | Seq Scan | 19.83 | 42.80 | 115.8% | operator |
| 17913 | Index Scan | 0.07 | 0.12 | 65.5% | operator |
| 17915 | Seq Scan | 4.06 | 10.62 | 161.4% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 17912 (Seq Scan) - LEAF
...
### Step 2: Node 17913 (Index Scan) - LEAF
...
### Step 3: Node 17915 (Seq Scan) - LEAF
...
### Step 4: Node 17917 (Seq Scan) - LEAF
...
### Step 5: Node 17910 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 17902, 17903, 17904, 17905, 17908, 17909, 17911, 17914
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=19372
  - HashJoin_nt1=19372
...
### Step 7: Node 17907 (Seq Scan) - LEAF
...
### Step 8: Node 17908 (Hash) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 7df893ad (Hash -> Hash Join (Outer))
- **Consumes:** Nodes 17902, 17903, 17904, 17905, 17909, 17910, 17911, 17914
- **Input Features:**
  - HashJoin_Outer_np=0
...
### Step 10: Node 17918 (Index Scan) - LEAF
...
### Step 11: Node 17902 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 0405d50f (Aggregate -> Gather -> Aggregate -> Nested Loop (Outer) (Outer) (Outer))
- **Consumes:** Nodes 17903, 17904, 17905, 17908, 17909, 17910, 17911, 17914
- **Input Features:**
