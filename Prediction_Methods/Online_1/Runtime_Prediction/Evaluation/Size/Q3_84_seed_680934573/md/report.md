    ...
    Node 5673 (Aggregate) [consumed]
      Node 5674 (Gather) [consumed]
        Node 5675 (Nested Loop)
          Node 5676 (Hash Join) [PATTERN: f4cb205a]
            Node 5677 (Seq Scan) [consumed] - LEAF
            Node 5678 (Hash) [consumed]
              Node 5679 (Seq Scan) [consumed] - LEAF
...
|---------|------|-----------|----------------|
| Limit -> Sort -> Aggregate ->  | 25df29b5 | 5671 | 5672, 5673, 5674, 5676, 5677, 5678, 5679 |
| Hash Join -> [Seq Scan (Outer) | f4cb205a | 5676 | 5671, 5672, 5673, 5674, 5677, 5678, 5679 |
...
| 5675 | Nested Loop | 1182.09 | 1122.04 | 5.1% | operator |
| 5676 | Hash Join | 213.04 | 219.40 | 3.0% | pattern |
| 5680 | Index Scan | 0.03 | -0.02 | 168.7% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 5676 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** f4cb205a (Hash Join -> [Seq Scan (Outer), Hash -> Seq Scan (Outer) (Inner)])
- **Consumes:** Nodes 5671, 5672, 5673, 5674, 5677, 5678, 5679
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=47011
  - HashJoin_nt1=233583
...
- **Output:** st=30.35, rt=219.40

### Step 2: Node 5680 (Index Scan) - LEAF
...
- **Output:** st=30.37, rt=1122.04

### Step 4: Node 5671 (Limit) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 25df29b5 (Limit -> Sort -> Aggregate -> Gather (Outer) (Outer) (Outer))
- **Consumes:** Nodes 5672, 5673, 5674, 5676, 5677, 5678, 5679
- **Input Features:**
