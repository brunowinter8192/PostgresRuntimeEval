...
```
Node 25090 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 25091 (Gather Merge) [consumed]
    Node 25092 (Aggregate)
      Node 25093 (Sort)
        Node 25094 (Nested Loop) [PATTERN: c53c4396]
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 25090 | 25091, 25094, 25095, 25096 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 25094 | 25090, 25091, 25095, 25096 |
...
|------|------|--------|-----------|-----|--------|
| 25090 | Aggregate | 995.69 | 1027.05 | 3.1% | pattern |
| 25092 | Aggregate | 974.52 | 940.64 | 3.5% | operator |
| 25093 | Sort | 973.91 | 1050.81 | 7.9% | operator |
| 25094 | Nested Loop | 972.75 | 976.07 | 0.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 25094 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 25090, 25091, 25095, 25096
- **Input Features:**
...
### Step 4: Node 25090 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 25091, 25094, 25095, 25096
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
