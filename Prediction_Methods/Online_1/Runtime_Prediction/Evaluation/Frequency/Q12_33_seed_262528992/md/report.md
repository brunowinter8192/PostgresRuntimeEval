...
```
Node 24852 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 24853 (Gather Merge) [consumed]
    Node 24854 (Aggregate)
      Node 24855 (Sort)
        Node 24856 (Nested Loop) [PATTERN: c53c4396]
          Node 24857 (Seq Scan) [consumed] - LEAF
          Node 24858 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 24852 | 24853, 24856, 24857, 24858 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 24856 | 24852, 24853, 24857, 24858 |
...
|------|------|--------|-----------|-----|--------|
| 24852 | Aggregate | 989.96 | 1027.05 | 3.7% | pattern |
| 24854 | Aggregate | 969.71 | 940.64 | 3.0% | operator |
| 24855 | Sort | 969.10 | 1050.79 | 8.4% | operator |
| 24856 | Nested Loop | 968.16 | 975.98 | 0.8% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 24856 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 24852, 24853, 24857, 24858
- **Input Features:**
  ...
  - NestedLoop_total_cost=147137.0200
  - SeqScan_Outer_np=112600
  - SeqScan_Outer_nt=5735
  - SeqScan_Outer_nt1=0
  - SeqScan_Outer_nt2=0
...
- **Output:** st=10.44, rt=975.98

### Step 2: Node 24855 (Sort)
...
### Step 4: Node 24852 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 24853, 24856, 24857, 24858
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
  ...
  - Aggregate_startup_cost=148495.1200
  - Aggregate_total_cost=148614.4400
  ...
  - GatherMerge_Outer_startup_cost=148495.1200
  - GatherMerge_Outer_total_cost=148614.1100
