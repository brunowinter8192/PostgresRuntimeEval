...
```
Node 24383 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 24384 (Gather Merge) [consumed]
    Node 24385 (Aggregate)
      Node 24386 (Sort)
        Node 24387 (Nested Loop) [PATTERN: c53c4396]
          Node 24388 (Seq Scan) [consumed] - LEAF
          Node 24389 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 24383 | 24384, 24387, 24388, 24389 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 24387 | 24383, 24384, 24388, 24389 |
...
|------|------|--------|-----------|-----|--------|
| 24383 | Aggregate | 987.99 | 1027.05 | 4.0% | pattern |
| 24385 | Aggregate | 968.21 | 940.64 | 2.8% | operator |
| 24386 | Sort | 967.57 | 1050.81 | 8.6% | operator |
| 24387 | Nested Loop | 966.61 | 976.06 | 1.0% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 24387 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 24383, 24384, 24388, 24389
- **Input Features:**
...
### Step 4: Node 24383 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 24384, 24387, 24388, 24389
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
