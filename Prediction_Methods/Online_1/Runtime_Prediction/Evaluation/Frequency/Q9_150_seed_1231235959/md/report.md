...
**Test Query:** Q9_150_seed_1231235959
...
```
Node 17647 (Sort) [PATTERN: 1d35fb97] - ROOT
  Node 17648 (Aggregate) [consumed]
    Node 17649 (Gather)
      Node 17650 (Aggregate)
        Node 17651 (Nested Loop)
          Node 17652 (Hash Join) [PATTERN: 895c6e8c]
            Node 17653 (Seq Scan) [consumed] - LEAF
            Node 17654 (Hash) [consumed]
              Node 17655 (Hash Join)
                Node 17656 (Hash Join) [PATTERN: 2e0f44ef]
                  Node 17657 (Nested Loop) [consumed]
                    Node 17658 (Seq Scan) - LEAF
                    Node 17659 (Index Scan) - LEAF
                  Node 17660 (Hash) [consumed]
                    Node 17661 (Seq Scan) - LEAF
                Node 17662 (Hash)
                  Node 17663 (Seq Scan) - LEAF
          Node 17664 (Index Scan) - LEAF
```
...
|---------|------|-----------|----------------|
| Hash Join -> [Seq Scan (Outer) | 895c6e8c | 17652 | 17647, 17648, 17653, 17654, 17656, 17657, 17660 |
| Sort -> Aggregate (Outer) | 1d35fb97 | 17647 | 17648, 17652, 17653, 17654, 17656, 17657, 17660 |
| Hash Join -> [Nested Loop (Out | 2e0f44ef | 17656 | 17647, 17648, 17652, 17653, 17654, 17657, 17660 |
...
|------|------|--------|-----------|-----|--------|
| 17647 | Sort | 1160.90 | 1114.80 | 4.0% | pattern |
| 17649 | Gather | 1159.51 | 1136.42 | 2.0% | operator |
| 17650 | Aggregate | 1141.34 | 1170.90 | 2.6% | operator |
| 17651 | Nested Loop | 1116.94 | 1176.25 | 5.3% | operator |
| 17652 | Hash Join | 949.90 | 733.48 | 22.8% | pattern |
| 17664 | Index Scan | 0.00 | -0.04 | 1512.7% | operator |
| 17655 | Hash Join | 175.51 | 821.50 | 368.1% | operator |
| 17656 | Hash Join | 161.77 | 981.58 | 506.8% | pattern |
| 17662 | Hash | 13.12 | 14.54 | 10.8% | operator |
| 17663 | Seq Scan | 13.11 | 7.19 | 45.1% | operator |
| 17658 | Seq Scan | 20.30 | 43.05 | 112.1% | operator |
| 17659 | Index Scan | 0.08 | 0.12 | 56.8% | operator |
| 17661 | Seq Scan | 3.36 | 10.62 | 215.8% | operator |

## Prediction Chain (Bottom-Up)

### Step 1: Node 17658 (Seq Scan) - LEAF
...
### Step 2: Node 17659 (Index Scan) - LEAF
...
### Step 3: Node 17661 (Seq Scan) - LEAF
...
### Step 4: Node 17663 (Seq Scan) - LEAF
...
### Step 5: Node 17656 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2e0f44ef (Hash Join -> [Nested Loop (Outer), Hash (Inner)])
- **Consumes:** Nodes 17647, 17648, 17652, 17653, 17654, 17657, 17660
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=16700
  - HashJoin_nt1=16700
  ...
  - NestedLoop_Outer_nt=16700
  - NestedLoop_Outer_nt1=4175
...
- **Output:** st=5.69, rt=981.58

### Step 6: Node 17662 (Hash)
...
- **Output:** st=26.36, rt=821.50

### Step 8: Node 17652 (Hash Join) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 895c6e8c (Hash Join -> [Seq Scan (Outer), Hash (Inner)])
- **Consumes:** Nodes 17647, 17648, 17653, 17654, 17656, 17657, 17660
- **Input Features:**
  - HashJoin_np=0
  - HashJoin_nt=60132
  ...
  - HashJoin_reltuples=0.0000
  - HashJoin_sel=0.0000
  - HashJoin_startup_cost=14736.2000
  - HashJoin_total_cost=148340.5600
  - Hash_Inner_np=0
  - Hash_Inner_nt=16700
  - Hash_Inner_nt1=16700
  - Hash_Inner_nt2=0
  - Hash_Inner_parallel_workers=0
  - Hash_Inner_plan_width=126
  - Hash_Inner_reltuples=0.0000
  - Hash_Inner_sel=1.0000
  - Hash_Inner_startup_cost=14485.7000
  - Hash_Inner_total_cost=14485.7000
...
- **Output:** st=176.24, rt=733.48

### Step 9: Node 17664 (Index Scan) - LEAF
...
### Step 10: Node 17651 (Nested Loop)
...
- **Output:** st=119.52, rt=1176.25

### Step 11: Node 17650 (Aggregate)
...
- **Output:** st=1159.86, rt=1170.90

### Step 12: Node 17649 (Gather)
...
- **Output:** st=577.76, rt=1136.42

### Step 13: Node 17647 (Sort) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 1d35fb97 (Sort -> Aggregate (Outer))
- **Consumes:** Nodes 17648, 17652, 17653, 17654, 17656, 17657, 17660
- **Input Features:**
