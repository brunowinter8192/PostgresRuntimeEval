...
**Test Query:** Q12_71_seed_574282170
**Timestamp:** 2026-01-01 18:59:30
...
```
Node 25146 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 25147 (Gather Merge) [consumed]
    Node 25148 (Aggregate)
      Node 25149 (Sort)
        Node 25150 (Nested Loop) [PATTERN: c53c4396]
          Node 25151 (Seq Scan) [consumed] - LEAF
          Node 25152 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 25146 | 25147, 25150, 25151, 25152 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 25150 | 25146, 25147, 25151, 25152 |
...
|------|------|--------|-----------|-----|--------|
| 25146 | Aggregate | 988.96 | 1027.05 | 3.9% | pattern |
| 25148 | Aggregate | 969.57 | 940.64 | 3.0% | operator |
| 25149 | Sort | 968.94 | 1050.81 | 8.4% | operator |
| 25150 | Nested Loop | 967.93 | 976.08 | 0.8% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 25150 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 25146, 25147, 25151, 25152
- **Input Features:**
  ...
  - NestedLoop_total_cost=147187.3400
  - SeqScan_Outer_np=112600
  - SeqScan_Outer_nt=5791
  - SeqScan_Outer_nt1=0
  - SeqScan_Outer_nt2=0
...
- **Output:** st=10.43, rt=976.08

### Step 2: Node 25149 (Sort)
...
- **Input Features:**
  ...
  - st2=0.0000
  - startup_cost=147549.2600
  - total_cost=147665.1500
- **Output:** st=925.96, rt=940.64

### Step 4: Node 25146 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 25147, 25150, 25151, 25152
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
