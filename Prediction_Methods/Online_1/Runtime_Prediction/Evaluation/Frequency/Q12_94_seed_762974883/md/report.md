...
**Test Query:** Q12_94_seed_762974883
**Timestamp:** 2026-01-01 19:00:05
...
```
Node 25321 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 25322 (Gather Merge) [consumed]
    Node 25323 (Aggregate)
      Node 25324 (Sort)
        Node 25325 (Nested Loop) [PATTERN: c53c4396]
          Node 25326 (Seq Scan) [consumed] - LEAF
          Node 25327 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 25321 | 25322, 25325, 25326, 25327 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 25325 | 25321, 25322, 25326, 25327 |
...
|------|------|--------|-----------|-----|--------|
| 25321 | Aggregate | 984.33 | 1027.05 | 4.3% | pattern |
| 25323 | Aggregate | 965.39 | 940.64 | 2.6% | operator |
| 25324 | Sort | 964.79 | 1050.80 | 8.9% | operator |
| 25325 | Nested Loop | 963.91 | 976.06 | 1.3% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 25325 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 25321, 25322, 25326, 25327
- **Input Features:**
  ...
  - NestedLoop_total_cost=147177.5500
  - SeqScan_Outer_np=112600
  - SeqScan_Outer_nt=5780
  - SeqScan_Outer_nt1=0
  - SeqScan_Outer_nt2=0
...
### Step 4: Node 25321 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 25322, 25325, 25326, 25327
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
