...
**Test Query:** Q12_118_seed_959871627
...
```
Node 24453 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 24454 (Gather Merge) [consumed]
    Node 24455 (Aggregate)
      Node 24456 (Sort)
        Node 24457 (Nested Loop) [PATTERN: c53c4396]
          Node 24458 (Seq Scan) [consumed] - LEAF
          Node 24459 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 24453 | 24454, 24457, 24458, 24459 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 24457 | 24453, 24454, 24458, 24459 |
...
|------|------|--------|-----------|-----|--------|
| 24453 | Aggregate | 995.35 | 1027.05 | 3.2% | pattern |
| 24455 | Aggregate | 976.08 | 940.64 | 3.6% | operator |
| 24456 | Sort | 975.44 | 1050.80 | 7.7% | operator |
| 24457 | Nested Loop | 974.40 | 976.06 | 0.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 24457 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 24453, 24454, 24458, 24459
- **Input Features:**
...
### Step 4: Node 24453 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 24454, 24457, 24458, 24459
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
