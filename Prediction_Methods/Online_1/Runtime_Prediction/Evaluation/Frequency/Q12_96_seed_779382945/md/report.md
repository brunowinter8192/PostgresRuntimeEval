...
Node 25335 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 25336 (Gather Merge) [consumed]
    Node 25337 (Aggregate)
      Node 25338 (Sort)
        Node 25339 (Nested Loop) [PATTERN: c53c4396]
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 25335 | 25336, 25339, 25340, 25341 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 25339 | 25335, 25336, 25340, 25341 |
...
| 25337 | Aggregate | 989.70 | 940.64 | 5.0% | operator |
| 25338 | Sort | 989.08 | 1050.81 | 6.2% | operator |
| 25339 | Nested Loop | 988.10 | 976.09 | 1.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 25339 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 25335, 25336, 25340, 25341
- **Input Features:**
...
### Step 4: Node 25335 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 25336, 25339, 25340, 25341
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
