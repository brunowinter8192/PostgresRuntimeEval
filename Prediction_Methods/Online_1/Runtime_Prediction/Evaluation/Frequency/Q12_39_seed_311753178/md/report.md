...
Node 24894 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 24895 (Gather Merge) [consumed]
    Node 24896 (Aggregate)
      Node 24897 (Sort)
        Node 24898 (Nested Loop) [PATTERN: c53c4396]
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 24894 | 24895, 24898, 24899, 24900 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 24898 | 24894, 24895, 24899, 24900 |
...
| 24896 | Aggregate | 972.30 | 940.64 | 3.3% | operator |
| 24897 | Sort | 971.63 | 1050.81 | 8.1% | operator |
| 24898 | Nested Loop | 970.49 | 976.09 | 0.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 24898 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 24894, 24895, 24899, 24900
- **Input Features:**
...
### Step 4: Node 24894 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 24895, 24898, 24899, 24900
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
