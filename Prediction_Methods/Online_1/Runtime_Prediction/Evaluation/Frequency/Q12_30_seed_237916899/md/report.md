...
```
Node 24831 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 24832 (Gather Merge) [consumed]
    Node 24833 (Aggregate)
      Node 24834 (Sort)
        Node 24835 (Nested Loop) [PATTERN: c53c4396]
          Node 24836 (Seq Scan) [consumed] - LEAF
          Node 24837 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 24831 | 24832, 24835, 24836, 24837 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 24835 | 24831, 24832, 24836, 24837 |
...
|------|------|--------|-----------|-----|--------|
| 24831 | Aggregate | 1001.15 | 1027.05 | 2.6% | pattern |
| 24833 | Aggregate | 983.24 | 940.63 | 4.3% | operator |
| 24834 | Sort | 982.63 | 1050.82 | 6.9% | operator |
| 24835 | Nested Loop | 981.64 | 976.16 | 0.6% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 24835 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 24831, 24832, 24836, 24837
- **Input Features:**
...
- **Output:** st=10.43, rt=976.16

### Step 2: Node 24834 (Sort)
...
- **Output:** st=1049.84, rt=1050.82

### Step 3: Node 24833 (Aggregate)
...
- **Output:** st=925.94, rt=940.63

### Step 4: Node 24831 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 24832, 24835, 24836, 24837
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
  ...
  - Aggregate_total_cost=148718.4500
  ...
  - GatherMerge_Outer_total_cost=148718.1200
