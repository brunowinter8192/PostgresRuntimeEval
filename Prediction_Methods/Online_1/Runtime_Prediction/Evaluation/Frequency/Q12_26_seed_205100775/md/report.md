...
**Test Query:** Q12_26_seed_205100775
...
```
Node 24796 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 24797 (Gather Merge) [consumed]
    Node 24798 (Aggregate)
      Node 24799 (Sort)
        Node 24800 (Nested Loop) [PATTERN: c53c4396]
          Node 24801 (Seq Scan) [consumed] - LEAF
          Node 24802 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 24796 | 24797, 24800, 24801, 24802 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 24800 | 24796, 24797, 24801, 24802 |
...
|------|------|--------|-----------|-----|--------|
| 24796 | Aggregate | 998.45 | 1027.05 | 2.9% | pattern |
| 24798 | Aggregate | 979.10 | 940.64 | 3.9% | operator |
| 24799 | Sort | 978.38 | 1050.80 | 7.4% | operator |
| 24800 | Nested Loop | 977.41 | 976.05 | 0.1% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 24800 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 24796, 24797, 24801, 24802
- **Input Features:**
  ...
  - NestedLoop_total_cost=147172.9600
  - SeqScan_Outer_np=112600
  - SeqScan_Outer_nt=5775
  - SeqScan_Outer_nt1=0
  - SeqScan_Outer_nt2=0
...
- **Input Features:**
  ...
  - st1=10.4335
  - st2=0.0000
  - startup_cost=147533.7700
  - total_cost=147548.2000
- **Output:** st=1049.82, rt=1050.80

### Step 3: Node 24798 (Aggregate)
...
- **Input Features:**
  ...
  - st2=0.0000
  - startup_cost=147533.7700
  - total_cost=147649.3400
- **Output:** st=925.96, rt=940.64

### Step 4: Node 24796 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 24797, 24800, 24801, 24802
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
  ...
  - Aggregate_startup_cost=148533.8400
  - Aggregate_total_cost=148653.9600
  ...
  - GatherMerge_Outer_startup_cost=148533.8400
  - GatherMerge_Outer_total_cost=148653.6300
