...
```
Node 24572 (Aggregate) [PATTERN: 2724c080] - ROOT
  Node 24573 (Gather Merge) [consumed]
    Node 24574 (Aggregate)
      Node 24575 (Sort)
        Node 24576 (Nested Loop) [PATTERN: c53c4396]
          Node 24577 (Seq Scan) [consumed] - LEAF
          Node 24578 (Index Scan) [consumed] - LEAF
```
...
|---------|------|-----------|----------------|
| Aggregate -> Gather Merge (Out | 2724c080 | 24572 | 24573, 24576, 24577, 24578 |
| Nested Loop -> [Seq Scan (Oute | c53c4396 | 24576 | 24572, 24573, 24577, 24578 |
...
|------|------|--------|-----------|-----|--------|
| 24572 | Aggregate | 998.09 | 1027.05 | 2.9% | pattern |
| 24574 | Aggregate | 975.90 | 940.64 | 3.6% | operator |
| 24575 | Sort | 975.29 | 1050.80 | 7.7% | operator |
| 24576 | Nested Loop | 974.36 | 976.02 | 0.2% | pattern |

## Prediction Chain (Bottom-Up)

### Step 1: Node 24576 (Nested Loop) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** c53c4396 (Nested Loop -> [Seq Scan (Outer), Index Scan (Inner)])
- **Consumes:** Nodes 24572, 24573, 24577, 24578
- **Input Features:**
...
- **Output:** st=10.43, rt=976.02

### Step 2: Node 24575 (Sort)
...
### Step 4: Node 24572 (Aggregate) - PATTERN ROOT

- **Source:** pattern
- **Pattern:** 2724c080 (Aggregate -> Gather Merge (Outer))
- **Consumes:** Nodes 24573, 24576, 24577, 24578
- **Input Features:**
  - Aggregate_np=0
  - Aggregate_nt=7
  - Aggregate_nt1=35
  ...
  - Aggregate_total_cost=148637.0800
  ...
  - GatherMerge_Outer_total_cost=148636.7500
