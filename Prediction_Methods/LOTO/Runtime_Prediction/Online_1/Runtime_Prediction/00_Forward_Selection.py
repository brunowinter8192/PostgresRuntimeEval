...
from ffs_config import (
    SEED, MIN_FEATURES, SVM_PARAMS, TARGET_TYPES, TARGET_NAME_MAP,
    OPERATOR_METADATA, OPERATOR_TARGETS, CHILD_FEATURES, CHILD_FEATURES_TIMING, LEAF_OPERATORS
)

CHILD_FEATURES_SET = set(CHILD_FEATURES)
LEAF_OPERATORS_CLEAN = set(op.replace(' ', '_') for op in LEAF_OPERATORS)
...
# Find all operator subdirectories in template directory
def find_operator_dirs(dataset_dir):
    dataset_path = Path(dataset_dir)
    operator_dirs = [d for d in dataset_path.iterdir() if d.is_dir()]
    return sorted(operator_dirs, key=lambda x: x.name)

# Load operator data from CSV
def load_operator_data(operator_dir):
    operator_name = operator_dir.name
    csv_file = operator_dir / f'{operator_name}.csv'
...
def collect_operator_results(dataset_dir, output_dir):
    overview_data = []
    operator_dirs = find_operator_dirs(dataset_dir)

    for operator_dir in operator_dirs:
        operator = operator_dir.name
        for target in TARGET_TYPES:
            result = process_operator_target(operator_dir, operator, target, output_dir)
            if result:
                overview_data.append(result)
    return overview_data

# Process forward feature selection for single operator-target combination
def process_operator_target(operator_dir, operator, target, output_dir):
    df_operator = load_operator_data(operator_dir)

    if len(df_operator) < 10:
        return None

    X, y, template_ids = prepare_features_and_target(df_operator, operator, target)
    ...
    two_step_result = evaluate_operator_two_step(df_operator, operator, target, template_ids, cv, selected_features)
...
def extract_available_features(df):
    non_feature_cols = OPERATOR_METADATA + OPERATOR_TARGETS + ['node_type']
...
def identify_missing_child_features(ffs_features, operator):
    if operator in LEAF_OPERATORS_CLEAN:
        return set()
...
def evaluate_operator_two_step(df, operator, target, template_ids, cv, ffs_features):
    ...
    missing_child_available = [f for f in missing_child if f in available_features]
    final_features = ffs_features + list(missing_child_available)

    mre_final = evaluate_feature_set(X, y, final_features, template_ids, cv)

    return {
        'ffs_features': ffs_features,
        'missing_child_features': list(missing_child_available),
        'final_features': final_features,
        'mre_ffs': mre_ffs,
        'mre_final': mre_final,
        'ffs_feature_count': len(ffs_features),
        'missing_child_count': len(missing_child_available),
...
if __name__ == "__main__":
    ...
    parser.add_argument("dataset_dir", help="Path to template split directory containing training_cleaned.csv")
