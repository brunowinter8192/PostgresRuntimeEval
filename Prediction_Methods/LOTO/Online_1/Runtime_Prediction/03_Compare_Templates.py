...
def create_comparison_plot(comparison_df: pd.DataFrame, output_path: Path) -> None:
    ...
    mean_mre_values = comparison_df['mean_mre_total_pct'].values

    x = np.arange(len(templates))
    width = 0.5

    bars = ax.bar(x, mean_mre_values, width, label='Mean MRE Total',
                  color='steelblue', alpha=0.8, edgecolor='black', linewidth=0.8)

    ax.set_xlabel('Template', fontsize=13, fontweight='bold')
    ax.set_ylabel('Mean Relative Error (%)', fontsize=13, fontweight='bold')
    ax.set_title('Dynamic Prediction: Query-Level MRE by Template (Root Operators)',
